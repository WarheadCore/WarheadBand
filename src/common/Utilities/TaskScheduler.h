...
class TaskScheduler {
  ...
  public:
    ...
    Task(timepoint_t const &end, duration_t const &duration,
         std::optional<group_t> const &group, repeated_t const repeated,
         task_handler_t const &task)
        : _end(end), _duration(duration), _group(group), _repeated(repeated),
          _task(task) {}

    // Minimal Argument construct
    Task(timepoint_t const &end, duration_t const &duration,
         task_handler_t const &task)
        : _end(end), _duration(duration), _group(std::nullopt), _repeated(0),
          _task(task) {}
    ...
    inline bool operator<(Task const &other) const { return _end < other._end; }
    ...
    inline bool IsInGroup(group_t const group) const { return _group == group; }
  };
...
public:
  ...
  template <typename P> TaskScheduler &SetValidator(P &&predicate) {
  ...
  /// Never call this from within a task context! Use TaskContext::Schedule
  /// instead!
  template <class _RepLeft, class _PeriodLeft, class _RepRight,
            class _PeriodRight>
  TaskScheduler &
...
public:
  ...
  TaskContext &Repeat(std::chrono::duration<_Rep, _Period> const &duration) {
    ...
    (*_consumed) = true;
    return Dispatch(
        std::bind(&TaskScheduler::InsertTask, std::placeholders::_1, _task));
  }
  ...
  /// Schedule a callable function that is executed at the next update tick from
  /// within the context. Its safe to modify the TaskScheduler from within the
  /// callable.
  ...
  TaskContext &Schedule(std::chrono::duration<_Rep, _Period> const &time,
    ...
    auto const end = _task->_end;
    return Dispatch(
        [end, time, group, task](TaskScheduler &scheduler) -> TaskScheduler & {
  ...
  Schedule(std::chrono::duration<_RepLeft, _PeriodLeft> const &min,
           ...
           TaskScheduler::group_t const group,
           TaskScheduler::task_handler_t const &task) {
    return Schedule(TaskScheduler::RandomDurationBetween(min, max), group,
                    task);
  }
  ...
  DelayGroup(TaskScheduler::group_t const group,
    ...
    return Dispatch(std::bind(&TaskScheduler::DelayGroup<_Rep, _Period>,
                              std::placeholders::_1, group, duration));
  }
  ...
  TaskContext &
  RescheduleAll(std::chrono::duration<_Rep, _Period> const &duration) {
    return Dispatch(std::bind(&TaskScheduler::RescheduleAll,
                              std::placeholders::_1, duration));
  }
  ...
  RescheduleGroup(TaskScheduler::group_t const group,
                  std::chrono::duration<_Rep, _Period> const &duration) {
    return Dispatch(std::bind(&TaskScheduler::RescheduleGroup<_Rep, _Period>,
                              std::placeholders::_1, group, duration));
  }
