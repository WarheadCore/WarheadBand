...
void AddKey(std::string const &optionName, std::string const &optionKey,
  ...
  if (itr != _configOptions.end()) {
    if (!replace) {
      LOG_ERROR("server", "> Config: Option '%s' is exist! Option key - '%s'",
                optionName.c_str(), itr->second.c_str());
      return;
    }
...
void ParseFile(std::string const &file) {
  ...
  if (in.fail())
    throw ConfigException(Warhead::StringFormat(
        "Config::LoadFile: Failed open file '%s'", file.c_str()));
  ...
  if (!count)
    throw ConfigException(Warhead::StringFormat(
        "Config::LoadFile: Empty file '%s'", file.c_str()));
}
...
template <class T>
T ConfigMgr::GetValueDefault(std::string const &name, T const &def,
                             bool showLogs /*= true*/) const {
  auto const &itr = _configOptions.find(name);
  if (itr == _configOptions.end()) {
    if (showLogs) {
      LOG_ERROR("server",
                "> Config: Missing name %s in config, add \"%s = %s\"",
...
template <class T>
T ConfigMgr::GetOption(std::string const &name, T const &def,
                       bool showLogs /*= true*/) const {
  return GetValueDefault<T>(name, def, showLogs);
}

template <>
bool ConfigMgr::GetOption<bool>(std::string const &name, bool const &def,
                                bool showLogs /*= true*/) const {
  std::string val =
...
void ConfigMgr::Configure(std::string const &initFileName,
                          ...
                          std::string const &modulesConfigList /*= ""*/) {
...
#define TEMPLATE_CONFIG_OPTION(__typename)                                     \
  template __typename ConfigMgr::GetOption<__typename>(                        \
      std::string const &name, __typename const &def,                          \
      bool showLogs /*= true*/) const;
