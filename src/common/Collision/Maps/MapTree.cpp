...
public:
  ...
  void operator()(const Vector3 &point, uint32 entry) {
    ...
    LOG_DEBUG("maps", "AreaInfoCallback: trying to intersect '%s'",
...
public:
  ...
  void operator()(const Vector3 &point, uint32 entry) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS) && defined(VMAP_DEBUG)
    LOG_DEBUG("maps", "LocationInfoCallback: trying to intersect '%s'",
              prims[entry].name.c_str());
#endif
...
bool StaticMapTree::isInLineOfSight(const Vector3 &pos1,
  ...
  // return false if distance is over max float, in case of cheater teleporting
...
bool StaticMapTree::InitMap(const std::string &fname, VMapManager2 *vm) {
  ...
  ModelSpawn spawn;
#ifdef VMAP_DEBUG
  // TC_LOG_DEBUG(LOG_FILTER_MAPS, "StaticMapTree::InitMap() : map isTiled: %u",
  // static_cast<uint32>(iIsTiled));
#endif
  if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn)) {
    WorldModel *model = vm->acquireModelInstance(iBasePath, spawn.name);
    // VMAP_DEBUG_LOG(LOG_FILTER_MAPS, "StaticMapTree::InitMap() : loading %s",
    // spawn.name.c_str());
    if (model) {
      // assume that global model always is the first and only tree value (could
      // be improved...)
      iTreeValues[0] = ModelInstance(spawn, model);
      iLoadedSpawns[0] = 1;
    } else {
      success = false;
      // VMAP_ERROR_LOG(LOG_FILTER_GENERAL, "StaticMapTree::InitMap() : could
      // not acquire WorldModel pointer for '%s'", spawn.name.c_str());
    }
  }
...
bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2 *vm) {
  if (!iIsTiled) {
    // currently, core creates grids for all maps, whether it has terrain tiles
    // or not so we need "fake" tile loads to know when we can unload map
    // geometry
        ...
        if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1) {
          if (!iLoadedSpawns.count(referencedVal)) {
            ...
            if (referencedVal > iNTreeValues) {
              LOG_DEBUG(
                  "maps",
                  "StaticMapTree::LoadMapTile() : invalid tree element (%u/%u)",
            ...
            if (iTreeValues[referencedVal].ID != spawn.ID)
              LOG_DEBUG("maps", "StaticMapTree::LoadMapTile() : trying to load "
                                "wrong spawn in node");
            else if (iTreeValues[referencedVal].name != spawn.name)
              LOG_DEBUG(
                  "maps",
                  "StaticMapTree::LoadMapTile() : name collision on GUID=%u",
                  spawn.ID);
#endif
          }
        } else
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY,
  ...
  if (tile == iLoadedTiles.end()) {
    LOG_ERROR("server",
              "StaticMapTree::UnloadMapTile() : trying to unload non-loaded "
              "tile - Map:%u X:%u Y:%u",
              iMapID, tileX, tileY);
    return;
  }
