...
bool MMapManager::loadMapData(uint32 mapId) {
  ...
  std::string fileName = Warhead::StringFormat(
      MAP_FILE_NAME_FORMAT,
      sConfigMgr->GetOption<std::string>("DataDir", ".").c_str(), mapId);

  FILE *file = fopen(fileName.c_str(), "rb");
  if (!file) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("maps", "MMAP:loadMapData: Error: Could not open mmap file '%s'",
              fileName.c_str());
#endif
    return false;
  }
  ...
  fclose(file);
  if (count != 1) {
    ; // TC_LOG_DEBUG(LOG_FILTER_MAPS, "MMAP:loadMapData: Error: Could not read
      // params from file '%s'", fileName);
    return false;
  }
...
bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y) {
  ...
  std::string fileName = Warhead::StringFormat(
      ...
      sConfigMgr->GetOption<std::string>("DataDir", ".").c_str(), mapId, x, y);
  FILE *file = fopen(fileName.c_str(), "rb");
  if (!file) {
    LOG_DEBUG("maps", "MMAP:loadMap: Could not open mmtile file '%s'",
              fileName.c_str());
    return false;
  }
  ...
  if (fread(&fileHeader, sizeof(MmapTileHeader), 1, file) != 1 ||
    ...
    LOG_ERROR("server", "MMAP:loadMap: Bad header in mmap %03u%02i%02i.mmtile",
              mapId, x, y);
    fclose(file);
    return false;
  }
  ...
  dtStatus stat;
  {
    ...
    stat = mmap->navMesh->addTile(data, fileHeader.size, DT_TILE_FREE_DATA, 0,
                                  &tileRef);
  }
  ...
  if (stat == DT_SUCCESS) {
    ...
    LOG_DEBUG(
        ...
        mapId, x, y, mapId, header->x, header->y);
#endif
    return true;
  } else {
    LOG_ERROR("server",
...
dtNavMeshQuery const *MMapManager::GetNavMeshQuery(uint32 mapId,
  ...
  MMapData *mmap = loadedMMaps[mapId];
  if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end()) {
    // pussywizard: different instances of the same map shouldn't access this
    // simultaneously
    ACORE_WRITE_GUARD(ACE_RW_Thread_Mutex, GetMMapLock(mapId));
