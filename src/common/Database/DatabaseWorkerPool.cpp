...
DatabaseWorkerPool<T>::DatabaseWorkerPool()
    : _mqueue(
          new ACE_Message_Queue<ACE_SYNCH>(2 * 1024 * 1024, 2 * 1024 * 1024)),
      _queue(new ACE_Activation_Queue(_mqueue)) {
  memset(_connectionCount, 0, sizeof(_connectionCount));
  _connections.resize(IDX_SIZE);

  WPFatal(mysql_thread_safe(), "Used MySQL library isn't thread-safe.");
  WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION,
          "WarheadCore does not support MySQL versions below 5.7");
}

template <class T>
bool DatabaseWorkerPool<T>::Open(const std::string &infoString,
                                 uint8 async_threads, uint8 synch_threads) {
  bool res = true;
  _connectionInfo = MySQLConnectionInfo(infoString);

  LOG_INFO("sql.driver",
           "Opening DatabasePool '%s'. Asynchronous connections: %u, "
           "synchronous connections: %u.",
  ...
  for (uint8 i = 0; i < async_threads; ++i) {
    ...
    if (res) // only check mysql version if connection is valid
      WPFatal(mysql_get_server_version(t->GetHandle()) >=
  ...
  if (res)
    LOG_INFO(
        "sql.driver",
        "DatabasePool '%s' opened successfully. %u total connections running.",
        GetDatabaseName(),
        (_connectionCount[IDX_SYNCH] + _connectionCount[IDX_ASYNC]));
  else
    LOG_ERROR("server",
              "DatabasePool %s NOT opened. There were errors opening the MySQL "
              "connections. Check your SQLDriverLogFile "
              "for specific errors.",
              GetDatabaseName());
...
template <class T> void DatabaseWorkerPool<T>::Close() {
  ...
  //! Shuts down delaythreads for this connection pool by underlying
  //! deactivate(). The next dequeue attempt in the worker thread tasks will
  //! result in an error, ultimately ending the worker thread task.
  _queue->queue()->close();
  ...
  //! Deletes the ACE_Activation_Queue object and its underlying
  //! ACE_Message_Queue
  delete _queue;
  delete _mqueue;

  LOG_INFO("sql.driver", "All connections on DatabasePool '%s' closed.",
           GetDatabaseName());
}

template <class T> void DatabaseWorkerPool<T>::Execute(const char *sql) {
  if (!sql)
    return;
...
template <class T> void DatabaseWorkerPool<T>::DirectExecute(const char *sql) {
  if (!sql)
    return;
...
DatabaseWorkerPool<T>::AsyncQuery(PreparedStatement *stmt) {
...
DatabaseWorkerPool<T>::DelayQueryHolder(SQLQueryHolder *holder) {
...
template <class T> SQLTransaction DatabaseWorkerPool<T>::BeginTransaction() {
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction transaction) {
  ...
  case 0:
    ...
    return;
  case 1:
    LOG_INFO("sql.driver", "Warning: Transaction only holds 1 query, consider "
                           "removing Transaction context in code.");
    break;
  default:
    break;
  }
