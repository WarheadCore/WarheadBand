...
bool SQLQueryHolder::SetQuery(size_t index, const char *sql) {
  if (m_queries.size() <= index) {
    LOG_ERROR("server",
              "Query index (%u) out of range (size: %u) for query: %s",
              uint32(index), (uint32)m_queries.size(), sql);
    return false;
  }
...
bool SQLQueryHolder::SetPQuery(size_t index, const char *format, ...) {
  ...
  if (res == -1) {
    LOG_ERROR("server", "SQL Query truncated (and not execute) for format: %s",
              format);
    return false;
  }
...
bool SQLQueryHolder::SetPreparedQuery(size_t index, PreparedStatement *stmt) {
  if (m_queries.size() <= index) {
    LOG_ERROR("server",
              "Query index (%u) out of range (size: %u) for prepared statement",
