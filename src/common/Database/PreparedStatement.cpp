...
void PreparedStatement::BindParameters() {
  ...
  if (i < m_stmt->m_paramCount)
    LOG_INFO("sql.driver",
             "[WARNING]: BindParameters() for statement %u did not bind all "
...
void PreparedStatement::setString(const uint8 index, const std::string &value) {
  ...
  memcpy(statement_data[index].binary.data(), value.c_str(),
         value.length() + 1);
...
static bool ParamenterIndexAssertFail(uint32 stmtIndex, uint8 index,
                                      uint32 paramCount) {
  LOG_ERROR(
      "server",
      "Attempted to bind parameter %u%s on a PreparedStatement %u (statement "
      "has only %u parameters)",
      uint32(index) + 1,
      (index == 1 ? "st" : (index == 2 ? "nd" : (index == 3 ? "rd" : "nd"))),
      stmtIndex, paramCount);
  return false;
}
...
bool MySQLPreparedStatement::CheckValidIndex(uint8 index) {
  ASSERT(index < m_paramCount ||
         ParamenterIndexAssertFail(m_stmt->m_index, index, m_paramCount));

  if (m_paramsSet[index])
    LOG_INFO("sql.driver",
             "[WARNING] Prepared Statement (id: %u) trying to bind value on "
             "already bound index (%u).",
...
std::string
MySQLPreparedStatement::getQueryString(std::string const &sqlPattern) const {
  ...
  for (uint32 i = 0; i < m_stmt->statement_data.size(); i++) {
    ...
    case TYPE_UI8:
      ss << uint16(
          m_stmt->statement_data[i]
              .data.ui8); // stringstream will append a character with that code
    ...
    case TYPE_UI32:
      ...
      break;
    case TYPE_I8:
      ss << int16(m_stmt->statement_data[i]
                      .data.i8); // stringstream will append a character with
                                 // that code instead of numeric representation
      break;
    case TYPE_I16:
...
PreparedStatementTask::~PreparedStatementTask() { delete m_stmt; }
