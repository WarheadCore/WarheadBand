...
PreparedResultSet::PreparedResultSet(MYSQL_STMT *stmt, MYSQL_RES *result,
    ...
    : m_rowCount(rowCount), m_rowPosition(0), m_fieldCount(fieldCount),
      m_rBind(nullptr), m_stmt(stmt), m_res(result), m_isNull(nullptr),
      m_length(nullptr) {
  if (!m_res)
    return;
  ...
  if (mysql_stmt_store_result(m_stmt)) {
    LOG_INFO("sql.driver",
             "%s:mysql_stmt_store_result, cannot bind result from MySQL "
             "server. Error: %s",
  ...
  if (mysql_stmt_bind_result(m_stmt, m_rBind)) {
    LOG_INFO("sql.driver",
             "%s:mysql_stmt_bind_result, cannot bind result from MySQL server. "
             "Error: %s",
             __FUNCTION__, mysql_stmt_error(m_stmt));
    delete[] m_rBind;
    delete[] m_isNull;
    ...
    for (uint64 fIndex = 0; fIndex < m_fieldCount; ++fIndex) {
      if (!*m_rBind[fIndex].is_null)
        ...
        case MYSQL_TYPE_VAR_STRING:
          m_rows[uint32(m_rowPosition)][fIndex].SetByteValue(
              "", m_rBind[fIndex].buffer_length, m_rBind[fIndex].buffer_type,
              *m_rBind[fIndex].length);
          break;
        default:
          m_rows[uint32(m_rowPosition)][fIndex].SetByteValue(
              0, m_rBind[fIndex].buffer_length, m_rBind[fIndex].buffer_type,
              *m_rBind[fIndex].length);
        }
    }
    m_rowPosition++;
...
void PreparedResultSet::CleanUp() {
  /// More of the in our code allocated sources are deallocated by the poorly
  /// documented mysql c api
  if (m_res)
