...
public:
  ...
  void outCharDump(std::string const &str, uint32 accountId, uint64 guid,
                   std::string const &name);

  template <typename Format, typename... Args>
  inline void outMessage(std::string_view filter, LogLevel const level,
                         Format &&fmt, Args &&... args) {
    outMessage(filter, level,
               Warhead::StringFormat(std::forward<Format>(fmt),
  ...
  void outCommand(uint32 account, Format &&fmt, Args &&... args) {
    ...
    outCommand(std::to_string(account),
               Warhead::StringFormat(std::forward<Format>(fmt),
                                     std::forward<Args>(args)...));
  }

private:
  void _Write(std::string_view filter, LogLevel const level,
              std::string const &&message);
  void _writeCommand(std::string &&message,
                     [[maybe_unused]] std::string const &&accountid);

  void outMessage(std::string_view filter, LogLevel const level,
                  std::string &&message);
  ...
  std::string_view GetPositionOptions(std::string_view options, uint8 position,
...
#define LOG_EXCEPTION_FREE(filterType__, level__, ...)                         \
  {                                                                            \
    try {                                                                      \
      ...
      sLog->outMessage("server", LogLevel::LOG_LEVEL_ERROR,                    \
                       "Wrong format occurred (%s) at %s:%u.", e.what(),       \
                       __FILE__, __LINE__);                                    \
    }                                                                          \
  }
