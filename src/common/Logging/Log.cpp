...
void AddFormattingChannel(std::string const &channelName,
                          FormattingChannel *channel) {
  if (GetFormattingChannel(channelName)) {
    SYS_LOG_ERROR("> Formatting channel (%s) is already exist!",
                  channelName.c_str());
    return;
  }
...
void Log::Initialize() { LoadFromConfig(); }
...
void Log::ReadLoggersFromConfig() {
  ...
  if (!keys.size()) {
    SYS_LOG_ERROR(
        "Log::ReadLoggersFromConfig - Not found loggers, change config file!");
    return;
  }
  ...
  if (!Logger::has(LOGGER_ROOT))
    SYS_LOG_ERROR("Log::ReadLoggersFromConfig - Logger '%s' not found!\nPlease "
                  "change or add 'Logger.%s' option in config file!\n",
...
void Log::ReadChannelsFromConfig() {
  ...
  if (keys.empty()) {
    SYS_LOG_ERROR("Log::ReadChannelsFromConfig - Not found channels, change "
                  "config file!");
    return;
  }
...
std::string_view Log::GetPositionOptions(std::string_view options,
                                         uint8 position,
...
bool Log::ShouldLog(std::string_view type, LogLevel level) const {
  // Don't even look for a logger if the LogLevel is higher than the highest log
  // levels across all loggers
...
std::string const Log::GetChannelsFromLogger(std::string const &loggerName) {
  std::string const &loggerOptions = sConfigMgr->GetStringDefault(
      PREFIX_LOGGER + loggerName, "6, Console Server");
...
void Log::CreateLoggerFromConfig(std::string const &configLoggerName) {
  ...
  std::string const &options =
      sConfigMgr->GetStringDefault(configLoggerName, "");
  std::string const &loggerName =
      configLoggerName.substr(PREFIX_LOGGER.length());

  if (loggerName == "system") {
    SYS_LOG_ERROR(
        "Log::CreateLoggerFromConfig: Is forbidden to use logger name %s\n",
        loggerName.c_str());
    return;
  }
  ...
  if (!tokens.size() || tokens.size() < LOGGER_OPTIONS_CHANNELS_NAME + 1) {
    SYS_LOG_ERROR(
        "Log::CreateLoggerFromConfig: Bad config options for Logger (%s)",
        loggerName.c_str());
    return;
  }

  LogLevel level = static_cast<LogLevel>(
      Warhead::StringTo<uint8>(
          GetPositionOptions(options, LOGGER_OPTIONS_LOG_LEVEL))
          .value_or(static_cast<uint8>(LogLevel::LOG_LEVEL_MAX)));
  if (level >= LogLevel::LOG_LEVEL_MAX) {
    SYS_LOG_ERROR("Log::CreateLoggerFromConfig: Wrong Log Level for logger %s",
                  loggerName.c_str());
    return;
  }
  ...
  auto const &channelsName =
      GetPositionOptions(options, LOGGER_OPTIONS_CHANNELS_NAME);

  for (auto const &tokensFmtChannels :
       Warhead::Tokenize(channelsName, ' ', false)) {
    std::string channelName = std::string(tokensFmtChannels);

    auto fmtChannel = GetFormattingChannel(channelName);
    if (!fmtChannel) {
      SYS_LOG_ERROR("Log::CreateLoggerFromConfig - Not found channel (%s)",
                    channelName.c_str());
      return;
    }
...
void Log::CreateChannelsFromConfig(std::string const &logChannelName) {
  ...
  std::string const &channelName =
      logChannelName.substr(PREFIX_CHANNEL.length());

  if (options.empty()) {
    SYS_LOG_ERROR(
        "Log::CreateLoggerFromConfig: Missing config option LogChannel.%s",
        channelName.c_str());
    return;
  }
  ...
  if (tokens.size() < CHANNEL_OPTIONS_PATTERN + 1) {
    SYS_LOG_ERROR("Log::CreateLoggerFromConfig: Wrong config option (< "
                  "CHANNEL_OPTIONS_PATTERN) LogChannel.%s=%s\n",
  ...
  if (tokens.size() > CHANNEL_OPTIONS_MAX) {
    SYS_LOG_ERROR("Log::CreateLoggerFromConfig: Wrong config option (> "
                  "CHANNEL_OPTIONS_MAX) LogChannel.%s=%s\n",
                  channelName.c_str(), options.c_str());
    return;
  }

  auto channelType = Warhead::StringTo<uint8>(
      GetPositionOptions(options, CHANNEL_OPTIONS_TYPE));
  if (!channelType ||
      (channelType &&
       channelType >
           (uint8)FormattingChannelType::FORMATTING_CHANNEL_TYPE_FILE)) {
    SYS_LOG_ERROR(
        "Log::CreateLoggerFromConfig: Wrong channel type for LogChannel.%s\n",
        channelName.c_str());
    return;
  }
  ...
  if (times.empty()) {
    SYS_LOG_ERROR("Log::CreateLoggerFromConfig: Empty times for LogChannel.%s",
                  channelName.c_str());
    return;
  }
  ...
  if (pattern.empty()) {
    SYS_LOG_ERROR(
        "Log::CreateLoggerFromConfig: Empty pattern for LogChannel.%s",
        channelName.c_str());
    return;
  }
  ...
  if (channelType.value() ==
      static_cast<uint8>(
          FormattingChannelType::FORMATTING_CHANNEL_TYPE_CONSOLE)) {
    ...
    if (!colorOptions.empty()) {
      ...
      if (tokensColor.size() == 8) {
        try {
          ...
          _channel->setProperty("informationColor",
                                std::string(tokensColor[5]));
    ...
    ::AddFormattingChannel(channelName,
                           new FormattingChannel(_pattern, _channel));
  } else if (channelType.value() ==
             (uint8)FormattingChannelType::FORMATTING_CHANNEL_TYPE_FILE) {
    ...
    ::AddFormattingChannel(channelName,
                           new FormattingChannel(_pattern, _fileChannel));
  } else
    SYS_LOG_ERROR("Log::CreateLoggerFromConfig: Invalid channel type (%u)",
                  channelType.value());
}

void Log::_Write(std::string_view filter, LogLevel const level,
...
void Log::_writeCommand(std::string &&message,
                        [[maybe_unused]] std::string const &&accountid) {
...
void Log::outMessage(std::string_view filter, LogLevel const level,
                     std::string &&message) {
...
void Log::outCharDump(std::string const &str, uint32 accountId, uint64 guid,
                      std::string const &name) {
  if (str.empty())
    return;

  _Write(LOGGER_PLAYER_DUMP, LogLevel::LOG_LEVEL_INFO,
         Warhead::StringFormat("== START DUMP ==\n(Account: %u. Guid: %u. "
                               "Name: %s)\n%s\n== END DUMP ==\n",
