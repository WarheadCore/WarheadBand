...
public:
  ...
  void outSys(uint8 logLevel, Format &&fmt, Args &&... args) {
    outSys(logLevel, Warhead::StringFormat(std::forward<Format>(fmt),
...
#define SYS_LOG_EXCEPTION_FREE(level__, ...)                                   \
  {                                                                            \
    try {                                                                      \
      ...
      sSysLog->outSys(3, "Wrong format occurred (%s) at %s:%u.", e.what(),     \
                      __FILE__, __LINE__);                                     \
    }                                                                          \
  }
...
#define SYS_LOG_ERROR(...) SYS_LOG_MSG_BODY(3, __VA_ARGS__)

// System Info level 6
#define SYS_LOG_INFO(...) SYS_LOG_MSG_BODY(6, __VA_ARGS__)
