...
void ACSoapRunnable::run() {
  ...
  while (!World::IsStopped()) {
    ...
    LOG_DEBUG("network", "ACSoap: accepted connection from IP=%d.%d.%d.%d",
...
int ns1__executeCommand(soap *soap, char *command, char **result) {
  ...
  if (!accountId) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "ACSoap: Client used invalid username '%s'",
              soap->userid);
#endif
    return 401;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "ACSoap: invalid password for account '%s'",
              soap->userid);
#endif
    return 401;
  }
  ...
  if (acc) {
    LOG_ERROR("server",
              "ACSoap: Error while acquiring lock, acc = %i, errno = %u", acc,
              errno);
  }
