...
RARunnable::~RARunnable() { delete m_Reactor; }
...
void RARunnable::run() {
  ...
  if (acceptor.open(listenAddress, m_Reactor) == -1) {
    LOG_ERROR("server",
              "Trinity RA can not bind to port %d on %s (possible error: port "
              "already in use)",
              raPort, stringIp.c_str());
    return;
  }

  LOG_INFO("server", "Starting Trinity RA on port %d on %s", raPort,
           stringIp.c_str());
