...
int RASocket::open(void *) {
  ...
  if (peer().get_remote_addr(remoteAddress) == -1) {
    LOG_ERROR("server", "RASocket::open: peer().get_remote_addr error is %s",
              ACE_OS::strerror(errno));
    return -1;
  }

  LOG_INFO("network", "Incoming connection from %s",
...
int RASocket::handle_close(ACE_HANDLE /*handle*/, ACE_Reactor_Mask /*mask*/) {
  ...
  peer().close_reader();
  wait();
  // While the above wait() will wait for the ::svc() to finish, it will not
  // wait for the async event RASocket::commandfinished to be completed. Calling
  // destroy() before the latter function ends will lead to using a freed
  // pointer -> crash.
...
int RASocket::process_command(const std::string &command) {
  ...
  CliCommandHolder *cmd = new CliCommandHolder(
      this, command.c_str(), &RASocket::zprint, &RASocket::commandFinished);
...
int RASocket::check_password(const std::string &user, const std::string &pass) {
  ...
  if (PreparedQueryResult result = LoginDatabase.Query(stmt)) {
    Warhead::Crypto::SRP6::Salt salt =
        (*result)[0].GetBinary<Warhead::Crypto::SRP6::SALT_LENGTH>();
    Warhead::Crypto::SRP6::Verifier verifier =
        (*result)[1].GetBinary<Warhead::Crypto::SRP6::VERIFIER_LENGTH>();
...
int RASocket::subnegotiate() {
  ...
  // Wait a maximum of 1000ms for negotiation packet - not all telnet clients
  // may send it
  ...
  if (n >= 1024) {
    LOG_INFO("network",
             "RASocket::subnegotiate: allocated buffer 1024 bytes was too "
             "small for negotiation packet, size: %u",
             uint32(n));
    return -1;
  }
...
void RASocket::commandFinished(void *callbackArg, bool /*success*/) {
  ...
  // the message is 0 size control message to tell that command output is
  // finished hence we don't put timeout, because it shouldn't increase queue
  // size and shouldn't block
  if (socket->putq(mb->duplicate()) == -1) {
    // getting here is bad, command can't be marked as complete
    // LOG_DEBUG("misc", "Failed to enqueue command end message. Error is %s",
    // ACE_OS::strerror(errno));
  }
