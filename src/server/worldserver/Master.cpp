...
public:
  FreezeDetectorRunnable(uint32 freezeDelay)
      : _loops(0), _lastChange(0), _delayTime(freezeDelay) {}
...
int Master::Run() {
  ...
  if (!pidFile.empty()) {
    if (uint32 pid = CreatePIDFile(pidFile))
      ...
      LOG_ERROR("server",
                "Cannot create PID file %s (possible error: permission)\n",
                pidFile.c_str());
      return 1;
    }
  }
  ...
  LoginDatabase.DirectPExecute(
      "UPDATE realmlist SET flag = (flag & ~%u) | %u WHERE id = '%d'",
      REALM_FLAG_OFFLINE, REALM_FLAG_INVALID, realmID);
  ...
  if (affinity > 0) {
    ...
    if (GetProcessAffinityMask(hProcess, &appAff, &sysAff)) {
      ULONG_PTR currentAffinity =
          affinity & appAff; // remove non accessible processors

      if (!currentAffinity)
        LOG_ERROR("server",
                  "Processors marked in UseProcessors bitmask (hex) %x are not "
                  "accessible for the worldserver. Accessible processors "
                  "bitmask (hex): %x",
                  affinity, appAff);
      else if (SetProcessAffinityMask(hProcess, currentAffinity))
        LOG_INFO("server", "Using processors (bitmask, hex): %x",
                 currentAffinity);
      else
        LOG_ERROR("server", "Can't set used processors (hex): %x",
                  currentAffinity);
    }
  }
  ...
  if (affinity > 0) {
    ...
    if (sched_setaffinity(0, sizeof(mask), &mask))
      LOG_ERROR("server", "Can't set used processors (hex): %x, error: %s",
                affinity, strerror(errno));
    else {
      CPU_ZERO(&mask);
      sched_getaffinity(0, sizeof(mask), &mask);
      LOG_INFO("server", "Using processors (bitmask, hex): %lx",
               *(__cpu_mask *)(&mask));
    }
  }
  ...
  if (highPriority) {
    if (setpriority(PRIO_PROCESS, 0, PROCESS_HIGH_PRIORITY))
      LOG_ERROR("server",
                "Can't set worldserver process priority class, error: %s",
                strerror(errno));
    else
      LOG_INFO("server", "worldserver process priority class set to %i",
  ...
  if (uint32 freezeDelay =
          sConfigMgr->GetOption<int32>("MaxCoreStuckTime", 0)) {
    FreezeDetectorRunnable *runnable =
        new FreezeDetectorRunnable(freezeDelay * 1000);
  ...
  LOG_INFO("server", "%s (worldserver-daemon) ready...",
  ...
  if (cliThread) {
    ...
    // this only way to terminate CLI thread exist at Win32 (alt. way exist only
    // in Windows Vista API)
    //_exit(1);
...
bool Master::_StartDB() {
  ...
  async_threads =
      uint8(sConfigMgr->GetOption<int32>("WorldDatabase.WorkerThreads", 1));
  ...
  synch_threads =
      uint8(sConfigMgr->GetOption<int32>("WorldDatabase.SynchThreads", 1));
  ///- Initialise the world database
  if (!WorldDatabase.Open(dbstring, async_threads, synch_threads)) {
    LOG_ERROR("server", "Cannot connect to world database %s",
              dbstring.c_str());
    return false;
  }
  ...
  async_threads =
      uint8(sConfigMgr->GetOption<int32>("CharacterDatabase.WorkerThreads", 1));
  if (async_threads < 1 || async_threads > 32) {
    LOG_ERROR("server",
  ...
  synch_threads =
      uint8(sConfigMgr->GetOption<int32>("CharacterDatabase.SynchThreads", 2));

  ///- Initialise the Character database
  if (!CharacterDatabase.Open(dbstring, async_threads, synch_threads)) {
    LOG_ERROR("server", "Cannot connect to Character database %s",
              dbstring.c_str());
    return false;
  }
  ...
  async_threads =
      uint8(sConfigMgr->GetOption<int32>("LoginDatabase.WorkerThreads", 1));
  if (async_threads < 1 || async_threads > 32) {
    LOG_ERROR("server",
  ...
  synch_threads =
      uint8(sConfigMgr->GetOption<int32>("LoginDatabase.SynchThreads", 1));
  ///- Initialise the login database
  if (!LoginDatabase.Open(dbstring, async_threads, synch_threads)) {
    LOG_ERROR("server", "Cannot connect to login database %s",
              dbstring.c_str());
    return false;
  }
...
void Master::ClearOnlineAccounts() {
  ...
  // pussywizard: tc query would set online=0 even if logged in on another realm
  // >_>
  LoginDatabase.DirectPExecute(
      "UPDATE account SET online = 0 WHERE online = %u", realmID);
