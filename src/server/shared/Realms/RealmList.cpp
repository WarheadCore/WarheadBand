...
void RealmList::UpdateRealm(uint32 id, const std::string &name,
                            ACE_INET_Addr const &address,
                            ACE_INET_Addr const &localAddr,
                            ACE_INET_Addr const &localSubmask, uint8 icon,
                            RealmFlags flag, uint8 timezone,
...
void RealmList::UpdateRealms(bool init) {
  ...
  if (result) {
    do {
      ...
      UpdateRealm(realmId, name, externalAddr, localAddr, submask, icon, flag,
                  timezone,
      ...
      if (init)
        LOG_INFO("server", "Added realm \"%s\" at %s:%u.", name.c_str(),
