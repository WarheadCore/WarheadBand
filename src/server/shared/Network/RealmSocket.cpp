...
RealmSocket::RealmSocket() : input_buffer_(4096), _remoteAddress() {
...
int RealmSocket::open(void *arg) {
  ...
  if (peer().get_remote_addr(addr) == -1) {
    LOG_ERROR("server", "Error %s while opening realm socket!",
...
uint16 RealmSocket::getRemotePort() const { return _remotePort; }

size_t RealmSocket::recv_len() const { return input_buffer_.length(); }
...
void RealmSocket::recv_skip(size_t len) { input_buffer_.rd_ptr(len); }
...
bool RealmSocket::send(const char *buf, size_t len) {
  ...
  ACE_Data_Block db(len, ACE_Message_Block::MB_DATA, (const char *)buf, nullptr,
                    nullptr, ACE_Message_Block::DONT_DELETE, nullptr);
