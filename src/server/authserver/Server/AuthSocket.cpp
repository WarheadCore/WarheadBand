...
// pack(push, N), also any gcc version not support it at some paltform
#if defined(__GNUC__)
#pragma pack(1)
#else
#pragma pack(push, 1)
...
// pack(pop), also any gcc version not support it at some paltform
#if defined(__GNUC__)
#pragma pack()
#else
#pragma pack(pop)
...
void AuthSocket::OnAccept() {
  LOG_INFO("server", "'%s:%d' Accepting connection",
  ...
  while (true) {
    ...
    if (_cmd == AUTH_LOGON_CHALLENGE) {
      ...
      if (challengesInARow == MAX_AUTH_LOGON_CHALLENGES_IN_A_ROW) {
        LOG_INFO("server",
                 "Got %u AUTH_LOGON_CHALLENGE in a row from '%s', possible "
                 "ongoing DoS",
      ...
      if (challengesInARowRealmList == MAX_AUTH_GET_REALM_LIST) {
        LOG_INFO("server",
                 "Got %u REALM_LIST in a row from '%s', possible ongoing DoS",
                 challengesInARowRealmList,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("network", "Got data for cmd %u recv length %u", (uint32)_cmd,
                  (uint32)socket().recv_len());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("network", "Got unknown packet from '%s'",
                socket().getRemoteAddress().c_str());
#endif
      socket().shutdown();
      return;
...
bool AuthSocket::_HandleLogonChallenge() {
  ...
  // pussywizard: logon flood protection:
  {
    ...
    std::map<std::string, uint32>::iterator itr =
        LastLoginAttemptTimeForIP.find(ipaddr);
  ...
  // Read the first 4 bytes (header) to get the length of the remaining of the
  // packet
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "[AuthChallenge] got header, body is %#04x bytes",
            remaining);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "[AuthChallenge] got full packet, %#04x bytes",
            ch->size);
  ...
  PreparedStatement *stmt =
      LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_BANNED);
  stmt->setString(0, ip_address);
  PreparedQueryResult result = LoginDatabase.Query(stmt);
  if (result) {
    pkt << uint8(WOW_FAIL_BANNED);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "'%s:%d' [AuthChallenge] Banned ip tries to login!",
              socket().getRemoteAddress().c_str(), socket().getRemotePort());
#endif
  } else {
    ...
    if (res2) {
      ...
      if (fields[1].GetUInt8() == 1) // if ip is locked
      {
        LOG_DEBUG("network",
                  ...
                  _login.c_str(), fields[2].GetCString());
        LOG_DEBUG("network", "[AuthChallenge] Player address is '%s'",
                  ip_address.c_str());
        ...
        LOG_DEBUG("network", "[AuthChallenge] Account '%s' is not locked to ip",
                  _login.c_str());
      ...
      if (!locked) {
        ...
        LoginDatabase.DirectExecute(
            LoginDatabase.GetPreparedStatement(LOGIN_UPD_EXPIRED_ACCOUNT_BANS));
        ...
        if (banresult) {
          if ((*banresult)[0].GetUInt32() == (*banresult)[1].GetUInt32()) {
            ...
            LOG_DEBUG("network",
                      "'%s:%d' [AuthChallenge] Temporarily banned account %s "
                      "tried to login!",
                      socket().getRemoteAddress().c_str(),
                      socket().getRemotePort(), _login.c_str());
          }
        } else {
          _srp6.emplace(
              _login, fields[5].GetBinary<Warhead::Crypto::SRP6::SALT_LENGTH>(),
              fields[6].GetBinary<Warhead::Crypto::SRP6::VERIFIER_LENGTH>());
          ...
          _accountSecurityLevel = secLevel <= SEC_ADMINISTRATOR
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("network",
                    "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' "
                    "locale (%u)",
                    socket().getRemoteAddress().c_str(),
                    socket().getRemotePort(), _login.c_str(), ch->country[3],
...
bool AuthSocket::_HandleLogonProof() {
  ...
  if (std::optional<SessionKey> K =
          _srp6->VerifyChallengeResponse(lp.A, lp.clientM)) {
    _sessionKey = *K;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "'%s:%d' User '%s' successfully authenticated",
              socket().getRemoteAddress().c_str(), socket().getRemotePort(),
              _login.c_str());
#endif
    ...
    Warhead::Crypto::SHA1::Digest M2 =
        Warhead::Crypto::SRP6::GetSessionVerifier(lp.A, lp.clientM,
                                                  _sessionKey);
    ...
    if (_expversion & POST_BC_EXP_FLAG) // 2.x and 3.x clients
    {
      ...
      proof.error = 0;
      proof.unk1 = 0x00800000; // Accountflags. 0x01 = GM, 0x08 = Trial,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "'%s:%d' [AuthChallenge] account %s tried to login with invalid "
              "password!",
              socket().getRemoteAddress().c_str(), socket().getRemotePort(),
              _login.c_str());
#endif
        ...
        if (failed_logins >= MaxWrongPassCount) {
          ...
          if (WrongPassBanType) {
            ...
            LOG_DEBUG(
                ...
                socket().getRemoteAddress().c_str(), socket().getRemotePort(),
                socket().getRemoteAddress().c_str(), WrongPassBanTime,
...
bool AuthSocket::_HandleReconnectChallenge() {
  ...
  uint16 remaining = ((sAuthLogonChallenge_C *)&buf[0])->size;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "[ReconnectChallenge] got header, body is %#04x bytes",
            remaining);
#endif
  ...
  socket().recv((char *)&buf[4], remaining);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "[ReconnectChallenge] got full packet, %#04x bytes",
            ch->size);
  ...
  PreparedStatement *stmt =
      LoginDatabase.GetPreparedStatement(LOGIN_SEL_SESSIONKEY);
  stmt->setString(0, _login);
  PreparedQueryResult result = LoginDatabase.Query(stmt);
  ...
  if (!result) {
    LOG_ERROR("server",
              "'%s:%d' [ERROR] user %s tried to login and we cannot find his "
              "session key in the database.",
              socket().getRemoteAddress().c_str(), socket().getRemotePort(),
              _login.c_str());
    socket().shutdown();
    return false;
  ...
  _accountSecurityLevel = secLevel <= SEC_ADMINISTRATOR ? AccountTypes(secLevel)
                                                        : SEC_ADMINISTRATOR;
...
ACE_INET_Addr const &
AuthSocket::GetAddressForClient(Realm const &realm,
                                ACE_INET_Addr const &clientAddr) {
  ...
  if (clientAddr.is_loopback()) {
    ...
    // Assume that user connecting from the machine that authserver is located
    // on has all realms available in his local network
...
bool AuthSocket::_HandleRealmList() {
  ...
  if (!result) {
    LOG_ERROR("server",
              "'%s:%d' [ERROR] user %s tried to login but we cannot find him "
              "in the database.",
              socket().getRemoteAddress().c_str(), socket().getRemotePort(),
              _login.c_str());
    socket().shutdown();
  ...
  for (RealmList::RealmMap::const_iterator i = sRealmList->begin();
    ...
    bool okBuild =
        ((_expversion & POST_BC_EXP_FLAG) && realm.gamebuild == _build) ||
        ((_expversion & PRE_BC_EXP_FLAG) &&
         !AuthHelper::IsPreBCAcceptedClientBuild(realm.gamebuild));
    ...
    // We don't need the port number from which client connects with but the
    // realm's port
...
bool AuthSocket::_HandleXferAccept() {
  ...
  // Launch a PatcherRunnable thread, starting at the beginning of the patch
  // file
  socket().recv_skip(1); // clear input buffer
  fseek(pPatch, 0, 0);
...
PatcherRunnable::PatcherRunnable(class AuthSocket *as) { mySocket = as; }
...
Patcher::Patcher() { LoadPatchesInfo(); }
