...
public:
  ...
  class spell_rog_savage_combat_AuraScript : public AuraScript {
    ...
    void CalcPeriodic(AuraEffect const * /*effect*/, bool &isPeriodic,
                      int32 &amplitude) {
      isPeriodic = true;
      amplitude = 1000;
    }
    ...
    void Register() override {
      DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(
          spell_rog_savage_combat_AuraScript::CalcPeriodic, EFFECT_0,
          SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
      OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
          spell_rog_savage_combat_AuraScript::Update, EFFECT_0,
          SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
    }
  };
...
public:
  ...
  class spell_rog_combat_potency_AuraScript : public AuraScript {
    ...
    void Register() override {
      DoCheckProc +=
          AuraCheckProcFn(spell_rog_combat_potency_AuraScript::CheckProc);
    }
  };
...
public:
  ...
  class spell_rog_blade_flurry_AuraScript : public AuraScript {
    ...
    void HandleProc(AuraEffect const *aurEff, ProcEventInfo &eventInfo) {
      ...
      // Unit::CalculateAOEDamageReduction (this=0x0, damage=4118, schoolMask=1,
      // caster=0x7ffdad089000)
      ...
      if (procTarget && eventInfo.GetDamageInfo()) {
        ...
        damage = procTarget->CalculateAOEDamageReduction(
            damage, SPELL_SCHOOL_MASK_NORMAL, GetTarget());
        ...
        GetTarget()->CastCustomSpell(SPELL_ROGUE_BLADE_FLURRY_EXTRA_ATTACK,
                                     values, procTarget, TRIGGERED_FULL_MASK,
                                     nullptr, aurEff);
      }
    }
    ...
    void Register() override {
      DoCheckProc +=
          AuraCheckProcFn(spell_rog_blade_flurry_AuraScript::CheckProc);
      OnEffectProc +=
          AuraEffectProcFn(spell_rog_blade_flurry_AuraScript::HandleProc,
                           EFFECT_0, SPELL_AURA_MOD_MELEE_HASTE);
    }
...
public:
  ...
  class spell_rog_cheat_death_AuraScript : public AuraScript {
    ...
    void Absorb(AuraEffect * /*aurEff*/, DamageInfo &dmgInfo,
      ...
      target->AddSpellCooldown(SPELL_ROGUE_CHEAT_DEATH_COOLDOWN, 0,
    ...
    void Register() override {
      DoEffectCalcAmount += AuraEffectCalcAmountFn(
          spell_rog_cheat_death_AuraScript::CalculateAmount, EFFECT_0,
          SPELL_AURA_SCHOOL_ABSORB);
      OnEffectAbsorb += AuraEffectAbsorbFn(
          spell_rog_cheat_death_AuraScript::Absorb, EFFECT_0);
    }
  };
    ...
    void HandleBeforeHit() {
      if (Unit *target = GetHitUnit())
        ...
        if (AuraEffect const *aurEff = target->GetAuraEffect(
                SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x10000, 0x80000,
                0, GetCaster()->GetGUID()))
      ...
      if (Unit *target = GetHitUnit()) {
        ...
        for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot) {
          ...
          for (uint8 s = 0; s < 3; ++s) {
            ...
            if (!spellInfo) {
              LOG_ERROR("server",
                        "Player::CastItemCombatSpell Enchant %i, player (Name: "
                        "%s, GUID: %u) cast unknown spell %i",
                        enchant->ID, player->GetName().c_str(),
                        player->GetGUIDLow(), enchant->spellid[s]);
              continue;
            }
...
public:
  ...
  class spell_rog_killing_spree_SpellScript : public SpellScript {
    ...
    void Register() override {
      OnCheckCast +=
          SpellCheckCastFn(spell_rog_killing_spree_SpellScript::CheckCast);
      OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
          spell_rog_killing_spree_SpellScript::FilterTargets, EFFECT_1,
  ...
  class spell_rog_killing_spree_AuraScript : public AuraScript {
    ...
    void HandleApply(AuraEffect const * /*aurEff*/,
      ...
      GetTarget()->CastSpell(GetTarget(), SPELL_ROGUE_KILLING_SPREE_DMG_BUFF,
                             true);
    }
    ...
    void HandleEffectPeriodic(AuraEffect const * /*aurEff*/) {
      while (!_targets.empty()) {
        uint64 guid =
            Warhead::Containers::SelectRandomContainerElement(_targets);
        if (Unit *target = ObjectAccessor::GetUnit(*GetTarget(), guid)) {
          ...
          GetTarget()->CastSpell(target, SPELL_ROGUE_KILLING_SPREE_TELEPORT,
                                 true);

          // xinef: ensure fast coordinates switch, dont wait for client to send
          // opcode
          ...
          GetTarget()->CastSpell(
              target, SPELL_ROGUE_KILLING_SPREE_WEAPON_DMG,
    ...
    void Register() override {
      AfterEffectApply += AuraEffectApplyFn(
          spell_rog_killing_spree_AuraScript::HandleApply, EFFECT_0,
          SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
      OnEffectPeriodic += AuraEffectPeriodicFn(
          spell_rog_killing_spree_AuraScript::HandleEffectPeriodic, EFFECT_0,
          SPELL_AURA_PERIODIC_DUMMY);
      AfterEffectRemove += AuraEffectRemoveFn(
          spell_rog_killing_spree_AuraScript::HandleRemove, EFFECT_0,
          SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
...
public:
  ...
  class spell_rog_nerves_of_steel_AuraScript : public AuraScript {
    ...
    void Absorb(AuraEffect * /*aurEff*/, DamageInfo &dmgInfo,
      ...
      if (GetTarget()->GetUInt32Value(UNIT_FIELD_FLAGS) & (UNIT_FLAG_FLEEING) ||
    ...
    void Register() override {
      DoEffectCalcAmount += AuraEffectCalcAmountFn(
          spell_rog_nerves_of_steel_AuraScript::CalculateAmount, EFFECT_0,
          SPELL_AURA_SCHOOL_ABSORB);
      OnEffectAbsorb += AuraEffectAbsorbFn(
          spell_rog_nerves_of_steel_AuraScript::Absorb, EFFECT_0);
    }
  };
  ...
  class spell_rog_preparation_SpellScript : public SpellScript {
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/) {
      ...
      for (PlayerSpellMap::const_iterator itr = spellMap.begin();
           itr != spellMap.end(); ++itr) {
        SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(itr->first);
        if (spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE) {
          if (spellInfo->SpellFamilyFlags[1] &
                  SPELLFAMILYFLAG1_ROGUE_COLDB_SHADOWSTEP || // Cold Blood,
                                                             // Shadowstep
              spellInfo->SpellFamilyFlags[0] &
                  SPELLFAMILYFLAG_ROGUE_VAN_EVAS_SPRINT) // Vanish, Evasion,
                                                         // Sprint
          {
    ...
    void Register() override {
      OnEffectHitTarget +=
          SpellEffectFn(spell_rog_preparation_SpellScript::HandleDummy,
...
public:
  ...
  class spell_rog_prey_on_the_weak_AuraScript : public AuraScript {
    ...
    void HandleEffectPeriodic(AuraEffect const * /*aurEff*/) {
      ...
      if (victim && (target->GetHealthPct() > victim->GetHealthPct())) {
        if (!target->HasAura(SPELL_ROGUE_PREY_ON_THE_WEAK)) {
          ...
          target->CastCustomSpell(target, SPELL_ROGUE_PREY_ON_THE_WEAK, &bp, 0,
                                  0, true);
        }
      } else
    ...
    void Register() override {
      OnEffectPeriodic += AuraEffectPeriodicFn(
          spell_rog_prey_on_the_weak_AuraScript::HandleEffectPeriodic, EFFECT_0,
          SPELL_AURA_PERIODIC_DUMMY);
    }
  };
...
public:
  ...
  class spell_rog_rupture_AuraScript : public AuraScript {
    ...
    void CalculateAmount(AuraEffect const * /*aurEff*/, int32 &amount,
      ...
      if (Unit *caster = GetCaster()) {
        ...
        float const attackpowerPerCombo[6] = {
            0.0f,
            0.015f, // 1 point:  ${($m1 + $b1*1 + 0.015 * $AP) * 4} damage over
                    // 8 secs
            0.024f, // 2 points: ${($m1 + $b1*2 + 0.024 * $AP) * 5} damage over
                    // 10 secs
            0.03f, // 3 points: ${($m1 + $b1*3 + 0.03 * $AP) * 6} damage over 12
                   // secs
            0.03428571f, // 4 points: ${($m1 + $b1*4 + 0.03428571 * $AP) * 7}
                         // damage over 14 secs
            0.0375f // 5 points: ${($m1 + $b1*5 + 0.0375 * $AP) * 8} damage over
                    // 16 secs
        };
    ...
    void Register() override {
      DoEffectCalcAmount +=
          AuraEffectCalcAmountFn(spell_rog_rupture_AuraScript::CalculateAmount,
...
public:
  ...
  class spell_rog_shiv_SpellScript : public SpellScript {
    ...
    void Register() override {
      OnEffectHitTarget +=
          SpellEffectFn(spell_rog_shiv_SpellScript::HandleDummy, EFFECT_0,
                        SPELL_EFFECT_DUMMY);
    }
  };
...
public:
  ...
  class spell_rog_tricks_of_the_trade_AuraScript : public AuraScript {
    ...
    void HandleProc(AuraEffect const * /*aurEff*/,
      ...
      Unit *target = GetTarget();
      target->CastSpell(_redirectTarget,
                        SPELL_ROGUE_TRICKS_OF_THE_TRADE_DMG_BOOST, true);
    ...
    void Register() override {
      AfterEffectRemove += AuraEffectRemoveFn(
          spell_rog_tricks_of_the_trade_AuraScript::OnRemove, EFFECT_1,
          SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
      DoCheckProc +=
          AuraCheckProcFn(spell_rog_tricks_of_the_trade_AuraScript::CheckProc);
      OnEffectProc +=
          AuraEffectProcFn(spell_rog_tricks_of_the_trade_AuraScript::HandleProc,
...
public:
  ...
  class spell_rog_tricks_of_the_trade_proc_AuraScript : public AuraScript {
    ...
    void Register() override {
      AfterEffectRemove += AuraEffectRemoveFn(
          spell_rog_tricks_of_the_trade_proc_AuraScript::HandleRemove, EFFECT_0,
          SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
  };
