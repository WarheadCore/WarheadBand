...
#define TALK_HAMMER_LEARN                                                      \
  "Ah, a seasoned veteran you once were. I know you are capable, you merely "  \
  "need to ask and I shall teach you the way of the hammersmith."
#define TALK_AXE_LEARN                                                         \
  "Ah, a seasoned veteran you once were. I know you are capable, you merely "  \
  "need to ask and I shall teach you the way of the axesmith."
#define TALK_SWORD_LEARN                                                       \
  "Ah, a seasoned veteran you once were. I know you are capable, you merely "  \
  "need to ask and I shall teach you the way of the swordsmith."
...
#define TALK_AXE_UNLEARN                                                       \
  "Forgetting your Axesmithing skill is not something to do lightly. If you "  \
  "choose to abandon it you will forget all recipes that require Axesmithing " \
  "to create!"
    ...
    for (uint8 j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j) {
      ...
      if (item && item->GetTemplate()->RequiredSpell == reqSpell) {
        ...
        LOG_DEBUG("scripts.ai",
                  "TSCR: player attempt to unlearn spell %u, but item %u is "
                  "equipped.",
                  reqSpell, item->GetEntry());
#endif
        return false;
      }
    }
...
void ProfessionUnlearnSpells(Player *player, uint32 type) {
  ...
  case S_UNLEARN_WEAPON:                              // S_UNLEARN_WEAPON
    ...
    player->removeSpell(36128, SPEC_MASK_ALL,
                        false); // Light Emberforged Hammer
  ...
  case S_UNLEARN_ARMOR:                               // S_UNLEARN_ARMOR
    ...
    player->removeSpell(36129, SPEC_MASK_ALL,
                        false); // Heavy Earthforged Breastplate
    ...
    player->removeSpell(36257, SPEC_MASK_ALL,
                        false); // Bulwark of the Ancient Kings
    player->removeSpell(36256, SPEC_MASK_ALL,
                        false); // Embrace of the Twisting Nether
    player->removeSpell(34530, SPEC_MASK_ALL,
                        false); // Twisting Nether Chain Shirt
  ...
  case S_UNLEARN_HAMMER:                              // S_UNLEARN_HAMMER
    ...
    player->removeSpell(36137, SPEC_MASK_ALL,
                        false); // Great Earthforged Hammer
    break;
  case S_UNLEARN_AXE: // S_UNLEARN_AXE
    player->removeSpell(36260, SPEC_MASK_ALL,
                        false); // Wicked Edge of the Planes
  ...
  case S_UNLEARN_DRAGON:                              // S_UNLEARN_DRAGON
    ...
    player->removeSpell(36079, SPEC_MASK_ALL,
                        false); // Golden Dragonstrike Breastplate
    player->removeSpell(35576, SPEC_MASK_ALL, false); // Ebon Netherscale Belt
    player->removeSpell(35577, SPEC_MASK_ALL,
                        false); // Ebon Netherscale Bracers
    player->removeSpell(35575, SPEC_MASK_ALL,
                        false); // Ebon Netherscale Breastplate
    ...
    player->removeSpell(35580, SPEC_MASK_ALL,
                        false); // Netherstrike Breastplate
    break;
  ...
  case S_UNLEARN_TRIBAL:                              // S_UNLEARN_TRIBAL
    ...
    player->removeSpell(36078, SPEC_MASK_ALL,
                        false); // Living Crystal Breastplate
    break;
  ...
  case S_UNLEARN_MOONCLOTH:                           // S_UNLEARN_MOONCLOTH
    ...
    player->removeSpell(26761, SPEC_MASK_ALL,
                        false); // Primal Mooncloth Shoulders
    player->removeSpell(26762, SPEC_MASK_ALL, false); // Primal Mooncloth Robe
    break;
  case S_UNLEARN_SHADOWEAVE: // S_UNLEARN_SHADOWEAVE
    player->removeSpell(26756, SPEC_MASK_ALL,
                        false); // Frozen Shadoweave Shoulders
...
void ProcessCastaction(Player *player, Creature *creature, uint32 spellId,
                       uint32 triggeredSpellId, int32 cost) {
...
void ProcessUnlearnAction(Player *player, Creature *creature, uint32 spellId,
                          uint32 alternativeSpellId, int32 cost) {
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    ...
    if (creature->IsVendor())
      AddGossipItemFor(player, GOSSIP_ICON_VENDOR, GOSSIP_TEXT_BROWSE_GOODS,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);

    if (creature->IsTrainer())
      AddGossipItemFor(player, GOSSIP_ICON_TRAINER, GOSSIP_TEXT_TRAIN,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRAIN);

    if (player->HasSkill(SKILL_ALCHEMY) &&
        player->GetBaseSkillValue(SKILL_ALCHEMY) >= 350 &&
        player->getLevel() > 67) {
        ...
        case N_TRAINER_TRANSMUTE: // Zarevhi
          if (!HasAlchemySpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_TRANSMUTE,
                             GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 1);
          if (player->HasSpell(S_TRANSMUTE))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_TRANSMUTE,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 4);
          break;
        case N_TRAINER_ELIXIR: // Lorokeem
          if (!HasAlchemySpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_ELIXIR,
                             GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 2);
          if (player->HasSpell(S_ELIXIR))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_ELIXIR,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 5);
          break;
        case N_TRAINER_POTION: // Lauranna Thar'well
          if (!HasAlchemySpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_POTION,
                             GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 3);
          if (player->HasSpell(S_POTION))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_POTION,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 6);
          break;
        }
      }
    }
  ...
  void SendActionMenu(Player *player, Creature *creature, uint32 action) {
    ...
    case GOSSIP_ACTION_INFO_DEF + 1:
      ProcessCastaction(player, creature, S_TRANSMUTE, S_LEARN_TRANSMUTE,
                        DoLearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 2:
      ProcessCastaction(player, creature, S_ELIXIR, S_LEARN_ELIXIR,
                        DoLearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 3:
      ProcessCastaction(player, creature, S_POTION, S_LEARN_POTION,
                        DoLearnCost(player));
      break;
    // Unlearn Alchemy
    case GOSSIP_ACTION_INFO_DEF + 4:
      ProcessUnlearnAction(player, creature, S_UNLEARN_TRANSMUTE, 0,
                           DoHighUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 5:
      ProcessUnlearnAction(player, creature, S_UNLEARN_ELIXIR, 0,
                           DoHighUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 6:
      ProcessUnlearnAction(player, creature, S_UNLEARN_POTION, 0,
                           DoHighUnlearnCost(player));
      break;
    }
  }
  ...
  void SendConfirmUnlearn(Player *player, Creature *creature, uint32 action) {
    if (action) {
      switch (creature->GetEntry()) {
      case N_TRAINER_TRANSMUTE: // Zarevhi
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_TRANSMUTE,
                         GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_ALCHEMY_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
                          creature->GetGUID());
        break;
      case N_TRAINER_ELIXIR: // Lorokeem
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_ELIXIR, GOSSIP_SENDER_CHECK,
                         action, BOX_UNLEARN_ALCHEMY_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
                          creature->GetGUID());
        break;
      case N_TRAINER_POTION: // Lauranna Thar'well
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_POTION, GOSSIP_SENDER_CHECK,
                         action, BOX_UNLEARN_ALCHEMY_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
...
public:
      ...
      if (player->GetQuestRewardStatus(5284) ||
        ...
        case N_TRAINER_SMITHOMNI2: // Krathok Moltenfist
          if (!player->HasSpell(S_ARMOR) && !player->HasSpell(S_WEAPON))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_ARMOR_LEARN,
                             GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
          if (!player->HasSpell(S_WEAPON) && !player->HasSpell(S_ARMOR))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_WEAPON_LEARN,
        ...
        case N_TRAINER_WEAPON2: // Borgosh Corebender
          if (player->HasSpell(S_WEAPON))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_WEAPON_UNLEARN,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 3);
          break;
        case N_TRAINER_ARMOR1: // Grumnus Steelshaper
        case N_TRAINER_ARMOR2: // Okothos Ironrager
          if (player->HasSpell(S_ARMOR))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_ARMOR_UNLEARN,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 4);
          break;
        }
      }
    ...
    if (player->HasSpell(S_WEAPON) && player->getLevel() > 49 &&
      ...
      case N_TRAINER_HAMMER: // Lilith the Lithe
        if (!HasWeaponSub(player))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_HAMMER,
                           GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 5);
        if (player->HasSpell(S_HAMMER))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_HAMMER,
                           GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 8);
        break;
      case N_TRAINER_AXE: // Kilram
        if (!HasWeaponSub(player))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_AXE,
                           GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 6);
        if (player->HasSpell(S_AXE))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_AXE,
                           GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 9);
        break;
      case N_TRAINER_SWORD: // Seril Scourgebane
        if (!HasWeaponSub(player))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_SWORD,
                           GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 7);
        if (player->HasSpell(S_SWORD))
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_SWORD,
  ...
  void SendActionMenu(Player *player, Creature *creature, uint32 action) {
    ...
    case GOSSIP_ACTION_INFO_DEF + 3:
      if (HasWeaponSub(player))
        ...
        ProcessUnlearnAction(player, creature, S_UNLEARN_WEAPON, S_REP_ARMOR,
                             DoLowUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 4:
      ProcessUnlearnAction(player, creature, S_UNLEARN_ARMOR, S_REP_WEAPON,
                           DoLowUnlearnCost(player));
      break;
    ...
    case GOSSIP_ACTION_INFO_DEF + 8:
      ProcessUnlearnAction(player, creature, S_UNLEARN_HAMMER, 0,
                           DoMedUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 9:
      ProcessUnlearnAction(player, creature, S_UNLEARN_AXE, 0,
                           DoMedUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 10:
      ProcessUnlearnAction(player, creature, S_UNLEARN_SWORD, 0,
                           DoMedUnlearnCost(player));
      break;
    }
  }
  ...
  void SendConfirmUnlearn(Player *player, Creature *creature, uint32 action) {
    if (action) {
      ...
      case N_TRAINER_ARMOR1:  // Grumnus Steelshaper
      case N_TRAINER_ARMOR2:  // Okothos Ironrager
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SMITH_SPEC,
                         GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_ARMORORWEAPON,
                         DoLowUnlearnCost(player), false);
      ...
      case N_TRAINER_HAMMER:
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_HAMMER, GOSSIP_SENDER_CHECK,
                         action, BOX_UNLEARN_WEAPON_SPEC,
                         DoMedUnlearnCost(player), false);
        // unknown textID (TALK_HAMMER_UNLEARN)
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
                          creature->GetGUID());
        break;
      case N_TRAINER_AXE:
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_AXE, GOSSIP_SENDER_CHECK,
                         action, BOX_UNLEARN_WEAPON_SPEC,
                         DoMedUnlearnCost(player), false);
...
public:
  ...
  bool CanLearn(Player *player, uint32 textId, uint32 altTextId,
                uint32 skillValue, uint32 reqSpellId, uint32 spellId,
                uint32 &npcTextId) {
    bool res = false;
    npcTextId = textId;
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    ...
    if (player->HasSkill(SKILL_ENGINEERING)) {
      ...
      case NPC_ZAP:
        ...
        break;
      case NPC_JHORDY:
        canLearn = CanLearn(player, 7251, 7252, 260, S_GNOMISH, SPELL_TO_GADGET,
                            npcTextId);
        if (canLearn)
          gossipItem = GOSSIP_ITEM_JHORDY;
        break;
      case NPC_KABLAM:
        canLearn = CanLearn(player, 10365, 0, 350, S_GOBLIN, SPELL_TO_AREA52,
                            npcTextId);
        if (canLearn)
          gossipItem = GOSSIP_ITEM_KABLAM;
        break;
      case NPC_SMILES:
        canLearn = CanLearn(player, 10363, 0, 350, S_GNOMISH, SPELL_TO_TOSHLEY,
                            npcTextId);
        if (canLearn)
          gossipItem = GOSSIP_ITEM_KABLAM;
        break;
    ...
    if (canLearn)
      AddGossipItemFor(player, GOSSIP_ICON_CHAT, gossipItem,
                       creature->GetEntry(), GOSSIP_ACTION_INFO_DEF + 1);

    SendGossipMenuFor(player,
                      npcTextId ? npcTextId : player->GetGossipTextId(creature),
                      creature->GetGUID());
    return true;
  }
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    ...
    // pussywizard: NO UNLEARNING! LEATHERWORKING SPECIALTY IS A PERMANENT
    // DECISION AND CANNOT BE CHANGED IN ANY WAY! pussywizard: CAN RE-LEARN ONLY
    // THE ONE FOR WHICH QUEST IS COMPLETED!

    if (player->HasSkill(SKILL_LEATHERWORKING) &&
        player->GetBaseSkillValue(SKILL_LEATHERWORKING) >= 250 &&
        player->getLevel() > 49) {
      switch (creature->GetEntry()) {
      ...
      case N_TRAINER_DRAGON2: // Thorkaf Dragoneye
        if (!HasLeatherSpecialty(player) &&
          ...
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_DRAGON,
                           GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        break;
      ...
      case N_TRAINER_ELEMENTAL2: // Brumn Winterhoof
        if (!HasLeatherSpecialty(player) &&
          ...
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_ELEMENTAL,
                           GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
        break;
      ...
      case N_TRAINER_TRIBAL2: // Se'Jib
        if (!HasLeatherSpecialty(player) &&
          ...
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_TRIBAL,
...
public:
  ...
  inline bool HasTailorSpell(Player *player) {
    return (player->HasSpell(S_MOONCLOTH) || player->HasSpell(S_SHADOWEAVE) ||
      ...
      if (player->GetQuestRewardStatus(10831) ||
        ...
        case N_TRAINER_SPELLFIRE: // Gidge Spellweaver
          if (!HasTailorSpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_SPELLFIRE,
                             GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 1);
          if (player->HasSpell(S_SPELLFIRE))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_SPELLFIRE,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 4);
          break;
        case N_TRAINER_MOONCLOTH: // Nasmara Moonsong
          if (!HasTailorSpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_MOONCLOTH,
                             GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 2);
          if (player->HasSpell(S_MOONCLOTH))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_MOONCLOTH,
                             GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 5);
          break;
        case N_TRAINER_SHADOWEAVE: // Andrion Darkspinner
          if (!HasTailorSpell(player))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_SHADOWEAVE,
                             GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 3);
  ...
  void SendActionMenu(Player *player, Creature *creature, uint32 action) {
    ...
    // Learn Tailor
    case GOSSIP_ACTION_INFO_DEF + 1:
      ProcessCastaction(player, creature, S_SPELLFIRE, S_LEARN_SPELLFIRE,
                        DoLearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 2:
      ProcessCastaction(player, creature, S_MOONCLOTH, S_LEARN_MOONCLOTH,
                        DoLearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 3:
      ProcessCastaction(player, creature, S_SHADOWEAVE, S_LEARN_SHADOWEAVE,
                        DoLearnCost(player));
      break;
    // Unlearn Tailor
    case GOSSIP_ACTION_INFO_DEF + 4:
      ProcessUnlearnAction(player, creature, S_UNLEARN_SPELLFIRE, 0,
                           DoHighUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 5:
      ProcessUnlearnAction(player, creature, S_UNLEARN_MOONCLOTH, 0,
                           DoHighUnlearnCost(player));
      break;
    case GOSSIP_ACTION_INFO_DEF + 6:
      ProcessUnlearnAction(player, creature, S_UNLEARN_SHADOWEAVE, 0,
                           DoHighUnlearnCost(player));
      break;
    }
  }
  ...
  void SendConfirmUnlearn(Player *player, Creature *creature, uint32 action) {
    if (action) {
      switch (creature->GetEntry()) {
      case N_TRAINER_SPELLFIRE: // Gidge Spellweaver
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SPELLFIRE,
                         GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_TAILOR_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
                          creature->GetGUID());
        break;
      case N_TRAINER_MOONCLOTH: // Nasmara Moonsong
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_MOONCLOTH,
                         GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_TAILOR_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
        ...
        break;
      case N_TRAINER_SHADOWEAVE: // Andrion Darkspinner
        AddGossipItemFor(player, 0, GOSSIP_UNLEARN_SHADOWEAVE,
                         GOSSIP_SENDER_CHECK, action, BOX_UNLEARN_TAILOR_SPEC,
                         DoHighUnlearnCost(player), false);
        // unknown textID ()
        SendGossipMenuFor(player, player->GetGossipTextId(creature),
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *gameobject) override {
    ...
    if (player->HasSkill(SKILL_ENGINEERING) &&
      ...
      if (player->GetQuestRewardStatus(3643) ||
        ...
        {
          AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GOBLIN,
                           GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 3,
                           BOX_UNLEARN_ENGIN_SPEC, DoHighUnlearnCost(player),
                           false);
        } else if (player->HasSpell(S_GNOMISH)) // Has Gnomish specialization
        {
          AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GNOMISH,
                           GOSSIP_SENDER_UNLEARN, GOSSIP_ACTION_INFO_DEF + 4,
                           BOX_UNLEARN_ENGIN_SPEC, DoHighUnlearnCost(player),
                           false);
        } else // does not have any specialization
        {
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_GOBLIN,
                           GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 1,
                           BOX_LEARN_ENGIN_SPEC, DoLearnCost(player), false);
          AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_LEARN_GNOMISH,
                           GOSSIP_SENDER_LEARN, GOSSIP_ACTION_INFO_DEF + 2,
                           BOX_LEARN_ENGIN_SPEC, DoLearnCost(player), false);
        }
      }
    ...
    SendGossipMenuFor(player, player->GetGossipTextId(gameobject),
                      gameobject->GetGUID());
    return true;
  }

  void SendActionMenu(Player *player, GameObject * /*gameobject*/,
                      uint32 uiAction) {
    switch (uiAction) {
    // Learn Goblin
    case GOSSIP_ACTION_INFO_DEF + 1:
      ProcessCastaction(player, nullptr, S_GOBLIN, S_LEARN_GOBLIN,
                        DoLearnCost(player));
      break;
    // Learn Gnomish
    case GOSSIP_ACTION_INFO_DEF + 2:
      ProcessCastaction(player, nullptr, S_GNOMISH, S_LEARN_GNOMISH,
                        DoLearnCost(player));
      break;
    // Unlearn Goblin
    case GOSSIP_ACTION_INFO_DEF + 3:
      ProcessUnlearnAction(player, nullptr, S_UNLEARN_GOBLIN, 0,
                           DoHighUnlearnCost(player));
      break;
    // Unlearn Gnomish
    case GOSSIP_ACTION_INFO_DEF + 4:
      ProcessUnlearnAction(player, nullptr, S_UNLEARN_GNOMISH, 0,
                           DoHighUnlearnCost(player));
      break;
    }
  }

  void SendConfirmLearn(Player *player, GameObject *gameobject,
                        uint32 uiAction) {
    switch (uiAction) {
    // Goblin
    case GOSSIP_ACTION_INFO_DEF + 1:
  ...
  void SendConfirmUnlearn(Player *player, GameObject *gameobject,
                          uint32 uiAction) {
    switch (uiAction) {
    // Goblin
    case GOSSIP_ACTION_INFO_DEF + 3:
      AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GOBLIN, GOSSIP_SENDER_CHECK,
                       uiAction, BOX_UNLEARN_ENGIN_SPEC,
                       DoHighUnlearnCost(player), false);
      SendGossipMenuFor(player, player->GetGossipTextId(gameobject),
                        gameobject->GetGUID());
      break;
    // Gnomish
    case GOSSIP_ACTION_INFO_DEF + 4:
      AddGossipItemFor(player, 0, GOSSIP_UNLEARN_GNOMISH, GOSSIP_SENDER_CHECK,
                       uiAction, BOX_UNLEARN_ENGIN_SPEC,
                       DoHighUnlearnCost(player), false);
      SendGossipMenuFor(player, player->GetGossipTextId(gameobject),
                        gameobject->GetGUID());
  ...
  bool OnGossipSelect(Player *player, GameObject *gameobject, uint32 uiSender,
