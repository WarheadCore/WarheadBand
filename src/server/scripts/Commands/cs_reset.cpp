...
public:
  ...
  static bool HandleResetAchievementsCommand(ChatHandler *handler,
                                             char const *args) {
    Player *target;
    uint64 targetGuid;
  ...
  static bool HandleResetStatsOrLevelHelper(Player *player) {
    ...
    if (!classEntry) {
      LOG_ERROR("server", "Class %u not found in DBC (Wrong DBC files?)",
                player->getClass());
      return false;
    }
  ...
  static bool HandleResetSpellsCommand(ChatHandler *handler, char const *args) {
    ...
    if (target) {
      ...
      if (!handler->GetSession() ||
        ...
        handler->PSendSysMessage(LANG_RESET_SPELLS_ONLINE,
  ...
  static bool HandleResetTalentsCommand(ChatHandler *handler,
    ...
    if (!handler->extractPlayerTarget((char *)args, &target, &targetGuid,
      ...
      if (!*args && creature && creature->IsPet()) {
        ...
        if (owner && owner->GetTypeId() == TYPEID_PLAYER &&
            creature->ToPet()->IsPermanentPetFor(owner->ToPlayer())) {
          creature->ToPet()->resetTalents();
          owner->ToPlayer()->SendTalentsInfoData(true);

          ChatHandler(owner->ToPlayer()->GetSession())
              .SendSysMessage(LANG_RESET_PET_TALENTS);
          if (!handler->GetSession() ||
              handler->GetSession()->GetPlayer() != owner->ToPlayer())
            handler->PSendSysMessage(
                LANG_RESET_PET_TALENTS_ONLINE,
                handler->GetNameLink(owner->ToPlayer()).c_str());
        }
        return true;
      }
    ...
    if (target) {
      ...
      if (!handler->GetSession() ||
          handler->GetSession()->GetPlayer() != target)
        handler->PSendSysMessage(LANG_RESET_TALENTS_ONLINE,
                                 handler->GetNameLink(target).c_str());
  ...
  static bool HandleResetAllCommand(ChatHandler *handler, char const *args) {
    ...
    if (caseName == "spells") {
      ...
      atLogin =
          AtLoginFlags(AT_LOGIN_RESET_TALENTS | AT_LOGIN_RESET_PET_TALENTS);
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = plist.begin();
         itr != plist.end(); ++itr)
