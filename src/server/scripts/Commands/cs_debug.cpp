...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> debugSendCommandTable = {
        ...
        {"setphaseshift", SEC_ADMINISTRATOR, false,
         &HandleDebugSendSetPhaseShiftCommand, ""},
        {"spellfail", SEC_ADMINISTRATOR, false,
    ...
    static std::vector<ChatCommand> debugCommandTable = {
        ...
        {"lootrecipient", SEC_ADMINISTRATOR, false,
        ...
        {"setaurastate", SEC_ADMINISTRATOR, false,
        ...
        {"itemexpire", SEC_ADMINISTRATOR, false, &HandleDebugItemExpireCommand,
         ""},
        {"areatriggers", SEC_ADMINISTRATOR, false,
  ...
  static bool HandleDebugPlayCinematicCommand(ChatHandler *handler,
    ...
    if (CinematicSequencesEntry const *cineSeq =
            sCinematicSequencesStore.LookupEntry(id)) {
      ...
      if (itr != sFlyByCameraStore.end()) {
        handler->PSendSysMessage("Waypoints for sequence %u, camera %u", id,
                                 cineSeq->cinematicCamera);
        uint32 count = 1;
        for (FlyByCamera cam : itr->second) {
          handler->PSendSysMessage(
              "%02u - %7ums [%f, %f, %f] Facing %f (%f degrees)", count,
              cam.timeStamp, cam.locations.x, cam.locations.y, cam.locations.z,
              cam.locations.w, cam.locations.w * (180 / M_PI));
          count++;
        }
  ...
  static bool HandleDebugSendOpcodeCommand(ChatHandler *handler,
    ...
    while (!parsedStream.eof()) {
      ...
      if (type == "uint8") {
        ...
        LOG_ERROR("server", "Sending opcode that has unknown type '%s'",
                  type.c_str());
        break;
      }
    }
  ...
  static bool HandleDebugSendChatMsgCommand(ChatHandler *handler,
    ...
    ChatHandler::BuildChatPacket(
        data, ChatMsg(type), LANG_UNIVERSAL, handler->GetSession()->GetPlayer(),
        handler->GetSession()->GetPlayer(), msg, 0, "chan");
    handler->GetSession()->SendPacket(&data);
    return true;
  }
  ...
  static bool HandleDebugGetLootRecipientCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(
        ...
        target->GetName().c_str(), target->GetGUIDLow(),
  ...
  static bool HandleDebugGetItemStateCommand(ChatHandler *handler,
            ...
            for (uint8 j = 0; j < bag->GetBagSize(); ++j)
              if (Item *item2 = bag->GetItemByPos(j))
                if (item2->GetState() == state)
                  handler->PSendSysMessage(
                      "bag: 255 slot: %d guid: %d owner: %d", item2->GetSlot(),
                      item2->GetGUIDLow(), GUID_LOPART(item2->GetOwnerGUID()));
          } else if (item->GetState() == state)
            handler->PSendSysMessage("bag: 255 slot: %d guid: %d owner: %d",
    ...
    if (listQueue) {
      ...
      for (size_t i = 0; i < updateQueue.size(); ++i) {
        ...
        uint8 bagSlot =
            container ? container->GetSlot() : uint8(INVENTORY_SLOT_BAG_0);
        ...
        handler->PSendSysMessage("bag: %d slot: %d guid: %d - state: %s",
                                 bagSlot, item->GetSlot(), item->GetGUIDLow(),
                                 st.c_str());
      }
    ...
    if (checkAll) {
      ...
      for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; ++i) {
        ...
        if (item->GetSlot() != i) {
          handler->PSendSysMessage(
              "Item with slot %d and guid %d has an incorrect slot value: %d",
        ...
        if (item->GetOwnerGUID() != player->GetGUID()) {
          handler->PSendSysMessage(
              "The item with slot %d and itemguid %d does have non-matching "
              "owner guid (%d) and player guid (%d) !",
              item->GetSlot(), item->GetGUIDLow(),
        ...
        if (Bag *container = item->GetContainer()) {
          handler->PSendSysMessage(
              "The item with slot %d and guid %d has a container (slot: %d, "
              "guid: %d) but shouldn't!",
              item->GetSlot(), item->GetGUIDLow(), container->GetSlot(),
        ...
        if (item->IsInUpdateQueue()) {
          ...
          if (qp > updateQueue.size()) {
            handler->PSendSysMessage(
                "The item with slot %d and guid %d has its queuepos (%d) "
          ...
          if (updateQueue[qp] == nullptr) {
            handler->PSendSysMessage(
                "The item with slot %d and guid %d has its queuepos (%d) "
                "pointing to nullptr in the queue!",
                item->GetSlot(), item->GetGUIDLow(), qp);
            error = true;
            continue;
          }
          ...
          if (updateQueue[qp] != item) {
            handler->PSendSysMessage(
                "The item with slot %d and guid %d has a queuepos (%d) that "
                "points to another item in the queue (bag: %d, slot: %d, guid: "
                "%d)",
                item->GetSlot(), item->GetGUIDLow(), qp,
          ...
          handler->PSendSysMessage("The item with slot %d and guid %d is not "
                                   "in queue but should be (state: %d)!",
                                   item->GetSlot(), item->GetGUIDLow(),
                                   item->GetState());
          error = true;
          continue;
        ...
        if (Bag *bag = item->ToBag()) {
          for (uint8 j = 0; j < bag->GetBagSize(); ++j) {
            ...
            if (item2->GetSlot() != j) {
              handler->PSendSysMessage("The item in bag %d and slot %d (guid: "
                                       "%d) has an incorrect slot value: %d",
            ...
            if (item2->GetOwnerGUID() != player->GetGUID()) {
              handler->PSendSysMessage(
                  "The item in bag %d at slot %d and with itemguid %d, the "
                  "owner's guid (%d) and the player's guid (%d) don't match!",
                  bag->GetSlot(), item2->GetSlot(), item2->GetGUIDLow(),
                  GUID_LOPART(item2->GetOwnerGUID()), player->GetGUIDLow());
              error = true;
              continue;
            }
            ...
            if (!container) {
              handler->PSendSysMessage("The item in bag %d at slot %d with "
                                       "guid %d has no container!",
            ...
            if (container != bag) {
              handler->PSendSysMessage(
                  "The item in bag %d at slot %d with guid %d has a different "
                  "container(slot %d guid %d)!",
                  bag->GetSlot(), item2->GetSlot(), item2->GetGUIDLow(),
            ...
            if (item2->IsInUpdateQueue()) {
              ...
              if (qp > updateQueue.size()) {
                handler->PSendSysMessage(
                    "The item in bag %d at slot %d having guid %d has a "
              ...
              if (updateQueue[qp] == nullptr) {
                handler->PSendSysMessage(
                    "The item in bag %d at slot %d having guid %d has a "
                    "queuepos (%d) that points to nullptr in the queue!",
                    bag->GetSlot(), item2->GetSlot(), item2->GetGUIDLow(), qp);
                error = true;
                continue;
              }
              ...
              if (updateQueue[qp] != item2) {
                handler->PSendSysMessage(
                    "The item in bag %d at slot %d having guid %d has a "
                    "queuepos (%d) that points to another item in the queue "
                    "(bag: %d, slot: %d, guid: %d)",
                    bag->GetSlot(), item2->GetSlot(), item2->GetGUIDLow(), qp,
                    updateQueue[qp]->GetBagSlot(), updateQueue[qp]->GetSlot(),
                    updateQueue[qp]->GetGUIDLow());
              ...
              handler->PSendSysMessage(
                  "The item in bag %d at slot %d having guid %d is not in "
                  "queue but should be (state: %d)!",
                  bag->GetSlot(), item2->GetSlot(), item2->GetGUIDLow(),
                  item2->GetState());
              error = true;
              continue;
      ...
      for (size_t i = 0; i < updateQueue.size(); ++i) {
        ...
        if (item->GetOwnerGUID() != player->GetGUID()) {
          handler->PSendSysMessage(
              "queue(" SZFMTD "): For the item with guid %d, the owner's guid "
                              "(%d) and the player's guid (%d) don't match!",
              i, item->GetGUIDLow(), GUID_LOPART(item->GetOwnerGUID()),
              player->GetGUIDLow());
          error = true;
          continue;
        }
        ...
        if (item->GetQueuePos() != i) {
          handler->PSendSysMessage("queue(" SZFMTD
                                   "): For the item with guid %d, the queuepos "
                                   "doesn't match it's position in the queue!",
                                   i, item->GetGUIDLow());
          error = true;
          continue;
        }
        ...
        if (test == nullptr) {
          handler->PSendSysMessage(
              "queue(" SZFMTD "): The bag(%d) and slot(%d) values for the item "
                              "with guid %d are incorrect, the player doesn't "
                              "have any item at that position!",
        ...
        if (test != item) {
          handler->PSendSysMessage(
              "queue(" SZFMTD
              "): The bag(%d) and slot(%d) values for the item with guid %d "
              "are incorrect, an item which guid is %d is there instead!",
              i, item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow(),
              test->GetGUIDLow());
          error = true;
          continue;
  ...
  static bool HandleDebugThreatListCommand(ChatHandler *handler,
    ...
    for (itr = threatList.begin(); itr != threatList.end(); ++itr) {
      ...
      if (!unit) {
        handler->PSendSysMessage("   %u.   No Unit  - threat %f", ++count,
                                 (*itr)->getThreat());
        continue;
      }
      handler->PSendSysMessage("   %u.   %s   (guid %u)  - threat %f", ++count,
                               unit->GetName().c_str(), unit->GetGUIDLow(),
                               (*itr)->getThreat());
    }
    ThreatContainer::StorageType const &threatList2 =
        target->getThreatManager().getOfflineThreatList();
    for (itr = threatList2.begin(); itr != threatList2.end(); ++itr) {
      Unit *unit = (*itr)->getTarget();
      if (!unit) {
        handler->PSendSysMessage("   %u.   [offline] No Unit  - threat %f",
                                 ++count, (*itr)->getThreat());
        continue;
      }
      handler->PSendSysMessage("   %u.   [offline] %s   (guid %u)  - threat %f",
                               ++count, unit->GetName().c_str(),
                               unit->GetGUIDLow(), (*itr)->getThreat());
    }
  ...
  static bool HandleDebugHostileRefListCommand(ChatHandler *handler,
    ...
    while (ref) {
      if (Unit *unit = ref->GetSource()->GetOwner())
        handler->PSendSysMessage("   %u.   %s %s   (guid %u)  - threat %f",
                                 ++count, (ref->isOnline() ? "" : "[offline]"),
                                 unit->GetName().c_str(), unit->GetGUIDLow(),
                                 ref->getThreat());
      else
        handler->PSendSysMessage("   %u.   No Owner  - threat %f", ++count,
                                 ref->getThreat());
      ref = ref->next();
    }
  ...
  static bool HandleDebugEnterVehicleCommand(ChatHandler *handler,
    ...
    if (!entry)
      ...
      Warhead::AllCreaturesOfEntryInRange check(
          handler->GetSession()->GetPlayer(), entry, 20.0f);
      Warhead::CreatureSearcher<Warhead::AllCreaturesOfEntryInRange> searcher(
  ...
  static bool HandleDebugSpawnVehicleCommand(ChatHandler *handler,
    ...
    if (!i)
      return handler->GetSession()->GetPlayer()->SummonCreature(entry, x, y, z,
                                                                o);
    ...
    if (!v->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_VEHICLE), map,
                   handler->GetSession()->GetPlayer()->GetPhaseMask(), entry,
                   id, x, y, z, o)) {
      delete v;
      return false;
    }
  ...
  static bool HandleDebugSendLargePacketCommand(ChatHandler *handler,
                                                char const * /*args*/) {
    const char *stuffingString = "This is a dummy string to push the packet's "
                                 "size beyond 128000 bytes. ";
  ...
  static bool HandleDebugLoSCommand(ChatHandler *handler,
    ...
    if (Unit *unit = handler->getSelectedUnit()) {
      Player *player = handler->GetSession()->GetPlayer();
      handler->PSendSysMessage(
          "Checking LoS %s -> %s:", player->GetName().c_str(),
          unit->GetName().c_str());
      handler->PSendSysMessage(
          "    VMAP LoS: %s",
          player->IsWithinLOSInMap(unit, LINEOFSIGHT_CHECK_VMAP)
              ? "clear"
              : "obstructed");
      handler->PSendSysMessage(
          "    GObj LoS: %s",
          player->IsWithinLOSInMap(unit, LINEOFSIGHT_CHECK_GOBJECT)
              ? "clear"
              : "obstructed");
      handler->PSendSysMessage("%s is %sin line of sight of %s.",
                               unit->GetName().c_str(),
  ...
  static bool HandleDebugSetValueCommand(ChatHandler *handler,
    ...
    if (opcode >= target->GetValuesCount()) {
      handler->PSendSysMessage(LANG_TOO_BIG_INDEX, opcode, GUID_LOPART(guid),
    ...
    if (isInt32) {
      ...
      handler->PSendSysMessage(LANG_SET_UINT_FIELD, GUID_LOPART(guid), opcode,
                               value);
    } else {
      float value = (float)atof(y);
      target->SetFloatValue(opcode, value);
      handler->PSendSysMessage(LANG_SET_FLOAT_FIELD, GUID_LOPART(guid), opcode,
                               value);
    }
  ...
  static bool HandleDebugGetValueCommand(ChatHandler *handler,
    ...
    if (isInt32) {
      ...
      handler->PSendSysMessage(LANG_GET_UINT_FIELD, GUID_LOPART(guid), opcode,
                               value);
    } else {
      float value = target->GetFloatValue(opcode);
      handler->PSendSysMessage(LANG_GET_FLOAT_FIELD, GUID_LOPART(guid), opcode,
                               value);
    }
  ...
  static bool HandleDebugMod32ValueCommand(ChatHandler *handler,
    ...
    int currentValue =
        (int)handler->GetSession()->GetPlayer()->GetUInt32Value(opcode);
  ...
  static bool HandleDebugUpdateCommand(ChatHandler *handler, char const *args) {
    ...
    if (!val) {
      ...
      handler->PSendSysMessage(LANG_UPDATE, unit->GetGUIDLow(), updateIndex,
                               value);
      return true;
    }
    ...
    handler->PSendSysMessage(LANG_UPDATE_CHANGE, unit->GetGUIDLow(),
                             updateIndex, value);
  ...
  static bool HandleDebugUnitStateCommand(ChatHandler *handler,
    ...
    if (!*args) {
      handler->PSendSysMessage(
          "Unit States: %u, React State: %u", target->GetUnitState(),
          target->ToCreature() ? target->ToCreature()->GetReactState() : 10);
    } else {
  ...
  static bool HandleWPGPSCommand(ChatHandler *handler, char const * /*args*/) {
    ...
    LOG_INFO("sql.dev", "(@PATH, XX, %.3f, %.3f, %.5f, 0,0, 0,100, 0),",
