...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {
        ...
        {"flusharenapoints", SEC_ADMINISTRATOR, false,
  ...
  static bool HandleSkirmishCommand(ChatHandler *handler, char const *args) {
    ...
    if (!*args || !tokens.size()) {
      ...
      handler->PSendSysMessage("[arena] can be \"all\" or comma-separated list "
                               "of possible arenas (NA,BE,RL,DS,RV).");
      handler->PSendSysMessage("[XvX] can be 1v1, 2v2, 3v3, 5v5. After [XvX] "
                               "specify enough nicknames for that mode.");
    ...
    for (Tokenizer::const_iterator itr = arenaTokens.begin();
         itr != arenaTokens.end(); ++itr) {
    ...
    BattlegroundTypeId randomizedArenaBgTypeId =
        Warhead::Containers::SelectRandomContainerElement(allowedArenas);
    ...
    case 2:
      ...
      break;
    case 3:
      handler->PSendSysMessage("Player %s is in instance/battleground/arena.",
                               last_name.c_str());
      break;
    case 4:
      ...
      break;
    case 5:
      handler->PSendSysMessage("Player %s is queued for battleground/arena.",
                               last_name.c_str());
      break;
    case 6:
    ...
    case 7:
      ...
      break;
    case 8:
      handler->PSendSysMessage("Group of player %s has invalid member count.",
                               last_name.c_str());
      break;
    case 9:
    ...
    Battleground *bg = sBattlegroundMgr->CreateNewBattleground(
        randomizedArenaBgTypeId, 80, 80, ArenaType(hcnt >= 2 ? hcnt : 2),
        false);
    if (!bg) {
      handler->PSendSysMessage("Couldn't create arena map!");
      handler->SetSentErrorMessage(true);
    ...
    for (uint8 i = 0; i < cnt; ++i) {
      ...
      sBattlegroundMgr->BuildBattlegroundStatusPacket(
          &data, bg, queueSlot, STATUS_IN_PROGRESS, 0, bg->GetStartTime(),
          bg->GetArenaType(), teamId);
      player->GetSession()->SendPacket(&data);

      player->SetBattlegroundId(bg->GetInstanceID(), bgTypeId, queueSlot, true,
                                false, teamId);
      sBattlegroundMgr->SendToBattleground(player, bg->GetInstanceID(),
                                           bgTypeId);
    }
  ...
  static bool HandleDevCommand(ChatHandler *handler, char const *enable) {
    ...
    if (!*enable) {
      handler->GetSession()->SendNotification(
          player->IsDeveloper() ? LANG_DEV_ON : LANG_DEV_OFF);
      return true;
    }
  ...
  static bool HandleGPSCommand(ChatHandler *handler, char const *args) {
    ...
    if (*args) {
      ...
      if (guid)
        object = (WorldObject *)ObjectAccessor::GetObjectByTypeMask(
            *handler->GetSession()->GetPlayer(), guid,
    ...
    CellCoord cellCoord = Warhead::ComputeCellCoord(object->GetPositionX(),
                                                    object->GetPositionY());
    Cell cell(cellCoord);
    ...
    float groundZ =
        map->GetHeight(object->GetPhaseMask(), object->GetPositionX(),
                       object->GetPositionY(), MAX_HEIGHT);
    float floorZ =
        map->GetHeight(object->GetPhaseMask(), object->GetPositionX(),
                       object->GetPositionY(), object->GetPositionZ());

    GridCoord gridCoord = Warhead::ComputeGridCoord(object->GetPositionX(),
                                                    object->GetPositionY());
    ...
    if (haveVMap) {
      if (map->IsOutdoors(object->GetPositionX(), object->GetPositionY(),
                          object->GetPositionZ()))
    ...
    handler->PSendSysMessage(
        LANG_MAP_POSITION, object->GetMapId(),
        ...
        (areaEntry ? areaEntry->area_name[handler->GetSessionDbcLocale()]
                   : "<unknown>"),
        object->GetPhaseMask(), object->GetPositionX(), object->GetPositionY(),
        object->GetPositionZ(), object->GetOrientation(), cell.GridX(),
        cell.GridY(), cell.CellX(), cell.CellY(), object->GetInstanceId(),
    ...
    if (status)
      handler->PSendSysMessage(LANG_LIQUID_STATUS, liquidStatus.level,
                               liquidStatus.depth_level, liquidStatus.entry,
                               liquidStatus.type_flags, status);
    ...
    if (target) {
      ...
      if (map->IsBattlegroundOrArena()) {
        ...
        if (!_player->IsGameMaster()) {
          handler->PSendSysMessage(LANG_CANNOT_GO_TO_BG_GM,
                                   chrNameLink.c_str());
          handler->SetSentErrorMessage(true);
          return false;
        }
        ...
        _player->SetBattlegroundId(
            target->GetBattlegroundId(), target->GetBattlegroundTypeId(),
        ...
        if (_player->GetGroup()) {
          ...
          if (_player->GetGroup() != target->GetGroup()) {
            handler->PSendSysMessage(LANG_CANNOT_GO_TO_INST_PARTY,
                                     chrNameLink.c_str());
            handler->SetSentErrorMessage(true);
            return false;
          }
          ...
          if (!_player->IsGameMaster()) {
            handler->PSendSysMessage(LANG_CANNOT_GO_TO_INST_GM,
                                     chrNameLink.c_str());
            handler->SetSentErrorMessage(true);
            return false;
          }
        ...
        // if the GM is bound to another instance, he will not be bound to
        // another one
        InstancePlayerBind *bind = sInstanceSaveMgr->PlayerGetBoundInstance(
            _player->GetGUIDLow(), target->GetMapId(),
            target->GetDifficulty(map->IsRaid()));
        if (!bind)
          if (InstanceSave *save =
                  sInstanceSaveMgr->GetInstanceSave(target->GetInstanceId()))
            sInstanceSaveMgr->PlayerBindToInstance(_player->GetGUIDLow(), save,
                                                   !save->CanReset(), _player);
      ...
      if (_player->TeleportTo(
              target->GetMapId(), target->GetPositionX(),
              target->GetPositionY(), target->GetPositionZ() + 0.25f,
              _player->GetOrientation(), TELE_TO_GM_MODE, target))
  ...
  static bool HandleSummonCommand(ChatHandler *handler, char const *args) {
    ...
    if (target) {
      ...
      if (map->IsBattlegroundOrArena()) {
        ...
        if (destMap->Instanceable() &&
          ...
          sInstanceSaveMgr->PlayerUnbindInstance(
              target->GetGUIDLow(), map->GetInstanceId(),
              target->GetDungeonDifficulty(), true, target);

        // we are in an instance, and can only summon players in our group with
        // us as leader
        if (!handler->GetSession()->GetPlayer()->GetGroup() ||
            !target->GetGroup() ||
            (target->GetGroup()->GetLeaderGUID() !=
      ...
      handler->GetSession()->GetPlayer()->GetClosePoint(
          x, y, z, target->GetObjectSize());
      target->TeleportTo(handler->GetSession()->GetPlayer()->GetMapId(), x, y,
                         z, target->GetOrientation(), 0,
    ...
    for (GroupReference *itr = group->GetFirstMember(); itr != nullptr;
      ...
      if (handler->needReportToTarget(player))
        ChatHandler(player->GetSession())
            .PSendSysMessage(LANG_SUMMONED_BY, handler->GetNameLink().c_str());
      ...
      player->TeleportTo(handler->GetSession()->GetPlayer()->GetMapId(), x, y,
                         z, player->GetOrientation(), 0,
                         handler->GetSession()->GetPlayer());
    }
  ...
  static bool HandleDieCommand(ChatHandler *handler, char const * /*args*/) {
    ...
    if (target->IsAlive()) {
      if (sWorld->getBoolConfig(CONFIG_DIE_COMMAND_MODE)) {
        ...
        Unit::DealDamage(handler->GetSession()->GetPlayer(), target,
                         target->GetHealth(), nullptr, DIRECT_DAMAGE,
  ...
  static bool HandleReviveCommand(ChatHandler *handler, char const *args) {
    ...
    if (target) {
      target->ResurrectPlayer(
          !AccountMgr::IsPlayerAccount(target->GetSession()->GetSecurity())
              ? 1.0f
              : 0.5f);
  ...
  static bool HandleGUIDCommand(ChatHandler *handler, char const * /*args*/) {
    ...
    handler->PSendSysMessage(LANG_OBJECT_GUID, GUID_LOPART(guid),
                             GUID_HIPART(guid));
    return true;
  }
  ...
  static bool HandleCooldownCommand(ChatHandler *handler, char const *args) {
    ...
    if (!*args) {
      ...
      if (!sSpellMgr->GetSpellInfo(spellIid)) {
        handler->PSendSysMessage(LANG_UNKNOWN_SPELL,
      ...
      handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid,
                               target == handler->GetSession()->GetPlayer()
                                   ? handler->GetAcoreString(LANG_YOU)
                                   : nameLink.c_str());
    }
  ...
  static bool HandleGetDistanceCommand(ChatHandler *handler, char const *args) {
    ...
    if (*args) {
      uint64 guid = handler->extractGuidFromLink((char *)args);
      if (guid)
        obj = (WorldObject *)ObjectAccessor::GetObjectByTypeMask(
            *handler->GetSession()->GetPlayer(), guid,
            TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT);
    ...
    handler->PSendSysMessage(
        LANG_DISTANCE, handler->GetSession()->GetPlayer()->GetDistance(obj),
        handler->GetSession()->GetPlayer()->GetDistance2d(obj),
        handler->GetSession()->GetPlayer()->GetExactDist(obj),
  ...
  static bool HandleRecallCommand(ChatHandler *handler, char const *args) {
    ...
    target->TeleportTo(target->m_recallMap, target->m_recallX,
                       target->m_recallY, target->m_recallZ, target->m_recallO);
    return true;
  }
  ...
  static bool HandleKickPlayerCommand(ChatHandler *handler, char const *args) {
    ...
    if (!handler->extractPlayerTarget((char *)args, &target, nullptr,
                                      &playerName))
      return false;
    ...
    if (sWorld->getBoolConfig(CONFIG_SHOW_KICK_IN_WORLD))
      sWorld->SendWorldText(
          ...
          (handler->GetSession()
               ...
               : "Server"),
          playerName.c_str(), kickReasonStr.c_str());
    else
  ...
  static bool HandleUnstuckCommand(ChatHandler *handler, char const *args) {
    ...
    if (player->IsInFlight() || player->IsInCombat()) {
      ...
      if (Player *caster = handler->GetSession()->GetPlayer())
        Spell::SendCastResult(caster, spellInfo, 0,
  ...
  static bool HandleLinkGraveCommand(ChatHandler *handler, char const *args) {
    ...
    if (!areaEntry || areaEntry->zone != 0) {
      handler->PSendSysMessage(LANG_COMMAND_GRAVEYARDWRONGZONE, graveyardId,
                               zoneId);
      handler->SetSentErrorMessage(true);
      return false;
    }
    ...
    if (sGraveyard->AddGraveyardLink(graveyardId, zoneId, teamId))
      handler->PSendSysMessage(LANG_COMMAND_GRAVEYARDLINKED, graveyardId,
                               zoneId);
    else
      handler->PSendSysMessage(LANG_COMMAND_GRAVEYARDALRLINKED, graveyardId,
                               zoneId);
  ...
  static bool HandleNearGraveCommand(ChatHandler *handler, char const *args) {
    ...
    GraveyardStruct const *graveyard = sGraveyard->GetClosestGraveyard(
        player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
        player->GetMapId(), teamId);
    ...
    if (graveyard) {
      ...
      handler->PSendSysMessage(LANG_COMMAND_GRAVEYARDNEAREST, graveyardId,
                               team_name.c_str(), zone_id);
    } else {
      std::string team_name;
      ...
      // else
      handler->PSendSysMessage(LANG_COMMAND_ZONENOGRAFACTION, zone_id,
                               team_name.c_str());
    }
  ...
  static bool HandleAddItemCommand(ChatHandler *handler, char const *args) {
    ...
    if (args[0] == '[') // [name] manual form
    {
      ...
      if (itemNameStr && itemNameStr[0]) {
        ...
        PreparedStatement *stmt =
            WorldDatabase.GetPreparedStatement(WORLD_SEL_ITEM_TEMPLATE_BY_NAME);
        ...
        return false;
    } else // item_id or [name] Shift-click form
           // |color|Hitem:item_id:0:0:0|h[name]|h|r
    {
    ...
    if (count < 0) {
      if (!playerTarget->HasItemCount(itemId, 0)) {
        ...
        handler->PSendSysMessage(LANG_REMOVEITEM_FAILURE,
        ...
        handler->PSendSysMessage(LANG_REMOVEITEM_ERROR,
                                 handler->GetNameLink(playerTarget).c_str(),
                                 itemId);
      } else {
        // output successful amount of destroyed items
        playerTarget->DestroyItemCount(itemId, -count, true, false);
        handler->PSendSysMessage(LANG_REMOVEITEM, itemId, -count,
                                 handler->GetNameLink(playerTarget).c_str());
      }
    ...
    if (handler->GetSession()->GetSecurity() < SEC_ADMINISTRATOR)
           {
               handler->PSendSysMessage("You may only remove items. Adding items
    is available for higher GMLevel."); return false;
           }
           */
    ...
    {
      handler->PSendSysMessage(LANG_ITEM_CANNOT_CREATE, itemId,
                               noSpaceForCount);
      handler->SetSentErrorMessage(true);
      return false;
    }
  ...
  static bool HandleAddItemSetCommand(ChatHandler *handler, char const *args) {
    ...
    char const *id = handler->extractKeyFromLink(
        (char *)args, "Hitemset"); // number or [name] Shift-click form
    ...
    for (ItemTemplateContainer::const_iterator itr = its->begin();
      ...
      if (itr->second.ItemSet == itemSetId) {
        ...
        ItemPosCountVec dest;
        InventoryResult msg = playerTarget->CanStoreNewItem(
            NULL_BAG, NULL_SLOT, dest, itr->second.ItemId, 1);
        if (msg == EQUIP_ERR_OK) {
          Item *item =
              playerTarget->StoreNewItem(dest, itr->second.ItemId, true);
          ...
          handler->PSendSysMessage(LANG_ITEM_CANNOT_CREATE, itr->second.ItemId,
                                   1);
        }
      }
    }
  ...
  static bool HandleChangeWeather(ChatHandler *handler, char const *args) {
    ...
    uint32 type = uint32(atoi(px)); // 0 to 3, 0: fine, 1: rain, 2: snow, 3:
                                    // sand
  ...
  static bool HandleSetSkillCommand(ChatHandler *handler, char const *args) {
    ...
    // If our target does not yet have the skill they are trying to add to them,
    // the chosen level also becomes the max level of the new profession.
    uint16 max = maxPureSkill
                     ? atol(maxPureSkill)
                     : targetHasSkill ? target->GetPureMaxSkillValue(skill)
                                      : uint16(level);

    if (level <= 0 || level > max || max <= 0)
      return false;

    // If the player has the skill, we get the current skill step. If they don't
    // have the skill, we add the skill to the player's book with step 1 (which
    // is the first rank, in most cases something like 'Apprentice <skill>'.
    target->SetSkill(skill, targetHasSkill ? target->GetSkillStep(skill) : 1,
                     level, max);
    handler->PSendSysMessage(LANG_SET_SKILL, skill,
                             skillLine->name[handler->GetSessionDbcLocale()],
                             handler->GetNameLink(target).c_str(), level, max);
    return true;
  }
  ...
  static bool HandlePInfoCommand(ChatHandler *handler, char const *args) {
    ...
    if (sObjectMgr->GetPlayerNameByGUID(parseGUID, targetName)) {
      target = ObjectAccessor::FindPlayerInOrOutOfWorld(
          MAKE_NEW_GUID(parseGUID, 0, HIGHGUID_PLAYER));
      targetGuid = parseGUID;
    } else if (!handler->extractPlayerTarget((char *)args, &target, &targetGuid,
                                             &targetName))
      return false;
    ...
    if (target) {
      ...
      alive = target->IsAlive() ? handler->GetAcoreString(LANG_YES)
    ...
    if (accInfoResult) {
      ...
      if (!handler->GetSession() ||
          handler->GetSession()->GetSecurity() >= AccountTypes(security)) {
    ...
    if (charXpResult) {
      ...
      if (gguid != 0) {
        PreparedStatement *guildQuery = CharacterDatabase.GetPreparedStatement(
    ...
    handler->PSendSysMessage(
        LANG_PINFO_PLAYER, target ? "" : handler->GetAcoreString(LANG_OFFLINE),
        nameLink.c_str(), GUID_LOPART(targetGuid));
    ...
    if (banTime >= 0)
      handler->PSendSysMessage(
          LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(),
          banTime > 0 ? secsToTimeString(banTime - time(nullptr), true).c_str()
    ...
    if (muteTime > 0)
      handler->PSendSysMessage(
          LANG_PINFO_MUTED, muteReason.c_str(),
          secsToTimeString(muteTime - time(nullptr), true).c_str(),
          muteBy.c_str());

    // Output V. LANG_PINFO_ACC_ACCOUNT
    handler->PSendSysMessage(LANG_PINFO_ACC_ACCOUNT, userName.c_str(), accId,
                             security);

    // Output VI. LANG_PINFO_ACC_LASTLOGIN
    handler->PSendSysMessage(LANG_PINFO_ACC_LASTLOGIN, lastLogin.c_str(),
                             failedLogins);
    ...
    handler->PSendSysMessage(LANG_PINFO_ACC_REGMAILS, regMail.c_str(),
                             eMail.c_str());

    // Output IX. LANG_PINFO_ACC_IP
    handler->PSendSysMessage(LANG_PINFO_ACC_IP, lastIp.c_str(),
                             locked ? handler->GetAcoreString(LANG_YES)
                                    : handler->GetAcoreString(LANG_NO));

    // Output X. LANG_PINFO_CHR_LEVEL
    if (level != sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
      handler->PSendSysMessage(LANG_PINFO_CHR_LEVEL_LOW, level, xp, xptotal,
                               (xptotal - xp));
    else
    ...
    // Output XIII. LANG_PINFO_CHR_PHASE if player is not in GM mode (GM is in
    // every phase)
    if (target && !target->IsGameMaster()) // IsInWorld() returns false on
                                           // loadingscreen, so it's more
      handler->PSendSysMessage(LANG_PINFO_CHR_PHASE,
    ...
    if (areaName)
      handler->PSendSysMessage(LANG_PINFO_CHR_MAP_WITH_AREA, map->name[locale],
                               zoneName, areaName);
    else
    ...
    if (!guildName.empty()) {
      handler->PSendSysMessage(LANG_PINFO_CHR_GUILD, guildName.c_str(),
                               guildId);
    ...
    handler->PSendSysMessage(LANG_PINFO_CHR_PLAYEDTIME,
  ...
  static bool HandleRespawnCommand(ChatHandler *handler,
    ...
    CellCoord p(Warhead::ComputeCellCoord(player->GetPositionX(),
                                          player->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();
    ...
    TypeContainerVisitor<Warhead::WorldObjectWorker<Warhead::RespawnDo>,
                         GridTypeMapContainer>
        obj_worker(worker);
    cell.Visit(p, obj_worker, *player->GetMap(), *player,
               player->GetGridActivationRange());
  ...
  static bool HandleMuteCommand(ChatHandler *handler, char const *args) {
    ...
    uint32 accountId = target
                           ...
                           : sObjectMgr->GetPlayerAccountIdByGUID(targetGuid);
    ...
    if (target) {
      ...
      if (sWorld->getBoolConfig(CONFIG_SHOW_MUTE_IN_WORLD))
        sWorld->SendWorldText(LANG_COMMAND_MUTEMESSAGE_WORLD, muteBy.c_str(),
                              nameLink.c_str(), notSpeakTime,
                              muteReasonStr.c_str());

      ChatHandler(target->GetSession())
          .PSendSysMessage(LANG_YOUR_CHAT_DISABLED, notSpeakTime,
                           muteBy.c_str(), muteReasonStr.c_str());
    } else {
    ...
    if (sWorld->getBoolConfig(CONFIG_SHOW_MUTE_IN_WORLD) && !target)
      ...
      ACORE_READ_GUARD(HashMapHolder<Player>::LockType,
        ...
        if (itr->second->GetSession()->GetSecurity())
          ChatHandler(itr->second->GetSession())
              .PSendSysMessage(
                  ...
                  nameLink.c_str(), notSpeakTime, muteReasonStr.c_str());
    }
  ...
  static bool HandleMuteInfoHelper(uint32 accountId, char const *accountName,
                                   ChatHandler *handler) {
    PreparedStatement *stmt =
    ...
    do {
      ...
      handler->PSendSysMessage(LANG_COMMAND_MUTEHISTORY_OUTPUT, buffer,
                               fields[1].GetUInt32(), fields[2].GetCString(),
                               fields[3].GetCString());
    ...
    for (uint8 i = 0; i < MAX_MOTION_SLOT; ++i) {
      ...
      case CHASE_MOTION_TYPE: {
        ...
        if (unit->GetTypeId() == TYPEID_PLAYER)
          ...
          target = static_cast<ChaseMovementGenerator<Creature> const *>(
                       movementGenerator)
                       ->GetTarget();
      ...
      case FOLLOW_MOTION_TYPE: {
        Unit *target = nullptr;
        if (unit->GetTypeId() == TYPEID_PLAYER)
          target = static_cast<FollowMovementGenerator<Player> const *>(
                       movementGenerator)
                       ->GetTarget();
        else
          target = static_cast<FollowMovementGenerator<Creature> const *>(
                       movementGenerator)
                       ->GetTarget();
      ...
      case EFFECT_MOTION_TYPE:
        ...
        break;
      default:
        handler->PSendSysMessage(LANG_MOVEGENS_UNKNOWN,
  ...
  PointMovementGenerator Without this function 3rd party scripting library will
  get linking errors (unresolved external) when attempting to use the
  ...
  */
  static bool HandleComeToMeCommand(ChatHandler *handler, char const *args) {
    ...
    caster->GetMotionMaster()->MovePoint(0, player->GetPositionX(),
                                         player->GetPositionY(),
                                         player->GetPositionZ());
  ...
  static bool HandleDamageCommand(ChatHandler *handler, char const *args) {
    ...
    if (target->GetTypeId() == TYPEID_UNIT &&
        handler->GetSession()->GetSecurity() == SEC_CONSOLE) // pussywizard
      target->ToCreature()->LowerPlayerDamageReq(target->GetMaxHealth());
    Unit::DealDamage(handler->GetSession()->GetPlayer(), target, damage,
                     nullptr, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr,
                     false, true);
    if (target != handler->GetSession()->GetPlayer())
      handler->GetSession()->GetPlayer()->SendAttackStateUpdate(
          HITINFO_AFFECTS_VICTIM, target, 1, SPELL_SCHOOL_MASK_NORMAL, damage,
          0, 0, VICTIMSTATE_HIT, 0);
    return true;
  }
  ...
  static bool HandleRepairitemsCommand(ChatHandler *handler, char const *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_REPAIR_ITEMS,
                             handler->GetNameLink(target).c_str());
    if (handler->needReportToTarget(target))
      ChatHandler(target->GetSession())
  ...
  static bool HandleSendMailCommand(ChatHandler *handler, char const *args) {
    ...
    MailDraft(subject, text)
        .SendMailTo(trans, MailReceiver(target, GUID_LOPART(targetGuid)),
                    sender);
  ...
  static bool HandleSendItemsCommand(ChatHandler *handler, char const *args) {
    ...
    if (!handler->extractPlayerTarget((char *)args, &receiver, &receiverGuid,
                                      &receiverName))
      return false;
    ...
    while (char *itemStr = strtok(tail, " ")) {
      ...
      if (itemCount < 1 || (item_proto->MaxCount > 0 &&
                            itemCount > uint32(item_proto->MaxCount))) {
        handler->PSendSysMessage(LANG_COMMAND_INVALID_ITEM_COUNT, itemCount,
                                 itemId);
        handler->SetSentErrorMessage(true);
        return false;
      }
    ...
    for (ItemPairs::const_iterator itr = items.begin(); itr != items.end();
      ...
      if (Item *item = Item::CreateItem(
        ...
        item->SaveToDB(trans); // save for prevent lost at next mail load, if
                               // send fail then item will deleted
        draft.AddItem(item);
      }
    }

    draft.SendMailTo(trans, MailReceiver(receiver, GUID_LOPART(receiverGuid)),
                     sender);
    CharacterDatabase.CommitTransaction(trans);
  ...
  static bool HandleSendMoneyCommand(ChatHandler *handler, char const *args) {
    ...
    MailDraft(subject, text)
        .AddMoney(money)
        .SendMailTo(trans, MailReceiver(receiver, GUID_LOPART(receiverGuid)),
                    sender);
  ...
  static bool HandleCreatePetCommand(ChatHandler *handler,
                                     char const * /*args*/) {
    Player *player = handler->GetSession()->GetPlayer();
  ...
  static bool HandlePetLearnCommand(ChatHandler *handler, char const *args) {
    ...
    if (bounds.first != bounds.second || spellDifficultyId) {
      handler->PSendSysMessage("Spell %u cannot be learnt using a command!",
                               spellId);
      handler->SetSentErrorMessage(true);
      return false;
    }
  ...
  static bool HandleFreezeCommand(ChatHandler *handler, char const *args) {
    ...
    if (player && (player != handler->GetSession()->GetPlayer())) {
      ...
      if (SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(9454))
        Aura::TryRefreshStackOrCreate(spellInfo, MAX_EFFECT_MASK, player,
                                      player);
    }
  ...
  static bool HandleGroupJoinCommand(ChatHandler *handler, char const *args) {
    ...
    if (handler->GetPlayerGroupAndGUIDByName(nameplgrStr, playerSource,
                                             groupSource, guidSource, true)) {
      if (groupSource) {
        Group *groupTarget = nullptr;
        Player *playerTarget = nullptr;
        if (handler->GetPlayerGroupAndGUIDByName(
                nameplStr, playerTarget, groupTarget, guidTarget, true)) {
          if (!groupTarget && playerTarget->GetGroup() != groupSource) {
            if (!groupSource->IsFull()) {
              ...
              handler->PSendSysMessage(LANG_GROUP_PLAYER_JOINED,
            ...
            handler->PSendSysMessage(LANG_GROUP_ALREADY_IN_GROUP,
                                     playerTarget->GetName().c_str());
            return true;
          }
        }
  ...
  static bool HandleGroupListCommand(ChatHandler *handler, char const *args) {
    ...
    if (sObjectMgr->GetPlayerNameByGUID(parseGUID, nameTarget)) {
      playerTarget = ObjectAccessor::FindPlayerInOrOutOfWorld(
          MAKE_NEW_GUID(parseGUID, 0, HIGHGUID_PLAYER));
      guidTarget = parseGUID;
    } else if (!handler->extractPlayerTarget((char *)args, &playerTarget,
                                             &guidTarget, &nameTarget))
      return false;
    ...
    if (!groupTarget && guidTarget)
      if (uint32 groupId =
              Player::GetGroupIdFromStorage(GUID_LOPART(guidTarget)))
        groupTarget = sGroupMgr->GetGroupByGUID(groupId);

    if (groupTarget) {
      handler->PSendSysMessage(LANG_GROUP_TYPE,
                               (groupTarget->isRaidGroup() ? "raid" : "party"));
      Group::MemberSlotList const &members = groupTarget->GetMemberSlots();
      for (Group::MemberSlotList::const_iterator itr = members.begin();
           itr != members.end(); ++itr) {
        ...
        handler->PSendSysMessage(LANG_GROUP_PLAYER_NAME_GUID, slot.name.c_str(),
        onlineState, GUID_LOPART(slot.guid), flags.c_str(),
