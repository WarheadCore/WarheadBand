...
public:
  ...
  static bool HandleGameObjectActivateCommand(ChatHandler *handler,
    ...
    if (GameObjectData const *goData = sObjectMgr->GetGOData(guidLow))
      object = handler->GetObjectGlobalyWithGuidOrNearWithDbGuid(guidLow,
                                                                 goData->id);
  ...
  static bool HandleGameObjectAddCommand(ChatHandler *handler,
    ...
    if (objectInfo->displayId &&
      ...
      LOG_ERROR("sql.sql",
                "Gameobject (Entry %u GoType: %u) have invalid displayId (%u), "
                "not spawned.",
    ...
    if (!object->Create(guidLow, objectInfo->entry, map,
                        player->GetPhaseMaskForSpawn(), x, y, z, o, G3D::Quat(),
                        0, GO_STATE_READY)) {
      delete object;
      return false;
    }
    ...
    // GameObject instance. this is required to avoid weird behavior and memory
    // leaks
    delete object;
    ...
    handler->PSendSysMessage(LANG_GAMEOBJECT_ADD, objectId,
                             objectInfo->name.c_str(), guidLow, x, y, z);
    return true;
  }
  ...
  static bool HandleGameObjectTargetCommand(ChatHandler *handler,
    ...
    if (*args) {
      ...
      if (objectId)
        result = WorldDatabase.PQuery(
            ...
            "map, phaseMask, (POW(position_x - '%f', 2) + POW(position_y - "
            "'%f', 2) + POW(position_z - '%f', 2)) AS order_ FROM gameobject "
            "WHERE map = '%i' AND id = '%u' ORDER BY order_ ASC LIMIT 1",
            player->GetPositionX(), player->GetPositionY(),
            player->GetPositionZ(), player->GetMapId(), objectId);
      else {
        std::string name = id;
        WorldDatabase.EscapeString(name);
        result = WorldDatabase.PQuery(
            "SELECT guid, id, position_x, position_y, position_z, orientation, "
            "map, phaseMask, (POW(position_x - %f, 2) + POW(position_y - %f, "
            "2) + POW(position_z - %f, 2)) AS order_ "
            "FROM gameobject, gameobject_template WHERE "
            "gameobject_template.entry = gameobject.id AND map = %i AND "
            "name " _LIKE_ " " _CONCAT3_("'%%'", "'%s'",
                                         "'%%'") " ORDER BY order_ ASC LIMIT 1",
            player->GetPositionX(), player->GetPositionY(),
            player->GetPositionZ(), player->GetMapId(), name.c_str());
      }
    } else {
      ...
      result = WorldDatabase.PQuery(
          ...
          "orientation, map, phaseMask, "
          "(POW(position_x - %f, 2) + POW(position_y - %f, 2) + POW(position_z "
          "- %f, 2)) AS order_ FROM gameobject "
          "LEFT OUTER JOIN game_event_gameobject on gameobject.guid = "
          "game_event_gameobject.guid WHERE map = '%i' %s ORDER BY order_ ASC "
          "LIMIT 10",
    ...
    GameObject *target =
        handler->GetSession()->GetPlayer()->GetMap()->GetGameObject(
            MAKE_NEW_GUID(guidLow, id, HIGHGUID_GAMEOBJECT));

    handler->PSendSysMessage(LANG_GAMEOBJECT_DETAIL, guidLow,
                             objectInfo->name.c_str(), guidLow, id, x, y, z,
                             mapId, o, phase);
  ...
  static bool HandleGameObjectDeleteCommand(ChatHandler *handler,
    ...
    if (GameObjectData const *gameObjectData = sObjectMgr->GetGOData(guidLow))
      object = handler->GetObjectGlobalyWithGuidOrNearWithDbGuid(
          guidLow, gameObjectData->id);
    ...
    if (ownerGuid) {
      Unit *owner = ObjectAccessor::GetUnit(*handler->GetSession()->GetPlayer(),
                                            ownerGuid);
      if (!owner || !IS_PLAYER_GUID(ownerGuid)) {
        handler->PSendSysMessage(LANG_COMMAND_DELOBJREFERCREATURE,
                                 GUID_LOPART(ownerGuid), object->GetGUIDLow());
        handler->SetSentErrorMessage(true);
        return false;
      }
  ...
  static bool HandleGameObjectTurnCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(LANG_COMMAND_TURNOBJMESSAGE, object->GetGUIDLow(),
                             object->GetGOInfo()->name.c_str(),
                             object->GetGUIDLow(), oz, oy, ox);
  ...
  static bool HandleGameObjectMoveCommand(ChatHandler *handler,
    ...
    if (!toX) {
      Player *player = handler->GetSession()->GetPlayer();
      object->GetMap()->GameObjectRelocation(
          object, player->GetPositionX(), player->GetPositionY(),
      ...
      object->GetMap()->GameObjectRelocation(object, x, y, z,
    ...
    handler->PSendSysMessage(LANG_COMMAND_MOVEOBJMESSAGE, object->GetGUIDLow(),
                             object->GetGOInfo()->name.c_str(),
                             object->GetGUIDLow());
    ...
    if (result) {
      do {
        ...
        handler->PSendSysMessage(LANG_GO_LIST_CHAT, guid, entry, guid,
                                 gameObjectInfo->name.c_str(), x, y, z, mapId);
  ...
  static bool HandleGameObjectSetStateCommand(ChatHandler *handler,
    ...
    if (guidLow > 0) {
      ...
      object = handler->GetSession()->GetPlayer()->FindNearestGameObject(
          -guidLow, 30.0f);
    ...
    handler->PSendSysMessage("Set gobject type %d state %d", objectType,
                             objectState);
    return true;
  }
};
