...
public:
  ...
  static bool HandleModifyManaCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_MANA,
                             handler->GetNameLink(target).c_str(), mana, manam);
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_MANA_CHANGED,
                           handler->GetNameLink().c_str(), mana, manam);
  ...
  static bool HandleModifyEnergyCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_ENERGY,
                             handler->GetNameLink(target).c_str(), energy / 10,
                             energym / 10);
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_ENERGY_CHANGED,
                           handler->GetNameLink().c_str(), energy / 10,
                           energym / 10);
    ...
    LOG_DEBUG("server", handler->GetAcoreString(LANG_CURRENT_ENERGY),
  ...
  static bool HandleModifyRageCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_RAGE,
                             handler->GetNameLink(target).c_str(), rage / 10,
                             ragem / 10);
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_RAGE_CHANGED,
                           handler->GetNameLink().c_str(), rage / 10,
                           ragem / 10);
  ...
  static bool HandleModifyRunicPowerCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_RUNIC_POWER,
                             handler->GetNameLink(target).c_str(), rune / 10,
                             runem / 10);
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_RUNIC_POWER_CHANGED,
                           handler->GetNameLink().c_str(), rune / 10,
                           runem / 10);
  ...
  static bool HandleModifyFactionCommand(ChatHandler *handler,
    ...
    if (!pfactionid) {
      ...
      handler->PSendSysMessage(LANG_CURRENT_FACTION, target->GetGUIDLow(),
                               factionid, flag, npcflag, dyflag);
      return true;
    }
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_FACTION, target->GetGUIDLow(),
                             factionid, flag, npcflag, dyflag);
  ...
  static bool HandleModifySpellCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_SPELLFLATID, spellflatid, val,
                             mark, handler->GetNameLink(target).c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_SPELLFLATID_CHANGED,
                           handler->GetNameLink().c_str(), spellflatid, val,
                           mark);
  ...
  // Edit Player Aspeed
  static bool HandleModifyASpeedCommand(ChatHandler *handler,
                                        const char *args) {
    if (!*args)
      return false;
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_ASPEED, ASpeed,
                             targetNameLink.c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_ASPEED_CHANGED,
                           handler->GetNameLink().c_str(), ASpeed);
  ...
  static bool HandleModifySpeedCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_SPEED, Speed,
                             targetNameLink.c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_SPEED_CHANGED,
                           handler->GetNameLink().c_str(), Speed);
  ...
  static bool HandleModifySwimCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_SWIM_SPEED, Swim,
                             targetNameLink.c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_SWIM_SPEED_CHANGED,
                           handler->GetNameLink().c_str(), Swim);
  ...
  static bool HandleModifyBWalkCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_BACK_SPEED, BSpeed,
                             targetNameLink.c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_BACK_SPEED_CHANGED,
                           handler->GetNameLink().c_str(), BSpeed);
  ...
  static bool HandleModifyFlyCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_FLY_SPEED, FSpeed,
                             handler->GetNameLink(target).c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOURS_FLY_SPEED_CHANGED,
                           handler->GetNameLink().c_str(), FSpeed);
  ...
  static bool HandleModifyMountCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_YOU_GIVE_MOUNT,
                             handler->GetNameLink(target).c_str());
    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_MOUNT_GIVED, handler->GetNameLink().c_str());
  ...
  static bool HandleModifyMoneyCommand(ChatHandler *handler, const char *args) {
    ...
    if (moneyToAdd < 0) {
      ...
      if (newmoney <= 0) {
        ...
        handler->PSendSysMessage(LANG_YOU_TAKE_MONEY, abs(moneyToAdd),
                                 handler->GetNameLink(target).c_str());
        if (handler->needReportToTarget(target))
          ChatHandler(target->GetSession())
              .PSendSysMessage(LANG_YOURS_MONEY_TAKEN,
                               handler->GetNameLink().c_str(), abs(moneyToAdd));
        target->SetMoney(newmoney);
      }
    } else {
      handler->PSendSysMessage(LANG_YOU_GIVE_MONEY, moneyToAdd,
                               handler->GetNameLink(target).c_str());
      if (handler->needReportToTarget(target))
        ChatHandler(target->GetSession())
            .PSendSysMessage(LANG_YOURS_MONEY_GIVEN,
                             handler->GetNameLink().c_str(), moneyToAdd);
  ...
  static bool HandleModifyBitCommand(ChatHandler *handler, const char *args) {
    ...
    // check online security
    if (target->GetTypeId() == TYPEID_PLAYER &&
        handler->HasLowerSecurity(target->ToPlayer(), 0))
      return false;
  ...
  // change standstate
  static bool HandleModifyStandStateCommand(ChatHandler *handler,
                                            const char *args) {
    if (!*args)
      return false;
  ...
  static bool HandleModifyGenderCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(LANG_YOU_CHANGE_GENDER,
                             handler->GetNameLink(target).c_str(), gender_full);

    if (handler->needReportToTarget(target))
      (ChatHandler(target->GetSession()))
          .PSendSysMessage(LANG_YOUR_GENDER_CHANGED, gender_full,
                           handler->GetNameLink().c_str());
