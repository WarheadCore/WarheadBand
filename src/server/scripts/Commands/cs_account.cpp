...
public:
  ...
  static bool HandleAccountCreateCommand(ChatHandler *handler,
    ...
    AccountOpResult result = AccountMgr::CreateAccount(std::string(accountName),
                                                       std::string(password));
    switch (result) {
    case AOR_OK:
      ...
      if (handler->GetSession()) {
        LOG_DEBUG(
            "warden",
            "Account: %d (IP: %s) Character:[%s] (GUID: %u) Change Password.",
  ...
  static bool HandleAccountLockCountryCommand(ChatHandler *handler,
    ...
    if (!param.empty()) {
      if (param == "on") {
        PreparedStatement *stmt =
            LoginDatabase.GetPreparedStatement(LOGIN_SEL_LOGON_COUNTRY);
        uint32 ip =
            inet_addr(handler->GetSession()->GetRemoteAddress().c_str());
        ...
        if (result) {
          ...
          stmt =
              LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_LOCK_CONTRY);
  ...
  static bool HandleAccountSetGmLevelCommand(ChatHandler *handler,
    ...
    // If gmRealmID is -1, delete all values for the account id, else, insert
    // values for the specific realmID
    PreparedStatement *stmt;
