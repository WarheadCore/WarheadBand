...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> npcSetCommandTable = {
        {"allowmove", SEC_ADMINISTRATOR, false,
         &HandleNpcSetAllowMovementCommand, ""},
        {"entry", SEC_ADMINISTRATOR, false, &HandleNpcSetEntryCommand, ""},
        {"faction", SEC_ADMINISTRATOR, false, nullptr, "",
         npcFactionCommandTable},
        ...
        {"wanderdistance", SEC_ADMINISTRATOR, false,
    ...
    static std::vector<ChatCommand> npcCommandTable = {
        ...
        {"delete", SEC_ADMINISTRATOR, false, nullptr, "",
         npcDeleteCommandTable},
  ...
  static bool HandleNpcAddCommand(ChatHandler *handler, char const *args) {
    ...
    if (!creature->Create(sObjectMgr->GenerateRecycledLowGuid(HIGHGUID_UNIT),
                          map, chr->GetPhaseMaskForSpawn(), id, 0, x, y, z,
                          o)) {
      delete creature;
      return false;
    }
  ...
  static bool HandleNpcAddVendorItemCommand(ChatHandler *handler,
    ...
    uint32 vendor_entry = addMulti ? handler->GetSession()->GetCurrentVendor()
                                   : vendor->GetEntry();

    if (!sObjectMgr->IsVendorItemValid(vendor_entry, itemId, maxcount, incrtime,
                                       extendedcost,
    ...
    handler->PSendSysMessage(LANG_ITEM_ADDED_TO_LIST, itemId,
                             itemTemplate->Name1.c_str(), maxcount, incrtime,
                             extendedcost);
    return true;
  }
  ...
  static bool HandleNpcAddMoveCommand(ChatHandler *handler, const char *args) {
    ...
    if (lowguid)
        creature =
    ObjectAccessor::GetCreature(*handler->GetSession()->GetPlayer(),
    MAKE_GUID(lowguid, HIGHGUID_UNIT));
    */
    ...
    PreparedStatement *stmt =
        WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_MOVEMENT_TYPE);
    ...
    if (creature && creature->GetWaypointPath()) {
      ...
      if (creature->IsAlive()) // dead creature will reset movement generator at
                               // respawn
      {
  ...
  static bool HandleNpcSetLevelCommand(ChatHandler *handler, const char *args) {
    ...
    if (creature->IsPet()) {
      if (((Pet *)creature)->getPetType() == HUNTER_PET) {
        creature->SetUInt32Value(UNIT_FIELD_PETNEXTLEVELEXP,
  ...
  static bool HandleNpcDeleteVendorItemCommand(ChatHandler *handler,
    ...
    if (!sObjectMgr->RemoveVendorItem(
            addMulti ? handler->GetSession()->GetCurrentVendor()
                     : vendor->GetEntry(),
            itemId)) {
  ...
  static bool HandleNpcSetDataCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage(LANG_NPC_SETDATA, creature->GetGUID(),
                             creature->GetEntry(), creature->GetName().c_str(),
                             data_1, data_2, AIorScript.c_str());
    return true;
  }
  ...
  static bool HandleNpcInfoCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage(LANG_NPCINFO_CHAR, target->GetDBTableGUIDLow(),
                             target->GetGUIDLow(), faction, npcflags, Entry,
                             displayid, nativeid);
    ...
    handler->PSendSysMessage(
        LANG_NPCINFO_FLAGS, target->GetUInt32Value(UNIT_FIELD_FLAGS),
        target->GetUInt32Value(UNIT_FIELD_FLAGS_2),
    ...
    handler->PSendSysMessage(
        LANG_NPCINFO_POSITION, float(target->GetPositionX()),
        float(target->GetPositionY()), float(target->GetPositionZ()));
    handler->PSendSysMessage(LANG_NPCINFO_AIINFO, target->GetAIName().c_str(),
  ...
  static bool HandleNpcNearCommand(ChatHandler *handler, char const *args) {
    ...
    if (result) {
      do {
        ...
        handler->PSendSysMessage(LANG_CREATURE_LIST_CHAT, guid, guid,
                                 creatureTemplate->Name.c_str(), x, y, z,
                                 mapId);
  ...
  static bool HandleNpcSetWanderDistanceCommand(ChatHandler *handler,
    ...
    PreparedStatement *stmt =
        WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_WANDER_DISTANCE);
  ...
  static bool HandleNpcSetSpawnTimeCommand(ChatHandler *handler,
    ...
    PreparedStatement *stmt =
        WorldDatabase.GetPreparedStatement(WORLD_UPD_CREATURE_SPAWN_TIME_SECS);
  ...
  static bool HandleNpcTameCommand(ChatHandler *handler,
    ...
    // set pet to defensive mode by default (some classes can't control
    // controlled pets in fact).
    pet->SetReactState(REACT_DEFENSIVE);

    // calculate proper level
    uint8 level = (creatureTarget->getLevel() < (player->getLevel() - 5))
                      ? (player->getLevel() - 5)
  ...
  static bool HandleNpcAddFormationCommand(ChatHandler *handler,
    ...
    handler->PSendSysMessage("Creature %u added to formation with leader %u",
                             lowguid, leaderGUID);
  ...
  static bool HandleNpcSetLinkCommand(ChatHandler *handler, const char *args) {
    ...
    if (!creature->GetDBTableGUIDLow()) {
      handler->PSendSysMessage("Selected creature %u isn't in creature table",
                               creature->GetGUIDLow());
      handler->SetSentErrorMessage(true);
      return false;
    }

    if (!sObjectMgr->SetCreatureLinkedRespawn(creature->GetDBTableGUIDLow(),
                                              linkguid)) {
      handler->PSendSysMessage("Selected creature can't link with guid '%u'",
                               linkguid);
      handler->SetSentErrorMessage(true);
      return false;
    }
    ...
    handler->PSendSysMessage(
        "LinkGUID '%u' added to creature with DBTableGUID: '%u'", linkguid,
  ...
  static bool HandleNpcAddWeaponCommand(ChatHandler * /*handler*/,
    ...
    Creature* creature =
    ObjectAccessor::GetCreature(*handler->GetSession()->GetPlayer(), guid);
    ...
    if (tmpItem)
    {
        ...
        {
            case 1:
                creature->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY,
    ItemID); added = true; break; case 2:
                creature->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY_01,
    ItemID); added = true; break; case 3:
                creature->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY_02,
    ItemID); added = true; break; default:
        ...
        if (added)
            handler->PSendSysMessage(LANG_ITEM_ADDED_TO_SLOT, ItemID,
    tmpItem->Name1, SlotID);
    }
    else
    {
  ...
  static bool HandleNpcSetSubNameCommand(ChatHandler * /*handler*/,
    ...
    uint32 idname = sObjectMgr->AddCreatureSubName(creature->GetName().c_str(),
    args, creature->GetUInt32Value(UNIT_FIELD_DISPLAYID));
    creature->SetUInt32Value(OBJECT_FIELD_ENTRY, idname);
