  ...
  struct npc_eye_of_acherusAI : public NullCreatureAI {
    ...
    void UpdateAI(uint32 diff) override {
      ...
      case EVENT_LAUNCH: {
        ...
        const Position EYE_DESTINATION_1 = {me->GetPositionX() - 40.0f,
                                            me->GetPositionY(),
  ...
  class spell_q12641_death_comes_from_on_high_summon_ghouls_SpellScript
    ...
    void HandleScriptEffect(SpellEffIndex effIndex) {
      ...
      if (Unit *target = GetHitUnit())
        GetCaster()->CastSpell(target->GetPositionX(), target->GetPositionY(),
                               target->GetPositionZ(), 54522, true);
    }
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    if (player->GetQuestStatus(QUEST_DEATH_CHALLENGE) ==
      ...
      if (!creature->AI()->GetData(player->GetGUIDLow()))
        AddGossipItemFor(player, 9765, 0, GOSSIP_SENDER_MAIN,
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/) {
      if (GetOriginalCaster() && GetHitUnit())
        GetOriginalCaster()->CastSpell(
            GetHitUnit(),
            urand(0, 1) ? GetEffectValue() : SPELL_SUMMON_SCARLET_GHOST, true);
    }

    void Register() override {
      OnEffectHitTarget +=
          SpellEffectFn(spell_q12698_the_gift_that_keeps_on_giving_SpellScript::
                            HandleScriptEffect,
                        EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
  };
...
public:
  ...
  struct npc_dkc1_gothikAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) override {
      ...
      if (!who->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC) &&
          who->GetEntry() == NPC_GHOUL && me->IsWithinDistInMap(who, 10.0f))
        ...
        if (summonAttackers >= 15000) {
          for (uint8 i = 0; i < 15; ++i)
            if (Creature *summon = me->SummonCreature(
                    ...
                    2192.56f + irand(-10, 10), -6147.90f + irand(-10, 10), 5.2f,
                    4.7f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 45000)) {
...
uint32 acherus_soul_prison[12] = {191577, 191580, 191581, 191582,
                                  191583, 191584, 191585, 191586,
                                  191587, 191588, 191589, 191590};
      ...
      case PHASE_CHAINED:
        if (!anchorGUID) {
          ...
          for (uint8 i = 0; i < 12; ++i) {
            if (GameObject *temp_prison =
                    me->FindNearestGameObject(acherus_soul_prison[i], 100)) {
        ...
        if (wait_timer) {
          if (wait_timer > diff)
            ...
            me->GetMotionMaster()->MovePoint(1, anchorX, anchorY,
                                             me->GetPositionZ());
            // LOG_DEBUG("scripts.ai", "npc_unworthy_initiateAI: move to %f %f
            // %f", anchorX, anchorY, me->GetPositionZ());
...
public:
  ...
  struct npc_unworthy_initiate_anchorAI : public PassiveAI {
    npc_unworthy_initiate_anchorAI(Creature *creature)
        : PassiveAI(creature), prisonerGUID(0) {}
    ...
    uint64 GetGUID(int32 /*id*/) const override { return prisonerGUID; }
  };
};
...
public:
  ...
  struct npc_scarlet_miner_cartAI : public PassiveAI {
    npc_scarlet_miner_cartAI(Creature *creature)
        : PassiveAI(creature), minerGUID(0) {
    ...
    void SetGUID(uint64 guid, int32 /*id*/) override { minerGUID = guid; }

    void DoAction(int32 /*param*/) override {
      if (Creature *miner = ObjectAccessor::GetCreature(*me, minerGUID)) {
        me->SetWalk(false);

        // Not 100% correct, but movement is smooth. Sometimes miner walks
        // faster than normal, this speed is fast enough to keep up at those
        // times.
    ...
    void WaypointReached(uint32 waypointId) override {
      ...
      case 1:
        if (Unit *car = ObjectAccessor::GetCreature(*me, carGUID)) {
          ...
          car->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE |
                                                    UNIT_FLAG_IMMUNE_TO_PC |
                                                    UNIT_FLAG_IMMUNE_TO_NPC);
          car->setFaction(35);
        }
      ...
      case 23:
        if (Creature *car = ObjectAccessor::GetCreature(*me, carGUID)) {
          car->SetPosition(car->GetPositionX(), car->GetPositionY(),
                           me->GetPositionZ() + 1, car->GetOrientation());
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject * /*go*/) override {
    if (player->GetQuestStatus(12701) == QUEST_STATUS_INCOMPLETE) {
      ...
      if (Creature *miner = player->SummonCreature(
              28841, 2383.869629f, -5900.312500f, 107.996086f,
              player->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 1)) {
        ...
        if (Creature *car = player->GetVehicleCreatureBase()) {
          if (car->GetEntry() == 28817) {
            ...
            CAST_AI(npc_scarlet_miner::npc_scarlet_minerAI, miner->AI())
