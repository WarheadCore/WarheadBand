        ...
        if (Player *player = caster->ToPlayer()) {
          if (player->GetQuestStatus(QUEST_HOW_TO_WIN_FRIENDS) ==
            ...
            sCreatureTextMgr->SendChat(
                me, SAY_PERSUADE_RAND, nullptr, CHAT_MSG_ADDON, LANG_ADDON,
                TEXT_RANGE_NORMAL, 0, TEAM_NEUTRAL, false, player);
            Talk(SAY_CRUSADER);
          }
        }
      }
    ...
    void UpdateAI(uint32 diff) override {
      if (speechCounter) {
        if (speechTimer <= diff) {
          ...
          case 5:
            sCreatureTextMgr->SendChat(
                me, SAY_PERSUADED5, nullptr, CHAT_MSG_ADDON, LANG_ADDON,
                TEXT_RANGE_NORMAL, 0, TEAM_NEUTRAL, false, player);
            speechTimer = 8000;
            break;
  ...
  struct npc_koltira_deathweaverAI : public npc_escortAI {
    ...
    void EnterEvadeMode() override {
      ...
      if (HasEscortState(STATE_ESCORT_ESCORTING)) {
        ...
        LOG_DEBUG("scripts.ai", "TSCR: EscortAI has left combat and is now "
                                "returning to last point");
#endif
      } else {
    ...
    void SummonAcolyte(uint32 uiAmount) {
      for (uint32 i = 0; i < uiAmount; ++i)
        me->SummonCreature(NPC_CRIMSON_ACOLYTE, 1642.329f, -6045.818f, 127.583f,
                           0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
    }
    ...
    void UpdateAI(uint32 uiDiff) override {
      ...
      if (HasEscortState(STATE_ESCORT_PAUSED)) {
        if (m_uiWave_Timer <= uiDiff) {
          ...
          case 3:
            Talk(SAY_BREAKOUT6);
            me->SummonCreature(NPC_HIGH_INQUISITOR_VALROTH, 1642.329f,
                               -6045.818f, 127.583f, 0.0f,
      ...
      if (uiStage && !me->IsInCombat()) {
        if (uiStage_timer <= diff) {
          ...
          case 1:
            me->SetWalk(true);
            if (GameObject *tree =
                    me->FindNearestGameObject(GO_INCONSPICUOUS_TREE, 40.0f)) {
              Talk(SAY_TREE1);
              float x, y, z;
        ...
        if (ExecuteSpeech_Timer <= diff) {
          ...
          case RACE_HUMAN:
            ...
            case 7:
              ...
              break;
            case 8:
              if (Creature *Plaguefist =
                      GetClosestCreatureWithEntry(me, NPC_PLAGUEFIST, 85.0f))
  ...
  class spell_q12779_an_end_to_all_things_SpellScript : public SpellScript {
    ...
    void Register() override {
      OnEffectHitTarget += SpellEffectFn(
          spell_q12779_an_end_to_all_things_SpellScript::HandleScriptEffect,
