  ...
  struct instance_blackrock_depths_InstanceMapScript : public InstanceScript {
    ...
    void OnCreatureCreate(Creature *creature) override {
      ...
      case NPC_MAGMUS:
        ...
        if (!creature->IsAlive())
          HandleGameObject(GetData64(DATA_THRONE_DOOR),
    ...
    void SetData64(uint32 type, uint64 data) override {
      ...
      LOG_DEBUG("scripts.ai",
                "TSCR: Instance Blackrock Depths: SetData64 update (Type: %u "
                "Data " UI64FMTD ")",
                type, data);
#endif
      ...
      case DATA_OPEN_COFFER_DOORS:
        ...
        if (OpenedCoofers == 12) {
          ...
          if (TempSummon *summon =
                  instance->SummonCreature(NPC_WATCHMAN_DOOMGRIP, pos))
    ...
    void TombOfSevenEvent() {
      if (GhostKillCount < 7 && TombBossGUIDs[TombEventCounter]) {
        if (Creature *boss =
                instance->GetCreature(TombBossGUIDs[TombEventCounter])) {
    ...
    void TombOfSevenReset() {
      ...
      HandleGameObject(GoTombEnterGUID,
                       true); // event reseted, open entrance door
      for (uint8 i = 0; i < 7; ++i) {
    ...
    void TombOfSevenStart() {
      ...
      HandleGameObject(GoTombEnterGUID,
                       false); // event started, close entrance door
