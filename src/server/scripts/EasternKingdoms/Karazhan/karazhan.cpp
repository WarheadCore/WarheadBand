...
#define SAY_RAJ_INTRO1                                                         \
  "The romantic plays are really tough, but you'll do better this time. You "  \
  "have TALENT. Ready?"
...
public:
  ...
  struct npc_barnesAI : public npc_escortAI {
    ...
    void WaypointReached(uint32 waypointId) override {
      ...
      case 4:
        ...
        if (Creature *spotlight =
                me->SummonCreature(NPC_SPOTLIGHT, me->GetPositionX(),
                                   me->GetPositionY(), me->GetPositionZ(), 0.0f,
    ...
    void PrepareEncounter() {
      ...
      LOG_DEBUG("scripts.ai",
                "TSCR: Barnes Opera Event - Introduction complete - preparing "
                "encounter %d",
                m_uiEventId);
#endif
      uint8 index = 0;
      ...
      for (; index < count; ++index) {
        ...
        if (Creature *creature = me->SummonCreature(
                entry, PosX, SPAWN_Y, SPAWN_Z, SPAWN_O,
        ...
        if (TalkTimer <= diff) {
          if (TalkCount > 3) {
            if (Creature *pSpotlight =
                    ObjectAccessor::GetCreature(*me, m_uiSpotlightGUID))
  ...
  bool OnGossipSelect(Player *player, Creature *creature, uint32 /*sender*/,
    ...
    case GOSSIP_ACTION_INFO_DEF + 1:
      AddGossipItemFor(player, GOSSIP_ICON_CHAT, OZ_GOSSIP2, GOSSIP_SENDER_MAIN,
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    if (InstanceScript *instance = creature->GetInstanceScript()) {
      ...
      if (instance->GetBossState(DATA_MOROES) == DONE &&
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, OZ_GOSSIP1,
                         GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

        if (player->IsGameMaster()) {
          AddGossipItemFor(player, GOSSIP_ICON_DOT, OZ_GM_GOSSIP1,
                           GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
          AddGossipItemFor(player, GOSSIP_ICON_DOT, OZ_GM_GOSSIP2,
                           GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
          AddGossipItemFor(player, GOSSIP_ICON_DOT, OZ_GM_GOSSIP3,
                           GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5);
        }

        if (npc_barnesAI *pBarnesAI =
...
#define SAY_DIALOG_MEDIVH_1                                                    \
  "You've got my attention, dragon. You'll find I'm not as easily scared as "  \
  "the villagers below."
#define SAY_DIALOG_ARCANAGOS_2                                                 \
  "Your dabbling in the arcane has gone too far, Medivh. You've attracted "    \
  "the attention of powers beyond your understanding. You must leave "         \
  "Karazhan at once!"
#define SAY_DIALOG_MEDIVH_3                                                    \
  "You dare challenge me at my own dwelling? Your arrogance is astounding, "   \
  "even for a dragon!"
#define SAY_DIALOG_ARCANAGOS_4                                                 \
  "A dark power seeks to use you, Medivh! If you stay, dire days will "        \
  "follow. You must hurry, we don't have much time!"
#define SAY_DIALOG_MEDIVH_5                                                    \
  "I do not know what you speak of, dragon... but I will not be bullied by "   \
  "this display of insolence. I'll leave Karazhan when it suits me!"
#define SAY_DIALOG_ARCANAGOS_6                                                 \
  "You leave me no alternative. I will stop you by force if you won't listen " \
  "to reason!"
#define EMOTE_DIALOG_MEDIVH_7                                                  \
  "begins to cast a spell of great power, weaving his own essence into the "   \
  "magic."
  ...
  struct npc_image_of_medivhAI : public ScriptedAI {
    ...
    void Reset() override {
      ...
      if (instance && instance->GetData64(DATA_IMAGE_OF_MEDIVH) == 0) {
        Creature *Arcanagos = me->SummonCreature(
            NPC_ARCANAGOS, ArcanagosPos[0], ArcanagosPos[1], ArcanagosPos[2], 0,
    ...
    uint32 NextStep(uint32 Step) {
      ...
      case 14:
        if (me->GetMap()->IsDungeon()) {
          ...
          for (InstanceMap::PlayerList::const_iterator i = PlayerList.begin();
               i != PlayerList.end(); ++i) {
