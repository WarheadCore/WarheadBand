  ...
  struct instance_zulaman_InstanceMapScript : public InstanceScript {
    ...
    void SummonHostage(uint8 num) {
      ...
      if (Player *i_pl = i->GetSource()) {
        if (Unit *Hostage = i_pl->SummonCreature(
                HostageInfo[num].npc, HostageInfo[num].x, HostageInfo[num].y,
                HostageInfo[num].z, HostageInfo[num].o, TEMPSUMMON_DEAD_DESPAWN,
                0)) {
    ...
    void Load(const char *load) override {
      ...
      if (dataHead == 'S') {
        ...
        LOG_ERROR("server", "Zul'aman: corrupted save data.");
    }
