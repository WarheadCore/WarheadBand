...
SDName: Netherstorm
SD%Complete: 80
SDComment: Quest support: 10337, 10438, 10652 (special flight paths), 10198,
10191 SDCategory: Netherstorm EndScriptData */
...
enum RoleplayActions {
  ...
  EVENT_KAYLAAN_WALK_TO_ADYEN =
      23, // Adyen talks and 3s later he triggers next event
...
enum Ishanah { ISHANAH_PATH_ID = 500050 };

// Ishanah script part is in zone shattrath city . cpp

const Position AdyenSpawnPosition{4804.839355f, 3773.218750f, 210.530884f,
                                  5.517495f};
const Position OrelisSpawnPosition{4805.345215f, 3774.829346f, 210.535095f,
                                   5.517495f};
const Position KarjaSpawnPosition{4803.249512f, 3772.649170f, 210.535095f,
                                  5.517495f};
const Position KaylaanSpawnPosition{4955.089355f, 3916.570557f, 209.577209f,
                                    4.603052f};
const Position IshanahSpawnPosition{4926.066895f, 3825.549072f, 211.494125f,
                                    0.510522f};

class deathblow_to_the_legion_trigger : public CreatureScript {
public:
  deathblow_to_the_legion_trigger()
      : CreatureScript("deathblow_to_the_legion_trigger") {}
  ...
  struct deathblow_to_the_legion_triggerAI : public ScriptedAI {
    deathblow_to_the_legion_triggerAI(Creature *creature)
        : ScriptedAI(creature), _summons(me) {}
    ...
    void MoveInLineOfSight(Unit *who) override {
      if (who->GetTypeId() == TYPEID_PLAYER && who->IsAlive()) {
        if (who->ToPlayer()->GetQuestStatus(DEATHBLOW_TO_THE_LEGION) ==
                QUEST_STATUS_INCOMPLETE &&
            me->FindNearestCreature(SOCRETHAR, 500.0f, true)) {
          if (!_summons.HasEntry(ADYEN_THE_LIGHTBRINGER)) {
            _summons.RemoveNotExisting();
            me->SummonCreature(ADYEN_THE_LIGHTBRINGER, AdyenSpawnPosition,
                               TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300000);
            if (!_summons.HasEntry(EXARCH_ORELIS))
              me->SummonCreature(EXARCH_ORELIS, OrelisSpawnPosition,
                                 TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                 300000);
            if (!_summons.HasEntry(ANCHORITE_KARJA))
              me->SummonCreature(ANCHORITE_KARJA, KarjaSpawnPosition,
                                 TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                 300000);
          }
        }
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    if (player->GetQuestStatus(DEATHBLOW_TO_THE_LEGION) ==
        QUEST_STATUS_INCOMPLETE)
  ...
  bool OnGossipSelect(Player *player, Creature *creature, uint32 /*sender*/,
    ...
    if (action == GOSSIP_ACTION_INFO_DEF + 1) {
      ...
      if (Creature *orelis =
              creature->FindNearestCreature(EXARCH_ORELIS, 15.0f, true))
        orelis->AI()->DoAction(EVENT_ORELIS_WALK);
      if (Creature *karja =
              creature->FindNearestCreature(ANCHORITE_KARJA, 15.0f, true))
        karja->AI()->DoAction(EVENT_KARJA_WALK);
      if (Creature *socrethar =
              creature->FindNearestCreature(SOCRETHAR, 500.0f, true))
        socrethar->SetFlag(UNIT_FIELD_FLAGS,
  ...
  struct adyen_the_lightbringerAI : public ScriptedAI {
    adyen_the_lightbringerAI(Creature *creature)
        : ScriptedAI(creature), event_started(false) {}
    ...
    void MovementInform(uint32 type, uint32 point) override {
      if (type != POINT_MOTION_TYPE)
        if (point == 9) {
          if (Creature *socrethar =
                  me->FindNearestCreature(SOCRETHAR, 50.0f, true))
    ...
    void UpdateAI(uint32 diff) override {
      ...
      while (uint32 eventId = _events.ExecuteEvent()) {
        ...
        case EVENT_HOLY_LIGHT:
          ...
          if (me->GetHealthPct() <= 45) {
          ...
          } else if (Unit *who = me->FindNearestCreature(ANCHORITE_KARJA, 30.0f,
                                                         true)) {
            if (who->GetHealthPct() <= 45) {
              ...
              _events.RepeatEvent(urand(18000, 22000));
                         ...
                         me->FindNearestCreature(EXARCH_ORELIS, 30.0f, true)) {
            if (who->GetHealthPct() <= 45) {
              me->CastSpell(who, HOLY_LIGHT, false);
              _events.RepeatEvent(urand(18000, 22000));
            }
    ...
    void UpdateAI(uint32 diff) override {
      ...
      case EVENT_SPELL_DEMORALIZING_SHOUT:
        if (me->FindNearestCreature(me->GetVictim()->GetEntry(), 10.0f, true)) {
          ...
          _events.ScheduleEvent(EVENT_SPELL_DEMORALIZING_SHOUT,
                                urand(10000, 12000));
        } else
          _events.ScheduleEvent(EVENT_SPELL_DEMORALIZING_SHOUT, 1000);
        break;
...
public:
  ...
  struct socretharAI : public ScriptedAI {
    socretharAI(Creature *creature)
        : ScriptedAI(creature), adyen(nullptr), orelis(nullptr), karja(nullptr),
          kaylaan(nullptr), ishanah(nullptr) {}
    ...
    bool GetCreature(uint32 CreatureID) {
      ...
      case ISHANAH_HIGH_PRIESTESS:
        ...
        if (ishanah == nullptr) {
          // Ishanah may be dead; in this case we also need a reference to the
          // creature for the respawn
          ishanah =
              me->FindNearestCreature(ISHANAH_HIGH_PRIESTESS, 100.0f, false);
          if (ishanah != nullptr)
            return true;
        } else
          return true;
    ...
    void JustDied(Unit * /*killer*/) override {
      ...
      if (GetCreature(ISHANAH_HIGH_PRIESTESS)) {
        ishanah->SetFlag(UNIT_NPC_FLAGS,
    ...
    void UpdateAI(uint32 diff) override {
      if (DeathblowToTheLegionRunning) {
        ...
        case EVENT_SOCRETHAR_SAY_2:
          Talk(1);
          if (Creature *summonKaylaan =
                  me->SummonCreature(KAYLAAN_THE_LOST, KaylaanSpawnPosition,
                                     TEMPSUMMON_CORPSE_TIMED_DESPAWN, 240000))
        ...
        case EVENT_KAYLAAN_SAY_5:
          if (GetCreature(KAYLAAN_THE_LOST))
            kaylaan->AI()->Talk(4);
          if (Creature *summonIshanah = me->SummonCreature(
                  ISHANAH_HIGH_PRIESTESS, IshanahSpawnPosition,
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 180000)) {
            summonIshanah->GetMotionMaster()->MovePath(ISHANAH_PATH_ID, false);
            summonIshanah->RemoveFlag(UNIT_NPC_FLAGS,
        ...
        case EVENT_ISHANAH_IS_BACK_AGAIN:
          if (GetCreature(ISHANAH_HIGH_PRIESTESS)) {
            ishanah->Respawn();
            ishanah->setActive(true); // ensure that Ishanah disappears, even
                                      // when no player is near
            ishanah->DespawnOrUnsummon(
                600000); // ensure that Ishanah disappears after 10 minutes
...
public:
  ...
  struct kaylaan_the_lostAI : public ScriptedAI {
    ...
    void
    ResetDeathblowEvent(bool event_over /* If true then reset the event*/) {
    ...
    void MovementInform(uint32 type, uint32 point) override {
      if (type != POINT_MOTION_TYPE) {
        if (point == 4) {
          ...
          if (Creature *socrethar =
                  me->FindNearestCreature(SOCRETHAR, 30.0f, true))
          ...
          if (first_waypath_done) {
            if (!second_waypath_done) {
              ...
              me->SetHomePosition(me->GetPosition());
              if (Creature *adyen = me->FindNearestCreature(
                      ADYEN_THE_LIGHTBRINGER, 30.0f, true))
    ...
    void UpdateAI(uint32 diff) override {
      ...
      if (me->GetHealthPct() <= 30)
        if (Creature *socrethar =
                me->FindNearestCreature(SOCRETHAR, 200.0f, true))
  ...
  struct npc_captain_saeedAI : public npc_escortAI {
    ...
    void SetGUID(uint64 playerGUID, int32 type) override {
      if (type == DATA_START_ENCOUNTER) {
        ...
        for (std::list<Creature *>::iterator itr = cl.begin(); itr != cl.end();
             ++itr) {
          summons.Summon(*itr);
    ...
    void SummonsAction(Unit *who) {
      float i = 0;
      for (std::list<uint64>::iterator itr = summons.begin();
           itr != summons.end(); ++itr, i += 1.0f)
        if (Creature *cr = ObjectAccessor::GetCreature(*me, *itr)) {
          if (who == nullptr) {
            cr->GetMotionMaster()->Clear(false);
            cr->GetMotionMaster()->MoveFollow(
                me, 2.0f, M_PI / 2.0f + (i / summons.size() * M_PI));
          } else {
            cr->SetHomePosition(cr->GetPositionX(), cr->GetPositionY(),
                                cr->GetPositionZ(), cr->GetOrientation());
    ...
    void EnterCombat(Unit *who) override { SummonsAction(who); }
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    ...
    if (player->GetQuestStatus(QUEST_DIMENSIUS_DEVOURING) ==
      ...
      if (!creature->AI()->GetData(1))
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Let's move out.",
                         GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
      else
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Let's start the battle.",
...
public:
  ...
  struct npc_commander_dawnforgeAI : public ScriptedAI {
    ...
    bool CanStartEvent(Player *player) {
      ...
      LOG_DEBUG("scripts.ai", "TSCR: npc_commander_dawnforge event already in "
                              "progress, need to wait.");
#endif
      return false;
    }
    ...
    void UpdateAI(uint32 diff) override {
      ...
      case 4:
        ...
        me->SummonCreature(CreatureEntry[2], 2325.851563f, 2799.534668f,
                           133.084229f, 6.038996f, TEMPSUMMON_TIMED_DESPAWN,
                           90000);
        ++Phase;
        Phase_Timer = 500;
        break;
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    ...
    if (player->GetQuestStatus(QUEST_ON_NETHERY_WINGS) ==
            QUEST_STATUS_INCOMPLETE &&
        !player->HasItemCount(29778))
      AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_ITEM,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
  ...
  struct npc_phase_hunterAI : public ScriptedAI {
    ...
    void UpdateAI(uint32 diff) override {
      ...
      if (me->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED) ||
          me->HasUnitState(
              UNIT_STATE_ROOT)) // if the mob is rooted/slowed by spells eg.:
                                // Entangling Roots, Frost Nova, Hamstring,
                                // Crippling Poison, etc. => remove it
      ...
      if (ManaBurnTimer <= diff) {
        ...
        if (!UnitsWithMana.empty()) {
          DoCast(
              Warhead::Containers::SelectRandomContainerElement(UnitsWithMana),
              SPELL_MANA_BURN);
...
public:
  ...
  struct npc_bessyAI : public npc_escortAI {
    ...
    void WaypointReached(uint32 waypointId) override {
      ...
      case 3: // first spawn
        Talk(SAY_BESSY_1);
        me->SummonCreature(SPAWN_FIRST, 2449.67f, 2183.11f, 96.85f, 6.20f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(SPAWN_FIRST, 2449.53f, 2184.43f, 96.36f, 6.27f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(SPAWN_FIRST, 2449.85f, 2186.34f, 97.57f, 6.08f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        break;
      case 7:
        Talk(SAY_BESSY_1);
        me->SummonCreature(SPAWN_SECOND, 2309.64f, 2186.24f, 92.25f, 6.06f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(SPAWN_SECOND, 2309.25f, 2183.46f, 91.75f, 6.22f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        break;
      case 12:
  ...
  struct npc_maxx_a_million_escortAI : public npc_escortAI {
    ...
    void WaypointReached(uint32 waypointId) override {
      ...
      case 29:
        ...
        if (GetClosestGameObjectWithEntry(me, GO_DRAENEI_MACHINE,
  ...
  bool OnQuestAccept(Player *player, Creature *creature,
    ...
    if (quest->GetQuestId() == QUEST_MARK_V_IS_ALIVE) {
      if (npc_maxx_a_million_escortAI *pEscortAI =
              CAST_AI(npc_maxx_a_million_escort::npc_maxx_a_million_escortAI,
                      creature->AI())) {
        creature->setFaction(113);
