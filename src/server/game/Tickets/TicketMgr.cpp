...
// GM ticket
GmTicket::GmTicket()
    : _id(0), _playerGuid(0), _type(TICKET_TYPE_OPEN), _posX(0), _posY(0),
      _posZ(0), _mapId(0), _createTime(0), _lastModifiedTime(0), _closedBy(0),
      _resolvedBy(0), _assignedTo(0), _completed(false),
      _escalatedStatus(TICKET_UNASSIGNED), _viewed(false), _needResponse(false),
      _needMoreHelp(false) {}

GmTicket::GmTicket(Player *player)
    : _type(TICKET_TYPE_OPEN), _createTime(time(nullptr)),
      _lastModifiedTime(time(nullptr)), _closedBy(0), _resolvedBy(0),
      _assignedTo(0), _completed(false), _escalatedStatus(TICKET_UNASSIGNED),
      _viewed(false), _needMoreHelp(false) {
...
void GmTicket::SaveToDB(SQLTransaction &trans) const {
  ...
  // posZ, lastModifiedTime, closedBy, assignedTo, comment, response, completed,
  // escalated, viewed, needMoreHelp, resolvedBy
  uint8 index = 0;
...
void GmTicket::WritePacket(WorldPacket &data) const {
  ...
  // I am not sure how blizzlike this is, and we don't really have a way to find
  // out
  data << GetAge(sTicketMgr->GetLastChange());

  data << uint8(
      std::min(_escalatedStatus, TICKET_IN_ESCALATION_QUEUE)); // escalated data
  data << uint8(
      _viewed ? GMTICKET_OPENEDBYGM_STATUS_OPENED
              : GMTICKET_OPENEDBYGM_STATUS_NOT_OPENED); // whether or not it has
                                                        // been viewed
}
...
void GmTicket::SetGmAction(uint32 needResponse, bool needMoreHelp) {
  ...
  _needMoreHelp =
      needMoreHelp; // Requests further GM interaction on a ticket to which a GM
                    // has already responded. Basically means "has a new ticket"
}
...
void TicketMgr::LoadTickets() {
  ...
  LOG_INFO("server", ">> Loaded %u GM tickets in %u ms", count,
...
void TicketMgr::LoadSurveys() {
  ...
  LOG_INFO("server", ">> Loaded GM Survey count from database in %u ms",
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
