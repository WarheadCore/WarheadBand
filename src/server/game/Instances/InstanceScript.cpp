...
void InstanceScript::LoadDoorData(const DoorData *data) {
  while (data->entry) {
    if (data->bossId < bosses.size())
      doors.insert(std::make_pair(data->entry,
...
void InstanceScript::AddDoor(GameObject *door, bool add) {
  ...
  for (; range.first != range.second; ++range.first) {
    ...
    if (add) {
      ...
      case BOUNDARY_NW:
      case BOUNDARY_SE:
        data.bossInfo->boundary[data.boundary] =
...
bool InstanceScript::SetBossState(uint32 id, EncounterState state) {
  if (id < bosses.size()) {
    ...
    if (bossInfo->state == TO_BE_DECIDED) // loading
    {
      ...
      // LOG_ERROR("server", "Inialize boss %u state as %u.", id,
      // (uint32)state);
      return false;
    } else {
...
bool InstanceScript::CheckAchievementCriteriaMeet(
    uint32 criteria_id, Player const * /*source*/,
    Unit const * /*target*/ /*= nullptr*/, uint32 /*miscvalue1*/ /*= 0*/) {
  LOG_ERROR("server",
            "Achievement system call "
            "InstanceScript::CheckAchievementCriteriaMeet but instance script "
            "for map %u not have implementation for achievement criteria %u",
...
void InstanceScript::SendEncounterUnit(uint32 type, Unit *unit /*= nullptr*/,
                                       uint8 param1 /*= 0*/,
                                       uint8 param2 /*= 0*/) {
