...
#define OUT_SAVE_INST_DATA                                                     \
  LOG_DEBUG(                                                                   \
      "scripts.ai",                                                            \
      "TSCR: Saving Instance Data for Instance %s (Map %d, Instance Id %d)",   \
      instance->GetMapName(), instance->GetId(), instance->GetInstanceId())
#define OUT_SAVE_INST_DATA_COMPLETE                                            \
  LOG_DEBUG("scripts.ai",                                                      \
            "TSCR: Saving Instance Data for Instance %s (Map %d, Instance Id " \
            "%d) completed.",                                                  \
            instance->GetMapName(), instance->GetId(),                         \
            instance->GetInstanceId())
#define OUT_LOAD_INST_DATA(a)                                                  \
  LOG_DEBUG("scripts.ai",                                                      \
            "TSCR: Loading Instance Data for Instance %s (Map %d, Instance "   \
            "Id %d). Input is '%s'",                                           \
            instance->GetMapName(), instance->GetId(),                         \
            instance->GetInstanceId(), a)
#define OUT_LOAD_INST_DATA_COMPLETE                                            \
  LOG_DEBUG("scripts.ai",                                                      \
            "TSCR: Instance Data Load for Instance %s (Map %d, Instance Id: "  \
            "%d) is complete.",                                                \
            instance->GetMapName(), instance->GetId(),                         \
            instance->GetInstanceId())
#define OUT_LOAD_INST_DATA_FAIL                                                \
  LOG_ERROR("server",                                                          \
            "TSCR: Unable to load Instance Data for Instance %s (Map %d, "     \
            "Instance Id: %d).",                                               \
            instance->GetMapName(), instance->GetId(),                         \
            instance->GetInstanceId())
...
public:
  ...
  virtual void OnPlayerAreaUpdate(Player * /*player*/, uint32 /*oldArea*/,
                                  uint32 /*newArea*/) {}
  ...
  void SendEncounterUnit(uint32 type, Unit *unit = nullptr, uint8 param1 = 0,
                         uint8 param2 = 0);
...
template <class AI, class T> AI *GetInstanceAI(T *obj) {
  if (InstanceMap *instance = obj->GetMap()->ToInstanceMap())
    if (instance->GetInstanceScript())
      return new AI(obj);
