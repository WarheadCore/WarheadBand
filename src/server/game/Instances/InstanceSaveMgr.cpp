...
InstanceSave *InstanceSaveManager::AddInstanceSave(uint32 mapId,
  ...
  if (!entry) {
    LOG_ERROR("server",
              "InstanceSaveManager::AddInstanceSave: wrong mapid = %d, "
              "instanceid = %d!",
              mapId, instanceId);
    return nullptr;
  }
  ...
  if (instanceId == 0) {
    LOG_ERROR("server",
              "InstanceSaveManager::AddInstanceSave: mapid = %d, wrong "
              "instanceid = %d!",
              mapId, instanceId);
    return nullptr;
  }
  ...
  if (difficulty >=
    ...
    LOG_ERROR("server",
              "InstanceSaveManager::AddInstanceSave: mapid = %d, instanceid = "
              "%d, wrong dificalty %u!",
  ...
  if (entry->map_type == MAP_RAID || difficulty > DUNGEON_DIFFICULTY_NORMAL) {
    ...
    resetTime =
        time(nullptr) + 3 * DAY; // normals expire after 3 days even if someone
                                 // is still bound to them, cleared on startup
    extendedResetTime = 0;
  }
...
bool InstanceSaveManager::DeleteInstanceSaveIfNeeded(uint32 InstanceId,
                                                     bool skipMapCheck) {
  return DeleteInstanceSaveIfNeeded(GetInstanceSave(InstanceId), skipMapCheck);
}

bool InstanceSaveManager::DeleteInstanceSaveIfNeeded(InstanceSave *save,
                                                     bool skipMapCheck) {
  // pussywizard: save is removed only when there are no more players bound AND
  // the map doesn't exist pussywizard: this function is called when unbinding a
  // player and when unloading a map
  if (!lock_instLists && save && save->m_playerList.empty() &&
    ...
    // clear respawn times (if map is loaded do it just to be sure, if already
    // unloaded it won't do it by itself)
...
InstanceSave::~InstanceSave() { ASSERT(m_playerList.empty()); }
...
bool InstanceSave::RemovePlayer(uint32 guidLow, InstanceSaveManager *ism) {
  ...
  // ism passed as an argument to avoid calling via singleton (might result in a
  // deadlock)
...
void InstanceSaveManager::LoadInstances() {
  ...
  CharacterDatabase.DirectExecute(
      "DELETE ci.* FROM character_instance AS ci LEFT JOIN characters AS c ON "
      "ci.guid = c.guid WHERE c.guid IS NULL");
  ...
  CharacterDatabase.DirectExecute(
      "DELETE FROM creature_respawn WHERE instanceId > 0 AND instanceId NOT IN "
      "(SELECT id FROM instance)");
  CharacterDatabase.DirectExecute(
      "DELETE FROM gameobject_respawn WHERE instanceId > 0 AND instanceId NOT "
  ...
  LOG_INFO("server", ">> Loaded instances and binds in %u ms",
...
void InstanceSaveManager::LoadResetTimes() {
  ...
  if (result) {
    do {
      ...
      if (!mapDiff) {
        LOG_ERROR("server",
                  "InstanceSaveManager::LoadResetTimes: invalid "
                  "mapid(%u)/difficulty(%u) pair in instance_reset!",
                  mapid, difficulty);
        CharacterDatabase.DirectPExecute("DELETE FROM instance_reset WHERE "
                                         "mapid = '%u' AND difficulty = '%u'",
                                         mapid, difficulty);
        continue;
      }
  ...
  // calculate new global reset times for expired instances and those that have
  // never been reset yet add the global reset times to the priority queue
  for (MapDifficultyMap::const_iterator itr = sMapDifficultyMap.begin();
    ...
    if (!t) {
      ...
      CharacterDatabase.DirectPExecute(
          "INSERT INTO instance_reset VALUES ('%u', '%u', '%u')", mapid,
          difficulty, (uint32)t);
    }
    ...
    if (t < now) {
      ...
      CharacterDatabase.DirectPExecute(
          "UPDATE instance_reset SET resettime = '%u' WHERE mapid = '%u' AND "
          "difficulty = '%u'",
          (uint32)t, mapid, difficulty);
    }
...
void InstanceSaveManager::LoadInstanceSaves() {
  ...
  if (result) {
    do {
      ...
      InstanceSave *save =
          AddInstanceSave(mapId, instanceId, Difficulty(difficulty), true);
      if (save) {
    ...
    do {
      ...
      if (InstanceSave *save = GetInstanceSave(instanceId)) {
        ...
        if (bind.save) // pussywizard: another bind for the same map and
                       // difficulty! may happen because of mysql thread races
        {
          if (bind.perm) // already loaded perm -> delete currently checked one
                         // from db
          {
...
void InstanceSaveManager::Update() {
  ...
  if (resetOccurred) {
    LOG_INFO("server", "Instance ID reset occurred, sending updated calendar "
                       "and raid info to all players!");
    WorldPacket dummy;
...
void InstanceSaveManager::_ResetSave(InstanceSaveHashMap::iterator &itr) {
  ...
  for (InstanceSave::PlayerListType::iterator iter = pList.begin(), iter2;
       iter != pList.end();) {
    iter2 = iter++;
    PlayerUnbindInstanceNotExtended(
        *iter2, itr->second->GetMapId(), itr->second->GetDifficulty(),
        ObjectAccessor::GetObjectInOrOutOfWorld(
            MAKE_NEW_GUID(*iter2, 0, HIGHGUID_PLAYER), (Player *)nullptr));
  }
  ...
  if (pList.empty()) {
    ...
    // delete character_instance per id where extended = 0, transtaction with
    // set extended = 0, transaction is used to avoid mysql thread races
    ...
    itr->second->SetResetTime(
        GetResetTimeFor(itr->second->GetMapId(), itr->second->GetDifficulty()));
...
void InstanceSaveManager::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty,
  ...
  if (!warn) {
    ...
    if (!mapDiff || !mapDiff->resetTime) {
      LOG_ERROR("server",
                "InstanceSaveManager::ResetOrWarnAll: not valid difficulty or "
                "no reset delay for map %d",
                mapid);
      return;
    }
    ...
    // remove all binds to instances of the given map and delete from db (delete
    // per instance id, no mass deletion!) do this after new reset time is
    // calculated
...
InstancePlayerBind *
InstanceSaveManager::PlayerBindToInstance(uint32 guidLow, InstanceSave *save,
                                          bool permanent,
                                          Player *player /*= nullptr*/) {
  InstancePlayerBind &bind =
      playerBindStorage[guidLow]->m[save->GetDifficulty()][save->GetMapId()];
  ASSERT(!bind.perm ||
         permanent); // ensure there's no changing permanent to temporary, this
                     // can be done only by unbinding
  ...
  if (bind.save) {
    ...
    // DURING LOADING FROM DB! example: enter instance -> bind -> update old id
    // to new id -> exit -> delete new id if delete by new id is executed before
    // update, then we end up with shit in db
    /*SQLTransaction trans = CharacterDatabase.BeginTransaction();
    ...
    i.id WHERE ci.guid = ? AND i.map = ? AND i.difficulty = ? stmt->setUInt32(0,
    guidLow); stmt->setUInt16(1, uint16(save->GetMapId())); stmt->setUInt8(2,
    uint8(save->GetDifficulty())); trans->Append(stmt); stmt =
  ...
  if (player)
    sScriptMgr->OnPlayerBindToInstance(player, save->GetDifficulty(),
                                       save->GetMapId(), permanent);

  return &bind;
}

void InstanceSaveManager::PlayerUnbindInstance(uint32 guidLow, uint32 mapid,
                                               Difficulty difficulty,
                                               bool deleteFromDB,
                                               Player *player /*= nullptr*/) {
...
void InstanceSaveManager::PlayerUnbindInstanceNotExtended(
    uint32 guidLow, uint32 mapid, Difficulty difficulty,
    Player *player /*= nullptr*/) {
  BoundInstancesMapWrapper *w = playerBindStorage[guidLow];
  BoundInstancesMap::iterator itr = w->m[difficulty].find(mapid);
  if (itr != w->m[difficulty].end()) {
...
InstancePlayerBind *
InstanceSaveManager::PlayerGetBoundInstance(uint32 guidLow, uint32 mapid,
                                            Difficulty difficulty) {
  Difficulty difficulty_fixed =
      (IsSharedDifficultyMap(mapid) ? Difficulty(difficulty % 2) : difficulty);
...
bool InstanceSaveManager::PlayerIsPermBoundToInstance(uint32 guidLow,
                                                      uint32 mapid,
                                                      Difficulty difficulty) {
  if (InstancePlayerBind *bind =
          PlayerGetBoundInstance(guidLow, mapid, difficulty))
    if (bind->perm)
      return true;
  return false;
}
...
InstanceSaveManager::PlayerGetBoundInstances(uint32 guidLow,
                                             Difficulty difficulty) {
...
InstanceSave *
InstanceSaveManager::PlayerGetInstanceSave(uint32 guidLow, uint32 mapid,
                                           Difficulty difficulty) {
...
uint32 InstanceSaveManager::PlayerGetDestinationInstanceId(
    Player *player, uint32 mapid, Difficulty difficulty) {
  // returning 0 means a new instance will be created
  // non-zero implicates that InstanceSave exists

  InstancePlayerBind *ipb =
      PlayerGetBoundInstance(player->GetGUIDLow(), mapid, difficulty);
  if (ipb && ipb->perm) // 1. self perm
    return ipb->save->GetInstanceId();
  if (Group *g = player->GetGroup()) {
    if (InstancePlayerBind *ilb =
            PlayerGetBoundInstance(GUID_LOPART(g->GetLeaderGUID()), mapid,
                                   difficulty)) // 2. leader temp/perm
...
void InstanceSaveManager::CopyBinds(uint32 from, uint32 to, Player *toPlr) {
  ...
  for (uint8 d = 0; d < MAX_DIFFICULTY; ++d) {
    ...
    for (BoundInstancesMap::const_iterator itr = bi.begin(); itr != bi.end();
         ++itr)
...
void InstanceSaveManager::UnbindAllFor(InstanceSave *save) {
  ...
  while (!pList.empty())
    PlayerUnbindInstance(
        *(pList.begin()), save->GetMapId(), save->GetDifficulty(), true,
        ObjectAccessor::GetObjectInOrOutOfWorld(
            MAKE_NEW_GUID(*(pList.begin()), 0, HIGHGUID_PLAYER),
            (Player *)nullptr));
}
