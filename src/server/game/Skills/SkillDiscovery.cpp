...
void LoadSkillDiscoveryTable() {
  ...
  if (!result) {
    LOG_ERROR("sql.sql", ">> Loaded 0 skill discovery definitions. DB table "
  ...
  do {
    ...
    if (chance <= 0) // chance
    {
      ssNonDiscoverableEntries
          << "spellId = " << spellId << " reqSkillOrSpell = " << reqSkillOrSpell
    ...
    if (reqSkillOrSpell > 0) // spell case
    {
      ...
      if (!reqSpellInfo) {
        if (reportedReqSpells.find(absReqSkillOrSpell) ==
          ...
          LOG_ERROR("sql.sql",
                    "Spell (ID: %u) have not existed spell (ID: %i) in "
      ...
      if (reqSpellInfo->Mechanic != MECHANIC_DISCOVERY &&
        ...
        if (reportedReqSpells.find(absReqSkillOrSpell) ==
          ...
          LOG_ERROR(
              "sql.sql",
              "Spell (ID: %u) not have MECHANIC_DISCOVERY (28) value in "
              "Mechanic field in spell.dbc"
              " and not 100%% chance random discovery ability but listed for "
              "spellId %u (and maybe more) in `skill_discovery_template` table",
    ...
    } else if (reqSkillOrSpell == 0) // skill case
    {
      ...
      if (bounds.first == bounds.second) {
        LOG_ERROR(
            "sql.sql",
            "Spell (ID: %u) not listed in `SkillLineAbility.dbc` but listed "
  ...
  if (!ssNonDiscoverableEntries.str().empty())
    LOG_ERROR(
        ...
        "value < 0.000001 in `skill_discovery_template` DB table . List:\n%s",
  ...
  for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize();
    ...
    if (SkillDiscoveryStore.find(int32(spell_id)) == SkillDiscoveryStore.end())
      LOG_ERROR("sql.sql",
                "Spell (ID: %u) is 100%% chance random discovery ability but "
                "not have data in `skill_discovery_template` table",
                spell_id);
  }

  LOG_INFO("server", ">> Loaded %u skill discovery definitions in %u ms", count,
...
uint32 GetExplicitDiscoverySpell(uint32 spellId, Player *player) {
  ...
  SkillLineAbilityMapBounds bounds =
      sSpellMgr->GetSkillLineAbilityMapBounds(spellId);
  uint32 skillvalue = bounds.first != bounds.second
                          ? player->GetSkillValue(bounds.first->second->skillId)
                          : uint32(0);
