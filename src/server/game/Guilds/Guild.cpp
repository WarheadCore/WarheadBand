...
void Guild::RankInfo::CreateMissingTabsIfNeeded(
    uint8 tabs, SQLTransaction &trans, bool logOnCreate /* = false */) {
...
bool Guild::BankTab::WriteSlotPacket(WorldPacket &data, uint8 slotId,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("guild",
              "MSG_QUERY_GUILD_BANK_TEXT [Broadcast]: Tabid: %u, Text: %s",
...
void Guild::Member::SetStats(std::string const &name, uint8 level, uint8 _class,
                             uint8 gender, uint32 zoneId, uint32 accountId) {
  m_name = name;
  m_level = level;
  m_class = _class;
  m_gender = gender;
...
bool Guild::Member::LoadFromDB(Field *fields) {
  ...
  if (!m_zoneId) {
    LOG_ERROR("server", "Player (GUID: %u) has broken zone-data",
              GUID_LOPART(m_guid));
...
bool Guild::Member::CheckStats() const {
  if (m_level < 1) {
    LOG_ERROR("server",
              "Player (GUID: %u) has a broken data in field "
...
void Guild::Member::WritePacket(WorldPacket &data, bool sendOfficerNote) const {
  data << uint64(m_guid) << uint8(m_flags) << m_name << uint32(m_rankId)
       << uint8(m_level) << uint8(m_class) << uint8(m_gender)
       << uint32(m_zoneId);
...
// Guild
Guild::Guild()
    : m_id(0), m_leaderGuid(0), m_createdDate(0), m_accountsNumber(0),
      m_bankMoney(0), m_eventLog(nullptr) {
...
bool Guild::Create(Player *pLeader, std::string const &name) {
  ...
  for (short i = 0;
       i <
       static_cast<short>(sWorld->getIntConfig(CONFIG_GUILD_BANK_INITIAL_TABS));
       i++) {
    _CreateNewBankTab();
  }
...
void Guild::HandleRoster(WorldSession *session) {
  ...
  WorldPacket data(SMSG_GUILD_ROSTER,
                   (4 + m_motd.length() + 1 + m_info.length() + 1 + 4 +
                    _GetRanksSize() * (4 + 4 + GUILD_BANK_MAX_TABS * (4 + 4)) +
                    m_members.size() * 50));
  ...
  for (Ranks::const_iterator ritr = m_ranks.begin(); ritr != m_ranks.end();
       ++ritr)
    ritr->WritePacket(data);

  for (Members::const_iterator itr = m_members.begin(); itr != m_members.end();
       ++itr)
    itr->second->WritePacket(
        data, _HasRankRight(session->GetPlayer(), GR_RIGHT_VIEWOFFNOTE));
...
void Guild::HandleQuery(WorldSession *session) {
  ...
  session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "SMSG_GUILD_QUERY_RESPONSE [%s]",
            session->GetPlayerInfo().c_str());
#endif
}
...
void Guild::HandleSetLeader(WorldSession *session, std::string const &name) {
  ...
  // Old leader must be a member of guild
    ...
    if (Member *pNewLeader = GetMember(name)) {
      ...
      _BroadcastEvent(GE_LEADER_CHANGED, 0, player->GetName().c_str(),
                      name.c_str());
    }
  }
}
...
void Guild::HandleSetBankTabInfo(WorldSession *session, uint8 tabId,
  ...
  _BroadcastEvent(GE_BANK_TAB_UPDATED, 0, std::to_string(tabId).c_str(),
                  name.c_str(), icon.c_str());
}

void Guild::HandleSetMemberNote(WorldSession *session, std::string const &name,
                                std::string const &note, bool isPublic) {
...
void Guild::HandleSetRankInfo(WorldSession *session, uint8 rankId,
  ...
  if (!_IsLeader(session->GetPlayer()))
    ...
    for (GuildBankRightsAndSlotsVec::const_iterator itr =
             rightsAndSlots.begin();
         itr != rightsAndSlots.end(); ++itr)
      _SetRankBankTabRightsAndSlots(rankId, *itr);

    _BroadcastEvent(GE_RANK_UPDATED, 0, std::to_string(rankId).c_str(),
                    name.c_str());
  }
}
...
void Guild::HandleInviteMember(WorldSession *session, std::string const &name) {
  ...
  if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
      pInvitee->GetTeamId() != player->GetTeamId()) {
...
void Guild::HandleUpdateMemberRank(WorldSession *session,
                                   std::string const &name, bool demote) {
  Player *player = session->GetPlayer();
  ...
  // Promoted player must be a member of guild
    ...
    _LogEvent(demote ? GUILD_EVENT_LOG_DEMOTE_PLAYER
              ...
              player->GetGUIDLow(), GUID_LOPART(member->GetGUID()), newRankId);
...
void Guild::HandleAddNewRank(WorldSession *session, std::string const &name) {
  ...
  if (_IsLeader(session->GetPlayer()))
    if (_CreateRank(name, GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK))
      _BroadcastEvent(GE_RANK_UPDATED, 0, std::to_string(size).c_str(),
                      name.c_str());
}
...
void Guild::HandleMemberDepositMoney(WorldSession *session, uint32 amount) {
  ...
  std::string aux = Warhead::Impl::ByteArrayToHexStr(
      reinterpret_cast<uint8 *>(&m_bankMoney), 8, true);
  _BroadcastEvent(GE_BANK_MONEY_SET, 0, aux.c_str());

  if (amount > 10 * GOLD)
    CharacterDatabase.PExecute(
        "INSERT INTO log_money VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", %u, "
        "\"<GB DEPOSIT> %s (guild id: %u, members: %u, new amount: %u, leader "
        "guid low: %u, char level: %u)\", NOW())",
        session->GetAccountId(), player->GetGUIDLow(),
        player->GetName().c_str(), session->GetRemoteAddress().c_str(), 0, "",
        amount, GetName().c_str(), GetId(), GetMemberCount(),
        GetTotalBankMoney(), (uint32)(GetLeaderGUID() & 0xFFFFFFFF),
        player->getLevel());
}
...
bool Guild::HandleMemberWithdrawMoney(WorldSession *session, uint32 amount,
  ...
  if (amount > 10 * GOLD)
    CharacterDatabase.PExecute(
        "INSERT INTO log_money VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", %u, "
        "\"<GB WITHDRAW> %s (guild id: %u, members: %u, new amount: %u, leader "
        "guid low: %u, char level: %u)\", NOW())",
        session->GetAccountId(), player->GetGUIDLow(),
        player->GetName().c_str(), session->GetRemoteAddress().c_str(), 0, "",
        amount, GetName().c_str(), GetId(), GetMemberCount(),
...
void Guild::SendEventLog(WorldSession *session) const {
  ...
  session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "MSG_GUILD_EVENT_LOG_QUERY [%s]",
            session->GetPlayerInfo().c_str());
#endif
}
...
void Guild::SendBankLog(WorldSession *session, uint8 tabId) const {
  ...
  if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS) {
    ...
    session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("guild", "MSG_GUILD_BANK_LOG_QUERY [%s]",
              session->GetPlayerInfo().c_str());
#endif
  }
}
...
void Guild::SendPermissions(WorldSession *session) const {
  ...
  session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "MSG_GUILD_PERMISSIONS [%s] Rank: %u",
...
void Guild::SendMoneyInfo(WorldSession *session) const {
  ...
  session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "MSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: %u",
...
bool Guild::LoadMemberFromDB(Field *fields) {
  ...
  Member *member = new Member(m_id, MAKE_NEW_GUID(lowguid, 0, HIGHGUID_PLAYER),
                              fields[2].GetUInt8());
...
void Guild::LoadBankTabFromDB(Field *fields) {
  ...
  if (tabId >= _GetPurchasedTabsSize())
    LOG_ERROR("server", "Invalid tab (tabId: %u) in guild bank, skipped.",
              tabId);
  else
...
bool Guild::Validate() {
  ...
  if (ranks < GUILD_RANKS_MIN_COUNT || ranks > GUILD_RANKS_MAX_COUNT) {
    LOG_ERROR("server", "Guild %u has invalid number of ranks, creating new...",
              m_id);
    broken_ranks = true;
  } else {
  ...
  if (session && session->GetPlayer() &&
    ...
    for (Members::const_iterator itr = m_members.begin();
      ...
      if (Player *player = itr->second->FindPlayer())
        if (_HasRankRight(player, officerOnly ? GR_RIGHT_OFFCHATLISTEN
  ...
  if (player) {
    ...
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt)) {
      ...
      member->SetStats(name, fields[1].GetUInt8(), fields[2].GetUInt8(),
                       fields[3].GetUInt8(), fields[4].GetUInt16(),
                       fields[5].GetUInt32());
...
void Guild::DeleteMember(uint64 guid, bool isDisbanding, bool isKicked,
  ...
  if (m_leaderGuid == guid && !isDisbanding) {
    ...
    for (Guild::Members::iterator i = m_members.begin(); i != m_members.end();
         ++i) {
      if (i->first == lowguid)
        oldLeader = i->second;
...
void Guild::_CreateLogHolders() {
  m_eventLog =
      new LogHolder(m_id, sWorld->getIntConfig(CONFIG_GUILD_EVENT_LOG_COUNT));
  for (uint8 tabId = 0; tabId <= GUILD_BANK_MAX_TABS; ++tabId)
    m_bankEventLog[tabId] = new LogHolder(
        m_id, sWorld->getIntConfig(CONFIG_GUILD_BANK_EVENT_LOG_COUNT));
}
...
inline void Guild::_UpdateMemberWithdrawSlots(SQLTransaction &trans,
                                              uint64 guid, uint8 tabId) {
  if (Member *member = GetMember(guid)) {
    uint8 rankId = member->GetRankId();
    if (rankId != GR_GUILDMASTER &&
        member->GetBankWithdrawValue(tabId) <
...
inline bool Guild::_MemberHasTabRights(uint64 guid, uint8 tabId,
                                       uint32 rights) const {
...
inline void Guild::_LogEvent(GuildEventLogTypes eventType, uint32 playerGuid1,
  ...
  SQLTransaction trans = CharacterDatabase.BeginTransaction();
  m_eventLog->AddEvent(trans, new EventLogEntry(m_id, m_eventLog->GetNextGUID(),
                                                eventType, playerGuid1,
                                                playerGuid2, newRank));
  CharacterDatabase.CommitTransaction(trans);
...
void Guild::_LogBankEvent(SQLTransaction &trans,
                          ...
                          uint32 lowguid, uint32 itemOrMoney,
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, uint64 guid,
                            const char *param1, const char *param2,
                            const char *param3) const {
...
void Guild::_SendBankList(WorldSession *session /* = nullptr*/,
                          uint8 tabId /*= 0*/, bool sendAllSlots /*= false*/,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("guild",
              "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %d",
              session->GetPlayerInfo().c_str(), tabId, sendAllSlots, numSlots);
#endif
  } else // TODO - Probably this is just sent to session + those that have sent
         // CMSG_GUILD_BANKER_ACTIVATE
  {
    for (Members::const_iterator itr = m_members.begin();
      ...
      LOG_DEBUG(
          "guild",
          "SMSG_GUILD_BANK_LIST [%s]: TabId: %u, FullSlots: %u, slots: %u",
