...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ,
                                  bool forceDest) {
  float x, y, z;
...
bool PathGenerator::CalculatePath(float x, float y, float z, float destX,
                                  float destY, float destZ, bool forceDest) {
  if (!Warhead::IsValidMapCoord(destX, destY, destZ) ||
      !Warhead::IsValidMapCoord(x, y, z))
    return false;
...
dtPolyRef PathGenerator::GetPolyByLocation(float const *point,
  ...
  if (dtStatusSucceed(_navMeshQuery->findNearestPoly(point, extents, &_filter,
                                                     &polyRef, closestPoint)) &&
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const &startPos,
  ...
  if (startFarFromPoly || endFarFromPoly) {
    ...
    bool isUnderWaterStart =
        _source->GetMap()->IsUnderWater(startPos.x, startPos.y, startPos.z);
    bool isUnderWaterEnd =
        _source->GetMap()->IsUnderWater(endPos.x, endPos.y, endPos.z);
    bool isFarUnderWater =
        startFarFromPoly ? isUnderWaterStart : isUnderWaterEnd;

    Unit const *_sourceUnit = _source->ToUnit();

    if (_sourceUnit) {
      bool isUnderWater =
          (_sourceUnit->CanSwim() && isUnderWaterStart && isUnderWaterEnd) ||
          (isFarUnderWater && _useRaycast);

      if (isUnderWater || _sourceUnit->CanFly() ||
  ...
  // handle this case as if they were 2 different polygons, building a line path
  // split in some few points
  ...
  if (startPolyFound && endPolyFound) {
    ...
    if (!suffixPolyLength || dtStatusFailed(dtResult)) {
      ...
      LOG_ERROR("server", "PathGenerator::BuildPolyPath: Path Build failed %u",
                _source->GetGUIDLow());
    }
    ...
    if (_useRaycast) {
      ...
      dtResult = _navMeshQuery->raycast(
          startPoly, startPoint, endPoint, &_filter, &hit, hitNormal,
      ...
      if (hit != FLT_MAX) {
        ...
        // (sometimes the point is actually outside of the polygons due to float
        // precision issues)
        hit *= 0.99f;
        ...
        if (dtStatusFailed(_navMeshQuery->getPolyHeight(
                _pathPolyRefs[_polyLength - 1], endPoint, &endPoint[1])))
          _navMeshQuery->closestPointOnPolyBoundary(
    ...
    if (!_polyLength || dtStatusFailed(dtResult)) {
      ...
      LOG_ERROR(
          "server",
          "PathGenerator::BuildPolyPath: %u Path Build failed: 0 length path",
          _source->GetGUIDLow());
      BuildShortcut();
      _type = PATHFIND_NOPATH;
      return;
...
void PathGenerator::BuildPointPath(const float *startPoint,
  ...
  if (_useRaycast) {
    // _straightLine uses raycast and it currently doesn't support building a
    // point path, only a 2-point path with start and hitpoint/end is returned
    LOG_ERROR(
        "server",
        "PathGenerator::BuildPointPath() called with _useRaycast for unit %u",
        _source->GetGUIDLow());
    BuildShortcut();
    _type = PATHFIND_NOPATH;
    return;
  ...
  for (uint32 i = 0; i < pointCount; ++i) {
    G3D::Vector3 vector = G3D::Vector3(pathPoints[i * VERTEX_SIZE + 2],
                                       pathPoints[i * VERTEX_SIZE],
                                       pathPoints[i * VERTEX_SIZE + 1]);
    ZLiquidStatus status = _source->GetMap()->getLiquidStatus(
        vector.x, vector.y, vector.z, MAP_ALL_LIQUIDS, nullptr);
...
void PathGenerator::NormalizePath() {
  for (uint32 i = 0; i < _pathPoints.size(); ++i) {
    _source->UpdateAllowedPositionZ(_pathPoints[i].x, _pathPoints[i].y,
                                    _pathPoints[i].z);
  }
}
...
void PathGenerator::UpdateFilter() {
  ...
  if (Unit const *_sourceUnit = _source->ToUnit()) {
    if (_sourceUnit->IsInWater() || _sourceUnit->IsUnderWater()) {
      ...
      includedFlags |=
          GetNavTerrain(_source->GetPositionX(), _source->GetPositionY(),
...
NavTerrain PathGenerator::GetNavTerrain(float x, float y, float z) const {
  ...
  ZLiquidStatus liquidStatus =
      _source->GetMap()->getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS, &data);
...
uint32 PathGenerator::FixupCorridor(dtPolyRef *path, uint32 npath,
                                    uint32 maxPath, dtPolyRef const *visited,
                                    uint32 nvisited) {
...
bool PathGenerator::GetSteerTarget(float const *startPos, float const *endPos,
                                   float minTargetDist, dtPolyRef const *path,
                                   uint32 pathSize, float *steerPos,
  ...
  dtStatus dtResult = _navMeshQuery->findStraightPath(
      startPos, endPos, path, pathSize, steerPath, steerPathFlags,
...
dtStatus PathGenerator::FindSmoothPath(float const *startPos,
                                       ...
                                       uint32 polyPathSize, float *smoothPath,
                                       int *smoothPathSize,
  ...
  *smoothPathSize = 0;
  ...
  while (npolys && nsmoothPath < maxSmoothPathSize) {
    ...
    if (dtStatusFailed(_navMeshQuery->moveAlongSurface(
            polys[0], iterPos, moveTgt, &_filter, result, visited,
    ...
    if (dtStatusFailed(
      ...
      LOG_DEBUG("maps",
                "PathGenerator::FindSmoothPath: Cannot find height at position "
                "X: %f Y: %f Z: %f for %u",
                result[2], result[0], result[1], _source->GetGUIDLow());
    result[1] += 0.5f;
...
bool PathGenerator::IsWalkableClimb(float x, float y, float z, float destX,
                                    float destY, float destZ) const {
  return IsWalkableClimb(x, y, z, destX, destY, destZ,
 ...
 */
float PathGenerator::GetRequiredHeightToClimb(float x, float y, float z,
                                              float destX, float destY,
                                              float destZ, float sourceHeight) {
...
bool PathGenerator::InRange(G3D::Vector3 const &p1, G3D::Vector3 const &p2,
                            float r, float h) const {
  G3D::Vector3 d = p1 - p2;
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const &target,
                                         float dist) {
  if (GetPathType() == PATHFIND_BLANK || _pathPoints.size() < 2) {
    LOG_ERROR("server", "PathGenerator::ReducePathLengthByDist called before "
                        "path was successfully built");
    return;
  }
  ...
  while (1) {
    ...
    // walkable
    _source->GetHitSpherePointFor({_pathPoints[i - 1].x, _pathPoints[i - 1].y,
                                   _pathPoints[i - 1].z + collisionHeight},
                                  x, y, z);
    if (!_source->GetMap()->isInLineOfSight(
            x, y, z, _pathPoints[i - 1].x, _pathPoints[i - 1].y,
            _pathPoints[i - 1].z + collisionHeight, _source->GetPhaseMask(),
            LINEOFSIGHT_ALL_CHECKS) ||
        (canCheckSlope &&
         !IsSwimmableSegment(_source->GetPositionX(), _source->GetPositionY(),
                             _source->GetPositionZ(), _pathPoints[i - 1].x,
                             _pathPoints[i - 1].y, _pathPoints[i - 1].z) &&
         !IsWalkableClimb(_source->GetPositionX(), _source->GetPositionY(),
                          _source->GetPositionZ(), _pathPoints[i - 1].x,
                          _pathPoints[i - 1].y, _pathPoints[i - 1].z))) {
      // whenver we find a point that is not valid anymore, simply use last
      // valid path
 ...
 * @return false
 */
bool PathGenerator::IsSwimmableSegment(float const *v1, float const *v2,
                                       bool checkSwim) const {
  return IsSwimmableSegment(v1[2], v1[0], v1[1], v2[2], v2[0], v2[1],
                            checkSwim);
}
 ...
 */
bool PathGenerator::IsSwimmableSegment(float x, float y, float z, float destX,
                                       float destY, float destZ,
                                       bool checkSwim) const {
...
bool PathGenerator::IsWaterPath(Movement::PointsArray _pathPoints) const {
  ...
  for (uint32 i = 0; i < _pathPoints.size(); ++i) {
    NavTerrain terrain =
        GetNavTerrain(_pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z);
    // One of the points is not in the water
