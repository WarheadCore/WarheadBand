...
ChaseRange::ChaseRange(float _minRange, float _minTolerance,
                       float _maxTolerance, float _maxRange)
...
void MotionMaster::InitDefault() {
  ...
  if (_owner->GetTypeId() == TYPEID_UNIT && _owner->IsAlive()) {
    MovementGenerator *movement =
        FactorySelector::selectMovementGenerator(_owner->ToCreature());
...
void MotionMaster::UpdateMotion(uint32 diff) {
  ...
  if (_owner->HasUnitState(
          UNIT_STATE_ROOT |
          UNIT_STATE_STUNNED)) // what about UNIT_STATE_DISTRACTED? Why is this
                               // not included?
  {
    // pussywizard: the same as at the bottom of this function
    if (_owner->GetTypeId() == TYPEID_PLAYER)
      _owner->UpdateUnderwaterState(_owner->GetMap(), _owner->GetPositionX(),
                                    _owner->GetPositionY(),
                                    _owner->GetPositionZ());
    else
...
void MotionMaster::MoveRandom(float wanderDistance) {
  ...
  if (_owner->GetTypeId() == TYPEID_UNIT) {
    ...
    LOG_DEBUG("server", "Creature (GUID: %u) start moving random",
              _owner->GetGUIDLow());
#endif
...
void MotionMaster::MoveTargetedHome() {
  ...
  if (_owner->GetTypeId() == TYPEID_UNIT &&
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Creature (Entry: %u GUID: %u) targeted home",
    ...
    if (target) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "Following %s (GUID: %u)",
                target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
                target->GetTypeId() == TYPEID_PLAYER
                    ? target->GetGUIDLow()
                    : ((Creature *)target)->GetDBTableGUIDLow());
#endif
      Mutate(new FollowMovementGenerator<Creature>(target, PET_FOLLOW_DIST,
                                                   _owner->GetFollowAngle()),
             MOTION_SLOT_ACTIVE);
    }
  } else {
    LOG_ERROR("server", "Player (GUID: %u) attempt targeted home",
              _owner->GetGUIDLow());
  }
}
...
void MotionMaster::MoveConfused() {
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Player (GUID: %u) move confused",
              _owner->GetGUIDLow());
#endif
    Mutate(new ConfusedMovementGenerator<Player>(), MOTION_SLOT_CONTROLLED);
  } else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Creature (Entry: %u GUID: %u) move confused",
              _owner->GetEntry(), _owner->GetGUIDLow());
#endif
...
void MotionMaster::MoveChase(Unit *target, std::optional<ChaseRange> dist,
                             std::optional<ChaseAngle> angle) {
  // Xinef: do not allow to move with UNIT_FLAG_DISABLE_MOVE
  // ignore movement request if target not exist
  if (!target || target == _owner ||
      _owner->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE))
    return;
...
void MotionMaster::MoveCircleTarget(Unit *target) {
  ...
  Movement::MoveSplineInit init(_owner);
  init.MoveTo(point->m_positionX, point->m_positionY, point->m_positionZ,
              false);
  init.SetWalk(true);
  init.SetFacing(target);
  init.Launch();
...
void MotionMaster::MoveFollow(Unit *target, float dist, float angle,
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Player (GUID: %u) follow to %s (GUID: %u)",
              _owner->GetGUIDLow(),
              target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
              target->GetTypeId() == TYPEID_PLAYER
                  ? target->GetGUIDLow()
...
void MotionMaster::MovePoint(uint32 id, float x, float y, float z,
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
    ...
    LOG_DEBUG("server",
              ...
              _owner->GetGUIDLow(), id, x, y, z);
#endif
    Mutate(new PointMovementGenerator<Player>(id, x, y, z, 0.0f, orientation,
                                              nullptr, generatePath,
                                              forceDestination),
           slot);
  } else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "Creature (Entry: %u GUID: %u) targeted point (ID: %u X: %f Y: "
              "%f Z: %f)",
              _owner->GetEntry(), _owner->GetGUIDLow(), id, x, y, z);
#endif
    Mutate(new PointMovementGenerator<Creature>(id, x, y, z, 0.0f, orientation,
                                                nullptr, generatePath,
                                                forceDestination),
           slot);
  }
...
void MotionMaster::MoveLand(uint32 id, float x, float y, float z,
                            float speed /* = 0.0f*/) {
...
void MotionMaster::MoveJump(float x, float y, float z, float speedXY,
                            float speedZ, uint32 id, Unit const *target) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Unit (GUID: %u) jump to point (X: %f Y: %f Z: %f)",
...
void MotionMaster::MoveFall(uint32 id /*=0*/, bool addFlagForNPC) {
  ...
  float tz =
      _owner->GetMapHeight(_owner->GetPositionX(), _owner->GetPositionY(),
                           _owner->GetPositionZ(), true, MAX_FALL_DISTANCE);
  if (tz <= INVALID_HEIGHT) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "MotionMaster::MoveFall: unable retrive a proper height at map "
              "%u (x: %f, y: %f, z: %f).",
              _owner->GetMap()->GetId(), _owner->GetPositionX(),
              _owner->GetPositionX(),
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
    ...
    _owner->ToPlayer()->SetFallInformation(time(nullptr),
                                           _owner->GetPositionZ());
  ...
  Movement::MoveSplineInit init(_owner);
  init.MoveTo(_owner->GetPositionX(), _owner->GetPositionY(),
...
void MotionMaster::MoveCharge(float x, float y, float z, float speed, uint32 id,
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Player (GUID: %u) charge point (X: %f Y: %f Z: %f)",
              _owner->GetGUIDLow(), x, y, z);
#endif
    Mutate(new PointMovementGenerator<Player>(id, x, y, z, speed, orientation,
...
#endif
    Mutate(new PointMovementGenerator<Creature>(id, x, y, z, speed, orientation,
                                                path, generatePath,
                                                generatePath),
           MOTION_SLOT_CONTROLLED);
  }
...
void MotionMaster::MoveSeekAssistance(float x, float y, float z) {
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
    LOG_ERROR("server", "Player (GUID: %u) attempt to seek assistance",
              _owner->GetGUIDLow());
  } else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG(
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "Creature (Entry: %u GUID: %u) is distracted after assistance "
              "call (Time: %u)",
...
void MotionMaster::MoveFleeing(Unit *enemy, uint32 time) {
  ...
  if (_owner->GetTypeId() == TYPEID_PLAYER) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Player (GUID: %u) flee from %s (GUID: %u)",
              _owner->GetGUIDLow(),
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "%s taxi to (Path %u node %u)",
...
void MotionMaster::MoveDistract(uint32 timer) {
  ...
  {
  #if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "Player (GUID: %u) distracted (timer: %u)",
  _owner->GetGUIDLow(), timer); #endif
  }
  else
  {
  #if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
...
void MotionMaster::DelayedDelete(_Ty curr) {
  LOG_FATAL("server",
            ...
            _owner->GetEntry(), curr->GetMovementGeneratorType());
  if (isStatic(curr))
    return;
  if (!_expList)
