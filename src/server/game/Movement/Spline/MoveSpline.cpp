...
void MoveSpline::computeFallElevation(float &el) const {
  float z_now = spline.getPoint(spline.first(), false).z -
...
struct FallInitializer {
  ...
  inline int32 operator()(Spline<int32> &s, int32 i) {
    return Movement::computeFallTime(
               start_elevation - s.getPoint(i + 1, false).z, false) *
           1000.f;
  }
};
...
void MoveSpline::init_spline(const MoveSplineInitArgs &args) {
  ...
  if (args.flags.cyclic) {
    ...
    spline.init_spline(&args.path[0], args.path.size(),
  ...
  // TODO: what to do in such cases? problem is in input data (all points are at
  // same coords)
  if (spline.length() < minimal_duration) {
    // LOG_ERROR("server", "MoveSpline::init_spline: zero length spline, wrong
    // input data?"); // ZOMG! temp comment to avoid console spam from transports
...
#define CHECK(exp)                                                             \
  if (!(exp)) {                                                                \
    LOG_ERROR("server",                                                        \
              "MoveSplineInitArgs::Validate: expression '%s' failed for "      \
              "GUID: %u Entry: %u",                                            \
              #exp,                                                            \
              unit->GetTypeId() == TYPEID_PLAYER                               \
                  ? unit->GetGUIDLow()                                         \
                  : unit->ToCreature()->GetDBTableGUIDLow(),                   \
              unit->GetEntry());                                               \
    return false;                                                              \
  }
  CHECK(path.size() > 1);
