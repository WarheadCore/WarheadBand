...
inline Player* Map::_GetScriptPlayerSourceOrTarget(Object* source, Object* target, const ScriptInfo* scriptInfo) const
{
    ...
    if (!source && !target)
        LOG_ERROR("server", "{} source and target objects are nullptr.", scriptInfo->GetDebugInfo());
    else
    {
...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
    ...
    else
    {
        unit = obj->ToUnit();
        if (!unit)
            LOG_ERROR("server", "{} {} object could not be casted to unit.",
...
inline WorldObject* Map::_GetScriptWorldObject(Object* obj, bool isSource, const ScriptInfo* scriptInfo) const
{
    ...
    if (!obj)
        LOG_ERROR("server", "{} {} object is nullptr.",
                       scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
    else
    {
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, const ScriptInfo* scriptInfo) const
{
    ...
    switch (scriptInfo->command)
    {
        ...
        case SCRIPT_COMMAND_CLOSE_DOOR:
            break;
        default:
            LOG_ERROR("server", "{} unknown command for _ScriptProcessDoor.", scriptInfo->GetDebugInfo());
            return;
    }
    if (!guid)
        LOG_ERROR("server", "{} door guid is not specified.", scriptInfo->GetDebugInfo());
    else if (!source)
        LOG_ERROR("server", "{} source object is nullptr.", scriptInfo->GetDebugInfo());
    else if (!source->isType(TYPEMASK_UNIT))
        LOG_ERROR("server", "{} source object is not unit ({}), skipping.", scriptInfo->GetDebugInfo(), source->GetGUID().ToString());
    else
    {
        WorldObject* wSource = dynamic_cast <WorldObject*> (source);
        if (!wSource)
            LOG_ERROR("server", "{} source object could not be casted to world object ({}), skipping.", scriptInfo->GetDebugInfo(), source->GetGUID().ToString());
        else
        {
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= GameTime::GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                {
                    LOG_ERROR("server", "{} invalid chat type ({}) specified, skipping.", step.script->GetDebugInfo(), step.script->Talk.ChatType);
                    break;
                }
                            ...
                            case CHAT_MSG_RAID_BOSS_WHISPER:
                                {
                                    ...
                                    if (!targetGUID || !targetGUID.IsPlayer())
                                        LOG_ERROR("server", "{} attempt to whisper to non-player unit, skipping.", step.script->GetDebugInfo());
                                    else
                ...
                else
                {
                    ...
                    if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                    {
                        ...
                        switch (step.script->Talk.ChatType)
                        {
                            ...
                            case CHAT_MSG_RAID_BOSS_WHISPER:
                                if (!targetGUID || !targetGUID.IsPlayer())
                                    LOG_ERROR("server", "{} attempt to raidbosswhisper to non-player unit, skipping.", step.script->GetDebugInfo());
                                else
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
                {
                    if (!source)
                    {
                        LOG_ERROR("server", "{} source object is nullptr.", step.script->GetDebugInfo());
                        break;
                    }
                    if (!target)
                    {
                        LOG_ERROR("server", "{} target object is nullptr.", step.script->GetDebugInfo());
                        break;
                    }
                    ...
                    if (player)
                    {
                        ...
                        {
                            LOG_ERROR("server", "{} source is not unit, gameobject or player ({}), skipping.", step.script->GetDebugInfo(), source->GetGUID().ToString());
                            break;
                        }
                    ...
                    else
                    {
                        ...
                        if (player)
                        {
                            ...
                            {
                                LOG_ERROR("server", "{} target is not unit, gameobject or player ({}), skipping.", step.script->GetDebugInfo(), target->GetGUID().ToString());
                                break;
                            }
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
                ...
                {
                    LOG_ERROR("server", "{} gameobject guid (datalong) is not specified.", step.script->GetDebugInfo());
                    break;
                }
                ...
                if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
                {
                    ...
                    if (!pGO)
                    {
                        LOG_ERROR("server", "{} gameobject was not found (guid: {}).", step.script->GetDebugInfo(), step.script->RespawnGameobject.GOGuid);
                        break;
                    }
                    ...
                    {
                        LOG_ERROR("server", "{} can not be used with gameobject of type {} (guid: {}).",
            ...
            case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
                {
                    ...
                    if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
                    {
                        if (!step.script->TempSummonCreature.CreatureEntry)
                            LOG_ERROR("server", "{} creature entry (datalong) is not specified.", step.script->GetDebugInfo());
                        else
                        {
                            ...
                            if (!pSummoner->SummonCreature(entry, x, y, z, o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, step.script->TempSummonCreature.DespawnDelay))
                                LOG_ERROR("server", "{} creature was not spawned (entry: {}).", step.script->GetDebugInfo(), step.script->TempSummonCreature.CreatureEntry);
                        }
                    }
                    break;
                }
            ...
            case SCRIPT_COMMAND_ACTIVATE_OBJECT:
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    ...
                    {
                        LOG_ERROR("server", "{} target object is not gameobject ({}), skipping.", step.script->GetDebugInfo(), target->GetGUID().ToString());
                        break;
                    }
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
                {
                    ...
                    if (!source && !target)
                    {
                        LOG_ERROR("server", "{} source and target objects are nullptr.", step.script->GetDebugInfo());
                        break;
                    }
                    ...
                    {
                        LOG_ERROR("server", "{} no source unit found for spell {}", step.script->GetDebugInfo(), step.script->CastSpell.SpellID);
                        break;
                    }

                    if (!uTarget || !uTarget->isType(TYPEMASK_UNIT))
                    {
                        LOG_ERROR("server", "{} no target unit found for spell {}", step.script->GetDebugInfo(), step.script->CastSpell.SpellID);
                        break;
                    }
            ...
            case SCRIPT_COMMAND_LOAD_PATH:
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    if (!sWaypointMgr->GetPath(step.script->LoadPath.PathID))
                        LOG_ERROR("server", "{} source object has an invalid path ({}), skipping.", step.script->GetDebugInfo(), step.script->LoadPath.PathID);
                    else
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
                {
                    ...
                    {
                        LOG_ERROR("server", "{} creature entry is not specified, skipping.", step.script->GetDebugInfo());
                        break;
                    }
                    if (!step.script->CallScript.ScriptID)
                    {
                        LOG_ERROR("server", "{} script id is not specified, skipping.", step.script->GetDebugInfo());
                        break;
                    }
                    ...
                    if (!cTarget)
                    {
                        LOG_ERROR("server", "{} target was not found (entry: {})", step.script->GetDebugInfo(), step.script->CallScript.CreatureEntry);
                        break;
                    }
                    ...
                    if (!datamap)
                    {
                        LOG_ERROR("server", "{} unknown scriptmap ({}) specified, skipping.", step.script->GetDebugInfo(), step.script->CallScript.ScriptType);
                        break;
                    }
            ...
            case SCRIPT_COMMAND_KILL:
                ...
                {
                    if (cSource->isDead())
                        LOG_ERROR("server", "{} creature is already dead ({})", step.script->GetDebugInfo(), cSource->GetGUID().ToString());
                    else
                    {
            ...
            default:
                LOG_ERROR("server", "Unknown script command {}.", step.script->GetDebugInfo());
                break;
        }
