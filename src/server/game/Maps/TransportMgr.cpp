...
void TransportMgr::LoadTransportTemplates() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 transport templates. DB table "
  ...
  do {
    ...
    // paths are generated per template, saves us from generating it again in
    // case of instanced transports
  ...
  LOG_INFO("server", ">> Loaded %u transport templates in %u ms", count,
...
void TransportMgr::GeneratePath(GameObjectTemplate const *goInfo,
    ...
    if (!mapChange) {
      ...
      if (i != path.size() - 1 &&
        ...
        k.InitialOrientation =
            Position::NormalizeOrientation(atan2(h.y, h.x) + M_PI);
  ...
  if (transport->mapsUsed.size() > 1) {
    for (std::set<uint32>::const_iterator itr = transport->mapsUsed.begin();
         itr != transport->mapsUsed.end(); ++itr)
  ...
  for (size_t i = 0; i < keyFrames.size(); ++i) {
    ...
    if (total_dist < 2 * accel_dist) // won't reach full speed
    {
          ...
          keyFrames[i].DistUntilStop) // is still accelerating
      {
        ...
        float segment_time =
            2.0f *
            sqrt((keyFrames[i].DistUntilStop + keyFrames[i].DistSinceStop) /
                 accel);
...
void TransportMgr::AddPathNodeToTransport(uint32 transportEntry, uint32 timeSeg,
...
MotionTransport *TransportMgr::CreateTransport(uint32 entry,
  ...
  if (map) {
    // SetZoneScript() is called after adding to map, so fetch the script using
    // map
    if (map->IsDungeon())
  ...
  if (!tInfo) {
    LOG_ERROR("server",
              "Transport %u will not be loaded, `transport_template` missing",
              entry);
    return nullptr;
  }
  ...
  uint32 guidLow =
      guid ? guid : sObjectMgr->GenerateLowGuid(HIGHGUID_MO_TRANSPORT);
  ...
  if (MapEntry const *mapEntry = sMapStore.LookupEntry(mapId)) {
    if (mapEntry->Instanceable() != tInfo->inInstance) {
      LOG_ERROR("server",
                ...
                entry, trans->GetName().c_str(), mapId);
      delete trans;
      return nullptr;
    }
  }
  ...
  // xinef: transports are active so passengers can be relocated (grids must be
  // loaded)
...
void TransportMgr::SpawnContinentTransports() {
  ...
  if (sWorld->getBoolConfig(CONFIG_ENABLE_CONTINENT_TRANSPORT)) {
    ...
    LOG_INFO("server", ">> Spawned %u continent motion transports in %u ms",
    ...
    if (sWorld->getBoolConfig(CONFIG_ENABLE_CONTINENT_TRANSPORT_PRELOADING)) {
      ...
      LOG_INFO("server",
               ">> Preloaded grids for %u continent static transports in %u ms",
               count, GetMSTimeDiffToNow(oldMSTime));
    }
  }
}
...
void TransportMgr::CreateInstanceTransports(Map *map) {
  ...
  for (std::set<uint32>::const_iterator itr = mapTransports->second.begin();
       itr != mapTransports->second.end(); ++itr)
...
bool TransportAnimation::GetAnimNode(uint32 time,
  ...
  for (TransportPathContainer::const_reverse_iterator itr = Path.rbegin();
    ...
    if (time >= itr->first) {
      ...
      next = itr->second;
      percPos =
          float(time - curr->TimeSeg) / float(next->TimeSeg - curr->TimeSeg);
      return true;
    }
...
void TransportAnimation::GetAnimRotation(uint32 time, G3D::Quat &curr,
                                         G3D::Quat &next,
                                         float &percRot) const {
  if (Rotations.empty()) {
  ...
  for (TransportPathRotationContainer::const_reverse_iterator itr =
           Rotations.rbegin();
       itr != Rotations.rend(); ++itr)
    if (time >= itr->first) {
      uint32 currSeg = itr->second->TimeSeg, nextSeg;
      curr = G3D::Quat(itr->second->X, itr->second->Y, itr->second->Z,
                       itr->second->W);
      if (itr != Rotations.rbegin()) {
        --itr;
        next = G3D::Quat(itr->second->X, itr->second->Y, itr->second->Z,
                         itr->second->W);
        nextSeg = itr->second->TimeSeg;
      } else {
        next = G3D::Quat(
            Rotations.begin()->second->X, Rotations.begin()->second->Y,
            Rotations.begin()->second->Z, Rotations.begin()->second->W);
