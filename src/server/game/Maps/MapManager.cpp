...
bool MapManager::CanPlayerEnter(uint32 mapid, Player *player, bool loginCheck) {
  ...
  // Get the highest available difficulty if current setting is higher than the
  // instance allows
  ...
  if (entry->IsRaid()) {
    ...
    if ((!group || !group->isRaidGroup()) &&
      ...
      player->GetSession()->SendAreaTriggerMessage(
          player->GetSession()->GetAcoreString(LANG_INSTANCE_RAID_GROUP_ONLY),
          mapName);
  ...
  if (group)
    if (group->isLFGGroup())
      if (!sLFGMgr->inLfgDungeonMap(group->GetGUID(), mapid,
                                    targetDifficulty)) {
  ...
  if (!player->IsAlive()) {
    if (Corpse *corpse = player->GetCorpse()) {
      // let enter in ghost mode in instance that connected to inner instance
      // with corpse
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("maps",
                  "MAP: Player '%s' does not have a corpse in instance '%s' "
                  "and cannot enter.",
                  player->GetName().c_str(), mapName);
#endif
        return false;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("maps",
                "Map::CanPlayerEnter - player '%s' is dead but does not have a "
                "corpse!",
...
void MapManager::Update(uint32 diff) {
  ...
  // if (mapUpdateStep == 0)
  {
    if (m_updater.activated())
      ...
      lfgDiffTracker.Update(10000 +
                            totalTime); // +10k to mark it was NOT multithreaded
    }
  }
  ...
  for (; iter != i_maps.end(); ++iter) {
    ...
    if (m_updater.activated())
      m_updater.schedule_update(
          *iter->second, uint32(full ? i_timer[mapUpdateStep].GetCurrent() : 0),
          diff);
    else
      iter->second->Update(
...
void MapManager::GetNumInstances(uint32 &dungeons, uint32 &battlegrounds,
                                 uint32 &arenas) {
...
void MapManager::GetNumPlayersInInstances(uint32 &dungeons,
                                          uint32 &battlegrounds, uint32 &arenas,
                                          uint32 &spectators) {
  for (MapMapType::iterator itr = i_maps.begin(); itr != i_maps.end(); ++itr) {
    Map *map = itr->second;
    if (!map->Instanceable())
