...
Map::~Map() {
  ...
  MMAP::MMapFactory::createOrGetMMapManager()->unloadMapInstance(GetId(),
                                                                 i_InstanceId);
}
  ...
  if (!pf)
    ...
    if (fread(&header, sizeof(header), 1, pf) == 1) {
      if (header.mapMagic != MapMagic.asUInt ||
          header.versionMagic != MapVersionMagic.asUInt)
        LOG_ERROR("server",
                  "Map file '%s' is from an incompatible clientversion. Please "
...
bool Map::ExistVMap(uint32 mapid, int gx, int gy) {
  if (VMAP::IVMapManager *vmgr = VMAP::VMapFactory::createOrGetVMapManager()) {
    if (vmgr->isMapLoadingEnabled()) {
      bool exists = vmgr->existsMap((sWorld->GetDataPath() + "vmaps").c_str(),
                                    mapid, gx, gy);
      if (!exists) {
        std::string name = vmgr->getDirFileName(mapid, gx, gy);
        LOG_ERROR(
            "server",
            "VMap file '%s' is missing or points to wrong version of vmap "
            "file. Redo vmaps with latest version of vmap_assembler.exe.",
...
void Map::LoadMMap(int gx, int gy) {
  ...
  case MMAP::MMAP_LOAD_RESULT_OK:
    ...
    LOG_DEBUG("server",
              "MMAP loaded name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "Could not load MMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: "
              "x:%d, y:%d)",
              GetMapName(), GetId(), gx, gy, gx, gy);
#endif
    break;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "Ignored MMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)",
              GetMapName(), GetId(), gx, gy, gx, gy);
#endif
    break;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "Could not load VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: "
              "x:%d, y:%d)",
              GetMapName(), GetId(), gx, gy, gx, gy);
#endif
    break;
...
void Map::LoadMap(int gx, int gy, bool reload) {
  ...
  // map already load, delete it before reloading (Is it necessary? Do we really
  // need the ability the reload maps during runtime?)
  if (GridMaps[gx][gy]) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Unloading previously loaded map %u before reloading.",
              GetId());
#endif
...
template <> void Map::SwitchGridContainers(Creature *obj, bool on) {
  ...
  CellCoord p =
      Warhead::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
  if (!p.IsCoordValid()) {
    LOG_ERROR("server",
              "Map::SwitchGridContainers: Object " UI64FMTD
              " has invalid coordinates X:%f Y:%f grid cell [%u:%u]",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Switch object " UI64FMTD " from grid[%u, %u] %u",
  ...
  obj->RemoveFromGrid(); // This step is not really necessary but we want to do
                         // ASSERT in remove/add
...
template <> void Map::SwitchGridContainers(GameObject *obj, bool on) {
  ...
  // TC_LOG_DEBUG(LOG_FILTER_MAPS, "Switch object " UI64FMTD " from grid[%u, %u]
  // %u", obj->GetGUID(), cell.data.Part.grid_x, cell.data.Part.grid_y, on);
  NGridType *ngrid = getNGrid(cell.GridX(), cell.GridY());
  ASSERT(ngrid != nullptr);
...
template <class T> void Map::DeleteFromWorld(T *obj) {
  // Note: In case resurrectable corpse and pet its removed from global lists in
  // own destructor
  delete obj;
}
...
template <> void Map::DeleteFromWorld(Player *player) {
  ...
  sObjectAccessor->RemoveUpdateObject(
      player); // TODO: I do not know why we need this, it should be removed in
               // ~Object anyway
  delete player;
}
...
void Map::EnsureGridCreated_i(const GridCoord &p) {
  if (!getNGrid(p.x_coord, p.y_coord)) {
    ...
    NGridType *ngt = new NGridType(p.x_coord * MAX_NUMBER_OF_GRIDS + p.y_coord,
                                   p.x_coord, p.y_coord);
...
bool Map::EnsureGridLoaded(const Cell &cell) {
  ...
  if (!isGridObjectDataLoaded(cell.GridX(), cell.GridY())) {
    ...
    //{
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("maps", "Loading grid[%u, %u] for map %u instance %u",
              cell.GridX(), cell.GridY(), GetId(), i_InstanceId);
#endif
    ...
    sObjectAccessor->AddCorpsesToGrid(
        ...
        grid->GetGridType(cell.CellX(), cell.CellY()), this);
    Balance();
    return true;
    //}
  }
...
bool Map::AddPlayerToMap(Player *player) {
  CellCoord cellCoord =
      Warhead::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
  if (!cellCoord.IsCoordValid()) {
    LOG_ERROR("server",
              "Map::Add: Player (GUID: %u) has invalid coordinates X:%f Y:%f "
              "grid cell [%u:%u]",
              player->GetGUIDLow(), player->GetPositionX(),
              player->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
    return false;
  }
...
template <class T> bool Map::AddToMap(T *obj, bool checkTransport) {
  ...
  CellCoord cellCoord =
      Warhead::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
  // It will create many problems (including crashes) if an object is not added
  // to grid after creation The correct way to fix it is to make AddToMap return
  // false and delete the object if it is not added to grid But now AddToMap is
  // used in too many places, I will just see how many ASSERT failures it will
  // cause
  ASSERT(cellCoord.IsCoordValid());
  if (!cellCoord.IsCoordValid()) {
    LOG_ERROR("server",
              "Map::Add: Object " UI64FMTD
              " has invalid coordinates X:%f Y:%f grid cell [%u:%u]",
              obj->GetGUID(), obj->GetPositionX(), obj->GetPositionY(),
  ...
  if (checkTransport)
    if (!(obj->GetTypeId() == TYPEID_GAMEOBJECT &&
      ...
      if (Transport *transport =
              GetTransportForPos(obj->GetPhaseMask(), obj->GetPositionX(),
                                 obj->GetPositionY(), obj->GetPositionZ(), obj))
  ...
  // Creature::AIM_Initialize Initialize vehicle, this is done only for summoned
  // npcs, DB creatures are handled by grid loaders
...
void Map::VisitNearbyCellsOfPlayer(
    Player *player,
    TypeContainerVisitor<Warhead::ObjectUpdater, GridTypeMapContainer>
        &gridVisitor,
    TypeContainerVisitor<Warhead::ObjectUpdater, WorldTypeMapContainer>
        &worldVisitor,
    TypeContainerVisitor<Warhead::ObjectUpdater, GridTypeMapContainer>
        &largeGridVisitor,
    TypeContainerVisitor<Warhead::ObjectUpdater, WorldTypeMapContainer>
  ...
  VisitNearbyCellsOf(player, gridVisitor, worldVisitor, largeGridVisitor,
                     largeWorldVisitor);

  // check maximum visibility distance for large creatures
  CellArea area = Cell::CalculateCellArea(
      player->GetPositionX(), player->GetPositionY(), MAX_VISIBILITY_DISTANCE);
...
void Map::VisitNearbyCellsOf(
  ...
  CellArea area = Cell::CalculateCellArea(
      obj->GetPositionX(), obj->GetPositionY(), obj->GetGridActivationRange());
...
void Map::Update(const uint32 t_diff, const uint32 s_diff, bool /*thread*/) {
  ...
  for (m_activeNonPlayersIter = m_activeNonPlayers.begin();
    ...
    VisitNearbyCellsOf(obj, grid_object_update, world_object_update,
                       grid_large_object_update, world_large_object_update);
  }
  ...
  for (m_mapRefIter = m_mapRefManager.begin();
    ...
    VisitNearbyCellsOfPlayer(player, grid_object_update, world_object_update,
    ...
    if (WorldObject *viewPoint = player->GetViewpoint()) {
      if (Creature *viewCreature = viewPoint->ToCreature()) {
        VisitNearbyCellsOf(viewCreature, grid_object_update,
                           world_object_update, grid_large_object_update,
                           world_large_object_update);
    ...
    // handle updates for creatures in combat with player and are more than X
    // yards away
    if (player->IsInCombat()) {
      ...
      for (std::vector<Creature *>::const_iterator itr = updateList.begin();
           itr != updateList.end(); ++itr)
  ...
  for (_transportsUpdateIter = _transports.begin();
       ...
       _transports
           .end();) // pussywizard: transports updated after VisitNearbyCellsOf,
                    // grids around are loaded, everything ok
  {
...
void Map::HandleDelayedVisibility() {
  ...
  for (std::unordered_set<Unit *>::iterator itr =
       ...
       itr != i_objectsForDelayedVisibility.end(); ++itr)
...
void Map::RemovePlayerFromMap(Player *player, bool remove) {
  player->getHostileRefManager()
      .deleteReferences(); // pussywizard: multithreading crashfix
  ...
  if (!inWorld) // pussywizard: if was in world, RemoveFromWorld() called
    ...
    player->DestroyForNearbyPlayers(); // pussywizard: previous
...
void Map::GameObjectRelocation(GameObject *go, float x, float y, float z,
                               float o) {
...
void Map::DynamicObjectRelocation(DynamicObject *dynObj, float x, float y,
                                  float z, float o) {
...
void Map::MoveAllGameObjectsInMoveList() {
  for (std::vector<GameObject *>::iterator itr = _gameObjectsToMove.begin();
       itr != _gameObjectsToMove.end(); ++itr) {
    GameObject *go = *itr;
    if (go->FindMap() != this)
      continue;
...
void Map::MoveAllDynamicObjectsInMoveList() {
  for (std::vector<DynamicObject *>::iterator itr =
       ...
       itr != _dynamicObjectsToMove.end(); ++itr) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Unloading grid[%u, %u] for map %u finished", x, y,
            GetId());
#endif
  return true;
}
  ...
  if (HavePlayers()) {
    for (MapRefManager::iterator itr = m_mapRefManager.begin();
      ...
      if (!player->IsBeingTeleportedFar()) {
        ...
        LOG_ERROR("server",
                  "Map::UnloadAll: player %s is still in map %u during unload, "
...
float GridMap::getMinHeight(float x, float y) const {
  ...
  static float const boundGridCoords[] = {
      0.0f,        0.0f, 0.0f,        -266.66666f, 0.0f,        -533.33331f,
      -266.66666f, 0.0f, -266.66666f, -266.66666f, -266.66666f, -533.33331f,
...
inline ZLiquidStatus GridMap::getLiquidStatus(float x, float y, float z,
  ...
  if (_liquidEntry) {
    if (LiquidTypeEntry const *liquidEntry =
            sLiquidTypeStore.LookupEntry(_liquidEntry[idx])) {
  ...
  // Get water level
  float liquid_level =
      _liquidMap ? _liquidMap[lx_int * _liquidWidth + ly_int] : _liquidLevel;
...
float Map::GetWaterOrGroundLevel(uint32 phasemask, float x, float y, float z,
                                 float *ground /*= nullptr*/,
                                 bool /*swim = false*/,
                                 float collisionHeight) const {
  if (const_cast<Map *>(this)->GetGrid(x, y)) {
    // we need ground level (including grid height version) for proper return
    // water level in point
    float ground_z =
    ...
    ZLiquidStatus res = getLiquidStatus(x, y, ground_z, MAP_ALL_LIQUIDS,
                                        &liquid_status, collisionHeight);
    switch (res) {
...
Transport *Map::GetTransportForPos(uint32 phase, float x, float y, float z,
                                   WorldObject *worldobject) {
  G3D::Vector3 v(x, y, z + 2.0f);
  G3D::Ray r(v, G3D::Vector3(0, 0, -1));
  for (TransportsContainer::const_iterator itr = _transports.begin();
       itr != _transports.end(); ++itr)
    if ((*itr)->IsInWorld() &&
        (*itr)->GetExactDistSq(x, y, z) < 75.0f * 75.0f && (*itr)->m_model) {
      float dist = 30.0f;
...
float Map::GetHeight(float x, float y, float z, bool checkVMap /*= true*/,
...
bool Map::IsOutdoors(float x, float y, float z) const {
  ...
  if (wmoEntry) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Got WMOAreaTableEntry! flag %u, areaid %u",
...
bool Map::GetAreaInfo(float x, float y, float z, uint32 &flags, int32 &adtId,
  ...
  if (vmgr->getAreaInfo(GetId(), x, y, vmap_z, flags, adtId, rootId, groupId)) {
    ...
    if (GridMap *gmap = const_cast<Map *>(this)->GetGrid(x, y)) {
      ...
      // z + 2.0f condition taken from GetHeight(), not sure if it's such a
      // great choice...
...
void Map::GetZoneAndAreaId(uint32 &zoneid, uint32 &areaid, float x, float y,
                           float z) const {
...
ZLiquidStatus Map::getLiquidStatus(float x, float y, float z,
    ...
    if (liquid_level > ground_level && z > ground_level - 2) {
      ...
      if (data) {
        ...
        if (liquid_type && liquid_type < 21) {
          if (AreaTableEntry const *area =
                  sAreaTableStore.LookupEntry(GetAreaId(x, y, z))) {
  ...
  if (GridMap *gmap = const_cast<Map *>(this)->GetGrid(x, y)) {
    LiquidData map_data;
    ZLiquidStatus map_result = gmap->getLiquidStatus(
        x, y, z, ReqLiquidType, collisionHeight, &map_data);
...
bool Map::isInLineOfSight(float x1, float y1, float z1, float x2, float y2,
                          float z2, uint32 phasemask,
                          LineOfSightChecks checks) const {
  if ((checks & LINEOFSIGHT_CHECK_VMAP) &&
      !VMAP::VMapFactory::createOrGetVMapManager()->isInLineOfSight(
          GetId(), x1, y1, z1, x2, y2, z2))
    return false;

  if (sWorld->getBoolConfig(CONFIG_CHECK_GOBJECT_LOS) &&
      (checks & LINEOFSIGHT_CHECK_GOBJECT) &&
      !_dynamicTree.isInLineOfSight(x1, y1, z1, x2, y2, z2, phasemask))
    return false;
  return true;
}

bool Map::getObjectHitPos(uint32 phasemask, float x1, float y1, float z1,
                          float x2, float y2, float z2, float &rx, float &ry,
                          float &rz, float modifyDist) {
  G3D::Vector3 startPos(x1, y1, z1);
  G3D::Vector3 dstPos(x2, y2, z2);

  G3D::Vector3 resultPos;
  bool result = _dynamicTree.getObjectHitPos(phasemask, startPos, dstPos,
                                             resultPos, modifyDist);
...
float Map::GetHeight(uint32 phasemask, float x, float y, float z,
                     bool vmap /*=true*/,
                     float maxSearchDist /*= DEFAULT_HEIGHT_SEARCH*/) const {
  float h1, h2;
...
bool Map::IsInWater(float x, float y, float pZ, LiquidData *data) const {
  ...
  return getLiquidStatus(x, y, pZ, MAP_ALL_LIQUIDS, liquid_ptr) &
         (LIQUID_MAP_IN_WATER | LIQUID_MAP_UNDER_WATER);
}

bool Map::IsUnderWater(float x, float y, float z) const {
  return getLiquidStatus(x, y, z,
                         MAP_LIQUID_TYPE_WATER | MAP_LIQUID_TYPE_OCEAN) &
         LIQUID_MAP_UNDER_WATER;
}

bool Map::HasEnoughWater(WorldObject const *searcher, float x, float y,
                         float z) const {
  LiquidData liquidData;
  liquidData.level = INVALID_HEIGHT;
  ZLiquidStatus liquidStatus =
      getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS, &liquidData);
  return (liquidStatus & (LIQUID_MAP_IN_WATER | LIQUID_MAP_UNDER_WATER)) &&
         HasEnoughWater(searcher, liquidData);
}

bool Map::HasEnoughWater(WorldObject const *searcher,
                         LiquidData liquidData) const {
  ...
  return liquidData.level > INVALID_HEIGHT &&
         ...
         liquidData.level - liquidData.depth_level >= minHeightInWater;
}
...
void Map::UpdateObjectVisibility(WorldObject *obj, Cell cell,
  ...
  TypeContainerVisitor<Warhead::VisibleChangesNotifier, WorldTypeMapContainer>
...
void Map::UpdateObjectsVisibilityFor(Player *player, Cell cell,
  ...
  cell.SetNoCreate();
  TypeContainerVisitor<Warhead::VisibleNotifier, WorldTypeMapContainer>
      world_notifier(notifier);
  TypeContainerVisitor<Warhead::VisibleNotifier, GridTypeMapContainer>
      grid_notifier(notifier);
  cell.Visit(cellpair, world_notifier, *this, *player->m_seer,
             player->GetSightRange());
  cell.Visit(cellpair, grid_notifier, *this, *player->m_seer,
             player->GetSightRange());
...
void Map::SendInitSelf(Player *player) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Creating player data for himself %u",
            player->GetGUIDLow());
#endif
  ...
  // build other passengers at transport also (they always visible and marked as
  // visible and will not send at visibility update at add to map
  if (Transport *transport = player->GetTransport())
...
void Map::SendRemoveTransports(Player *player) {
  ...
  for (Player::ClientGUIDs::const_iterator it = player->m_clientGUIDs.begin();
...
void Map::AddObjectToRemoveList(WorldObject *obj) {
  ...
  // LOG_DEBUG("maps", "Object (GUID: %u TypeId: %u) added to removing list.",
  // obj->GetGUIDLow(), obj->GetTypeId());
}
...
void Map::RemoveAllObjectsInRemoveList() {
  ...
  while (!i_objectsToRemove.empty()) {
    ...
    case TYPEID_GAMEOBJECT:
      ...
      break;
    case TYPEID_UNIT:
      // in case triggered sequence some spell can continue casting after prev
      // CleanupsBeforeDelete call make sure that like sources auras/etc removed
      // before destructor start
      obj->ToCreature()->CleanupsBeforeDelete();
      RemoveFromMap(obj->ToCreature(), true);
      break;
    default:
      LOG_ERROR("server",
                "Non-grid object (TypeId: %u) is in grid object remove list, "
                "ignored.",
                obj->GetTypeId());
      break;
    }
...
template <class T> void Map::AddToActive(T *obj) { AddToActiveHelper(obj); }

template <> void Map::AddToActive(Creature *c) { AddToActiveHelper(c); }

template <> void Map::AddToActive(DynamicObject *d) { AddToActiveHelper(d); }

template <> void Map::AddToActive(GameObject *d) { AddToActiveHelper(d); }
...
InstanceMap::InstanceMap(uint32 id, uint32 InstanceId, uint8 SpawnMode,
    ...
    : Map(id, InstanceId, SpawnMode, _parent), m_resetAfterUnload(false),
      m_unloadWhenEmpty(false), instance_script(nullptr), i_script_id(0) {
  ...
  if (IsRaid())
    if (time_t resetTime =
      ...
      if (time_t extendedResetTime = sInstanceSaveMgr->GetExtendedResetTimeFor(
...
bool InstanceMap::CanEnter(Player *player, bool loginCheck) {
  if (!loginCheck && player->GetMapRef().getTarget() == this) {
    LOG_ERROR(
        ...
        player->GetName().c_str(), player->GetGUIDLow(), GetId(),
  ...
  if (GetPlayersCountExceptGMs() >=
    ...
    LOG_DEBUG("server",
              ...
              GetInstanceId(), GetMapName(), maxPlayers,
  ...
  if (Group *group = player->GetGroup())
    if (group->isLFGGroup())
      if (!sLFGMgr->inLfgDungeonMap(group->GetGUID(), GetId(),
                                    GetDifficulty())) {
    ...
    for (PlayerList::const_iterator i = playerList.begin();
      ...
      if (Player *iPlayer = i->GetSource()) {
        ...
        if (iPlayer->IsGameMaster()) // bypass GMs
          continue;
        if (!player->GetGroup()) // player has not group and there is someone
                                 // inside, deny entry
        {
          player->SendTransferAborted(GetId(), TRANSFER_ABORT_MAX_PLAYERS);
          return false;
        }
        // player inside instance has no group or his groups is different to
        // entering player's one, deny entry
...
bool InstanceMap::AddPlayerToMap(Player *player) {
  if (m_resetAfterUnload) // this instance has been reset, it's not meant to be
                          // used anymore
    return false;
  ...
  if (IsDungeon()) {
    ...
    if (!mapSave) {
      LOG_ERROR("server",
                "InstanceMap::Add: InstanceSave does not exist for map %d "
                "spawnmode %d with instance id %d",
                GetId(), GetSpawnMode(), GetInstanceId());
      return false;
    }

    // check for existing instance binds
    InstancePlayerBind *playerBind = sInstanceSaveMgr->PlayerGetBoundInstance(
        player->GetGUIDLow(), GetId(), Difficulty(GetSpawnMode()));
    if (playerBind && playerBind->perm) {
      if (playerBind->save != mapSave) {
        LOG_ERROR(
            "server",
            "InstanceMap::Add: player %s(%d) is permanently bound to instance "
            "%d, %d, %d, %d but he is being put into instance %d, %d, %d, %d",
            player->GetName().c_str(), player->GetGUIDLow(),
            playerBind->save->GetMapId(), playerBind->save->GetInstanceId(),
            playerBind->save->GetDifficulty(), playerBind->save->CanReset(),
            mapSave->GetMapId(), mapSave->GetInstanceId(),
            mapSave->GetDifficulty(), mapSave->CanReset());
        return false;
      }
    } else {
      playerBind = sInstanceSaveMgr->PlayerBindToInstance(
          player->GetGUIDLow(), mapSave, false, player);
      ...
      if (Group *g = player->GetGroup())
        if (g->GetLeaderGUID() != player->GetGUID())
          if (!sInstanceSaveMgr->PlayerGetBoundInstance(
                  GUID_LOPART(g->GetLeaderGUID()), mapSave->GetMapId(),
                  mapSave->GetDifficulty())) {
            sInstanceSaveMgr->PlayerCreateBoundInstancesMaps(
                GUID_LOPART(g->GetLeaderGUID()));
            sInstanceSaveMgr->PlayerBindToInstance(
                GUID_LOPART(g->GetLeaderGUID()), mapSave, false,
                ObjectAccessor::FindPlayerInOrOutOfWorld(g->GetLeaderGUID()));
          }
    }
    ...
    if (!playerBind->perm && !mapSave->CanReset() &&
        (!group || !group->isLFGGroup() || !group->IsLfgRandomInstance())) {
      WorldPacket data(SMSG_INSTANCE_LOCK_WARNING_QUERY, 9);
      data << uint32(60000);
      data << uint32(
          instance_script ? instance_script->GetCompletedEncounterMask() : 0);
      data << uint8(0);
...
void InstanceMap::Update(const uint32 t_diff, const uint32 s_diff,
                         bool /*thread*/) {
...
void InstanceMap::RemovePlayerFromMap(Player *player, bool remove) {
  ...
  // InstanceMap::AfterPlayerUnlinkFromMap(), in this function if 2 players run
  // out at the same time the instance won't close
...
void InstanceMap::CreateInstanceScript(bool load, std::string data,
  ...
  if (method == INSTANCE_RESET_GLOBAL) {
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin();
      ...
      if (!globalResetSkipList ||
          std::find(globalResetSkipList->begin(), globalResetSkipList->end(),
                    itr->GetSource()->GetGUIDLow()) ==
    ...
    if (!globalResetSkipList || globalResetSkipList->empty()) {
      // pussywizard: setting both m_unloadWhenEmpty and m_unloadTimer intended,
      // in case RepopAtGraveyard failed
      if (HavePlayers())
...
void InstanceMap::PermBindAllPlayers() {
  ...
  if (!save) {
    LOG_ERROR("server",
              "Cannot bind players because no instance save is available for "
              "instance map (Name: %s, Entry: %u, InstanceId: %u)!",
              GetMapName(), GetId(), GetInstanceId());
    return;
  }
  ...
  for (MapRefManager::iterator itr = m_mapRefManager.begin();
    ...
    InstancePlayerBind *bind = sInstanceSaveMgr->PlayerGetBoundInstance(
        player->GetGUIDLow(), save->GetMapId(), save->GetDifficulty());

    if (!bind || !bind->perm) {
      WorldPacket data(SMSG_INSTANCE_SAVE_CREATED, 4);
      data << uint32(0);
      player->GetSession()->SendPacket(&data);
      sInstanceSaveMgr->PlayerBindToInstance(player->GetGUIDLow(), save, true,
                                             player);
    }
...
void InstanceMap::SendResetWarnings(uint32 timeLeft) const {
  for (MapRefManager::const_iterator itr = m_mapRefManager.begin();
    ...
    itr->GetSource()->SendInstanceResetWarning(
        GetId(), itr->GetSource()->GetDifficulty(IsRaid()), timeLeft, false);
}
...
BattlegroundMap::BattlegroundMap(uint32 id, uint32 InstanceId, Map *_parent,
                                 uint8 spawnMode)
...
void BattlegroundMap::InitVisibilityDistance() {
  ...
  if (IsBattleArena()) // pussywizard: start with 30yd visibility range on
                       // arenas to ensure players can't get informations about
                       // the opponents in any way
    m_VisibleDistance = 30.0f;
}

bool BattlegroundMap::CanEnter(Player *player, bool loginCheck) {
  if (!loginCheck && player->GetMapRef().getTarget() == this) {
    LOG_ERROR("server", "BGMap::CanEnter - player %u is already in map!",
              player->GetGUIDLow());
    ABORT();
    return false;
  }
  ...
  // pussywizard: no need to check player limit here, invitations are limited by
  // Battleground::GetFreeSlotsForTeam
...
Transport *Map::GetTransport(uint64 guid) {
  if (GUID_HIPART(guid) != HIGHGUID_MO_TRANSPORT &&
      GUID_HIPART(guid) != HIGHGUID_TRANSPORT)
    return nullptr;
...
void Map::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry,
                               Unit *source) {
  ...
  sScriptMgr->OnAfterUpdateEncounterState(this, type, creditEntry, source,
                                          difficulty_fixed, encounters,
                                          dungeonId, updated);
...
void Map::LogEncounterFinished(EncounterCreditType type, uint32 creditEntry) {
  if (!IsRaid() || !GetEntry() ||
      GetEntry()->Expansion() <
          2) // only for wotlk raids, because logs take up tons of mysql memory
    return;
  ...
  for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
    if (Player *p = itr->GetSource()) {
      ...
      for (Unit::AuraApplicationMap::const_iterator itr = a.begin();
           itr != a.end(); ++itr) {
        snprintf(buffer2, 255, "%u(%u) ", itr->first,
                 itr->second->GetEffectMask());
        auraStr += buffer2;
      }

      snprintf(buffer, 16384,
               "%s (guid: %u, acc: %u, ip: %s, guild: %u), xyz: (%.1f, %.1f, "
               "%.1f), auras: %s\n",
               p->GetName().c_str(), p->GetGUIDLow(),
               p->GetSession()->GetAccountId(),
               p->GetSession()->GetRemoteAddress().c_str(), p->GetGuildId(),
  ...
  CharacterDatabase.PExecute(
      "INSERT INTO log_encounter VALUES(NOW(), %u, %u, %u, %u, '%s')", GetId(),
      (uint32)GetDifficulty(), type, creditEntry, playersInfo.c_str());
}
...
void Map::SendZoneDynamicInfo(Player *player) {
  uint32 zoneId = GetZoneId(player->GetPositionX(), player->GetPositionY(),
                            player->GetPositionZ());
...
void Map::SetZoneOverrideLight(uint32 zoneId, uint32 lightId,
                               uint32 fadeInTime) {
  if (_zoneDynamicInfo.find(zoneId) == _zoneDynamicInfo.end())
    _zoneDynamicInfo.insert(
        ZoneDynamicInfoMap::value_type(zoneId, ZoneDynamicInfo()));
 ...
 *
 */
bool Map::CanReachPositionAndGetValidCoords(const WorldObject *source,
                                            PathGenerator *path, float &destX,
  ...
  for (auto &vector : path->GetPath()) {
    ...
    if (!CanReachPositionAndGetValidCoords(source, prevPath.x, prevPath.y,
                                           prevPath.z, x, y, z, failOnCollision,
                                           failOnSlopes)) {
      destX = x;
      destY = y;
      destZ = z;
...
bool Map::CanReachPositionAndGetValidCoords(const WorldObject *source,
                                            ...
                                            float &destZ, bool failOnCollision,
                                            bool failOnSlopes) const {
  return CanReachPositionAndGetValidCoords(
      source, source->GetPositionX(), source->GetPositionY(),
      source->GetPositionZ(), destX, destY, destZ, failOnCollision,
      failOnSlopes);
}

bool Map::CanReachPositionAndGetValidCoords(const WorldObject *source,
                                            float startX, float startY,
                                            float startZ, float &destX,
                                            float &destY, float &destZ,
                                            bool failOnCollision,
                                            bool failOnSlopes) const {
  if (!CheckCollisionAndGetValidCoords(source, startX, startY, startZ, destX,
                                       destY, destZ, failOnCollision)) {
    return false;
  }
  ...
  bool cannotWalkOrFly = !isWaterNext && !source->ToPlayer() &&
                         !unit->CanFly() && (creature && !creature->CanWalk());
  if (cannotEnterWater || cannotWalkOrFly ||
      (failOnSlopes &&
       !PathGenerator::IsWalkableClimb(startX, startY, startZ, destX, destY,
                                       destZ, source->GetCollisionHeight()))) {
    return false;
  }
 ...
 *
 **/
bool Map::CheckCollisionAndGetValidCoords(const WorldObject *source,
                                          float startX, float startY,
                                          float startZ, float &destX,
                                          float &destY, float &destZ,
                                          bool failOnCollision) const {
  // Prevent invalid coordinates here, position is unchanged
  if (!Warhead::IsValidMapCoord(startX, startY, startZ) ||
      !Warhead::IsValidMapCoord(destX, destY, destZ)) {
    LOG_FATAL(
        "server",
        "Map::CheckCollisionAndGetValidCoords invalid coordinates startX: %f, "
        "startY: %f, startZ: %f, destX: %f, destY: %f, destZ: %f",
  ...
  if (notOnGround) {
    bool col = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(
        source->GetMapId(), startX, startY, startZ + halfHeight, destX, destY,
        destZ + halfHeight, destX, destY, destZ, -CONTACT_DISTANCE);
  ...
  bool col = source->GetMap()->getObjectHitPos(
      source->GetPhaseMask(), startX, startY, startZ + halfHeight, destX, destY,
      destZ + halfHeight, destX, destY, destZ, -CONTACT_DISTANCE);
