...
PoolObject *PoolGroup<T>::RollOne(ActivePoolData &spawns, uint32 triggerFrom) {
  if (!ExplicitlyChanced.empty()) {
    ...
    for (uint32 i = 0; i < ExplicitlyChanced.size(); ++i) {
      ...
      // Triggering object is marked as spawned at this time and can be also
      // rolled (respawn case) so this need explicit check for this case
      if (roll < 0 && (ExplicitlyChanced[i].guid == triggerFrom ||
                       !spawns.IsActiveObject<T>(ExplicitlyChanced[i].guid)))
...
template <> void PoolGroup<Creature>::Despawn1Object(uint32 guid) {
  if (CreatureData const *data = sObjectMgr->GetCreatureData(guid)) {
    sObjectMgr->RemoveCreatureFromGrid(guid, data);

    if (Creature *creature = ObjectAccessor::GetObjectInWorld(
            MAKE_NEW_GUID(guid, data->id, HIGHGUID_UNIT), (Creature *)nullptr))
      creature->AddObjectToRemoveList();
  }
}

// Same on one gameobject
template <> void PoolGroup<GameObject>::Despawn1Object(uint32 guid) {
...
void PoolGroup<T>::SpawnObject(ActivePoolData &spawns, uint32 limit,
  ...
  // spawned and also counted into m_SpawnedPoolAmount so we need increase count
  // to be spawned by 1
  if (triggerFrom)
    ++count;
...
template <> void PoolGroup<Quest>::Spawn1Object(PoolObject *obj) {
  ...
  for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second; ++itr) {
    ...
    LOG_DEBUG("pool", "PoolGroup<Quest>: Adding quest %u to creature %u",
  ...
  for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second; ++itr) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("pool", "PoolGroup<Quest>: Adding quest %u to GO %u", itr->first,
              itr->second);
#endif
    questMap->insert(QuestRelations::value_type(itr->second, itr->first));
  }
...
void PoolGroup<Quest>::SpawnObject(ActivePoolData &spawns, uint32 limit,
  ...
  if (limit > newQuests.size() && !currentQuests.empty()) {
    do {
      uint32 questId =
          Warhead::Containers::SelectRandomContainerElement(currentQuests);
  ...
  do {
    uint32 questId =
        Warhead::Containers::SelectRandomContainerElement(newQuests);
    spawns.ActivateObject<Quest>(questId, poolId);
    PoolObject tempObj(questId, 0.0f);
    Spawn1Object(&tempObj);
  ...
  // if we are here it means the pool is initialized at startup and did not have
...
void PoolMgr::LoadFromDB() {
  // Pool templates
  {
    ...
    LOG_INFO("server", ">> Loaded %u objects pools in %u ms", count,
  ...
  LOG_INFO("server", "Loading Creatures Pooling Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 creatures in  pools. DB table "
      ...
      do {
        ...
        if (it == mPoolTemplate.end()) {
          LOG_ERROR("sql.sql",
                    "`pool_creature` pool id (%u) is not in `pool_template`, "
                    "skipped.",
                    pool_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u creatures in pools in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Gameobject Pooling Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 gameobjects in  pools. DB table "
      ...
      do {
        ...
        if (it == mPoolTemplate.end()) {
          LOG_ERROR("sql.sql",
                    "`pool_gameobject` pool id (%u) is not in `pool_template`, "
                    "skipped.",
                    pool_id);
          continue;
        }
        ...
        if (chance < 0 || chance > 100) {
          LOG_ERROR("sql.sql",
                    ...
                    "gameobject guid (%u) in pool id (%u), skipped.",
                    chance, guid, pool_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u gameobject in pools in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
    ...
    if (!result) {
      ...
      do {
...

        {
          ...
          if (it == mPoolTemplate.end()) {
            LOG_ERROR("sql.sql",
                      "`pool_pool` mother_pool id (%u) is not in "
        ...
        }
        {
          ...
          if (it == mPoolTemplate.end()) {
            LOG_ERROR("sql.sql",
                      "`pool_pool` included pool_id (%u) is not in "
                      "`pool_template`, skipped.",
                      child_pool_id);
            continue;
          }
        ...
        if (mother_pool_id == child_pool_id) {
          LOG_ERROR("sql.sql",
                    "`pool_pool` pool_id (%u) includes itself, dead-lock "
                    "detected, skipped.",
                    child_pool_id);
          continue;
        }
      ...
      for (auto const it : mPoolTemplate) {
        ...
        for (SearchMap::iterator poolItr = mPoolSearchMap.find(it.first);
          ...
          if (checkedPools.find(poolItr->second) != checkedPools.end()) {
            ...
            ss << "The pool(s) ";
            for (std::set<uint32>::const_iterator itr = checkedPools.begin();
                 itr != checkedPools.end(); ++itr)
              ss << *itr << ' ';
            ss << "create(s) a circular reference, which can cause the server "
      ...
      LOG_INFO("server", ">> Loaded %u pools in mother pools in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Quest Pooling Data...");
  {
    ...
    if (!result) {
      ...
      enum eQuestTypes { QUEST_NONE = 0, QUEST_DAILY = 1, QUEST_WEEKLY = 2 };
      ...
      do {
        ...
        if (!quest) {
          LOG_ERROR("sql.sql",
                    "`pool_quest` has a non existing quest template (Entry: "
                    "%u) defined for pool id (%u), skipped.",
                    entry, pool_id);
          continue;
        }
        ...
        if (it == mPoolTemplate.end()) {
          LOG_ERROR(
              "sql.sql",
              "`pool_quest` pool id (%u) is not in `pool_template`, skipped.",
              pool_id);
          continue;
        }
        ...
        if (!quest->IsDailyOrWeekly()) {
          LOG_ERROR(
              "sql.sql",
              "`pool_quest` has an quest (%u) which is not daily or weekly in "
              "pool id (%u), use ExclusiveGroup instead, skipped.",
              entry, pool_id);
          continue;
        }
        ...
        if (poolTypeMap[pool_id] != currType) {
          LOG_ERROR("sql.sql",
                    "`pool_quest` quest %u is %s but pool (%u) is specified "
                    "for %s, mixing not allowed, skipped.",
                    entry,
                    currType == QUEST_DAILY ? "QUEST_DAILY" : "QUEST_WEEKLY",
                    pool_id,
                    poolTypeMap[pool_id] == QUEST_DAILY ? "QUEST_DAILY"
                                                        : "QUEST_WEEKLY");
          continue;
        }
        ...
        if (creBounds.first == creBounds.second &&
          ...
          LOG_ERROR("sql.sql",
                    "`pool_quest` lists entry (%u) as member of pool (%u) but "
                    "is not started anywhere, skipped.",
                    entry, pool_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u quests in pools in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Starting objects pooling system...");
  {
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        " LEFT JOIN pool_pool ON pool_template.entry=pool_pool.pool_id WHERE "
    ...
    if (!result) {
      ...
      do {
        ...
        if (!CheckPool(pool_entry)) {
          if (pool_pool_id)
            // The pool is a child pool in pool_pool table. Ideally we should
            // remove it from the pool handler to ensure it never gets spawned,
            // however that could recursively invalidate entire chain of mother
            // pools. It can be done in the future but for now we'll do nothing.
            LOG_ERROR("sql.sql",
                      "Pool Id %u has no equal chance pooled entites defined "
                      "and explicit chance sum is not 100. This broken pool is "
                      "a child pool of Id %u and cannot be safely removed.",
                      pool_entry, fields[2].GetUInt32());
          else
            LOG_ERROR(
                "sql.sql",
                "Pool Id %u has no equal chance pooled entites defined and "
                "explicit chance sum is not 100. The pool will not be spawned.",
                pool_entry);
          continue;
        }

        // Don't spawn child pools, they are spawned recursively by their parent
        // pools
        if (!pool_pool_id) {
      ...
      LOG_INFO("server",
               "Pool handling system initialized, %u pools spawned in %u ms",
...
template <> void PoolMgr::SpawnPool<Creature>(uint32 pool_id, uint32 db_guid) {
  ...
  if (it != mPoolCreatureGroups.end() && !it->second.isEmpty())
    it->second.SpawnObject(mSpawnedData, mPoolTemplate[pool_id].MaxLimit,
                           db_guid);
}
