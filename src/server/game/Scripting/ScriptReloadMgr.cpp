...
public:
    ...
    void operator() (HandleType handle) const
    {
        ...
        if (cache_path_)
        {
            ...
            {
                LOG_ERROR("scripts.hotswap", "Failed to delete the cached shared library \"{}\" ({}).",
...
ScriptModule::CreateFromPath(fs::path const& path, Optional<fs::path> cache_path)
{
    ...
    if (!GetFunctionFromSharedLibrary(handle, "GetScriptModuleRevisionHash", getScriptModuleRevisionHash))
    {
        LOG_ERROR("scripts.hotswap", "Could not extract '{}' function from library: {}. Is module? {}",
            "GetScriptModuleRevisionHash", load_path.generic_string(), isModule);
        return {};
    }
    ...
    {
        LOG_ERROR("scripts.hotswap", "Could not extract '{}' function from library: {}. Is module? {}",
            fnAddScripts, load_path.generic_string(), isModule);
        return {};
    }
    ...
    {
        LOG_ERROR("scripts.hotswap", "Could not extract '{}' function from library: {}. Is module? {}",
            "GetScriptModule", load_path.generic_string(), isModule);
        return {};
    }
    ...
    {
        LOG_ERROR("scripts.hotswap", "Could not extract '{}' function from library: {}. Is module? {}",
            fnGetBuildDirective, load_path.generic_string(), isModule);
        return {};
    }
...
static int InvokeCMakeCommand(T&&... args)
{
    ...
    return Warhead::StartProcess(executable, { std::forward<T>(args)... }, "scripts.hotswap");
}
...
{
    auto const executable = BuiltInConfig::GetCMakeCommand();
    return Warhead::StartAsyncProcess(executable, { std::forward<T>(args)... }, "scripts.hotswap");
}

/// Calculates the C++ project name of the given module which is
/// the lowercase string of scripts_${module}.
static std::string CalculateProjectName(std::string_view name)
{
    std::string module_project{ "scripts_" };

    if (IsModuleName(name))
    {
        module_project = "module_";

        std::string saveName{ name };
        std::replace(saveName.begin(), saveName.end(), '-', '_');

        module_project.append(saveName);
    }
    else
        module_project.append(std::string{ name });
    ...
    : public ScriptReloadMgr
{
    ...
    public:
        ...
        void UpdateCurrentJob(BuildJobType type, std::shared_ptr<Warhead::AsyncProcessResult> async_result)
        {
    ...
    template<typename T>
    auto MakeMessage(T&& dispatcher) -> ScriptReloaderMessageImplementation<typename std::decay<T>::type>*
    {
...
public:
    HotSwapScriptReloadMgr()
        : _libraryWatcher(-1),
        _unique_library_name_counter(0),
        _last_time_library_changed(0),
        _last_time_sources_changed(0),
        _last_time_user_informed(0),
    ...
    static fs::path GetLibraryDirectory()
    {
        ...
        fs::path path(CONF_GET_STR("HotSwap.ScriptDir"));
        if (path.is_absolute())
            return path;
        else
    ...
    static fs::path GetScriptsSourceDirectory()
    {
    ...
    /// Returns the absolute path to the modules directory in the source tree.
    static fs::path GetModulesSourceDirectory()
    {
        fs::path dir = BuiltInConfig::GetSourceDirectory();
        dir /= "modules";
        return dir;
    ...
    void Initialize() final override
    {
        ...
        {
            LOG_ERROR("scripts.hotswap", "Your build directory path \"{}\" "
...
private:
    ...
    void InitializeDefaultLibraries()
    {
        ...
        for (fs::directory_iterator dir_itr(libraryDirectory); dir_itr != dir_end; ++dir_itr)
        {
            ...
            if (HasValidScriptModuleName(dir_itr->path().filename().generic_string()))
            {
                LOG_INFO("scripts.hotswap", "Loading script module \"{}\"...", dir_itr->path().filename().generic_string());
            ...
            if (HasValidModuleName(dir_itr->path().filename().generic_string()))
            {
                LOG_INFO("scripts.hotswap", "Loading script module \"{}\"...", dir_itr->path().filename().generic_string());
        ...
        LOG_INFO("scripts.hotswap", ">> Loaded {} script modules", count);
    }
    ...
    void InitializeFileWatchers()
    {
        ...
        if (_libraryWatcher >= 0)
        {
            LOG_INFO("scripts.hotswap", ">> Library reloader is listening on \"{}\".",
                GetLibraryDirectory().generic_string());
        }
        else
        {
            LOG_ERROR("scripts.hotswap", "Failed to initialize the library reloader on \"{}\".",
                GetLibraryDirectory().generic_string());
        }
    ...
    static fs::path CalculateTemporaryCachePath()
    {
        ...
        path /= Warhead::StringFormat("wh_script_cache_{}_{}",
            GitRevision::GetBranch(),
            ByteArrayToHexStr(Warhead::Crypto::SHA1::GetDigestOf(sConfigMgr->GetFilename())));
    ...
    fs::path GenerateUniquePathForLibraryInCache(fs::path path)
    {
        ...
        fs::path cache_path{ temporary_cache_path_ };
        cache_path /= Warhead::StringFormat("{}.{}{}",
    ...
    void UpdateSourceChangeRequest(std::string const& module_name, fs::path const& path, ChangeStateRequest state)
    {
        ...
        {
            //module_itr->second.emplace(path, state);
    ...
    void ProcessLoadScriptModule(fs::path const& path, bool swap_context = true)
    {
...

        {
            ...
            LOG_TRACE("scripts.hotswap", ">> Copied the shared library \"{}\" to \"{}\" for caching.",
                path.filename().generic_string(), cache_path.generic_string());
        }
        ...
        LOG_INFO("scripts.hotswap", ">> Loaded script module \"{}\" (\"{}\" - {}).",
            path.filename().generic_string(), module_name, module_revision);

        if (module_revision.empty())
        {
            LOG_WARN("scripts.hotswap", ">> Script module \"{}\" has an empty revision hash!",
                path.filename().generic_string());
        }
        else
        {
            ...
            {
                LOG_WARN("scripts.hotswap", ">> Script module \"{}\" has a different revision hash! "
...

        {
            ...
            {
                LOG_ERROR("scripts.hotswap", ">> Attempt to load a module twice \"{}\" (loaded module is at {})!",
        ...
        fs::path listenerPath{ sScriptReloadMgr->GetScriptsSourceDirectory() / module_name };

        if (IsModuleName(module_name))
            listenerPath = sScriptReloadMgr->GetModulesSourceDirectory() / module_name / "src";

        // Create the source listener
        auto listener = std::make_unique<SourceUpdateListener>(listenerPath, module_name);

        // Store the module
        _known_modules_build_directives.emplace(module_name, (*module)->GetBuildDirective());
        _running_script_modules.emplace(module_name, std::make_pair(*module, std::move(listener)));
        _running_script_module_names.emplace(path, module_name);

        // Process the script loading after the module was registered correctly (#17557).
        sScriptMgr->SetScriptContext(module_name);
        (*module)->AddScripts();
        LOG_TRACE("scripts.hotswap", ">> Registered all scripts of module {}.", module_name);
    ...
    void ProcessUnloadScriptModule(fs::path const& path, bool finish = true)
    {
        ...
        LOG_INFO("scripts.hotswap", "Released script module \"{}\" (\"{}\")...",
    ...
    void DispatchRunningBuildJobs()
    {
        if (_build_job)
        {
            ...
            if (_build_job->GetProcess()->GetFutureResult().wait_for(0s) == std::future_status::ready)
        ...
        if (IsDebuggerBlockingRebuild())
        {
            if ((_last_time_user_informed == 0) || (GetMSTimeDiffToNow(_last_time_user_informed) > 7500))
            {
                ...
                LOG_INFO("scripts.hotswap", "Your attached debugger is blocking the WarheadCore automatic script rebuild, please detach it!");
            }
        ...
        auto module_name = [&]
        {
            ...
            if (sLog->ShouldLog("scripts.hotswap", Warhead::LogLevel::Trace))
            {
                ...
                {
                    LOG_TRACE("scripts.hotswap", "Source file {} was {}.", entry.first.generic_string(),
                        ((entry.second == ChangeStateRequest::CHANGE_REQUEST_ADDED) ? "added" : "removed"));
                }
            }
        ...
        LOG_INFO("scripts.hotswap", "Recompiling module \"{}\"...", module_name);

        // Calculate the project name of the script module
        auto project_name = CalculateProjectName(module_name);

        // Find the best build directive for the module
        auto build_directive = [&]() -> std::string
        {
            auto directive = CONF_GET_STR("HotSwap.ReCompilerBuildType");
            if (!directive.empty())
                return directive;
        ...
        if (rebuild_buildfiles && CONF_GET_BOOL("HotSwap.EnableBuildFileRecreation"))
            DoRerunCMake();
        else
    ...
    void ProcessReadyBuildJob()
    {
        ...
        case BuildJobType::BUILD_JOB_RERUN_CMAKE:
        {
            ...
            {
                LOG_ERROR("scripts.hotswap", ">> Failed to update the build files at \"{}\", "
        ...
        case BuildJobType::BUILD_JOB_COMPILE:
        {
            if (!error) // Build was successful
            {
                if (CONF_GET_BOOL("HotSwap.EnableInstall"))
                {
                    ...
                    LOG_INFO("scripts.hotswap", ">> Successfully build module {}, continue with installing...",
                        _build_job->GetModuleName());
                ...
                LOG_INFO("scripts.hotswap", ">> Successfully build module {}, skipped the installation.",
                    _build_job->GetModuleName());
            }
            else // Build wasn't successful
            {
                LOG_ERROR("scripts.hotswap", ">> The build of module {} failed! See the log for details.",
                    _build_job->GetModuleName());
            }
            break;
        }
        case BuildJobType::BUILD_JOB_INSTALL:
        {
            if (!error)
            {
                ...
                LOG_INFO("scripts.hotswap", ">> Successfully installed module {} in {}s",
                    _build_job->GetModuleName(),
            ...
            else
            {
                ...
                LOG_INFO("scripts.hotswap", ">> The installation of module {} failed! See the log for details.",
                    _build_job->GetModuleName());
            }
            break;
        }
    ...
    void DoCompileCurrentProcessedModule()
    {
        ...
        LOG_INFO("scripts.hotswap", "Starting asynchronous build job for module {}...",
            _build_job->GetModuleName());
    ...
    void DoInstallCurrentProcessedModule()
    {
        ...
        LOG_INFO("scripts.hotswap", "Starting asynchronous install job for module {}...",
            _build_job->GetModuleName());
    ...
    {
        LOG_INFO("scripts.hotswap", "Correcting your CMAKE_INSTALL_PREFIX in \"{}\"...",
            BuiltInConfig::GetBuildDirectory());
        ...
        LOG_TRACE("scripts.hotswap", "Checking CMake cache (\"{}\") "
        ...
        std::string cmake_cache_content;
        {
            ...
            if (!in.is_open())
            {
                LOG_ERROR("scripts.hotswap", ">> Failed to read the CMake cache at \"{}\"!",
                    cmake_cache_path.generic_string());
        ...
        if (begin != std::string::npos)
        {
            ...
            if (end != std::string::npos)
            {
                ...
                if (value != current_path)
                {
                    ...
                    LOG_INFO("scripts.hotswap", ">> Found outdated CMAKE_INSTALL_PREFIX (\"{}\"), "
        ...
        auto const error = InvokeCMakeCommand("-DCMAKE_INSTALL_PREFIX:PATH=" + fs::current_path().generic_string(),
            BuiltInConfig::GetBuildDirectory());
        ...
        {
            LOG_INFO("scripts.hotswap", ">> Successfully corrected your CMAKE_INSTALL_PREFIX variable"
    ...
    std::string const& filename, efsw::Action action, std::string oldFilename)
{
    ...
    sScriptReloadMgr->QueueMessage([=](HotSwapScriptReloadMgr* reloader) mutable
    {
        ...
        if (!HasValidScriptModuleName(filename) && !HasValidModuleName(filename))
            return;

        switch (action)
        {
        case efsw::Actions::Add:
            LOG_TRACE("scripts.hotswap", ">> Loading \"{}\" ({})...",
                path.generic_string(), ActionToString(action));
            reloader->QueueSharedLibraryChanged(path);
            break;
        case efsw::Actions::Delete:
            LOG_TRACE("scripts.hotswap", ">> Unloading \"{}\" ({})...",
                path.generic_string(), ActionToString(action));
            reloader->QueueSharedLibraryChanged(path);
            break;
        case efsw::Actions::Modified:
            LOG_TRACE("scripts.hotswap", ">> Reloading \"{}\" ({})...",
                path.generic_string(), ActionToString(action));
            reloader->QueueSharedLibraryChanged(path);
            break;
        default:
    ...
    watcher_id_(sScriptReloadMgr->_fileWatcher.addWatch(path_.generic_string(), this, true))
{
    if (watcher_id_ >= 0)
    {
        LOG_TRACE("scripts.hotswap", ">> Attached the source recompiler to \"{}\".",
            path_.generic_string());
    }
    else
    {
        LOG_ERROR("scripts.hotswap", "Failed to initialize thesource recompiler on \"{}\".",
            path_.generic_string());
    }
}
...
SourceUpdateListener::~SourceUpdateListener()
{
    if (watcher_id_ >= 0)
    {
        ...
        LOG_TRACE("scripts.hotswap", ">> Detached the source recompiler from \"{}\".",
            path_.generic_string());
    }
}
    ...
    std::string const& filename, efsw::Action action, std::string oldFilename)
{
    ...
    {
        LOG_TRACE("scripts.hotswap", "Detected source change on module \"{}\", "
            "queued for recompilation...", script_module_name_);

        switch (action)
        {
        case efsw::Actions::Add:
            LOG_TRACE("scripts.hotswap", "Source file {} of module {} was added.",
                path.generic_string(), script_module_name_);
            reloader->QueueAddSourceFile(script_module_name_, path);
            break;
        case efsw::Actions::Delete:
            LOG_TRACE("scripts.hotswap", "Source file {} of module {} was deleted.",
                path.generic_string(), script_module_name_);
            reloader->QueueRemoveSourceFile(script_module_name_, path);
            break;
        case efsw::Actions::Modified:
            LOG_TRACE("scripts.hotswap", "Source file {} of module {} was modified.",
                path.generic_string(), script_module_name_);
...
ScriptReloadMgr::AcquireModuleReferenceOfContext(std::string_view context)
{
    ...
    auto const itr = sScriptReloadMgr->_running_script_modules.find(std::string{ context });
