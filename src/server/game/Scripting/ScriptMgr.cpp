...
#define SCR_CLEAR(T)                                                           \
  for (SCR_REG_ITR(T) itr = SCR_REG_LST(T).begin();                            \
...
void ScriptMgr::CheckIfScriptsInDatabaseExist() {
  ...
  for (ObjectMgr::ScriptNameContainer::iterator itr = sn.begin();
       itr != sn.end(); ++itr)
    if (uint32 sid = sObjectMgr->GetScriptId((*itr).c_str())) {
      if (!ScriptRegistry<SpellScriptLoader>::GetScriptById(sid) &&
        ...
        LOG_ERROR(
            "sql.sql",
            "Script named '%s' is assigned in the database, but has no code!",
            (*itr).c_str());
    }
}
...
void ScriptMgr::FillSpellSummary() {
  ...
  for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i) {
    ...
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j) {
      ...
      // Make sure that this spell includes a healing effect (or an apply aura
      // with a periodic heal).
...
void ScriptMgr::CreateSpellScripts(uint32 spellId,
  ...
  for (SpellScriptsContainer::iterator itr = bounds.first; itr != bounds.second;
       ++itr) {
...
bool ScriptMgr::OnCastItemCombatSpell(Player *player, Unit *victim,
...
void ScriptMgr::OnGossipSelect(Player *player, Item *item, uint32 sender,
                               uint32 action) {
  ASSERT(player);
  ASSERT(item);
#ifdef ELUNA
...
void ScriptMgr::OnGossipSelectCode(Player *player, Item *item, uint32 sender,
...
void ScriptMgr::OnGossipSelect(Player *player, uint32 menu_id, uint32 sender,
                               uint32 action) {
#ifdef ELUNA
...
void ScriptMgr::OnGossipSelectCode(Player *player, uint32 menu_id,
  ...
  FOREACH_SCRIPT(PlayerScript)
      ->OnGossipSelectCode(player, menu_id, sender, action, code);
}
...
void ScriptMgr::OnGameObjectLootStateChanged(GameObject *go, uint32 state,
                                             Unit *unit) {
  ASSERT(go);
...
bool ScriptMgr::OnAreaTrigger(Player *player, AreaTrigger const *trigger) {
  ...
  if (sEluna->OnAreaTrigger(player, trigger))
    return false;
#endif
  GET_SCRIPT_RET(AreaTriggerScript,
                 sObjectMgr->GetAreaTriggerScriptId(trigger->entry), tmpscript,
                 false);
...
std::vector<ChatCommand> ScriptMgr::GetChatCommands() {
  ...
  std::sort(table.begin(), table.end(),
            [](const ChatCommand &a, const ChatCommand &b) {
...
void ScriptMgr::OnBeforeAuctionHouseMgrSendAuctionWonMail(
  ...
  FOREACH_SCRIPT(AuctionHouseScript)
      ->OnBeforeAuctionHouseMgrSendAuctionWonMail(
          auctionHouseMgr, auction, bidder, bidder_accId, sendNotification,
          updateAchievementCriteria, sendMail);
}
...
void ScriptMgr::OnBeforeAuctionHouseMgrSendAuctionSuccessfulMail(
  ...
  FOREACH_SCRIPT(AuctionHouseScript)
      ->OnBeforeAuctionHouseMgrSendAuctionSuccessfulMail(
          auctionHouseMgr, auction, owner, owner_accId, profit,
          sendNotification, updateAchievementCriteria, sendMail);
}
...
void ScriptMgr::OnBeforeAuctionHouseMgrSendAuctionExpiredMail(
  ...
  FOREACH_SCRIPT(AuctionHouseScript)
      ->OnBeforeAuctionHouseMgrSendAuctionExpiredMail(
          auctionHouseMgr, auction, owner, owner_accId, sendNotification,
          sendMail);
}

void ScriptMgr::OnBeforeAuctionHouseMgrSendAuctionOutbiddedMail(
    AuctionHouseMgr *auctionHouseMgr, AuctionEntry *auction, Player *oldBidder,
    uint32 &oldBidder_accId, Player *newBidder, uint32 &newPrice,
    bool &sendNotification, bool &sendMail) {
  FOREACH_SCRIPT(AuctionHouseScript)
      ->OnBeforeAuctionHouseMgrSendAuctionOutbiddedMail(
          auctionHouseMgr, auction, oldBidder, oldBidder_accId, newBidder,
          newPrice, sendNotification, sendMail);
}
...
void ScriptMgr::OnSendInitialPacketsBeforeAddToMap(Player *player,
                                                   WorldPacket &data) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnSendInitialPacketsBeforeAddToMap(player, data);
}
...
void ScriptMgr::OnPlayerReputationRankChange(Player *player, uint32 factionID,
                                             ReputationRank newRank,
                                             ReputationRank oldRank,
                                             bool increased) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnReputationRankChange(player, factionID, newRank, oldRank, increased);
}
...
void ScriptMgr::OnPlayerDuelEnd(Player *winner, Player *loser,
...
void ScriptMgr::OnBeforeSendChatMessage(Player *player, uint32 &type,
...
void ScriptMgr::OnPlayerTextEmote(Player *player, uint32 textEmote,
...
void ScriptMgr::OnPlayerUpdateZone(Player *player, uint32 newZone,
                                   uint32 newArea) {
#ifdef ELUNA
...
void ScriptMgr::OnPlayerUpdateArea(Player *player, uint32 oldArea,
                                   uint32 newArea) {
  FOREACH_SCRIPT(PlayerScript)->OnUpdateArea(player, oldArea, newArea);
}

bool ScriptMgr::OnBeforePlayerTeleport(Player *player, uint32 mapid, float x,
                                       float y, float z, float orientation,
                                       uint32 options, Unit *target) {
  bool ret = true;
  FOR_SCRIPTS_RET(PlayerScript, itr, end,
                  ret) // return true by default if not scripts
  if (!itr->second->OnBeforeTeleport(player, mapid, x, y, z, orientation,
                                     options, target))
...
void ScriptMgr::OnAchievementSave(SQLTransaction &trans, Player *player,
                                  uint16 achiId,
                                  CompletedAchievementData achiData) {
  FOREACH_SCRIPT(PlayerScript)->OnAchiSave(trans, player, achiId, achiData);
}

void ScriptMgr::OnCriteriaSave(SQLTransaction &trans, Player *player,
                               uint16 critId, CriteriaProgress criteriaData) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnCriteriaSave(trans, player, critId, criteriaData);
}

void ScriptMgr::OnPlayerBeingCharmed(Player *player, Unit *charmer,
                                     uint32 oldFactionId, uint32 newFactionId) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeingCharmed(player, charmer, oldFactionId, newFactionId);
}

void ScriptMgr::OnAfterPlayerSetVisibleItemSlot(Player *player, uint8 slot,
                                                Item *item) {
  FOREACH_SCRIPT(PlayerScript)->OnAfterSetVisibleItemSlot(player, slot, item);
}

void ScriptMgr::OnAfterPlayerMoveItemFromInventory(Player *player, Item *it,
                                                   uint8 bag, uint8 slot,
                                                   bool update) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnAfterMoveItemFromInventory(player, it, bag, slot, update);
}

void ScriptMgr::OnEquip(Player *player, Item *it, uint8 bag, uint8 slot,
                        bool update) {
...
void ScriptMgr::GetCustomGetArenaTeamId(const Player *player, uint8 slot,
                                        uint32 &teamID) const {
  FOREACH_SCRIPT(PlayerScript)->GetCustomGetArenaTeamId(player, slot, teamID);
}

void ScriptMgr::GetCustomArenaPersonalRating(const Player *player, uint8 slot,
                                             uint32 &rating) const {
  FOREACH_SCRIPT(PlayerScript)
      ->GetCustomArenaPersonalRating(player, slot, rating);
}

void ScriptMgr::OnGetMaxPersonalArenaRatingRequirement(
    const Player *player, uint32 minSlot, uint32 &maxArenaRating) const {
  FOREACH_SCRIPT(PlayerScript)
      ->OnGetMaxPersonalArenaRatingRequirement(player, minSlot, maxArenaRating);
}
...
bool ScriptMgr::CanJoinInBattlegroundQueue(Player *player,
  ...
  FOR_SCRIPTS_RET(PlayerScript, itr, end,
                  ret) // return true by default if not scripts
  if (!itr->second->CanJoinInBattlegroundQueue(player, BattlemasterGuid,
                                               BGTypeID, joinAsGroup, err))
    ret = false; // we change ret value only when scripts return false
...
void ScriptMgr::OnBeforeTempSummonInitStats(Player *player,
                                            ...
                                            uint32 &duration) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeforeTempSummonInitStats(player, tempSummon, duration);
}
...
void ScriptMgr::OnBeforeGuardianInitStatsForLevel(Player *player,
                                                  ...
                                                  PetType &petType) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeforeGuardianInitStatsForLevel(player, guardian, cinfo, petType);
}

void ScriptMgr::OnAfterGuardianInitStatsForLevel(Player *player,
                                                 Guardian *guardian) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnAfterGuardianInitStatsForLevel(player, guardian);
}

void ScriptMgr::OnBeforeLoadPetFromDB(Player *player, uint32 &petentry,
                                      uint32 &petnumber, bool &current,
                                      bool &forceLoadFromDB) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeforeLoadPetFromDB(player, petentry, petnumber, current,
                              forceLoadFromDB);
}
...
#ifdef ELUNA
  sEluna->OnItemMove(guild, player, pItem, isSrcBank, srcContainer, srcSlotId,
...
#ifdef ELUNA
  sEluna->OnBankEvent(guild, eventType, tabId, playerGuid, itemOrMoney,
...
void ScriptMgr::OnGlobalMirrorImageDisplayItem(const Item *item,
                                               uint32 &display) {
...
void ScriptMgr::OnAfterRefCount(Player const *player, Loot &loot, bool canRate,
                                uint16 lootMode, LootStoreItem *LootStoreItem,
                                uint32 &maxcount, LootStore const &store) {
  FOREACH_SCRIPT(GlobalScript)
      ->OnAfterRefCount(player, LootStoreItem, loot, canRate, lootMode,
                        maxcount, store);
}

void ScriptMgr::OnBeforeDropAddItem(Player const *player, Loot &loot,
                                    bool canRate, uint16 lootMode,
                                    LootStoreItem *LootStoreItem,
                                    LootStore const &store) {
  FOREACH_SCRIPT(GlobalScript)
      ->OnBeforeDropAddItem(player, loot, canRate, lootMode, LootStoreItem,
                            store);
}

void ScriptMgr::OnItemRoll(Player const *player,
                           LootStoreItem const *LootStoreItem, float &chance,
                           Loot &loot, LootStore const &store) {
  FOREACH_SCRIPT(GlobalScript)
      ->OnItemRoll(player, LootStoreItem, chance, loot, store);
}

void ScriptMgr::OnInitializeLockedDungeons(Player *player, uint8 &level,
                                           uint32 &lockData) {
  FOREACH_SCRIPT(GlobalScript)
      ->OnInitializeLockedDungeons(player, level, lockData);
}
...
void ScriptMgr::OnAfterUpdateEncounterState(
    Map *map, EncounterCreditType type, uint32 creditEntry, Unit *source,
    Difficulty difficulty_fixed, DungeonEncounterList const *encounters,
    uint32 dungeonCompleted, bool updated) {
  FOREACH_SCRIPT(GlobalScript)
      ->OnAfterUpdateEncounterState(map, type, creditEntry, source,
                                    difficulty_fixed, encounters,
                                    dungeonCompleted, updated);
}
...
void ScriptMgr::OnBeforeWorldObjectSetPhaseMask(WorldObject const *worldObject,
                                                ...
                                                bool &update) {
  FOREACH_SCRIPT(GlobalScript)
      ->OnBeforeWorldObjectSetPhaseMask(worldObject, oldPhaseMask, newPhaseMask,
                                        useCombinedPhases, update);
}

// Unit
uint32 ScriptMgr::DealDamage(Unit *AttackerUnit, Unit *pVictim, uint32 damage,
...
void ScriptMgr::ModifyPeriodicDamageAurasTick(Unit *target, Unit *attacker,
                                              uint32 &damage) {
  FOREACH_SCRIPT(UnitScript)
      ->ModifyPeriodicDamageAurasTick(target, attacker, damage);
}

void ScriptMgr::ModifyMeleeDamage(Unit *target, Unit *attacker,
                                  uint32 &damage) {
...
void ScriptMgr::ModifyHealRecieved(Unit *target, Unit *attacker,
                                   uint32 &damage) {
...
void ScriptMgr::OnBeforeRollMeleeOutcomeAgainst(
  ...
  FOREACH_SCRIPT(UnitScript)
      ->OnBeforeRollMeleeOutcomeAgainst(
          attacker, victim, attType, attackerMaxSkillValueForLevel,
...
void ScriptMgr::OnPlayerMove(Player *player, MovementInfo movementInfo,
                             uint32 opcode) {
  FOREACH_SCRIPT(MovementHandlerScript)
      ->OnPlayerMove(player, movementInfo, opcode);
}

void ScriptMgr::OnBeforeBuyItemFromVendor(Player *player, uint64 vendorguid,
                                          uint32 vendorslot, uint32 &item,
                                          uint8 count, uint8 bag, uint8 slot) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeforeBuyItemFromVendor(player, vendorguid, vendorslot, item, count,
                                  bag, slot);
}

void ScriptMgr::OnAfterStoreOrEquipNewItem(
    Player *player, uint32 vendorslot, uint32 &item, uint8 count, uint8 bag,
    uint8 slot, ItemTemplate const *pProto, Creature *pVendor,
    VendorItem const *crItem, bool bStore) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnAfterStoreOrEquipNewItem(player, vendorslot, item, count, bag, slot,
                                   pProto, pVendor, crItem, bStore);
}

void ScriptMgr::OnAfterUpdateMaxPower(Player *player, Powers &power,
                                      float &value) {
...
void ScriptMgr::OnBeforeUpdateAttackPowerAndDamage(Player *player, float &level,
                                                   float &val2, bool ranged) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeforeUpdateAttackPowerAndDamage(player, level, val2, ranged);
}

void ScriptMgr::OnAfterUpdateAttackPowerAndDamage(Player *player, float &level,
                                                  float &base_attPower,
                                                  float &attPowerMod,
                                                  float &attPowerMultiplier,
                                                  bool ranged) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnAfterUpdateAttackPowerAndDamage(player, level, base_attPower,
                                          attPowerMod, attPowerMultiplier,
                                          ranged);
}

void ScriptMgr::OnBeforeInitTalentForLevel(Player *player, uint8 &level,
                                           uint32 &talentPointsForLevel) {
  FOREACH_SCRIPT(PlayerScript)
      ->OnBeforeInitTalentForLevel(player, level, talentPointsForLevel);
}
...
void ScriptMgr::OnAfterArenaRatingCalculation(Battleground *const bg,
                                              ...
                                              int32 &loserChange) {
  FOREACH_SCRIPT(FormulaScript)
      ->OnAfterArenaRatingCalculation(bg, winnerMatchmakerChange,
                                      loserMatchmakerChange, winnerChange,
                                      loserChange);
}
...
void ScriptMgr::OnBattlegroundEndReward(Battleground *bg, Player *player,
                                        TeamId winnerTeamId) {
...
void ScriptMgr::OnAddGroup(BattlegroundQueue *queue, GroupQueueInfo *ginfo,
                           uint32 &index, Player *leader, Group *grp,
                           PvPDifficultyEntry const *bracketEntry,
                           bool isPremade) {
  FOREACH_SCRIPT(BGScript)->OnAddGroup(queue, ginfo, index, leader, grp,
                                       bracketEntry, isPremade);
}

bool ScriptMgr::CanFillPlayersToBG(BattlegroundQueue *queue, Battleground *bg,
                                   const int32 aliFree, const int32 hordeFree,
                                   BattlegroundBracketId bracket_id) {
  bool ret = true;

  FOR_SCRIPTS_RET(BGScript, itr, end,
                  ret) // return true by default if not scripts
  if (!itr->second->CanFillPlayersToBG(queue, bg, aliFree, hordeFree,
                                       bracket_id))
    ret = false; // we change ret value only when scripts return false
...
bool ScriptMgr::CanFillPlayersToBGWithSpecific(
    BattlegroundQueue *queue, Battleground *bg, const int32 aliFree,
    const int32 hordeFree, BattlegroundBracketId thisBracketId,
    BattlegroundQueue *specificQueue, BattlegroundBracketId specificBracketId) {
  bool ret = true;

  FOR_SCRIPTS_RET(BGScript, itr, end,
                  ret) // return true by default if not scripts
  if (!itr->second->CanFillPlayersToBGWithSpecific(
          queue, bg, aliFree, hordeFree, thisBracketId, specificQueue,
          specificBracketId))
    ret = false; // we change ret value only when scripts return false
...
void ScriptMgr::OnCheckNormalMatch(BattlegroundQueue *queue, uint32 &Coef,
                                   Battleground *bgTemplate,
                                   BattlegroundBracketId bracket_id,
                                   uint32 &minPlayers, uint32 &maxPlayers) {
  FOREACH_SCRIPT(BGScript)->OnCheckNormalMatch(
      queue, Coef, bgTemplate, bracket_id, minPlayers, maxPlayers);
}

bool ScriptMgr::CanSendMessageQueue(BattlegroundQueue *queue, Player *leader,
                                    Battleground *bg,
                                    PvPDifficultyEntry const *bracketEntry) {
  bool ret = true;
...
// Mail
void ScriptMgr::OnBeforeMailDraftSendMailTo(
  ...
  FOREACH_SCRIPT(MailScript)
      ->OnBeforeMailDraftSendMailTo(mailDraft, receiver, sender, checked,
                                    deliver_delay, custom_expiration,
