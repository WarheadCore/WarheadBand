...
void SystemMgr::LoadScriptWaypoints() {
  ...
  do {
    ...
    if (!pCInfo) {
      LOG_ERROR("sql.sql",
                "TSCR: DB table script_waypoint has waypoint for non-existant "
                "creature entry %u",
    ...
    if (!pCInfo->ScriptID)
      LOG_ERROR(
          "sql.sql",
          "TSCR: DB table script_waypoint has waypoint for creature entry %u, "
          "but creature does not have ScriptName defined and then useless.",
          temp.uiCreatureEntry);
  ...
  LOG_INFO("server", ">> Loaded %u Script Waypoint nodes in %u ms", count,
