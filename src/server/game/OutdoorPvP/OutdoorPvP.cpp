...
OPvPCapturePoint::OPvPCapturePoint(OutdoorPvP *pvp)
    : m_capturePointGUID(0), m_capturePoint(nullptr), m_maxValue(0.0f),
      m_minValue(0.0f), m_maxSpeed(0), m_value(0), m_team(TEAM_NEUTRAL),
      m_OldState(OBJECTIVESTATE_NEUTRAL), m_State(OBJECTIVESTATE_NEUTRAL),
      m_neutralValuePct(0), m_PvP(pvp) {}
...
bool OPvPCapturePoint::AddObject(uint32 type, uint32 entry, uint32 map, float x,
                                 float y, float z, float o, float rotation0,
                                 float rotation1, float rotation2,
                                 float rotation3) {
  if (uint32 guid = sObjectMgr->AddGOData(entry, map, x, y, z, o, 0, rotation0,
...
bool OPvPCapturePoint::AddCreature(uint32 type, uint32 entry, uint32 map,
...
bool OPvPCapturePoint::SetCapturePointData(uint32 entry, uint32 map, float x,
  ...
  uint32 lowguid = sObjectMgr->AddGOData(entry, map, x, y, z, o, 0, rotation0,
                                         rotation1, rotation2, rotation3);
  if (!lowguid)
    return false;
...
void OutdoorPvP::HandlePlayerLeaveZone(Player *player, uint32 /*zone*/) {
  ...
  // remove the world state information from the player (we can't keep everyone
  // up to date, so leave out those who are not in the concerning zones)
  if (!player->GetSession()->PlayerLogout())
    SendRemoveWorldStates(player);
  m_players[player->GetTeamId()].erase(player->GetGUID());
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("outdoorpvp", "Player %s left an outdoorpvp zone",
...
bool OPvPCapturePoint::Update(uint32 diff) {
    ...
    if (ChallengerId == TEAM_ALLIANCE &&
      ...
      m_State = OBJECTIVESTATE_HORDE_ALLIANCE_CHALLENGE;
             ...
             (m_OldState == OBJECTIVESTATE_ALLIANCE ||
              m_OldState == OBJECTIVESTATE_NEUTRAL_ALLIANCE_CHALLENGE))
  ...
  if (m_OldState != m_State) {
    // LOG_ERROR("server", LOG_FILTER_OUTDOORPVP, "%u->%u", m_OldState,
    // m_State);
    if (oldTeam != m_team)
      ChangeTeam(oldTeam);
    ChangeState();
...
void OutdoorPvP::HandleKill(Player *killer, Unit *killed) {
  if (Group *group = killer->GetGroup()) {
    for (GroupReference *itr = group->GetFirstMember(); itr != nullptr;
      ...
      // outdoor pvp active player kills only count if active and inside
      // objective
...
bool OutdoorPvP::HandleCustomSpell(Player *player, uint32 spellId,
                                   GameObject *go) {
  for (OPvPCapturePointMap::iterator itr = m_capturePoints.begin();
       itr != m_capturePoints.end(); ++itr)
...
bool OPvPCapturePoint::HandleCustomSpell(Player *player, uint32 /*spellId*/,
                                         GameObject * /*go*/) {
  if (!player->IsOutdoorPvPActive())
    return false;
  return false;
...
bool OutdoorPvP::CanTalkTo(Player *player, Creature *c,
...
bool OPvPCapturePoint::HandleGossipOption(Player * /*player*/, uint64 /*guid*/,
                                          uint32 /*id*/) {
  return false;
}

bool OPvPCapturePoint::CanTalkTo(Player * /*player*/, Creature * /*c*/,
                                 GossipMenuItems const & /*gso*/) {
  return false;
}
...
void OutdoorPvP::TeamApplyBuff(TeamId teamId, uint32 spellId, uint32 spellId2,
                               Player *sameMapPlr) {
  TeamCastSpell(teamId, spellId, sameMapPlr);
  TeamCastSpell(teamId == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE,
                spellId2 ? -(int32)spellId2 : -(int32)spellId, sameMapPlr);
}
