...
Weather::Weather(uint32 zone, WeatherData const *weatherChances)
  ...
  LOG_DEBUG(
      ...
      m_zone, (uint32)(m_timer.GetInterval() / (MINUTE * IN_MILLISECONDS)));
#endif
}
...
bool Weather::ReGenerate() {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  static char const *seasonName[WEATHER_SEASONS] = {"spring", "summer", "fall",
                                                    "winter"};
  LOG_DEBUG("server", "Generating a change in %s weather for zone %u.",
  ...
  // At this point, only weather that isn't doing anything remains but that have
  // weather data
