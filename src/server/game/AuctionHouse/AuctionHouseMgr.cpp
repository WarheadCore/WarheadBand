...
uint32 AuctionHouseMgr::GetAuctionDeposit(AuctionHouseEntry const* entry, uint32 time, Item* pItem, uint32 count)
{
    ...
    uint32 deposit = uint32(((multiplier * MSV * count / 3) * timeHr * 3) * CONF_GET_FLOAT("Rate.Auction.Deposit"));
...
void AuctionHouseMgr::SendAuctionSalePendingMail(AuctionEntry* auction, CharacterDatabaseTransaction trans, bool sendMail)
{
    ...
    if (owner || owner_accId)
    {
        ...
        uint32 deliveryDelay = CONF_GET_UINT("MailDeliveryDelay");
...
void AuctionHouseMgr::SendAuctionSuccessfulMail(AuctionEntry* auction, CharacterDatabaseTransaction trans, bool sendNotification, bool updateAchievementCriteria, bool sendMail)
{
    ...
    if (owner || owner_accId)
    {
        ...
        if (sendMail) // can be changed in the hook
            ...
            .AddMoney(profit)
            .SendMailTo(trans, MailReceiver(owner, auction->owner.GetCounter()), auction, MAIL_CHECK_MASK_COPIED, CONF_GET_INT("MailDeliveryDelay"), 0, false, true, auction->Id);
        ...
        if (auction->bid >= 500 * GOLD)
            if (const GlobalPlayerData* gpd = sWorld->GetGlobalPlayerData(auction->bidder.GetCounter()))
            {
                ...
                CharacterDatabase.PExecute("INSERT INTO log_money VALUES({}, {}, \"{}\", \"{}\", {}, \"{}\", {}, \"<AH> profit: {}g, bidder: {} {} lvl (guid: {}), seller: {} {} lvl (guid: {}), item {} ({})\", NOW())",
                    gpd->accountId, auction->bidder.GetCounter(), gpd->name, bidder ? bidder->GetSession()->GetRemoteAddress() : "", owner_accId, owner_name, auction->bid, (profit / GOLD), gpd->name, gpd->level, auction->bidder.GetCounter(), owner_name, owner_level, auction->owner.GetCounter(), auction->item_template, auction->itemCount);
            }
    }
}
...
{
    int32 cut = int32(CalculatePct(bid, auctionHouseEntry->cutPercent) * CONF_GET_FLOAT("Rate.Auction.Cut"));
