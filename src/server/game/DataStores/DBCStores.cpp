...
static bool LoadDBC_assert_print(uint32 fsize, uint32 rsize,
                                 const std::string &filename) {
  LOG_ERROR("server",
            "Size of '%s' set by format string (%u) not equal size of C++ "
            "structure (%u).",
...
inline void LoadDBC(uint32 &availableDbcLocales, StoreProblemList &errors,
  ...
  if (!existDBData) {
    ...
    if (FILE *f = fopen(dbcFilename.c_str(), "rb")) {
      ...
      stream << dbcFilename << " exists, and has " << storage.GetFieldCount()
             << " field(s) (expected " << strlen(storage.GetFormat())
             << "). Extracted file might be from wrong client version or a "
                "database-update has been forgotten.";
...
#undef LOAD_DBC
  ...
  for (MapDifficultyEntry const *entry : sMapDifficultyStore)
    sMapDifficultyMap[MAKE_PAIR32(entry->MapId, entry->Difficulty)] =
        MapDifficulty(entry->resetTime, entry->maxPlayers,
    ...
    for (uint8 x = 0; x < MAX_DIFFICULTY; ++x) {
      if (spellDiff->SpellID[x] <= 0 ||
        ...
        if (spellDiff->SpellID[x] >
            0) // don't show error if spell is <= 0, not all modes have spells
               // and there are unknown negative values
          LOG_ERROR("sql.sql",
                    "spelldifficulty_dbc: spell %i at field id: %u at spellid "
                    "%i does not exist in SpellStore (spell.dbc), loaded as 0",
                    spellDiff->SpellID[x], spellDiff->ID, x);
  ...
  {
    // now have all max ranks (and then bit amount used for store talent ranks
    // in inspect)
    ...
    for (uint32 i = 1; i < sTaxiNodesStore.GetNumRows(); ++i) {
      ...
      if (src_i != sTaxiPathSetBySource.end() && !src_i->second.empty()) {
        ...
        for (TaxiPathSetForSource::const_iterator dest_i =
                 src_i->second.begin();
             dest_i != src_i->second.end(); ++dest_i) {
          // not spell path
          if (dest_i->second.price ||
              spellPaths.find(dest_i->second.ID) == spellPaths.end()) {
            ok = true;
            break;
          }
        }
  ...
  for (TransportAnimationEntry const *anim : sTransportAnimationStore)
    sTransportMgr->AddPathNodeToTransport(anim->TransportEntry, anim->TimeSeg,
                                          anim);

  for (TransportRotationEntry const *rot : sTransportRotationStore)
    sTransportMgr->AddPathRotationToTransport(rot->TransportEntry, rot->TimeSeg,
                                              rot);

  for (WMOAreaTableEntry const *entry : sWMOAreaTableStore)
    sWMOAreaInfoByTripple[WMOAreaTableKey(entry->rootId, entry->adtId,
                                          entry->groupId)] = entry;
  ...
  {
    LOG_ERROR("server", "You have _outdated_ DBC data. Please extract correct "
  ...
  LOG_INFO("server", ">> Initialized %d data stores in %u ms", DBCFileCount,
...
bool readCamera(M2Camera const *cam, uint32 buffSize, M2Header const *header,
  ...
  for (uint32 k = 0; k < cam->target_positions.timestamps.number; ++k) {
    ...
    uint32 const *targTimestamps =
        reinterpret_cast<uint32 const *>(buffer + targTsArray->offset_elements);
  ...
  // Read camera positions and timestamps (translating first position of 3 only,
  // we don't need to translate the whole spline)
  for (uint32 k = 0; k < cam->positions.timestamps.number; ++k) {
    ...
    uint32 const *posTimestamps =
        reinterpret_cast<uint32 const *>(buffer + posTsArray->offset_elements);
    ...
    for (uint32 i = 0; i < posTsArray->number; ++i) {
      ...
      // Translate co-ordinates
      G3D::Vector3 newPos =
          TranslateLocation(&DBCData, &cam->position_base, &positions->p0);
      ...
      if (targetcam.size() > 0) {
        ...
        // Now, the timestamps for target cam and position can be different. So,
        // if they differ we interpolate
        if (lastTarget.timeStamp != posTimestamps[i]) {
          ...
          x = lastTarget.locations.x +
              (xDiff * (float(timeDiffThis) / float(timeDiffTarget)));
          y = lastTarget.locations.y +
              (yDiff * (float(timeDiffThis) / float(timeDiffTarget)));
          // z = lastTarget.locations.z + (zDiff * (float(timeDiffThis) /
          // float(timeDiffTarget)));
        }
...
void LoadM2Cameras(const std::string &dataPath) {
  ...
  for (uint32 i = 0; i < sCinematicCameraStore.GetNumRows(); ++i) {
    if (CinematicCameraEntry const *dbcentry =
            sCinematicCameraStore.LookupEntry(i)) {
      ...
      if (static_cast<uint32>(fileSize) < sizeof(M2Header)) {
        LOG_ERROR("server",
                  "Camera file %s is damaged. File is smaller than header size",
                  filename.c_str());
        m2file.close();
        continue;
      }
      ...
      if (strcmp(fileCheck, "MD20")) {
        LOG_ERROR("server",
                  "Camera file %s is damaged. File identifier not found",
                  filename.c_str());
        m2file.close();
        continue;
      }
      ...
      if (header->ofsCameras + sizeof(M2Camera) >
        ...
        LOG_ERROR("server",
                  "Camera file %s is damaged. Camera references position "
                  "beyond file end",
                  filename.c_str());
        continue;
      }
  ...
  LOG_INFO("server", ">> Loaded %u cinematic waypoint sets in %u ms",
           (uint32)sFlyByCameraStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
CharStartOutfitEntry const *GetCharStartOutfitEntry(uint8 race, uint8 class_,
                                                    uint8 gender) {
