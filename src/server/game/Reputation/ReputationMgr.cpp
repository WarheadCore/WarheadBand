...
bool ReputationMgr::SetReputation(FactionEntry const *factionEntry,
  ...
  if (const RepSpilloverTemplate *repTemplate =
      ...
      if (repTemplate->faction[i]) {
        if (_player->GetReputationRank(repTemplate->faction[i]) <=
          ...
          SetOneFactionReputation(
              sFactionStore.LookupEntry(repTemplate->faction[i]), spilloverRep,
              incremental);
        }
      }
    }
    ...
    if (!flist && factionEntry->team &&
      ...
      if (FactionEntry const *parent =
        ...
        // some team factions have own reputation standing, in this case do not
        // spill to other sub-factions
        ...
        if (FactionEntry const *factionEntryCalc =
          ...
          if (factionEntryCalc == factionEntry ||
              GetRank(factionEntryCalc) >
                  ReputationRank(factionEntryCalc->spilloverMaxRankIn))
            continue;
          int32 spilloverRep =
              int32(spillOverRepOut * factionEntryCalc->spilloverRateIn);
          if (spilloverRep != 0 || !incremental)
            res = SetOneFactionReputation(factionEntryCalc, spilloverRep,
                                          incremental);
        }
      }
    }
  ...
  if (faction != _factions.end()) {
    // Xinef: if we update spillover only, do not update main reputation (rank
    // exceeds creature reward rate)
    if (!spillOverOnly)
      res = SetOneFactionReputation(factionEntry, standing, incremental);
    // only this faction gets reported to client, even if it has no own visible
    // standing
...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const *factionEntry,
  ...
  if (itr != _factions.end()) {
    ...
    if (incremental) {
      ...
      standing = int32(floor(
          (float)standing * sWorld->getRate(RATE_REPUTATION_GAIN) + 0.5f));
    ...
    sScriptMgr->OnPlayerReputationChange(_player, factionEntry->ID, standing,
                                         incremental);
    ...
    _player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KNOWN_FACTIONS,
                                       factionEntry->ID);
    _player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION, factionEntry->ID);
    _player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_GAIN_EXALTED_REPUTATION, factionEntry->ID);
    _player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_GAIN_REVERED_REPUTATION, factionEntry->ID);
    _player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_GAIN_HONORED_REPUTATION, factionEntry->ID);
...
void ReputationMgr::LoadFromDB(PreparedQueryResult result) {
  ...
  // QueryResult* result = CharacterDatabase.PQuery("SELECT faction, standing,
  // flags FROM character_reputation WHERE guid = '%u'", GetGUIDLow());
  ...
  if (result) {
    do {
      ...
      FactionEntry const *factionEntry =
          sFactionStore.LookupEntry(fields[0].GetUInt16());
      if (factionEntry && (factionEntry->reputationListID >= 0)) {
        ...
        if (dbFactionFlags & FACTION_FLAG_AT_WAR) // DB at war
        {
          ...
          SetAtWar(faction, bool(!(dbFactionFlags & FACTION_FLAG_HIDDEN) ||
                                 GetRank(factionEntry) <
                                     REP_NEUTRAL)); // have internal checks for
                                                    // FACTION_FLAG_PEACE_FORCED
        } else                                      // DB not at war
        {
          // allow remove if visible (and then not FACTION_FLAG_INVISIBLE_FORCED
          // or FACTION_FLAG_HIDDEN)
          if (faction->Flags & FACTION_FLAG_VISIBLE)
            SetAtWar(
                faction,
