...
public:
  AchievementChatBuilder(Player const &player, ChatMsg msgtype, int32 textId,
                         uint32 ach_id)
      : i_player(player), i_msgtype(msgtype), i_textId(textId),
        i_achievementId(ach_id) {}

  void operator()(WorldPacket &data, LocaleConstant loc_idx) {
    std::string text = sObjectMgr->GetAcoreString(i_textId, loc_idx);
    ChatHandler::BuildChatPacket(data, i_msgtype, LANG_UNIVERSAL, &i_player,
                                 &i_player, text, i_achievementId);
  }
...
bool AchievementCriteriaData::IsValid(
  ...
  if (dataType >= MAX_ACHIEVEMENT_CRITERIA_DATA_TYPE) {
    LOG_ERROR("sql.sql",
              "Table `achievement_criteria_data` for criteria (Entry: %u) has "
              "wrong data type (%u), ignored.",
  ...
  default:
    if (dataType != ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT) {
      LOG_ERROR("sql.sql",
                ...
                "criteria type (Entry: %u Type: %u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_CREATURE:
    if (!creature.id || !sObjectMgr->GetCreatureTemplate(creature.id)) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_CREATURE (%u) has "
                "non-existing creature id in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE:
    if (classRace.class_id &&
      ...
      LOG_ERROR("sql.sql",
                ...
                "(%u) has non-existing class in value1 (%u), ignored.",
    ...
    if (classRace.race_id &&
      ...
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_CLASS_RACE "
                "(%u) has non-existing race in value2 (%u), ignored.",
                criteria->ID, criteria->requiredType, dataType,
                classRace.race_id);
      return false;
    }
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_LESS_HEALTH:
    if (health.percent < 1 || health.percent > 100) {
      LOG_ERROR("sql.sql",
                ...
                "(%u) has wrong percent value in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_DEAD:
    if (player_dead.own_team_flag > 1) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_PLAYER_DEAD (%u) "
                "has wrong boolean value1 (%u).",
                criteria->ID, criteria->requiredType, dataType,
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA: {
    ...
    if (!spellEntry) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type %s (%u) has wrong spell id in value1 (%u), ignored.",
                criteria->ID, criteria->requiredType,
                (dataType == ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA
                     ? "ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA"
                     : "ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA"),
                dataType, aura.spell_id);
      return false;
    }
    if (aura.effect_idx >= 3) {
      LOG_ERROR(
          "sql.sql",
          "Table `achievement_criteria_data` (Entry: %u Type: %u) for data "
          "type %s (%u) has wrong spell effect index in value2 (%u), ignored.",
          criteria->ID, criteria->requiredType,
          (dataType == ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA
               ? "ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA"
               : "ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA"),
          dataType, aura.effect_idx);
      return false;
    }
    if (!spellEntry->Effects[aura.effect_idx].ApplyAuraName) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type %s (%u) has non-aura spell effect (ID: %u Effect: "
                "%u), ignores.",
                criteria->ID, criteria->requiredType,
                (dataType == ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA
                     ? "ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA"
                     : "ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA"),
                dataType, aura.spell_id, aura.effect_idx);
      return false;
    }
    return true;
  }
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AREA:
    if (!sAreaTableStore.LookupEntry(area.id)) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AREA (%u) has "
                "wrong area id in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE:
    if (value.compType >= COMP_TYPE_MAX) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE (%u) has wrong "
                "ComparisionType in value2 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL:
    if (level.minlevel > STRONG_MAX_LEVEL) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL (%u) has "
                "wrong minlevel in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_GENDER:
    if (gender.gender > GENDER_NONE) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_GENDER (%u) has "
                "wrong gender in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT:
    if (!ScriptId) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT (%u) does not "
                "have ScriptName set, ignored.",
                criteria->ID, criteria->requiredType, dataType);
      return false;
    }
    return true;
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY:
    if (difficulty.difficulty >= MAX_DIFFICULTY) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY (%u) "
                "has wrong difficulty in value1 (%u), ignored.",
                criteria->ID, criteria->requiredType, dataType,
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_PLAYER_COUNT:
    if (map_players.maxcount <= 0) {
      LOG_ERROR("sql.sql",
                ...
                "(%u) has wrong max players count in value1 (%u), ignored.",
                criteria->ID, criteria->requiredType, dataType,
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_TEAM:
    if (team.team != ALLIANCE && team.team != HORDE) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_TEAM (%u) has "
                "unknown team in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_DRUNK:
    if (drunk.state >= MAX_DRUNKEN) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_S_DRUNK (%u) has "
                "unknown drunken state in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_HOLIDAY:
    if (!sHolidaysStore.LookupEntry(holiday.id)) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: %u Type: %u) for "
                "data type ACHIEVEMENT_CRITERIA_DATA_TYPE_HOLIDAY (%u) has "
                "unknown holiday in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_EQUIPED_ITEM:
    if (equipped_item.item_quality >= MAX_ITEM_QUALITY) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_requirement` (Entry: %u Type: %u) "
                "for requirement ACHIEVEMENT_CRITERIA_REQUIRE_S_EQUIPED_ITEM "
                "(%u) has unknown quality state in value1 (%u), ignored.",
                criteria->ID, criteria->requiredType, dataType,
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_ID:
    if (!sMapStore.LookupEntry(map_id.mapId)) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_requirement` (Entry: %u Type: %u) "
                "for requirement ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_ID (%u) "
                "has unknown map id in value1 (%u), ignored.",
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_PLAYER_CLASS_RACE:
    if (!classRace.class_id && !classRace.race_id) {
      LOG_ERROR("sql.sql",
                ...
                "(%u) must not have 0 in either value field, ignored.",
                criteria->ID, criteria->requiredType, dataType);
      return false;
    }
    if (classRace.class_id &&
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_KNOWN_TITLE: {
    if (!sCharTitlesStore.LookupEntry(known_title.title_id)) {
      LOG_ERROR("sql.sql",
                ...
                "(%u) have unknown title_id in value1 (%u), ignore.",
                criteria->ID, criteria->requiredType, dataType,
  ...
  default:
    LOG_ERROR("sql.sql",
              "Table `achievement_criteria_data` (Entry: %u Type: %u) has data "
              "for non-supported data type (%u), ignored.",
              criteria->ID, criteria->requiredType, dataType);
    return false;
  }
}
...
bool AchievementCriteriaData::Meets(uint32 criteria_id, Player const *source,
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_VALUE:
    return CompareValues(ComparisionType(value.compType), miscvalue1,
                         value.value);
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL:
    if (!target)
      return false;
  ...
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT: {
    ...
    if (!map->IsDungeon()) {
      LOG_ERROR("sql.sql",
                ...
                "ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT (%u) for "
                "achievement criteria %u for non-dungeon/non-raid map %u",
                ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT, criteria_id,
                map->GetId());
      return false;
    }
    ...
    if (!instance) {
      LOG_ERROR(
          "sql.sql",
          "Achievement system call "
          "ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT (%u) for achievement "
          "criteria %u for map %u but map does not have a instance script",
          ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT, criteria_id,
          map->GetId());
      return false;
    }
    return instance->CheckAchievementCriteriaMeet(criteria_id, source, target,
                                                  miscvalue1);
  }
  case ACHIEVEMENT_CRITERIA_DATA_TYPE_S_EQUIPED_ITEM: {
...
AchievementMgr::AchievementMgr(Player *player) { m_player = player; }
...
void AchievementMgr::ResetAchievementCriteria(
  ...
  LOG_DEBUG("achievement",
            "AchievementMgr::ResetAchievementCriteria(%u, %u, %u)", condition,
            value, evenIfCriteriaComplete);
  ...
  for (AchievementCriteriaEntryList::const_iterator i =
       ...
       i != achievementCriteriaList->end(); ++i) {
    AchievementCriteriaEntry const *achievementCriteria = (*i);
    AchievementEntry const *achievement =
        sAchievementStore.LookupEntry(achievementCriteria->referredAchievement);
    if (!achievement)
      continue;
...
void AchievementMgr::SaveToDB(SQLTransaction &trans) {
  if (!m_completedAchievements.empty()) {
    for (CompletedAchievementMap::iterator iter =
      ...
      sScriptMgr->OnAchievementSave(trans, GetPlayer(), iter->first,
                                    iter->second);
    }
  }
  ...
  if (!m_criteriaProgress.empty()) {
    for (CriteriaProgressMap::iterator iter = m_criteriaProgress.begin();
      ...
      // pussywizard: insert only for (counter != 0) is very important! this is
      // how criteria of completed achievements gets deleted from db (by setting
      // counter to 0); if conflicted during merge - contact me
  ...
  if (achievementResult) {
    do {
      ...
      if (AchievementReward const *reward =
        ...
        if (uint32 titleId =
                reward->titleId[Player::TeamIdForRace(GetPlayer()->getRace())])
  ...
  if (criteriaResult) {
    do {
      ...
      if (!criteria) {
        ...
        LOG_ERROR("server",
                  "Non-existing achievement criteria %u data removed from "
      ...
      if (criteria->timeLimit &&
          time_t(date + criteria->timeLimit) < time(nullptr))
        continue;
...
void AchievementMgr::SendAchievementEarned(
  ...
  LOG_DEBUG("achievement", "AchievementMgr::SendAchievementEarned(%u)",
            achievement->ID);
#endif

  if (Guild *guild = sGuildMgr->GetGuildById(GetPlayer()->GetGuildId())) {
    Warhead::AchievementChatBuilder say_builder(
        *GetPlayer(), CHAT_MSG_GUILD_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED,
        achievement->ID);
    Warhead::LocalizedPacketDo<Warhead::AchievementChatBuilder> say_do(
        say_builder);
  ...
  if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_KILL |
    ...
    sWorld->SendGlobalMessage(
        &data, nullptr, teamId == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);
  }
  // if player is in world he can tell his friends about new achievement
  else if (GetPlayer()->IsInWorld()) {
    CellCoord p = Warhead::ComputeCellCoord(GetPlayer()->GetPositionX(),
    ...
    Warhead::AchievementChatBuilder say_builder(
        *GetPlayer(), CHAT_MSG_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED,
        achievement->ID);
    Warhead::LocalizedPacketDo<Warhead::AchievementChatBuilder> say_do(
        say_builder);
    Warhead::PlayerDistWorker<
        Warhead::LocalizedPacketDo<Warhead::AchievementChatBuilder>>
        say_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY),
                   say_do);
    TypeContainerVisitor<Warhead::PlayerDistWorker<Warhead::LocalizedPacketDo<
                             Warhead::AchievementChatBuilder>>,
...
void AchievementMgr::SendCriteriaUpdate(AchievementCriteriaEntry const *entry,
  ...
  if (!entry->timeLimit)
    ...
    data << uint32(
        timedCompleted ? 0 : 1); // 1 is for keeping the counter at 0 in client
...
static const uint32 achievIdForDungeon[][4] = {
    ...
    {321, true, true, true},    {916, false, true, false},
    {917, false, true, false},  {918, true, false, false},
    {2219, false, false, true}, {0, false, false, false}};
...
void AchievementMgr::UpdateAchievementCriteria(AchievementCriteriaTypes type,
  ...
  case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LINE:
    if (miscValue1) {
      achievementCriteriaList =
          sAchievementMgr->GetSpecialAchievementCriteriaByType(type,
                                                               miscValue1);
      break;
    }
    achievementCriteriaList =
  ...
  for (AchievementCriteriaEntryList::const_iterator i =
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_ACCEPTED_SUMMONINGS:
      // AchievementMgr::UpdateAchievementCriteria might also be called on login
      // - skip in this case
      if (!miscValue1)
        continue;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_GOLD_EARNED_BY_AUCTIONS: /* FIXME: for online
                                                               player only
                                                               currently */
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_AUCTION_SOLD: /* FIXME: for online
                                                            player only
                                                            currently */
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_DAILY_QUEST_DAILY: {
      ...
      if (!progress)
        ...
        // last progress is older than 2 days. Player missed 1 day => Retart
        // count.
        ...
        // last progress is within the day before the reset => Already counted
        // today.
        continue;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUESTS_IN_ZONE: {
      ...
      if (miscValue1 &&
          miscValue1 != achievementCriteria->complete_quests_in_zone.zoneID)
        continue;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_DEATH: {
      ...
      if (!miscValue1)
        continue;
      // skip wrong arena achievements, if not achievIdByArenaSlot then normal
      // total death counter
      bool notfit = false;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM:
      ...
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_LOOT_ITEM:
      // You _have_ to loot that item, just owning it when logging in does _not_
      // count!
      if (!miscValue1)
        continue;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_EXPLORE_AREA: {
      ...
      for (int j = 0; j < MAX_WORLD_MAP_OVERLAY_AREA_IDX; ++j) {
        AreaTableEntry const *area =
            sAreaTableStore.LookupEntry(worldOverlayEntry->areatableID[j]);
        if (!area)
          break;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS: {
      if (miscValue1 &&
          miscValue1 != achievementCriteria->learn_skillline_spell.skillLine)
        continue;

      uint32 spellCount = 0;
      for (PlayerSpellMap::const_iterator spellIter =
               GetPlayer()->GetSpellMap().begin();
           spellIter != GetPlayer()->GetSpellMap().end(); ++spellIter) {
        SkillLineAbilityMapBounds bounds =
            sSpellMgr->GetSkillLineAbilityMapBounds(spellIter->first);
        for (SkillLineAbilityMap::const_iterator skillIter = bounds.first;
             skillIter != bounds.second; ++skillIter) {
          if (skillIter->second->skillId ==
              achievementCriteria->learn_skillline_spell.skillLine) {
            // xinef: do not add couter twice if by any chance skill is listed
            // twice in dbc (eg. skill 777 and spell 22717)
            ++spellCount;
            break;
          }
        }
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL_AT_AREA: {
      if (!miscValue1 ||
          miscValue1 != achievementCriteria->honorable_kill_at_area.areaID)
        continue;
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_TEAM_RATING: {
      ...
      } else // login case
      {
        for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT; ++arena_slot) {
          ...
          SetCriteriaProgress(achievementCriteria, team->GetStats().Rating,
                              PROGRESS_HIGHEST);
          break;
        }
      }
      ...
      } else // login case
      {
        for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT; ++arena_slot) {
          ...
          if (ArenaTeamMember const *member =
            ...
            SetCriteriaProgress(achievementCriteria, member->PersonalRating,
                                PROGRESS_HIGHEST);
            break;
          }
        }
...
bool AchievementMgr::IsCompletedCriteria(
  ...
  // since not finding progress would start some timed achievements and probably
  // other things
...
bool AchievementMgr::IsCompletedAchievement(AchievementEntry const *entry) {
  ...
  for (AchievementCriteriaEntryList::const_iterator itr = cList->begin();
    ...
    // found an uncompleted criteria, but DONT return false yet - there might be
    // a completed criteria with ACHIEVEMENT_CRITERIA_COMPLETE_FLAG_ALL
    if (completed)
      ++count;
    else
      completed_all = false;
...
void AchievementMgr::SetCriteriaProgress(AchievementCriteriaEntry const *entry,
  ...
  if (!progress) {
    ...
    case PROGRESS_HIGHEST:
      newValue =
          progress->counter < changeValue ? changeValue : progress->counter;
      break;
    }
...
void AchievementMgr::RemoveCriteriaProgress(
  ...
  CriteriaProgressMap::iterator criteriaProgress =
      m_criteriaProgress.find(entry->ID);
...
void AchievementMgr::CompletedAchievement(AchievementEntry const *achievement) {
  ...
  if (m_player->IsGameMaster()) {
    ...
    ChatHandler(m_player->GetSession())
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "AchievementMgr::CompletedAchievement(%u)",
            achievement->ID);
#endif
  ...
  if (achievement->flags & (ACHIEVEMENT_FLAG_REALM_FIRST_REACH |
                            ACHIEVEMENT_FLAG_REALM_FIRST_KILL) &&
      AccountMgr::IsPlayerAccount(m_player->GetSession()->GetSecurity()))
  ...
  //! titleRewardFlags field) we explicitly check by ID. Maybe in the future we
  //! could move the achievement_reward condition fields to the condition
  //! system.
  ...
  if (reward->sender) {
    ...
    if (!reward->mailTemplate) {
      ...
      if (localeConstant != LOCALE_enUS) {
        if (AchievementRewardLocale const *loc =
                sAchievementMgr->GetAchievementRewardLocale(achievement)) {
    ...
    if (item) {
      ...
      item->SaveToDB(trans); // save for prevent lost at next mail load, if send
...
void AchievementMgr::SendAllAchievementData() const {
  WorldPacket data(SMSG_ALL_ACHIEVEMENT_DATA,
                   m_completedAchievements.size() * 8 + 4 +
                       m_criteriaProgress.size() * 38 + 4);
...
void AchievementMgr::SendRespondInspectAchievements(Player *player) const {
  WorldPacket data(SMSG_RESPOND_INSPECT_ACHIEVEMENTS,
                   9 + m_completedAchievements.size() * 8 + 4 + 4);
  data.append(GetPlayer()->GetPackGUID());
...
bool AchievementMgr::HasAchieved(uint32 achievementId) const {
  return m_completedAchievements.find(achievementId) !=
         m_completedAchievements.end();
}

bool AchievementMgr::CanUpdateCriteria(AchievementCriteriaEntry const *criteria,
                                       AchievementEntry const *achievement) {
  if (DisableMgr::IsDisabledFor(DISABLE_TYPE_ACHIEVEMENT_CRITERIA, criteria->ID,
                                nullptr))
    return false;
  ...
  if ((achievement->requiredFaction == ACHIEVEMENT_FACTION_HORDE &&
       GetPlayer()->GetTeamId() != TEAM_HORDE) ||
      (achievement->requiredFaction == ACHIEVEMENT_FACTION_ALLIANCE &&
       GetPlayer()->GetTeamId() != TEAM_ALLIANCE))
    return false;
...
bool AchievementGlobalMgr::IsRealmCompleted(
  ...
  // Allow completing the realm first kill for entire minute after first person
  // did it it may allow more than one group to achieve it (highly unlikely) but
  // apparently this is how blizz handles it as well
  if (achievement->flags & ACHIEVEMENT_FLAG_REALM_FIRST_KILL)
    return (std::chrono::system_clock::now() - itr->second) >
...
void AchievementGlobalMgr::SetRealmCompleted(
  ...
  m_allCompletedAchievements[achievement->ID] =
...
void AchievementGlobalMgr::LoadAchievementCriteriaList() {
  ...
  for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows();
    ...
    m_AchievementCriteriaListByAchievement[criteria->referredAchievement]
        .push_back(criteria);
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE:
      ...
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_WIN_BG:
      m_SpecialList[criteria->requiredType][criteria->win_bg.bgMapID].push_back(
          criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_REACH_SKILL_LEVEL:
      m_SpecialList[criteria->requiredType][criteria->reach_skill_level.skillID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ACHIEVEMENT:
      m_SpecialList[criteria->requiredType]
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_CREATURE:
      ...
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUEST:
      m_SpecialList[criteria->requiredType][criteria->complete_quest.questID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET:
      m_SpecialList[criteria->requiredType][criteria->be_spell_target.spellID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL:
      m_SpecialList[criteria->requiredType][criteria->cast_spell.spellID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE:
      m_SpecialList[criteria->requiredType][criteria->bg_objective.objectiveId]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL_AT_AREA:
      m_SpecialList[criteria->requiredType]
      ...
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SPELL:
      m_SpecialList[criteria->requiredType][criteria->learn_spell.spellID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM:
      m_SpecialList[criteria->requiredType][criteria->own_item.itemID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LEVEL:
      m_SpecialList[criteria->requiredType][criteria->learn_skill_level.skillID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_USE_ITEM:
      m_SpecialList[criteria->requiredType][criteria->use_item.itemID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_LOOT_ITEM:
      m_SpecialList[criteria->requiredType][criteria->own_item.itemID]
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION:
      m_SpecialList[criteria->requiredType][criteria->gain_reputation.factionID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_EQUIP_EPIC_ITEM:
      m_SpecialList[criteria->requiredType][criteria->equip_epic_item.itemSlot]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_HK_CLASS:
      m_SpecialList[criteria->requiredType][criteria->hk_class.classID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_HK_RACE:
      m_SpecialList[criteria->requiredType][criteria->hk_race.raceID].push_back(
          criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_DO_EMOTE:
      m_SpecialList[criteria->requiredType][criteria->do_emote.emoteID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_EQUIP_ITEM:
      m_SpecialList[criteria->requiredType][criteria->equip_item.itemID]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_USE_GAMEOBJECT:
      m_SpecialList[criteria->requiredType][criteria->use_gameobject.goEntry]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2:
      m_SpecialList[criteria->requiredType][criteria->be_spell_target.spellID]
    ...
    case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS:
      ...
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_LOOT_TYPE:
      m_SpecialList[criteria->requiredType][criteria->loot_type.lootType]
          .push_back(criteria);
      break;
    case ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL2:
      m_SpecialList[criteria->requiredType][criteria->cast_spell.spellID]
  ...
  LOG_INFO("server", ">> Loaded %u achievement criteria in %u ms", loaded,
...
void AchievementGlobalMgr::LoadAchievementReferenceList() {
  ...
  LOG_INFO("server", ">> Loaded %u achievement references in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void AchievementGlobalMgr::LoadAchievementCriteriaData() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 additional achievement criteria data. DB "
                       "table `achievement_criteria_data` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    if (!criteria) {
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` has data for non-existing "
                "criteria (Entry: %u), ignore.",
                criteria_id);
      continue;
    }
    ...
    if (scriptName.length()) // not empty
    {
      if (dataType != ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT)
        LOG_ERROR("server",
                  "Table `achievement_criteria_data` has ScriptName set for "
                  "non-scripted data type (Entry: %u, type %u), useless data.",
  ...
  for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows();
    ...
    if (!GetCriteriaDataSet(criteria) &&
        !DisableMgr::IsDisabledFor(DISABLE_TYPE_ACHIEVEMENT_CRITERIA, entryId,
                                   nullptr))
      LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` does not have expected data "
                "for criteria (Entry: %u Type: %u) for achievement %u.",
                criteria->ID, criteria->requiredType,
...
void AchievementGlobalMgr::LoadCompletedAchievements() {
  ...
  // Populate _allCompletedAchievements with all realm first achievement ids to
  // make multithreaded access safer while it will not prevent races, it will
  // prevent crashes that happen because std::unordered_map key was added
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 completed achievements. DB table "
  ...
  do {
    ...
    if (!achievement) {
      ...
      LOG_ERROR("server",
                "Non-existing achievement %u data removed from table "
      ...
      continue;
                                     ...
                                     ACHIEVEMENT_FLAG_REALM_FIRST_KILL))
      m_allCompletedAchievements[achievementId] =
          std::chrono::system_clock::time_point::max();
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %lu completed achievements in %u ms",
...
void AchievementGlobalMgr::LoadRewards() {
  ...
  //                                               5        6     7
  QueryResult result =
      WorldDatabase.Query("SELECT ID, TitleA, TitleH, ItemID, Sender, Subject, "
                          "Body, MailTemplateID FROM achievement_reward");

  if (!result) {
    LOG_ERROR("sql.sql", ">> Loaded 0 achievement rewards. DB table "
  ...
  do {
    ...
    if (!achievement) {
      LOG_ERROR("sql.sql",
                "Table `achievement_reward` has wrong achievement (Entry: %u). "
                "Ignoring.",
                entry);
      continue;
    }
    ...
    reward.sender = fields[4].GetUInt32(); // The sender of the mail (a creature
    ...
    if (!reward.titleId[0] && !reward.titleId[1] && !reward.sender) {
      LOG_ERROR("sql.sql",
                "Table `achievement_reward` (Entry: %u) does not have any "
                "title or item reward data. Ignoring.",
                entry);
      continue;
    }
    ...
    if (achievement->requiredFaction == ACHIEVEMENT_FACTION_ANY &&
      ...
      LOG_DEBUG("achievement",
                "Table `achievement_reward` (Entry: %u) has title (A: %u H: "
                "%u) set for only one team.",
    ...
    if (reward.titleId[0]) {
      ...
      if (!titleEntry) {
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has invalid title id "
                  "(%u) in `title_A`. Setting it to 0.",
    ...
    if (reward.sender) {
      if (!sObjectMgr->GetCreatureTemplate(reward.sender)) {
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has invalid "
                  "creature_template entry %u as Sender. Will not send the "
                  "mail reward.",
                  entry, reward.sender);
        reward.sender = 0;
      }
      ...
      if (reward.itemId)
        LOG_ERROR(
            "sql.sql",
            "Table `achievement_reward` (Entry: %u) has itemId reward set but "
            "does not have Sender data set. Item will not be sent.",
            entry);

      if (!reward.subject.empty())
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has mail Subject but "
                  "does not have Sender data set.",
                  entry); // Maybe add "Mail will not be sent." ?

      if (!reward.text.empty())
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has mail text (Body) "
                  "set but does not have Sender data set.",
                  entry); // Maybe add "Mail will not be sent." ?

      if (reward.mailTemplate)
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has mailTemplate set "
                  "does not have Sender data set.",
                  entry); // Maybe add "Mail will not be sent." ?
    }
    ...
    if (reward.mailTemplate) {
      if (!sMailTemplateStore.LookupEntry(reward.mailTemplate)) {
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has invalid "
                  "mailTemplate (%u) (check the DBC).",
                  entry, reward.mailTemplate);
        reward.mailTemplate = 0;
      } else if (!reward.subject.empty() || !reward.text.empty())
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has mailTemplate "
                  "(%u) and mail Subject/Body. To use the column mailTemplate, "
                  "Subject and Body must be empty.",
                  entry, reward.mailTemplate);
    }

    if (reward.itemId) {
      if (!sObjectMgr->GetItemTemplate(reward.itemId)) {
        // Not sure it's an error, it's probably an outDebug instead, because we
        // can simply send a mail with no reward, right?
        LOG_ERROR("sql.sql",
                  "Table `achievement_reward` (Entry: %u) has invalid "
                  "item_template id %u. Reward mail will not contain any item.",
                  entry, reward.itemId);
        reward.itemId = 0;
      }
    }
  ...
  LOG_INFO("server", ">> Loaded %u achievement rewards in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void AchievementGlobalMgr::LoadRewardLocales() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 achievement reward locale strings.  DB "
                       "table `achievement_reward_locale` is empty");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    if (m_achievementRewards.find(ID) == m_achievementRewards.end()) {
      LOG_ERROR("sql.sql",
                "Table `achievement_reward_locale` (Entry: %u) has locale "
                "strings for non-existing achievement reward.",
                ID);
      continue;
    }
  ...
  LOG_INFO("server", ">> Loaded %lu Achievement Reward Locale strings in %u ms",
