    ...
    if (result) {
      do {
        ...
        SetDataForCommandInTable(commandTableCache, name.c_str(),
                                 fields[1].GetUInt8(), fields[2].GetString(),
                                 name);
  ...
  for (uint32 i = 0; i < table.size(); ++i) {
...
#ifdef ELUNA
        if (!sEluna->OnCommand(
                GetSession() ? GetSession()->GetPlayer() : nullptr, oldtext))
          return true;
#endif
        if (text[0] != '\0')
    ...
    if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext)) {
      ...
      if (!AccountMgr::IsPlayerAccount(m_session->GetSecurity())) {
        ...
        LOG_GM(m_session->GetAccountId(),
               "Command: %s [Player: %s (%u) (Account: %u) X: %f Y: %f Z: %f "
               "Map: %u (%s) Area: %u (%s) Zone: %s Selected: %s (%ul)]",
               fullcmd.c_str(), player->GetName().c_str(),
               GUID_LOPART(player->GetGUID()), m_session->GetAccountId(),
               player->GetPositionX(), player->GetPositionY(),
               player->GetPositionZ(), player->GetMapId(),
               player->GetMap()->GetMapName(), areaId, areaName.c_str(),
               zoneName.c_str(),
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand> &table,
                                           char const *text, uint32 security,
  ...
  for (uint32 i = 0; i < table.size(); i++) {
    ...
    if (!table[i].ChildCommands.empty()) {
      if (SetDataForCommandInTable(table[i].ChildCommands, text, security, help,
                                   fullcommand))
        return true;
      else if (*text)
        return false;
    ...
    //    LOG_DEBUG("server", "Table `command` overwrite for command '%s'
    //    default security (%u) by %u", fullcommand.c_str(),
  ...
  if (!cmd.empty()) {
    if (&table == &getCommandTable())
      LOG_ERROR("server",
                "Table `command` have non-existing command '%s', skip.",
                cmd.c_str());
    else
      LOG_ERROR("server",
                "Table `command` have non-existing subcommand '%s' in command "
                "'%s', skip.",
...
bool ChatHandler::ParseCommands(char const *text) {
  ...
  if (strlen(text) < 2)
    return false;
  // original `text` can't be used. It content destroyed in command code
  // processing.
  ...
  /// skip first . or ! (in console allowed use command with . and ! and without
  /// its)
  if (text[0] == '!' || text[0] == '.')
    ++text;
...
#ifdef ELUNA
    if (!sEluna->OnCommand(GetSession() ? GetSession()->GetPlayer() : nullptr,
                           text))
      return true;
#endif
    if (m_session && AccountMgr::IsPlayerAccount(m_session->GetSecurity()))
...
size_t ChatHandler::BuildChatPacket(
    WorldPacket &data, ChatMsg chatType, Language language, uint64 senderGUID,
    uint64 receiverGUID, std::string const &message, uint8 chatTag,
...
size_t ChatHandler::BuildChatPacket(
    ...
    WorldObject const *sender, WorldObject const *receiver,
    std::string const &message, uint32 achievementId /*= 0*/,
  ...
  return BuildChatPacket(data, chatType, language, senderGUID, receiverGUID,
                         ...
                         achievementId, gmMessage, channelName);
}
...
GameObject *ChatHandler::GetNearbyGameObject() {
  ...
  Warhead::GameObjectLastSearcher<Warhead::NearestGameObjectCheck> searcher(
      pl, obj, check);
...
ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid,
  ...
  GameObject *obj = pl->GetMap()->GetGameObject(
      MAKE_NEW_GUID(lowguid, entry, HIGHGUID_GAMEOBJECT));

  if (!obj && sObjectMgr->GetGOData(lowguid)) // guid is DB guid of object
  {
    // search near player then
    CellCoord p(
        Warhead::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
    Cell cell(p);
...
bool ChatHandler::extractPlayerTarget(char *args, Player **player,
  ...
  if (args && *args) {
    ...
    uint64 guid = !pl && (player_guid || player_name)
