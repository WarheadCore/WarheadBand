...
// client, at shift click in glyphs dialog, GlyphSlot.dbc, GlyphProperties.dbc
...
// client, in some messages, at click copy only name instead link
// |color|Hquest:quest_id:quest_level|h[name]|h|r                         -
// client, quest list name shift-click |color|Hskill:skill_id|h[name]|h|r
// |color|Hspell:spell_id|h[name]|h|r                                     -
// client, spellbook spell icon shift-click |color|Htalent:talent_id,
// rank|h[name]|h|r                              - client, talent icon
// shift-click |color|Htaxinode:id|h[name]|h|r |color|Htele:id|h[name]|h|r
// |color|Htitle:id|h[name]|h|r
// |color|Htrade:spell_id:cur_value:max_value:unk3int:unk3str|h[name]|h|r -
// client, spellbook profession icon shift-click
...
inline bool CheckDelimiter(std::istringstream &iss, char delimiter,
                           const char *context) {
  ...
  if (c != delimiter) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): invalid %s link "
              "structure ('%c' expected, '%c' found)",
...
bool ItemChatLink::Initialize(std::istringstream &iss) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading item entry",
              iss.str().c_str());
#endif
    return false;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): got invalid itemEntry %u "
              "in |item command",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): linked item has color "
              "%u, but user claims %u",
              iss.str().c_str(), ItemQualityColors[_item->Quality], _color);
#endif
    return false;
  }
  ...
  for (uint8 index = 0; index < propsCount; ++index) {
    ...
    if (!ReadInt32(iss, id)) {
      ...
      LOG_DEBUG("chat.system",
                "ChatHandler::isValidChatMessage('%s'): sequence finished "
                "unexpectedly while reading item property (%u)",
                iss.str().c_str(), index);
#endif
      return false;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("chat.system",
                    "ChatHandler::isValidChatMessage('%s'): got invalid item "
                    "property id %u in |item command",
                    iss.str().c_str(), id);
#endif
          return false;
        }
...
inline std::string ItemChatLink::FormatName(uint8 index,
                                            ItemLocale const *locale,
                                            char *const *suffixStrings) const {
  std::stringstream ss;
...
bool ItemChatLink::ValidateName(char *buffer, const char *context) {
  ...
  char *const *suffixStrings =
      _suffix ? _suffix->nameSuffix
              : (_property ? _property->nameSuffix : nullptr);
  ...
  if (!res)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): linked item (id: %u) "
              "name wasn't found in any localization",
              context, _item->ItemId);
#endif
  return res;
}
...
bool QuestChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, questId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading quest entry",
              iss.str().c_str());
#endif
    return false;
  }
  ...
  if (!_quest) {
    ...
    LOG_DEBUG(
        "chat.system",
        "ChatHandler::isValidChatMessage('%s'): quest template %u not found",
        iss.str().c_str(), questId);
#endif
    return false;
  }
  // Check delimiter
  ...
  if (!ReadInt32(iss, _questLevel)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading quest level",
              iss.str().c_str());
#endif
    return false;
  }
  ...
  if (_questLevel >= STRONG_MAX_LEVEL) {
    ...
    LOG_DEBUG(
        "chat.system",
        "ChatHandler::isValidChatMessage('%s'): quest level %d is too big",
        iss.str().c_str(), _questLevel);
#endif
    return false;
  }
  return true;
...
bool QuestChatLink::ValidateName(char *buffer, const char *context) {
  ...
  if (!res)
    if (QuestLocale const *ql =
            sObjectMgr->GetQuestLocale(_quest->GetQuestId()))
  ...
  if (!res)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): linked quest (id: %u) "
              "title wasn't found in any localization",
              context, _quest->GetQuestId());
#endif
  return res;
}
...
bool SpellChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, spellId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading spell entry",
              iss.str().c_str());
#endif
    return false;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u "
              "in |spell command",
...
bool SpellChatLink::ValidateName(char *buffer, const char *context) {
  ...
  if (_spell->HasAttribute(SPELL_ATTR0_TRADESPELL)) {
    SkillLineAbilityMapBounds bounds =
        sSpellMgr->GetSkillLineAbilityMapBounds(_spell->Id);
    if (bounds.first == bounds.second) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("chat.system",
                "ChatHandler::isValidChatMessage('%s'): skill line not found "
                "for spell %u",
                context, _spell->Id);
#endif
      return false;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("chat.system",
                "ChatHandler::isValidChatMessage('%s'): skill line ability not "
                "found for spell %u",
                context, _spell->Id);
#endif
      return false;
  ...
  if (!res)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): linked spell (id: %u) "
              "name wasn't found in any localization",
              context, _spell->Id);
#endif
  return res;
...
bool AchievementChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, achievementId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading achievement entry",
              iss.str().c_str());
#endif
    return false;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): got invalid achivement "
              "id %u in |achievement command",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): invalid hexadecimal "
              "number while reading char's guid",
              iss.str().c_str());
#endif
    return false;
  }
  ...
  for (uint8 index = 0; index < propsCount; ++index) {
    ...
    if (!ReadUInt32(iss, _data[index])) {
      ...
      LOG_DEBUG("chat.system",
                "ChatHandler::isValidChatMessage('%s'): sequence finished "
                "unexpectedly while reading achievement property (%u)",
                iss.str().c_str(), index);
#endif
      return false;
    }
...
bool AchievementChatLink::ValidateName(char *buffer, const char *context) {
  ...
  if (!res)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): linked achievement (id: "
              "%u) name wasn't found in any localization",
              context, _achievement->ID);
#endif
  return res;
}
...
bool TradeChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadInt32(iss, _minSkillLevel)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading minimum talent level",
              iss.str().c_str());
#endif
    return false;
  }
  ...
  if (!ReadInt32(iss, _maxSkillLevel)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading maximum talent level",
              iss.str().c_str());
#endif
    return false;
  }
  ...
  if (!ReadHex(iss, _guid, 0)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading achievement's owner guid",
              iss.str().c_str());
#endif
    return false;
  }
...
bool TalentChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, _talentId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading talent entry",
              iss.str().c_str());
#endif
    return false;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): got invalid talent id %u "
              "in |talent command",
  ...
  if (!_spell) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u "
              "in |trade command",
              iss.str().c_str(), talentInfo->RankID[0]);
#endif
    return false;
  }
  // Delimiter
  ...
  if (!ReadInt32(iss, _rankId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading talent rank",
              iss.str().c_str());
#endif
    return false;
  }
...
bool EnchantmentChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, spellId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading enchantment spell entry",
              iss.str().c_str());
#endif
    return false;
  }
...
bool GlyphChatLink::Initialize(std::istringstream &iss) {
  ...
  if (!ReadUInt32(iss, _slotId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading slot id",
              iss.str().c_str());
#endif
    return false;
  }
  ...
  if (!ReadUInt32(iss, glyphId)) {
    ...
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): sequence finished "
              "unexpectedly while reading glyph entry",
              iss.str().c_str());
#endif
    return false;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("chat.system",
              "ChatHandler::isValidChatMessage('%s'): got invalid glyph id %u "
              "in |glyph command",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("chat.system",
                "ChatHandler::isValidChatMessage('%s'): sequence aborted "
                "unexpectedly",
                _iss.str().c_str());
#endif
      return false;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("chat.system",
                  "ChatHandler::isValidChatMessage('%s'): invalid sequence, "
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("chat.system",
                  "ChatHandler::isValidChatMessage('%s'): user sent "
                  "unsupported link type '%s'",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("chat.system",
                    "ChatHandler::isValidChatMessage('%s'): link caption "
                    "doesn't start with '['",
                    _iss.str().c_str());
#endif
          return false;
