...
Channel::Channel(std::string const &name, uint32 channelId, uint32 channelDBId,
                 TeamId teamId, bool announce, bool ownership)
    : _announce(announce), _ownership(ownership), _IsSaved(false),
      _isOwnerGM(false), _flags(0), _channelId(channelId),
      _channelDBId(channelDBId), _teamId(teamId), _ownerGUID(0), _name(name),
      _password("") {
...
void Channel::JoinChannel(Player *player, std::string const &pass) {
  ...
  if (!IsConstant()) {
    ...
    if (_channelRights.moderators.find(player->GetSession()->GetAccountId()) !=
    ...
    // owner. If the channel owner is a GM and the config SilentGMJoinChannel is
    // enabled, set the new owner
    if ((!_ownerGUID ||
         (_isOwnerGM &&
          sWorld->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL))) &&
        _ownership) {
...
void Channel::LeaveChannel(Player *player, bool send) {
  ...
  if (!IsConstant()) {
    ...
    // If the channel owner left and there are still playersStore inside, pick a
    // new owner
    if (changeowner && _ownership) {
      if (!playersStore.empty()) {
        ...
        for (Channel::PlayerContainer::const_iterator itr =
                 playersStore.begin();
             itr != playersStore.end(); ++itr) {
          newowner = itr->second.player;
          if (AccountMgr::IsGMAccount(
                  itr->second.plrPtr->GetSession()->GetSecurity()))
            _isOwnerGM = true;
          else
            _isOwnerGM = false;
...
void Channel::KickOrBan(Player const *player, std::string const &badname,
    ...
    if (ban && (AccountMgr::IsGMAccount(sec) || isGoodConstantModerator)) {
      if (uint32 lowGuid = sWorld->GetGlobalPlayerGUID(badname))
        if (const GlobalPlayerData *gpd =
                sWorld->GetGlobalPlayerData(lowGuid)) {
  ...
  if (ban) {
    if (!IsBanned(victim)) {
      ...
      AddChannelBanToDB(GUID_LOPART(victim),
                        time(nullptr) + CHANNEL_BAN_DURATION);
  ...
  if (changeowner && _ownership) {
    if (good != victim)
      ...
      uint64 newowner = 0;
      for (Channel::PlayerContainer::const_iterator itr = playersStore.begin();
           itr != playersStore.end(); ++itr) {
        newowner = itr->second.player;
        if (!itr->second.plrPtr->GetSession()->GetSecurity())
          break;
...
void Channel::UnBan(Player const *player, std::string const &badname) {
  ...
  if (_channelRights.flags & CHANNEL_RIGHT_CANT_BAN)
    LOG_GM(player->GetSession()->GetAccountId(),
           "Command: /unban %s %s (Moderator %s [guid: %u, account: %u] "
           "unbanned %s [guid: %u])",
           GetName().c_str(), badname.c_str(), player->GetName().c_str(),
           player->GetGUIDLow(), player->GetSession()->GetAccountId(),
           badname.c_str(), GUID_LOPART(victim));
...
void Channel::SetMode(Player const *player, std::string const &p2n, bool mod,
  ...
  if (mod) {
    bool isBadConstantModerator =
        _channelRights.moderators.find(newp->GetSession()->GetAccountId()) !=
        _channelRights.moderators.end();
...
#endif
  WorldPacket data(SMSG_CHANNEL_LIST, 1 + (GetName().size() + 1) + 1 + 4 +
...
void Channel::Say(uint64 guid, std::string const &what, uint32 lang) {
  ...
  if (player && player->GetSession()->GetSecurity() ==
                    AccountTypes::SEC_PLAYER) // pussywizard: prevent spam on
                                              // populated channels
  {
    uint32 speakDelay = 0;
    ...
    if (!pinfo.IsAllowedToSpeak(speakDelay)) {
      std::string timeStr = secsToTimeString(pinfo.lastSpeakTime + speakDelay -
                                             sWorld->GetGameTime());
      if (_channelRights.speakMessage.length() > 0)
        player->GetSession()->SendNotification(
            "%s", _channelRights.speakMessage.c_str());
      player->GetSession()->SendNotification(
          "You must wait %s before speaking again.", timeStr.c_str());
      return;
    }
  }
  ...
  WorldPacket data;
  if (player)
    ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), player,
                                 player, what, 0, _name);
  else
    ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), guid,
                                 guid, what, 0, "", "", 0, false, _name);
...
void Channel::EveryoneSayToSelf(const char *what) {
  ...
  WorldPacket data(SMSG_MESSAGECHAT, 1 + 4 + 8 + 4 + _name.size() + 1 + 8 + 4 +
                                         messageLength + 1);
...
void Channel::SetOwner(uint64 guid, bool exclaim) {
  ...
  if (_ownerGUID) {
    ...
    if (player) {
      uint32 sec = player->GetSession()->GetSecurity();
      notify = !(AccountMgr::IsGMAccount(sec) &&
                 sWorld->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL));
    }
