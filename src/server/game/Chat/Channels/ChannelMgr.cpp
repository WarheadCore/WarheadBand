...
void ChannelMgr::LoadChannels() {
  ...
  QueryResult result = CharacterDatabase.PQuery(
      ...
      "channels WHERE team = %u ORDER BY channelId ASC",
      _teamId);
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 channels for %s",
             _teamId == TEAM_ALLIANCE ? "Alliance" : "Horde");
    return;
  }
  ...
  do {
    ...
    Channel *newChannel =
        new Channel(channelName, 0, channelDBId, TeamId(fields[2].GetUInt32()),
                    fields[3].GetUInt8(), fields[4].GetUInt8());
    newChannel->SetPassword(password);
    channels[channelWName] = newChannel;

    if (QueryResult banResult =
            CharacterDatabase.PQuery("SELECT playerGUID, banTime FROM "
                                     "channels_bans WHERE channelId = %u",
                                     channelDBId)) {
      do {
  ...
  LOG_INFO("server", ">> Loaded %u channels for %s in %ums", count,
           _teamId == TEAM_ALLIANCE ? "Alliance" : "Horde",
...
Channel *ChannelMgr::GetChannel(std::string const &name, Player *player,
                                bool pkt) {
  std::wstring wname;
  Utf8toWStr(name, wname);
  wstrToLower(wname);
...
void ChannelMgr::LoadChannelRights() {
  ...
  QueryResult result =
      CharacterDatabase.Query("SELECT name, flags, speakdelay, joinmessage, "
                              "delaymessage, moderators FROM channels_rights");
  if (!result) {
  ...
  do {
    ...
    if (moderatorList) {
      ...
      for (Tokenizer::const_iterator i = tokens.begin(); i != tokens.end();
           ++i) {
    ...
    SetChannelRightsFor(fields[0].GetString(), fields[1].GetUInt32(),
                        fields[2].GetUInt32(), fields[3].GetString(),
                        fields[4].GetString(), moderators);

    ++count;
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %d Channel Rights in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
