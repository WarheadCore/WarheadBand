...
bool PetAI::_needToStop() {
  // This is needed for charmed creatures, as once their target was reset other
  // effects can trigger threat
...
void PetAI::_stopAttack() {
  if (!me->IsAlive()) {
    ...
    LOG_DEBUG("server", "Creature stoped attacking cuz his dead [guid=%u]",
              me->GetGUIDLow());
#endif
...
void PetAI::UpdateAI(uint32 diff) {
  ...
  if (me->GetVictim() && me->GetVictim()->IsAlive()) {
    ...
    if (_needToStop()) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "Pet AI stopped attacking [guid=%u]",
                me->GetGUIDLow());
#endif
      _stopAttack();
      return;
    ...
    if (me->GetCharmInfo()->HasCommandState(COMMAND_STAY)) {
      ...
      _doMeleeAttack();
  } else if (!me->GetCharmInfo() || (!me->GetCharmInfo()->GetForcedSpell() &&
                                     !me->HasUnitState(UNIT_STATE_CASTING))) {
    if (me->HasReactState(REACT_AGGRESSIVE) || me->GetCharmInfo()->IsAtStay()) {
      // Every update we need to check targets only in certain cases
      // Aggressive - Allow auto select if owner or pet don't have a target
      // Stay - Only pick from pet or owner targets / attackers so targets won't
      // run by
      //   while chasing our owner. Don't do auto select.
      // All other cases (ie: defensive) - Targets are assigned by AttackedBy(),
      // OwnerAttackedBy(), OwnerAttacked(), etc.
  ...
  if (!me->HasUnitState(UNIT_STATE_CASTING)) {
    if (owner && owner->GetTypeId() == TYPEID_PLAYER &&
      ...
      owner->ToPlayer()->GetSession()->HandlePetActionHelper(
          me, me->GetGUID(), abs(me->GetCharmInfo()->GetForcedSpell()),
          ACT_ENABLED, me->GetCharmInfo()->GetForcedTarget());

      // xinef: if spell was casted properly and we are in passive mode, handle
      // return
      ...
      if (spellInfo->IsPositive()) {
        ...
        if (!spellUsed) {
          for (std::set<uint64>::const_iterator tar = m_AllySet.begin();
            ...
            // only buff targets that are in combat, unless the spell can only
            // be cast while out of combat
            if (!ally)
              continue;
...
void PetAI::UpdateAllies() {
  ...
  m_updateAlliesTimer =
      10 * IN_MILLISECONDS; // update friendly targets every 10 seconds, lesser
                            // checks increase performance
...
void PetAI::KilledUnit(Unit *victim) {
  ...
  if (me->HasUnitState(UNIT_STATE_CASTING) &&
      ...
      me->GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT) != victim->GetGUID())
    return;
...
void PetAI::OwnerAttackedBy(Unit *attacker) {
  // Called when owner takes damage. This function helps keep pets from running
  // off
...
Unit *PetAI::SelectNextTarget(bool allowAutoSelect) const {
  ...
  // _CanAttack() The parameter: allowAutoSelect lets us disable aggressive pet
  // auto targeting for certain situations
  ...
  if (Unit *myAttacker = me->getAttackerForHelper())
    if (!myAttacker->HasBreakableByDamageCrowdControlAura() &&
        me->_CanDetectFeignDeathOf(myAttacker) &&
    ...
    if (!tauntAuras.empty())
      for (Unit::AuraEffectList::const_reverse_iterator itr =
        ...
        if (Unit *caster = (*itr)->GetCaster())
          if (me->_CanDetectFeignDeathOf(caster) &&
  ...
  if (Unit *ownerAttacker = owner->getAttackerForHelper())
    if (!ownerAttacker->HasBreakableByDamageCrowdControlAura() &&
        me->_CanDetectFeignDeathOf(ownerAttacker) &&
  ...
  if (Unit *ownerVictim = owner->GetVictim())
    if (me->_CanDetectFeignDeathOf(ownerVictim) &&
        me->CanCreatureAttack(ownerVictim))
      return ownerVictim;
  ...
  if (me->GetCharmInfo()->HasCommandState(COMMAND_STAY)) {
    if (!me->GetCharmInfo()->IsAtStay() && !me->GetCharmInfo()->IsReturning()) {
      if (me->GetCharmInfo()->HasStayPosition()) {
        ...
        if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_CONTROLLED) ==
            NULL_MOTION_TYPE) {
          float x, y, z;
          ...
          me->GetMotionMaster()->MovePoint(
              me->GetUInt32Value(OBJECT_FIELD_GUID), x, y, z);
        }
      }
    }
  } else // COMMAND_FOLLOW
    ...
    if (!me->GetCharmInfo()->IsFollowing() &&
      ...
      if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_CONTROLLED) ==
        ...
        me->GetMotionMaster()->MoveFollow(
            me->GetCharmerOrOwner(), PET_FOLLOW_DIST, me->GetFollowAngle());
      }
    }
  }
...
bool PetAI::CanAttack(Unit *target, const SpellInfo *spellInfo) {
  // Evaluates wether a pet can attack a specific target based on CommandState,
  // ReactState and other flags IMPORTANT: The order in which things are checked
  // is important, be careful if you add or remove checks
  ...
  if (!me->GetCharmInfo()) {
    LOG_INFO("misc", "PetAI::CanAttack (A1) - %u, %u", me->GetEntry(),
             GUID_LOPART(me->GetOwnerGUID()));
    return false;
  }
  ...
  if (target->HasBreakableByDamageCrowdControlAura() &&
      (!spellInfo ||
       !spellInfo->HasAttribute(SPELL_ATTR4_DAMAGE_DOESNT_BREAK_AURAS)))
    return me->GetCharmInfo()->IsCommandAttack();
  ...
  //  Pets attacking something (or chasing) should only switch targets if owner
  //  tells them to
