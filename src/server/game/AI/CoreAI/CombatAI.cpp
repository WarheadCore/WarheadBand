...
void CombatAI::EnterCombat(Unit *who) {
  for (SpellVct::iterator i = spells.begin(); i != spells.end(); ++i) {
    if (AISpellInfo[*i].condition == AICOND_AGGRO)
      ...
      events.ScheduleEvent(*i, AISpellInfo[*i].cooldown +
...
void CombatAI::UpdateAI(uint32 diff) {
  ...
  if (uint32 spellId = events.ExecuteEvent()) {
    DoCast(spellId);
    events.ScheduleEvent(spellId, AISpellInfo[spellId].cooldown +
...
void CasterAI::UpdateAI(uint32 diff) {
  ...
  if (uint32 spellId = events.ExecuteEvent()) {
    ...
    events.ScheduleEvent(spellId, (casttime ? casttime : 500) +
...
ArcherAI::ArcherAI(Creature *c) : CreatureAI(c) {
  if (!me->m_spells[0])
    LOG_ERROR("server",
              "ArcherAI set for creature (entry = %u) with spell1=0. AI will "
              "do nothing",
              me->GetEntry());
...
TurretAI::TurretAI(Creature *c) : CreatureAI(c) {
  if (!me->m_spells[0])
    LOG_ERROR("server",
              "TurretAI set for creature (entry = %u) with spell1=0. AI will "
              "do nothing",
              me->GetEntry());
...
void VehicleAI::LoadConditions() {
  ...
  if (!conditions.empty())
    LOG_DEBUG("condition", "VehicleAI::LoadConditions: loaded %u conditions",
