...
void UnitAI::DoMeleeAttackIfReady() {
  ...
  if (me->haveOffhandWeapon() && me->isAttackReady(OFF_ATTACK)) {
    // xinef: delay main hand attack if both will hit at the same time (players
    // code)
...
void UnitAI::SelectTargetList(std::list<Unit *> &targetList, uint32 num,
                              SelectAggroTarget targetType, float dist,
                              bool playerOnly, int32 aura) {
  SelectTargetList(targetList,
                   DefaultTargetSelector(me, dist, playerOnly, aura), num,
                   targetType);
}
...
void UnitAI::DoCast(uint32 spellId) {
  Unit *target = nullptr;
  // LOG_ERROR("server", "aggre %u %u", spellId,
  // (uint32)AISpellInfo[spellId].target);
  ...
  case AITARGET_ENEMY: {
    ...
    target = SelectTarget(SELECT_TARGET_RANDOM, 0,
                          spellInfo->GetMaxRange(false), playerOnly);
    break;
  }
  case AITARGET_ALLY:
    target = me;
