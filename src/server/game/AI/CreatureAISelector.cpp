...
CreatureAI *selectAI(Creature *creature) {
  ...
  // xinef: if we have controlable guardian, define petai for players as they
  // can steer him, otherwise db / normal ai xinef: dont remember why i changed
  // this qq commented out as may break some quests
  ...
  if (!ai_factory) {
    if (creature->IsVehicle())
      ai_factory = ai_registry.GetRegistryItem("VehicleAI");
             ...
             ((Guardian *)creature)->GetOwner()->GetTypeId() == TYPEID_PLAYER)
      ai_factory = ai_registry.GetRegistryItem("PetAI");
  ...
  LOG_DEBUG("scripts.ai", "Creature %u used AI is %s.", creature->GetGUIDLow(),
            ainame.c_str());
#endif
  return (ai_factory == nullptr ? new NullCreatureAI(creature)
                                : ai_factory->Create(creature));
}

MovementGenerator *selectMovementGenerator(Creature *creature) {
  MovementGeneratorRegistry &mv_registry(
      *MovementGeneratorRegistry::instance());
  ASSERT(creature->GetCreatureTemplate());
  const MovementGeneratorCreator *mv_factory =
      mv_registry.GetRegistryItem(creature->GetDefaultMovementType());
  ...
  /* if (mv_factory == nullptr)
  {
      ...
      {
      const MovementGeneratorCreator *factory =
  mv_registry.GetRegistryItem((*iter).c_str()); const SelectableMovement *p =
  dynamic_cast<const SelectableMovement *>(factory); ASSERT(p != nullptr); int
  val = p->Permit(creature); if (val > best_val)
      {
          best_val = val;
          mv_factory = p;
      }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  std::string ainame = (ai_factory == nullptr || go->GetScriptId())
                           ? "NullGameObjectAI"
                           : ai_factory->key();
  LOG_DEBUG("scripts.ai", "GameObject %u used AI is %s.", go->GetGUIDLow(),
            ainame.c_str());
#endif

  return (ai_factory == nullptr ? new NullGameObjectAI(go)
                                : ai_factory->Create(go));
}
} // namespace FactorySelector
