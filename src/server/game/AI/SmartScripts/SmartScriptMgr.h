...
enum SMART_EVENT {
  ...
  SMART_EVENT_OOC_LOS =
      10, // NoHostile, MaxRnage, CooldownMin, CooldownMax, PlayerOnly
  ...
  SMART_EVENT_IC_LOS =
      26, // NoHostile, MaxRnage, CooldownMin, CooldownMax, PlayerOnly
  ...
  SMART_EVENT_LINK =
      61, // INTERNAL USAGE, no params, used to link together multiple events,
          // does not use any extra resources to iterate event lists needlessly
  ...
  SMART_EVENT_NEAR_PLAYERS_NEGATION =
      102, // min, radius, first timer, check timer
...
enum SMART_ACTION {
  ...
  SMART_ACTION_TALK =
      1, // groupID from creature_text, duration to wait before TEXT_OVER event
         // is triggered, use first target from targetlist as invoker,
  ...
  SMART_ACTION_SUMMON_CREATURE = 12, // CreatureID, summonType, duration in ms,
                                     // attackInvoker, attackScriptOwner
  ...
  SMART_ACTION_AUTO_ATTACK = 20, // AllowAttackState (0 = stop attack, anything
                                 // else means continue attacking)
  SMART_ACTION_ALLOW_COMBAT_MOVEMENT =
      21, // AllowCombatMovement (0 = stop combat based movement, anything else
          // continue attacking)
  ...
  SMART_ACTION_MOUNT_TO_ENTRY_OR_MODEL =
      43, // Creature_template entry(param1) OR ModelId (param2) (or 0 for both
          // to dismount)
  ...
  SMART_ACTION_MOVE_TO_POS = 69, // PointId (optional x,y,z offset), transport,
                                 // controlled, ContactDistance
  SMART_ACTION_RESPAWN_TARGET = 70, // force / goRespawnTime
  SMART_ACTION_EQUIP = 71, // entry, slotmask slot1, slot2, slot3   , only slots
                           // with mask set will be sent to client, bits are 1,
                           // 2, 4, leaving mask 0 is defaulted to mask 7 (send
                           // all), slots1-3 are only used if no entry is set
  ...
  SMART_ACTION_INVOKER_CAST =
      85, // spellID, castFlags,   if avaliable, last used invoker will cast
          // spellId with castFlags on targets
  SMART_ACTION_CROSS_CAST =
      86, // spellID, castFlags, CasterTargetType, CasterTarget param1,
          // CasterTarget param2, CasterTarget param3, ( + the origonal target
          // fields as Destination target),   CasterTargets will cast spellID on
          // all Targets (use with caution if targeting multiple * multiple
          // units)
  ...
  SMART_ACTION_RANDOM_SOUND =
      115, // SoundId1, SoundId2, SoundId3, SoundId4, onlySelf
  ...
  // SMART_ACTION_INVOKER_CAST                    = 134,    // TODO: solve name
  // conflicts
  ...
  SMART_ACTION_CONE_SUMMON =
      219, // entry, duration (0 = perm), dist between rings, dist between earch
           // summon in a row, length of cone, width of cone (angle)
  ...
  SMART_ACTION_VORTEX_SUMMON =
      221, // entry, duration (0 = perm), spiral scaling, spiral appearance,
           // range max, phi_delta     <-- yes confusing math, try it ingame and
           // see, my lovely AC boys!
...
enum SMARTAI_TEMPLATE {
  ...
  SMARTAI_TEMPLATE_CASTER = 1, // spellid, repeatMin, repeatMax, range, manaPCT
...
enum SMARTAI_TARGETS {
  ...
  SMART_TARGET_HOSTILE_SECOND_AGGRO =
      3, // Second highest aggro, maxdist, playerOnly, powerType + 1
  SMART_TARGET_HOSTILE_LAST_AGGRO =
      4, // Dead last on aggro, maxdist, playerOnly, powerType + 1
  SMART_TARGET_HOSTILE_RANDOM = 5, // Just any random target on our threat list,
                                   // maxdist, playerOnly, powerType + 1
  SMART_TARGET_HOSTILE_RANDOM_NOT_TOP =
      6, // Any random target except top threat, maxdist, playerOnly, powerType
         // + 1
  ...
  SMART_TARGET_CREATURE_DISTANCE =
      11, // CreatureEntry(0any), maxDist, alive(0 - both, 1 - alive, 2 - dead)
  ...
  SMART_TARGET_PLAYER_RANGE =
      17, // min, max, maxCount (maxCount by pussywizard), set target.o to 1 if
          // u want to search for all in range if min, max fails
  ...
  SMART_TARGET_ROLE_SELECTION = 203, // Range Max, TargetMask (Tanks (1), Healer
                                     // (2) Damage (4)), resize list
...
struct SmartTarget {
  SmartTarget(SMARTAI_TARGETS t = SMART_TARGET_NONE, uint32 p1 = 0,
              uint32 p2 = 0, uint32 p3 = 0, uint32 p4 = 0) {
    type = t;
    raw.param1 = p1;
    raw.param2 = p2;
    raw.param3 = p3;
...
enum SmartCastFlags {
  ...
  // CAST_FORCE_CAST             = 0x04,                     //Forces cast even
  // if creature is out of mana or out of range CAST_NO_MELEE_IF_OOM        =
  // 0x08,                     //Prevents creature from entering melee if out of
  // mana or out of range CAST_FORCE_TARGET_SELF      = 0x10, //Forces the target
  // to cast this spell on itself
  SMARTCAST_AURA_NOT_PRESENT = 0x20, // Only casts the spell if the target does
                                     // not have an aura from the spell
  SMARTCAST_COMBAT_MOVE = 0x40 // Prevents combat movement if cast successful.
                               // Allows movement on range, OOM, LOS
};
...
struct SmartScriptHolder {
  SmartScriptHolder()
      : entryOrGuid(0), source_type(SMART_SCRIPT_TYPE_CREATURE), event_id(0),
        link(0), event(), action(), target(), timer(0), active(false),
        runOnce(false), enableTimed(false) {}
...
public:
  ...
  ObjectList *GetObjectList() {
    if (m_baseObject) {
      ...
      for (GuidList::iterator itr = m_guidList->begin();
           itr != m_guidList->end(); ++itr) {
        if (WorldObject *obj =
                ObjectAccessor::GetWorldObject(*m_baseObject, *itr))
          m_objectList->push_back(obj);
        // else
        //    TC_LOG_DEBUG("scripts.ai", "SmartScript::mTargetStorage stores a
        //    guid to an invalid object: " UI64FMTD, *itr);
      }
    }
  ...
  bool Equals(ObjectList *objectList) { return m_objectList == objectList; }
...
public:
  ...
  SmartAIEventList GetScript(int32 entry, SmartScriptType type) {
    ...
    if (mEventMap[uint32(type)].find(entry) != mEventMap[uint32(type)].end())
      ...
      if (entry > 0) // first search is for guid (negative), do not drop error
                     // if not found
        LOG_DEBUG("sql.sql",
                  "SmartAIMgr::GetScript: Could not load Script for Entry %d "
                  "ScriptType %u.",
                  entry, uint32(type));
#endif
      return temp;
...
private:
  ...
  /*inline bool IsTargetValid(SmartScriptHolder e, int32 target)
  {
      ...
      {
          LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u
  Action %u uses invalid Target type %d, skipped.", e.entryOrGuid,
  ...
  /*inline bool IsPercentValid(SmartScriptHolder e, int32 pct)
  {
      ...
      {
          LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u
  Action %u has invalid Percent set (%d), skipped.", e.entryOrGuid,
