...
void SmartAI::GenerateWayPointArray(Movement::PointsArray *points) {
  ...
  if (me->m_movementInfo.HasMovementFlag((MovementFlags)(
    ...
    points->clear();
    points->push_back(G3D::Vector3(me->GetPositionX(), me->GetPositionY(),
                                   me->GetPositionZ()));
    ...
    for (float size = 1.0f; size; size *= 0.5f) {
      ...
      // xinef: first point in vector is unit real position
      pVector.push_back(G3D::Vector3(me->GetPositionX(), me->GetPositionY(),
                                     me->GetPositionZ()));
      ...
      if (pVector.size() > 2) // more than source + dest
      {
        ...
        for (uint32 i = 1; i < pVector.size() - 1; ++i) {
          ...
          if (fabs(offset.x) >= 0xFF || fabs(offset.y) >= 0xFF ||
              fabs(offset.z) >= 0x7F) {
...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit *invoker) {
  ...
  if (WayPoint *wp = GetNextWayPoint()) {
    ...
    GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_START, nullptr, wp->id,
                                  GetScript()->GetPathId());
  }
}
...
void SmartAI::PausePath(uint32 delay, bool forced) {
  ...
  if (HasEscortState(SMART_ESCORT_PAUSED)) {
    LOG_ERROR("server",
              "SmartAI::StartPath: Creature entry %u wanted to pause waypoint "
  ...
  GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_PAUSED, nullptr,
                                mCurrentWPID, GetScript()->GetPathId());
}
...
void SmartAI::EndPath(bool fail) {
  ...
  if (targets && mEscortQuestID) {
    if (targets->size() == 1 && GetScript()->IsPlayer((*targets->begin()))) {
      ...
      for (ObjectList::iterator iter = targets->begin(); iter != targets->end();
           ++iter) {
  ...
  GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_ENDED, nullptr,
                                mCurrentWPID, GetScript()->GetPathId());
  mCurrentWPID = 0;
...
void SmartAI::UpdatePath(const uint32 diff) {
  ...
  if (mEscortInvokerCheckTimer < diff) {
    ...
    if (!IsEscortInvokerInRange()) {
      ...
      // Xinef: allow to properly hook out of range despawn action, which in
      // most cases should perform the same operation as dying
  ...
  if (HasEscortState(SMART_ESCORT_PAUSED)) {
    if (mWPPauseTimer < diff) {
      if (!me->IsInCombat() && !HasEscortState(SMART_ESCORT_RETURNING) &&
        ...
        GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_RESUMED, nullptr,
                                      mCurrentWPID, GetScript()->GetPathId());
      ...
      me->movespline->Finalized()) // reached WP
  {
    ...
    if (mCurrentWPID == GetWPCount())
      ...
      SetRun(mRun);
      // xinef: if we have reached waypoint, and there is no working spline
      // movement it means our splitted array has ended, make new one
  ...
  if (m_ConditionsTimer < diff) {
    if (!conditions.empty()) {
      for (SeatMap::iterator itr = vehicle->Seats.begin();
           itr != vehicle->Seats.end(); ++itr)
        if (Unit *passenger =
                ObjectAccessor::GetUnit(*me, itr->second.Passenger.Guid)) {
          if (Player *player = passenger->ToPlayer()) {
            if (!sConditionMgr->IsObjectMeetToConditions(player, me,
                                                         conditions)) {
              player->ExitVehicle();
...
void SmartAI::UpdateAI(uint32 diff) {
  ...
  if (mFollowGuid) {
    if (mFollowArrivedTimer < diff) {
      if (me->FindNearestCreature(mFollowArrivedEntry, INTERACTION_DISTANCE,
                                  mFollowArrivedAlive)) {
        StopFollow(true);
        return;
      }
...
bool SmartAI::IsEscortInvokerInRange() {
  ObjectList *targets = GetScript()->GetTargetList(SMART_ESCORT_TARGETS);
  if (targets) {
    float checkDist = me->GetInstanceScript() ? SMART_ESCORT_MAX_PLAYER_DIST * 2
...
void SmartAI::MovepointReached(uint32 id) {
  ...
  mWPReached = true;
  GetScript()->ProcessEventsFor(SMART_EVENT_WAYPOINT_REACHED, nullptr,
                                mCurrentWPID);
...
void SmartAI::MovementInform(uint32 MovementType, uint32 Data) {
  ...
  GetScript()->ProcessEventsFor(SMART_EVENT_MOVEMENTINFORM, nullptr,
                                MovementType, Data);
  if (!HasEscortState(SMART_ESCORT_ESCORTING))
    return;
...
void SmartAI::EnterEvadeMode() {
  ...
  if (IS_PLAYER_GUID(me->GetCharmerGUID()) ||
      me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED)) {
    me->AttackStop();
    return;
  }
  ...
  GetScript()->ProcessEventsFor(
      SMART_EVENT_EVADE); // must be after aura clear so we can cast spells from
                          // db
...
void SmartAI::JustRespawned() {
  ...
  mFollowGuid =
      0; // do not reset follower on Reset(), we need it after combat evade
  mFollowDist = 0;
  mFollowAngle = 0;
  mFollowCredit = 0;
  mFollowArrivedTimer = 1000;
  ...
  if (who && me->Attack(who, me->IsWithinMeleeRange(who))) {
    if (mCanCombatMove || GetScript()->GetMaxCombatDist()) {
      ...
      me->GetMotionMaster()->MoveChase(
          who, GetScript()->GetCasterActualDist()
                   ? GetScript()->GetCasterActualDist()
                   : GetScript()->GetActualCombatDist());
    }
  }
}

void SmartAI::SpellHit(Unit *unit, const SpellInfo *spellInfo) {
  GetScript()->ProcessEventsFor(SMART_EVENT_SPELLHIT, unit, 0, 0, false,
                                spellInfo);
}
...
void SmartAI::DamageTaken(Unit *doneBy, uint32 &damage,
  ...
  if (damagetype != NODAMAGE && mInvincibilityHpLevel &&
    ...
    damage = me->GetHealth() -
             mInvincibilityHpLevel; // damage should not be nullified, because
                                    // of player damage req.
}
...
void SmartAI::DamageDealt(Unit *doneTo, uint32 &damage,
                          DamageEffectType /*damagetype*/) {
...
void SmartAI::CorpseRemoved(uint32 &respawnDelay) {
  GetScript()->ProcessEventsFor(SMART_EVENT_CORPSE_REMOVED, nullptr,
                                respawnDelay);
...
uint64 SmartAI::GetGUID(int32 /*id*/) const { return 0; }
...
void SmartAI::SetCombatMove(bool on) {
  ...
  if (GetScript()
          ->GetMaxCombatDist() /* || GetScript()->GetCasterMaxDist()*/) // Xinef:
                                                                        // we
                                                                        // only
                                                                        // need
                                                                        // this
...
void SmartAI::SetFollow(Unit *target, float dist, float angle, uint32 credit,
                        uint32 end, uint32 creditType, bool aliveState) {
  if (!target) {
    StopFollow(false);
    return;
  }
...
#endif
  GetScript()->ProcessEventsFor(SMART_EVENT_GOSSIP_HELLO, player,
                                (uint32)reportUse, 0, false, nullptr, go);
  return false;
}
...
bool SmartGameObjectAI::QuestReward(Player *player, Quest const *quest,
                                    uint32 opt) {
  GetScript()->ProcessEventsFor(SMART_EVENT_REWARD_QUEST, player,
                                quest->GetQuestId(), opt, false, nullptr, go);
  return false;
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("sql.sql", "AreaTrigger %u is using SmartTrigger script",
              trigger->entry);
#endif
    SmartScript script;
