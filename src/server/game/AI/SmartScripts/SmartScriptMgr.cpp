...
void SmartWaypointMgr::LoadFromDB() {
  ...
  for (std::unordered_map<uint32, WPPath *>::iterator itr =
           waypoint_map.begin();
       itr != waypoint_map.end(); ++itr) {
    for (WPPath::iterator pathItr = itr->second->begin();
         pathItr != itr->second->end(); ++pathItr)
...
void SmartAIMgr::LoadSmartAIFromDB() {
  ...
  do {
    ...
    if (temp.entryOrGuid >= 0) {
      ...
      case SMART_SCRIPT_TYPE_CREATURE: {
        if (!sObjectMgr->GetCreatureTemplate((uint32)temp.entryOrGuid)) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: Creature entry (%u) does "
      ...
      case SMART_SCRIPT_TYPE_GAMEOBJECT: {
        if (!sObjectMgr->GetGameObjectTemplate((uint32)temp.entryOrGuid)) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr::LoadSmartAIFromDB: GameObject entry (%u) does "
                    "not exist, skipped loading.",
                    uint32(temp.entryOrGuid));
          continue;
        }
      ...
      if (!sObjectMgr->GetCreatureData(uint32(abs(temp.entryOrGuid)))) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr::LoadSmartAIFromDB: Creature guid (%u) does not "
                  "exist, skipped loading.",
    ...
    if (temp.action.type == SMART_ACTION_MOVE_TO_POS)
      if (temp.target.type == SMART_TARGET_SELF &&
          (fabs(temp.target.x) > 200.0f || fabs(temp.target.y) > 200.0f ||
           fabs(temp.target.z) > 200.0f))
        temp.target.type = SMART_TARGET_POSITION;

    // creature entry / guid not found in storage, create empty event list for
    // it and increase counters
  ...
  LOG_INFO("server", ">> Loaded %u SmartAI scripts in %u ms", count,
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const &e) {
  ...
  case SMART_TARGET_CREATURE_RANGE: {
    if (e.target.unitDistance.creature &&
      ...
      LOG_ERROR("sql.sql",
                ...
                "non-existent Creature entry %u as target_param1, skipped.",
  ...
  case SMART_TARGET_GAMEOBJECT_RANGE: {
    if (e.target.goDistance.entry &&
      ...
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses "
                "non-existent GameObject entry %u as target_param1, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_TARGET_CLOSEST_PLAYER: {
    if (e.target.playerDistance.dist == 0) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has "
                "maxDist 0 as target_param1, skipped.",
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder &e) {
  ...
  if (e.action.type <= 0 ||
    ...
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid %d using event(%u) has an invalid "
              "action type (%u), skipped.",
  ...
  case SMART_ACTION_SCENE_CANCEL:
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid %d using event(%u) has an action type "
              "that is not supported on 3.3.5a (%u), skipped.",
              e.entryOrGuid, e.event_id, e.GetActionType());
    return false;
  ...
  case SMART_ACTION_RESPAWN_BY_SPAWNID:
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid %d using event(%u) has an action type "
              "that is not yet supported on AzerothCore (%u), skipped.",
              e.entryOrGuid, e.event_id, e.GetActionType());
    return false;
  default:
    break;
  ...
  if (e.target.type < 0 ||
    ...
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid %d using event(%u) has an invalid "
              "target type (%u), skipped.",
              e.entryOrGuid, e.event_id, e.GetTargetType());
    return false;
  }
  if (e.target.type == SMART_TARGET_LOOT_RECIPIENTS ||
      e.target.type == SMART_TARGET_VEHICLE_PASSENGER) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid %d using event(%u) has a target type "
              "that is not yet supported on AzerothCore (%u), skipped.",
              e.entryOrGuid, e.event_id, e.GetTargetType());
    return false;
  }
  if (e.event.event_phase_mask > SMART_EVENT_PHASE_ALL) {
    LOG_ERROR("sql.sql",
              "SmartAIMgr: EntryOrGuid %d using event(%u) has invalid phase "
              "mask (%u), skipped.",
  ...
  if (e.GetScriptType() == SMART_SCRIPT_TYPE_TIMED_ACTIONLIST) {
    e.event.type = SMART_EVENT_UPDATE_OOC; // force default OOC, can change when
                                           // calling the script!
    ...
    case SMART_EVENT_SPELLHIT_TARGET:
      if (e.event.spellHit.spell) {
        ...
        if (e.event.spellHit.school &&
          ...
          LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry %d SourceType %u Event %u Action %u "
                    "uses Spell entry %u with invalid school mask, skipped.",
                    e.entryOrGuid, e.GetScriptType(), e.event_id,
                    e.GetActionType(), e.event.spellHit.spell);
          return false;
        }
    ...
    case SMART_EVENT_RESPAWN:
      if (e.event.respawn.type == SMART_SCRIPT_RESPAWN_CONDITION_MAP &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses "
                  "non-existent Map entry %u, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
      ...
      if (e.event.respawn.type == SMART_SCRIPT_RESPAWN_CONDITION_AREA &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses "
                  "non-existent Area entry %u, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
    ...
    case SMART_EVENT_VICTIM_CASTING:
      ...
      if (!IsMinMaxValid(e, e.event.targetCasting.repeatMin,
    ...
    case SMART_EVENT_TEXT_OVER:
      ...
      // e.event.textOver.textGroupID)) return false;// 0 is a valid text group!
      break;
    case SMART_EVENT_LINK: {
      if (e.link && e.link == e.event_id) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry %d SourceType %u, Event %u, Link Event is "
                  "linking self (infinite loop), skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id);
        return false;
      }
      break;
    ...
    case SMART_EVENT_FRIENDLY_HEALTH_PCT:
      if (!IsMinMaxValid(e, e.event.friendlyHealthPct.repeatMin,
      ...
      default:
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses "
                  "invalid target_type %u, skipped.",
                  e.entryOrGuid, e.GetScriptType(), e.event_id,
                  e.GetActionType(), e.GetTargetType());
        return false;
      }
      break;
    case SMART_EVENT_DISTANCE_CREATURE:
      if (e.event.distance.guid == 0 && e.event.distance.entry == 0) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE did not "
                  "provide creature guid or entry, skipped.");
        return false;
      }

      if (e.event.distance.guid != 0 && e.event.distance.entry != 0) {
        LOG_ERROR("sql.sql", "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE "
      ...
      if (e.event.distance.guid != 0 &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using "
                  "invalid creature guid %u, skipped.",
      ...
      if (e.event.distance.entry != 0 &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using "
                  "invalid creature entry %u, skipped.",
    ...
    case SMART_EVENT_DISTANCE_GAMEOBJECT:
      if (e.event.distance.guid == 0 && e.event.distance.entry == 0) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT did not "
                  "provide gameobject guid or entry, skipped.");
        return false;
      }

      if (e.event.distance.guid != 0 && e.event.distance.entry != 0) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT provided "
                  "both an entry and guid, skipped.");
        return false;
      }

      if (e.event.distance.guid != 0 &&
          !sObjectMgr->GetGOData(e.event.distance.guid)) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using "
                  "invalid gameobject guid %u, skipped.",
                  e.event.distance.guid);
        return false;
      }
      ...
      if (e.event.distance.entry != 0 &&
        ...
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using "
                  "invalid gameobject entry %u, skipped.",
                  e.event.distance.entry);
        return false;
      }
      break;
    case SMART_EVENT_COUNTER_SET:
      if (!IsMinMaxValid(e, e.event.counter.cooldownMin,
                         e.event.counter.cooldownMax))
        return false;

      if (e.event.counter.id == 0) {
        LOG_ERROR("sql.sql",
                  "SmartAIMgr: Event SMART_EVENT_COUNTER_SET using invalid "
                  "counter id %u, skipped.",
                  e.event.counter.id);
        return false;
      }
    ...
    if (e.action.morphOrMount.creature || e.action.morphOrMount.model) {
      ...
      if (e.action.morphOrMount.model) {
        if (e.action.morphOrMount.creature) {
          LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has "
                    "ModelID set with also set CreatureId, skipped.",
                    e.entryOrGuid, e.GetScriptType(), e.event_id,
                    e.GetActionType());
          return false;
  ...
  case SMART_ACTION_SET_EVENT_PHASE:
    if (e.action.setEventPhase.phase >= SMART_EVENT_PHASE_MAX) {
      LOG_ERROR(
          "sql.sql",
          "SmartAIMgr: Entry %d SourceType %u Event %u Action %u attempts to "
          "set phase %u. Phase mask cannot be used past phase %u, skipped.",
          e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
  ...
  case SMART_ACTION_RANDOM_PHASE: {
    ...
    if (e.action.randomPhase.phase1 == 0 && e.action.randomPhase.phase2 == 0 &&
        e.action.randomPhase.phase3 == 0 && e.action.randomPhase.phase4 == 0 &&
        e.action.randomPhase.phase5 == 0 && e.action.randomPhase.phase6 == 0) {
      LOG_ERROR("sql.sql",
                "SmartAIMgr: Entry %d SourceType %u Event %u Action %u "
                "attempts to set invalid phase, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id,
  ...
  case SMART_ACTION_CALL_RANDOM_RANGE_TIMED_ACTIONLIST: {
    if (!IsMinMaxValid(e, e.action.randTimedActionList.entry1,
  ...
  case SMART_ACTION_GAME_EVENT_STOP: {
    ...
    if (eventId < 1 || eventId >= events.size()) {
      LOG_ERROR("server",
                "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses "
                "non-existent event, eventId %u, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
      ...
      if (equipId) {
        ...
        if (!einfo) {
          LOG_ERROR("server",
                    ...
                    "equipment info id %u for creature %u, skipped.",
  ...
  case SMART_ACTION_LOAD_GRID: {
    if (!Warhead::IsValidMapCoord(e.target.x, e.target.y)) {
      LOG_ERROR("server",
                "SmartScript: SMART_ACTION_LOAD_GRID uses invalid map coords: "
                "%u, skipped.",
                e.entryOrGuid);
      return false;
    }
  ...
  case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: {
    if (e.GetScriptType() == SMART_SCRIPT_TYPE_GAMEOBJECT) {
      LOG_ERROR("sql.sql",
                "SmartScript: action_type %u is not allowed with source_type "
                "%u. Entry %u, skipped.",
                e.GetActionType(), e.GetScriptType(), e.entryOrGuid);
      return false;
    }
    break;
  }
  ...
  default:
    LOG_ERROR("sql.sql",
              "SmartAIMgr: Not handled action_type(%u), event_type(%u), Entry "
              "%d SourceType %u Event %u, skipped.",
...
/*bool SmartAIMgr::IsTextValid(SmartScriptHolder const& e, uint32 id) // unused
{
    ...
    if (e.entryOrGuid >= 0)
        ...
        if (!data)
        {
            LOG_ERROR("server", "SmartAIMgr: Entry %d SourceType %u Event %u
Action %u using non-existent Creature guid %d, skipped.", e.entryOrGuid,
e.GetScriptType(), e.event_id, e.GetActionType(), entry); return false;
        }
        else
            entry = data->id;
    }
    ...
    if (error)
    {
        LOG_ERROR("server", "SmartAIMgr: Entry %d SourceType %u Event %u Action
%u using non-existent Text id %d, skipped.", e.entryOrGuid, e.GetScriptType(),
e.source_type, e.GetActionType(), id); return false;
    }
    return true;
}*/
