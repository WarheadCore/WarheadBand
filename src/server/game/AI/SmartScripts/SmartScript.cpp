...
public:
  AcoreStringTextBuilder(WorldObject *obj, ChatMsg msgtype, int32 id,
                         uint32 language, WorldObject *target)
  ...
  size_t operator()(WorldPacket *data, LocaleConstant locale) const {
    ...
    return ChatHandler::BuildChatPacket(*data, _msgType, Language(_language),
                                        _source, _target, text, 0, "", locale);
  }
...
void SmartScript::ProcessEventsFor(SMART_EVENT e, Unit *unit, uint32 var0,
                                   uint32 var1, bool bvar,
                                   const SpellInfo *spell, GameObject *gob) {
  for (SmartAIEventList::iterator i = mEvents.begin(); i != mEvents.end();
       ++i) {
...
void SmartScript::ProcessAction(SmartScriptHolder &e, Unit *unit, uint32 var0,
                                uint32 var1, bool bvar, const SpellInfo *spell,
                                GameObject *gob) {
  // calc random
  ...
  if (Unit *tempInvoker = GetLastInvoker())
    LOG_DEBUG("sql.sql",
              ...
              tempInvoker->GetName().c_str(), tempInvoker->GetGUIDLow());
#endif
  ...
  case SMART_ACTION_TALK: {
    ...
    Creature *talker =
        e.target.type == 0 ? me : nullptr; // xinef: tc retardness fix
    Unit *talkTarget = nullptr;
    if (targets) {
      for (ObjectList::const_iterator itr = targets->begin();
           itr != targets->end(); ++itr) {
        if (IsCreature((*itr)) &&
            !(*itr)->ToCreature()->IsPet()) // Prevented sending text to pets.
        {
    ...
    if (!sCreatureTextMgr->TextExist(talker->GetEntry(),
      ...
      LOG_ERROR("sql.sql",
                "SmartScript::ProcessAction: SMART_ACTION_TALK: EntryOrGuid %d "
                "SourceType %u EventType %u TargetType %u using non-existent "
                "Text id %u for talker %u, ignored.",
                e.entryOrGuid, e.GetScriptType(), e.GetEventType(),
                e.GetTargetType(), e.action.talk.textGroupID,
                talker->GetEntry());
      break;
    }
    ...
    LOG_DEBUG("sql.sql",
              ...
              "(GuidLow: %u), textId: %u",
              talker->GetName().c_str(), talker->GetGUIDLow(), mLastTextID);
#endif
    break;
  }
      ...
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (IsCreature(*itr))
          sCreatureTextMgr->SendChat(
              (*itr)->ToCreature(), uint8(e.action.talk.textGroupID),
              IsPlayer(GetLastInvoker()) ? GetLastInvoker() : 0);
        else if (IsPlayer(*itr) && me) {
          Unit *templastInvoker = GetLastInvoker();
          sCreatureTextMgr->SendChat(
              me, uint8(e.action.talk.textGroupID),
              IsPlayer(templastInvoker) ? templastInvoker : 0, CHAT_MSG_ADDON,
              LANG_ADDON, TEXT_RANGE_NORMAL, 0, TEAM_NEUTRAL, false,
              (*itr)->ToPlayer());
        }
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("sql.sql",
                  ...
                  (*itr)->GetName().c_str(), (*itr)->GetGUIDLow(),
                  uint8(e.action.talk.textGroupID));
#endif
      }
  ...
  case SMART_ACTION_SOUND: {
    ...
    if (targets) {
      for (ObjectList::const_iterator itr = targets->begin();
           itr != targets->end(); ++itr) {
        if (IsUnit(*itr)) {
          (*itr)->SendPlaySound(e.action.sound.sound,
                                e.action.sound.onlySelf > 0);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("sql.sql",
                    "SmartScript::ProcessAction:: SMART_ACTION_SOUND: target: "
                    "%s (GuidLow: %u), sound: %u, onlyself: %u",
                    (*itr)->GetName().c_str(), (*itr)->GetGUIDLow(),
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        ...
        LOG_DEBUG("sql.sql",
                  ...
                  "target: %s (GuidLow: %u), sound: %u, onlyself: %u",
                  (*itr)->GetName().c_str(), (*itr)->GetGUIDLow(), sound,
  ...
  case SMART_ACTION_MUSIC: {
    ...
    if (targets) {
      for (ObjectList::const_iterator itr = targets->begin();
           itr != targets->end(); ++itr) {
        if (IsUnit(*itr)) {
          (*itr)->SendPlayMusic(e.action.music.sound,
                                e.action.music.onlySelf > 0);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("sql.sql",
                    "SmartScript::ProcessAction:: SMART_ACTION_MUSIC: target: "
                    "%s (GuidLow: %u), sound: %u, onlySelf: %u, type: %u",
                    (*itr)->GetName().c_str(), (*itr)->GetGUIDLow(),
                    e.action.music.sound, e.action.music.onlySelf,
                    e.action.music.type);
#endif
        }
      }
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        ...
        LOG_DEBUG("sql.sql",
                  ...
                  "target: %s (GuidLow: %u), sound: %u, onlyself: %u, type: %u",
                  (*itr)->GetName().c_str(), (*itr)->GetGUIDLow(), sound,
                  e.action.randomMusic.onlySelf, e.action.randomMusic.type);
#endif
      }
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
            LOG_DEBUG("sql.sql",
                      "SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: "
                      "Creature entry %u, GuidLow %u set faction to %u",
                      (*itr)->GetEntry(), (*itr)->GetGUIDLow(),
                      e.action.faction.factionID);
#endif
          } else {
            if (CreatureTemplate const *ci = sObjectMgr->GetCreatureTemplate(
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsPlayer(*itr)) {
        ...
        LOG_DEBUG("sql.sql",
                  ...
                  "Player guidLow %u fails quest %u",
  ...
  case SMART_ACTION_OFFER_QUEST: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (Player *pTarget = (*itr)->ToPlayer()) {
        if (Quest const *q =
                sObjectMgr->GetQuestTemplate(e.action.questOffer.questID)) {
          if (me && e.action.questOffer.directAdd == 0) {
            if (pTarget->CanTakeQuest(q, true))
              if (WorldSession *session = pTarget->GetSession()) {
                ...
                LOG_DEBUG(
                    ...
                    "Player guidLow %u - offering quest %u",
            ...
            LOG_DEBUG("sql.sql",
                      "SmartScript::ProcessAction:: SMART_ACTION_OFFER_QUEST: "
                      "Player guidLow %u - quest %u added",
                      (*itr)->GetGUIDLow(), e.action.questOffer.questID);
#endif
          }
        }
  ...
  case SMART_ACTION_THREAT_ALL_PCT: {
    ...
    for (ThreatContainer::StorageType::const_iterator i = threatList.begin();
         i != threatList.end(); ++i) {
  ...
  case SMART_ACTION_THREAT_SINGLE_PCT: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        me->getThreatManager().modifyThreatPercent(
            (*itr)->ToUnit(), e.action.threatPCT.threatINC
                                  ? (int32)e.action.threatPCT.threatINC
                                  : -(int32)e.action.threatPCT.threatDEC);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG(
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        if (Vehicle *vehicle = (*itr)->ToUnit()->GetVehicleKit())
          for (SeatMap::iterator it = vehicle->Seats.begin();
               it != vehicle->Seats.end(); ++it)
            if (Player *player = ObjectAccessor::GetPlayer(
                    *(*itr), it->second.Passenger.Guid))
              player->AreaExploredOrEventHappens(e.action.quest.quest);

        if (Player *player =
                (*itr)->ToUnit()->GetCharmerOrOwnerPlayerOrPlayerItself()) {
  ...
  case SMART_ACTION_CAST: {
    ...
    if (e.GetScriptType() == SMART_SCRIPT_TYPE_AREATRIGGER)
      caster = unit->SummonTrigger(unit->GetPositionX(), unit->GetPositionY(),
                                   unit->GetPositionZ(), unit->GetOrientation(),
                                   5000);

    if (e.action.cast.targetsLimit > 0 &&
        targets->size() > e.action.cast.targetsLimit)
      Warhead::Containers::RandomResizeList(*targets,
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      {
        caster->CastSpell((*itr)->ToUnit(), e.action.cast.spell,
                          (e.action.cast.flags & SMARTCAST_TRIGGERED));
      } else if (me && (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) ||
                        !(*itr)->ToUnit()->HasAura(e.action.cast.spell))) {
        if (e.action.cast.flags & SMARTCAST_INTERRUPT_PREVIOUS)
          me->InterruptNonMeleeSpells(false);

        // Xinef: flag usable only if caster has max dist set
        if ((e.action.cast.flags & SMARTCAST_COMBAT_MOVE) &&
            GetCasterMaxDist() > 0.0f &&
            me->GetMaxPower(GetCasterPowerType()) > 0) {
          // Xinef: check mana case only and operate movement accordingly, LoS
          // and range is checked in targetet movement generator
          ...
          } else if (GetCasterActualDist() == 0.0f &&
                     me->GetPowerPct(GetCasterPowerType()) > 30.0f) {
            RestoreCasterMaxDist();
            CAST_AI(SmartAI, me->AI())
                ->SetForcedCombatMove(GetCasterActualDist());
          }
        }

        me->CastSpell((*itr)->ToUnit(), e.action.cast.spell,
                      (e.action.cast.flags & SMARTCAST_TRIGGERED));
      }
    }
  ...
  case SMART_ACTION_INVOKER_CAST: {
    Unit *tempLastInvoker =
        GetLastInvoker(unit); // xinef: can be used for area triggers cast
    if (!tempLastInvoker)
      break;
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) ||
        ...
        tempLastInvoker->CastSpell((*itr)->ToUnit(), e.action.cast.spell,
                                   (e.action.cast.flags & SMARTCAST_TRIGGERED));
      }
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("sql.sql",
                  "SmartScript::ProcessAction:: SMART_ACTION_ADD_AURA: Adding "
                  "aura %u to unit %u",
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsGameObject(*itr)) {
        ...
        LOG_DEBUG("sql.sql",
                  ...
                  "Gameobject %u (entry: %u) activated",
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        if (!e.action.unitFlag.type) {
          ...
          LOG_DEBUG("sql.sql",
                    "SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. "
                    "Unit %u added flag %u to UNIT_FIELD_FLAGS",
                    (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
        } else {
          (*itr)->ToUnit()->SetFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
          LOG_DEBUG("sql.sql",
                    "SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. "
                    "Unit %u added flag %u to UNIT_FIELD_FLAGS_2",
                    (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
        }
      }
    }
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        if (!e.action.unitFlag.type) {
          ...
          LOG_DEBUG(
              "sql.sql",
              "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. "
              "Unit %u removed flag %u to UNIT_FIELD_FLAGS",
              (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
        } else {
          (*itr)->ToUnit()->RemoveFlag(UNIT_FIELD_FLAGS_2,
                                       e.action.unitFlag.flag);
          LOG_DEBUG(
              "sql.sql",
              "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. "
              "Unit %u removed flag %u to UNIT_FIELD_FLAGS_2",
              (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
        }
      }
    }
  ...
  case SMART_ACTION_ALLOW_COMBAT_MOVEMENT: {
    ...
    if (move && GetMaxCombatDist() &&
        e.GetEventType() == SMART_EVENT_MANA_PCT) {
    ...
    LOG_DEBUG("sql.sql",
              "SmartScript::ProcessAction:: "
              "SMART_ACTION_ALLOW_COMBAT_MOVEMENT: Creature %u bool on = %u",
  ...
  case SMART_ACTION_FLEE_FOR_ASSIST: {
    ...
    if (e.action.flee.withEmote) {
      AcoreStringTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_FLEE,
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit((*itr))) {
        ...
        LOG_DEBUG(
            ...
            "Player %u, group credit for quest %u",
            (*itr)->GetGUIDLow(), e.action.quest.quest);
#endif
      }
    }
      ...
      if (e.action.removeAura.spell) {
        if (e.action.removeAura.charges) {
          if (Aura *aur = (*itr)->ToUnit()->GetAura(e.action.removeAura.spell))
            aur->ModCharges(-static_cast<int32>(e.action.removeAura.charges),
  ...
  case SMART_ACTION_FOLLOW: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit((*itr))) {
        ...
        CAST_AI(SmartAI, me->AI())
            ->SetFollow((*itr)->ToUnit(),
                        float(int32(e.action.follow.dist)) + 0.1f, angle,
                        e.action.follow.credit, e.action.follow.entry,
                        e.action.follow.creditType, e.action.follow.aliveState);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("sql.sql",
                  "SmartScript::ProcessAction: SMART_ACTION_FOLLOW: Creature "
                  "%u following target %u",
                  me->GetGUIDLow(), (*itr)->GetGUIDLow());
#endif
        break;
      }
  ...
  case SMART_ACTION_CALL_KILLEDMONSTER: {
                   ...
                   SMART_TARGET_SELF) // Loot recipient and his group members
    {
      ...
      if (Player *player = me->GetLootRecipient()) {
        player->RewardPlayerAndGroupAtEvent(e.action.killedMonster.creature,
                                            player);
        LOG_DEBUG("sql.sql",
                  "SmartScript::ProcessAction: "
                  "SMART_ACTION_CALL_KILLEDMONSTER: Player %u, Killcredit: %u",
  ...
  case SMART_ACTION_SET_INST_DATA64: {
    ...
    instance->SetData64(e.action.setInstanceData64.field,
  ...
  case SMART_ACTION_UPDATE_TEMPLATE: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsCreature(*itr))
        (*itr)->ToCreature()->UpdateEntry(
            e.action.updateTemplate.creature, nullptr,
  ...
  case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: {
    ...
    if (!me->GetMap()->IsDungeon()) {
      ObjectList *units =
          GetWorldObjectsInDist((float)e.action.combatZone.range);
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsCreature(*itr)) {
        ...
        if (e.action.callHelp.withEmote) {
          AcoreStringTextBuilder builder(*itr, CHAT_MSG_MONSTER_EMOTE,
                                         LANG_CALL_FOR_HELP, LANG_UNIVERSAL,
                                         nullptr);
          sCreatureTextMgr->SendChatPacket(*itr, builder,
  ...
  case SMART_ACTION_SET_INGAME_PHASE_MASK: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr))
        ...
        (*itr)->ToGameObject()->SetPhaseMask(e.action.ingamePhaseMask.mask,
                                             true);
    }
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (e.action.morphOrMount.creature || e.action.morphOrMount.model) {
        if (e.action.morphOrMount.creature > 0) {
          if (CreatureTemplate const *cInfo = sObjectMgr->GetCreatureTemplate(
                  e.action.morphOrMount.creature))
      ...
      if (IsCreature(*itr)) {
        ...
        if (e.action.invincHP.percent)
          ai->SetInvincibilityHpLevel(
              (*itr)->ToCreature()->CountPctFromMaxHealth(
  ...
  case SMART_ACTION_RISE_UP: {
    ...
    me->GetMotionMaster()->MovePoint(
        SMART_RANDOM_POINT, me->GetPositionX(), me->GetPositionY(),
  ...
  case SMART_ACTION_ATTACK_START: {
    ...
    if (Unit *target =
            Warhead::Containers::SelectRandomContainerElement(*targets)
                ->ToUnit())
  ...
  case SMART_ACTION_SUMMON_CREATURE: {
    ...
    if (e.GetTargetType() == SMART_TARGET_RANDOM_POINT) {
      ...
      for (uint32 i = 0; i < e.target.randomPoint.amount; i++) {
        ...
        if (Creature *summon = summoner->SummonCreature(
                e.action.summonCreature.creature, randomPoint,
      ...
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (Creature *summon = summoner->SummonCreature(
          ...
          if (e.action.summonCreature.attackInvoker ==
              2) // pussywizard: proper attackInvoker implementation, but not
                 // spoiling tc shitness
            summon->AI()->AttackStart(unit);
    ...
    if (Creature *summon = summoner->SummonCreature(
            e.action.summonCreature.creature, e.target.x, e.target.y,
            e.target.z, e.target.o,
            (TempSummonType)e.action.summonCreature.type,
            e.action.summonCreature.duration)) {
      if (unit && e.action.summonCreature.attackInvoker)
        summon->AI()->AttackStart(unit);
  ...
  case SMART_ACTION_SUMMON_GO: {
    ...
    if (targets) {
      ...
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (!e.action.summonGO.targetsummon)
          GetBaseObject()->SummonGameObject(e.action.summonGO.entry, x, y, z, o,
                                            0, 0, 0, 0,
                                            e.action.summonGO.despawnTime);
        else
          (*itr)->SummonGameObject(
              e.action.summonGO.entry, GetBaseObject()->GetPositionX(),
              GetBaseObject()->GetPositionY(), GetBaseObject()->GetPositionZ(),
              GetBaseObject()->GetOrientation(), 0, 0, 0, 0,
              e.action.summonGO.despawnTime);
      }
    ...
    GetBaseObject()->SummonGameObject(e.action.summonGO.entry, e.target.x,
                                      e.target.y, e.target.z, e.target.o, 0, 0,
                                      0, 0, e.action.summonGO.despawnTime);
    break;
  }
  ...
  case SMART_ACTION_TELEPORT: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsPlayer(*itr))
        ...
        (*itr)->ToUnit()->NearTeleportTo(e.target.x, e.target.y, e.target.z,
                                         e.target.o);
    }
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsCreature(*itr)) {
        if (IsSmart((*itr)->ToCreature()))
          CAST_AI(SmartAI, (*itr)->ToCreature()->AI())
              ->SetRun(e.action.setRun.run);
        else
          (*itr)->ToCreature()->SetWalk(
    ...
    if (ObjectList *targets = GetTargets(e, unit)) {
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (IsCreature(*itr)) {
          ...
          if (SmartGameObjectAI *ai =
                  CAST_AI(SmartGameObjectAI, (*itr)->ToGameObject()->AI()))
            ai->GetScript()->StoreCounter(e.action.setCounter.counterId,
                                          e.action.setCounter.value,
                                          e.action.setCounter.reset);
          else
      ...
      StoreCounter(e.action.setCounter.counterId, e.action.setCounter.value,
                   e.action.setCounter.reset);
  ...
  case SMART_ACTION_WP_START: {
    ...
    // Xinef: ensure that SMART_ESCORT_TARGETS contains at least one player
    // reference
    bool stored = false;
    ObjectList *targets = GetTargets(e, unit);
    if (targets) {
  ...
  case SMART_ACTION_WP_PAUSE: {
    ...
    CAST_AI(SmartAI, me->AI())
        ->PausePath(delay, e.GetEventType() == SMART_EVENT_WAYPOINT_REACHED
                               ? false
                               : true);
    break;
  }
  ...
  case SMART_ACTION_SET_ORIENTATION: {
    ...
    if (e.GetTargetType() == SMART_TARGET_SELF) {
      me->SetFacingTo((me->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) &&
                               me->GetTransGUID()
                           ? me->GetTransportHomePosition()
                           : me->GetHomePosition())
                          .GetOrientation());
      if (e.action.orientation.quickChange)
        me->SetOrientation((me->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) &&
                                    me->GetTransGUID()
                                ? me->GetTransportHomePosition()
                                : me->GetHomePosition())
                               .GetOrientation());
  ...
  case SMART_ACTION_MOVE_TO_POS: {
    ...
    if (e.GetTargetType() == SMART_TARGET_RANDOM_POINT) {
      if (me) {
        ...
        me->GetRandomPoint(srcPos, range, randomPoint);
        me->GetMotionMaster()->MovePoint(
            e.action.MoveToPos.pointId, randomPoint.m_positionX,
            randomPoint.m_positionY, randomPoint.m_positionZ, true, true,
    ...
    /*if (e.GetTargetType() == SMART_TARGET_CREATURE_RANGE || e.GetTargetType()
    == SMART_TARGET_CREATURE_GUID || e.GetTargetType() ==
    ...
    e.GetTargetType() == SMART_TARGET_STORED) // Xinef: bieda i rozpierdol TC)*/
    {
      if (ObjectList *targets = GetTargets(e, unit)) {
    ...
    if (!target) {
      ...
      me->GetMotionMaster()->MovePoint(
          e.action.MoveToPos.pointId, dest.x, dest.y, dest.z, true, true,
          isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE);
    } else // Xinef: we can use dest.x, dest.y, dest.z to make offset
    {
      ...
      me->GetMotionMaster()->MovePoint(
          e.action.MoveToPos.pointId, x + e.target.x, y + e.target.y,
          z + e.target.z, true, true,
          isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE);
    }
    break;
  }
  case SMART_ACTION_MOVE_TO_POS_TARGET: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsCreature(*itr)) {
        ...
        target->GetMotionMaster()->MovePoint(
            e.action.MoveToPos.pointId, e.target.x, e.target.y, e.target.z,
            true, true,
            isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE);
      }
    }
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsCreature(*itr))
        ...
        if ((*itr)->ToGameObject()->isSpawnedByDefault())
          ...
          (*itr)->ToGameObject()->SetRespawnTime(
        ...
        if (equipId) {
          ...
          if (!einfo) {
            LOG_ERROR("server",
                      ...
                      "equipment info id %u for creature %u",
                      equipId, npc->GetEntry());
            break;
          }
  ...
  case SMART_ACTION_TRIGGER_TIMED_EVENT:
    ProcessEventsFor((SMART_EVENT)SMART_EVENT_TIMED_EVENT_TRIGGERED, nullptr,
  ...
  case SMART_ACTION_CROSS_CAST: {
    ObjectList *casters = GetTargets(
        CreateSmartEvent(
            SMART_EVENT_UPDATE_IC, 0, 0, 0, 0, 0, 0, SMART_ACTION_NONE, 0, 0, 0,
            0, 0, 0, (SMARTAI_TARGETS)e.action.crossCast.targetType,
            e.action.crossCast.targetParam1, e.action.crossCast.targetParam2,
            e.action.crossCast.targetParam3, 0, 0),
        unit);
    if (!casters)
      break;
    ...
    for (ObjectList::const_iterator itr = casters->begin();
      ...
      for (ObjectList::const_iterator it = targets->begin();
        ...
        if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) ||
            !(*it)->ToUnit()->HasAura(e.action.cast.spell)) {
          if (!interruptedSpell &&
              e.action.cast.flags & SMARTCAST_INTERRUPT_PREVIOUS) {
            (*itr)->ToUnit()->InterruptNonMeleeSpells(false);
            interruptedSpell = true;
          }

          (*itr)->ToUnit()->CastSpell(
              (*it)->ToUnit(), e.action.cast.spell,
              (e.action.cast.flags & SMARTCAST_TRIGGERED));
        } else
          LOG_DEBUG("sql.sql",
                    "Spell %u not casted because it has flag "
                    "SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD
                    " Entry: %u Type: %u) already has the aura",
                    e.action.cast.spell, (*it)->GetGUID(), (*it)->GetEntry(),
                    uint32((*it)->GetTypeId()));
      }
    }
  ...
  case SMART_ACTION_SET_UNIT_FIELD_BYTES_1: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsUnit(*itr))
        (*itr)->ToUnit()->SetByteFlag(UNIT_FIELD_BYTES_1,
  ...
  case SMART_ACTION_REMOVE_UNIT_FIELD_BYTES_1: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsUnit(*itr))
        (*itr)->ToUnit()->RemoveByteFlag(UNIT_FIELD_BYTES_1,
      ...
      if (IsCreature(*itr)) {
        if (SmartAI *ai = CAST_AI(SmartAI, (*itr)->ToCreature()->AI()))
          ai->GetScript()->StoreTargetList(
              new ObjectList(*storedTargets),
              e.action.sendTargetToTarget.id); // store a copy of target list
        else
          LOG_ERROR("sql.sql", "SmartScript: Action target for "
                               "SMART_ACTION_SEND_TARGET_TO_TARGET is not "
                               "using SmartAI, skipping");
      } else if (IsGameObject(*itr)) {
        if (SmartGameObjectAI *ai =
                CAST_AI(SmartGameObjectAI, (*itr)->ToGameObject()->AI()))
          ai->GetScript()->StoreTargetList(
          ...
          LOG_ERROR("sql.sql", "SmartScript: Action target for "
                               "SMART_ACTION_SEND_TARGET_TO_TARGET is not "
                               "using SmartGameObjectAI, skipping");
      }
    }
  ...
  case SMART_ACTION_SEND_GOSSIP_MENU: {
    ...
    LOG_DEBUG("sql.sql",
              ...
              "gossipMenuId %d, gossipNpcTextId %d",
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (Player *player = (*itr)->ToPlayer()) {
        if (e.action.sendGossipMenu.gossipMenuId)
          player->PrepareGossipMenu(GetBaseObject(),
                                    e.action.sendGossipMenu.gossipMenuId, true);
        else
          ClearGossipMenuFor(player);

        SendGossipMenuFor(player, e.action.sendGossipMenu.gossipNpcTextId,
      ...
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (IsCreature(*itr)) {
          if (e.action.setHomePos.spawnPos) {
            ...
            (*itr)->ToCreature()->SetHomePosition(
                (*itr)->GetPositionX(), (*itr)->GetPositionY(),
                (*itr)->GetPositionZ(), (*itr)->GetOrientation());
        }
      delete targets;
  ...
  /*{
  ObjectList* movers = GetTargets(CreateSmartEvent(SMART_EVENT_UPDATE_IC, 0, 0,
  0, 0, 0, SMART_ACTION_NONE, 0, 0, 0, 0, 0, 0,
  (SMARTAI_TARGETS)e.action.sethome.targetType, e.action.sethome.targetParam1,
  e.action.sethome.targetParam2, e.action.sethome.targetParam3, 0), unit); if
  (!movers) break;

  if (e.GetTargetType() == SMART_TARGET_POSITION)
  {
  for (ObjectList::const_iterator itr = movers->begin(); itr != movers->end();
  ++itr) if (IsCreature(*itr))
  (*itr)->ToCreature()->SetHomePosition(e.target.x, e.target.y, e.target.z,
  e.target.o);
  }
  else if (ObjectList* targets = GetTargets(e, unit))
  {
  ...
  for (ObjectList::const_iterator itr = movers->begin(); itr != movers->end();
  ++itr) if (IsCreature(*itr))
  (*itr)->ToCreature()->SetHomePosition(target->GetPositionX(),
  ...
  case SMART_ACTION_SET_HEALTH_REGEN: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsCreature(*itr))
        (*itr)->ToCreature()->SetRegeneratingHealth(
  ...
  case SMART_ACTION_SET_ROOT: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsCreature(*itr))
        (*itr)->ToCreature()->SetControlled(e.action.setRoot.root,
                                            UNIT_STATE_ROOT);
  ...
  case SMART_ACTION_SET_GO_FLAG: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsGameObject(*itr))
        (*itr)->ToGameObject()->SetUInt32Value(GAMEOBJECT_FLAGS,
  ...
  case SMART_ACTION_REMOVE_GO_FLAG: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsGameObject(*itr))
        (*itr)->ToGameObject()->RemoveFlag(GAMEOBJECT_FLAGS,
                                           e.action.goFlag.flag);
  ...
  case SMART_ACTION_SET_POWER: {
    ...
    if (targets)
      for (ObjectList::const_iterator itr = targets->begin();
           itr != targets->end(); ++itr)
        if (IsUnit(*itr))
          (*itr)->ToUnit()->SetPower(Powers(e.action.power.powerType),
  ...
  case SMART_ACTION_ADD_POWER: {
    ...
    if (targets)
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (IsUnit(*itr))
          (*itr)->ToUnit()->SetPower(
              Powers(e.action.power.powerType),
  ...
  case SMART_ACTION_GAME_EVENT_STOP: {
    ...
    if (!sGameEventMgr->IsActiveEvent(eventId)) {
      LOG_ERROR("server",
                ...
                "SMART_ACTION_GAME_EVENT_STOP, inactive event (id: %u)",
                eventId);
      break;
    }
              ...
              if (itrWp != path->end()) {
                if (WayPoint *wp = itrWp->second) {
                  float distToThisPath =
                      target->GetDistance(wp->x, wp->y, wp->z);
  ...
  case SMART_ACTION_FLEE: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsCreature(*itr))
        (*itr)->ToCreature()->GetMotionMaster()->MoveFleeing(
  ...
  case SMART_ACTION_LOAD_EQUIPMENT: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsCreature(*itr))
        (*itr)->ToCreature()->LoadEquipment(e.action.loadEquipment.id,
                                            e.action.loadEquipment.force);
  ...
  case SMART_ACTION_TRIGGER_RANDOM_TIMED_EVENT: {
    uint32 eventId =
        urand(e.action.randomTimedEvent.minId, e.action.randomTimedEvent.maxId);
    ProcessEventsFor((SMART_EVENT)SMART_EVENT_TIMED_EVENT_TRIGGERED, nullptr,
                     eventId);
    break;
  }
  ...
  case SMART_ACTION_ADD_IMMUNITY: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
         itr != targets->end(); ++itr)
      if (IsUnit(*itr))
        (*itr)->ToUnit()->ApplySpellImmune(e.action.immunity.id,
                                           e.action.immunity.type,
                                           e.action.immunity.value, true);
  ...
  case SMART_ACTION_PLAYER_TALK: {
    ObjectList *targets = GetTargets(e, unit);
    char const *text =
        sObjectMgr->GetAcoreString(e.action.playerTalk.textId, DEFAULT_LOCALE);
      ...
      for (ObjectList::const_iterator itr = targets->begin();
        ...
        if (IsPlayer(*itr))
          !e.action.playerTalk.flag
              ? (*itr)->ToPlayer()->Say(text, LANG_UNIVERSAL)
              : (*itr)->ToPlayer()->Yell(text, LANG_UNIVERSAL);
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      if (IsUnit(*itr)) {
        ...
        if (e.action.castCustom.flags & SMARTCAST_COMBAT_MOVE) {
          // If cast flag SMARTCAST_COMBAT_MOVE is set combat movement will not
          // be allowed unless target is outside spell range, out of mana, or
          // LOS.

          bool _allowMove = false;
          SpellInfo const *spellInfo =
              sSpellMgr->GetSpellInfo(e.action.castCustom.spell);
          int32 mana = me->GetPower(POWER_MANA);

          if (me->GetDistance((*itr)->ToUnit()) >
                  spellInfo->GetMaxRange(true) ||
              me->GetDistance((*itr)->ToUnit()) <
        ...
        if (!(e.action.castCustom.flags & SMARTCAST_AURA_NOT_PRESENT) ||
            !(*itr)->ToUnit()->HasAura(e.action.castCustom.spell)) {
          ...
          me->CastCustomSpell(e.action.castCustom.spell, values,
                              (*itr)->ToUnit(),
  ...
  case SMART_ACTION_VORTEX_SUMMON: {
    ...
    for (ObjectList::const_iterator itr = targets->begin();
      ...
      do {
        ...
        me->SummonCreature(e.action.summonVortex.summonEntry, summonPosition,
                           summon_type, e.action.summonVortex.summonDuration);
  ...
  case SMART_ACTION_CONE_SUMMON: {
    ...
    for (uint32 radius = 0; radius <= e.action.coneSummon.coneLength;
      ...
      for (uint32 index = 0; index < count; ++index) {
        ...
        me->SummonCreature(e.action.coneSummon.summonEntry, spawnPosition,
                           spawnType, e.action.coneSummon.summonDuration);
      }
    }
  ...
  if (e.link && e.link != e.event_id) {
    ...
    if (linked.GetActionType() && linked.GetEventType() == SMART_EVENT_LINK)
      ...
      LOG_ERROR("sql.sql",
                "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, "
                "Link Event %u not found or invalid, skipped.",
                e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
  }
}

void SmartScript::ProcessTimedAction(SmartScriptHolder &e, uint32 const &min,
                                     uint32 const &max, Unit *unit, uint32 var0,
                                     uint32 var1, bool bvar,
                                     const SpellInfo *spell, GameObject *gob) {
...
void SmartScript::InstallTemplate(SmartScriptHolder const &e) {
  ...
  if (mTemplate) {
    LOG_ERROR("sql.sql",
              "SmartScript::InstallTemplate: Entry %d SourceType %u AI "
  ...
  case SMARTAI_TEMPLATE_CASTER: {
    AddEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, e.action.installTtemplate.param2,
             e.action.installTtemplate.param3, 0, SMART_ACTION_CAST,
             e.action.installTtemplate.param1, e.target.raw.param1, 0, 0, 0, 0,
             SMART_TARGET_VICTIM, 0, 0, 0, 0, 1);
    AddEvent(SMART_EVENT_RANGE, 0, e.action.installTtemplate.param4, 300, 0, 0,
             0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    AddEvent(SMART_EVENT_RANGE, 0, 0,
             e.action.installTtemplate.param4 > 10
                 ? e.action.installTtemplate.param4 - 10
                 : 0,
             0, 0, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 0, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    AddEvent(SMART_EVENT_MANA_PCT, 0,
             e.action.installTtemplate.param5 - 15 > 100
                 ? 100
                 : e.action.installTtemplate.param5 + 15,
             100, 1000, 1000, 0, SMART_ACTION_SET_EVENT_PHASE, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_MANA_PCT, 0, 0, e.action.installTtemplate.param5, 1000,
             1000, 0, SMART_ACTION_SET_EVENT_PHASE, 0, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_MANA_PCT, 0, 0, e.action.installTtemplate.param5, 1000,
             1000, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_NONE, 0, 0, 0, 0, 0);
    break;
  }
  case SMARTAI_TEMPLATE_TURRET: {
    AddEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, e.action.installTtemplate.param2,
             e.action.installTtemplate.param3, 0, SMART_ACTION_CAST,
             e.action.installTtemplate.param1, e.target.raw.param1, 0, 0, 0, 0,
             SMART_TARGET_VICTIM, 0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_JUST_CREATED, 0, 0, 0, 0, 0, 0,
  ...
  case SMARTAI_TEMPLATE_CAGED_NPC_PART: {
    if (!me)
      return;
    // store cage as id1
    AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0,
             SMART_ACTION_STORE_TARGET_LIST, 1, 0, 0, 0, 0, 0,
             SMART_TARGET_CLOSEST_GAMEOBJECT, e.action.installTtemplate.param1,
             10, 0, 0, 0);

    // reset(close) cage on hostage(me) respawn
    AddEvent(SMART_EVENT_UPDATE, SMART_EVENT_FLAG_NOT_REPEATABLE, 0, 0, 0, 0, 0,
             SMART_ACTION_RESET_GOBJECT, 0, 0, 0, 0, 0, 0,
             SMART_TARGET_GAMEOBJECT_DISTANCE, e.action.installTtemplate.param1,
             5, 0, 0, 0);

    AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0, SMART_ACTION_SET_RUN,
             e.action.installTtemplate.param3, 0, 0, 0, 0, 0, SMART_TARGET_NONE,
             0, 0, 0, 0, 0);
    AddEvent(SMART_EVENT_DATA_SET, 0, 0, 0, 0, 0, 0,
             SMART_ACTION_SET_EVENT_PHASE, 1, 0, 0, 0, 0, 0, SMART_TARGET_NONE,
             0, 0, 0, 0, 0);

    AddEvent(SMART_EVENT_UPDATE, SMART_EVENT_FLAG_NOT_REPEATABLE, 1000, 1000, 0,
             0, 0, SMART_ACTION_MOVE_FORWARD, e.action.installTtemplate.param4,
             0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
    // phase 1: give quest credit on movepoint reached
    AddEvent(SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE,
             SMART_RANDOM_POINT, 0, 0, 0, SMART_ACTION_SET_DATA, 0, 0, 0, 0, 0,
             0, SMART_TARGET_STORED, 1, 0, 0, 0, 1);
    // phase 1: despawn after time on movepoint reached
    AddEvent(SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE,
             SMART_RANDOM_POINT, 0, 0, 0, SMART_ACTION_FORCE_DESPAWN,
             e.action.installTtemplate.param2, 0, 0, 0, 0, 0, SMART_TARGET_NONE,
             0, 0, 0, 0, 1);

    if (sCreatureTextMgr->TextExist(me->GetEntry(),
                                    (uint8)e.action.installTtemplate.param5))
      AddEvent(SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE,
               SMART_RANDOM_POINT, 0, 0, 0, SMART_ACTION_TALK,
  ...
  case SMARTAI_TEMPLATE_CAGED_GO_PART: {
    ...
    AddEvent(SMART_EVENT_GO_STATE_CHANGED, 0, 2, 0, 0, 0, 0,
             SMART_ACTION_STORE_TARGET_LIST, 1, 0, 0, 0, 0, 0,
    ...
    // signal hostage
    AddEvent(SMART_EVENT_GO_STATE_CHANGED, 0, 2, 0, 0, 0, 0,
             SMART_ACTION_SET_DATA, 0, 0, 0, 0, 0, 0, SMART_TARGET_STORED, 1, 0,
             0, 0, 0);
...
void SmartScript::AddEvent(SMART_EVENT e, uint32 event_flags,
                           uint32 event_param1, uint32 event_param2,
                           uint32 event_param3, uint32 event_param4,
                           uint32 event_param5, SMART_ACTION action,
                           uint32 action_param1, uint32 action_param2,
                           uint32 action_param3, uint32 action_param4,
                           uint32 action_param5, uint32 action_param6,
                           SMARTAI_TARGETS t, uint32 target_param1,
                           uint32 target_param2, uint32 target_param3,
                           uint32 target_param4, uint32 phaseMask) {
  mInstallEvents.push_back(CreateSmartEvent(
      e, event_flags, event_param1, event_param2, event_param3, event_param4,
      event_param5, action, action_param1, action_param2, action_param3,
      action_param4, action_param5, action_param6, t, target_param1,
      target_param2, target_param3, target_param4, phaseMask));
}

SmartScriptHolder SmartScript::CreateSmartEvent(
    SMART_EVENT e, uint32 event_flags, uint32 event_param1, uint32 event_param2,
    uint32 event_param3, uint32 event_param4, uint32 event_param5,
    ...
    uint32 target_param2, uint32 target_param3, uint32 target_param4,
    uint32 phaseMask) {
...
ObjectList *SmartScript::GetTargets(SmartScriptHolder const &e,
      ...
      if (e.target.hostilRandom.powerType) {
        if (Unit *u = me->AI()->SelectTarget(
                ...
                PowerUsersSelector(me,
                                   Powers(e.target.hostilRandom.powerType - 1),
                                   (float)e.target.hostilRandom.maxDist,
                                   e.target.hostilRandom.playerOnly)))
          l->push_back(u);
  ...
  case SMART_TARGET_FARTHEST:
    if (me) {
      if (Unit *u = me->AI()->SelectTarget(
              SELECT_TARGET_FARTHEST, 0,
              FarthestTargetSelector(me, e.target.farthest.maxDist,
      ...
      if (Player *player = scriptTrigger->ToPlayer()) {
        if (Group *group = player->GetGroup()) {
          for (GroupReference *groupRef = group->GetFirstMember();
               groupRef != nullptr; groupRef = groupRef->next())
  ...
  case SMART_TARGET_CREATURE_RANGE: {
    ...
    ObjectList *units =
        GetWorldObjectsInDist((float)e.target.unitRange.maxDist);
    for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
         ++itr) {
      if (!IsCreature(*itr))
        continue;
      ...
      if (((e.target.unitRange.creature &&
          ...
          baseObject->IsInRange(*itr, (float)e.target.unitRange.minDist,
  ...
  case SMART_TARGET_CREATURE_DISTANCE: {
    // will always return a valid pointer, even if empty list
    ObjectList *units =
        GetWorldObjectsInDist((float)e.target.unitDistance.dist);
    for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
         ++itr) {
      if (!IsCreature(*itr))
        continue;
  ...
  case SMART_TARGET_GAMEOBJECT_DISTANCE: {
    ...
    for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
      ...
      if ((e.target.goDistance.entry &&
           (*itr)->ToGameObject()->GetEntry() == e.target.goDistance.entry) ||
  ...
  case SMART_TARGET_GAMEOBJECT_RANGE: {
    ...
    for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
      ...
      if (((e.target.goRange.entry && IsGameObject(*itr) &&
            (*itr)->ToGameObject()->GetEntry() == e.target.goRange.entry) ||
           !e.target.goRange.entry) &&
          baseObject->IsInRange((*itr), (float)e.target.goRange.minDist,
  ...
  case SMART_TARGET_CREATURE_GUID: {
    ...
    if (e.target.unitGUID.getFromHashMap) {
      if ((target = ObjectAccessor::GetCreature(
               scriptTrigger ? *scriptTrigger : *GetBaseObject(),
               MAKE_NEW_GUID(e.target.unitGUID.dbGuid, e.target.unitGUID.entry,
                             HIGHGUID_UNIT))))
        l->push_back(target);
    } else {
      target = FindCreatureNear(scriptTrigger ? scriptTrigger : GetBaseObject(),
  ...
  case SMART_TARGET_GAMEOBJECT_GUID: {
    ...
    if (e.target.goGUID.getFromHashMap) {
      if ((target = ObjectAccessor::GetGameObject(
               scriptTrigger ? *scriptTrigger : *GetBaseObject(),
               MAKE_NEW_GUID(e.target.goGUID.dbGuid, e.target.goGUID.entry,
                             HIGHGUID_GAMEOBJECT))))
        l->push_back(target);
    } else {
      target =
  ...
  case SMART_TARGET_PLAYER_RANGE: {
    // will always return a valid pointer, even if empty list
    ObjectList *units =
        GetWorldObjectsInDist((float)e.target.playerRange.maxDist);
    if (!units->empty() && GetBaseObject()) {
      for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
           ++itr)
        if (IsPlayer(*itr) &&
            ...
            !(*itr)->ToPlayer()->IsGameMaster())
          l->push_back(*itr);

      // If Orientation is also set and we didnt find targets, try it with all
      // the range
      if (l->empty() && e.target.o > 0)
        for (ObjectList::const_iterator itr = units->begin();
             itr != units->end(); ++itr)
          if (IsPlayer(*itr) &&
              baseObject->IsInRange(*itr, 0.0f,
                                    float(e.target.playerRange.maxDist)) &&
              (*itr)->ToPlayer()->IsAlive() &&
              !(*itr)->ToPlayer()->IsGameMaster())
            l->push_back(*itr);
      ...
      if (e.target.playerRange.maxCount > 0)
        Warhead::Containers::RandomResizeList(*l,
  ...
  case SMART_TARGET_PLAYER_DISTANCE: {
    // will always return a valid pointer, even if empty list
    ObjectList *units =
        GetWorldObjectsInDist((float)e.target.playerDistance.dist);
    for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
         ++itr)
      if (IsPlayer(*itr))
        l->push_back(*itr);
  ...
  case SMART_TARGET_STORED: {
    ...
    // xinef: return l, retardness... what if list is empty? will return empty
    // list instead of null pointer
    break;
  }
  case SMART_TARGET_CLOSEST_CREATURE: {
    Creature *target = GetClosestCreatureWithEntry(
        GetBaseObject(), e.target.closest.entry,
        (float)(e.target.closest.dist ? e.target.closest.dist : 100),
        !e.target.closest.dead);
    if (target)
      l->push_back(target);
    break;
  }
  ...
  case SMART_TARGET_CLOSEST_PLAYER: {
    if (WorldObject *obj = GetBaseObject()) {
      Player *target =
          obj->SelectNearestPlayer((float)e.target.playerDistance.dist);
      if (target)
        l->push_back(target);
    }
    break;
  ...
  case SMART_TARGET_OWNER_OR_SUMMONER: {
    if (me) {
      if (Unit *owner =
              ObjectAccessor::GetUnit(*me, me->GetCharmerOrOwnerGUID()))
        l->push_back(owner);
    ...
    if (e.target.owner.useCharmerOrOwner && !l->empty()) {
      ...
      if (Unit *base =
              ObjectAccessor::GetUnit(*owner, owner->GetCharmerOrOwnerGUID()))
        l->push_back(base);
    }
    break;
  }
  ...
  case SMART_TARGET_CLOSEST_ENEMY: {
    if (me)
      if (Unit *target =
              me->SelectNearestTarget(e.target.closestAttackable.maxDist,
  ...
  case SMART_TARGET_CLOSEST_FRIENDLY: {
    if (me)
      if (Unit *target =
              DoFindClosestFriendlyInRange(e.target.closestFriendly.maxDist,
  ...
  case SMART_TARGET_PLAYER_WITH_AURA: {
    ...
    ObjectList *units = GetWorldObjectsInDist(
        e.target.z ? e.target.z : float(e.target.playerWithAura.distMax));
    for (ObjectList::const_iterator itr = units->begin(); itr != units->end();
         ++itr)
      if (IsPlayer(*itr) && (*itr)->ToPlayer()->IsAlive() &&
          !(*itr)->ToPlayer()->IsGameMaster())
        if (GetBaseObject()->IsInRange(*itr,
                                       (float)e.target.playerWithAura.distMin,
                                       (float)e.target.playerWithAura.distMax))
          if (bool(e.target.playerWithAura.negation) !=
              (*itr)->ToPlayer()->HasAura(e.target.playerWithAura.spellId))
            l->push_back(*itr);
  ...
  case SMART_TARGET_ROLE_SELECTION: {
    // will always return a valid pointer, even if empty list
    ObjectList *units =
        GetWorldObjectsInDist(float(e.target.roleSelection.maxDist));
...
ObjectList *SmartScript::GetWorldObjectsInDist(float dist) {
  ...
  if (obj) {
    ...
    Warhead::WorldObjectListSearcher<Warhead::AllWorldObjectsInRange> searcher(
        obj, *targets, u_check);
...
void SmartScript::ProcessEvent(SmartScriptHolder &e, Unit *unit, uint32 var0,
  ...
  case SMART_EVENT_UPDATE:
    ProcessTimedAction(e, e.event.minMaxRepeat.repeatMin,
  ...
  case SMART_EVENT_TARGET_HEALTH_PCT: {
    ...
    if (perc > e.event.minMaxRepeat.max || perc < e.event.minMaxRepeat.min)
      return;
    ProcessTimedAction(e, e.event.minMaxRepeat.repeatMin,
                       e.event.minMaxRepeat.repeatMax, me->GetVictim());
    break;
  }
  ...
  case SMART_EVENT_VICTIM_CASTING: {
    ...
    ProcessTimedAction(e, e.event.targetCasting.repeatMin,
                       e.event.targetCasting.repeatMax, me->GetVictim());
    break;
  }
  case SMART_EVENT_FRIENDLY_HEALTH: {
    ...
    Unit *target = DoSelectLowestHpFriendly(
        (float)e.event.friendlyHealth.radius, e.event.friendlyHealth.hpDeficit);
    if (!target || !target->IsInCombat()) {
      // Xinef: if there are at least two same npcs, they will perform the same
      // action immediately even if this is useless...
      RecalcTimer(e, 1000, 3000);
      return;
    }
    ProcessTimedAction(e, e.event.friendlyHealth.repeatMin,
                       e.event.friendlyHealth.repeatMax, target);
    break;
  }
  case SMART_EVENT_FRIENDLY_IS_CC: {
    ...
    ProcessTimedAction(
        ...
        Warhead::Containers::SelectRandomContainerElement(pList));
    break;
  }
  ...
  case SMART_EVENT_IS_BEHIND_TARGET: {
    ...
    if (Unit *victim = me->GetVictim()) {
      if (!victim->HasInArc(static_cast<float>(M_PI), me))
        ProcessTimedAction(e, e.event.behindTarget.cooldownMin,
                           e.event.behindTarget.cooldownMax, victim);
    }
    break;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("sql.sql", "SmartScript: Gossip Select:  menu %u action %u", var0,
  ...
  case SMART_EVENT_FRIENDLY_HEALTH_PCT: {
    ...
    for (ObjectList::const_iterator itr = _targets->begin();
         itr != _targets->end(); ++itr) {
      if (IsUnit(*itr) && me->IsFriendlyTo((*itr)->ToUnit()) &&
          (*itr)->ToUnit()->IsAlive() && (*itr)->ToUnit()->IsInCombat()) {
    ...
    ProcessTimedAction(e, e.event.friendlyHealthPct.repeatMin,
                       e.event.friendlyHealthPct.repeatMax, target);
    break;
  }
  case SMART_EVENT_DISTANCE_CREATURE: {
    ...
    if (e.event.distance.guid != 0) {
      ...
      me->GetCreatureListWithEntryInGrid(list, e.event.distance.entry,
                                         (float)e.event.distance.dist);
  ...
  case SMART_EVENT_DISTANCE_GAMEOBJECT: {
    ...
    if (e.event.distance.guid != 0) {
      ...
      me->GetGameObjectListWithEntryInGrid(list, e.event.distance.entry,
                                           (float)e.event.distance.dist);
  ...
  case SMART_EVENT_COUNTER_SET:
    ...
    ProcessTimedAction(e, e.event.counter.cooldownMin,
  ...
  case SMART_EVENT_NEAR_PLAYERS: {
    ...
    if (!units->empty()) {
      units->remove_if(
          [](WorldObject *unit) { return unit->GetTypeId() != TYPEID_PLAYER; });

      if (units->size() >= e.event.nearPlayer.minCount)
        ProcessAction(e, unit);
    }
    RecalcTimer(e, e.event.nearPlayer.checkTimer,
                e.event.nearPlayer.checkTimer);
    break;
  }
  case SMART_EVENT_NEAR_PLAYERS_NEGATION: {
    ...
    RecalcTimer(e, e.event.nearPlayerNegation.checkTimer,
                e.event.nearPlayerNegation.checkTimer);
    break;
  }
  default:
    LOG_ERROR("sql.sql", "SmartScript::ProcessEvent: Unhandled Event type %u",
              e.GetEventType());
    break;
  }
}
...
void SmartScript::InitTimer(SmartScriptHolder &e) {
  ...
  case SMART_EVENT_NEAR_PLAYERS:
  case SMART_EVENT_NEAR_PLAYERS_NEGATION:
    RecalcTimer(e, e.event.nearPlayer.firstTimer,
                e.event.nearPlayer.firstTimer);
    break;
  case SMART_EVENT_UPDATE:
  case SMART_EVENT_UPDATE_IC:
  case SMART_EVENT_UPDATE_OOC:
  ...
  case SMART_EVENT_OOC_LOS:
  case SMART_EVENT_IC_LOS:
  // Xinef: wtf is this bullshit? cooldown should be processed AFTER action is
  // done, not before...
  // RecalcTimer(e, e.event.los.cooldownMin, e.event.los.cooldownMax);
  // break;
  case SMART_EVENT_DISTANCE_CREATURE:
...
void SmartScript::UpdateTimer(SmartScriptHolder &e, uint32 const diff) {
  ...
  if (e.GetEventType() == SMART_EVENT_UPDATE_OOC &&
      (me && me->IsInCombat())) // can be used with me=nullptr (go script)
    return;
  ...
  if (e.timer < diff) {
    ...
    case SMART_EVENT_DISTANCE_GAMEOBJECT: {
      ...
      if (e.GetScriptType() == SMART_SCRIPT_TYPE_TIMED_ACTIONLIST) {
        e.enableTimed = false; // disable event if it is in an ActionList and
                               // was processed once
...
void SmartScript::FillScript(SmartAIEventList e, WorldObject *obj,
  ...
  if (e.empty()) {
    ...
    if (obj)
      LOG_DEBUG("sql.sql",
                "SmartScript: EventMap for Entry %u is empty but is using "
                "SmartScript.",
                obj->GetEntry());
  ...
  for (SmartAIEventList::iterator i = e.begin(); i != e.end(); ++i) {
    ...
    if ((*i).event.event_flags &
        SMART_EVENT_FLAG_DIFFICULTY_ALL) // if has instance flag add only if in
                                         // it
    {
    ...
    mEvents.push_back((*i)); // NOTE: 'world(0)' events still get processed in
                             // ANY instance mode
  }
}
...
void SmartScript::OnInitialize(WorldObject *obj, AreaTrigger const *at) {
  if (obj) // handle object based scripts
  {
    ...
    case TYPEID_UNIT:
      ...
      me = obj->ToCreature();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("sql.sql", "SmartScript::OnInitialize: source is Creature %u",
                me->GetEntry());
#endif
      break;
    case TYPEID_GAMEOBJECT:
      ...
      go = obj->ToGameObject();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("sql.sql", "SmartScript::OnInitialize: source is GameObject %u",
                go->GetEntry());
#endif
      break;
    default:
    ...
    trigger = at;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("sql.sql", "SmartScript::OnInitialize: source is AreaTrigger %u",
              trigger->entry);
#endif
  } else {
    LOG_ERROR("server", "SmartScript::OnInitialize: !WARNING! Initialized "
                        "objects are nullptr.");
    return;
  }
  ...
  for (SmartAIEventList::iterator i = mEvents.begin(); i != mEvents.end();
    ...
    if (i->GetEventType() == SMART_EVENT_RANGE &&
        i->GetActionType() == SMART_ACTION_ALLOW_COMBAT_MOVEMENT) {
      if (i->action.combatMove.move == 1 &&
          i->event.minMaxRepeat.min > minEnableDist)
        minEnableDist = i->event.minMaxRepeat.min;
      else if (i->action.combatMove.move == 0 &&
               (i->event.minMaxRepeat.max < maxDisableDist ||
                maxDisableDist == 0))
        maxDisableDist = i->event.minMaxRepeat.max;
    }

    // Xinef: if smartcast combat move flag is present
    if (i->GetActionType() == SMART_ACTION_CAST &&
        (i->action.cast.flags & SMARTCAST_COMBAT_MOVE)) {
      if (const SpellInfo *spellInfo =
              sSpellMgr->GetSpellInfo(i->action.cast.spell)) {
  ...
  if (maxDisableDist > 0 && minEnableDist >= maxDisableDist)
    mMaxCombatDist =
        uint32(maxDisableDist + ((minEnableDist - maxDisableDist) / 2));
...
void SmartScript::OnMoveInLineOfSight(Unit *who) {
  ...
  ProcessEventsFor(me->IsInCombat() ? SMART_EVENT_IC_LOS : SMART_EVENT_OOC_LOS,
                   who);
}
...
Unit *SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff) {
  ...
  Warhead::UnitLastSearcher<Warhead::MostHPMissingInRange> searcher(me, unit,
                                                                    u_check);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature *> &_list, float range) {
  ...
  Warhead::CreatureListSearcher<Warhead::FriendlyCCedInRange> searcher(
      me, _list, u_check);
  me->VisitNearbyObject(range, searcher);
}
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature *> &list,
  ...
  Warhead::CreatureListSearcher<Warhead::FriendlyMissingBuffInRange> searcher(
      me, list, u_check);
  me->VisitNearbyObject(range, searcher);
}
...
Unit *SmartScript::DoFindClosestFriendlyInRange(float range, bool playerOnly) {
  ...
  Unit *unit = nullptr;
  Warhead::AnyFriendlyNotSelfUnitInObjectRangeCheck u_check(me, me, range,
                                                            playerOnly);
  Warhead::UnitLastSearcher<Warhead::AnyFriendlyNotSelfUnitInObjectRangeCheck>
...
void SmartScript::SetScript9(SmartScriptHolder &e, uint32 entry) {
  ...
  if (isProcessingTimedActionList) {
    LOG_ERROR(
        "server",
        "Entry %d SourceType %u Event %u Action %u is trying to overwrite "
        "timed action list from a timed action, this is not allowed!.",
        e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
    return;
  }
  ...
  for (SmartAIEventList::iterator i = mTimedActionList.begin();
    ...
    i->enableTimed =
        i ==
        mTimedActionList.begin(); // enable processing only for the first action
