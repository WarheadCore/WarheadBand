...
npc_escortAI::npc_escortAI(Creature *creature)
    : ScriptedAI(creature), m_uiPlayerGUID(0), m_uiWPWaitTimer(1000),
      m_uiPlayerCheckTimer(0), m_uiEscortState(STATE_ESCORT_NONE),
      ...
      for (GroupReference *groupRef = group->GetFirstMember();
        ...
        if (Player *member = groupRef->GetSource())
          if (member->IsInMap(player) &&
              member->GetQuestStatus(m_pQuestForEscort->GetQuestId()) ==
...
void npc_escortAI::EnterEvadeMode() {
  ...
  if (HasEscortState(STATE_ESCORT_ESCORTING)) {
    ...
    LOG_DEBUG(
        "scripts.ai",
        "TSCR: EscortAI has left combat and is now returning to last point");
#endif
  } else {
  ...
  if (HasEscortState(STATE_ESCORT_ESCORTING) && !me->GetVictim() &&
    ...
    if (m_uiWPWaitTimer <= diff) {
      if (CurrentWP == WaypointList.end()) {
        ...
        // xinef: remove escort state, escort was finished (lack of this line
        // resulted in skipping UpdateEscortAI calls after finished escort)
  ...
  if (moveType == POINT_MOTION_TYPE) {
    ...
    if (pointId == POINT_LAST_POINT) {
      ...
      LOG_DEBUG("scripts.ai",
                "TSCR: EscortAI has returned to original home location and "
    ...
    if (m_uiWPWaitTimer <= 1 && !HasEscortState(STATE_ESCORT_PAUSED) &&
      ...
      // Call WP function
      me->SetPosition(CurrentWP->x, CurrentWP->y, CurrentWP->z,
                      me->GetOrientation());
...
void npc_escortAI::FillPointMovementListForCreature() {
  ...
  for (ScriptPointVector::const_iterator itr = movePoints.begin();
       itr != itrEnd; ++itr) {
    Escort_Waypoint point(itr->uiPointId, itr->fX, itr->fY, itr->fZ,
                          itr->uiWaitTime);
...
void npc_escortAI::Start(bool isActiveAttacker /* = true*/,
                         bool run /* = false */, uint64 playerGUID /* = 0 */,
  ...
  if (me->GetVictim()) {
    LOG_ERROR("server",
              "TSCR ERROR: EscortAI (script: %s, creature entry: %u) attempts "
  ...
  if (HasEscortState(STATE_ESCORT_ESCORTING)) {
    LOG_ERROR("server",
              "TSCR: EscortAI (script: %s, creature entry: %u) attempts to "
              "Start while already escorting",
              me->GetScriptName().c_str(), me->GetEntry());
    return;
  }
  ...
  if (WaypointList.empty()) {
    LOG_ERROR(
        "sql.sql",
        "TSCR: EscortAI (script: %s, creature entry: %u) starts with 0 "
        "waypoints (possible missing entry in script_waypoint. Quest: %u).",
        me->GetScriptName().c_str(), me->GetEntry(),
  ...
  if (m_bCanReturnToStart && m_bCanInstantRespawn)
    LOG_DEBUG("scripts.ai",
              "TSCR: EscortAI is set to return home after waypoint end and "
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("scripts.ai",
            "TSCR: EscortAI started with " UI64FMTD
            " waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = " UI64FMTD
            "",
...
bool npc_escortAI::SetNextWaypoint(uint32 pointId, float x, float y, float z,
                                   float orientation) {
...
bool npc_escortAI::SetNextWaypoint(uint32 pointId, bool setPosition) {
  ...
  for (CurrentWP = WaypointList.begin(); CurrentWP != WaypointList.end();
    ...
    if (CurrentWP->id == pointId) {
      if (setPosition)
        me->UpdatePosition(CurrentWP->x, CurrentWP->y, CurrentWP->z,
                           me->GetOrientation());
      return true;
    }
  }
...
bool npc_escortAI::GetWaypointPosition(uint32 pointId, float &x, float &y,
                                       float &z) {
...
void npc_escortAI::GenerateWaypointArray(Movement::PointsArray *points) {
  ...
  if (me->m_movementInfo.HasMovementFlag((MovementFlags)(
    ...
    for (float size = 1.0f; size; size *= 0.5f) {
      ...
      uint32 cnt = 0;
      for (std::list<Escort_Waypoint>::const_iterator itr = CurrentWP;
           itr != WaypointList.end() && cnt <= length; ++itr, ++cnt)
