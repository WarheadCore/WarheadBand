...
ScriptedAI::ScriptedAI(Creature *creature)
    : CreatureAI(creature), me(creature), IsFleeing(false),
      _evadeCheckCooldown(2500), _isCombatMovementAllowed(true) {
...
void ScriptedAI::DoCastSpell(Unit *target, SpellInfo const *spellInfo,
                             bool triggered) {
  if (!target || me->IsNonMeleeSpellCast(false))
    return;

  me->StopMoving();
  me->CastSpell(target, spellInfo,
...
void ScriptedAI::DoPlaySoundToSet(WorldObject *source, uint32 soundId) {
  ...
  if (!sSoundEntriesStore.LookupEntry(soundId)) {
    LOG_ERROR("server",
              "TSCR: Invalid soundId %u used in DoPlaySoundToSet (Source: "
              "TypeId %u, GUID %u)",
              soundId, source->GetTypeId(), source->GetGUIDLow());
    return;
  }
...
Creature *ScriptedAI::DoSpawnCreature(uint32 entry, float offsetX,
                                      float offsetY, float offsetZ, float angle,
                                      uint32 type, uint32 despawntime) {
  return me->SummonCreature(
      entry, me->GetPositionX() + offsetX, me->GetPositionY() + offsetY,
      me->GetPositionZ() + offsetZ, angle, TempSummonType(type), despawntime);
}

SpellInfo const *ScriptedAI::SelectSpell(Unit *target, uint32 school,
                                         uint32 mechanic,
                                         SelectTargetType targets,
                                         uint32 powerCostMin,
                                         uint32 powerCostMax, float rangeMin,
                                         float rangeMax, SelectEffect effects) {
  ...
  for (uint32 i = 0; i < MAX_CREATURE_SPELLS; i++) {
    ...
    if (me->IsWithinDistInMap(
            target, float(me->GetSpellMinRangeForTarget(target, tempSpell))) ||
        !me->IsWithinDistInMap(
            target, float(me->GetSpellMaxRangeForTarget(target, tempSpell))))
      continue;
...
void ScriptedAI::DoResetThreat() {
  if (!me->CanHaveThreatList() || me->getThreatManager().isThreatListEmpty()) {
    LOG_ERROR("server",
              "DoResetThreat called for creature that either cannot have "
              "threat list or has empty threat list (me entry = %d)",
              me->GetEntry());
    return;
  }
...
void ScriptedAI::DoTeleportPlayer(Unit *unit, float x, float y, float z,
                                  float o) {
  if (!unit)
    return;
  ...
  if (Player *player = unit->ToPlayer())
    ...
    LOG_ERROR("server",
              "TSCR: Creature " UI64FMTD
              " (Entry: %u) Tried to teleport non-player unit (Type: %u "
              "GUID: " UI64FMTD ") to x: %f y:%f z: %f o: %f. Aborted.",
              me->GetGUID(), me->GetEntry(), unit->GetTypeId(), unit->GetGUID(),
              x, y, z, o);
}
...
void ScriptedAI::DoTeleportAll(float x, float y, float z, float o) {
  ...
  for (Map::PlayerList::const_iterator itr = PlayerList.begin();
    ...
    if (Player *player = itr->GetSource())
      if (player->IsAlive())
        player->TeleportTo(me->GetMapId(), x, y, z, o,
...
std::list<Creature *> ScriptedAI::DoFindFriendlyCC(float range) {
  ...
  Warhead::CreatureListSearcher<Warhead::FriendlyCCedInRange> searcher(me, list,
                                                                       u_check);
  me->VisitNearbyObject(range, searcher);
  return list;
}
...
Player *ScriptedAI::GetPlayerAtMinimumRange(float minimumRange) {
  ...
  CellCoord pair(
      Warhead::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
  Cell cell(pair);
  cell.SetNoCreate();

  Warhead::PlayerAtMinimumRangeAway check(me, minimumRange);
  Warhead::PlayerSearcher<Warhead::PlayerAtMinimumRangeAway> searcher(
      me, player, check);
  TypeContainerVisitor<
      Warhead::PlayerSearcher<Warhead::PlayerAtMinimumRangeAway>,
...
Player *ScriptedAI::SelectTargetFromPlayerList(float maxdist,
  ...
  for (Map::PlayerList::const_iterator itr = pList.begin(); itr != pList.end();
       ++itr) {
    if (!me->IsWithinDistInMap(itr->GetSource(), maxdist) ||
...
bool BossAI::CheckBoundary(Unit *who) {
  ...
  for (BossBoundaryMap::const_iterator itr = GetBoundary()->begin();
       itr != GetBoundary()->end(); ++itr) {
    switch (itr->first) {
    case BOUNDARY_N:
...
void GetCreatureListWithEntryInGrid(std::list<Creature *> &list,
                                    WorldObject *source, uint32 entry,
                                    float maxSearchRange) {
