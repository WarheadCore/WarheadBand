...
Group::Group()
    : m_leaderGuid(0), m_leaderName(""), m_groupType(GROUPTYPE_NORMAL),
      ...
      m_masterLooterGuid(0), m_subGroupsCounts(nullptr), m_guid(0),
...
Group::~Group() {
  if (m_bgGroup) {
    ...
    if (m_bgGroup->GetBgRaid(TEAM_ALLIANCE) == this)
      ...
      LOG_ERROR("server", "Group::~Group: battleground group is not linked to "
                          "the correct battleground.");
  }
  Rolls::iterator itr;
  while (!RollId.empty()) {
    itr = RollId.begin();
...
bool Group::LoadGroupFromDB(Field *fields) {
  ...
  m_masterLooterGuid =
      MAKE_NEW_GUID(fields[15].GetUInt32(), 0, HIGHGUID_PLAYER);
...
void Group::LoadMemberFromDB(uint32 guidLow, uint8 memberFlags, uint8 subgroup,
                             uint8 roles) {
  MemberSlot member;
...
void Group::ConvertToLFG() {
  m_groupType =
      GroupType(m_groupType | GROUPTYPE_LFG | GROUPTYPE_LFG_RESTRICTED);
...
bool Group::AddMember(Player *player) {
  ...
  if (player->GetGroup()) {
    if (isBGGroup() ||
        isBFGroup()) // if player is in group and he is being added to BG raid
                     // group, then call SetBattlegroundRaid()
      player->SetBattlegroundOrBattlefieldRaid(this, subGroup);
    else // if player is in bg raid and we are adding him to normal group, then
  ...
  if (player) {
    ...
    if (!IsLeader(player->GetGUID()) && !isBGGroup() && !isBFGroup()) {
      Player::ResetInstances(player->GetGUIDLow(), INSTANCE_RESET_GROUP_JOIN,
                             false);
...

    {
      ...
      for (GroupReference *itr = GetFirstMember(); itr != nullptr;
           itr = itr->next()) {
        if (itr->GetSource() == player) // pussywizard: no check same map,
                                        // adding members is single threaded
          continue;
...
bool Group::RemoveMember(
    uint64 guid, const RemoveMethod &method /*= GROUP_REMOVEMETHOD_DEFAULT*/,
    uint64 kicker /*= 0*/, const char *reason /*= nullptr*/) {
  BroadcastGroupUpdate();

  // LFG group vote kick handled in scripts
  if (isLFGGroup() && method == GROUP_REMOVEMETHOD_KICK) {
    sLFGMgr->InitBoot(GetGUID(), kicker, guid,
  ...
  if (GetMembersCount() >
    ...
    // Reevaluate group enchanter if the leaving player had enchanting skill or
    ...
    for (Rolls::iterator it = RollId.begin(); it != RollId.end();) {
      ...
      // Xinef: player is removed from all vote lists so it will not pass above
      // playerVote == playerVote.end statement during second iteration
    ...
    if (isLFGGroup() && GetMembersCount() == 1) {
      Player *leader =
          ObjectAccessor::FindPlayerInOrOutOfWorld(GetLeaderGUID());
...
void Group::ChangeLeader(uint64 newLeaderGuid) {
  ...
  if (Player *oldLeader =
          ObjectAccessor::FindPlayerInOrOutOfWorld(m_leaderGuid))
...
void Group::SendLootStartRollToPlayer(uint32 countDown, uint32 mapId, Player *p,
                                      bool canNeed, Roll const &r) {
  if (!p)
    return;
...
void Group::SendLootRoll(uint64 sourceGuid, uint64 targetGuid, uint8 rollNumber,
                         uint8 rollType, Roll const &roll) {
...
void Group::SendLootRollWon(uint64 sourceGuid, uint64 targetGuid,
...
void Group::GroupLoot(Loot *loot, WorldObject *pLootedObject) {
  ...
  for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot) {
    ...
    if (item->Quality >= uint32(m_lootThreshold)) {
      uint64 newitemGUID = MAKE_NEW_GUID(
          sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM), 0, HIGHGUID_ITEM);
        ...
        if (member->IsAtGroupRewardDistance(pLootedObject)) {
          if (i->AllowedForPlayer(member)) {
            ...
            if (member->GetPassOnGroupLoot()) {
              ...
              r->totalPass++;
              // can't broadcast the pass now. need to wait until all rolling
              // players are known.
            } else
...
void Group::NeedBeforeGreed(Loot *loot, WorldObject *lootedObject) {
    ...
    if (item->Quality >= uint32(m_lootThreshold)) {
      ...
      for (GroupReference *itr = GetFirstMember(); itr != nullptr;
        ...
        if (i->AllowedForPlayer(playerToRoll) &&
            playerToRoll->IsAtGroupRewardDistance(lootedObject)) {
          r->totalPlayersRolling++;
        ...
        for (Roll::PlayerVote::const_iterator itr = r->playerVote.begin();
          ...
          if (itr->second == PASS)
            ...
            SendLootStartRollToPlayer(
                60000, lootedObject->GetMapId(), p,
                p->CanRollForItemInLFG(item, lootedObject) == EQUIP_ERR_OK, *r);
        }
  ...
  for (std::vector<LootItem>::iterator i = loot->quest_items.begin();
       i != loot->quest_items.end(); ++i, ++itemSlot) {
    if (!i->follow_loot_rules)
      continue;
...
void Group::EndRoll(Loot *pLoot, Map *allowedMap) {
  for (Rolls::iterator itr = RollId.begin(); itr != RollId.end();) {
    if ((*itr)->getLoot() == pLoot) {
      CountTheRoll(itr, allowedMap); // i don't have to edit player votes, who
                                     // didn't vote ... he will pass
      itr = RollId.begin();
    } else
      ++itr;
  }
...
void Group::CountTheRoll(Rolls::iterator rollI, Map *allowedMap) {
  ...
  if (roll->totalNeed > 0) {
    if (!roll->playerVote.empty()) {
      ...
      for (Roll::PlayerVote::const_iterator itr = roll->playerVote.begin();
        ...
        if (!player ||
            (allowedMap != nullptr && player->FindMap() != allowedMap)) {
          --roll->totalNeed;
          continue;
        }
      ...
      if (maxguid) // pussywizard: added condition
      {
        ...
        if (player) {
          player->UpdateAchievementCriteria(
              ACHIEVEMENT_CRITERIA_TYPE_ROLL_NEED_ON_LOOT, roll->itemid,
              maxresul);
                            ...
                            // above if player is on a different map
  {
    if (!roll->playerVote.empty()) {
      ...
      if (maxguid) // pussywizard: added condition
      {
        ...
        if (player) {
          player->UpdateAchievementCriteria(
              ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED_ON_LOOT, roll->itemid,
              maxresul);
          ...
          if (rollvote == GREED) {
            ...
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL, 13262); // Disenchant
          }
        }
      } else
        roll->totalGreed = 0;
  ...
  if (roll->totalNeed == 0 &&
      roll->totalGreed ==
          0) // pussywizard: if, not else, because numbers can be modified above
             // if player is on a different map
  {
    ...
    LootItem *item = &(
        roll->itemSlot >= roll->getLoot()->items.size()
            ? roll->getLoot()
                  ->quest_items[roll->itemSlot - roll->getLoot()->items.size()]
            : roll->getLoot()->items[roll->itemSlot]);
    if (item)
      item->is_blocked = false;
  }
...
void Group::SendUpdateToPlayer(uint64 playerGUID, MemberSlot *slot) {
  ...
  WorldPacket data(SMSG_GROUP_LIST,
                   (1 + 1 + 1 + 1 + 1 + 4 + 8 + 4 + 4 +
                    (GetMembersCount() - 1) * (13 + 8 + 1 + 1 + 1 + 1) + 8 + 1 +
                    8 + 1 + 1 + 1 + 1));
  ...
  for (member_citerator citr = m_memberSlots.begin();
    ...
    onlineState =
        onlineState | ((isBGGroup() || isBFGroup()) ? MEMBER_STATUS_PVP : 0);
  ...
  if (GetMembersCount() - 1) {
    ...
    data << uint8(m_raidDifficulty >=
                  RAID_DIFFICULTY_10MAN_HEROIC); // 3.3 Dynamic Raid Difficulty
                                                 // - 0 normal/1 heroic
  }
...
void Group::BroadcastPacket(WorldPacket *packet, bool ignorePlayersInBGRaid,
  ...
  for (GroupReference *itr = GetFirstMember(); itr != nullptr;
    ...
    if (!player || (ignore != 0 && player->GetGUID() == ignore) ||
...
void Group::ChangeMembersGroup(uint64 guid, uint8 group) {
  ...
  if (Player *player = ObjectAccessor::FindPlayerInOrOutOfWorld(guid)) {
    ...
    else // If player is in BG raid, it is possible that he is also in normal
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(
    Battleground const *bgTemplate, BattlegroundQueueTypeId /*bgQueueTypeId*/,
  ...
  for (GroupReference *itr = GetFirstMember(); itr != nullptr;
    ...
    // pussywizard: check for free slot, this is actually ensured before calling
    ...
    // don't let join to bg queue random if someone from the group is already in
    // bg queue
    if (bgTemplate->GetBgTypeID() == BATTLEGROUND_RB &&
        member->InBattlegroundQueue())
      return ERR_IN_NON_RANDOM_BG;

    // don't let Death Knights join BG queues when they are not allowed to be
    // teleported yet
    if (member->getClass() == CLASS_DEATH_KNIGHT && member->GetMapId() == 609 &&
...
void Group::ResetInstances(uint8 method, bool isRaid, Player *leader) {
  ...
  case INSTANCE_RESET_ALL: {
    ...
    BoundInstancesMap const &m_boundInstances =
        sInstanceSaveMgr->PlayerGetBoundInstances(
            leader->GetGUIDLow(), Difficulty(DUNGEON_DIFFICULTY_NORMAL));
  ...
  case INSTANCE_RESET_CHANGE_DIFFICULTY: {
    ...
    BoundInstancesMap const &m_boundInstances =
        sInstanceSaveMgr->PlayerGetBoundInstances(
            leader->GetGUIDLow(), leader->GetDifficulty(isRaid));
    for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
         itr != m_boundInstances.end(); ++itr) {
      InstanceSave *instanceSave = itr->second.save;
      const MapEntry *entry = sMapStore.LookupEntry(itr->first);
...
void Group::_cancelHomebindIfInstance(Player *player) {
  ...
  InstancePlayerBind *bind = sInstanceSaveMgr->PlayerGetBoundInstance(
      player->GetGUIDLow(), player->FindMap()->GetId(),
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "-- Forced group value update for '%s'",
                pp->GetName().c_str());
#endif
    }
  }
...
void Group::SetLooterGuid(uint64 guid) { m_looterGuid = guid; }

void Group::SetMasterLooterGuid(uint64 guid) { m_masterLooterGuid = guid; }
...
bool Group::isLFGGroup() const { return m_groupType & GROUPTYPE_LFG; }
...
uint64 Group::GetLeaderGUID() const { return m_leaderGuid; }

uint64 Group::GetGUID() const { return m_guid; }

uint32 Group::GetLowGUID() const { return GUID_LOPART(m_guid); }

const char *Group::GetLeaderName() const { return m_leaderName.c_str(); }

LootMethod Group::GetLootMethod() const { return m_lootMethod; }

uint64 Group::GetLooterGuid() const { return m_looterGuid; }

uint64 Group::GetMasterLooterGuid() const { return m_masterLooterGuid; }
...
bool Group::IsLeader(uint64 guid) const { return (GetLeaderGUID() == guid); }

uint64 Group::GetMemberGUID(const std::string &name) {
  for (member_citerator itr = m_memberSlots.begin(); itr != m_memberSlots.end();
       ++itr)
    if (itr->name == name)
      return itr->guid;
  return 0;
...
void Group::SetGroupMemberFlag(uint64 guid, bool apply, GroupMemberFlags flag) {
  ...
  case MEMBER_FLAG_MAINASSIST:
    RemoveUniqueGroupMemberFlag(
        MEMBER_FLAG_MAINASSIST); // Remove main assist flag from current if any.
    break;
...
Group::member_witerator Group::_getMemberWSlot(uint64 Guid) {
  for (member_witerator itr = m_memberSlots.begin(); itr != m_memberSlots.end();
       ++itr)
    if (itr->guid == Guid)
      return itr;
  return m_memberSlots.end();
