...
void AuraEffect::HandleAuraMounted(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (!ci)
        {
            LOG_ERROR("sql.sql", "AuraMounted: `creature_template`='{}' not found in database (only need its modelid)", GetMiscValue());
            return;
        }
...
void AuraEffect::HandleAuraModStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        LOG_ERROR("spells.aura.effect", "WARNING: Spell {} effect {} has an unsupported misc value (%i) for SPELL_AURA_MOD_STAT ", GetId(), GetEffIndex(), GetMiscValue());
        return;
    }
