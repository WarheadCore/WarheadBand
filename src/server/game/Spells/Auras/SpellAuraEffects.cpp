...
// will Speedup trinity change amount/send for client operations because for
...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS] = {
    ...
    &AuraEffect::HandleNULL, // 48 SPELL_AURA_48 spell Napalm (area damage spell
                             // with additional delayed damage effect)
    ...
    &AuraEffect::HandleModStateImmunityMask, // 147
    ...
    &AuraEffect::HandleAuraModIncreaseHealth, // 230
    ...
    &AuraEffect::HandleAuraModIncreaseHealth, // 250
                                              // SPELL_AURA_MOD_INCREASE_HEALTH_2
    ...
    &AuraEffect::HandleModCombatSpeedPct, // 252 SPELL_AURA_252 Is there any
                                          ...
                                          // stacking mod?
    &AuraEffect::HandleNoImmediateEffect, // 253
    ...
    &AuraEffect::HandleNoImmediateEffect, // 259 SPELL_AURA_MOD_HOT_PCT
                                          // implemented in
                                          // Unit::SpellHealingBonus
    &AuraEffect::HandleNoImmediateEffect, // 260 SPELL_AURA_SCREEN_EFFECT
                                          // (miscvalue = id in ScreenEffect.dbc)
                                          // not required any code
    ...
    &AuraEffect::HandleAuraAllowOnlyAbility, // 263
                                             ...
                                             // player can use only abilities set
                                             // in SpellClassMask
    ...
    &AuraEffect::HandleNoImmediateEffect, // 275
                                          // SPELL_AURA_MOD_IGNORE_SHAPESHIFT Use
                                          // SpellClassMask for spell select
...
AuraEffect::AuraEffect(Aura *base, uint8 effIndex, int32 *baseAmount,
    ...
    : m_base(base), m_spellInfo(base->GetSpellInfo()),
      ...
      m_critChance(0), m_oldAmount(0), m_isAuraEnabled(true),
      m_channelData(nullptr), m_spellmod(nullptr), m_periodicTimer(0),
      m_tickNumber(0), m_effIndex(effIndex), m_canBeRecalculated(true),
      m_isPeriodic(false) {
...
int32 AuraEffect::CalculateAmount(Unit *caster) {
                ...
                if (pEnchant) {
                  for (int t = 0; t < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; t++)
                    if (pEnchant->spellid[t] == m_spellInfo->Id) {
                      amount = uint32((item_rand_suffix->prefix[k] *
    ...
    if (!m_spellInfo->ProcFlags ||
        m_spellInfo->HasAura(
            SPELL_AURA_PROC_TRIGGER_SPELL)) // xinef: skip auras with
                                            // proctriggerspell, they must have
                                            // procflags...
      break;
  ...
  case SPELL_AURA_MOD_BASE_RESISTANCE_PCT:
    ...
    if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_DRUID) {
      if (GetId() == 1178 && caster->HasAura(5229) &&
          !caster->HasAura(70726)) // Feral t10 4p
        amount -= 48;              // percentage
      else if (GetId() == 9635 && caster->HasAura(5229) &&
               !caster->HasAura(70726)) // Feral t10 4p
  ...
  if (caster)
    ...
    case SPELL_AURA_PERIODIC_LEECH:
      // xinef: save caster depending auras, always pass 1 as stack amount,
      // effect will be multiplicated at the end of the function by correct
      // value!
      if (GetBase()->GetType() == UNIT_AURA_TYPE)
        amount = caster->SpellDamageBonusDone(GetBase()->GetUnitOwner(),
                                              GetSpellInfo(), amount, DOT,
                                              GetPctMods(), 1);
      break;
    case SPELL_AURA_PERIODIC_HEAL:
      if (GetBase()->GetType() == UNIT_AURA_TYPE)
        amount = caster->SpellHealingBonusDone(GetBase()->GetUnitOwner(),
                                               GetSpellInfo(), amount, DOT,
                                               GetPctMods(), 1);
      break;
...
void AuraEffect::CalculatePeriodicData() {
  ...
  if (GetBase()->GetType() == UNIT_AURA_TYPE && GetCaster()) {
    if (m_spellInfo->HasAura(SPELL_AURA_PERIODIC_HEAL))
      ...
      m_pctMods = GetCaster()->SpellPctDamageModsDone(GetBase()->GetUnitOwner(),
                                                      GetSpellInfo(), DOT);
  }

  if (GetCaster())
    SetCritChance(CalcPeriodicCritChance(GetCaster(),
                                         (GetBase()->GetType() == UNIT_AURA_TYPE
...
void AuraEffect::CalculatePeriodic(Unit *caster, bool create, bool load) {
  ...
  GetBase()->CallScriptEffectCalcPeriodicHandlers(this, m_isPeriodic,
                                                  m_amplitude);
  ...
  if (m_amplitude) {
    ...
    if (caster) {
      if (caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE,
        ...
        m_amplitude =
            int32(m_amplitude * caster->GetFloatValue(UNIT_MOD_CAST_SPEED));
    }
  }
  ...
  } else // aura just created or reapplied
  {
    ...
    // possibly we should not reset periodic timers only when aura is triggered
    // by proc or maybe there's a spell attribute somewhere
...
void AuraEffect::CalculateSpellMod() {
  ...
  case SPELL_AURA_ADD_PCT_MODIFIER:
    if (!m_spellmod) {
      ...
      m_spellmod->type =
          SpellModType(GetAuraType()); // SpellModType value == spell aura types
...
void AuraEffect::HandleEffect(AuraApplication *aurApp, uint8 mode, bool apply) {
  // check if call is correct, we really don't want using bitmasks here (with 1
  // exception)
  ASSERT(mode == AURA_EFFECT_HANDLE_REAL ||
         mode == AURA_EFFECT_HANDLE_SEND_FOR_CLIENT ||
         mode == AURA_EFFECT_HANDLE_CHANGE_AMOUNT ||
         mode == AURA_EFFECT_HANDLE_STAT || mode == AURA_EFFECT_HANDLE_SKILL ||
  ...
  // registration/unregistration is done always before real effect handling
  // (some effect handlers code is depending on this)
  if (mode & AURA_EFFECT_HANDLE_REAL)
    aurApp->GetTarget()->_RegisterAuraEffect(this, apply);

  // xinef: stacking system, force return if effect is disabled, prevents double
  // apply / unapply xinef: placed here so above line can unregister effect from
  // the list xinef: this condition works under assumption that effect handlers
  // performs ALL necessery action with CHANGE_AMOUNT mode xinef: as far as i've
  // checked, all grouped spells meet this condition
  ...
  if (apply)
    prevented = GetBase()->CallScriptEffectApplyHandlers(
        this, const_cast<AuraApplication const *>(aurApp),
  ...
  if (apply)
    GetBase()->CallScriptAfterEffectApplyHandlers(this, aurApp,
                                                  (AuraEffectHandleModes)mode);
  else
    GetBase()->CallScriptAfterEffectRemoveHandlers(this, aurApp,
                                                   (AuraEffectHandleModes)mode);
}
...
void AuraEffect::ApplySpellMod(Unit *target, bool apply) {
  ...
  case SPELLMOD_EFFECT3: {
    ...
    for (Unit::AuraApplicationMap::iterator iter = auras.begin();
      ...
      // only passive and permament auras-active auras should have amount set on
      // spellcast and not be affected if aura is casted by others, it will not
      // be affected
    ...
    for (Unit::AuraApplicationMap::iterator iter = petauras.begin();
         iter != petauras.end(); ++iter) {
      Aura *aura = iter->second->GetBase();
      // only passive auras-active auras should have amount set on spellcast and
      // not be affected if aura is casted by others, it will not be affected
      if ((aura->IsPassive() || aura->IsPermanent()) &&
...
void AuraEffect::Update(uint32 diff, Unit *caster) {
  if (m_isPeriodic && (GetBase()->GetDuration() >= 0 ||
    ...
    while (m_periodicTimer <= 0) {
      ...
      // update before tick (aura can be removed in TriggerSpell or PeriodicTick
      // calls)
      m_periodicTimer += m_amplitude;
      ...
      case 61830:
        ...
        if (AuraEffect *aurEff = GetBase()->GetEffect(0)) {
          if (aurEff->GetAuraType() != SPELL_AURA_MOD_POWER_REGEN) {
            m_isPeriodic = false;
            LOG_ERROR("server",
                      "Aura %d structure has been changed - first aura is no "
                      "longer SPELL_AURA_MOD_POWER_REGEN",
                      GetId());
          } else {
...
float AuraEffect::CalcPeriodicCritChance(Unit const *caster,
      ...
      for (Unit::AuraEffectList::const_iterator itr =
        ...
        if ((*itr)->IsAffectedOnSpell(GetSpellInfo())) {
          critChance = modOwner->SpellDoneCritChance(
              nullptr, GetSpellInfo(), GetSpellInfo()->GetSchoolMask(),
  ...
  if (target && critChance > 0.0f)
    critChance = target->SpellTakenCritChance(caster, GetSpellInfo(),
                                              GetSpellInfo()->GetSchoolMask(),
                                              critChance, BASE_ATTACK, true);
...
void AuraEffect::CleanupTriggeredSpells(Unit *target) {
  ...
  // TODO: is there a spell flag, which can solve this in a more sophisticated
  // way?
  ...
  if (apply) {
    ...
    if (player) {
      ...
      for (PlayerSpellMap::const_iterator itr = sp_list.begin();
        ...
        if (!spellInfo ||
            !spellInfo->HasAttribute(SpellAttr0(SPELL_ATTR0_PASSIVE |
      ...
      for (PlayerTalentMap::const_iterator itr = tl_list.begin();
        ...
        if (!spellInfo ||
            !spellInfo->HasAttribute(SpellAttr0(
                SPELL_ATTR0_PASSIVE | SPELL_ATTR0_HIDDEN_CLIENTSIDE)) ||
      ...
      for (uint32 i = 0; i < MAX_GLYPH_SLOT_INDEX; ++i) {
        if (uint32 glyphId = player->GetGlyph(i)) {
          if (GlyphPropertiesEntry const *glyph =
            ...
            if (spellInfo->Stances & (1 << (GetMiscValue() - 1)))
              target->CastSpell(
                  target, glyph->SpellId,
                  TriggerCastFlags(TRIGGERED_FULL_MASK &
                                   ~(TRIGGERED_IGNORE_SHAPESHIFT |
                                     TRIGGERED_IGNORE_CASTER_AURASTATE)),
                  nullptr, this, target->GetGUID());
          }
        }
      }
      ...
      if (player->HasTalent(63410, player->GetActiveSpec()) ||
        ...
        if (GetMiscValue() == FORM_TRAVEL ||
            GetMiscValue() ==
                FORM_NONE) // "while in Travel Form or while not shapeshifted"
      ...
      if (HotWSpellId) {
        ...
        for (Unit::AuraEffectList::const_iterator i = mModTotalStatPct.begin();
             i != mModTotalStatPct.end(); ++i) {
          // Heart of the Wild
          if ((*i)->GetSpellInfo()->SpellIconID == 240 &&
              (*i)->GetMiscValue() == STAT_INTELLECT) {
            int32 HotWMod =
                (*i)->GetAmount() / 2; // For each 2% Intelligence, you get 1%
                                       // stamina and 1% attack power.

            target->CastCustomSpell(target, HotWSpellId, &HotWMod, nullptr,
                                    nullptr, true, nullptr, this,
                                    target->GetGUID());
            break;
          }
        }
      ...
      case FORM_CAT:
        // Savage Roar
        if (target->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, 0,
                                  0x10000000, 0))
          target->CastSpell(target, 62071, true);
        // Nurturing Instinct
        if (AuraEffect const *aurEff = target->GetAuraEffect(
                SPELL_AURA_MOD_SPELL_HEALING_OF_STAT_PERCENT, SPELLFAMILY_DRUID,
                2254, 0)) {
          uint32 spellId3 = 0;
          switch (aurEff->GetId()) {
          case 33872:
        ...
        if (AuraEffect const *aurEff =
                target->GetDummyAuraEffect(SPELLFAMILY_GENERIC, 2851, 0)) {
      ...
      case FORM_BEAR:
        ...
        if (AuraEffect const *aurEff =
                target->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE,
                                      SPELLFAMILY_DRUID, 961, 0)) {
          int32 bp = aurEff->GetSpellInfo()->Effects[EFFECT_2].CalcValue();
          target->CastCustomSpell(target, 62069, &bp, nullptr, nullptr, true, 0,
                                  this, target->GetGUID());
        }
        break;
      case FORM_MOONKIN:
    ...
    // Iterate through all the shapeshift auras that the target has, if there is
    // another aura with SPELL_AURA_MOD_SHAPESHIFT, then this aura is being
    // removed due to that one being applied
    for (Unit::AuraEffectList::const_iterator itr = shapeshifts.begin();
         itr != shapeshifts.end(); ++itr) {
      if ((*itr) != this) {
        newAura = *itr;
        break;
      }
    ...
    for (Unit::AuraApplicationMap::iterator itr = tAuras.begin();
      ...
      if (itr->second->GetBase()->IsRemovedOnShapeLost(target) &&
          (!(itr->second->GetBase()->GetSpellInfo()->Stances & newStance) ||
...
void AuraEffect::HandleModInvisibilityDetect(AuraApplication const *aurApp,
  ...
  target->UpdateObjectVisibility(target->GetTypeId() == TYPEID_PLAYER ||
                                 IS_PLAYER_GUID(target->GetOwnerGUID()));
}
...
void AuraEffect::HandleDetectAmore(AuraApplication const *aurApp, uint8 mode,
                                   bool apply) const {
...
void AuraEffect::HandleAuraGhost(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    ...
    target->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GHOST,
                                                  GHOST_VISIBILITY_GHOST);
  } else {
...
void AuraEffect::HandlePhase(AuraApplication const *aurApp, uint8 mode,
    ...
    if (!newPhase) {
      ...
      if (Creature *creature = target->ToCreature())
        if (CreatureData const *data =
                sObjectMgr->GetCreatureData(creature->GetDBTableGUIDLow()))
  ...
  // need triggering visibility update base at phase update of not GM invisible
  // (other GMs anyway see in any phases)
  if (target->IsVisible()) {
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const *aurApp,
  ...
  if (apply) {
    ...
    if (PowerType != POWER_MANA) {
      ...
      case FORM_DIREBEAR: {
        ...
        case FORM_CAT: {
          ...
          target->CastCustomSpell(target, 17099, &basePoints, nullptr, nullptr,
                                  true, nullptr, this);
          break;
        }
        case FORM_BEAR:
    ...
    // xinef: allow shapeshift to override model id if forced transform aura is
    // not present!
    if (modelid > 0) {
      bool allow = true;
    ...
    case FORM_BERSERKERSTANCE: {
      ...
      // Stance mastery + Tactical mastery (both passive, and last have aura
      // only in defense stance, but need apply at any stance switch)
      if (target->GetTypeId() == TYPEID_PLAYER) {
    ...
    if (!target->CanUseAttackType(BASE_ATTACK)) {
      if (Item *pItem = target->ToPlayer()->GetItemByPos(
        ...
        target->ToPlayer()->_ApplyWeaponDamage(
            EQUIPMENT_SLOT_MAINHAND, pItem->GetTemplate(), nullptr, apply);
      }
    }
  }
  ...
  if (target->GetTypeId() == TYPEID_PLAYER) {
    ...
    for (uint8 i = 0; i < MAX_SHAPESHIFT_SPELLS; ++i) {
      if (!shapeInfo->stanceSpell[i])
        continue;
      if (apply)
        target->ToPlayer()->_addSpell(shapeInfo->stanceSpell[i], SPEC_MASK_ALL,
                                      true);
      else
        target->ToPlayer()->removeSpell(shapeInfo->stanceSpell[i],
                                        SPEC_MASK_ALL, true);
    }
  }
}
...
void AuraEffect::HandleAuraTransform(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    // update active transform spell only when transform or shapeshift not set
    // or not overwriting negative by positive case
    if (GetSpellInfo()->HasAttribute(
            SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) ||
        !target->GetModelForForm(target->GetShapeshiftForm()) ||
      ...
      if (GetMiscValue() == 0) {
        ...
        case 16739: {
          ...
          // Blood Elf
          case RACE_BLOODELF:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 17829
                                                                    : 17830);
            break;
          // Orc
          case RACE_ORC:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10139
                                                                    : 10140);
            break;
          // Troll
          case RACE_TROLL:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10135
                                                                    : 10134);
            break;
          // Tauren
          case RACE_TAUREN:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10136
                                                                    : 10147);
            break;
          // Undead
          case RACE_UNDEAD_PLAYER:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10146
                                                                    : 10145);
            break;
          // Draenei
          case RACE_DRAENEI:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 17827
                                                                    : 17828);
            break;
          // Dwarf
          case RACE_DWARF:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10141
                                                                    : 10142);
            break;
          // Gnome
          case RACE_GNOME:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10148
                                                                    : 10149);
            break;
          // Human
          case RACE_HUMAN:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10137
                                                                    : 10138);
            break;
          // Night Elf
          case RACE_NIGHTELF:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 10143
                                                                    : 10144);
            break;
          default:
            break;
        ...
        case 51926: {
          ...
          // Blood Elf
          case RACE_BLOODELF:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25032
                                                                    : 25043);
            break;
          // Orc
          case RACE_ORC:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25039
                                                                    : 25050);
            break;
          // Troll
          case RACE_TROLL:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25041
                                                                    : 25052);
            break;
          // Tauren
          case RACE_TAUREN:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25040
                                                                    : 25051);
            break;
          // Undead
          case RACE_UNDEAD_PLAYER:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25042
                                                                    : 25053);
            break;
          // Draenei
          case RACE_DRAENEI:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25033
                                                                    : 25044);
            break;
          // Dwarf
          case RACE_DWARF:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25034
                                                                    : 25045);
            break;
          // Gnome
          case RACE_GNOME:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25035
                                                                    : 25046);
            break;
          // Human
          case RACE_HUMAN:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25037
                                                                    : 25048);
            break;
          // Night Elf
          case RACE_NIGHTELF:
            target->SetDisplayId(target->getGender() == GENDER_MALE ? 25038
                                                                    : 25049);
            break;
          default:
            break;
        ...
        case 65386:
        case 65495:
          target->SetDisplayId(target->getGender() == GENDER_MALE ? 29203
                                                                  : 29204);
          break;
        // Darkspear Pride
        case 75532:
          target->SetDisplayId(target->getGender() == GENDER_MALE ? 31737
                                                                  : 31738);
          break;
        // Gnomeregan Pride
        case 75531:
          target->SetDisplayId(target->getGender() == GENDER_MALE ? 31654
                                                                  : 31655);
          break;
        default:
          break;
        ...
        if (!ci) {
          ...
          LOG_ERROR("server",
                    ...
                    "From Spell Aura Transform in Spell ID = %d",
                    GetMiscValue(), GetId());
        } else {
          uint32 model_id = 0;
    ...
    // HandleEffect(this, AURA_EFFECT_HANDLE_SEND_FOR_CLIENT, true) will reapply
    // it if need
...
void AuraEffect::HandleAuraCloneCaster(AuraApplication const *aurApp,
  ...
  if (apply) {
    ...
    // we need retail info about how scaling is handled (aura maybe?)
...
void AuraEffect::HandleFeignDeath(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    ...
    */
    ...
    UnitList targets;
    Warhead::AnyUnfriendlyUnitInObjectRangeCheck u_check(
        target, target,
        target->GetVisibilityRange()); // no VISIBILITY_COMPENSATION, distance
                                       // is enough
    Warhead::UnitListSearcher<Warhead::AnyUnfriendlyUnitInObjectRangeCheck>
        searcher(target, targets, u_check);
    target->VisitNearbyObject(
        target->GetVisibilityRange(),
        searcher); // no VISIBILITY_COMPENSATION, distance is enough
    for (UnitList::iterator iter = targets.begin(); iter != targets.end();
      ...
      for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL;
        ...
        if ((*iter)->GetCurrentSpell(i) &&
          ...
          if (si->HasAttribute(SPELL_ATTR6_CAN_TARGET_INVISIBLE) &&
          ...
          bool interrupt =
              false; // pussywizard: skip spells that don't target units, but
                     // casted on unit (eg. TARGET_DEST_TARGET_ENEMY)
    ...
    if (target->GetInstanceScript() &&
      ...
      target->ToPlayer()->SendAttackSwingCancelAttack(); // melee and ranged
                                                         // forced attack cancel
    } else {
      target->CombatStop();
...
void AuraEffect::HandleModUnattackable(AuraApplication const *aurApp,
  ...
  // do not remove unit flag if there are more than this auraEffect of that kind
  // on unit on unit
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const *aurApp, uint8 mode,
  ...
  if (target->GetTypeId() == TYPEID_PLAYER &&
    ...
    if (Item *pItem =
      ...
      if (attacktype < MAX_ATTACK) {
        target->ToPlayer()->_ApplyWeaponDamage(slot, pItem->GetTemplate(),
                                               nullptr, !apply);
  ...
  // if disarm effects should be applied, wait to set flag until damage mods are
  // unapplied
  if (apply)
...
void AuraEffect::HandleAuraModSilence(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    ...
    // call functions which may have additional effects after chainging state of
    // unit Stop cast only spells vs PreventionType ==
...
void AuraEffect::HandleAuraModPacify(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    ...
    // target->AttackStop(); // pussywizard: wtf? why having this flag prevents
...
void AuraEffect::HandleAuraTrackCreatures(AuraApplication const *aurApp,
  ...
  if (apply)
    ...
    target->RemoveFlag(PLAYER_TRACK_CREATURES, uint32(1)
                                                   << (GetMiscValue() - 1));
}
...
void AuraEffect::HandleAuraTrackResources(AuraApplication const *aurApp,
  ...
  if (apply)
    ...
    target->RemoveFlag(PLAYER_TRACK_RESOURCES, uint32(1)
                                                   << (GetMiscValue() - 1));
}
...
void AuraEffect::HandleAuraUntrackable(AuraApplication const *aurApp,
  ...
  if (apply)
    target->SetByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_VIS_FLAG,
    ...
    if (target->HasAuraType(GetAuraType()))
      return;
    target->RemoveByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_VIS_FLAG,
                           UNIT_STAND_FLAGS_UNTRACKABLE);
  }
}
...
void AuraEffect::HandleAuraModSkill(AuraApplication const *aurApp, uint8 mode,
                                    bool apply) const {
  if (!(mode &
...
void AuraEffect::HandleAuraMounted(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    ...
    // spell
    // need to remove ALL arura related to mounts, this will stop client crash
    // with broom stick and never endless flying after using Headless Horseman's
    // Mount
    if (mode & AURA_EFFECT_HANDLE_REAL)
      target->RemoveAurasByType(SPELL_AURA_MOUNTED);
  }
...
void AuraEffect::HandleAuraFeatherFall(AuraApplication const *aurApp,
  ...
  if (!apply && target->GetTypeId() == TYPEID_PLAYER)
    target->ToPlayer()->SetFallInformation(time(nullptr),
                                           target->GetPositionZ());
}
...
void AuraEffect::HandleModThreat(AuraApplication const *aurApp, uint8 mode,
  ...
  for (int8 i = 0; i < MAX_SPELL_SCHOOL; ++i)
    if (GetMiscValue() & (1 << i))
      ApplyPercentModFloatVar(target->m_threatModifier[i], float(GetAmount()),
                              apply);
}
...
void AuraEffect::HandleModTaunt(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply)
    ...
    // When taunt aura fades out, mob will switch to previous target if current
    // has less than 1.1 * secondthreat
...
void AuraEffect::HandleModPossessPet(AuraApplication const *aurApp, uint8 mode,
  ...
  if (apply) {
    ...
    if (!pet->IsWithinDistInMap(caster, pet->GetMap()->GetVisibilityRange()))
      ...
      // Follow owner only if not fighting or owner didn't click "stay" at new
      // location This may be confusing because pet bar shows "stay" when under
      // the spell but it retains
...
void AuraEffect::HandleAuraControlVehicle(AuraApplication const *aurApp,
  ...
  if (apply) {
    ...
    // VEHICLE_SEAT_FLAG_CAN_ENTER_OR_EXIT and -1 checks for it, so this break
    // such spells or most of them. Current formula about m_amount: effect base
    // points + dieside - 1 TO DO: Reasearch more about 0/0 and fix it.
    ...
    caster->_ExitVehicle();
    // some SPELL_AURA_CONTROL_VEHICLE auras have a dummy effect on the player -
    // remove them
...
void AuraEffect::HandleAuraModIncreaseFlightSpeed(AuraApplication const *aurApp,
  ...
  if (GetAuraType() == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED) {
    ...
    //! Someone should clean up these hacks and remove it from this function. It
...
void AuraEffect::HandleModStateImmunityMask(AuraApplication const *aurApp,
  ...
  case 1615: {
    if (!GetAmount()) {
      mechanic_immunity_list =
          (1 << MECHANIC_SNARE) | (1 << MECHANIC_ROOT) | (1 << MECHANIC_FEAR) |
      ...
      target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_POLYMORPH,
                               apply);
      target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_DISORIENTED,
                               apply);
  ...
  case 1614:
  case 1694: {
    target->ApplySpellImmune(GetId(), IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME,
                             apply);
  ...
  case 1632: {
    if (!GetAmount()) {
      ...
      target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_TURN,
                               apply);
      target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_BANISH,
                               apply);
      target->ApplySpellImmune(GetId(), IMMUNITY_EFFECT,
                               SPELL_EFFECT_KNOCK_BACK, apply);
      target->ApplySpellImmune(GetId(), IMMUNITY_EFFECT,
                               SPELL_EFFECT_KNOCK_BACK_DEST, apply);
      aura_immunity_list.push_back(SPELL_AURA_MOD_STUN);
  ...
  if (aura_immunity_list.empty()) {
    ...
    if (GetMiscValue() & (1 << 3)) {
      ...
      target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_INTERRUPT,
                               apply);
    }
    // Transform?
    if (GetMiscValue() & (1 << 4)) {
    ...
    if (GetMiscValue() & (1 << 5)) {
      ...
      target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_KNOCKOUT,
                               apply);
    }
    // // Slowing effects
    if (GetMiscValue() & (1 << 6)) {
  ...
  for (std::list<AuraType>::iterator iter = aura_immunity_list.begin();
       iter != aura_immunity_list.end(); ++iter)
    target->ApplySpellImmune(GetId(), IMMUNITY_STATE, *iter, apply);

  // Patch 3.0.3 Bladestorm now breaks all snares and roots on the warrior when
  // activated.
  if (GetId() == 46924) {
    if (apply) {
    ...
    // Knockback and hex
    target->ApplySpellImmune(GetId(), IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK,
                             apply);
  }

  if (apply &&
      GetSpellInfo()->HasAttribute(SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY)) {
    target->RemoveAurasWithMechanic(mechanic_immunity_list,
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const *aurApp,
  ...
  case 19574: // Bestial Wrath
    ...
    target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_BANISH,
                             apply);
    target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SHACKLE,
                             apply);
  ...
  case 46227: // Medalion of Immunity
    ...
    target->RemoveAurasByType(
        SPELL_AURA_PREVENTS_FLEEING); // xinef: Patch 2.3.0 PvP Trinkets:
                                      // Insignia of the Alliance, Insignia of
                                      // the Horde, Medallion of the Alliance,
                                      // and Medallion of the Horde now clear
                                      // the debuff from Judgement of Justice.
    // Actually we should apply immunities here, too, but the aura has only 100
    // ms duration, so there is practically no point
    break;
...
void AuraEffect::HandleAuraModSchoolImmunity(AuraApplication const *aurApp,
  ...
  // remove all flag auras (they are positive, but they must be removed when you
  // are immune)
...
void AuraEffect::HandleAuraModDispelImmunity(AuraApplication const *aurApp,
  ...
  target->ApplySpellDispelImmunity(m_spellInfo, DispelType(GetMiscValue()),
                                   (apply));
}
  ...
  for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++) {
    if (GetMiscValue() & int32(1 << x)) {
      ...
      if (target->GetTypeId() == TYPEID_PLAYER || target->IsPet())
        target->ApplyResistanceBuffModsMod(SpellSchools(x), GetAmount() > 0,
...
void AuraEffect::HandleAuraModBaseResistancePCT(AuraApplication const *aurApp,
  ...
  if (target->GetTypeId() != TYPEID_PLAYER) {
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++) {
      if (GetMiscValue() & int32(1 << x))
        target->HandleStatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + x),
                                   BASE_PCT, float(GetAmount()), apply);
    }
  }
}
    ...
    if (GetMiscValue() & int32(1 << i)) {
      ...
      if (target->GetTypeId() == TYPEID_PLAYER || target->IsPet()) {
        target->ApplyResistanceBuffModsPercentMod(SpellSchools(i), true,
                                                  (float)GetAmount(), apply);
        target->ApplyResistanceBuffModsPercentMod(SpellSchools(i), false,
                                                  (float)GetAmount(), apply);
      }
    }
  }
...
void AuraEffect::HandleModTargetResistance(AuraApplication const *aurApp,
  ...
  // applied to damage as HandleNoImmediateEffect in Unit::CalcAbsorbResist and
  ...
  if (target->GetTypeId() == TYPEID_PLAYER &&
    ...
    target->ApplyModInt32Value(PLAYER_FIELD_MOD_TARGET_RESISTANCE, GetAmount(),
                               apply);
}
...
void AuraEffect::HandleAuraModStat(AuraApplication const *aurApp, uint8 mode,
  ...
  if (GetMiscValue() < -2 || GetMiscValue() > 4) {
    LOG_ERROR("server",
              "WARNING: Spell %u effect %u has an unsupported misc value (%i) "
  ...
  for (int32 i = STAT_STRENGTH; i < MAX_STATS; i++) {
    ...
    if (GetMiscValue() < 0 || GetMiscValue() == i) {
      ...
      target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_VALUE,
                                 float(GetAmount()), apply);
      if (target->GetTypeId() == TYPEID_PLAYER || target->IsPet())
...
void AuraEffect::HandleModPercentStat(AuraApplication const *aurApp, uint8 mode,
  ...
  for (int32 i = STAT_STRENGTH; i < MAX_STATS; ++i) {
    if (GetMiscValue() == i || GetMiscValue() == -1)
      target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), BASE_PCT,
                                 float(m_amount), apply);
  }
}
...
void AuraEffect::HandleModTotalPercentStat(AuraApplication const *aurApp,
  ...
  if (GetId() == 67480) // xinef: hack fix for blessing of sanctuary stats stack
                        // with blessing of kings...
  {
    ...
    for (int32 i = STAT_STRENGTH; i < MAX_STATS; i++) {
      if (i == STAT_STRENGTH || i == STAT_STAMINA) {
        ...
        target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_PCT,
                                   value, apply);
  ...
  // spells with SPELL_ATTR0_UNK4 0x00000010 flag) this check is total bullshit
  // i think
  if (GetMiscValue() == STAT_STAMINA &&
      m_spellInfo->HasAttribute(SPELL_ATTR0_ABILITY))
    target->SetHealth(std::max<uint32>(
        uint32(healthPct * target->GetMaxHealth() * 0.01f), (alive ? 1 : 0)));
}
...
void AuraEffect::HandleAuraModResistenceOfStatPercent(
  ...
  if (GetMiscValue() != SPELL_SCHOOL_MASK_NORMAL) {
    // support required adding replace UpdateArmor by loop by UpdateResistence
    // at intellect update and include in UpdateResistence same code as in
    // UpdateArmor for aura mod apply.
    LOG_ERROR("server", "Aura SPELL_AURA_MOD_RESISTANCE_OF_STAT_PERCENT(182) "
...
void AuraEffect::HandleModPowerRegen(AuraApplication const *aurApp, uint8 mode,
  ...
  // other powers are not immediate effects - implemented in Player::Regenerate,
  // Creature::Regenerate
}
...
void AuraEffect::HandleAuraModIncreaseHealthPercent(
  ...
  // Xinef: idiots, pct was rounded down and could "kill" creature by setting
  // its health to 0 making npc zombie
  if (target->IsAlive())
...
void AuraEffect::HandleAuraModWeaponCritPercent(AuraApplication const *aurApp,
  ...
  for (int i = 0; i < MAX_ATTACK; ++i)
    if (Item *pItem =
            target->ToPlayer()->GetWeaponForAttack(WeaponAttackType(i), true))
  ...
  // EquippedItemInventoryTypeMask GetMiscValue() comparison with item generated
  // damage types
...
void AuraEffect::HandleModRating(AuraApplication const *aurApp, uint8 mode,
  ...
  for (uint32 rating = 0; rating < MAX_COMBAT_RATING; ++rating)
    if (GetMiscValue() & (1 << rating))
      target->ToPlayer()->ApplyRatingMod(CombatRating(rating), GetAmount(),
                                         apply);
}
...
void AuraEffect::HandleAuraModAttackPower(AuraApplication const *aurApp,
  ...
  target->HandleStatModifier(UNIT_MOD_ATTACK_POWER, TOTAL_VALUE,
                             float(GetAmount()), apply);
}
...
void AuraEffect::HandleAuraModRangedAttackPower(AuraApplication const *aurApp,
  ...
  target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED, TOTAL_VALUE,
                             float(GetAmount()), apply);
}
...
void AuraEffect::HandleAuraModAttackPowerPercent(AuraApplication const *aurApp,
  ...
  target->HandleStatModifier(UNIT_MOD_ATTACK_POWER, TOTAL_PCT,
                             float(GetAmount()), apply);
}
...
void AuraEffect::HandleAuraModRangedAttackPowerPercent(
  ...
  target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED, TOTAL_PCT,
                             float(GetAmount()), apply);
}
...
void AuraEffect::HandleModDamageDone(AuraApplication const *aurApp, uint8 mode,
  ...
  if ((GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) != 0) {
    ...
    if (GetSpellInfo()->EquippedItemClass == -1 ||
      ...
      if (target->GetTypeId() == TYPEID_PLAYER) {
        if (GetAmount() > 0)
          target->ApplyModInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS,
                                     GetAmount(), apply);
        else
          target->ApplyModInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG,
                                     GetAmount(), apply);
      }
    } else {
      // done in Player::_ApplyWeaponDependentAuraMods
    ...
    if (GetAmount() > 0) {
      for (uint32 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; i++) {
        if ((GetMiscValue() & (1 << i)) != 0)
          target->ApplyModInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i,
                                     GetAmount(), apply);
      }
    } else {
      for (uint32 i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; i++) {
        if ((GetMiscValue() & (1 << i)) != 0)
          target->ApplyModInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + i,
                                     GetAmount(), apply);
      }
    }
...
void AuraEffect::HandleModPowerCostPCT(AuraApplication const *aurApp,
  ...
  for (int i = 0; i < MAX_SPELL_SCHOOL; ++i)
    if (GetMiscValue() & (1 << i))
      target->ApplyModSignedFloatValue(UNIT_FIELD_POWER_COST_MULTIPLIER + i,
                                       amount, apply);
}
...
void AuraEffect::HandleAuraRetainComboPoints(AuraApplication const *aurApp,
  ...
  // remove only if aura expire by time (in case combo points amount change aura
  // removed without combo points lost)
  if (!(apply) && GetBase()->GetDuration() == 0 &&
      target->ToPlayer()->GetComboTarget())
    if (Unit *unit = ObjectAccessor::GetUnit(
            *target, target->ToPlayer()->GetComboTarget()))
  ...
  if (mode & (AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY)) {
    ...
    if (apply) {
      ...
      case 1515: // Tame beast
        // FIX_ME: this is 2.0.12 threat effect replaced in 2.1.x by dummy aura,
      ...
      case 37096: // Blood Elf Illusion
      {
        if (caster) {
          ...
          case GENDER_FEMALE:
            caster->CastSpell(target, 37095, true, nullptr,
    ...
    // AT REMOVE
      ...
      if ((GetSpellInfo()->IsQuestTame()) && caster && caster->IsAlive() &&
          target->IsAlive() &&
      ...
      case SPELLFAMILY_GENERIC:
        ...
        case 2584: // Waiting to Resurrect
          // Waiting to resurrect spell cancel, we must remove player from
          // resurrect queue
          if (target->GetTypeId() == TYPEID_PLAYER) {
            if (Battleground *bg = target->ToPlayer()->GetBattleground())
              bg->RemovePlayerFromResurrectQueue(target->ToPlayer());
            if (Battlefield *bf = sBattlefieldMgr->GetBattlefieldToZoneId(
                    target->GetZoneId()))
        ...
        case 52921: {
          ...
          Warhead::PlayerSearcher<Warhead::AnyPlayerInObjectRangeCheck>
    ...
    case 24658: {
      ...
      if (apply && caster) {
        ...
        for (uint32 i = 0; i < spell->StackAmount; ++i)
          caster->CastSpell(target, spell->Id, true, nullptr, nullptr,
                            GetCasterGUID());
        break;
      }
    ...
    case 62061: // Festive Holiday Mount
      if (target->HasAuraType(SPELL_AURA_MOUNTED)) {
        ...
        if (apply) {
          ...
          creatureEntry = target->GetAuraEffectsByType(SPELL_AURA_MOUNTED)
                              .front()
                              ->GetMiscValue();
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const *aurApp,
  ...
  if (GetSpellInfo()->Effects[m_effIndex].ItemType == 6265) {
    ...
    if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_WARLOCK &&
        (GetSpellInfo()->SpellFamilyFlags[0] & 0x00004000)) {
      // Glyph of Drain Soul - chance to create an additional Soul Shard
      if (AuraEffect *aur = caster->GetAuraEffect(58070, 0))
        if (roll_chance_i(aur->GetMiscValue()))
          caster->CastSpell(caster, 58068, true, 0,
                            aur); // We _could_ simply do ++count here, but
                                  // Blizz does it this way :)
    }
  }
  ...
  InventoryResult msg = plCaster->CanStoreNewItem(
      NULL_BAG, NULL_SLOT, dest, GetSpellInfo()->Effects[m_effIndex].ItemType,
      count, &noSpaceForCount);
  if (msg != EQUIP_ERR_OK) {
    count -= noSpaceForCount;
    plCaster->SendEquipError(msg, nullptr, nullptr,
...
void AuraEffect::HandleAuraEmpathy(AuraApplication const *aurApp, uint8 mode,
  ...
  if (target->GetCreatureType() == CREATURE_TYPE_BEAST)
    target->ApplyModUInt32Value(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_SPECIALINFO,
                                apply);
}
...
void AuraEffect::HandleAuraConvertRune(AuraApplication const *aurApp,
  ...
  // convert number of runes specified in aura amount of rune type in miscvalue
  // to runetype in miscvalueb
  if (apply) {
...
void AuraEffect::HandleAuraLinked(AuraApplication const *aurApp, uint8 mode,
  ...
  if (mode & AURA_EFFECT_HANDLE_REAL) {
    if (apply) {
      ...
      if (GetAmount())
        caster->CastCustomSpell(target, triggeredSpellId, &m_amount, nullptr,
                                nullptr, true, nullptr, this);
      else
        caster->CastSpell(target, triggeredSpellId, true, nullptr, this);
    } else {
      uint64 casterGUID = triggeredSpellInfo->NeedsToBeTriggeredByCaster(
                              m_spellInfo, GetEffIndex())
                              ? GetCasterGUID()
                              : target->GetGUID();
      target->RemoveAura(triggeredSpellId, casterGUID, 0,
...
void AuraEffect::HandleAuraSetVehicle(AuraApplication const *aurApp, uint8 mode,
                                      bool apply) const {
  if (!(mode & AURA_EFFECT_HANDLE_REAL))
    return;
...
void AuraEffect::HandlePreventResurrection(AuraApplication const *aurApp,
  ...
  if (apply)
    aurApp->GetTarget()->RemoveByteFlag(PLAYER_FIELD_BYTES, 0,
                                        PLAYER_FIELD_BYTE_RELEASE_TIMER);
  else if (!aurApp->GetTarget()->GetBaseMap()->Instanceable())
    aurApp->GetTarget()->SetByteFlag(PLAYER_FIELD_BYTES, 0,
                                     PLAYER_FIELD_BYTE_RELEASE_TIMER);
}
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit *target, Unit *caster) const {
  ...
  case SPELLFAMILY_DRUID: {
    ...
    case 22842: {
      // Converts up to 10 rage per second into health for $d.  Each point of
      // rage is converted into ${$m2/10}.1% of max health. Should be manauser
  ...
  case SPELLFAMILY_DEATHKNIGHT:
    ...
    case 49016: // Hysteria
      ...
      Unit::DealDamage(target, target, damage, nullptr, NODAMAGE,
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit *target,
  ...
  if (triggeredSpellInfo == nullptr) {
    ...
    case SPELLFAMILY_GENERIC: {
      ...
      case 29528:
        if (target->GetTypeId() !=
            TYPEID_UNIT) // prevent error reports in case ignored player target
          return;
        break;
      ...
      case 30576:
        ...
        break;
      // Doom
      // TODO: effect trigger spell may be independant on spell targets, and
      // executed in spell finish phase so instakill will be naturally done
      // before trigger spell
      case 31347: {
    ...
    case SPELLFAMILY_SHAMAN: {
      switch (auraId) {
      // Lightning Shield (The Earthshatterer set trigger after cast Lighting
      // Shield)
      case 28820: {
    ...
    // Mana Tide
    case 16191:
      target->CastCustomSpell(target, triggerSpellId, &m_amount, nullptr,
                              nullptr, true, nullptr, this);
      return;
    // Poison (Grobbulus)
    case 28158:
    ...
    case 66882:
      target->CastCustomSpell(
          triggerSpellId, SPELLVALUE_RADIUS_MOD,
    ...
    case 56432:
      if (triggerSpellId == 56438) {
        target->CastCustomSpell(
            triggerSpellId, SPELLVALUE_RADIUS_MOD,
            (int32)(10000 * (1.0f - 0.02f * (m_tickNumber + 1))), nullptr, true,
            nullptr, this);
        return;
      }
      break;
    ...
    case 53563: {
      ...
      GetBase()->GetUnitOwner()->CastSpell(
          target, triggeredSpellInfo, true, 0, this,
    ...
    case 69508: {
      if (caster)
        caster->CastSpell(target, triggerSpellId, true, nullptr, nullptr,
                          caster->GetGUID());
      return;
    }
    ...
    case 58882:
      ...
      target->CastCustomSpell(target, triggerSpellId, &amount, nullptr, nullptr,
                              true);
      return;
    }
  }
    ...
    if (Unit *triggerCaster = triggeredSpellInfo->NeedsToBeTriggeredByCaster(
      ...
      if (GetSpellInfo()->Effects[GetEffIndex()].TargetA.GetCheckType() ==
        ...
        triggerFlags = TriggerCastFlags(
            TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_POWER_AND_REAGENT_COST);

      triggerCaster->CastSpell(targets, triggeredSpellInfo, nullptr,
                               triggerFlags, nullptr, this);
...
void AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick(
  ...
  if (SpellInfo const *triggeredSpellInfo =
    ...
    if (Unit *triggerCaster = triggeredSpellInfo->NeedsToBeTriggeredByCaster(
      ...
      triggerCaster->CastSpell(targets, triggeredSpellInfo, &values,
                               TRIGGERED_FULL_MASK, nullptr, this);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    ...
    if (c && caster)
      sEluna->OnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()),
                            target->ToCreature());
#endif
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick: Spell "
              "%u has non-existent spell %u in EffectTriggered[%d] and is "
              "therefor not triggered.",
              GetId(), triggerSpellId, GetEffIndex());
#endif
  }
}
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit *target,
  ...
  if (target->HasUnitState(UNIT_STATE_ISOLATED) ||
      target->IsImmunedToDamageOrSchool(GetSpellInfo()) || target->IsTotem()) {
  ...
  if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE) {
    // xinef: leave only target depending bonuses, rest is handled in calculate
    // amount
    if (GetBase()->GetType() == DYNOBJ_AURA_TYPE && caster)
      damage = caster->SpellDamageBonusDone(target, GetSpellInfo(), damage, DOT,
                                            0.0f, GetBase()->GetStackAmount());
    damage = target->SpellDamageBonusTaken(caster, GetSpellInfo(), damage, DOT,
                                           GetBase()->GetStackAmount());

    // Calculate armor mitigation
    if (Unit::IsDamageReducedByArmor(GetSpellInfo()->GetSchoolMask(),
                                     GetSpellInfo(), GetEffIndex())) {
  ...
  } else // xinef: ceil obtained value, it may happen that 10 ticks for 10%
         // damage may not kill owner
    damage = uint32(
        ceil(CalculatePct<float, float>(target->GetMaxHealth(), damage)));
  ...
  Unit::CalcAbsorbResist(caster, target, GetSpellInfo()->GetSchoolMask(), DOT,
                         damage, &absorb, &resist, GetSpellInfo());

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server",
            "PeriodicTick: %u (TypeId: %u) attacked %u (TypeId: %u) for %u dmg "
            "inflicted by %u abs is %u",
            GUID_LOPART(GetCasterGUID()),
            GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(),
            target->GetTypeId(), damage, GetId(), absorb);
#endif
  ...
  if (GetSpellInfo()->Effects[GetEffIndex()].IsAreaAuraEffect() ||
      ...
      GetSpellInfo()->Effects[GetEffIndex()].Effect ==
          SPELL_EFFECT_PERSISTENT_AREA_AURA) // some persistent area auras have
                                             // targets like A=53 B=28
  {
  ...
  uint32 procEx =
      (crit ? PROC_EX_CRITICAL_HIT : PROC_EX_NORMAL_HIT) | PROC_EX_INTERNAL_DOT;
  if (absorb > 0)
  ...
  SpellPeriodicAuraLogInfo pInfo(this, damage, overkill, absorb, resist, 0.0f,
                                 crit);
  target->SendPeriodicAuraLog(&pInfo);

  Unit::DealDamage(caster, target, damage, &cleanDamage, DOT,
                   GetSpellInfo()->GetSchoolMask(), GetSpellInfo(), true);
  // allow null caster to call this function
  caster->ProcDamageAndSpell(target, caster ? procAttacker : 0, procVictim,
                             procEx, damage, BASE_ATTACK, GetSpellInfo());
}
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit *target,
  ...
  Unit::CalcAbsorbResist(caster, target, GetSpellInfo()->GetSchoolMask(), DOT,
                         damage, &absorb, &resist, m_spellInfo);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server",
            "PeriodicTick: %u (TypeId: %u) health leech of %u (TypeId: %u) for "
            "%u dmg inflicted by %u abs is %u",
            GUID_LOPART(GetCasterGUID()),
            GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(),
            target->GetTypeId(), damage, GetId(), absorb);
#endif
  if (caster)
    caster->SendSpellNonMeleeDamageLog(
        target, GetId(), damage + absorb + resist,
        GetSpellInfo()->GetSchoolMask(), absorb, resist, false, 0, crit);
  ...
  uint32 heal = uint32(caster->SpellHealingBonusDone(
      caster, GetSpellInfo(), uint32(new_damage * gainMultiplier), DOT, 0.0f,
      GetBase()->GetStackAmount()));
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura", "PeriodicTick: donator %u target %u damage %u.",
            caster->GetEntry(), target->GetEntry(), damage);
#endif
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit *target, Unit *caster) const {
  ...
  if (target != caster &&
      GetSpellInfo()->HasAttribute(SPELL_ATTR2_HEALTH_FUNNEL) &&
  ...
  if (GetAuraType() == SPELL_AURA_OBS_MOD_HEALTH) {
    ...
    float minval =
        (float)target->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_HEALING_PCT);
    if (minval)
      AddPct(TakenTotalMod, minval);

    float maxval =
        (float)target->GetMaxPositiveAuraModifier(SPELL_AURA_MOD_HEALING_PCT);
    if (maxval)
    ...
    if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_DRUID &&
      ...
      damage +=
          GetTotalTicks() * tempAmount * (6 - (drop * tickNumber)) * 0.01f;
    }

    if (GetBase()->GetType() == DYNOBJ_AURA_TYPE)
      damage =
          caster->SpellHealingBonusDone(target, GetSpellInfo(), damage, DOT,
                                        0.0f, GetBase()->GetStackAmount());
  ...
  if ((crit = roll_chance_f(GetCritChance())))
    damage =
        Unit::SpellCriticalHealingBonus(caster, GetSpellInfo(), damage, target);

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server",
            "PeriodicTick: %u (TypeId: %u) heal of %u (TypeId: %u) for %u "
            "health inflicted by %u",
            GUID_LOPART(GetCasterGUID()),
            GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(),
            target->GetTypeId(), damage, GetId());
#endif
  uint32 absorb = 0;
  ...
  SpellPeriodicAuraLogInfo pInfo(this, heal, heal - gain, absorb, 0, 0.0f,
                                 crit);
  target->SendPeriodicAuraLog(&pInfo);
  ...
  if (target != caster &&
    ...
    CleanDamage cleanDamage = CleanDamage(0, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
    Unit::DealDamage(caster, caster, damage, &cleanDamage, SELF_DAMAGE,
                     GetSpellInfo()->GetSchoolMask(), GetSpellInfo(), true);
  }
  ...
  uint32 procVictim = PROC_FLAG_TAKEN_PERIODIC;
  uint32 procEx =
      (crit ? PROC_EX_CRITICAL_HIT : PROC_EX_NORMAL_HIT) | PROC_EX_INTERNAL_HOT;
  if (absorb > 0)
    procEx |= PROC_EX_ABSORB;
  ...
  if (!haveCastItem &&
      GetAuraType() !=
          SPELL_AURA_OBS_MOD_HEALTH) // xinef: dont allow obs_mod_health to proc
                                     // spells, this is passive regeneration and
                                     // not hot
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit *target,
  ...
  // Special case: draining x% of mana (up to a maximum of 2*x% of the caster's
  // maximum mana) It's mana percent cost spells, m_amount is percent drain from
  // target
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server",
            "PeriodicTick: %u (TypeId: %u) power leech of %u (TypeId: %u) for "
            "%u dmg inflicted by %u",
            GUID_LOPART(GetCasterGUID()),
            GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(),
            target->GetTypeId(), drainAmount, GetId());
#endif
  ...
  SpellPeriodicAuraLogInfo pInfo(this, drainedAmount, 0, 0, 0, gainMultiplier,
                                 false);
  target->SendPeriodicAuraLog(&pInfo);
  ...
  if (gainAmount) {
    ...
    target->AddThreat(caster, float(gainedAmount) * 0.5f,
                      GetSpellInfo()->GetSchoolMask(), GetSpellInfo());
  }
  ...
  if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK &&
    ...
    if (manaFeedVal > 0) {
      ...
      caster->CastCustomSpell(caster, 32554, &feedAmount, nullptr, nullptr,
                              true, nullptr, this);
    }
  }
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server",
            "PeriodicTick: %u (TypeId: %u) energize %u (TypeId: %u) for %u dmg "
            "inflicted by %u",
            GUID_LOPART(GetCasterGUID()),
            GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(),
            target->GetTypeId(), amount, GetId());
#endif
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit *target,
  ...
  caster->CalculateSpellDamageTaken(&damageInfo, int32(gain * dmgMultiplier),
                                    spellProto);
  ...
  uint32 procVictim = PROC_FLAG_TAKEN_PERIODIC;
  uint32 procEx =
      createProcExtendMask(&damageInfo, SPELL_MISS_NONE) | PROC_EX_INTERNAL_DOT;
  if (damageInfo.damage)
    procVictim |= PROC_FLAG_TAKEN_DAMAGE;

  caster->DealSpellDamage(&damageInfo, true);
  caster->ProcDamageAndSpell(damageInfo.target, procAttacker, procVictim,
                             procEx, damageInfo.damage, BASE_ATTACK,
                             spellProto);
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "AuraEffect::HandleProcTriggerSpellAuraProc: Triggering spell %u "
              "from aura %u proc",
              triggeredSpellInfo->Id, GetId());
#endif
    triggerCaster->CastSpell(triggerTarget, triggeredSpellInfo, true, nullptr,
                             this);
  } else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "AuraEffect::HandleProcTriggerSpellAuraProc: Could not trigger "
              "spell %u from aura %u proc, because the spell does not have an "
              "entry in Spell.dbc.",
              triggerSpellId, GetId());
#endif
  }
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Triggering "
              "spell %u with value %d from aura %u proc",
              triggeredSpellInfo->Id, basepoints0, GetId());
#endif
    triggerCaster->CastCustomSpell(triggerTarget, triggerSpellId, &basepoints0,
                                   nullptr, nullptr, true, nullptr, this);
  } else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Could not "
              "trigger spell %u from aura %u proc, because the spell does not "
              "have an entry in Spell.dbc.",
              triggerSpellId, GetId());
#endif
  }
}
...
void AuraEffect::HandleProcTriggerDamageAuraProc(AuraApplication *aurApp,
  ...
  Unit *target = aurApp->GetTarget();
  Unit *triggerTarget = eventInfo.GetProcTarget();
  SpellNonMeleeDamage damageInfo(target, triggerTarget, GetId(),
                                 GetSpellInfo()->SchoolMask);
  uint32 damage = target->SpellDamageBonusDone(
      triggerTarget, GetSpellInfo(), GetAmount(), SPELL_DIRECT_DAMAGE);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura",
            "AuraEffect::HandleProcTriggerDamageAuraProc: Triggering %u spell "
            "damage from aura %u proc",
            damage, GetId());
#endif
  target->DealSpellDamage(&damageInfo, true);
}

void AuraEffect::HandleRaidProcFromChargeAuraProc(
    AuraApplication *aurApp, ProcEventInfo & /*eventInfo*/) {
  Unit *target = aurApp->GetTarget();
  ...
  if (jumps > 0) {
    if (Unit *caster = GetCaster()) {
      ...
      if (Unit *triggerTarget = target->GetNextRandomRaidMemberOrPet(radius)) {
        target->CastSpell(triggerTarget, GetSpellInfo(), true, nullptr, this,
                          GetCasterGUID());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura",
            "AuraEffect::HandleRaidProcFromChargeAuraProc: Triggering spell %u "
            "from aura %u proc",
            triggerSpellId, GetId());
#endif
  target->CastSpell(target, triggerSpellId, true, nullptr, this,
                    GetCasterGUID());
}
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "AuraEffect::HandleRaidProcFromChargeWithValueAuraProc: received "
              "not handled spell: %u",
              GetId());
#endif
    return;
  ...
  if (jumps > 0) {
    if (Unit *caster = GetCaster()) {
      ...
      if (Unit *triggerTarget = target->GetNextRandomRaidMemberOrPet(radius)) {
        target->CastCustomSpell(triggerTarget, GetId(), &value, nullptr,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura",
            "AuraEffect::HandleRaidProcFromChargeWithValueAuraProc: Triggering "
            "spell %u from aura %u proc",
            triggerSpellId, GetId());
#endif
  target->CastCustomSpell(target, triggerSpellId, &value, nullptr, nullptr,
                          true, nullptr, this, GetCasterGUID());
}
