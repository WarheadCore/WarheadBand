...
AuraApplication::AuraApplication(Unit *target, Unit *caster, Aura *aura,
  ...
  if (GetBase()->CanBeSentToClient()) {
    ...
    if (AuraApplication *foundAura = GetTarget()->GetAuraApplication(
            GetBase()->GetId(), GetBase()->GetCasterGUID(),
...
void AuraApplication::_Remove() {
  ...
  if (AuraApplication *foundAura = _target->GetAuraApplication(
          GetBase()->GetId(), GetBase()->GetCasterGUID(),
          GetBase()->GetCastItemGUID())) {
    // Reuse visible aura slot by aura which is still applied - prevent storing
    // dead pointers
  ...
  if (uint32 groupId = aurEff->GetAuraGroup()) {
    ...
    if (!aurEff->IsPeriodic() &&
      ...
      for (Unit::AuraEffectList::const_iterator iter = auraList.begin();
        ...
        if ((*iter)->GetAuraGroup() != groupId || (*iter) == strongestEff ||
            (*iter)->GetBase()->IsRemoved())
          continue;
        ...
        if (amount < abs((*iter)->GetForcedAmount())) {
          ...
          if (strongestApp && strongestEff &&
            ...
            strongestEff->HandleEffect(strongestApp,
                                       AURA_EFFECT_HANDLE_CHANGE_AMOUNT, false);
        ...
        // xinef: itered aura is weaker, deactivate if active
          ...
          (*iter)->HandleEffect(aurApp, AURA_EFFECT_HANDLE_CHANGE_AMOUNT,
                                false);
      ...
      if (strongestApp && strongestEff &&
        ...
        strongestEff->HandleEffect(strongestApp,
                                   AURA_EFFECT_HANDLE_CHANGE_AMOUNT, true);
      }
    }
  }
  SetNeedClientUpdate();
...
void AuraApplication::BuildUpdatePacket(ByteBuffer &data, bool remove) const {
  ...
  if (aura->GetMaxDuration() > 0 &&
      !aura->GetSpellInfo()->HasAttribute(SPELL_ATTR5_HIDE_DURATION))
    ...
    if (const Player *plr = GetTarget()->ToPlayer())
      if (Aura *aura = GetBase())
        if (plr->NeedSendSpectatorData() &&
            ArenaSpectator::ShouldSendAura(aura, GetEffectMask(),
                                           GetTarget()->GetGUID(), remove))
          ArenaSpectator::SendCommand_Aura(
              plr->FindMap(), plr->GetGUID(), "AUR", aura->GetCasterGUID(),
              aura->GetSpellInfo()->Id, aura->GetSpellInfo()->IsPositive(),
              aura->GetSpellInfo()->Dispel, aura->GetDuration(),
              aura->GetMaxDuration(),
...
Aura *Aura::TryRefreshStackOrCreate(
    SpellInfo const *spellproto, uint8 tryEffMask, WorldObject *owner,
    Unit *caster, int32 *baseAmount /*= nullptr*/, Item *castItem /*= nullptr*/,
    uint64 casterGUID /*= 0*/, bool *refresh /*= nullptr*/,
  ...
  if (Aura *foundAura = owner->ToUnit()->_TryStackingOrRefreshingExistingAura(
          spellproto, effMask, caster, baseAmount, castItem, casterGUID,
          periodicReset)) {
...
Aura *Aura::TryCreate(SpellInfo const *spellproto, uint8 tryEffMask,
                      WorldObject *owner, Unit *caster,
                      int32 *baseAmount /*= nullptr*/,
                      Item *castItem /*= nullptr*/, uint64 casterGUID /*= 0*/) {
  ASSERT(spellproto);
  ASSERT(owner);
  ASSERT(caster || casterGUID);
  ASSERT(tryEffMask <= MAX_EFFECT_MASK);
...
Aura *Aura::Create(SpellInfo const *spellproto, uint8 effMask,
  ...
  if (owner->isType(TYPEMASK_UNIT))
    if (!owner->IsInWorld() || ((Unit *)owner)->IsDuringRemoveFromWorld())
      // owner not in world so don't allow to own not self casted single target
      // auras
...
Aura::Aura(SpellInfo const *spellproto, WorldObject *owner, Unit *caster,
    ...
    : m_spellInfo(spellproto),
      ...
      m_applyTime(time(nullptr)), m_owner(owner), m_timeCla(0),
  ...
  if ((m_spellInfo->ManaPerSecond || m_spellInfo->ManaPerSecondPerLevel) &&
      !m_spellInfo->HasAttribute(SPELL_ATTR2_HEALTH_FUNNEL))
  ...
  // m_casterLevel = cast item level/caster level, caster level should be saved
  // to db, confirmed with sniffs
}
...
void Aura::_UnapplyForTarget(Unit *target, Unit *caster,
  ...
  if (itr == m_applications.end()) {
    LOG_ERROR("server",
              "Aura::_UnapplyForTarget, target:%u, caster:%u, spell:%u was not "
              "found in owners application map!",
              target->GetGUIDLow(), caster ? caster->GetGUIDLow() : 0,
...
void Aura::UpdateTargetMap(Unit *caster, bool apply) {
  ...
  for (ApplicationMap::iterator appIter = m_applications.begin();
    ...
    if (existing == targets.end())
      ...
      // xinef: check immunities here, so aura wont get removed on every tick
      // and then reapplied
      if (IsArea())
      ...
      // (dbcs do not have auras which apply on same type of targets but have
      // different radius, so this is not really needed)
  ...
  for (std::map<Unit *, uint8>::iterator itr = targets.begin();
    ...
    if (AuraApplication *aurApp =
      ...
      // but because we know the reason of a crash we can remove the assertion
      // for now
    ...
    if (addUnit) {
      ...
      else // (GetType() == UNIT_AURA_TYPE)
      {
        ...
        if (itr->first != GetOwner()) {
          ...
          // this one prevents unwanted usefull buff loss because of stacking
          // and prevents overriding auras periodicaly by 2 near area aura
          // owners
          for (Unit::AuraApplicationMap::iterator iter =
                   itr->first->GetAppliedAuras().begin();
               iter != itr->first->GetAppliedAuras().end(); ++iter) {
    ...
    if (!addUnit)
      ...
      if (!GetOwner()->IsSelfOrInSameMap(itr->first)) {
        ...
        LOG_FATAL("server",
                  "Aura %u: Owner %s (map %u) is not in the same map as target "
                  "%s (map %u).",
                  GetSpellInfo()->Id, GetOwner()->GetName().c_str(),
    ...
    if (m_timeCla) {
      if (m_timeCla > int32(diff))
        ...
        if (int32 ManaPerSecond =
                m_spellInfo->ManaPerSecond +
                m_spellInfo->ManaPerSecondPerLevel * caster->getLevel()) {
          m_timeCla += 1000 - diff;
...
void Aura::RefreshTimersWithMods() {
  ...
  // xinef: we should take ModSpellDuration into account, but none of the spells
  // using this function is affected by contents of ModSpellDuration
  RefreshDuration();
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    if (HasEffect(i))
...
void Aura::SetStackAmount(uint8 stackAmount) {
  ...
  for (std::list<AuraApplication *>::const_iterator apptItr =
           applications.begin();
       apptItr != applications.end(); ++apptItr)
    if (!(*apptItr)->GetRemoveMode())
      HandleAuraSpecificMods(*apptItr, caster, false, true);

  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    if (HasEffect(i))
      m_effects[i]->ChangeAmount(m_effects[i]->CalculateAmount(caster), false,
                                 true);
...
bool Aura::ModStackAmount(int32 num, AuraRemoveMode removeMode,
  ...
  // limit the stack amount (only on stack increase, stack amount may be changed
  // manually)
  ...
  bool refresh =
      stackAmount >= GetStackAmount() &&
      (m_spellInfo->StackAmount ||
       !m_spellInfo->HasAttribute(SPELL_ATTR1_DONT_REFRESH_DURATION_ON_RECAST));
...
bool Aura::CanBeSaved() const {
  if (IsPassive() ||
      (GetSpellInfo()->HasAttribute(SPELL_ATTR0_HIDDEN_CLIENTSIDE) &&
       GetSpellInfo()->Stances))
    return false;
  ...
  // No point in saving this, since the stable dialog can't be open on aura load
  // anyway.
  ...
  if (HasEffectType(SPELL_AURA_MOD_POSSESS) ||
      HasEffectType(SPELL_AURA_MOD_POSSESS_PET))
    return false;

  // xinef: no charming auras can be saved
  if (HasEffectType(SPELL_AURA_MOD_CHARM) ||
      HasEffectType(SPELL_AURA_AOE_CHARM))
    return false;
...
void Aura::UnregisterSingleTarget() {
  ...
  if (!caster)
    caster = ObjectAccessor::GetObjectInOrOutOfWorld(GetCasterGUID(),
                                                     (Unit *)nullptr);
  if (!caster) {
    LOG_INFO("misc", "Aura::UnregisterSingleTarget (A1) - %u, %u, %u, %s",
             GetId(), GetOwner()->GetTypeId(), GetOwner()->GetEntry(),
...
int32 Aura::CalcDispelChance(Unit *auraTarget, bool offensive) const {
  ...
  if (Unit *caster = GetCaster())
    if (Player *modOwner = caster->GetSpellModOwner())
      modOwner->ApplySpellMod(GetId(), SPELLMOD_RESIST_DISPEL_CHANCE,
                              resistChance);
...
void Aura::SetLoadedState(int32 maxduration, int32 duration, int32 charges,
                          uint8 stackamount, uint8 recalculateMask,
                          int32 *amount) {
...
void Aura::HandleAuraSpecificMods(AuraApplication const *aurApp, Unit *caster,
  ...
  if (!onReapply) {
    ...
    if (apply) {
      if (std::vector<int32> const *spellTriggered =
        ...
        for (std::vector<int32>::const_iterator itr = spellTriggered->begin();
             itr != spellTriggered->end(); ++itr) {
          if (*itr < 0)
      ...
      if (std::vector<int32> const *spellTriggered =
        ...
        for (std::vector<int32>::const_iterator itr = spellTriggered->begin();
          ...
          if (*itr < 0)
            ...
            target->CastSpell(target, *itr, true, nullptr, nullptr,
                              GetCasterGUID());
        }
      }
      if (std::vector<int32> const *spellTriggered =
      ...
      for (std::vector<int32>::const_iterator itr = spellTriggered->begin();
        ...
        if (*itr > 0)
          if (Aura *triggeredAura = target->GetAura(*itr, GetCasterGUID()))
            triggeredAura->ModStackAmount(GetStackAmount() -
  ...
  if (apply) {
    ...
    case SPELLFAMILY_MAGE:
      ...
      case 12043: // Presence of Mind
        ...
        if (AuraEffect const *aurEff = caster->GetAuraEffect(
          ...
          default:
            LOG_ERROR("server",
                      "Aura::HandleAuraSpecificMods: Unknown rank of Arcane "
                      "Potency (%d) found",
                      aurEff->GetId());
          }
          if (spellId)
    ...
    case SPELLFAMILY_DRUID:
      ...
      if (GetSpellInfo()->SpellFamilyFlags[0] & 0x10 && GetEffect(0)) {
        ...
        if (caster->GetAuraEffectDummy(64760)) {
          ...
          damage = target->SpellHealingBonusTaken(caster, GetSpellInfo(),
                                                  damage, DOT);

          int32 basepoints0 = damage;
          caster->CastCustomSpell(target, 64801, &basepoints0, nullptr, nullptr,
                                  true, nullptr, GetEffect(0));
        }
      }
      break;
    case SPELLFAMILY_PRIEST:
      ...
      if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0)) {
        ...
        if (AuraEffect const *aurEff =
                caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3790, 1)) {
          uint32 damage = GetEffect(0)->GetAmount();
          damage = target->SpellDamageBonusTaken(caster, GetSpellInfo(), damage,
                                                 DOT);
          int32 basepoints0 = aurEff->GetAmount() *
                              GetEffect(0)->GetTotalTicks() * int32(damage) /
                              100;
          int32 heal = int32(CalculatePct(basepoints0, 15));

          caster->CastCustomSpell(target, 63675, &basepoints0, nullptr, nullptr,
                                  true, nullptr, GetEffect(0));
          caster->CastCustomSpell(caster, 75999, &heal, nullptr, nullptr, true,
                                  nullptr, GetEffect(0));
        }
      }
      // Power Word: Shield
        ...
        if (AuraEffect *glyph = caster->GetAuraEffect(55672, 0)) {
          ...
          caster->CastCustomSpell(GetUnitOwner(), 56160, &heal, nullptr,
                                  nullptr, true, 0, GetEffect(0));
        }
      }
      break;
    ...
    case SPELLFAMILY_DEATHKNIGHT:
      ...
      if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2) {
        ...
        for (Unit::AuraEffectList::const_iterator itr = TalentAuras.begin();
             itr != TalentAuras.end(); ++itr) {
        ...
        if (aurEff) {
          ...
          default:
            LOG_ERROR("server",
                      "Aura::HandleAuraSpecificMods: Unknown rank of Crypt "
                      "Fever/Ebon Plague (%d) found",
                      aurEff->GetId());
          }
            ...
            for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
              if (GetEffect(i))
                GetEffect(i)->SetAmount(CalculatePct(
                    ...
                    100 + sSpellMgr->GetSpellMixologyBonus(GetId())));
  ...
  // mods at aura remove
    ...
    case SPELLFAMILY_GENERIC:
      ...
      case 61987: // Avenging Wrath
        // Remove the immunity shield marker on Avenging Wrath removal if
        // Forbearance is not present
      ...
      case 72369:
        if (AuraEffect *aurEff = GetEffect(0)) {
          ...
          if (remainingDamage > 0)
            caster->CastCustomSpell(caster, 72373, nullptr, &remainingDamage,
                                    nullptr, true);
        }
        break;
      }
      ...
      if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL &&
        ...
        if (AuraEffect *absorb = GetEffect(EFFECT_0))
          if (absorb->GetAmount() <= 0) // removed by damage, not dispel
            if (AuraEffect *dummy =
                    caster->GetDummyAuraEffect(SPELLFAMILY_MAGE, 2945, 0))
    ...
    case SPELLFAMILY_WARLOCK:
      ...
      if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400) {
        if (AuraEffect *aurEff = caster->GetAuraEffect(
          ...
          default:
            LOG_ERROR("server",
                      "Aura::HandleAuraSpecificMods: Unknown rank of Improved "
                      "Fear (%d) found",
                      aurEff->GetId());
          }
          if (spellId)
    ...
    case SPELLFAMILY_PRIEST:
      ...
      if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL &&
        ...
        // Shadow Affinity
        if (AuraEffect const *aurEff =
                caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 178, 1)) {
          int32 basepoints0 =
              aurEff->GetAmount() * caster->GetCreateMana() / 100;
          caster->CastCustomSpell(caster, 64103, &basepoints0, nullptr, nullptr,
                                  true, nullptr, GetEffect(0));
        }
      }
      // Power word: shield
        ...
        if (Aura const *aura = caster->GetAuraOfRankedSpell(47535)) {
          ...
          if (caster->GetTypeId() == TYPEID_PLAYER) {
            if (caster->ToPlayer()->HasSpellCooldown(aura->GetId())) {
              // This additional check is needed to add a minimal delay before
              // cooldown in in effect to allow all bubbles broken by a single
              // damage source proc mana return
              if (caster->ToPlayer()->GetSpellCooldownDelay(aura->GetId()) <=
                  11500)
                break;
          ...
          if (AuraEffect const *aurEff = aura->GetEffect(0)) {
            ...
            int32 basepoints0 = int32(
                CalculatePct(caster->GetMaxPower(POWER_MANA), multiplier));
            caster->CastCustomSpell(caster, 47755, &basepoints0, nullptr,
                                    nullptr, true);
          }
          ...
          if (AuraEffect const *aurEff = aura->GetEffect(1)) {
            ...
            case POWER_MANA: {
              int32 basepoints0 =
                  int32(CalculatePct(target->GetMaxPower(POWER_MANA), 2));
              caster->CastCustomSpell(target, 63654, &basepoints0, nullptr,
                                      nullptr, true);
              break;
            }
            case POWER_RAGE:
      ...
      case 47788: // Guardian Spirit
        ...
        if (AuraEffect *aurEff = player->GetAuraEffect(63231, 0)) {
          ...
          player->AddSpellCooldown(GetSpellInfo()->Id, 0,
                                   aurEff->GetAmount() * IN_MILLISECONDS);
    ...
    case SPELLFAMILY_PALADIN:
      // Remove the immunity shield marker on Forbearance removal if AW marker
      // is not present
  ...
  case SPELLFAMILY_DRUID:
    ...
    if (GetId() == 5229) {
      ...
      } else if (AuraEffect *aurEff =
                     caster->GetAuraEffect(SPELL_AURA_MOD_BASE_RESISTANCE_PCT,
                                           SPELLFAMILY_DRUID, 107, 0))
...
bool Aura::CanStackWith(Aura const *existingAura, bool remove) const {
  ...
  // xinef: dont allow different ranks (or the same rank) of same spell to stack
  // (many flamestrikes for example)
  ...
  if (stackFlags) {
    ...
    if (sameCaster &&
        m_spellInfo->SpellFamilyName == existingSpellInfo->SpellFamilyName &&
  ...
  if (!sameCaster) {
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      ...
      case SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE:
        // periodic auras which target areas are not allowed to stack this way
    ...
    if (GetSpellInfo()->EquippedItemClass == ITEM_CLASS_WEAPON) {
      ...
      if (eventInfo.GetDamageInfo()) {
        ...
        if (attType == BASE_ATTACK)
          ...
          item = target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0,
                                                         EQUIPMENT_SLOT_RANGED);

        if (!item || item->IsBroken() ||
            item->GetTemplate()->Class != ITEM_CLASS_WEAPON ||
            !((1 << item->GetTemplate()->SubClass) &
              GetSpellInfo()->EquippedItemSubClassMask))
          return false;
      }
    } else if (GetSpellInfo()->EquippedItemClass == ITEM_CLASS_ARMOR) {
      // Check if player is wearing shield
      Item *item = target->ToPlayer()->GetUseableItemByPos(
          INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
      if (!item || item->IsBroken() ||
...
float Aura::CalcProcChance(SpellProcEntry const &procEntry,
  ...
  if (Unit *caster = GetCaster()) {
    ...
    if (eventInfo.GetDamageInfo() && procEntry.ratePerMinute != 0) {
      uint32 WeaponSpeed =
          caster->GetAttackTime(eventInfo.GetDamageInfo()->GetAttackType());
      chance = caster->GetPPMProcChance(WeaponSpeed, procEntry.ratePerMinute,
                                        GetSpellInfo());
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "Aura::LoadScripts: Script `%s` for aura `%u` is loaded now",
...
void Aura::CallScriptEffectAfterManaShieldHandlers(
    AuraEffect *aurEff, AuraApplication const *aurApp, DamageInfo &dmgInfo,
    uint32 &absorbAmount) {
  for (std::list<AuraScript *>::iterator scritr = m_loadedScripts.begin();
       scritr != m_loadedScripts.end(); ++scritr) {
...
UnitAura::UnitAura(SpellInfo const *spellproto, uint8 effMask,
                   WorldObject *owner, Unit *caster, int32 *baseAmount,
                   Item *castItem, uint64 casterGUID)
    ...
    if (GetSpellInfo()->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AURA) {
      ...
      if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED)) {
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_RAID: {
          targetList.push_back(GetUnitOwner());
          Warhead::AnyGroupedUnitInObjectRangeCheck u_check(
              GetUnitOwner(), GetUnitOwner(), radius,
              GetSpellInfo()->Effects[effIndex].Effect ==
                  SPELL_EFFECT_APPLY_AREA_AURA_RAID);
          Warhead::UnitListSearcher<Warhead::AnyGroupedUnitInObjectRangeCheck>
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND: {
          targetList.push_back(GetUnitOwner());
          Warhead::AnyFriendlyUnitInObjectRangeCheck u_check(
              GetUnitOwner(), GetUnitOwner(), radius);
          Warhead::UnitListSearcher<Warhead::AnyFriendlyUnitInObjectRangeCheck>
              searcher(GetUnitOwner(), targetList, u_check);
          GetUnitOwner()->VisitNearbyObject(radius, searcher);
          break;
        }
        case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY: {
          Warhead::AnyAoETargetUnitInObjectRangeCheck u_check(
              GetUnitOwner(), GetUnitOwner(),
              radius); // No GetCharmer in searcher
          Warhead::UnitListSearcher<Warhead::AnyAoETargetUnitInObjectRangeCheck>
              searcher(GetUnitOwner(), targetList, u_check);
          GetUnitOwner()->VisitNearbyObject(radius, searcher);
          break;
        }
...
DynObjAura::DynObjAura(SpellInfo const *spellproto, uint8 effMask,
                       WorldObject *owner, Unit *caster, int32 *baseAmount,
                       Item *castItem, uint64 casterGUID)
    : Aura(spellproto, owner, caster, castItem, casterGUID) {
  LoadScripts();
...
void DynObjAura::FillTargetMap(std::map<Unit *, uint8> &targets,
  ...
  for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
    if (!HasEffect(effIndex))
      continue;
    UnitList targetList;
    if (GetSpellInfo()->Effects[effIndex].TargetB.GetTarget() ==
            TARGET_DEST_DYNOBJ_ALLY ||
        GetSpellInfo()->Effects[effIndex].TargetB.GetTarget() ==
    ...
    else if (GetSpellInfo()->Effects[effIndex].TargetB.GetTarget() ==
             TARGET_DEST_DYNOBJ_NONE) {
      Warhead::AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck
          u_check(GetDynobjOwner(), dynObjOwnerCaster, radius);
      Warhead::UnitListSearcher<
          Warhead::AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck>
          searcher(GetDynobjOwner(), targetList, u_check);
      GetDynobjOwner()->VisitNearbyObject(radius, searcher);
    } else {
