...
SpellDestination::SpellDestination(float x, float y, float z, float orientation,
                                   uint32 mapId) {
...
SpellDestination::SpellDestination(WorldObject const &wObj) {
  ...
  _transportOffset.Relocate(wObj.GetTransOffsetX(), wObj.GetTransOffsetY(),
...
void SpellCastTargets::Read(ByteBuffer &data, Unit *caster) {
  ...
  if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION) {
    ...
    if (m_src._transportGUID)
      m_src._transportOffset.Relocate(
          caster->GetTransOffsetX(), caster->GetTransOffsetY(),
          caster->GetTransOffsetZ(), caster->GetTransOffsetO());
    else
  ...
  if (m_targetMask & TARGET_FLAG_DEST_LOCATION) {
    ...
    if (m_dst._transportGUID)
      m_dst._transportOffset.Relocate(
          caster->GetTransOffsetX(), caster->GetTransOffsetY(),
          caster->GetTransOffsetZ(), caster->GetTransOffsetO());
    else
...
void SpellCastTargets::Write(ByteBuffer &data) {
  ...
  if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION) {
    data.appendPackGUID(m_src._transportGUID); // relative position guid here -
  ...
  if (m_targetMask & TARGET_FLAG_DEST_LOCATION) {
    data.appendPackGUID(m_dst._transportGUID); // relative position guid here -
                                               // transport for example
    if (m_dst._transportGUID)
      data << m_dst._transportOffset.PositionXYZStream();
    else
...
WorldObject *SpellCastTargets::GetObjectTargetChannel(Unit *caster) const {
  return m_objectTargetGUIDChannel
             ? ((m_objectTargetGUIDChannel == caster->GetGUID())
                    ? caster
                    : ObjectAccessor::GetWorldObject(*caster,
...
void SpellCastTargets::Update(Unit *caster) {
  ...
  if (caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    if (m_targetMask & TARGET_FLAG_ITEM)
      ...
      if (m_itemTargetGUID ==
          TRADE_SLOT_NONTRADED) // here it is not guid but slot. Also prevents
                                // hacking slots
...
void SpellCastTargets::OutDebug() const {
  ...
  if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
    LOG_INFO("server",
             "Source location: transport guid:" UI64FMTD
             " trans offset: %s position: %s",
             m_src._transportGUID, m_src._transportOffset.ToString().c_str(),
             m_src._position.ToString().c_str());
  if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
    LOG_INFO("server",
             "Destination location: transport guid:" UI64FMTD
             " trans offset: %s position: %s",
             m_dst._transportGUID, m_dst._transportOffset.ToString().c_str(),
...
Spell::Spell(Unit *caster, SpellInfo const *info, TriggerCastFlags triggerFlags,
    ...
    : m_spellInfo(sSpellMgr->GetSpellForDifficultyFromSpell(info, caster)),
      m_caster((info->HasAttribute(SPELL_ATTR6_CAST_BY_CHARMER) &&
  ...
  if (m_attackType == RANGED_ATTACK)
    ...
    if ((m_caster->getClassMask() & CLASSMASK_WAND_USERS) != 0 &&
      ...
      if (Item *pItem = m_caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK))
        m_spellSchoolMask =
            SpellSchoolMask(1 << pItem->GetTemplate()->Damage[0].DamageType);
  ...
  if (info->HasAttribute(SPELL_ATTR4_CAN_CAST_WHILE_CASTING))
    _triggeredCastFlags = TriggerCastFlags(uint32(_triggeredCastFlags) |
...
Spell::~Spell() {
  ...
  if (m_referencedFromCurrentSpell && m_selfContainer &&
    ...
    // If this error is repeating, we may have to add an ASSERT to better track
    // down how we get into this case.
    LOG_ERROR("server",
              "SPELL: deleting spell for spell ID %u. However, spell still "
              "referenced.",
              m_spellInfo->Id);
    *m_selfContainer = nullptr;
  }
...
void Spell::InitExplicitTargets(SpellCastTargets const &targets) {
  ...
  // such spells serverside this also makes sure that we correctly send explicit
  // targets to client (removes redundant data)
  uint32 neededTargets = m_spellInfo->GetExplicitTargetMask();

  if (WorldObject *target = m_targets.GetObjectTarget()) {
    // check if object target is valid with needed target flags
    // for unit case allow corpse target mask because player with not released
    // corpse is a unit target
    if ((target->ToUnit() && !(neededTargets & (TARGET_FLAG_UNIT_MASK |
    ...
    if (neededTargets & (TARGET_FLAG_UNIT_MASK)) {
      ...
      if (Player *playerCaster = m_caster->ToPlayer()) {
        ...
        if (Unit *selectedUnit =
                ObjectAccessor::GetUnit(*m_caster, playerCaster->GetTarget()))
          if (m_spellInfo->CheckExplicitTarget(m_caster, selectedUnit) ==
              SPELL_CAST_OK)
            unit = selectedUnit;
      }
      // try to use attacked unit as a target
      else if ((m_caster->GetTypeId() == TYPEID_UNIT) &&
               neededTargets & (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT))
        unit = m_caster->GetVictim();

      // didn't find anything - let's use self as target
      if (!unit &&
          neededTargets & (TARGET_FLAG_UNIT_RAID | TARGET_FLAG_UNIT_PARTY |
...
void Spell::SelectSpellTargets() {
  ...
  for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    // Also some spells use not used effect targets for store targets for dummy
    // effect in triggered spells
    ...
    // those are used when no valid target could be added for spell effect based
    // on spell target type some spell effects use explicit target as a default
    // target added to target map (like SPELL_EFFECT_LEARN_SPELL) some spell
    // effects add target to target map only when target type specified (like
    // SPELL_EFFECT_WEAPON) some spell effects don't add anything to target map
    // (confirmed with sniffs) (like SPELL_EFFECT_DESTROY_ALL_TOTEMS)
  ...
  if (m_targets.HasDst()) {
    if (m_targets.HasTraj()) {
      ...
      if (speed > 0.0f)
        m_delayTrajectory =
            (uint64)floor(m_targets.GetDist2d() / speed * 1000.0f);
...
void Spell::SelectEffectImplicitTargets(
  ...
  case TARGET_SELECT_CATEGORY_AREA:
    ...
    for (uint32 j = effIndex + 1; j < MAX_SPELL_EFFECTS; ++j) {
      ...
      if (effects[effIndex].TargetA.GetTarget() ==
  ...
  case TARGET_SELECT_CATEGORY_TRAJ:
    // xinef: just in case there is no dest, explanation in
  ...
  case TARGET_SELECT_CATEGORY_DEFAULT:
    ...
    case TARGET_OBJECT_TYPE_SRC:
      ...
      default:
        ASSERT(false &&
               "Spell::SelectEffectImplicitTargets: received not implemented "
               "select target reference type for TARGET_TYPE_OBJECT_SRC");
        break;
      }
      break;
    case TARGET_OBJECT_TYPE_DEST:
      ...
      default:
        ASSERT(false &&
               "Spell::SelectEffectImplicitTargets: received not implemented "
               "select target reference type for TARGET_TYPE_OBJECT_DEST");
        break;
      }
      break;
    default:
      ...
      default:
        ASSERT(false &&
               "Spell::SelectEffectImplicitTargets: received not implemented "
               "select target reference type for TARGET_TYPE_OBJECT");
        break;
      }
      break;
    }
  ...
  case TARGET_SELECT_CATEGORY_NYI:
    ...
    LOG_DEBUG("spells.aura",
              "SPELL: target type %u, found in spellID %u, effect %u is not "
              "implemented yet!",
...
void Spell::SelectImplicitChannelTargets(
  ...
  case TARGET_DEST_CHANNEL_TARGET:
    if (m_targets.HasDstChannel())
      ...
      if (target)
        m_targets.SetDst(*target);
    } else if (Spell *channeledSpell =
                   m_originalCaster->GetCurrentSpell(CURRENT_CHANNELED_SPELL)) {
...
void Spell::SelectImplicitNearbyTargets(
  ...
  // handle emergency case - try to use other provided targets if no conditions
  // provided
  if (targetType.GetCheckType() == TARGET_CHECK_ENTRY &&
    ...
    LOG_DEBUG("spells.aura",
              ...
              "target with TARGET_CHECK_ENTRY of spell ID %u, effect %u - "
  ...
  WorldObject *target = SearchNearbyTarget(range, targetType.GetObjectType(),
                                           targetType.GetCheckType(), condList);
  if (!target) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "Spell::SelectImplicitNearbyTargets: cannot find nearby target "
              "for spell ID %u, effect %u",
              m_spellInfo->Id, effIndex);
#endif
    return;
  }
  ...
  CallScriptObjectTargetSelectHandlers(target, effIndex, targetType);
  if (!target) {
    // TC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets:
    // OnObjectTargetSelect script hook for spell Id %u set nullptr target,
    // effect %u", m_spellInfo->Id, effIndex);
    return;
  }
  ...
  case TARGET_OBJECT_TYPE_UNIT: {
    if (Unit *unitTarget = target->ToUnit()) {
      ...
      // xinef: important! if channeling spell have nearby entry, it has no
      // unitTarget by default and if channeled spell has target 77, it requires
      // unitTarget, set it here! xinef: if we have NO unit target
      if (!m_targets.GetUnitTarget())
        m_targets.SetUnitTarget(unitTarget);
    } else {
      // TC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets:
      // OnObjectTargetSelect script hook for spell Id %u set object of wrong
      // type, expected unit, got %s, effect %u", m_spellInfo->Id,
      // GetLogNameForGuid(target->GetGUID()), effMask);
      return;
    }
    break;
  }
  case TARGET_OBJECT_TYPE_GOBJ:
    if (GameObject *gobjTarget = target->ToGameObject())
      ...
      // TC_LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets:
      // OnObjectTargetSelect script hook for spell Id %u set object of wrong
      // type, expected gameobject, got %s, effect %u", m_spellInfo->Id,
      // GetLogNameForGuid(target->GetGUID()), effMask);
      return;
    }
    break;
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex,
  ...
  if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList)) {
    Warhead::WorldObjectSpellConeTargetCheck check(
        coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
    Warhead::WorldObjectListSearcher<Warhead::WorldObjectSpellConeTargetCheck>
        searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<Warhead::WorldObjectListSearcher<
        Warhead::WorldObjectSpellConeTargetCheck>>(searcher, containerTypeMask,
                                                   m_caster, m_caster, radius);
    ...
    if (!targets.empty()) {
      ...
      if (uint32 maxTargets = m_spellValue->MaxAffectedTargets) {
        Unit::AuraEffectList const &Auras =
            m_caster->GetAuraEffectsByType(SPELL_AURA_MOD_MAX_AFFECTED_TARGETS);
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex,
  ...
  case TARGET_REFERENCE_TYPE_LAST: {
    ...
    for (std::list<TargetInfo>::reverse_iterator ihit =
         ...
         ihit != m_UniqueTargetInfo.rend(); ++ihit) {
  ...
  // Xinef: the distance should be increased by caster size, it is neglected in
  // latter calculations
  std::list<WorldObject *> targets;
  float radius = m_spellInfo->Effects[effIndex].CalcRadius(m_caster) *
                 m_spellValue->RadiusMod;
  SearchAreaTargets(targets, radius, center, referer,
                    targetType.GetObjectType(), targetType.GetCheckType(),
                    m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
...
void Spell::SelectImplicitCasterDestTargets(
  ...
  case TARGET_DEST_HOME:
    if (Player *playerCaster = m_caster->ToPlayer())
      dest = SpellDestination(
          playerCaster->m_homebindX, playerCaster->m_homebindY,
          playerCaster->m_homebindZ, playerCaster->GetOrientation(),
      ...
      if (m_spellInfo->HasEffect(SPELL_EFFECT_TELEPORT_UNITS) ||
        ...
        dest =
            SpellDestination(st->target_X, st->target_Y, st->target_Z,
                             st->target_Orientation, (int32)st->target_mapId);
      else if (st->target_mapId == m_caster->GetMapId())
        dest = SpellDestination(st->target_X, st->target_Y, st->target_Z,
  ...
  case TARGET_DEST_CASTER_FISHING: {
    ...
    // this contains extra code that breaks fishing
    m_caster->GetNearPoint(m_caster, x, y, z, DEFAULT_WORLD_OBJECT_SIZE, dis,
                           m_caster->GetOrientation() + angle);

    float ground = m_caster->GetMapHeight(x, y, z, true);
    float liquidLevel = VMAP_INVALID_HEIGHT_VALUE;
    LiquidData liquidData;
    if (m_caster->GetMap()->getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS,
                                            &liquidData))
      liquidLevel = liquidData.level;

    if (liquidLevel <=
        ground) // When there is no liquid Map::GetWaterOrGroundLevel returns
                // ground level
    {
...
void Spell::SelectImplicitCasterObjectTargets(
  ...
  case TARGET_UNIT_PASSENGER_7:
    if (m_caster->GetTypeId() == TYPEID_UNIT &&
      ...
      target = m_caster->GetVehicleKit()->GetPassenger(targetType.GetTarget() -
...
void Spell::SelectImplicitChainTargets(
    SpellEffIndex effIndex, SpellImplicitTargetInfo const &targetType,
    WorldObject *target, uint32 effMask) {
  uint32 maxTargets = m_spellInfo->Effects[effIndex].ChainTarget;
  if (Player *modOwner = m_caster->GetSpellModOwner())
    modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_JUMP_TARGETS, maxTargets,
                            this);
  ...
  if (maxTargets > 1) {
    ...
    std::list<WorldObject *> targets;
    SearchChainTargets(
        targets, maxTargets - 1, target, targetType.GetObjectType(),
        targetType.GetCheckType(), targetType.GetSelectionCategory(),
        m_spellInfo->Effects[effIndex].ImplicitTargetConditions,
...
float tangent(float x) {
  ...
  // std::numeric_limits<float>::max()) return std::numeric_limits<float>::max();
  // if (x <= -std::numeric_limits<float>::max()) return
...
void Spell::SelectImplicitTrajTargets(
  ...
  std::list<WorldObject *> targets;
  Warhead::WorldObjectSpellTrajTargetCheck check(
      dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo,
      TARGET_CHECK_ENEMY /*targetCheckType*/,
      m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
  Warhead::WorldObjectListSearcher<Warhead::WorldObjectSpellTrajTargetCheck>
      searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
  SearchTargets<Warhead::WorldObjectListSearcher<
      Warhead::WorldObjectSpellTrajTargetCheck>>(
      searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(),
      dist2d);
  if (targets.empty())
    return;
  ...
  for (; itr != targets.end(); ++itr) {
    ...
    const float objDist2d =
        fabs(m_targets.GetSrcPos()->GetExactDist2d(*itr) *
             cos(m_targets.GetSrcPos()->GetRelativeAngle(*itr)));
    const float dz =
        fabs((*itr)->GetPositionZ() - m_targets.GetSrcPos()->m_positionZ);

    DEBUG_TRAJ(LOG_ERROR("server",
                         "Spell::SelectTrajTargets: check %u, dist between %f "
                         "%f, height between %f %f.",
                         (*itr)->GetEntry(), objDist2d - size, objDist2d + size,
                         dz - size, dz + size);)

    float dist = objDist2d - size;
    float height = dist * (a * dist + b);
    DEBUG_TRAJ(LOG_ERROR("server",
                         "Spell::SelectTrajTargets: dist %f, height %f.", dist,
                         height);)
...
#define CHECK_DIST                                                             \
  {                                                                            \
    DEBUG_TRAJ(LOG_ERROR("server",                                             \
                         "Spell::SelectTrajTargets: dist %f, height %f.",      \
                         dist, height);)                                       \
    if (dist > bestDist)                                                       \
      continue;                                                                \
    ...
    if (m_CastItem) {
      if (dist < bestDist &&
          fabs(dz) < 6.0f) // closes target, also check Z difference)
      {
        bestDist = dist;
        break;
      }
  ...
  if (m_targets.GetSrcPos()->GetExactDist2d(m_targets.GetDstPos()) > bestDist) {
    ...
    if (itr != targets.end()) {
      ...
      sizeSq *= sizeSq;
      DEBUG_TRAJ(LOG_ERROR("server", "Initial %f %f %f %f %f", x, y, z, distSq,
                           sizeSq);)
      if (distSq > sizeSq) {
        ...
        DEBUG_TRAJ(LOG_ERROR("server", "Initial %f %f %f %f %f", x, y, z,
                             distSq, sizeSq);)
      }
    }
...
void Spell::SelectEffectTypeImplicitTargets(uint8 effIndex) {
  ...
  // TODO: this is a workaround - target shouldn't be stored in target map for
  // those spells
  ...
  case EFFECT_IMPLICIT_TARGET_EXPLICIT:
    // player which not released his spirit is Unit, but target flag for it is
    // TARGET_FLAG_CORPSE_MASK
    if (targetMask & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK)) {
      if (Unit *unitTarget = m_targets.GetUnitTarget())
        ...
        if (Corpse *corpseTarget = m_targets.GetCorpseTarget()) {
          // TODO: this is a workaround - corpses should be added to spell
          // target map too, but we can't do that so we add owner instead
          if (Player *owner =
...
void Spell::SearchTargets(SEARCHER &searcher, uint32 containerMask,
                          Unit *referer, Position const *pos, float radius) {
  if (!containerMask)
    return;
  ...
  bool searchInWorld =
      containerMask & (GRID_MAP_TYPE_MASK_CREATURE | GRID_MAP_TYPE_MASK_PLAYER |
  ...
  if (searchInGrid || searchInWorld) {
    ...
    if (searchInWorld) {
      TypeContainerVisitor<SEARCHER, WorldTypeMapContainer>
          world_object_notifier(searcher);
      cell.Visit(p, world_object_notifier, map,
                 radius + SPELL_SEARCHER_COMPENSATION, x, y);
    }
    if (searchInGrid) {
      TypeContainerVisitor<SEARCHER, GridTypeMapContainer> grid_object_notifier(
          searcher);
      cell.Visit(p, grid_object_notifier, map,
                 radius + SPELL_SEARCHER_COMPENSATION, x, y);
    }
  }
}
...
WorldObject *Spell::SearchNearbyTarget(float range,
  ...
  if (!containerTypeMask)
    return nullptr;
  Warhead::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo,
                                                   selectionType, condList);
  Warhead::WorldObjectLastSearcher<Warhead::WorldObjectSpellNearbyTargetCheck>
      searcher(m_caster, target, check, containerTypeMask);
  SearchTargets<Warhead::WorldObjectLastSearcher<
      Warhead::WorldObjectSpellNearbyTargetCheck>>(searcher, containerTypeMask,
                                                   m_caster, m_caster, range);
  return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject *> &targets, float range,
  ...
  if (!containerTypeMask)
    return;
  Warhead::WorldObjectSpellAreaTargetCheck check(
      range, position, m_caster, referer, m_spellInfo, selectionType, condList);
  Warhead::WorldObjectListSearcher<Warhead::WorldObjectSpellAreaTargetCheck>
      searcher(m_caster, targets, check, containerTypeMask);
  SearchTargets<Warhead::WorldObjectListSearcher<
      Warhead::WorldObjectSpellAreaTargetCheck>>(searcher, containerTypeMask,
                                                 m_caster, position, range);
}
...
void Spell::SearchChainTargets(
    ...
    SpellTargetSelectionCategories /*selectCategory*/, ConditionList *condList,
    bool isChainHeal) {
  ...
  // chain lightning/heal spells and similar - allow to jump at larger distance
  // and go out of los
  bool isBouncingFar =
      (m_spellInfo->HasAttribute(SPELL_ATTR4_AREA_TARGET_CHAIN) ||
  ...
  SearchAreaTargets(tempTargets, searchRadius, target, m_caster, objectType,
                    selectType, condList);
      ...
      for (std::list<WorldObject *>::iterator itr = tempTargets.begin();
        ...
        if (foundItr == tempTargets.end()) {
          if ((!isBouncingFar || target->IsWithinDist(*itr, jumpRadius)) &&
              target->IsWithinLOSInMap(*itr, LINEOFSIGHT_ALL_CHECKS))
            foundItr = itr;
...
void Spell::AddUnitTarget(Unit *target, uint32 effectMask,
                          bool checkIfValid /*= true*/,
                          bool implicit /*= true*/) {
  ...
  if (m_originalCaster) {
    targetInfo.missCondition =
        m_originalCaster->SpellHitResult(target, m_spellInfo, m_canReflect);
  ...
  if (targetInfo.missCondition == SPELL_MISS_REFLECT) {
    ...
    targetInfo.reflectResult =
        m_caster->SpellHitResult(m_caster, m_spellInfo, m_canReflect);

    if (targetInfo.reflectResult ==
        SPELL_MISS_REFLECT) // Impossible reflect again, so simply deflect spell
      targetInfo.reflectResult = SPELL_MISS_PARRY;

    // Increase time interval for reflected spells by 1.5
    m_caster->m_Events.AddEvent(
        new ReflectEvent(m_caster->GetGUID(), targetInfo.targetGUID,
                         m_spellInfo),
...
void Spell::AddGOTarget(GameObject *go, uint32 effectMask) {
  ...
  if (m_spellInfo->Speed > 0.0f) {
    // calculate spell incoming interval
    float dist = m_caster->GetDistance(go->GetPositionX(), go->GetPositionY(),
                                       go->GetPositionZ());
    if (dist < 5.0f)
      dist = 5.0f;
...
void Spell::DoAllEffectOnTarget(TargetInfo *target) {
  ...
  if (!effectUnit || m_spellInfo->Id == 45927) {
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
      if (farMask & (1 << i))
        HandleEffects(effectUnit, nullptr, nullptr, i,
  ...
  if (getState() == SPELL_STATE_DELAYED && !m_spellInfo->IsTargetingArea() &&
      !m_spellInfo->IsPositive() &&
      (World::GetGameTimeMS() - target->timeDelay) <=
          effectUnit->m_lastSanctuaryTime &&
      World::GetGameTimeMS() < (effectUnit->m_lastSanctuaryTime + 500) &&
  ...
  // Or on missInfo != SPELL_MISS_NONE unitTarget undefined (but need in trigger
  // subsystem)
  ...
  bool canEffectTrigger =
      !m_spellInfo->HasAttribute(SPELL_ATTR3_CANT_TRIGGER_PROC) &&
      unitTarget->CanProc() &&
  ...
  if (missInfo ==
      SPELL_MISS_NONE) // In case spell hit target, do all effect on that target
    spellHitTarget = unitTarget;
  else if (missInfo == SPELL_MISS_REFLECT) // In case spell reflect from target,
                                           // do all effect on caster (if hit)
  {
    missInfo = target->reflectResult;
    if (missInfo == SPELL_MISS_NONE) // If reflected spell hit caster -> do all
                                     // effect on him
    {
  ...
  if (canEffectTrigger && !procAttacker && !procVictim) {
    ...
    if (m_damage > 0)
      ...
      for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        ...
        // Xinef: if missInfo is immune state check all effects to properly
        // determine positiveness of spell
  ...
  if (m_healing > 0) {
    ...
    if (crit) {
      ...
      addhealth = Unit::SpellCriticalHealingBonus(caster, m_spellInfo,
                                                  addhealth, nullptr);
    } else
    ...
    unitTarget->getHostileRefManager().threatAssist(caster, float(gain) * 0.5f,
                                                    m_spellInfo);
    m_healing = gain;
    ...
    // drop charge)
    if (canEffectTrigger)
      caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx,
                                 addhealth, m_attackType, m_spellInfo,
  ...
  // Do damage and triggers
    ...
    if (m_caster->GetEntry() == 27893) {
      if (Unit *owner = m_caster->GetOwner())
        owner->CalculateSpellDamageTaken(&damageInfo, m_damage, m_spellInfo,
    ...
    Unit::DealDamageMods(damageInfo.target, damageInfo.damage,
                         &damageInfo.absorb);
    ...
    if (canEffectTrigger) {
      caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx,
                                 damageInfo.damage, m_attackType, m_spellInfo,
                                 m_triggeredByAuraSpell);
      if (caster->GetTypeId() == TYPEID_PLAYER &&
        ...
        caster->ToPlayer()->CastItemCombatSpell(unitTarget, m_attackType,
                                                procVictim, procEx);
    }
  ...
  // Passive spell hits/misses or active spells only misses (only triggers)
    ...
    if (canEffectTrigger) {
      caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx,
                                 0, m_attackType, m_spellInfo,
                                 m_triggeredByAuraSpell);
      // Xinef: eg. rogue poisons can proc off cheap shot, etc. so this block
      // should be here also Xinef: ofc count only spells that HIT the target,
      // little hack used to fool the system
      if ((procEx & PROC_EX_NORMAL_HIT || procEx & PROC_EX_CRITICAL_HIT) &&
        ...
        caster->ToPlayer()->CastItemCombatSpell(
            unitTarget, m_attackType, procVictim | PROC_FLAG_TAKEN_DAMAGE,
            procEx);
    }
  ...
  if (m_caster) {
    if (missInfo != SPELL_MISS_EVADE && !m_caster->IsFriendlyTo(effectUnit) &&
        (!m_spellInfo->IsPositive() ||
         m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL))) {
      m_caster->CombatStart(effectUnit, !(m_spellInfo->AttributesEx3 &
                                          SPELL_ATTR3_NO_INITIAL_AGGRO));

      // Unsure if there are more spells that are not supposed to stop enemy
      // from regenerating HP from food, so for now it stays as an ID.
  ...
  if (spellHitTarget) {
    ...
    // Needs to be called after dealing damage/healing to not remove breaking on
    // damage auras
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask,
  ...
  if (m_spellInfo->Speed &&
      ((m_damage > 0 && unit->IsImmunedToDamage(m_spellInfo)) ||
       unit->IsImmunedToSchool(m_spellInfo) ||
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
    ...
    if (effectMask & (1 << effectNumber)) {
      ...
      // resistance twice? Xinef: And why we check every spell effect basing on
      // rand and generic dispel info? some effects will be appliend and some
      // wont? WTF?
  ...
  if (unit->GetTypeId() == TYPEID_PLAYER) {
    unit->ToPlayer()->StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_SPELL_TARGET,
                                            m_spellInfo->Id);
    unit->ToPlayer()->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, m_spellInfo->Id, 0,
        m_caster);
    unit->ToPlayer()->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2, m_spellInfo->Id, 0,
        m_caster);
  }
  ...
  if (m_caster != unit) {
    ...
    if (m_caster->_IsValidAttackTarget(unit, m_spellInfo) &&
      ...
      // for delayed spells ignore negative spells (after duel end) for friendly
      // targets
  ...
  m_diminishGroup =
      GetDiminishingReturnsGroupForSpell(m_spellInfo, m_triggeredByAuraSpell);
    ...
    if ((type == DRTYPE_PLAYER &&
         (unit->GetCharmerOrOwnerPlayerOrPlayerItself() ||
          ...
          (m_diminishGroup == DIMINISHING_TAUNT &&
           (flagsExtra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH)))) ||
        type == DRTYPE_ALL)
  ...
  if (aura_effmask) {
    ...
    // Unit has to be target only of aura effect, both caster and target have to
    // be players, target has to be other than unit target
    ...
    if (m_originalCaster) {
      ...
      m_spellAura = Aura::TryRefreshStackOrCreate(
          aurSpellInfo, effectMask, unit, m_originalCaster,
          (aurSpellInfo == m_spellInfo) ? &m_spellValue->EffectBasePoints[0]
                                        : &basePoints[0],
          m_CastItem, 0, &refresh,
      ...
      if (m_spellAura) {
        ...
        int32 limitduration =
            GetDiminishingReturnsLimitDuration(m_diminishGroup, aurSpellInfo);
        ...
        if (unit == m_caster && m_targets.GetUnitTarget())
          diminishMod = m_targets.GetUnitTarget()->ApplyDiminishingToDuration(
              m_diminishGroup, duration, m_originalCaster, m_diminishLevel,
              limitduration);
        else
        ...
        if (diminishMod == 0.0f) {
          ...
          for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (effectMask & (1 << i) &&
                m_spellInfo->Effects[i].Effect != SPELL_EFFECT_APPLY_AURA)
              found = true;
          if (!found)
            return SPELL_MISS_IMMUNE;
        } else {
          ...
          if (AuraApplication *aurApp = m_spellAura->GetApplicationOfTarget(
          ...
          if (m_originalCaster->HasAuraTypeWithAffectMask(
            ...
            duration = int32(duration * m_originalCaster->GetFloatValue(
                                            UNIT_MOD_CAST_SPEED));
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
    ...
    if (effectMask & (1 << effectNumber))
      HandleEffects(unit, nullptr, nullptr, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
  }

  if (sanct_effect >= 0 && (effectMask & (1 << sanct_effect)))
    HandleEffects(unit, nullptr, nullptr, sanct_effect,
                  SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoTriggersOnSpellHit(Unit *unit, uint8 effMask) {
  ...
  if (!m_hitTriggerSpells.empty()) {
    ...
    for (HitTriggerSpellList::const_iterator i = m_hitTriggerSpells.begin();
      ...
      if (CanExecuteTriggersOnHit(effMask, i->triggeredByAura) &&
        ...
        // SPELL_AURA_ADD_TARGET_TRIGGER auras shouldn't trigger auras without
        // duration set duration of current aura to the triggered spell
        if (i->triggeredSpell->GetDuration() == -1) {
          if (Aura *triggeredAur =
                  unit->GetAura(i->triggeredSpell->Id, m_caster->GetGUID())) {
  ...
  // TODO: remove/cleanup this, as this table is not documented and people are
...
void Spell::DoAllEffectOnTarget(GOTargetInfo *target) {
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
       ++effectNumber)
    if (effectMask & (1 << effectNumber))
      HandleEffects(nullptr, nullptr, go, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoAllEffectOnTarget(ItemTargetInfo *target) {
  ...
  for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS;
       ++effectNumber)
    if (effectMask & (1 << effectNumber))
      HandleEffects(nullptr, target->item, nullptr, effectNumber,
                    SPELL_EFFECT_HANDLE_HIT_TARGET);
...
bool Spell::UpdateChanneledTargetList() {
  ...
  for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
    ...
    if (ihit->missCondition == SPELL_MISS_NONE &&
      ...
      Unit *unit = m_caster->GetGUID() == ihit->targetGUID
                       ? m_caster
      ...
      if (IsValidDeadOrAliveTarget(unit)) {
        if (channelAuraMask & ihit->effectMask) {
          if (AuraApplication *aurApp = unit->GetAuraApplication(
            ...
            if (m_caster != unit) {
              ...
              // Xinef: Update Orientation server side (non players wont sent
              // appropriate packets)
        ...
        channelTargetEffectMask &=
            ~ihit->effectMask; // remove from need alive mask effect that have
                               // alive target
      }
    }
  }
...
SpellCastResult Spell::prepare(SpellCastTargets const *targets,
  ...
  if (m_caster->IsTotem() ||
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_APPLY_AURA ||
        ...
        // Change aura with ranks only if basepoints are taken from spellInfo
        // and aura is positive
  ...
  if (DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id,
                                m_caster)) {
  ...
  if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CAST_IN_PROGRESS) &&
      m_caster->IsNonMeleeSpellCast(false, true, true, m_spellInfo->Id == 75) &&
      m_cast_count) {
      ...
      !IsAutoRepeat()) // always cast autorepeat dummy for triggering
  {
    ...
    // can't be cast for example bladestorm aura should be removed on disarm as
    // of patch 3.3.5 channeled periodic spells should be affected by this
    // (arcane missiles, penance, etc) a possible alternative sollution for
    // those would be validating aura target on unit state change
  ...
  m_casttime = (_triggeredCastFlags & TRIGGERED_CAST_DIRECTLY)
                   ? 0
  ...
  if (m_CastItem) {
    ...
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i) {
      ...
      // xinef: by default set it to false, and to true if any valid target is
      // found
      selectTargets = true;
    }
    ...
    if (selectTargets) {
      ...
      if (m_UniqueTargetInfo.empty() && m_UniqueGOTargetInfo.empty()) {
        // no valid nearby target unit or game object found; check if nearby
        // destination type
        if (nearbyDest) {
          if (!m_targets.HasDst()) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura",
            "Spell::prepare: spell id %u source %u caster %d customCastFlags "
            "%u mask %u",
  ...
  if ((_triggeredCastFlags & TRIGGERED_CAST_DIRECTLY) &&
    ...
    if ((m_casttime || m_spellInfo->IsChanneled()) &&
      ...
      if (m_caster->GetTypeId() == TYPEID_UNIT &&
          ...
          ((m_targets.GetObjectTarget() &&
           ...
           m_spellInfo->IsPositive())) {
        // Xinef: Creature should focus to cast target if there is explicit
        // target or self if casting positive spell Xinef: Creature should not
        // rotate like a retard when casting spell... based on halion behavior
        m_caster->ToCreature()->FocusTarget(
            this, m_targets.GetObjectTarget() != nullptr
                      ? m_targets.GetObjectTarget()
                      : m_caster);
      }
    }
    ...
    // database
    if (!m_casttime &&
        /*!m_castItemGUID &&*/ GetCurrentContainer() == CURRENT_GENERIC_SPELL)
      cast(true);
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
      if (m_caster->ToPlayer()->NeedSendSpectatorData())
        ArenaSpectator::SendCommand_Spell(
            m_caster->FindMap(), m_caster->GetGUID(), "SPE", m_spellInfo->Id,
            bySelf ? 99998 : 99999);
    }
      ...
      if ((*ihit).missCondition == SPELL_MISS_NONE)
        if (Unit *unit =
                m_caster->GetGUID() == ihit->targetGUID
                    ? m_caster
                    : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID))
          unit->RemoveOwnedAura(m_spellInfo->Id, m_originalCasterGUID, 0,
...
void Spell::_cast(bool skipCheck) {
  ...
  // Xinef: implement attribute SPELL_ATTR1_DISMISS_PET, on spell cast current
  // pet is dismissed and charms are removed
  ...
  if (Player *playerCaster = m_caster->ToPlayer()) {
    ...
    // Let any pets know we've attacked something. Check DmgClass for harmful
    // spells only This prevents spells such as Hunter's Mark from triggering
    // pet attack xinef: take into account SPELL_ATTR3_NO_INITIAL_AGGRO
    if ((m_targets.GetTargetMask() & TARGET_FLAG_UNIT) &&
        GetSpellInfo()->DmgClass != SPELL_DAMAGE_CLASS_NONE &&
        !GetSpellInfo()->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO))
      if (!playerCaster->m_Controlled.empty())
        for (Unit::ControlSet::iterator itr =
                 playerCaster->m_Controlled.begin();
             itr != playerCaster->m_Controlled.end(); ++itr)
          if (Unit *pet = *itr)
  ...
  if (!(_triggeredCastFlags & TRIGGERED_IGNORE_SET_FACING))
    if (m_caster->GetTypeId() == TYPEID_UNIT && m_targets.GetObjectTarget() &&
        m_caster != m_targets.GetObjectTarget())
      ...
      if (m_caster->GetTypeId() == TYPEID_PLAYER) {
        if (TradeData *my_trade = m_caster->ToPlayer()->GetTradeData()) {
          if (!my_trade->IsInAcceptProcess()) {
            // Spell will be casted at completing the trade. Silently ignore at
            // this place
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER) {
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CAST_ITEM) && m_CastItem) {
      m_caster->ToPlayer()->StartTimedAchievement(ACHIEVEMENT_TIMED_TYPE_ITEM,
                                                  m_CastItem->GetEntry());
      m_caster->ToPlayer()->UpdateAchievementCriteria(
    ...
    m_caster->ToPlayer()->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_CAST_SPELL, m_spellInfo->Id, 0,
  ...
  if ((m_spellInfo->Speed > 0.0f && !m_spellInfo->IsChanneled())/* xinef: we dont need this shit || m_spellInfo->Id == 14157*/)
    ...
    // TakeReagents call in case delayed spell remove item at cast delay start
    TakeCastItem();
    ...
    // Xinef: Fucking retarded idiots, why do we keep focus after spell is sent
    // to air? Xinef: Because of this, in the middle of some animation after
    // setting targetguid to 0 etc Xinef: we get focused to it out of nowhere,
    // idiots...
  ...
  // handle this here, in other places SpellHitTarget can be set to nullptr, if
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER ||
      ...
      if (GetDelayMoment() > 0 && !m_caster->IsFriendlyTo(target) &&
        ...
        m_caster->CombatStartOnCast(
            target, !m_spellInfo->HasAttribute(SPELL_ATTR3_NO_INITIAL_AGGRO),
            GetDelayMoment() + 500); // xinef: increase this time so we dont
                                     // leave and enter combat in a moment
...
void Spell::handle_immediate() {
  ...
  if (m_spellInfo->IsChanneled()) {
    ...
    if (duration > 0) {
      ...
      if (m_caster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE,
          ...
          m_spellInfo->HasAttribute(SPELL_ATTR5_HASTE_AFFECT_DURATION))
        duration =
            int32(duration * m_caster->GetFloatValue(UNIT_MOD_CAST_SPEED));
  ...
  if (m_spellState != SPELL_STATE_CASTING)
    finish(true); // successfully finish spell cast (not last in case autorepeat
                  // or channel spell)
}
...
uint64 Spell::handle_delayed(uint64 t_offset) {
  ...
  // prevent adding immunity at first effect not allow apply second spell effect
  // and similar cases)
  for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
       ihit != m_UniqueTargetInfo.end(); ++ihit) {
...
void Spell::_handle_finish_phase() {
  if (m_caster->m_movedByPlayer && m_needComboPoints) {
    ...
    if (m_targets.GetUnitTarget() && m_targets.GetUnitTarget()->IsInWorld() &&
        m_targets.GetUnitTarget()->IsAlive())
      m_caster->m_movedByPlayer->ToPlayer()->AddComboPoints(
          m_targets.GetUnitTarget(),
          m_caster->m_movedByPlayer->ToPlayer()->GetComboPointGain());

    m_caster->m_movedByPlayer->ToPlayer()->SetComboPointGain(0);
  }

  if (m_caster->m_extraAttacks &&
      GetSpellInfo()->HasEffect(SPELL_EFFECT_ADD_EXTRA_ATTACKS))
...
void Spell::SendSpellCooldown() {
  ...
  // have infinity cooldown but set at aura apply                  // do not set
  // cooldown for triggered spells (needed by reincarnation)
...
void Spell::update(uint32 difftime) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura", "Spell %u is cancelled due to removal of target.",
              m_spellInfo->Id);
#endif
    cancel();
    return;
  ...
  case SPELL_STATE_CASTING: {
    ...
    // diffrent update routines for players / creatures Xinef: If creature gets
    // new aura in 800ms and in 840ms its updated with diff 270, not 40 is
    // removed from duration but 270 Xinef: so the aura can be removed in
...
void Spell::finish(bool ok) {
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_ENCOUNTER_REWARD) &&
      m_caster->FindMap())
    m_caster->FindMap()->UpdateEncounterState(ENCOUNTER_CREDIT_CAST_SPELL,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("spells.aura", "Statue %d is unsummoned in spell %d finish",
  ...
  if (IsAutoActionResetSpell()) {
    ...
    if (!found &&
        !m_spellInfo->HasAttribute(SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS)) {
      bool allow = true;
...
void Spell::WriteCastResultInfo(WorldPacket &data, Player *caster,
                                SpellInfo const *spellInfo, uint8 castCount,
...
void Spell::SendCastResult(
    Player *caster, SpellInfo const *spellInfo, uint8 castCount,
    SpellCastResult result,
...
void Spell::SendCastResult(SpellCastResult result) {
  ...
  // Xinef: override every possible result, except for gm fail result... breaks
  // many things and goes unnoticed because of this and makes me rage when i
  // find this out
...
void Spell::SendSpellStart() {
  ...
  // LOG_DEBUG("spells.aura", "Sending SMSG_SPELL_START id=%u",
  // m_spellInfo->Id);
  ...
  if (!m_spellInfo->IsChanneled() && m_caster->GetTypeId() == TYPEID_PLAYER &&
      m_caster->ToPlayer()->NeedSendSpectatorData())
    ArenaSpectator::SendCommand_Spell(m_caster->FindMap(), m_caster->GetGUID(),
                                      "SPE", m_spellInfo->Id, m_timer);
}
...
void Spell::SendSpellGo() {
  ...
  if ((m_caster->GetTypeId() == TYPEID_PLAYER ||
       (m_caster->GetTypeId() == TYPEID_UNIT && m_caster->IsPet())) &&
      m_spellInfo->PowerType != POWER_HEALTH)
    castFlags |= CAST_FLAG_POWER_LEFT_SELF; // should only be sent to self, but
                                            // the current messaging doesn't
                                            // make that possible

  if ((m_caster->GetTypeId() == TYPEID_PLAYER) &&
      (m_caster->getClass() == CLASS_DEATH_KNIGHT) && m_spellInfo->RuneCostID &&
  ...
  {
    // TODO: There is a crash caused by a spell with CAST_FLAG_RUNE_LIST casted
    // by a creature The creature is the mover of a player, so
    ...
    if (Player *player = m_caster->ToPlayer()) {
      ...
      for (uint8 i = 0; i < MAX_RUNES; ++i) {
        ...
        if (mask & runeMaskInitial &&
            !(mask & runeMaskAfterCast)) // usable before andon cooldown now...
        {
          // float casts ensure the division is performed on floats as we need
          // float result
          float baseCd = float(player->GetRuneBaseCooldown(i, true));
          data << uint8((baseCd - float(player->GetRuneCooldown(i))) / baseCd *
                        255); // rune cooldown passed
        }
      }
    }
  }
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    for (uint8 i = 0; i < 3; ++i) {
      if (uint32 item_id =
        ...
        if (ItemTemplate const *itemEntry =
                sObjectMgr->GetItemTemplate(item_id)) {
...
void Spell::WriteSpellGoTargets(WorldPacket *data) {
  ...
  // for each sending more than 255 targets crashes the client (since count sent
  // would be wrong) Spells like 40647 (with a huge radius) can easily reach
  // this limit (spell might need target conditions but we still need to limit
  // the number of targets sent and keeping correct count for both hit and
  // miss).
  ...
  *data << (uint8)0; // placeholder
  for (std::list<TargetInfo>::const_iterator ihit = m_UniqueTargetInfo.begin();
       ihit != m_UniqueTargetInfo.end() && hit < 255; ++ihit) {
  ...
  for (std::list<GOTargetInfo>::const_iterator ighit =
       ...
       ighit != m_UniqueGOTargetInfo.end() && hit < 255; ++ighit) {
  ...
  *data << (uint8)0; // placeholder
  for (std::list<TargetInfo>::const_iterator ihit = m_UniqueTargetInfo.begin();
       ihit != m_UniqueTargetInfo.end() && miss < 255; ++ihit) {
...
void Spell::ExecuteLogEffectTakeTargetPower(uint8 effIndex, Unit *target,
                                            uint32 PowerType, uint32 powerTaken,
...
void Spell::TakeCastItem() {
  ...
  if (!proto) {
    ...
    // I'm not sure, if this is really an error, but I guess every item needs a
    // prototype
    LOG_ERROR("server", "Cast item has no item prototype highId=%d, lowId=%d",
  ...
  for (int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i) {
    if (proto->Spells[i].SpellId) {
      ...
      if (proto->Spells[i].SpellCharges) {
        ...
        if (charges) {
          (charges > 0) ? --charges
                        : ++charges; // abs(charges) less at 1 after use
...
void Spell::TakeRunePower(bool didHit) {
  ...
  for (uint32 i = 0; i < MAX_RUNES; ++i) {
    ...
    if (!player->GetRuneCooldown(i) && runeCost[rune] > 0) {
      player->SetRuneCooldown(i, didHit ? player->GetRuneBaseCooldown(i, false)
    ...
    if (runeCost[RUNE_DEATH] > 0) {
      for (uint8 i = 0; i < MAX_RUNES; ++i) {
        ...
        if (!player->GetRuneCooldown(i) && rune == RUNE_DEATH &&
          ...
          player->SetRuneCooldown(i, didHit
...
void Spell::HandleThreatSpells() {
  ...
  for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
    ...
    // positive spells distribute threat among all units that are in combat with
    // target, like healing
    if (m_spellInfo->_IsPositiveSpell() && IsFriendly)
      target->getHostileRefManager().threatAssist(m_caster, threatToAdd,
                                                  m_spellInfo);
    // for negative spells threat gets distributed among affected targets
    else if (!m_spellInfo->_IsPositiveSpell() && !IsFriendly &&
             target->CanHaveThreatList())
      target->AddThreat(m_caster, threatToAdd, m_spellInfo->GetSchoolMask(),
                        m_spellInfo);
  }
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura",
            "Spell %u, added an additional %f threat for %s %u target(s)",
            m_spellInfo->Id, threat,
...
SpellCastResult Spell::CheckCast(bool strict) {
  ...
  // Spectator check
  if (m_caster->GetTypeId() == TYPEID_PLAYER)
    if (((const Player *)m_caster)->IsSpectator() &&
  ...
  if (!m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE)) {
    if (m_caster->GetTypeId() == TYPEID_PLAYER) {
      ...
      // check if we are using a potion in combat for the 2nd+ time. Cooldown is
      // added only after caster gets out of combat
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR7_IS_CHEAT_SPELL) &&
      !m_caster->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_ALLOW_CHEAT_SPELLS)) {
    m_customError = SPELL_CUSTOM_ERROR_GM_ONLY;
    return SPELL_FAILED_CUSTOM_ERROR;
  }

  // Check global cooldown
  if (strict && !(_triggeredCastFlags & TRIGGERED_IGNORE_GCD) &&
      HasGlobalCooldown())
    return SPELL_FAILED_NOT_READY;
  ...
  for (Unit::AuraEffectList::const_iterator blockItr = blockSpells.begin();
       blockItr != blockSpells.end(); ++blockItr)
  ...
  if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURASTATE)) {
    ...
    if (m_spellInfo->CasterAuraStateNot &&
        m_caster->HasAuraState(AuraStateType(m_spellInfo->CasterAuraStateNot),
                               m_spellInfo, m_caster))
      return SPELL_FAILED_CASTER_AURASTATE;
  ...
  // (not wand currently autorepeat cast delayed to moving stop anyway in spell
  // update code)
  if (m_caster->GetTypeId() == TYPEID_PLAYER &&
  ...
  if (vehicle &&
      !(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE)) {
    uint16 checkMask = 0;
    ...
    // All creatures should be able to cast as passengers freely, restriction
    // and attribute are only for players
    VehicleSeatEntry const *vehicleSeat =
        vehicle->GetSeatForPassenger(m_caster);
    if (!m_spellInfo->HasAttribute(SPELL_ATTR6_CASTABLE_WHILE_ON_VEHICLE) &&
  ...
  // check spell cast conditions from database
  {
    ...
    if (!conditions.empty() &&
      ...
      // mLastFailedCondition can be nullptr if there was an error processing
      // the condition in Condition::Meets (i.e. wrong data for ConditionTarget
      // or others)
      if (condInfo.mLastFailedCondition &&
        ...
        if (condInfo.mLastFailedCondition->ErrorType ==
          ...
          m_customError =
              SpellCustomErrors(condInfo.mLastFailedCondition->ErrorTextId);
  ...
  // are not targeting anyone using targeting system, they should apply directly
  // to caster instead also, such casts shouldn't be sent to client Xinef: do
  // not check explicit casts for self cast of triggered spells (eg. reflect
  // case)
  if (!(m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE) &&
        (!m_targets.GetUnitTarget() ||
    ...
    if (!m_triggeredByAuraSpell || m_targets.GetUnitTarget() != m_caster ||
      ...
      SpellCastResult castResult = m_spellInfo->CheckExplicitTarget(
          (m_originalCaster && m_caster->GetEntry() != WORLD_TRIGGER)
              ? m_originalCaster
              : m_caster,
          m_targets.GetObjectTarget(), m_targets.GetItemTarget());
      if (castResult != SPELL_CAST_OK)
        return castResult;
    }
  }
  ...
  if (Unit *target = m_targets.GetUnitTarget()) {
    ...
    if (target != m_caster) {
      ...
      if (m_caster->GetEntry() !=
          WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly casted by
                         // a trigger)
        if ((!m_caster->IsTotem() || !m_spellInfo->IsPositive()) &&
            ...
            !m_caster->IsWithinLOSInMap(target, LINEOFSIGHT_ALL_CHECKS) &&
            !(m_spellFlags & SPELL_FLAG_REDIRECTED))
  ...
  for (int j = 0; j < MAX_SPELL_EFFECTS; ++j) {
    if (m_spellInfo->Effects[j].TargetA.GetTarget() == TARGET_UNIT_PET) {
      if (!m_caster->GetGuardianPet() && !m_caster->GetCharm()) {
        if (m_triggeredByAuraSpell) // not report pet not existence for
                                    // triggered spells
          return SPELL_FAILED_DONT_REPORT;
        else
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR3_BATTLEGROUND) &&
      m_caster->GetTypeId() == TYPEID_PLAYER)
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR4_NOT_USABLE_IN_ARENA) ||
      (m_spellInfo->GetRecoveryTime() >= 10 * MINUTE * IN_MILLISECONDS &&
       !m_spellInfo->HasAttribute(SPELL_ATTR4_USABLE_IN_ARENA)))
  ...
  if (!m_spellInfo->IsPassive()) {
    // spell focus needs to be checked not only for players! there are vehicle
    // spells that require spell focus
  ...
  if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURAS) ||
      (m_spellInfo->PreventionType > SPELL_PREVENTION_TYPE_NONE &&
       m_triggeredByAuraSpell && m_triggeredByAuraSpell->IsPositive())) {
    castResult =
        CheckCasterAuras(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURAS);
    if (castResult != SPELL_CAST_OK)
      return castResult;

    // xinef: Enraged Regeneration: While this is active, the warrior is blocked
    // from using abilities that trigger being enraged (which would do nothing
    // and waste the cooldowns).
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    case SPELL_EFFECT_POWER_DRAIN: {
      // Can be area effect, Check only for players and not check if target -
      // caster (spell can have multiply drain/burn effects)
      if (m_caster->GetTypeId() == TYPEID_PLAYER)
        if (Unit *target = m_targets.GetUnitTarget())
          if (target != m_caster &&
              target->getPowerType() !=
    ...
    case SPELL_EFFECT_CHARGE: {
      if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR) {
        // Warbringer - can't be handled in proc system - should be done before
        // checkcast root check and charge effect process
      ...
      if (MMAP::MMapFactory::IsPathfindingEnabled(m_caster->FindMap()) &&
        ...
        Position pos;
        target->GetChargeContactPoint(m_caster, pos.m_positionX,
        ...
        if (m_caster->GetMapId() == 618) // pussywizard: 618 Ring of Valor
        {
          if (!((target->GetPositionZ() > 32.0f) ^
                (m_caster->GetPositionZ() > 32.0f)))
            break;
          return SPELL_FAILED_NOPATH;
        } else if (m_caster->GetMapId() ==
                   572) // pussywizard: 572 Ruins of Lordaeron
        {
          if (pos.GetPositionX() < 1275.0f ||
              m_caster->GetPositionX() < 1275.0f) // special case (acid)
            break; // can't force path because the way is around and the path is
                   // too long
        }
        ...
        m_pathFinder->CalculatePath(pos.m_positionX, pos.m_positionY,
                                    pos.m_positionZ + 0.15f, false);
        G3D::Vector3 endPos =
            m_pathFinder
                ->GetEndPosition(); // also check distance between target and
                                    // the point calculated by mmaps
        if (m_pathFinder->GetPathType() &
                (PATHFIND_NOPATH | PATHFIND_INCOMPLETE) ||
            target->GetExactDistSq(endPos.x, endPos.y, endPos.z) >
                maxdist * maxdist ||
            m_pathFinder->getPathLength() >
                (40.0f + (m_caster->HasAura(58097) ? 5.0f : 0.0f)))
          return SPELL_FAILED_NOPATH;
      }
      break;
    }
    case SPELL_EFFECT_SKINNING: {
      ...
      if (!(m_targets.GetUnitTarget()->GetUInt32Value(UNIT_FIELD_FLAGS) &
            UNIT_FLAG_SKINNABLE))
    ...
    case SPELL_EFFECT_OPEN_LOCK: {
      ...
      if (m_targets.GetTargetMask() & TARGET_FLAG_TRADE_ITEM) {
        if (TradeData *pTrade = m_caster->ToPlayer()->GetTradeData())
          pTempItem = pTrade->GetTraderData()->GetItem(
              TradeSlots(m_targets.GetItemTargetGUID()));
      } else if (m_targets.GetTargetMask() & TARGET_FLAG_ITEM)
        pTempItem =
            m_caster->ToPlayer()->GetItemByGuid(m_targets.GetItemTargetGUID());
        ...
        if (m_targets.GetGOTarget() &&
          ...
          if (!m_caster->ToPlayer()->CanUseBattlegroundObject(nullptr))
            return SPELL_FAILED_TRY_AGAIN;
                       ...
                       ->InBattleground() && // In Battleground players can use
                                             // only flags and banners, or
                                             // Gurubashi chest
    ...
    case SPELL_EFFECT_SUMMON: {
      ...
      if (!SummonProperties ||
          m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET))
        break;
    ...
    case SPELL_EFFECT_SUMMON_PET: {
      ...
      if (m_caster->GetTypeId() == TYPEID_PLAYER &&
          m_caster->getClass() == CLASS_WARLOCK && strict)
        if (Pet *pet = m_caster->ToPlayer()->GetPet())
          pet->CastSpell(
              pet, 32752, true, nullptr, nullptr,
              pet->GetGUID()); // starting cast, trigger pet stun (cast by pet
                               // so it doesn't attack player)
      break;
    }
    case SPELL_EFFECT_SUMMON_PLAYER: {
      if (m_caster->GetTypeId() != TYPEID_PLAYER)
      ...
      if (map->IsDungeon()) {
        ...
        /*if (map->IsRaid())
           ...
           difficulty)) if (targetBind->perm && targetBind !=
           m_caster->ToPlayer()->GetBoundInstance(mapId, difficulty)) return
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    case SPELL_AURA_AOE_CHARM: {
      ...
      // Xinef: allow SPELL_AURA_MOD_POSSESS to posses target if caster has some
      // pet
    ...
    case SPELL_AURA_MOUNTED: {
      // Xinef: disallow casting in water for mounts not increasing water
      // movement Speed
      if (m_caster->IsInWater() &&
          !m_spellInfo->HasAura(SPELL_AURA_MOD_INCREASE_SWIM_SPEED))
        return SPELL_FAILED_ONLY_ABOVEWATER;

      // Ignore map check if spell have AreaId. AreaId already checked and this
      // prevent special mount spells
      ...
      if (m_caster->getTransForm())
        if (SpellInfo const *transformSpellInfo =
          ...
          if (transformSpellInfo->HasAttribute(
              ...
              !transformSpellInfo->HasAttribute(SpellAttr0(
                  SPELL_ATTR0_CASTABLE_WHILE_MOUNTED | SPELL_ATTR0_NEGATIVE_1)))
            return SPELL_FAILED_NOT_SHAPESHIFT;
    ...
    case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED: {
      ...
      // not allow cast fly spells if not have req. skills  (all spells is self
      // target) allow always ghost flight spells
      if (m_originalCaster && m_originalCaster->GetTypeId() == TYPEID_PLAYER &&
          m_originalCaster->IsAlive()) {
        Battlefield *Bf = sBattlefieldMgr->GetBattlefieldToZoneId(
            m_originalCaster->GetZoneId());
        if (AreaTableEntry const *pArea =
                sAreaTableStore.LookupEntry(m_originalCaster->GetAreaId()))
  ...
  // check if caster has at least 1 combo point for spells that require combo
  // points
...
SpellCastResult Spell::CheckPetCast(Unit *target) {
  if (m_caster->HasUnitState(UNIT_STATE_CASTING) &&
      !(_triggeredCastFlags &
        TRIGGERED_IGNORE_CAST_IN_PROGRESS)) // prevent spellcast interruption by
                                            // another spellcast
    return SPELL_FAILED_SPELL_IN_PROGRESS;
  ...
  // properly and dont return shit results
...
SpellCastResult Spell::CheckCasterAuras(bool preventionOnly) const {
  ...
  if (m_spellInfo->HasAttribute(SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY)) {
    ...
    //         trinket               Bullheaded                  Bestial Wrath
    //         // Beath Within         // Medalion of Immunity
  ...
  bool usableInStun =
      m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_STUNNED);
  ...
  if (!preventionOnly) {
    if (unitflag & UNIT_FLAG_STUNNED) {
      // spell is usable while stunned, check if caster has only mechanic stun
      // auras, another stun types must prevent cast spell
      if (usableInStun) {
        ...
        if (m_spellInfo->Id == 22812)
          mask |=
              1 << MECHANIC_SAPPED | 1 << MECHANIC_HORROR | 1 << MECHANIC_SLEEP;
        ...
        prevented_reason = SPELL_FAILED_STUNNED;
               ...
               !m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_CONFUSED))
      prevented_reason = SPELL_FAILED_CONFUSED;
    else if (unitflag & UNIT_FLAG_FLEEING &&
             !m_spellInfo->HasAttribute(SPELL_ATTR5_USABLE_WHILE_FEARED))
  ...
  if (prevented_reason != SPELL_CAST_OK) {
    if (school_immune || mechanic_immune || dispel_immune) {
      // Checking auras is needed now, because you are prevented by some state
      // but the spell grants immunity.
      ...
      for (Unit::AuraApplicationMap::const_iterator itr = auras.begin();
        ...
        if (auraInfo->GetSchoolMask() & school_immune &&
            !auraInfo->HasAttribute(SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE))
          continue;
        if (auraInfo->GetDispelMask() & dispel_immune)
          continue;

        // Make a second check for spell failed so the right SPELL_FAILED
        // message is returned. That is needed when your casting is prevented by
        // multiple states and you are only immune to some of them.
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
...
bool Spell::CanAutoCast(Unit *target) {
  ...
  if (result == SPELL_CAST_OK || result == SPELL_FAILED_UNIT_NOT_INFRONT) {
    SelectSpellTargets();
    // check if among target units, our WANTED target is as well (->only self
    // cast spells return false)
    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
         ihit != m_UniqueTargetInfo.end(); ++ihit)
      if (ihit->targetGUID == targetguid)
        return true;
...
SpellCastResult Spell::CheckRange(bool strict) {
  ...
  if (m_spellInfo->RangeEntry) {
    // check needed by 68766 51693 - both spells are cast on enemies and have 0
    // max range these are triggered by other spells - possibly we should omit
  ...
  if (min_range && GetCaster()->GetTypeId() == TYPEID_UNIT &&
      !IS_PLAYER_GUID(GetCaster()->GetOwnerGUID()) && min_range <= 6.0f)
  ...
  if (target) {
    if (target != m_caster) {
      // Xinef: WHAT DA FUCK IS THIS SHIT? Spells with 5yd range can hit target
      // 9yd away? >.>
      if (range_type == SPELL_RANGE_MELEE) {
        float real_max_range = max_range;
        if (m_caster->GetTypeId() != TYPEID_UNIT && m_caster->isMoving() &&
            target->isMoving() && !m_caster->IsWalking() &&
            !target->IsWalking())
          real_max_range -=
              MIN_MELEE_REACH; // Because of lag, we can not check too strictly
                               // here (is only used if both caster and target
                               // are moving)
        else
          real_max_range -= 2 * MIN_MELEE_REACH;

        if (!m_caster->IsWithinMeleeRange(target,
                                          std::max(real_max_range, 0.0f)))
...
SpellCastResult Spell::CheckPower() {
  ...
  if (m_spellInfo->PowerType >= MAX_POWERS) {
    LOG_ERROR("server", "Spell::CheckPower: Unknown power type '%d'",
...
SpellCastResult Spell::CheckItems() {
  ...
  if (!m_CastItem) {
    ...
    if (proto->Class == ITEM_CLASS_CONSUMABLE && m_targets.GetUnitTarget()) {
      // such items should only fail if there is no suitable effect at all - see
      // Rejuvenation Potions for example
      SpellCastResult failReason = SPELL_CAST_OK;
      for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++) {
        // skip check, pet not required like checks, and for TARGET_UNIT_PET
        // m_targets.GetUnitTarget() is not the real target but the caster
        if (m_spellInfo->Effects[i].TargetA.GetTarget() == TARGET_UNIT_PET)
          continue;
  ...
  for (int i = 0; i < MAX_SPELL_EFFECTS; i++) {
    ...
    case SPELL_EFFECT_CREATE_ITEM_2: {
      // m_targets.GetUnitTarget() means explicit cast, otherwise we dont check
      // for possible equip error
      Unit *target =
          m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : player;
      if (target->GetTypeId() == TYPEID_PLAYER && !IsTriggered()) {
        // SPELL_EFFECT_CREATE_ITEM_2 differs from SPELL_EFFECT_CREATE_ITEM in
        // that it picks the random item to create from a pool of potential
        // items, so we need to make sure there is at least one free space in
        ...
        if (m_spellInfo->Effects[i].ItemType) {
          ...
          uint32 createCount =
              std::clamp<uint32>(m_spellInfo->Effects[i].CalcValue(), 1u,
                                 itemTemplate->GetMaxStackSize());
          ItemPosCountVec dest;
          InventoryResult msg = target->ToPlayer()->CanStoreNewItem(
              NULL_BAG, NULL_SLOT, dest, m_spellInfo->Effects[i].ItemType,
              createCount);
          if (msg != EQUIP_ERR_OK) {
            /// @todo Needs review
    ...
    case SPELL_EFFECT_ENCHANT_ITEM:
      if (m_spellInfo->Effects[i].ItemType && m_targets.GetItemTarget() &&
        ...
        // do not allow to enchant vellum from scroll made by vellum-prevent
        // exploit
        if (m_CastItem &&
            m_CastItem->GetTemplate()->Flags & ITEM_FLAG_NO_REAGENT_COST)
        ...
        InventoryResult msg = player->CanStoreNewItem(
            NULL_BAG, NULL_SLOT, dest, m_spellInfo->Effects[i].ItemType, 1);
        if (msg != EQUIP_ERR_OK) {
          player->SendEquipError(msg, nullptr, nullptr,
                                 m_spellInfo->Effects[i].ItemType);
          return SPELL_FAILED_DONT_REPORT;
    ...
    case SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC: {
      ...
      // do not allow adding usable enchantments to items that have use effect
      // already
      if (enchantEntry) {
    ...
    case SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY: {
      ...
      // Xinef: Apply item level restriction if the enchanting spell has max
      // level restrition set
    ...
    case SPELL_EFFECT_ENCHANT_HELD_ITEM:
      // check item existence in effect code (not output errors at offhand hold
      // item effect to main hand for example
      break;
  ...
  if (/*Moar retardnes! never skip those checks !(_triggeredCastFlags &
...
SpellCastResult Spell::CheckSpellFocus() {
  ...
  if (m_spellInfo->RequiresSpellFocus) {
    CellCoord p(Warhead::ComputeCellCoord(m_caster->GetPositionX(),
                                          m_caster->GetPositionY()));
    Cell cell(p);

    GameObject *ok = nullptr;
    Warhead::GameObjectFocusCheck go_check(m_caster,
                                           m_spellInfo->RequiresSpellFocus);
    Warhead::GameObjectSearcher<Warhead::GameObjectFocusCheck> checker(
        m_caster, ok, go_check);

    TypeContainerVisitor<
        Warhead::GameObjectSearcher<Warhead::GameObjectFocusCheck>,
...
void Spell::Delayed() // only called in DealDamage()
{
  ...
  //    return;                                             // spell is active
  //    and can't be time-backed
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Spell %u partially interrupted for (%d) ms at damage",
...
bool Spell::CheckEffectTarget(Unit const *target, uint32 eff) const {
  ...
  // requires specific entry this is only for target addition and target has to
  // have unselectable flag, this is valid for FLAG_EXTRA_TRIGGER and quest
  ...
  // if spell is triggered, need to check for LOS disable on the aura triggering
  // it and inherit that behaviour
  if (IsTriggered() && m_triggeredByAuraSpell &&
      (m_triggeredByAuraSpell->HasAttribute(
           SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) ||
       DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_triggeredByAuraSpell->Id,
                                 nullptr, SPELL_DISABLE_LOS)))
    return true;
  ...
  case SPELL_EFFECT_RESURRECT_NEW:
    ...
    if (target != m_caster &&
        !target->IsWithinLOSInMap(m_caster, LINEOFSIGHT_ALL_CHECKS)) {
      if (!m_targets.GetCorpseTargetGUID())
        return false;

      Corpse *corpse =
          ObjectAccessor::GetCorpse(*m_caster, m_targets.GetCorpseTargetGUID());
      if (!corpse)
        return false;
  ...
  case SPELL_EFFECT_SKIN_PLAYER_CORPSE: {
    if (!m_targets.GetCorpseTargetGUID()) {
      if (target->IsWithinLOSInMap(m_caster, LINEOFSIGHT_ALL_CHECKS) &&
          target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE))
        return true;
...
bool Spell::IsNeedSendToClient(bool go) const {
  return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] ||
         ...
         (go && m_triggeredByAuraSpell &&
          m_triggeredByAuraSpell->IsChanneled());
}
...
bool Spell::HaveTargetsForEffect(uint8 effect) const {
  ...
  for (std::list<ItemTargetInfo>::const_iterator itr = m_UniqueItemInfo.begin();
       itr != m_UniqueItemInfo.end(); ++itr)
    if (itr->effectMask & (1 << effect))
      return true;
...
SpellEvent::~SpellEvent() {
  ...
  if (m_Spell->IsDeletable()) {
    ...
    LOG_ERROR("server",
              "~SpellEvent: %s %u tried to delete non-deletable spell %u. Was "
              "not deleted, causes memory leak.",
              (m_Spell->GetCaster()->GetTypeId() == TYPEID_PLAYER ? "Player"
                                                                  : "Creature"),
    ...
    if (m_Spell->GetDelayStart() != 0) {
      {
        ...
        // event complete
        // finish update event will be re-added automatically at the end of
        // routine)
      }
    } else {
...
bool ReflectEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/) {
  ...
  if (caster && target && caster->IsInMap(target))
    caster->ProcDamageAndSpell(target, PROC_FLAG_NONE,
                               PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG,
                               PROC_EX_REFLECT, 1, BASE_ATTACK, _spellInfo);
  return true;
}
...
void Spell::HandleLaunchPhase() {
  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    if (m_applyMultiplierMask & (1 << i))
      multiplier[i] =
          m_spellInfo->Effects[i].CalcDamageMultiplier(m_originalCaster, this);
  ...
  for (Unit::AuraEffectList::const_iterator j = Auras.begin(); j != Auras.end();
       ++j) {
    if ((*j)->IsAffectedOnSpell(m_spellInfo))
      usesAmmo = false;
  }
  ...
  for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin();
    ...
    if (IsTriggered() && m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo &targetInfo, float *multiplier,
  ...
  for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    if (targetInfo.effectMask & (1 << i)) {
      ...
      HandleEffects(unit, nullptr, nullptr, i,
                    SPELL_EFFECT_HANDLE_LAUNCH_TARGET);

      if (m_damage > 0) {
        // Xinef: Area Auras, AoE Targetting spells AND Chain Target spells
        // (cleave etc.)
  ...
  critChance = unit->SpellTakenCritChance(
      caster, m_spellInfo, m_spellSchoolMask, critChance, m_attackType, false);
  targetInfo.crit = roll_chance_f(std::max(0.0f, critChance));

  // Sweeping strikes wtf shit ;d
  if (m_caster->getClass() == CLASS_WARRIOR && ssEffect < MAX_SPELL_EFFECTS &&
    ...
    if (Aura *aur = m_caster->GetAura(12328)) {
      ...
      HandleEffects(target, nullptr, nullptr, ssEffect,
                    SPELL_EFFECT_HANDLE_LAUNCH_TARGET);

      basepoints = (targetInfo.crit
                        ? Unit::SpellCriticalDamageBonus(m_caster, m_spellInfo,
                                                         m_damage, target)
                        : m_damage);
      m_damage = mdmg;
      m_caster->CastCustomSpell(target, 26654, &basepoints, nullptr, nullptr,
                                true);
...
SpellCastResult Spell::CanOpenLock(uint32 effIndex, uint32 lockId,
                                   SkillType &skillId, int32 &reqSkillValue,
                                   int32 &skillValue) {
  ...
  for (int j = 0; j < MAX_LOCK_CASE; ++j) {
    ...
    case LOCK_KEY_SKILL: {
      ...
      if (skillId != SKILL_NONE) {
        ...
        // castitem check: rogue using skeleton keys. the skill values should
        ...
        // add the effect base points modifier from the spell casted (cheat lock
        // / skeleton key etc.)
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "Spell::LoadScripts: Script `%s` for spell `%u` is loaded now",
              (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
#endif
    (*itr)->Register();
    ++itr;
  }
...
void Spell::CallScriptOnCastHandlers() {
  for (std::list<SpellScript *>::iterator scritr = m_loadedScripts.begin();
    ...
    std::list<SpellScript::CastHandler>::iterator hookItrEnd =
                                                      (*scritr)->OnCast.end(),
                                                  hookItr =
                                                      (*scritr)->OnCast.begin();
    for (; hookItr != hookItrEnd; ++hookItr)
...
void Spell::CallScriptObjectAreaTargetSelectHandlers(
    std::list<WorldObject *> &targets, SpellEffIndex effIndex,
    SpellImplicitTargetInfo const &targetType) {
  for (std::list<SpellScript *>::iterator scritr = m_loadedScripts.begin();
       scritr != m_loadedScripts.end(); ++scritr) {
...
void Spell::CallScriptObjectTargetSelectHandlers(
    WorldObject *&target, SpellEffIndex effIndex,
    SpellImplicitTargetInfo const &targetType) {
  for (std::list<SpellScript *>::iterator scritr = m_loadedScripts.begin();
       scritr != m_loadedScripts.end(); ++scritr) {
...
void Spell::CallScriptDestinationTargetSelectHandlers(
    SpellDestination &target, SpellEffIndex effIndex,
    SpellImplicitTargetInfo const &targetType) {
  for (std::list<SpellScript *>::iterator scritr = m_loadedScripts.begin();
       scritr != m_loadedScripts.end(); ++scritr) {
...
bool Spell::CanExecuteTriggersOnHit(uint8 effMask,
  ...
  bool only_on_caster =
      (triggeredByAura &&
       triggeredByAura->HasAttribute(SPELL_ATTR4_PROC_ONLY_ON_CASTER));
  // If triggeredByAura has SPELL_ATTR4_PROC_ONLY_ON_CASTER then it can only
  // proc on a casted spell with TARGET_UNIT_CASTER
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    if ((effMask & (1 << i)) &&
        (!only_on_caster ||
         (m_spellInfo->Effects[i].TargetA.GetTarget() == TARGET_UNIT_CASTER)))
      return true;
  }
  return effMask;
}
...
void Spell::PrepareTriggersExecutedOnHit() {
  ...
  case SPELLFAMILY_PALADIN: {
    if (m_spellInfo->SpellFamilyFlags[1] & 0x40000000) {
      ...
      for (Unit::AuraEffectList::const_iterator itr = mVindication.begin();
           itr != mVindication.end(); ++itr) {
  ...
  // handle SPELL_AURA_ADD_TARGET_TRIGGER auras:
  // save auras which were present on spell caster on cast, to prevent triggered
  // auras from affecting caster and to correctly calculate proc chance when
  // combopoints are present
  ...
  for (Unit::AuraEffectList::const_iterator i = targetTriggers.begin();
    ...
    if (SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(
      ...
      // calculate the chance using spell base amount, because aura amount is
      // not updated on combo-points change this possibly needs fixing
...
bool Spell::HasGlobalCooldown() const {
  ...
  if (m_caster->GetCharmInfo())
    return m_caster->GetCharmInfo()->GetGlobalCooldownMgr().HasGlobalCooldown(
        m_spellInfo);
...
void Spell::TriggerGlobalCooldown() {
  ...
  // There are some spells (mostly not casted directly by player) that have < 1
  // sec and > 1.5 sec global cooldowns but as tests show are not affected by
  // any spell mods.
  if (m_spellInfo->StartRecoveryTime >= MIN_GCD &&
      m_spellInfo->StartRecoveryTime <= MAX_GCD) {
    // gcd modifier auras are applied only to own spells and only players have
    // such mods
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
      m_caster->ToPlayer()->ApplySpellMod(m_spellInfo->Id,
                                          SPELLMOD_GLOBAL_COOLDOWN, gcd, this);
  ...
  if (m_caster->GetCharmInfo())
    ...
    m_caster->ToPlayer()->GetGlobalCooldownMgr().AddGlobalCooldown(m_spellInfo,
                                                                   gcd);
}
...
WorldObjectSpellTargetCheck::WorldObjectSpellTargetCheck(
    Unit *caster, Unit *referer, SpellInfo const *spellInfo,
...
WorldObjectSpellNearbyTargetCheck::WorldObjectSpellNearbyTargetCheck(
    float range, Unit *caster, SpellInfo const *spellInfo,
    SpellTargetCheckTypes selectionType, ConditionList *condList)
    : WorldObjectSpellTargetCheck(caster, caster, spellInfo, selectionType,
                                  condList),
...
WorldObjectSpellAreaTargetCheck::WorldObjectSpellAreaTargetCheck(
    float range, Position const *position, Unit *caster, Unit *referer,
    SpellInfo const *spellInfo, SpellTargetCheckTypes selectionType,
    ConditionList *condList)
    : WorldObjectSpellTargetCheck(caster, referer, spellInfo, selectionType,
                                  condList),
...
WorldObjectSpellConeTargetCheck::WorldObjectSpellConeTargetCheck(
    float coneAngle, float range, Unit *caster, SpellInfo const *spellInfo,
    SpellTargetCheckTypes selectionType, ConditionList *condList)
    : WorldObjectSpellAreaTargetCheck(range, caster, caster, caster, spellInfo,
                                      selectionType, condList),
...
WorldObjectSpellTrajTargetCheck::WorldObjectSpellTrajTargetCheck(
    float range, Position const *position, Unit *caster,
    SpellInfo const *spellInfo, SpellTargetCheckTypes selectionType,
    ConditionList *condList)
    : WorldObjectSpellAreaTargetCheck(range, position, caster, caster,
                                      spellInfo, selectionType, condList) {}
