...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS] = {
    ...
    &Spell::EffectSkinPlayerCorpse, // 116 SPELL_EFFECT_SKIN_PLAYER_CORPSE one
                                    // spell: Remove Insignia, bg usage, required
                                    // special corpse flags...
    ...
    &Spell::EffectCreateItem2, // 157 SPELL_EFFECT_CREATE_ITEM_2 create item or
                               // create item template and replace by some randon
                               // spell loot item
    &Spell::EffectMilling, // 158 SPELL_EFFECT_MILLING                  milling
    &Spell::EffectRenamePet, // 159 SPELL_EFFECT_ALLOW_RENAME_PET         allow
                             // rename pet once again
    &Spell::EffectNULL, // 160 SPELL_EFFECT_160                      1 spell -
                        // 45534
...
void Spell::EffectResurrectNew(SpellEffIndex effIndex) {
  ...
  target->setResurrectRequestData(
      m_caster->GetGUID(), m_caster->GetMapId(), m_caster->GetPositionX(),
      m_caster->GetPositionY(), m_caster->GetPositionZ(), health, mana);
...
void Spell::EffectInstaKill(SpellEffIndex /*effIndex*/) {
  ...
  Unit::DealDamage(m_caster, unitTarget, unitTarget->GetHealth(), nullptr,
                   NODAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr, false);
}
...
void Spell::EffectEnvironmentalDMG(SpellEffIndex /*effIndex*/) {
  ...
  Unit::CalcAbsorbResist(m_caster, unitTarget, m_spellInfo->GetSchoolMask(),
                         SPELL_DIRECT_DAMAGE, damage, &absorb, &resist,
                         m_spellInfo);

  m_caster->SendSpellNonMeleeDamageLog(
      unitTarget, m_spellInfo->Id, damage + absorb + resist,
      m_spellInfo->GetSchoolMask(), absorb, resist, false, 0, false);
  if (unitTarget->GetTypeId() == TYPEID_PLAYER)
...
void Spell::EffectSchoolDMG(SpellEffIndex effIndex) {
  ...
  if (unitTarget && unitTarget->IsAlive()) {
    ...
    case SPELLFAMILY_GENERIC: {
      ...
      // Consumption
      case 28865:
        damage = (m_caster->GetMap()->ToInstanceMap()->GetDifficulty() ==
                          REGULAR_DIFFICULTY
                      ? 2750
                      : 4250);
        break;
    ...
    case SPELLFAMILY_WARRIOR: {
      ...
      if (m_spellInfo->SpellFamilyFlags[1] & 0x200 &&
        ...
        uint32 block_value =
            m_caster->GetShieldBlockValue(uint32(float(level) * 24.5f * limit),
                                          uint32(float(level) * 34.5f * limit));

        damage += int32(m_caster->ApplyEffectModifiers(m_spellInfo, effIndex,
                                                       float(block_value)));
      }
      // Victory Rush
    ...
    case SPELLFAMILY_WARLOCK: {
      ...
      if ((m_spellInfo->SpellFamilyFlags[1] & 0x000040) &&
        ...
        // Incinerate does more dmg (dmg*0.25) if the target have Immolate
        // debuff. Check aura state for speed but aura state set not only for
        // Immolate spell
        if (unitTarget->HasAuraState(AURA_STATE_CONFLAGRATE)) {
          if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                        SPELLFAMILY_WARLOCK, 0x4, 0, 0))
            damage += damage / 4;
        }
      }
      ...
      // Conflagrate - consumes Immolate or Shadowflame
        ...
        for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin();
          ...
          if ((*i)->GetSpellInfo()->SpellFamilyFlags[2] & 0x00000002)
            aura =
                *i; // remember but wait possible Immolate as primary priority
        }
        ...
        if (aura) {
          ...
          pdamage = unitTarget->SpellDamageBonusTaken(
              m_caster, aura->GetSpellInfo(), pdamage, DOT,
          ...
          uint8 baseTotalTicks =
              uint8(m_caster->CalcSpellDuration(aura->GetSpellInfo()) /
          ...
          m_spellValue->EffectBasePoints[1] =
              m_spellInfo->Effects[EFFECT_1].CalcBaseValue(
          ...
          if (Player *owner = m_caster->GetOwner()->ToPlayer()) {
            if (AuraEffect *aurEff =
                    owner->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER,
                                         SPELLFAMILY_WARLOCK, 214, 0)) {
              int32 bp0 = aurEff->GetId() == 54037 ? 4 : 8;
              m_caster->CastCustomSpell(m_caster, 54425, &bp0, nullptr, nullptr,
                                        true);
            }
          }
        }
    ...
    case SPELLFAMILY_PRIEST: {
      ...
      if (m_caster->GetShapeshiftForm() == FORM_SHADOW &&
        ...
        for (Unit::AuraEffectList::const_iterator i = ImprMindBlast.begin();
             i != ImprMindBlast.end(); ++i) {
          if ((*i)->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_PRIEST &&
              ((*i)->GetSpellInfo()->SpellIconID == 95)) {
            int chance =
                (*i)->GetSpellInfo()->Effects[EFFECT_1].CalcValue(m_caster);
    ...
    case SPELLFAMILY_DRUID: {
      ...
      if (m_caster->GetTypeId() == TYPEID_PLAYER &&
        ...
        // converts each extra point of energy into ($f1+$AP/410) additional
        // damage
        ...
        damage +=
            int32(CalculatePct(m_caster->ToPlayer()->GetComboPoints() * ap, 7));
      }
      // Wrath
      else if (m_spellInfo->SpellFamilyFlags[0] & 0x00000001) {
        // Improved Insect Swarm
        if (AuraEffect const *aurEff =
                m_caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 1771, 0))
          if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                        SPELLFAMILY_DRUID, 0x00200000, 0, 0))
        ...
        if (Player *player = m_caster->ToPlayer()) {
          ...
          if (uint32 combo = player->GetComboPoints()) {
            ...
            if (AuraEffect const *aurEff = unitTarget->GetAuraEffect(
                    SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x00010000,
                    0, 0, m_caster->GetGUID())) {
              ...
              damage *= doses;
              damage += int32(player->GetTotalAttackPowerValue(BASE_ATTACK) *
                              0.09f * combo);
            }
    ...
    case SPELLFAMILY_HUNTER: {
      ...
      // Steady Shot
        ...
        for (Unit::AuraEffectList::const_iterator iter = decSpeedList.begin();
             iter != decSpeedList.end(); ++iter) {
        ...
        if (Player *caster = m_caster->ToPlayer()) {
          ...
          if (Item *item = caster->GetWeaponForAttack(RANGED_ATTACK)) {
            ...
            damage +=
                int32(caster->GetAmmoDPS() * weaponTemplate->Delay * 0.001f);
          }
        }
      }
      break;
    ...
    case SPELLFAMILY_PALADIN: {
      ...
      if (m_spellInfo->SpellFamilyFlags[1] & 0x00040000) {
        ...
        if (Player *player = m_caster->ToPlayer()) {
          ...
          player->CalculateMinMaxDamage(BASE_ATTACK, false, false, mindamage,
                                        maxdamage);
          float average = (mindamage + maxdamage) / 2;
          int32 count =
              m_caster->CalculateSpellDamage(unitTarget, m_spellInfo, EFFECT_2);
      ...
      if (m_spellInfo->SpellFamilyFlags[EFFECT_1] & 0x100000) {
        ...
        if (m_caster->GetAuraEffect(64882, EFFECT_0))
          block_value += 225;
        damage += CalculatePct(block_value,
    ...
    if (m_originalCaster /*&& damage > 0 Xinef: this can be increased from 0*/
        && apply_direct_bonus) {
      // Xinef: protection
      if (damage < 0)
        damage = 0;
...
void Spell::EffectDummy(SpellEffIndex effIndex) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura", "Spell ScriptStart spellid %u in EffectDummy(%u)",
  ...
  if (gameObjTarget)
    ...
    sEluna->OnDummyEffect(m_caster, m_spellInfo->Id, effIndex,
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_TRIGGER_SPELL &&
    ...
    case 18461: {
      ...
      if (unitTarget->GetTypeId() == TYPEID_PLAYER &&
          unitTarget->ToPlayer()->HasSpellCooldown(1784))
    ...
    case 35729: {
      ...
      for (Unit::AuraApplicationMap::iterator iter = Auras.begin();
        ...
        if ((spell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC ||
            ...
            (!spell->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) ||
             spell->SpellFamilyName != SPELLFAMILY_GENERIC)) {
    ...
    // launch is handled before hit triggers, thus silence removes current
    // casted spell and interrupt is unable to detect any cast and doesnt work
    case 24259:
      return;
    }
  }
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex) {
  ...
  // original caster guid only for GO cast
  m_caster->CastSpell(targets, spellInfo, &values, TRIGGERED_FULL_MASK, nullptr,
                      nullptr, m_originalCasterGUID);
}
...
void Spell::EffectForceCast(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_FORCE_CAST &&
    ...
    case 52349: // Overtake
      unitTarget->CastCustomSpell(unitTarget, spellInfo->Id, &damage, nullptr,
...
void Spell::EffectJumpDest(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->Id == 49376) // feral charge
  {
    ...
    m_caster->GetMotionMaster()->MoveJump(
        x, y, z, speedXY, speedZ, 0,
        ObjectAccessor::GetUnit(*m_caster,
                                m_caster->GetUInt64Value(UNIT_FIELD_TARGET)));
    return;
  }
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/) {
  ...
  case 70746: // Teleport Into Sunwell (for Battered Hilt)
    if (Player *target = unitTarget->ToPlayer()) {
      ...
      target->TeleportTo(mapid, x, y, z, orientation,
                         TELE_TO_GM_MODE); // skip CanPlayerEnter check
    }
    return;
  }
  ...
  if (mapid == unitTarget->GetMapId()) {
    if (unitTarget->GetVehicleKit()) // we are vehicle!
      ...
      bool withPet = unitTarget->GetTypeId() == TYPEID_PLAYER &&
    ...
    unitTarget->ToPlayer()->TeleportTo(mapid, x, y, z, orientation,
                                       unitTarget == m_caster ? TELE_TO_SPELL
                                                              : 0);
  else {
    LOG_ERROR("server",
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex) {
  ...
  LOG_DEBUG("spells.aura",
            ...
            player->GetGUIDLow(), spellToUnlearn, m_caster->GetGUIDLow());
#endif
}
...
void Spell::EffectPowerDrain(SpellEffIndex effIndex) {
  ...
  damage = m_caster->SpellDamageBonusDone(unitTarget, m_spellInfo,
...
void Spell::EffectSendEvent(SpellEffIndex effIndex) {
  ...
  if (ZoneScript *zoneScript = m_caster->GetZoneScript())
    zoneScript->ProcessEvent(target, m_spellInfo->Effects[effIndex].MiscValue);
               ...
               m_caster
                   ->GetInstanceScript()) // needed in case Player is the caster
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget && unitTarget->IsAlive() && damage >= 0) {
    ...
    // Swiftmend - consumes Regrowth or Rejuvenation
        ...
        if ((*i)->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_DRUID &&
          ...
          if (m_caster->GetGUID() == (*i)->GetCasterGUID()) {
            if (!forcedTargetAura ||
              ...
              forcedTargetAura = *i;
          } else if (!targetAura || (*i)->GetBase()->GetDuration() <
      ...
      if (!targetAura) {
        LOG_ERROR("server",
                  "Target(GUID:" UI64FMTD
                  ") has aurastate AURA_STATE_SWIFTMEND but no matching aura.",
                  unitTarget->GetGUID());
        return;
      }
      ...
      // addhealth += tickheal * tickcount;
      // addhealth = caster->SpellHealingBonus(m_spellInfo, addhealth, HEAL,
      // unitTarget);
    }
...
void Spell::EffectHealMechanical(SpellEffIndex /*effIndex*/) {
  ...
  uint32 heal = m_originalCaster->SpellHealingBonusDone(unitTarget, m_spellInfo,
                                                        uint32(damage), HEAL);

  m_damage -= unitTarget->SpellHealingBonusTaken(m_originalCaster, m_spellInfo,
                                                 heal, HEAL);
}
...
void Spell::EffectHealthLeech(SpellEffIndex /*effIndex*/) {
  ...
  // m_spellInfo->Effects[effIndex].CalcValueMultiplier(m_originalCaster, this);
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex) {
  ...
  if (!m_spellAura) {
    ...
    if (!dynObj->CreateDynamicObject(
            sObjectMgr->GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), caster,
            m_spellInfo->Id, *destTarget, radius, DYNAMIC_OBJECT_AREA_SPELL)) {
      delete dynObj;
      return;
    }
...
void Spell::EffectEnergize(SpellEffIndex effIndex) {
  ...
  case 48542: // Revitalize
    ...
    break;
  case 67490: // Runic Mana Injector (mana gain increased by 25% for engineers
              // - 3.2.0 patch change)
  {
    if (Player *player = m_caster->ToPlayer())
      if (player->HasSkill(SKILL_ENGINEERING))
  ...
  if (m_spellInfo->Id == 45051) {
    ...
    for (Unit::AuraApplicationMap::iterator itr = Auras.begin();
         itr != Auras.end(); ++itr) {
      SpellGroupSpecialFlags sFlag =
          sSpellMgr->GetSpellGroupSpecialFlags(itr->second->GetBase()->GetId());
      if (!guardianFound)
    ...
    if (!availableElixirs.empty()) {
      ...
      m_caster->CastSpell(
          unitTarget,
          Warhead::Containers::SelectRandomContainerElement(availableElixirs),
          true, m_CastItem);
    }
  }
}
  ...
  if (gameObjTarget) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_ERROR("server",
                "Possible hacking attempt: Player %s [guid: %u] tried to loot "
                "a gameobject [entry: %u id: %u] which is on respawn time "
    ...
    case GAMEOBJECT_TYPE_QUESTGIVER:
      player->PrepareGossipMenu(
          gameObjTarget, gameObjTarget->GetGOInfo()->questgiver.gossipID, true);
...
void Spell::EffectOpenLock(SpellEffIndex effIndex) {
  ...
  if (gameObjTarget) {
    ...
    // use this spell handle outdoor pvp object opening, return true if go was
    // registered for handling these objects must have been spawned by
    // outdoorpvp!
    else if (gameObjTarget->GetGOInfo()->type == GAMEOBJECT_TYPE_GOOBER &&
             sOutdoorPvPMgr->HandleOpenGo(player, gameObjTarget->GetGUID()))
      return;
  ...
  ExecuteLogEffectOpenLock(effIndex, gameObjTarget ? (Object *)gameObjTarget
                                                   : (Object *)itemTarget);
}
...
void Spell::EffectSummonType(SpellEffIndex effIndex) {
  ...
  if (!properties) {
    LOG_ERROR("server", "EffectSummonType: Unhandled summon type %u",
              m_spellInfo->Effects[effIndex].MiscValueB);
    return;
  }
  ...
  case SUMMON_CATEGORY_UNK:
    ...
    case SUMMON_TYPE_VEHICLE2:
      summon = m_caster->GetMap()->SummonCreature(
          entry, *destTarget, properties, duration, m_originalCaster,
          m_spellInfo->Id);
      break;
    ...
    case SUMMON_TYPE_MINIPET: {
      ...
      summon->SelectLevel(); // some summoned creaters have different from 1 DB
                             // data for level/hp
      summon->SetUInt32Value(UNIT_NPC_FLAGS,
                             summon->GetCreatureTemplate()->npcflag);

      summon->SetUInt32Value(UNIT_FIELD_FLAGS,
                             UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
      summon->SetReactState(REACT_PASSIVE);

      // Xinef: Pet can have some auras in creature_addon or in scripts, do not
      // remove them instantly
      // summon->AI()->EnterEvadeMode();
      if (properties->Type != SUMMON_TYPE_JEEVES) {
        summon->GetMotionMaster()->Clear(false);
        summon->GetMotionMaster()->MoveFollow(m_originalCaster, PET_FOLLOW_DIST,
  ...
  case SUMMON_CATEGORY_PET:
    // Xinef: SummonGuardian function can summon a few npcs of same type, remove
    // old summons with same entry here
    if (m_originalCaster)
  ...
  case SUMMON_CATEGORY_VEHICLE:
    ...
    // vehicle and that cause the clicker to cast a ride vehicle spell on the
    // summoned unit.
    // float x, y, z;
    ...
    summon = m_originalCaster->GetMap()->SummonCreature(
        entry, *destTarget, properties, duration, m_caster, m_spellInfo->Id);
    ...
    if (basePoints >
        1) // xinef: some summoning spells have value 1 - indicates vehicle seat
    {
    ...
    if (basePoints > 0 && basePoints < MAX_VEHICLE_SEATS)
      m_originalCaster->CastCustomSpell(spellId, SPELLVALUE_BASE_POINT0,
                                        basePoints, summon, true);
    else
      m_originalCaster->CastSpell(summon, spellId, true);

    // xinef: i think this is wrong, found only 2 vehicles with faction override
    // and one of them should inherit caster faction...
...
void Spell::EffectDispel(SpellEffIndex effIndex) {
  ...
  for (int32 count = 0; count < damage && !dispel_list.empty();) {
    ...
    if (!chance) {
      ...
      if (roll_chance_i(chance)) {
        ...
        for (DispelChargesList::iterator successItr = success_list.begin();
             successItr != success_list.end(); ++successItr) {
  ...
  for (DispelChargesList::iterator itr = success_list.begin();
    ...
    unitTarget->RemoveAurasDueToSpellByDispel(
        itr->first->GetId(), m_spellInfo->Id, itr->first->GetCasterGUID(),
        m_caster, itr->second);
  }
  ...
  if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK &&
    ...
    m_caster->CastCustomSpell(m_caster, 19658, &heal_amount, nullptr, nullptr,
                              true);
    // Glyph of Felhunter
    if (Unit *owner = m_caster->GetOwner())
      if (owner->GetAura(56249))
        owner->CastCustomSpell(owner, 19658, &heal_amount, nullptr, nullptr,
                               true);
  }
}
...
void Spell::EffectAddFarsight(SpellEffIndex effIndex) {
  ...
  if (!dynObj->CreateDynamicObject(
          sObjectMgr->GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), m_caster,
          m_spellInfo->Id, *destTarget, radius,
...
void Spell::EffectTeleUnitsFaceCaster(SpellEffIndex /*effIndex*/) {
  ...
  if (m_targets.HasDst()) {
    unitTarget->NearTeleportTo(
        destTarget->GetPositionX(), destTarget->GetPositionY(),
        destTarget->GetPositionZ(), destTarget->GetAbsoluteAngle(m_caster),
        unitTarget == m_caster);
  }
}
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "SpellEffect::AddHonor (spell_id %u) rewards %d honor points "
              "(item %u) for player: %u",
  ...
  // do not allow to add too many honor for player (50 * 21) = 1040 at level 70,
  // or (50 * 31) = 1550 at level 80
  if (damage <= 50) {
    uint32 honor_reward = Warhead::Honor::hk_honor_at_level(
        unitTarget->getLevel(), float(damage));
    unitTarget->ToPlayer()->RewardHonor(nullptr, 1, honor_reward, false);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "SpellEffect::AddHonor (spell_id %u) rewards %u honor points "
              "(scale) to player: %u",
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex) {
  ...
  // support only enchantings with add socket in this slot
  {
    ...
    if (!add_socket) {
      LOG_ERROR(
          "server",
          "Spell::EffectEnchantItemPrismatic: attempt apply enchant spell %u "
          "with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u) but without "
          "ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not suppoted yet.",
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN &&
    ...
    // Effect[1] base value Note: damage calculated (correctly) with rounding
    // int32(float(v)) but RW enchantments applied damage int32(float(v)+0.5),
    // this create  0..1 difference sometime
    switch (damage) {
    // Rank 1
    case 2:
      spell_id = 36744;
    ...
    if (!spellInfo) {
      LOG_ERROR("server", "Spell::EffectEnchantItemTmp: unknown spell id %i",
                spell_id);
      return;
    }
  ...
  itemTarget->SetEnchantment(TEMP_ENCHANTMENT_SLOT, enchant_id, duration * 1000,
                             pEnchant->charges, m_caster->GetGUID());
...
void Spell::EffectSummonPet(SpellEffIndex effIndex) {
  ...
  if (OldSummon) {
    if (petentry == 0 || OldSummon->GetEntry() == petentry) {
      ...
      for (CreatureSpellCooldowns::const_iterator itr =
           ...
           itr != OldSummon->m_CreatureSpellCooldowns.end(); ++itr)
    ...
    if (owner->GetTypeId() == TYPEID_PLAYER)
      owner->ToPlayer()->RemovePet(OldSummon,
                                   (OldSummon->getPetType() == HUNTER_PET
                                        ? PET_SAVE_AS_DELETED
  ...
  owner->SummonPet(petentry, x, y, z, owner->GetOrientation(), SUMMON_PET, 0,
                   m_spellInfo->Id, m_caster->GetGUID(), PET_LOAD_SUMMON_PET);
  // if (!pet)
  //    return;
...
void Spell::EffectTaunt(SpellEffIndex /*effIndex*/) {
  ...
  if (!unitTarget->getThreatManager().getOnlineContainer().empty()) {
    ...
    float topThreat = unitTarget->getThreatManager()
                          .getOnlineContainer()
                          .getMostHated()
                          ->getThreat();
    if (topThreat > myThreat)
      unitTarget->getThreatManager().doAddThreat(m_caster,
                                                 topThreat - myThreat);

    // Set aggro victim to caster
    if (HostileReference *forcedVictim = unitTarget->getThreatManager()
                                             .getOnlineContainer()
  ...
  if (unitTarget->ToCreature()->IsAIEnabled &&
      !unitTarget->ToCreature()->HasReactState(REACT_PASSIVE))
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex) {
  ...
  case SPELLFAMILY_GENERIC: {
    ...
    case 67883: {
      AddPct(totalDamagePercentMod,
             unitTarget->GetDiseasesByCaster(m_caster->GetGUID(), 1) * 30.0f);
      break;
    }
    // sweeping strikes
    case 26654: {
  ...
  case SPELLFAMILY_WARRIOR: {
    ...
    if (m_spellInfo->SpellFamilyFlags[1] & 0x40) {
      ...
      if (Aura *aur = unitTarget->GetAura(58567)) {
        ...
        spell_bonus +=
            (aur->GetStackAmount() - 1) * CalculateSpellDamage(2, unitTarget);
      }
    }
    break;
  }
  case SPELLFAMILY_ROGUE: {
    ...
    // Mutilate (for each hand)
      ...
      bool found = false;
      // fast check
      if (unitTarget->HasAuraState(AURA_STATE_DEADLY_POISON, m_spellInfo,
                                   m_caster))
        found = true;
      // full aura scan
      else {
  ...
  case SPELLFAMILY_SHAMAN: {
    ...
    if (m_caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, 688,
                                EFFECT_0))
  ...
  case SPELLFAMILY_DEATHKNIGHT: {
    ...
    if (m_spellInfo->SpellFamilyFlags[0] & 0x400000) {
      ...
      AddPct(totalDamagePercentMod,
             disease_amt *
                 unitTarget->GetDiseasesByCaster(m_caster->GetGUID()) / 2.0f);
  ...
  // apply to non-weapon bonus weapon total pct effect, weapon total flat effect
  // included in weapon damage
  ...
  eff_damage = unitTarget->MeleeDamageBonusTaken(m_caster, eff_damage,
                                                 m_attackType, m_spellInfo);
...
void Spell::EffectThreat(SpellEffIndex /*effIndex*/) {
  ...
  // xinef: skip if target cannot have threat list or caster is friendly (ghoul
  // leap)
...
void Spell::EffectInterruptCast(SpellEffIndex effIndex) {
    ...
    if (Spell *spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i))) {
      ...
      if ((spell->getState() == SPELL_STATE_CASTING ||
          ...
          ((i == CURRENT_GENERIC_SPELL &&
            curSpellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT) ||
        ...
        if (m_originalCaster) {
          int32 duration = m_originalCaster->ModSpellDuration(
              ...
              m_originalCaster->CalcSpellDuration(m_spellInfo), false,
              1 << effIndex);
  ...
  if (m_spellInfo->Id == 19647)
    m_caster->CastSpell(unitTarget, m_spellInfo->Effects[EFFECT_1].TriggerSpell,
                        true);
}
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex) {
  ...
  GameObject *pGameObj = sObjectMgr->IsGameObjectStaticTransport(gameobject_id)
                             ? new StaticTransport()
                             : new GameObject();
  ...
  if (!pGameObj->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT),
                        gameobject_id, map, m_caster->GetPhaseMask(), x, y, z,
                        target->GetOrientation(), G3D::Quat(), 100,
                        GO_STATE_READY)) {
    delete pGameObj;
    return;
  }
    ...
    if (Player *player = m_caster->ToPlayer())
      if (Battleground *bg = player->GetBattleground())
        bg->SetDroppedFlagGUID(
            pGameObj->GetGUID(),
            player->GetTeamId() == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);

  if (uint32 linkedEntry = pGameObj->GetGOInfo()->GetLinkedGameObjectEntry()) {
    GameObject *linkedGO = sObjectMgr->IsGameObjectStaticTransport(linkedEntry)
                               ? new StaticTransport()
                               : new GameObject();
    if (linkedGO->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT),
                         linkedEntry, map, m_caster->GetPhaseMask(), x, y, z,
                         target->GetOrientation(), G3D::Quat(), 100,
                         GO_STATE_READY)) {
...
m_caster->ToPlayer()->RemoveItem(bag, slot, true); else
m_caster->ToPlayer()->GetItemByPos(bag,
slot)->SetCount(m_caster->ToPlayer()->GetItemByPos(bag, slot)->GetCount()-1);
    ...
    case 46642:
    {
        if( !m_originalCaster || !unitTarget || unitTarget->GetTypeId() !=
    ...
    case 47770: {
      ...
      if (m_caster->getGender() > 0)
        gender = "her";
      sprintf(buf,
              "%s rubs %s [Decahedral Dwarven Dice] between %s hands and "
              "rolls. One %u and one %u.",
              m_caster->GetName().c_str(), gender, gender, urand(1, 10),
              urand(1, 10));
    ...
    case 60123: // Lightwell
    {
      ...
      case 31883:
        ...
        break;
      default:
        LOG_ERROR("server", "Unknown Lightwell spell caster %u",
                  m_caster->GetEntry());
        return;
      }
      ...
      if (Aura *chargesAura = m_caster->GetAura(59907)) {
        m_caster->CastSpell(unitTarget, spell_heal, true, nullptr, nullptr,
    ...
    case 58591: // Rank 10
    {
      ...
      for (uint8 slot = SUMMON_SLOT_TOTEM; slot < MAX_TOTEM_SLOT; ++slot) {
        ...
        Creature *totem =
            unitTarget->GetMap()->GetCreature(unitTarget->m_SummonSlot[slot]);
        if (totem && totem->IsTotem()) {
          m_caster->CastCustomSpell(totem, 55277, &basepoints0, nullptr,
                                    nullptr, true);
        }
      }
      ...
      if (AuraEffect *aur = unitTarget->GetAuraEffect(63298, 0)) {
        ...
        m_caster->CastCustomSpell(unitTarget, 55277, &basepoints0, nullptr,
                                  nullptr, true);
      }
      break;
    }
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget->GetInstanceScript() &&
    ...
    if (unitTarget->GetTypeId() == TYPEID_PLAYER)
      unitTarget->ToPlayer()
          ->SendAttackSwingCancelAttack(); // melee and ranged forced attack
                                           // cancel
  } else {
  ...
  Warhead::AnyUnfriendlyUnitInObjectRangeCheck u_check(
      ...
      unitTarget->GetVisibilityRange()); // no VISIBILITY_COMPENSATION, distance
                                         // is enough
  Warhead::UnitListSearcher<Warhead::AnyUnfriendlyUnitInObjectRangeCheck>
      searcher(unitTarget, targets, u_check);
  unitTarget->VisitNearbyObject(
      unitTarget->GetVisibilityRange(),
  ...
  // Vanish allows to remove all threat and cast regular stealth so other spells
  // can be used
  if (m_caster->GetTypeId() == TYPEID_PLAYER &&
...
void Spell::EffectAddComboPoints(SpellEffIndex /*effIndex*/) {
  ...
  {
    m_caster->m_movedByPlayer->ToPlayer()->SetComboPointGain(
        m_caster->m_movedByPlayer->ToPlayer()->GetComboPointGain() + damage);
    return;
  }
...
void Spell::EffectDuel(SpellEffIndex effIndex) {
  ...
  if (caster->duel || target->duel || !target->GetSocial() ||
      target->GetSocial()->HasIgnore(caster->GetGUIDLow()))
    return;
  ...
  if (!pGameObj->Create(
          sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), gameobject_id, map,
          ...
          m_caster->GetPositionZ(), m_caster->GetOrientation(), G3D::Quat(), 0,
          GO_STATE_READY)) {
    delete pGameObj;
    return;
  }
...
void Spell::EffectActivateObject(SpellEffIndex /*effIndex*/) {
  ...
  Player *player = m_caster->GetTypeId() == TYPEID_PLAYER
                       ? m_caster->ToPlayer()
  ...
  // int32 unk = m_spellInfo->Effects[effIndex].MiscValue; // This is set for
  // EffectActivateObject spells; needs research

  // xinef: pass player to allow gossip scripts to work
  //
  // gameObjTarget->GetMap()->ScriptCommandStart(activateCommand, 0, player ?
  // player : m_caster, gameObjTarget);
}
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex) {
  ...
  if (uint32 glyph = m_spellInfo->Effects[effIndex].MiscValue) {
    if (GlyphPropertiesEntry const *glyphEntry =
      ...
      player->CastSpell(m_caster, glyphEntry->SpellId,
                        TriggerCastFlags(TRIGGERED_FULL_MASK &
...
void Spell::EffectEnchantHeldItem(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->Effects[effIndex].MiscValue) {
    ...
    if (!duration)
      duration =
          damage * IN_MILLISECONDS; //+1;            //Base points after ..
    if (!duration)
    ...
    item->SetEnchantment(slot, enchant_id, duration, pEnchant->charges,
                         m_caster->GetGUID());
...
void Spell::EffectFeedPet(SpellEffIndex effIndex) {
  ...
  // TODO: fix crash when a spell has two effects, both pointed at the same item
  // target

  m_caster->CastCustomSpell(pet, m_spellInfo->Effects[effIndex].TriggerSpell,
                            &benefit, nullptr, nullptr, true);
}
...
void Spell::EffectSummonObject(SpellEffIndex effIndex) {
  ...
  Map *map = m_caster->GetMap();
  if (!pGameObj->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT),
                        gameobjectId, map, m_caster->GetPhaseMask(), x, y, z,
                        m_caster->GetOrientation(), G3D::Quat(), 0,
                        GO_STATE_READY)) {
    delete pGameObj;
    return;
  }
...
void Spell::EffectLeap(SpellEffIndex /*effIndex*/) {
  ...
  unitTarget->NearTeleportTo(dstpos.GetPositionX(), dstpos.GetPositionY(),
                             dstpos.GetPositionZ(), dstpos.GetOrientation(),
                             unitTarget == m_caster);
}
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/) {
  ...
  if (m_spellInfo->Id == 58836) {
    ...
    for (Unit::ControlSet::const_iterator itr = m_caster->m_Controlled.begin();
    ...
    Warhead::AnyUnfriendlyUnitInObjectRangeCheck u_check(
        m_caster, m_caster,
        m_caster->GetVisibilityRange()); // no VISIBILITY_COMPENSATION, distance
                                         // is enough
    Warhead::UnitListSearcher<Warhead::AnyUnfriendlyUnitInObjectRangeCheck>
        searcher(m_caster, targets, u_check);
    m_caster->VisitNearbyObject(
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/) {
  if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET) {
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
      m_caster->ToPlayer()->SetFallInformation(time(nullptr),
                                               m_caster->GetPositionZ());

    if (m_pathFinder) {
      m_caster->GetMotionMaster()->MoveCharge(
          m_pathFinder->GetEndPosition().x, m_pathFinder->GetEndPosition().y,
          m_pathFinder->GetEndPosition().z, 42.0f, EVENT_CHARGE,
          &m_pathFinder->GetPath());
    } else {
      Position pos;
...
void Spell::EffectKnockBack(SpellEffIndex effIndex) {
  ...
  // Spells with SPELL_EFFECT_KNOCK_BACK(like Thunderstorm) can't knoback target
  // if target has ROOT
...
void Spell::EffectLeapBack(SpellEffIndex effIndex) {
  ...
  // 1891: Disengage
  m_caster->JumpTo(speedxy, speedz,
...
void Spell::EffectPullTowards(SpellEffIndex effIndex) {
  ...
  } else // if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_PULL_TOWARDS)
  {
    ...
    // through textures
    pos.Relocate(m_caster->GetPositionX(), m_caster->GetPositionY(),
                 m_caster->GetPositionZ() + 1.0f, m_caster->GetOrientation());
  }
  ...
  unitTarget->GetMotionMaster()->MoveJump(pos.GetPositionX(),
                                          pos.GetPositionY(),
                                          pos.GetPositionZ(), speedXY, speedZ);
}
...
void Spell::EffectResurrectPet(SpellEffIndex /*effIndex*/) {
  ...
  if (!pet) {
    player->SummonPet(0, x, y, z, player->GetOrientation(), SUMMON_PET, 0, 0,
                      (uint64)damage, PET_LOAD_SUMMON_DEAD_PET);
    return;
  }
...
void Spell::EffectTransmitted(SpellEffIndex effIndex) {
  ...
  if (!pGameObj->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT),
                        name_id, cMap, m_caster->GetPhaseMask(), fx, fy, fz,
                        m_caster->GetOrientation(), G3D::Quat(), 100,
                        GO_STATE_READY)) {
    delete pGameObj;
    return;
  }
  ...
  if (uint32 linkedEntry = pGameObj->GetGOInfo()->GetLinkedGameObjectEntry()) {
    GameObject *linkedGO = sObjectMgr->IsGameObjectStaticTransport(linkedEntry)
                               ...
                               : new GameObject();
    if (linkedGO->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT),
                         linkedEntry, cMap, m_caster->GetPhaseMask(), fx, fy,
                         fz, m_caster->GetOrientation(), G3D::Quat(), 100,
                         GO_STATE_READY)) {
      linkedGO->SetRespawnTime(duration > 0 ? duration / IN_MILLISECONDS : 0);
      linkedGO->SetSpellId(m_spellInfo->Id);
    ...
    if (!chance) {
      ...
      if (roll_chance_i(chance)) {
        success_list.push_back(
            std::make_pair(itr->first->GetId(), itr->first->GetCasterGUID()));
        --itr->second;
        if (itr->second <= 0)
          steal_list.erase(itr);
      } else {
...
void Spell::EffectActivateRune(SpellEffIndex effIndex) {
  ...
  for (uint32 j = 0; j < MAX_RUNES && count > 0; ++j) {
    if (player->GetRuneCooldown(j) &&
      ...
      player->SetGracePeriod(j,
                             player->IsInCombat()); // xinef: reset grace period
      --count;
    }
  }
  ...
  if (m_spellInfo->Id == 45529 && count > 0) {
    for (uint32 l = 0; l < MAX_RUNES && count > 0; ++l) {
      // Check if both runes are on cd as that is the only time when this needs
      // to come into effect
...
void Spell::EffectGameObjectDamage(SpellEffIndex /*effIndex*/) {
  ...
  FactionTemplateEntry const *targetFaction = sFactionTemplateStore.LookupEntry(
...
void Spell::SummonGuardian(uint32 i, uint32 entry,
  ...
  for (uint32 count = 0; count < numGuardians; ++count) {
    ...
    // xinef: do not use precalculated position for effect summon pet in this
    // function it means it was cast by NPC and should have its position
    // overridden
    if (totalNumGuardians == 1 &&
    ...
    summon = map->SummonCreature(entry, pos, properties, duration, caster,
                                 m_spellInfo->Id);
    if (!summon)
      return;
    ...
    if (summon->HasUnitTypeMask(UNIT_MASK_MINION) && totalNumGuardians > 1)
      ((Minion *)summon)
          ->SetFollowAngle(m_caster->GetAbsoluteAngle(pos.GetPositionX(),
                                                      pos.GetPositionY()));
    ...
    if (!summon->IsInCombat() && !summon->IsTrigger()) {
      //  summon->AI()->EnterEvadeMode();
      summon->GetMotionMaster()->Clear(false);
      summon->GetMotionMaster()->MoveFollow(caster, PET_FOLLOW_DIST,
                                            summon->GetFollowAngle(),
                                            MOTION_SLOT_ACTIVE);
    }
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex) {
  ...
  if (!sSoundEntriesStore.LookupEntry(soundid)) {
    LOG_ERROR("server",
              "EffectPlayMusic: Sound (Id: %u) not exist in spell %u.", soundid,
              m_spellInfo->Id);
    return;
  }
...
void Spell::EffectPlaySound(SpellEffIndex effIndex) {
  ...
  if (!sSoundEntriesStore.LookupEntry(soundId)) {
    LOG_ERROR("server",
              "EffectPlayerSound: Sound (Id: %u) not exist in spell %u.",
...
void Spell::EffectCastButtons(SpellEffIndex effIndex) {
  ...
  for (; n_buttons; --n_buttons, ++button_id) {
    ...
    uint32 cost =
        spellInfo->CalcPowerCost(m_caster, spellInfo->GetSchoolMask(), this);
