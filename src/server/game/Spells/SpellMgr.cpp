...
void SpellMgr::LoadSpellLinked()
{
    ...
    do
    {
        ...
        if (!spellInfo)
        {
            LOG_ERROR("sql.sql", "Spell {} listed in `spell_linked_spell` does not exist", abs(trigger));
            continue;
        }
        spellInfo = GetSpellInfo(abs(effect));
        if (!spellInfo)
        {
            LOG_ERROR("sql.sql", "Spell {} listed in `spell_linked_spell` does not exist", abs(effect));
            continue;
        }
...
void SpellMgr::LoadSpellAreas()
{
    ...
    do
    {
        ...
        if (spellArea.auraSpell)
        {
            ...
            if (!spellInfo)
            {
                LOG_ERROR("sql.sql", "Spell {} listed in `spell_area` have wrong aura spell ({}) requirement", spell, abs(spellArea.auraSpell));
                continue;
            }

            if (uint32(abs(spellArea.auraSpell)) == spellArea.spellId)
            {
                LOG_ERROR("sql.sql", "Spell {} listed in `spell_area` have aura spell ({}) requirement for itself", spell, abs(spellArea.auraSpell));
                continue;
            }
...
void SpellMgr::LoadSpellCustomAttr()
{
    ...
    else
    {
        for (count = 0; result->NextRow(); ++count)
        {
            ...
            if (!spellInfo)
            {
                LOG_INFO("spells", "Table `spell_custom_attr` has wrong spell (spell_id: {}), ignored.", spellId);
                continue;
            }
            ...
            if ((attributes & SPELL_ATTR0_CU_NEGATIVE) != 0)
            {
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                {
                    ...
                    {
                        LOG_ERROR("sql.sql", "Table `spell_custom_attr` has attribute SPELL_ATTR0_CU_NEGATIVE_EFF{} for spell {} with no EFFECT_{}", uint32(i), spellId, uint32(i));
                        continue;
                    }
                }
            }
