  ...
  for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
    ...
    case SPELL_EFFECT_CREATE_ITEM_2: {
      if (spellInfo->Effects[i].ItemType == 0) {
        // skip auto-loot crafting spells, its not need explicit item info (but
        // have special fake items sometime)
        if (!spellInfo->IsLootCrafting()) {
          if (msg)
            LOG_ERROR("sql.sql", "Craft spell %u not have create item entry.",
                      spellInfo->Id);
          return false;
        }
      }
      // also possible IsLootCrafting case but fake item must exist anyway
        ...
        if (msg)
          LOG_ERROR("sql.sql",
                    "Craft spell %u create not-exist in DB item (Entry: %u) "
                    "and then...",
                    spellInfo->Id, spellInfo->Effects[i].ItemType);
        return false;
      }
      ...
      if (!ComputeIsSpellValid(spellInfo2, msg)) {
        if (msg)
          LOG_ERROR("sql.sql",
                    ...
                    spellInfo->Id, spellInfo->Effects[i].TriggerSpell);
        return false;
      }
      break;
    }
  ...
  if (need_check_reagents) {
    for (uint8 j = 0; j < MAX_SPELL_REAGENTS; ++j) {
      if (spellInfo->Reagent[j] > 0 &&
        ...
        if (msg)
          LOG_ERROR("sql.sql",
                    "Craft spell %u have not-exist reagent in DB item (Entry: "
                    "%u) and then...",
...
bool SpellMgr::CheckSpellValid(SpellInfo const *spellInfo, uint32 spellId,
                               bool isTalent) {
  if (!spellInfo) {
    DeleteSpellFromAllPlayers(spellId);
    LOG_ERROR("server",
              "Player::%s: Non-existed in SpellStore spell #%u request.",
              (isTalent ? "AddTalent" : "addSpell"), spellId);
    return false;
  }

  if (!IsSpellValid(spellInfo)) {
    DeleteSpellFromAllPlayers(spellId);
    LOG_ERROR("server", "Player::%s: Broken spell #%u learning not allowed.",
              (isTalent ? "AddTalent" : "addSpell"), spellId);
    return false;
  }
...
uint32 SpellMgr::GetSpellIdForDifficulty(uint32 spellId,
  ...
  if (mode >= MAX_DIFFICULTY) {
    LOG_ERROR(
        "server",
        "SpellMgr::GetSpellIdForDifficulty: Incorrect Difficulty for spell %u.",
        spellId);
  ...
  if (!difficultyEntry) {
    ...
    LOG_DEBUG("spells.aura",
              ...
              "found for spell %u. This should never happen.",
              spellId);
#endif
    return spellId; // return source spell
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "SpellMgr::GetSpellIdForDifficulty: spell %u mode %u spell is "
              "nullptr, using mode %u",
              spellId, mode, mode - 2);
#endif
    mode -= 2;
  }
  ...
  if (difficultyEntry->SpellID[mode] <= 0) {
    LOG_ERROR("sql.sql",
              "SpellMgr::GetSpellIdForDifficulty: spell %u mode %u spell is 0. "
              "Check spelldifficulty_dbc!",
              spellId, mode);
    return spellId;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura",
            "SpellMgr::GetSpellIdForDifficulty: spellid for spell %u in mode "
            "%u is %d",
            spellId, mode, difficultyEntry->SpellID[mode]);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("spells.aura",
              "SpellMgr::GetSpellForDifficultyFromSpell: spell %u not found. "
              "Check spelldifficulty_dbc!",
              newSpellId);
#endif
    return spell;
...
uint32 SpellMgr::GetSpellWithRank(uint32 spell_id, uint32 rank,
                                  bool strict) const {
  if (SpellChainNode const *node = GetSpellChainNode(spell_id)) {
    if (rank != node->rank)
      return GetSpellWithRank(
          node->rank < rank ? node->next->Id : node->prev->Id, rank, strict);
...
SpellMgr::GetSpellTargetPosition(uint32 spell_id,
...
SpellMgr::CheckSpellGroupStackRules(SpellInfo const *spellInfo1,
                                    SpellInfo const *spellInfo2, bool remove,
                                    bool areaAura) const {
...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(
    SpellInfo const *spellProto, SpellProcEventEntry const *spellProcEvent,
    uint32 EventProcFlag, SpellInfo const *procSpell, uint32 procFlags,
    uint32 procExtra, bool active) const {
  // No extra req need
  ...
  if (procFlags & PROC_FLAG_DONE_PERIODIC) {
    if (procExtra & PROC_EX_INTERNAL_HOT) {
      if (EventProcFlag == PROC_FLAG_DONE_PERIODIC) {
        /// no aura with only PROC_FLAG_DONE_PERIODIC and spellFamilyName == 0
        /// can proc from a HOT.
  ...
  } else // Passive spells hits here only if resist/reflect/immune/evade
  {
    if (procExtra & AURA_SPELL_PROC_EX_MASK) {
      ...
      if (procEvent_procEx & PROC_EX_EX_TRIGGER_ALWAYS)
        return true;
      // PROC_EX_NOT_ACTIVE_SPELL and PROC_EX_ONLY_ACTIVE_SPELL flags handle: if
      // passed checks before
...
PetDefaultSpellsEntry const *
SpellMgr::GetPetDefaultSpellsEntry(int32 id) const {
...
bool SpellArea::IsFitToRequirements(Player const *player, uint32 newZone,
  ...
  if (questStart) // not in expected required quest state
    if (!player ||
        (((1 << player->GetQuestStatus(questStart)) & questStartStatus) == 0))
      return false;
...
void SpellMgr::LoadSpellTalentRanks() {
  ...
  for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i) {
    ...
    if (!firstSpell) {
      LOG_ERROR("sql.sql",
                "SpellMgr::LoadSpellTalentRanks: First Rank Spell %u for "
                "TalentEntry %u does not exist.",
                talentInfo->RankID[0], i);
      continue;
    }
    ...
    for (uint8 rank = 0; rank < MAX_TALENT_RANK; ++rank) {
      ...
      if (!currentSpell) {
        LOG_ERROR("sql.sql",
                  "SpellMgr::LoadSpellTalentRanks: Spell %u (Rank: %u) for "
                  "TalentEntry %u does not exist.",
                  spellId, rank + 1, i);
        break;
      }
...
void SpellMgr::LoadSpellRanks() {
  ...
  do {
    ...
    if (!first) {
      LOG_ERROR("sql.sql",
                "Spell rank identifier(first_spell_id) %u listed in "
    ...
    if (rankChain.size() < 2) {
      LOG_ERROR("sql.sql",
                "There is only 1 spell rank for identifier(first_spell_id) %u "
    ...
    for (std::list<std::pair<int32, int32>>::iterator itr = rankChain.begin();
         itr != rankChain.end(); ++itr) {
      SpellInfo const *spell = GetSpellInfo(itr->first);
      if (!spell) {
        LOG_ERROR("sql.sql",
                  "Spell %u (rank %u) listed in `spell_ranks` for chain %u "
                  "does not exist!",
      ...
      if (itr->second != curRank) {
        LOG_ERROR("sql.sql",
                  "Spell %u (rank %u) listed in `spell_ranks` for chain %u "
                  "does not have proper rank value(should be %u)!",
  ...
  LOG_INFO("server", ">> Loaded %u spell rank records in %u ms", count,
...
void SpellMgr::LoadSpellRequired() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell required records. DB table "
  ...
  do {
    ...
    if (!reqSpellInfo) {
      LOG_ERROR("sql.sql",
                "req_spell %u in `spell_required` table is not found in dbcs, "
                "skipped",
                spellReq);
      continue;
    }
    ...
    if (GetFirstSpellInChain(spellId) == GetFirstSpellInChain(spellReq)) {
      LOG_ERROR("sql.sql",
                "req_spell %u and spell_id %u in `spell_required` table are "
                "ranks of the same spell, entry not needed, skipped",
                spellReq, spellId);
      continue;
    }
    ...
    if (IsSpellRequiringSpell(spellId, spellReq)) {
      LOG_ERROR("sql.sql",
                "duplicated entry of req_spell %u and spell_id %u in "
    ...
    // xinef: fill additionalTalentInfo data, currently Blessing of Sanctuary
    // only
    if (GetTalentSpellCost(spellReq) > 0)
      mTalentSpellAdditionalSet.insert(spellId);
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %u spell required records in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellLearnSkills() {
  ...
  LOG_INFO("server", ">> Loaded %u Spell Learn Skills from DBC in %u ms",
...
void SpellMgr::LoadSpellTargetPositions() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell target coordinates. DB table "
  ...
  do {
    ...
    if (!mapEntry) {
      LOG_ERROR("sql.sql",
                "Spell (Id: %u, effIndex: %u) target map (ID: %u) does not "
                "exist in `Map.dbc`.",
                Spell_ID, effIndex, st.target_mapId);
      continue;
    }

    if (st.target_X == 0 && st.target_Y == 0 && st.target_Z == 0) {
      LOG_ERROR("sql.sql",
                "Spell (Id: %u, effIndex: %u) target coordinates not provided.",
                Spell_ID, effIndex);
      continue;
    }
    ...
    if (spellInfo->Effects[effIndex].TargetA.GetTarget() == TARGET_DEST_DB ||
      ...
      LOG_ERROR(
          "sql.sql",
          "Spell (Id: %u, effIndex: %u) listed in `spell_target_position` does "
          "not have target TARGET_DEST_DB (17).",
          Spell_ID, effIndex);
      continue;
    }
  } while (result->NextRow());
  ...
  #if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
              LOG_DEBUG("spells.aura", "Spell (ID: %u) does not have record in
  `spell_target_position`", i); #endif
      }
  }*/

  LOG_INFO("server", ">> Loaded %u spell teleport coordinates in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellGroups() {
  ...
  //                                                0     1            2
  QueryResult result =
      WorldDatabase.Query("SELECT id, spell_id, special_flag FROM spell_group");
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell group definitions. DB table "
                       "`spell_group` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    SpellGroupSpecialFlags specialFlag =
        (SpellGroupSpecialFlags)fields[2].GetUInt32();
    SpellInfo const *spellInfo = GetSpellInfo(spell_id);

    if (!spellInfo) {
      LOG_ERROR("sql.sql", "Spell %u listed in `spell_group` does not exist",
                spell_id);
      continue;
    ...
    if (mSpellGroupMap.find(spell_id) != mSpellGroupMap.end()) {
      LOG_ERROR("sql.sql",
                "Spell %u listed in `spell_group` has more than one group",
                spell_id);
      continue;
    }
    ...
    if (specialFlag >= SPELL_GROUP_SPECIAL_FLAG_MAX) {
      LOG_ERROR("sql.sql",
                "Spell %u listed in `spell_group` has invalid special flag!",
                spell_id);
      continue;
    }
  ...
  LOG_INFO("server", ">> Loaded %u spell group definitions in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellGroupStackRules() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell group stack rules. DB table "
  ...
  do {
    ...
    if (stack_rule >= SPELL_GROUP_STACK_FLAG_MAX) {
      LOG_ERROR("sql.sql",
                "SpellGroupStackRule %u listed in `spell_group_stack_rules` "
                "does not exist",
                stack_rule);
      continue;
    }
  ...
  LOG_INFO("server", ">> Loaded %u spell group stack rules in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellProcEvents() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell proc event conditions. DB table "
  ...
  do {
    ...
    if (allRanks) {
      if (!spellInfo->IsRanked())
        LOG_ERROR("sql.sql",
                  "Spell %u listed in `spell_proc_event` with all ranks, but "
                  "spell has no ranks.",
                  spellId);
      ...
      if (spellInfo->GetFirstRankSpell()->Id != uint32(spellId)) {
        LOG_ERROR(
            "sql.sql",
            "Spell %u listed in `spell_proc_event` is not first rank of spell.",
            spellId);
        continue;
      }
    }
    ...
    while (spellInfo) {
      if (mSpellProcEventMap.find(spellInfo->Id) != mSpellProcEventMap.end()) {
        LOG_ERROR("sql.sql",
                  "Spell %u listed in `spell_proc_event` already has its first "
                  "rank in table.",
                  spellInfo->Id);
        break;
      }
  ...
  LOG_INFO("server", ">> Loaded %u extra spell proc event conditions in %u ms",
...
void SpellMgr::LoadSpellProcs() {
  ...
  QueryResult result = WorldDatabase.Query(
      ...
      "spellFamilyMask1, spellFamilyMask2, typeMask, spellTypeMask, "
      "spellPhaseMask, hitMask, attributesMask, ratePerMinute, chance, "
      "cooldown, charges FROM spell_proc");
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell proc conditions and data. DB table "
                       "`spell_proc` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    SpellInfo const *spellInfo = GetSpellInfo(spellId);
    if (!spellInfo) {
      LOG_ERROR("sql.sql", "Spell %u listed in `spell_proc` does not exist",
                spellId);
      continue;
    }
    ...
    if (allRanks) {
      if (spellInfo->GetFirstRankSpell()->Id != uint32(spellId)) {
        LOG_ERROR("sql.sql",
                  "Spell %u listed in `spell_proc` is not first rank of spell.",
                  fields[0].GetInt32());
        continue;
      }
    }
    ...
    while (spellInfo) {
      if (mSpellProcMap.find(spellInfo->Id) != mSpellProcMap.end()) {
        LOG_ERROR(
            "sql.sql",
            "Spell %u listed in `spell_proc` has duplicate entry in the table",
            spellId);
        break;
      }
      ...
      if (procEntry.schoolMask & ~SPELL_SCHOOL_MASK_ALL)
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has wrong "
                  "`schoolMask` set: %u",
                  spellId, procEntry.schoolMask);
      if (procEntry.spellFamilyName &&
          (procEntry.spellFamilyName < 3 || procEntry.spellFamilyName > 17 ||
           procEntry.spellFamilyName == 14 || procEntry.spellFamilyName == 16))
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has wrong "
                  "`spellFamilyName` set: %u",
                  spellId, procEntry.spellFamilyName);
      if (procEntry.chance < 0) {
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has negative value "
                  "in `chance` field",
                  spellId);
        procEntry.chance = 0;
      }
      ...
      if (procEntry.chance == 0 && procEntry.ratePerMinute == 0)
        LOG_ERROR(
            "sql.sql",
            "`spell_proc` table entry for spellId %u doesn't have `chance` and "
            "`ratePerMinute` values defined, proc will not be triggered",
            spellId);
      if (procEntry.charges > 99) {
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has too big value "
                  "in `charges` field",
                  spellId);
      ...
      if (!procEntry.typeMask)
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u doesn't have "
                  "`typeMask` value defined, proc will not be triggered",
                  spellId);
      if (procEntry.spellTypeMask & ~PROC_SPELL_TYPE_MASK_ALL)
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has wrong "
                  "`spellTypeMask` set: %u",
                  spellId, procEntry.spellTypeMask);
      if (procEntry.spellTypeMask &&
          !(procEntry.typeMask &
            (SPELL_PROC_FLAG_MASK | PERIODIC_PROC_FLAG_MASK)))
        LOG_ERROR(
            "sql.sql",
            "`spell_proc` table entry for spellId %u has `spellTypeMask` value "
            "defined, but it won't be used for defined `typeMask` value",
            spellId);
      if (!procEntry.spellPhaseMask &&
          procEntry.typeMask & REQ_SPELL_PHASE_PROC_FLAG_MASK)
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u doesn't have "
                  "`spellPhaseMask` value defined, but it's required for "
                  "defined `typeMask` value, proc will not be triggered",
                  spellId);
      if (procEntry.spellPhaseMask & ~PROC_SPELL_PHASE_MASK_ALL)
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has wrong "
                  "`spellPhaseMask` set: %u",
                  spellId, procEntry.spellPhaseMask);
      if (procEntry.spellPhaseMask &&
          !(procEntry.typeMask & REQ_SPELL_PHASE_PROC_FLAG_MASK))
        LOG_ERROR(
            "sql.sql",
            "`spell_proc` table entry for spellId %u has `spellPhaseMask` "
            "value defined, but it won't be used for defined `typeMask` value",
            spellId);
      if (procEntry.hitMask & ~PROC_HIT_MASK_ALL)
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has wrong `hitMask` "
                  "set: %u",
                  spellId, procEntry.hitMask);
      if (procEntry.hitMask &&
          !(procEntry.typeMask & TAKEN_HIT_PROC_FLAG_MASK ||
            (procEntry.typeMask & DONE_HIT_PROC_FLAG_MASK &&
             (!procEntry.spellPhaseMask ||
              procEntry.spellPhaseMask &
                  (PROC_SPELL_PHASE_HIT | PROC_SPELL_PHASE_FINISH)))))
        LOG_ERROR("sql.sql",
                  "`spell_proc` table entry for spellId %u has `hitMask` value "
                  "defined, but it won't be used for defined `typeMask` and "
                  "`spellPhaseMask` values",
                  spellId);
  ...
  LOG_INFO("server", ">> Loaded %u spell proc conditions and data in %u ms",
           count, GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellBonusess() {
  ...
  LOG_INFO("server", ">> Loaded %u extra spell bonus data in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellThreats() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 aggro generating spells. DB table "
                       "`spell_threat` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    if (!GetSpellInfo(entry)) {
      LOG_ERROR("sql.sql", "Spell %u listed in `spell_threat` does not exist",
                entry);
      continue;
    }
  ...
  LOG_INFO("server", ">> Loaded %u SpellThreatEntries in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellMixology() {
  ...
  if (!result) {
    LOG_INFO(
        "server",
        ">> Loaded 0 mixology bonuses. DB table `spell_mixology` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    if (!GetSpellInfo(entry)) {
      LOG_ERROR("sql.sql", "Spell %u listed in `spell_mixology` does not exist",
                entry);
      continue;
    }
  ...
  LOG_INFO("server", ">> Loaded %u Mixology bonuses in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSkillLineAbilityMap() {
  ...
  LOG_INFO("server", ">> Loaded %u SkillLineAbility MultiMap Data in %u ms",
           count, GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellPetAuras() {
  ...
  LOG_INFO("server", ">> Loaded %u spell pet auras in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadEnchantCustomAttr() {
  ...
  LOG_INFO("server", ">> Loaded %u custom enchant attributes in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellEnchantProcData() {
  ...
  //                                                  0         1           2 3
  QueryResult result =
      WorldDatabase.Query("SELECT entry, customChance, PPMChance, procEx FROM "
                          "spell_enchant_proc_data");
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 spell enchant proc event conditions. DB "
                       "table `spell_enchant_proc_data` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  LOG_INFO("server", ">> Loaded %u enchant proc data definitions in %u ms",
           count, GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellLinked() {
  ...
  LOG_INFO("server", ">> Loaded %u linked spells in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
  ...
  for (uint32 i = 0; i < sCreatureFamilyStore.GetNumRows(); ++i) {
    ...
    for (uint8 j = 0; j < 2; ++j) {
      ...
      for (uint32 k = 0; k < sSkillLineAbilityStore.GetNumRows(); ++k) {
        ...
        //    (!creatureFamily->skillLine[1] || skillLine->skillId !=
        //    creatureFamily->skillLine[1])) continue;
        ...
        spellSet.insert(
            PetLevelupSpellSet::value_type(spell->SpellLevel, spell->Id));
        ++count;
      }
    }
  }
...
bool LoadPetDefaultSpells_helper(CreatureTemplate const *cInfo,
  ...
  if (PetLevelupSpellSet const *levelupSpells =
    ...
    for (uint8 j = 0; j < MAX_CREATURE_SPELL_DATA_SLOT; ++j) {
      ...
      for (PetLevelupSpellSet::const_iterator itr = levelupSpells->begin();
           itr != levelupSpells->end(); ++itr) {
...
void SpellMgr::LoadPetDefaultSpells() {
  ...
  for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
    ...
    for (uint8 k = 0; k < MAX_SPELL_EFFECTS; ++k) {
      if (spellEntry->Effects[k].Effect == SPELL_EFFECT_SUMMON ||
          spellEntry->Effects[k].Effect == SPELL_EFFECT_SUMMON_PET) {
        ...
        // for creature without PetSpellDataId get default pet spells from
        // creature_template
  ...
  LOG_INFO("server", ">> Loaded %u summonable creature templates in %u ms",
...
void SpellMgr::LoadSpellAreas() {
  ...
  //                                                  7       8         9
  QueryResult result = WorldDatabase.Query(
      "SELECT spell, area, quest_start, quest_start_status, quest_end_status, "
      "quest_end, aura_spell, racemask, gender, autocast FROM spell_area");
  ...
  do {
    ...
    if (SpellInfo const *spellInfo = GetSpellInfo(spell)) {
      ...
      LOG_ERROR("sql.sql", "Spell %u listed in `spell_area` does not exist",
                spell);
      continue;
    }

    {
      ...
      if (!ok) {
        LOG_ERROR("sql.sql",
                  "Spell %u listed in `spell_area` already listed with similar "
                  "requirements.",
                  spell);
        continue;
      }
    ...
    if (spellArea.auraSpell) {
      ...
      if (!spellInfo) {
        LOG_ERROR("sql.sql",
                  "Spell %u listed in `spell_area` have wrong aura spell (%u) "
                  "requirement",
  ...
  if (sWorld->getIntConfig(CONFIG_ICC_BUFF_HORDE) > 0) {
    LOG_INFO("server", ">> Using ICC buff Horde: %u",
  ...
  if (sWorld->getIntConfig(CONFIG_ICC_BUFF_ALLIANCE) > 0) {
    LOG_INFO("server", ">> Using ICC buff Alliance: %u",
  ...
  LOG_INFO("server", ">> Loaded %u spell area requirements in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellInfoStore() {
  ...
  LOG_INFO("server", ">> Loaded spell custom attributes in %u ms",
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellSpecificAndAuraState() {
  ...
  LOG_INFO("server", ">> Loaded spell specific and aura state in %u ms",
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void SpellMgr::LoadSpellCustomAttr() {
  ...
  QueryResult result =
      WorldDatabase.Query("SELECT spell_id, attributes FROM spell_custom_attr");

  if (!result)
    LOG_INFO("server", ">> Loaded 0 spell custom attributes from DB. DB table "
    ...
    for (count = 0; result->NextRow(); ++count) {
      ...
      if (!spellInfo) {
        LOG_INFO("server",
                 "Table `spell_custom_attr` has wrong spell (spell_id: %u), "
                 "ignored.",
                 spellId);
        continue;
      }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
          ...
          if ((attributes & (SPELL_ATTR0_CU_NEGATIVE_EFF0 << i)) != 0) {
            LOG_ERROR(
                ...
                "SPELL_ATTR0_CU_NEGATIVE_EFF%u for spell %u with no EFFECT_%u",
                uint32(i), spellId, uint32(i));
            continue;
          }
        }
      }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
          ...
          if ((attributes & (SPELL_ATTR0_CU_POSITIVE_EFF0 << i)) != 0) {
            LOG_ERROR(
                "sql.sql",
                "Table `spell_custom_attr` has attribute "
                "SPELL_ATTR0_CU_POSITIVE_EFF%u for spell %u with no EFFECT_%u",
                uint32(i), spellId, uint32(i));
            continue;
          }
        }
    ...
    LOG_INFO("server", ">> Loaded %u spell custom attributes from DB in %u ms",
             count, GetMSTimeDiffToNow(customAttrTime));
  }
        ...
        if (const SpellInfo *spellInfo = GetSpellInfo(spellId))
          for (uint8 k = 0; k < MAX_SPELL_EFFECTS; ++k)
            if (spellInfo->Effects[k].Effect == SPELL_EFFECT_LEARN_SPELL)
              if (const SpellInfo *learnSpell =
                      GetSpellInfo(spellInfo->Effects[k].TriggerSpell))
                if (learnSpell->IsRanked() &&
                    !learnSpell->HasAttribute(SpellAttr0(
  ...
  for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i) {
      ...
      case SPELL_EFFECT_ENCHANT_HELD_ITEM: {
        ...
        if (IsPartOfSkillLine(SKILL_ENCHANTING, i)) {
          ...
          for (uint8 s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; ++s) {
            ...
            SpellInfo *procInfo =
                (SpellInfo *)GetSpellInfo(enchant->spellid[s]);
            if (!procInfo)
              continue;

            // if proced directly from enchantment, not via proc aura
            // NOTE: Enchant Weapon - Blade Ward also has proc aura spell and is
            // proced directly however its not expected to stack so this check
            // is good
        ...
        if (spellInfo->Effects[j].Effect) {
          ...
          default:
            if (spellInfo->Effects[j].CalcValue() ||
              ...
              if (spellInfo->Id != 69649 && spellInfo->Id != 71056 &&
                  spellInfo->Id != 71057 && spellInfo->Id != 71058 &&
                  spellInfo->Id != 73061 && spellInfo->Id != 73062 &&
                  spellInfo->Id != 73063 &&
                  spellInfo->Id != 73064) // Sindragosa Frost Breath
                if (spellInfo->SpellFamilyName != SPELLFAMILY_MAGE ||
                    !(spellInfo->SpellFamilyFlags[0] & 0x20)) // frostbolt
                  if (spellInfo->Id != 55095)                 // frost fever
                    if (spellInfo->SpellFamilyName != SPELLFAMILY_WARLOCK ||
                        !(spellInfo->SpellFamilyFlags[1] & 0x40000)) // Haunt
                    {
    ...
    case 13877: // Blade Flurry (Rogue Spell) should ignore armor and share
                // damage to 2nd mob
      spellInfo->AttributesCu |= SPELL_ATTR0_CU_SHARE_DAMAGE;
      spellInfo->AttributesCu |= SPELL_ATTR0_CU_IGNORE_ARMOR;
      break;
    ...
    case 72465: // Icecrown Citadel, Sindragosa, Respite for a Tormented Soul
                // (weekly quest)
      ...
      if (spellInfo->Effects[j].ApplyAuraName &&
        ...
        case SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE:
          if (SpellInfo const *triggerSpell =
                  sSpellMgr->GetSpellInfo(spellInfo->Effects[j].TriggerSpell)) {
            overrideAttr = true;
...
inline void ApplySpellFix(std::initializer_list<uint32> spellIds,
  ...
  for (uint32 spellId : spellIds) {
    ...
    if (!spellInfo) {
      LOG_ERROR("sql.sql",
                "Spell info correction specified for non-existing spell %u",
                spellId);
      continue;
    }
...
void SpellMgr::LoadDbcDataCorrections() {
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->MaxAffectedTargets = 1; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->EffectMiscValueB[0] = 64; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) {
        spellInfo->AttributesEx4 &=
            ~SPELL_ATTR4_CAN_CAST_WHILE_CASTING; // Crashes client on pressing
                                                 // ESC
      });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->Effect[0] = 0; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) {
        spellInfo->AttributesEx3 |=
            SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED; // Entries were not updated
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->MaxAffectedTargets = 3; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->MaxAffectedTargets = 10; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ProcCharges = 1; });

  // Tidal Wave
  ApplySpellFix({53390},
                [](SpellEntry *spellInfo) { spellInfo->ProcCharges = 2; });
  ...
  ApplySpellFix({28200},
                [](SpellEntry *spellInfo) { spellInfo->ProcCharges = 6; });
  ...
  ApplySpellFix({47569}, [](SpellEntry *spellInfo) {
    spellInfo->Attributes &=
        ~SPELL_ATTR0_NOT_SHAPESHIFT; // with this spell atrribute aura can be
  ...
  ApplySpellFix({16834, 16835},
                [](SpellEntry *spellInfo) { spellInfo->DurationIndex = 21; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->RangeIndex = 13; });
  ...
  ApplySpellFix({54800}, [](SpellEntry *spellInfo) {
    ...
    // this is done because another spell also uses the same SpellFamilyFlags as
    // Icy Touch SpellFamilyFlags[0] & 0x00000040 in SPELLFAMILY_DEATHKNIGHT is
    // currently unused (3.3.5a) this needs research on modifier applying rules,
    // does not seem to be in Attributes fields
    spellInfo->EffectSpellClassMask[0] =
        flag96(0x00000040, 0x00000000, 0x00000000);
  });

  // Idol of the Flourishing Life
  ApplySpellFix({64949}, [](SpellEntry *spellInfo) {
    spellInfo->EffectSpellClassMask[EFFECT_0] =
  ...
  ApplySpellFix({45602}, [](SpellEntry *spellInfo) {
    spellInfo->EffectBasePoints[EFFECT_0] =
        0; // force seat 0, vehicle doesn't have the required seat flags for "no
           // seat specified (-1)"
  });
  ...
  ApplySpellFix({34490, 41084, 42671},
                [](SpellEntry *spellInfo) { spellInfo->Speed = 0.0f; });
  ...
  // Kill Command
  ApplySpellFix({34027},
                [](SpellEntry *spellInfo) { spellInfo->ProcCharges = 0; });
  ...
  // Envenom
  ApplySpellFix({32645, 32684, 57992, 57993},
                [](SpellEntry *spellInfo) { spellInfo->Dispel = DISPEL_NONE; });

  //////////////////////////////////////////
  ////////// ULDUAR
  //////////////////////////////////////////
  ApplySpellFix({64014, 64032, 64028, 64031, 64030, 64029, 64024, 64025, 65042},
                [](SpellEntry *spellInfo) {
  ...
  // Chains of Ice
  ApplySpellFix({45524},
                [](SpellEntry *spellInfo) { spellInfo->Effect[EFFECT_2] = 0; });
  ...
  ApplySpellFix({60549},
                [](SpellEntry *spellInfo) { spellInfo->Effect[1] = 0; });
  ...
  ApplySpellFix({49576, 49560}, [](SpellEntry *spellInfo) {
    ...
    //  should fix taunt on bosses and not break the pull protection at the same
    //  time (no aura provides immunity to grip mechanic)
  ...
  ApplySpellFix({47201, 47202, 47203, 47204, 47205}, [](SpellEntry *spellInfo) {
    spellInfo->EffectSpellClassMask[1][0] |=
  ...
  ApplySpellFix({54347, 54348, 54349}, [](SpellEntry *spellInfo) {
    ...
    spellInfo->EffectApplyAuraName[EFFECT_1] =
        spellInfo->EffectApplyAuraName[EFFECT_0];
    spellInfo->EffectImplicitTargetA[EFFECT_1] =
        spellInfo->EffectImplicitTargetA[EFFECT_0];
    spellInfo->EffectMiscValue[EFFECT_0] = SPELLMOD_EFFECT1;
    spellInfo->EffectMiscValue[EFFECT_1] = SPELLMOD_EFFECT2;
  });
  ...
  ApplySpellFix({29441, 29444},
                [](SpellEntry *spellInfo) { spellInfo->SpellLevel = 0; });
  ...
  ApplySpellFix(
      {7405, 8380, 11596, 11597, 25225, 47467}, [](SpellEntry *spellInfo) {
  ...
  // Playback Speech
  ApplySpellFix({74209},
                [](SpellEntry *spellInfo) { spellInfo->DurationIndex = 1; });
  ...
  ApplySpellFix({46638}, [](SpellEntry *spellInfo) {
    spellInfo->AttributesEx3 &=
        ~SPELL_ATTR3_ONLY_TARGET_PLAYERS; // Obvious fail, it targets
                                          // gameobject...
  });
  ...
  // Armageddon
  ApplySpellFix({45909},
                [](SpellEntry *spellInfo) { spellInfo->Speed = 8.0f; });
  ...
  ApplySpellFix(
      {
          ...
          56438, // Arcane Overload (-50% dmg taken) - this is to prevent apply
                 // -> unapply -> apply ... dunno whether it's correct
      },
      [](SpellEntry *spellInfo) {
        spellInfo->AttributesEx3 |= SPELL_ATTR3_STACK_FOR_DIFF_CASTERS;
      });
  ...
  // Hurl Pyrite
  ApplySpellFix({62490},
                [](SpellEntry *spellInfo) { spellInfo->Effect[EFFECT_1] = 0; });
  ...
  // Protective Gaze
  ApplySpellFix({64175},
                [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 25000; });

  // Shadow Beacon
  ApplySpellFix({64465}, [](SpellEntry *spellInfo) {
    spellInfo->EffectTriggerSpell[0] =
        64467; // why do they need two script effects :/ (this one has visual
               // effect)
  });
  ...
  ApplySpellFix({17647},
                [](SpellEntry *spellInfo) { spellInfo->DurationIndex = 37; });
  ...
  ApplySpellFix({17731, 69294}, [](SpellEntry *spellInfo) {
    ...
    spellInfo->EffectRadiusIndex[1] =
        19; // 18yd instead of 13yd to make sure all cracks erupt
  });
      ...
      [](SpellEntry *spellInfo) {
        ...
        if (spellInfo->Effect[1]) {
          ...
          spellInfo->EffectAmplitude[1] =
              ((spellInfo->CastingTimeIndex == 170) ? 50 : 215);
        }
      });
  ...
  // Trial of the Crusader, Gormok, player vehicle spell, CUSTOM! (default jump
  // to hand, not used)
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 65686; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67222; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67223; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67224; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 65684; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67176; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67177; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67178; });
  ...
  ApplySpellFix({69604, 70286}, [](SpellEntry *spellInfo) {
    spellInfo->MaxAffectedTargets = 1;
    spellInfo->AttributesEx3 |=
        (SPELL_ATTR3_IGNORE_HIT_RESULT | SPELL_ATTR3_ONLY_TARGET_PLAYERS);
  });
  ...
  ApplySpellFix({69029, 70850},
                [](SpellEntry *spellInfo) { spellInfo->Effect[2] = 0; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) {
        ...
        spellInfo->EffectImplicitTargetB[0] = 0;
        spellInfo->EffectImplicitTargetA[1] =
            TARGET_DEST_DB; // this target is for SPELL_EFFECT_TELEPORT_UNITS
        spellInfo->EffectImplicitTargetB[1] = 0;
        spellInfo->EffectImplicitTargetA[2] = TARGET_UNIT_TARGET_ANY;
        spellInfo->EffectImplicitTargetB[2] = 0;
      });
  ...
  ApplySpellFix({72701, 72702, 72703, 72704}, [](SpellEntry *spellInfo) {
    ...
    spellInfo->EffectImplicitTargetA[1] = TARGET_DEST_DEST;
    spellInfo->EffectImplicitTargetB[1] = 0;
    spellInfo->DurationIndex = 9; // 30 secs instead of 12, need him for longer,
                                  // but will stop his actions after 12 secs
  });
  ...
  ApplySpellFix({72255, 72444, 72445, 72446}, [](SpellEntry *spellInfo) {
    ...
    // power from Mark of the Fallen Champion. prevented in script, effect
    // needed for Prayer of Mending
    spellInfo->AttributesEx3 &= ~SPELL_ATTR3_CANT_TRIGGER_PROC;
  });
  ...
  ApplySpellFix({70530}, [](SpellEntry *spellInfo) {
    spellInfo->Effect[0] =
        SPELL_EFFECT_APPLY_AURA; // blizzard typo, 65 instead of 6, aura itself
                                 // is defined (dummy)
  });
  ...
  ApplySpellFix({71266, 72890}, [](SpellEntry *spellInfo) {
    spellInfo->AreaGroupId = 0; // originally, these require area 4522, which
                                // is... outside of Icecrown Citadel
  });
  ...
  ApplySpellFix({70227, 70232}, [](SpellEntry *spellInfo) {
    spellInfo->AreaGroupId = 2452; // Whole icc instead of Crimson Halls only,
                                   // remove when area calculation is fixed
  });
  ...
  // Meteor Strike
  ApplySpellFix({74637}, [](SpellEntry *spellInfo) { spellInfo->Speed = 0; });
  ...
  // Jormungar Strike
  ApplySpellFix({56513},
                [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 2000; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 3000; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 6000; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 10000; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 12000; });

  // Signal Helmet to Attack
  ApplySpellFix({51748},
                [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 15000; });
  ...
  ApplySpellFix(
      ...
      [](SpellEntry *spellInfo) { spellInfo->RecoveryTime = 20000; });
  ...
  ApplySpellFix(
      {7764, 10767, 16610, 16612, 16618, 17013}, [](SpellEntry *spellInfo) {
        spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
        spellInfo->AttributesEx5 |= SPELL_ATTR5_SKIP_CHECKCAST_LOS_CHECK;
      });
  ...
  ApplySpellFix({72623}, [](SpellEntry *spellInfo) {
    spellInfo->EffectBasePoints[EFFECT_0] =
        spellInfo->EffectBasePoints[EFFECT_1];
  });
  ...
  for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i) {
    ...
    for (uint8 j = 0; j < 3; ++j) {
      if (spellInfo->RangeIndex == 1 &&
          (spellInfo->EffectImplicitTargetA[j] == TARGET_DEST_TRAJ ||
           spellInfo->EffectImplicitTargetB[j] == TARGET_DEST_TRAJ)) {
        if (SpellEntry *spellInfo2 = (SpellEntry *)sSpellStore.LookupEntry(
                spellInfo->EffectTriggerSpell[j])) {
    ...
    case SPELLFAMILY_PALADIN:
      ...
      break;
    case SPELLFAMILY_DEATHKNIGHT:
      // Icy Touch - extend FamilyFlags (unused value) for Sigil of the Frozen
      // Conscience to use
  ...
  LOG_INFO("server", ">> Loading spell dbc data corrections  in %u ms",
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
