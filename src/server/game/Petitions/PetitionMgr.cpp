...
void PetitionMgr::LoadPetitions() {
  ...
  do {
    ...
    AddPetition(fields[1].GetUInt32(), fields[0].GetUInt32(),
                fields[2].GetString(), fields[3].GetUInt8());
    ++count;
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %d Petitions in %u ms", count,
...
void PetitionMgr::LoadSignatures() {
  ...
  do {
    Field *fields = result->Fetch();
    AddSignature(fields[0].GetUInt32(), fields[2].GetUInt32(),
                 fields[1].GetUInt32());
    ++count;
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %d Petition signs in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}

void PetitionMgr::AddPetition(uint32 petitionId, uint32 ownerGuid,
                              std::string const &name, uint8 type) {
...
Petition const *PetitionMgr::GetPetitionByOwnerWithType(uint32 ownerGuid,
                                                        uint8 type) const {
  for (PetitionContainer::const_iterator itr = PetitionStore.begin();
       itr != PetitionStore.end(); ++itr)
...
void PetitionMgr::AddSignature(uint32 petitionId, uint32 accountId,
                               uint32 playerGuid) {
  Signatures &s = SignatureStore[petitionId];
...
void PetitionMgr::RemoveSignaturesByPlayerAndType(uint32 playerGuid,
                                                  uint8 type) {
  for (SignatureContainer::iterator itr = SignatureStore.begin();
       itr != SignatureStore.end(); ++itr) {
