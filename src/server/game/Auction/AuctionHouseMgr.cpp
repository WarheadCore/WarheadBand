...
AuctionHouseObject* AuctionHouseMgr::GetAuctionsMapByHouseId(uint8 auctionHouseId)
{
    ...
    if (auctionHouseId == AUCTIONHOUSE_ALLIANCE)
        return &mAllianceAuctions;
    else if (auctionHouseId == AUCTIONHOUSE_HORDE)
        return &mHordeAuctions;
...
{
    uint32 minDeposit{ uint32(float(AH_MINIMUM_DEPOSIT) * CONF_GET_FLOAT("Rate.Auction.Deposit")) };
...
{
    return Warhead::Containers::MapGetValuePtr(_items, itemGuid);
}
