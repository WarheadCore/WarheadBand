...
bool AuctionBotConfig::Initialize()
{
    ...
    if (!GetConfig(CONFIG_AHBOT_ALLIANCE_ITEM_AMOUNT_RATIO) && !GetConfig(CONFIG_AHBOT_HORDE_ITEM_AMOUNT_RATIO) && !GetConfig(CONFIG_AHBOT_NEUTRAL_ITEM_AMOUNT_RATIO) &&
    ...
    if (CONF_GET_BOOL("AllowTwoSide.Interaction.Auction"))
    {
        ...
        if (GetConfig(CONFIG_AHBOT_ALLIANCE_ITEM_AMOUNT_RATIO) || GetConfig(CONFIG_AHBOT_HORDE_ITEM_AMOUNT_RATIO)
    ...
    if (uint32 ahBotAccId = GetConfig(CONFIG_AHBOT_ACCOUNT_ID))
    {
        ...
        if (AccountMgr::GetCharactersCount(GetConfig(CONFIG_AHBOT_ACCOUNT_ID)))
        {
            ...
            auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARS_BY_ACCOUNT_ID);
            stmt->SetArguments(ahBotAccId);
            ...
            if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            {
                do
                {
                    ...
                    _AHBotCharacters.emplace_back(fields[0].Get<uint32>());
                } while (result->NextRow());
            }

            LOG_DEBUG("ahbot", "AuctionHouseBot found {} characters", _AHBotCharacters.size());
        }
        else
            LOG_WARN("ahbot", "AuctionHouseBot Account ID {} has no associated characters.", ahBotAccId);
    }

    return true;
}

void AuctionBotConfig::SetConfig(AuctionBotConfigUInt32Values index, std::string_view fieldname, uint32 defvalue)
{
    SetConfig(index, sGameConfig->GetOption<uint32>(fieldname, defvalue));

    if (int32(GetConfig(index)) < 0)
    {
        LOG_ERROR("ahbot", "AHBot: {} ({}) can't be negative. Using {} instead.", fieldname, int32(GetConfig(index)), defvalue);
        SetConfig(index, defvalue);
    }
}

void AuctionBotConfig::SetConfigMax(AuctionBotConfigUInt32Values index, std::string_view fieldname, uint32 defvalue, uint32 maxvalue)
{
    SetConfig(index, sGameConfig->GetOption<uint32>(fieldname, defvalue));

    if (GetConfig(index) > maxvalue)
    {
        LOG_ERROR("ahbot", "AHBot: {} ({}) must be in range 0...{}. Using {} instead.", fieldname, GetConfig(index), maxvalue, maxvalue);
        SetConfig(index, maxvalue);
    }
}

void AuctionBotConfig::SetConfigMinMax(AuctionBotConfigUInt32Values index, std::string_view fieldname, uint32 defvalue, uint32 minvalue, uint32 maxvalue)
{
    SetConfig(index, sGameConfig->GetOption<uint32>(fieldname, defvalue));

    if (GetConfig(index) > maxvalue)
    {
        LOG_ERROR("ahbot", "AHBot: {} ({}) must be in range {}...{}. Using {} instead.", fieldname, GetConfig(index), minvalue, maxvalue, maxvalue);
        SetConfig(index, maxvalue);
    }

    if (GetConfig(index) < minvalue)
    {
        LOG_ERROR("ahbot", "AHBot: {} ({}) must be in range {}...{}. Using {} instead.", fieldname, GetConfig(index), minvalue, maxvalue, minvalue);
        SetConfig(index, minvalue);
    }
}

void AuctionBotConfig::SetConfig(AuctionBotConfigBoolValues index, std::string_view fieldname, bool defvalue)
{
    SetConfig(index, sGameConfig->GetOption<bool>(fieldname, defvalue));
}

void AuctionBotConfig::SetConfig(AuctionBotConfigFloatValues index, std::string_view fieldname, float defvalue)
{
    SetConfig(index, sGameConfig->GetOption<float>(fieldname, defvalue));
}
...
void AuctionBotConfig::GetConfigFromFile()
{
    ...
    SetAHBotIncludes(sGameConfig->GetOption<std::string>("AuctionHouseBot.forceIncludeItems", ""));
    SetAHBotExcludes(sGameConfig->GetOption<std::string>("AuctionHouseBot.forceExcludeItems", ""));
...
std::string_view AuctionBotConfig::GetHouseTypeName(AuctionHouseType houseType)
{
    static std::array<std::string_view, MAX_AUCTION_HOUSE_TYPE> names{ "Neutral", "Alliance", "Horde" };
...
uint32 AuctionBotConfig::GetRandCharExclude(uint32 exclude) const
{
    ...
    for (uint32 charId : _AHBotCharacters)
        if (charId != exclude)
            filteredCharacters.emplace_back(charId);
...
void AuctionHouseBot::InitializeAgents()
{
    ...
    {
        _seller = std::make_unique<AuctionBotSeller>();
    ...
    {
        _buyer = std::make_unique<AuctionBotBuyer>();
    ...
    if (_seller || _buyer)
    {
        _updateInterval = Seconds{ CONF_GET_UINT("AuctionHouseBot.Update.Interval") };
...
void AuctionHouseBot::Initialize()
{
    ...
    {
        InitializeAgents();
        LOG_INFO("server.loading", ">> AHBot: Initialized agents in {}", sw);
    }
...
void AuctionHouseBot::Update(Milliseconds diff)
{
    _scheduler->Update(diff);
}

void AuctionHouseBot::ScheduleAsyncUpdateAgents()
{
    _scheduler->Schedule(_updateInterval, [](TaskContext)
    {
        sAsyncAuctionMgr->UpdateBotAgents();
    });
}

void AuctionHouseBot::UpdateAgents()
{
    // nothing do...
    if (!_buyer && !_seller)
        return;
    ...
    LOG_DEBUG("ahbot", ">> Agents updated in {}", sw);
    LOG_DEBUG("ahbot", "");
