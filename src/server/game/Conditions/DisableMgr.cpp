...
void LoadDisables() {
  ...
  do {
    ...
    case DISABLE_TYPE_GO_LOS:
      if (!sObjectMgr->GetGameObjectTemplate(entry)) {
        LOG_ERROR("sql.sql",
                  "Gameobject entry %u from `disables` doesn't exist in dbc, "
                  "skipped.",
                  entry);
        continue;
      }
      if (flags)
        LOG_ERROR("sql.sql",
                  "Disable flags specified for gameobject %u, useless data.",
                  entry);
      break;
    case DISABLE_TYPE_SPELL:
      ...
      if (!flags || flags > MAX_SPELL_DISABLE_TYPE) {
        LOG_ERROR("sql.sql", "Disable flags for spell %u are invalid, skipped.",
                  entry);
        continue;
      }
    ...
    case DISABLE_TYPE_LFG_MAP: {
      ...
      case MAP_ARENA:
        LOG_ERROR("sql.sql",
                  "Battleground map %u specified to be disabled in map case, "
                  "skipped.",
                  entry);
        continue;
      }
      if (isFlagInvalid) {
        LOG_ERROR("sql.sql", "Disable flags for map %u are invalid, skipped.",
                  entry);
        continue;
      }
      break;
    ...
    case DISABLE_TYPE_BATTLEGROUND:
      if (!sBattlemasterListStore.LookupEntry(entry)) {
        LOG_ERROR("sql.sql",
                  "Battleground entry %u from `disables` doesn't exist in dbc, "
                  "skipped.",
                  entry);
        continue;
      }
    ...
    case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
      if (!sAchievementCriteriaStore.LookupEntry(entry)) {
        LOG_ERROR("sql.sql",
                  "Achievement Criteria entry %u from `disables` doesn't exist "
                  "in dbc, skipped.",
                  entry);
        continue;
      }
      if (flags)
        LOG_ERROR("sql.sql",
                  "Disable flags specified for Achievement Criteria %u, "
                  "useless data.",
                  entry);
      break;
  ...
  LOG_INFO("server", ">> Loaded %u disables in %u ms", total_count,
...
void CheckQuestDisables() {
  ...
  for (DisableTypeMap::iterator itr = m_DisableMap[DISABLE_TYPE_QUEST].begin();
    ...
    if (itr->second.flags)
      LOG_ERROR("sql.sql",
                "Disable flags specified for quest %u, useless data.", entry);
    ++itr;
  }

  LOG_INFO("server", ">> Checked %u quest disables in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}

bool IsDisabledFor(DisableType type, uint32 entry, Unit const *unit,
                   uint8 flags) {
      ...
      if ((spellFlags & SPELL_DISABLE_PLAYER &&
        ...
        if (spellFlags & SPELL_DISABLE_MAP) {
          ...
          if (!(spellFlags & SPELL_DISABLE_AREA))
            return false; // Spell is disabled on another map, but not this one,
                          // return false

          // Spell is disabled in an area, but not explicitly our current mapId.
          // Continue processing.
        }
        ...
        if (spellFlags & SPELL_DISABLE_AREA) {
          ...
          return false;  // Spell is disabled in another area, but not this one,
                         // return false
        } else
