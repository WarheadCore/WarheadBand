...
bool Condition::Meets(ConditionSourceInfo &sourceInfo) {
  ...
  case CONDITION_ITEM: {
    if (Player *player = object->ToPlayer()) {
      ...
      condMeets =
          player->HasItemCount(ConditionValue1, ConditionValue2, checkBank);
    }
    break;
  }
    ...
    if (Unit *unit = object->ToUnit())
      condMeets =
          CompareValues(static_cast<ComparisionType>(ConditionValue2),
                        static_cast<uint32>(unit->getLevel()), ConditionValue1);
    break;
  }
  ...
  case CONDITION_NEAR_CREATURE: {
    condMeets = !!GetClosestCreatureWithEntry(
        object, ConditionValue1, (float)ConditionValue2, !ConditionValue3);
    break;
  }
  case CONDITION_NEAR_GAMEOBJECT: {
    condMeets = !!GetClosestGameObjectWithEntry(object, ConditionValue1,
  ...
  case CONDITION_OBJECT_ENTRY_GUID: {
    if (ConditionValue3 == 1 &&
      ...
      if (object->HasFlag(UNIT_FIELD_FLAGS,
    ...
    if (uint32(object->GetTypeId()) == ConditionValue1) {
      ...
      if (ConditionValue3 > 1) {
        ...
        case TYPEID_UNIT:
          condMeets &=
              object->ToCreature()->GetDBTableGUIDLow() == ConditionValue3;
          break;
        case TYPEID_GAMEOBJECT:
          condMeets &=
              object->ToGameObject()->GetDBTableGUIDLow() == ConditionValue3;
          break;
        default:
          break;
        }
  ...
  case CONDITION_QUEST_OBJECTIVE_PROGRESS: {
    if (Player *player = object->ToPlayer()) {
      const Quest *quest =
          ASSERT_NOTNULL(sObjectMgr->GetQuestTemplate(ConditionValue1));
...
uint32 ConditionMgr::GetSearcherTypeMaskForConditionList(
  ...
  for (ConditionList::const_iterator i = conditions.begin();
    ...
    ASSERT((*i)->isLoaded() &&
           "ConditionMgr::GetSearcherTypeMaskForConditionList - not yet loaded "
           "condition found in list");
    ...
    if ((*i)->ReferenceId) // handle reference
    {
      ...
      ASSERT(ref != ConditionReferenceStore.end() &&
             "ConditionMgr::GetSearcherTypeMaskForConditionList - incorrect "
             "reference");
      ElseGroupStore[(*i)->ElseGroup] &=
          GetSearcherTypeMaskForConditionList((*ref).second);
    } else // handle normal condition
    {
      // object will match conditions in one ElseGroupStore only when it matches
      // all of them so, let's find a smallest possible mask which satisfies all
      // conditions
      ElseGroupStore[(*i)->ElseGroup] &=
...
bool ConditionMgr::IsObjectMeetToConditionList(
    ConditionSourceInfo &sourceInfo, ConditionList const &conditions) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("condition",
              "ConditionMgr::IsPlayerMeetToConditionList condType: %u val1: %u",
              (*i)->ConditionType, (*i)->ConditionValue1);
#endif
    if ((*i)->isLoaded()) {
      ...
      if ((*i)->ReferenceId) // handle reference
      {
        ...
        if (ref != ConditionReferenceStore.end()) {
          ...
          LOG_DEBUG(
              "condition",
              "IsPlayerMeetToConditionList: Reference template -%u not found",
              (*i)->ReferenceId);
#endif
        }
      } else // handle normal condition
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("condition",
                  "GetConditionsForNotGroupedEntry: found conditions for type "
                  "%u and entry %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("condition",
                "GetConditionsForSpellClickEvent: found conditions for Vehicle "
                "entry %u spell %u",
                creatureId, spellId);
#endif
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("condition",
                "GetConditionsForVehicleSpell: found conditions for Vehicle "
                "entry %u spell %u",
                creatureId, spellId);
#endif
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("condition",
                "GetConditionsForSmartEvent: found conditions for Smart Event "
                "entry or guid %d event_id %u",
                entryOrGuid, eventId);
#endif
    }
  }
...
void ConditionMgr::LoadConditions(bool isReload) {
  ...
  QueryResult result = WorldDatabase.Query(
      ...
      "ErrorType, ErrorTextId, ScriptName FROM conditions");
  ...
  do {
    ...
    if (iConditionTypeOrReference < 0) // it has a reference
    {
      ...
      if (cond->ConditionValue1)
        LOG_ERROR("sql.sql", "Condition %s %i has useless data in value1 (%u)!",
                  rowType, iSourceTypeOrReferenceId, cond->ConditionValue1);
      if (cond->ConditionValue2)
        LOG_ERROR("sql.sql", "Condition %s %i has useless data in value2 (%u)!",
                  rowType, iSourceTypeOrReferenceId, cond->ConditionValue2);
      if (cond->ConditionValue3)
        LOG_ERROR("sql.sql", "Condition %s %i has useless data in value3 (%u)!",
                  rowType, iSourceTypeOrReferenceId, cond->ConditionValue3);
      if (cond->NegativeCondition)
        LOG_ERROR("sql.sql",
                  "Condition %s %i has useless data in NegativeCondition (%u)!",
    ...
    if (iSourceTypeOrReferenceId < 0) // it is a reference template
    {
      ...
      if (ConditionReferenceStore.find(uRefId) ==
          ConditionReferenceStore.end()) // make sure we have a list for our
                                         // conditions, based on reference id
      {
    ...
    // Grouping is only allowed for some types (loot templates, gossip menus,
    // gossip items)
    ...
    if (cond->ErrorType && cond->SourceType != CONDITION_SOURCE_TYPE_SPELL) {
      LOG_ERROR(
          "server",
          "Condition type %u entry %i can't have ErrorType (%u), set to 0!",
          uint32(cond->SourceType), cond->SourceEntry, cond->ErrorType);
      cond->ErrorType = 0;
    }

    if (cond->ErrorTextId && !cond->ErrorType) {
      LOG_ERROR("server",
                "Condition type %u entry %i has any ErrorType, ErrorTextId "
                "(%u) is set, set to 0!",
                uint32(cond->SourceType), cond->SourceEntry, cond->ErrorTextId);
      cond->ErrorTextId = 0;
    }
    ...
    if (cond->SourceGroup) {
      ...
      case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE:
        valid = addToLootTemplate(
            cond, LootTemplates_Disenchant.GetLootForConditionFill(
                      cond->SourceGroup));
        break;
      ...
      case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE:
        valid = addToLootTemplate(
            cond, LootTemplates_Gameobject.GetLootForConditionFill(
                      cond->SourceGroup));
        break;
      ...
      case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE:
        valid = addToLootTemplate(
            cond, LootTemplates_Pickpocketing.GetLootForConditionFill(
                      cond->SourceGroup));
        break;
      case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE:
        valid = addToLootTemplate(
            cond, LootTemplates_Prospecting.GetLootForConditionFill(
                      cond->SourceGroup));
        break;
      ...
      case CONDITION_SOURCE_TYPE_GOSSIP_MENU_OPTION:
        ...
        break;
      case CONDITION_SOURCE_TYPE_SPELL_CLICK_EVENT: {
        SpellClickEventConditionStore[cond->SourceGroup][cond->SourceEntry]
            .push_back(cond);
        valid = true;
        ++count;
      ...
      case CONDITION_SOURCE_TYPE_NPC_VENDOR: {
        NpcVendorConditionContainerStore[cond->SourceGroup][cond->SourceEntry]
            .push_back(cond);
        valid = true;
        ++count;
        continue;
      ...
      if (!valid) {
        LOG_ERROR("sql.sql", "Not handled grouped condition, SourceGroup %u",
                  cond->SourceGroup);
        delete cond;
      } else {
  ...
  LOG_INFO("server", ">> Loaded %u conditions in %u ms", count,
...
bool ConditionMgr::addToLootTemplate(Condition *cond, LootTemplate *loot) {
  if (!loot) {
    LOG_ERROR("sql.sql", "ConditionMgr: LootTemplate %u not found",
              cond->SourceGroup);
    return false;
  }
  ...
  LOG_ERROR("sql.sql", "ConditionMgr: Item %u not found in LootTemplate %u",
...
bool ConditionMgr::addToGossipMenus(Condition *cond) {
  ...
  LOG_ERROR("sql.sql", "addToGossipMenus: GossipMenu %u not found",
            cond->SourceGroup);
  return false;
}
...
bool ConditionMgr::addToGossipMenuItems(Condition *cond) {
  ...
  LOG_ERROR("sql.sql",
            "addToGossipMenuItems: GossipMenuId %u Item %u not found",
...
bool ConditionMgr::addToSpellImplicitTargetConditions(Condition *cond) {
  ...
  SpellInfo *spellInfo =
      const_cast<SpellInfo *>(sSpellMgr->GetSpellInfo(cond->SourceEntry));
  ASSERT(spellInfo);
    ...
    if (uint32 commonMask = *itr & conditionEffMask) {
      ...
      if (sharedList) {
        ...
        if (conditionEffMask != *itr) {
          LOG_ERROR("sql.sql",
                    "SourceEntry %u in `condition` table, has incorrect "
                    "SourceGroup %u (spell effectMask) set - "
...
bool ConditionMgr::isSourceTypeValid(Condition *cond) {
  if (cond->SourceType == CONDITION_SOURCE_TYPE_NONE ||
    ...
    LOG_ERROR("sql.sql",
              "Invalid ConditionSourceType %u in `condition` table, ignoring.",
  ...
  case CONDITION_SOURCE_TYPE_GRAVEYARD: {
    LOG_ERROR("sql.sql",
              "ConditionSourceType %u in `condition` table is not supported on "
              "3.3.5a, ignoring.",
              uint32(cond->SourceType));
    return false;
  }
  case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE: {
    if (!LootTemplates_Creature.HaveLootFor(cond->SourceGroup)) {
      LOG_ERROR("sql.sql",
                "SourceGroup %u in `condition` table, does not exist in "
    ...
    if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
      LOG_ERROR("sql.sql",
                "SourceType %u, SourceEntry %u in `condition` table, does not "
                "exist in `item_template`, ignoring.",
  ...
  case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE: {
    ...
    LootTemplate *loot =
        LootTemplates_Pickpocketing.GetLootForConditionFill(cond->SourceGroup);
    ItemTemplate const *pItemProto =
        sObjectMgr->GetItemTemplate(cond->SourceEntry);
    if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
      LOG_ERROR("sql.sql",
  ...
  case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE: {
    ...
    LootTemplate *loot =
        LootTemplates_Prospecting.GetLootForConditionFill(cond->SourceGroup);
    ItemTemplate const *pItemProto =
        sObjectMgr->GetItemTemplate(cond->SourceEntry);
    if (!pItemProto && !loot->isReference(cond->SourceEntry)) {
      LOG_ERROR("sql.sql",
  ...
  case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET: {
    ...
    if (!spellInfo) {
      LOG_ERROR("sql.sql",
                "SourceEntry %u in `condition` table, does not exist in "
                "`spell.dbc`, ignoring.",
                cond->SourceEntry);
      return false;
    }
    ...
    if ((cond->SourceGroup > MAX_EFFECT_MASK) || !cond->SourceGroup) {
      LOG_ERROR("sql.sql",
                "SourceEntry %u in `condition` table, has incorrect "
                "SourceGroup %u (spell effectMask) set , ignoring.",
                cond->SourceEntry, cond->SourceGroup);
      return false;
    }
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      ...
      LOG_ERROR(
          "sql.sql",
          "SourceEntry %u SourceGroup %u in `condition` table - spell %u does "
          "not have implicit targets of types: _AREA_, _CONE_, _NEARBY_ for "
          "effect %u, SourceGroup needs correction, ignoring.",
          cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i));
  ...
  case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
    if (!sObjectMgr->GetQuestTemplate(cond->SourceEntry)) {
      LOG_ERROR("sql.sql",
                "CONDITION_SOURCE_TYPE_QUEST_AVAILABLE specifies non-existing "
                "quest (%u), skipped",
                cond->SourceEntry);
      return false;
    }
...
bool ConditionMgr::isConditionTypeValid(Condition *cond) {
  if (cond->ConditionType == CONDITION_NONE ||
    ...
    LOG_ERROR("sql.sql",
              "SourceEntry %u in `condition` table has an invalid "
              "ConditionType (%u), ignoring.",
  ...
  case CONDITION_DIFFICULTY_ID:
    LOG_ERROR("sql.sql",
              "SourceEntry %u in `condition` table has a ConditionType that is "
              "not supported on 3.3.5a (%u), ignoring.",
              cond->SourceEntry, uint32(cond->ConditionType));
    return false;
  ...
  case CONDITION_TAXI:
    LOG_ERROR("sql.sql",
              "SourceEntry %u in `condition` table has a ConditionType that is "
              "not yet supported on AzerothCore (%u), ignoring.",
              cond->SourceEntry, uint32(cond->ConditionType));
    return false;
  default:
    break;
  ...
  if (cond->ConditionTarget >= cond->GetMaxAvailableConditionTargets()) {
    LOG_ERROR("sql.sql",
              "SourceType %u, SourceEntry %u in `condition` table, has "
  ...
  case CONDITION_AURA: {
    ...
    if (cond->ConditionValue2 > EFFECT_2) {
      LOG_ERROR("sql.sql",
                "Aura condition has non existing effect index (%u) (must be "
                "0..2), skipped",
                cond->ConditionValue2);
      return false;
    }
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Aura condition has useless data in value3 (%u)!",
  ...
  case CONDITION_ITEM: {
    ...
    if (!proto) {
      LOG_ERROR("sql.sql", "Item condition has non existing item (%u), skipped",
                cond->ConditionValue1);
      return false;
    }
    ...
    if (!cond->ConditionValue2) {
      LOG_ERROR(
          "sql.sql",
          "Item condition has 0 set for item count in value2 (%u), skipped",
          cond->ConditionValue2);
      return false;
    }
    break;
  ...
  case CONDITION_ZONEID: {
    ...
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "ZoneID condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "ZoneID condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_REPUTATION_RANK: {
  ...
  case CONDITION_TEAM: {
    if (cond->ConditionValue1 != ALLIANCE && cond->ConditionValue1 != HORDE) {
      LOG_ERROR("sql.sql",
                "Team condition specifies unknown team (%u), skipped",
                cond->ConditionValue1);
      return false;
    }

    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "Team condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Team condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_SKILL: {
    ...
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Skill condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_QUESTREWARDED:
  ...
  case CONDITION_QUEST_SATISFY_EXCLUSIVE: {
    ...
    if (cond->ConditionValue2 > 1) {
      LOG_ERROR("sql.sql", "Quest condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    }
    if (cond->ConditionValue3) {
      LOG_ERROR("sql.sql", "Quest condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    }
    break;
  }
  case CONDITION_QUESTSTATE:
    if (cond->ConditionValue2 >= (1 << MAX_QUEST_STATUS)) {
      LOG_ERROR("sql.sql",
                "ConditionType (%u) has invalid state mask (%u), skipped.",
                cond->ConditionType, cond->ConditionValue2);
      return false;
    }
    break;
  case CONDITION_ACTIVE_EVENT: {
  ...
  case CONDITION_ACHIEVEMENT: {
    ...
    if (!achievement) {
      LOG_ERROR(
          "sql.sql",
          "Achivement condition has non existing achivement id (%u), skipped",
          cond->ConditionValue1);
      return false;
    }
    ...
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql",
                "Achivement condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql",
                "Achivement condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_CLASS: {
    ...
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "Class condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Class condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_RACE: {
    ...
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "Race condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Race condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_GENDER: {
    if (!Player::IsValidGender(uint8(cond->ConditionValue1))) {
      LOG_ERROR("server", "Gender condition has invalid gender (%u), skipped",
                cond->ConditionValue1);
      return false;
    }

    if (cond->ConditionValue2)
      LOG_ERROR("server", "Gender condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("server", "Gender condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_MAPID: {
    ...
    if (!me) {
      LOG_ERROR("sql.sql", "Map condition has non existing map (%u), skipped",
                cond->ConditionValue1);
      return false;
    }

    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "Map condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Map condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_SPELL: {
    ...
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "Spell condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Spell condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_LEVEL: {
    if (cond->ConditionValue2 >= COMP_TYPE_MAX) {
      LOG_ERROR("sql.sql", "Level condition has invalid option (%u), skipped",
                cond->ConditionValue2);
      return false;
    }
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Level condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_DRUNKENSTATE: {
  ...
  case CONDITION_OBJECT_ENTRY_GUID: {
      ...
      if (cond->ConditionValue3 > 1) {
        if (CreatureData const *creatureData =
          ...
          if (cond->ConditionValue2 &&
            ...
            LOG_ERROR("sql.sql",
                      "ObjectEntryGuid condition has guid %u set but does not "
                      "match creature entry (%u), skipped",
                      cond->ConditionValue3, cond->ConditionValue2);
            return false;
          }
        } else {
          LOG_ERROR("sql.sql",
                    "ObjectEntryGuid condition has non existing creature guid "
                    "(%u), skipped",
                    cond->ConditionValue3);
          return false;
        }
    ...
    case TYPEID_GAMEOBJECT:
      if (cond->ConditionValue2 &&
        ...
        LOG_ERROR("sql.sql",
                  "ObjectEntryGuid condition has non existing gameobject "
                  "template entry (%u), skipped",
                  cond->ConditionValue2);
        return false;
      }
      if (cond->ConditionValue3) {
        if (GameObjectData const *goData =
                sObjectMgr->GetGOData(cond->ConditionValue3)) {
          if (cond->ConditionValue2 && goData->id != cond->ConditionValue2) {
            LOG_ERROR("sql.sql",
                      "ObjectEntryGuid condition has guid %u set but does not "
                      "match gameobject entry (%u), skipped",
                      cond->ConditionValue3, cond->ConditionValue2);
            return false;
          }
        } else {
    ...
    case TYPEID_CORPSE:
      if (cond->ConditionValue2)
        LOG_ERROR("sql.sql",
                  "ObjectEntryGuid condition has useless data in value2 (%u)!",
                  cond->ConditionValue2);
      if (cond->ConditionValue3)
        LOG_ERROR("sql.sql",
                  "ObjectEntryGuid condition has useless data in value3 (%u)!",
                  cond->ConditionValue3);
      break;
    default:
      LOG_ERROR("sql.sql",
                "ObjectEntryGuid condition has wrong typeid set (%u), skipped",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_TYPE_MASK: {
    if (!cond->ConditionValue1 ||
        (cond->ConditionValue1 & ~(TYPEMASK_UNIT | TYPEMASK_PLAYER |
                                   TYPEMASK_GAMEOBJECT | TYPEMASK_CORPSE))) {
      LOG_ERROR("sql.sql",
    ...
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql",
                "TypeMask condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql",
                "TypeMask condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_RELATION_TO: {
  ...
  case CONDITION_DISTANCE_TO: {
    ...
    if (cond->ConditionValue3 >= COMP_TYPE_MAX) {
      LOG_ERROR(
          "sql.sql",
          "DistanceTo condition has invalid ComparisionType (%u), skipped",
          cond->ConditionValue3);
      return false;
    }
    break;
  ...
  case CONDITION_ALIVE: {
    if (cond->ConditionValue1)
      LOG_ERROR("sql.sql", "Alive condition has useless data in value1 (%u)!",
                cond->ConditionValue1);
    if (cond->ConditionValue2)
      LOG_ERROR("sql.sql", "Alive condition has useless data in value2 (%u)!",
                cond->ConditionValue2);
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "Alive condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_HP_VAL: {
    ...
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "HpVal condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_HP_PCT: {
    ...
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql", "HpPct condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_AREAID:
  ...
  case CONDITION_WORLD_STATE: {
    if (!sWorld->getWorldState(cond->ConditionValue1)) {
      LOG_ERROR("sql.sql",
                "World state condition has non existing world state in value1 "
                "(%u), skipped",
                cond->ConditionValue1);
      return false;
    }
    if (cond->ConditionValue3)
      LOG_ERROR("sql.sql",
                "World state condition has useless data in value3 (%u)!",
                cond->ConditionValue3);
    break;
  }
  case CONDITION_PHASEMASK: {
  ...
  case CONDITION_TITLE: {
    ...
    if (!titleEntry) {
      LOG_ERROR(
          "sql.sql",
          "Title condition has non existing title in value1 (%u), skipped",
          cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_UNIT_STATE: {
    if (!(cond->ConditionValue1 & UNIT_STATE_ALL_STATE_SUPPORTED)) {
      LOG_ERROR("server",
                "UnitState condition has non existing UnitState in value1 "
                "(%u), skipped",
                cond->ConditionValue1);
      return false;
    }
  ...
  case CONDITION_REALM_ACHIEVEMENT: {
    ...
    if (!achievement) {
      LOG_ERROR("server",
                "CONDITION_REALM_ACHIEVEMENT has non existing realm first "
                "achivement id (%u), skipped.",
                cond->ConditionValue1);
      return false;
    }
    break;
  ...
  case CONDITION_QUEST_OBJECTIVE_PROGRESS: {
    ...
    if (!quest) {
      LOG_ERROR("sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS points to non-existing "
                "quest (%u), skipped.",
                cond->ConditionValue1);
      return false;
    }
    ...
    if (cond->ConditionValue2 > 3) {
      LOG_ERROR(
          "sql.sql",
          "CONDITION_QUEST_OBJECTIVE_PROGRESS has out-of-range quest objective "
          "index specified (%u), it must be a number between 0 and 3. skipped.",
          cond->ConditionValue2);
      return false;
    }
    ...
    if (quest->RequiredNpcOrGo[cond->ConditionValue2] == 0) {
      LOG_ERROR("sql.sql",
                "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective %u for "
                "quest %u, but the field RequiredNPCOrGo%u is 0, skipped.",
                cond->ConditionValue2, cond->ConditionValue1,
                cond->ConditionValue2);
      return false;
    }
    ...
    if (cond->ConditionValue3 >
      ...
      LOG_ERROR("sql.sql",
                ...
                "%u in value3, but quest %u has a maximum objective count of "
                "%u in RequiredNPCOrGOCount%u, skipped.",
                cond->ConditionValue3, cond->ConditionValue2,
                quest->RequiredNpcOrGoCount[cond->ConditionValue2],
                cond->ConditionValue2);
      return false;
  ...
  case CONDITION_HAS_AURA_TYPE: {
    if (cond->ConditionValue1 == SPELL_AURA_NONE ||
      ...
      LOG_ERROR("sql.sql",
                "Has Aura Effect condition has non existing aura (%u), skipped",
                cond->ConditionValue1);
      return false;
    }
    break;
...
void ConditionMgr::Clean() {
  for (ConditionReferenceContainer::iterator itr =
       ...
       itr != ConditionReferenceStore.end(); ++itr) {
  ...
  for (CreatureSpellConditionContainer::iterator itr =
       ...
       itr != VehicleSpellConditionStore.end(); ++itr) {
    for (ConditionTypeContainer::iterator it = itr->second.begin();
         it != itr->second.end(); ++it) {
      for (ConditionList::const_iterator i = it->second.begin();
           i != it->second.end(); ++i)
  ...
  for (SmartEventConditionContainer::iterator itr =
       ...
       itr != SmartEventConditionStore.end(); ++itr) {
    for (ConditionTypeContainer::iterator it = itr->second.begin();
         it != itr->second.end(); ++it) {
      for (ConditionList::const_iterator i = it->second.begin();
           i != it->second.end(); ++i)
  ...
  for (CreatureSpellConditionContainer::iterator itr =
       ...
       itr != SpellClickEventConditionStore.end(); ++itr) {
    for (ConditionTypeContainer::iterator it = itr->second.begin();
         it != itr->second.end(); ++it) {
      for (ConditionList::const_iterator i = it->second.begin();
           i != it->second.end(); ++i)
  ...
  for (NpcVendorConditionContainer::iterator itr =
       ...
       itr != NpcVendorConditionContainerStore.end(); ++itr) {
    for (ConditionTypeContainer::iterator it = itr->second.begin();
         it != itr->second.end(); ++it) {
      for (ConditionList::const_iterator i = it->second.begin();
           i != it->second.end(); ++i)
  ...
  for (std::list<Condition *>::const_iterator itr =
       ...
       itr != AllocatedMemoryStore.end(); ++itr)
    delete *itr;
