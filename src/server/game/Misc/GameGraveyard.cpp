...
void Graveyard::LoadGraveyardFromDB() {
  ...
  do {
    ...
    if (!Utf8toWStr(Graveyard.name, Graveyard.wnameLow)) {
      LOG_ERROR(
          "sql.sql",
          "Wrong UTF8 name for id %u in `game_graveyard` table, ignoring.", ID);
      continue;
    }
  ...
  LOG_INFO("server", ">> Loaded %i graveyard in %u ms", Count,
...
GraveyardStruct const *Graveyard::GetClosestGraveyard(float x, float y, float z,
                                                      uint32 MapId,
                                                      TeamId teamId) {
  ...
  if (!zoneId) {
    if (z > -500) {
      LOG_ERROR("server", "ZoneId not found for map %u coords (%f, %f, %f)",
                MapId, x, y, z);
  ...
  //
  //   if mapId == graveyard.mapId (ghost in plain zone or city or battleground)
  //   and search graveyard at same map
  //     then check faction
  ...
  if (range.first == range.second && !map->IsBattlegroundOrArena()) {
    LOG_ERROR("sql.sql",
              "Table `graveyard_zone` incomplete: Zone %u Team %u does not "
  ...
  for (; range.first != range.second; ++range.first) {
    ...
    if (!entry) {
      LOG_ERROR("sql.sql",
                ...
                "`game_graveyard` table %u, skipped.",
                data.safeLocId);
      continue;
    }
    ...
    if (MapId != entry->Map) {
      // if find graveyard at different map from where entrance placed (or no
      // entrance data), use any first
...
bool Graveyard::AddGraveyardLink(uint32 id, uint32 zoneId, TeamId teamId,
                                 bool persist /*= true*/) {
...
void Graveyard::RemoveGraveyardLink(uint32 id, uint32 zoneId, TeamId teamId,
                                    bool persist /*= false*/) {
...
void Graveyard::LoadGraveyardZones() {
  ...
  //                                                0       1         2
  QueryResult result =
      WorldDatabase.Query("SELECT ID, GhostZone, Faction FROM graveyard_zone");

  if (!result) {
    LOG_INFO("server", ">> Loaded 0 graveyard-zone links. DB table "
                       "`graveyard_zone` is empty.");
    LOG_INFO("server", " ");
    return;
  }
  ...
  do {
    ...
    TeamId teamId = team == 0 ? TEAM_NEUTRAL
    ...
    if (!areaEntry) {
      LOG_ERROR("sql.sql",
                "Table `graveyard_zone` has a record for not existing zone id "
                "(%u), skipped.",
                zoneId);
      continue;
    }
    ...
    if (areaEntry->zone != 0) {
      LOG_ERROR("sql.sql",
                "Table `graveyard_zone` has a record for subzone id (%u) "
    ...
    if (team != 0 && team != HORDE && team != ALLIANCE) {
      LOG_ERROR("sql.sql",
                "Table `graveyard_zone` has a record for non player faction "
                "(%u), skipped.",
                team);
      continue;
    }
    ...
    if (!AddGraveyardLink(safeLocId, zoneId, teamId, false))
      LOG_ERROR("sql.sql",
                "Table `graveyard_zone` has a duplicate record for Graveyard "
                "(ID: %u) and Zone (ID: %u), skipped.",
                safeLocId, zoneId);
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %u graveyard-zone links in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
