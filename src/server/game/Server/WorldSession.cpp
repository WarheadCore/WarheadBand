...
WorldSession::WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec,
                           ...
                           bool isARecruiter, bool skipQueue, uint32 TotalTime)
    : m_muteTime(mute_time), m_timeOutTime(0), _lastAuctionListItemsMSTime(0),
      ...
      _accountId(id), m_expansion(expansion), m_total_time(TotalTime),
      _logoutTime(0), m_inQueue(false), m_playerLoading(false),
      m_playerLogout(false), m_playerSave(false),
      m_sessionDbcLocale(sWorld->GetDefaultDbcLocale()),
      m_sessionDbLocaleIndex(locale), m_latency(0), m_clientTimeDelay(0),
      m_TutorialsChanged(false), recruiterId(recruiter),
      isRecruiter(isARecruiter), m_currentVendorEntry(0),
      m_currentBankerGUID(0), timeWhoCommandAllowed(0),
...
WorldSession::~WorldSession() {
  LoginDatabase.PExecute("UPDATE account SET totaltime = %u WHERE id = %u",
                         GetTotalTime(), GetAccountId());
...
bool WorldSession::Update(uint32 diff, PacketFilter &updater) {
  ...
  while (m_Socket && !m_Socket->IsClosed() && !_recvQueue.empty() &&
         _recvQueue.peek(true) != firstDelayedPacket &&
         _recvQueue.next(packet, updater)) {
    if (packet->GetOpcode() >= NUM_MSG_TYPES) {
      LOG_ERROR("server",
                "WorldSession Packet filter: received non-existent opcode %s "
                "(0x%.4X)",
      ...
      try {
        ...
        case STATUS_LOGGEDIN:
          if (!_player) {
            // pussywizard: such packets were sent to do something for a
            // character that has already logged out, skip them
          } else if (!_player->IsInWorld()) {
            // pussywizard: such packets may do something important and the
            // player is just being teleported, move to the end of the queue
            // pussywizard: previously such were skipped, so leave it as it is
            // xD proper code below if we wish to change that

            // pussywizard: requeue only important packets not related to maps
    ...
    if (++processedPackets >=
        150) // limit (by count) packets processed in one update, prevent DDoS
      break;

    if (getMSTimeDiff(_startMSTime, getMSTime()) >=
        3) // limit (by time) packets processed in one update, prevent DDoS
      break;
  }
...
bool WorldSession::HandleSocketClosed() {
  if (m_Socket && m_Socket->IsClosed() && !IsKicked() && GetPlayer() &&
...
void WorldSession::HandleTeleportTimeout(bool updateInSessions) {
  ...
  if (m_Socket && !m_Socket->IsClosed() && GetPlayer() &&
...
void WorldSession::LogoutPlayer(bool save) {
  ...
  if (_player) {
    ...
    // FIXME: logout must be delayed in case lost connection with client in time
    // of combat
    ...
    for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
      if (BattlegroundQueueTypeId bgQueueTypeId =
        ...
        sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId)
            .RemovePlayer(_player->GetGUID(), false, i);
    ...
    // (before saving to db) there are some positive auras from boss encounters
    // that can be kept by logging out and logging in after boss is dead, and
    // may be used on next bosses
    ...
    SavingSystemMgr::InsertToSavingSkipListIfNeeded(
        _player->GetNextSave()); // pussywizard
    if (save) {
      uint32 eslot;
      for (int j = BUYBACK_SLOT_START; j < BUYBACK_SLOT_END; ++j) {
        eslot = j - BUYBACK_SLOT_START;
        _player->SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOT_1 + (eslot * 2),
                                0);
...
void WorldSession::Handle_NULL(WorldPacket &recvPacket) {
  LOG_ERROR("server", "SESSION: received unhandled opcode %s (0x%.4X)",
...
void WorldSession::Handle_ServerSide(WorldPacket &recvPacket) {
  LOG_ERROR("server", "SESSION: received server-side opcode %s (0x%.4X)",
            LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_Deprecated(WorldPacket &recvPacket) {
  LOG_ERROR("server", "SESSION: received deprecated opcode %s (0x%.4X)",
            LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask) {
  ...
  do {
    ...
    if (type >= NUM_ACCOUNT_DATA_TYPES) {
      LOG_ERROR(
          "server", "Table `%s` have invalid account data type (%u), ignore.",
    ...
    if ((mask & (1 << type)) == 0) {
      LOG_ERROR("server",
                "Table `%s` have non appropriate for table  account data type "
                "(%u), ignore.",
...
void WorldSession::LoadTutorialsData() {
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_TUTORIALS);
  stmt->setUInt32(0, GetAccountId());
...
void WorldSession::ReadMovementInfo(WorldPacket &data, MovementInfo *mi) {
  ...
  if (mi->HasMovementFlag(MOVEMENTFLAG_SPLINE_ELEVATION))
    ...
    //! Anti-cheat checks. Please keep them in seperate if() blocks to maintain
    //! a clear overview. Might be subject to latency, so just remove improper
    //! flags.
#ifdef ACORE_DEBUG
  ...
  {                                                                            \
    if (check) {                                                               \
      LOG_DEBUG("entities.unit",                                               \
                "WorldSession::ReadMovementInfo: Violation of MovementFlags "  \
                "found (%s). "                                                 \
                "MovementFlags: %u, MovementFlags2: %u for player GUID: %u. "  \
                "Mask %u will be removed.",                                    \
                STRINGIZE(check), mi->GetMovementFlags(),                      \
                mi->GetExtraMovementFlags(), GetPlayer()->GetGUIDLow(),        \
                maskToRemove);                                                 \
...
#define REMOVE_VIOLATING_FLAGS(check, maskToRemove)                            \
  ...
  */
  ...
  REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_FALLING_SLOW) &&
...
void WorldSession::ReadAddonsInfo(WorldPacket &data) {
  ...
  if (size > 0xFFFFF) {
    LOG_ERROR("server",
              "WorldSession::ReadAddonsInfo addon info too big, size %u", size);
    return;
  }
  ...
  if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos,
    ...
    for (uint32 i = 0; i < addonsCount; ++i) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server",
                "ADDON: Name: %s, Enabled: 0x%x, CRC: 0x%x, Unknown2: 0x%x",
      ...
      if (savedAddon) {
        ...
        if (!match)
          LOG_DEBUG("server",
                    "ADDON: %s was known, but didn't match known CRC (0x%x)!",
                    addon.Name.c_str(), savedAddon->CRC);
        else
          LOG_DEBUG("server", "ADDON: %s was known, CRC is correct (0x%x)",
                    addon.Name.c_str(), savedAddon->CRC);
#endif
      } else {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "ADDON: %s (0x%x) was not known, saving...",
                  addon.Name.c_str(), addon.CRC);
#endif
      }
...
void WorldSession::SendAddonsInfo() {
  ...
  for (AddonsList::iterator itr = m_addonsList.begin();
    ...
    if (crcpub) {
      uint8 usepk =
          (itr->CRC != STANDARD_ADDON_CRC); // If addon is Standard addon CRC
      data << uint8(usepk);
      if (usepk) // if CRC is wrong, add public key (client need it)
      {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server",
                  "ADDON: CRC (0x%x) for addon %s is wrong (does not match "
                  "expected 0x%x), sending pubkey",
...
void WorldSession::ProcessQueryCallbackPet() {
  ...
  if (_loadPetFromDBFirstCallback.IsReady()) {
    ...
    if (player->IsInWorld()) {
      ...
      uint8 loadResult =
          HandleLoadPetFromDBFirstCallback(result, asynchLoadType);
      if (loadResult != PET_LOAD_OK)
        Pet::HandleAsynchLoadFailed(
            _loadPetFromDBFirstCallback.GetSecondParam(), player,
            asynchLoadType, loadResult);
...
void WorldSession::ProcessQueryCallbackLogin() {
  ...
  if (_charCreateCallback.IsReady()) {
    ...
    // Don't call FreeResult() here, the callback handler will do that depending
    // on the events in the callback chain
  }
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket &p,
  ...
  LOG_INFO("server",
           ...
           opcodeTable[p.GetOpcode()].name, p.GetOpcode(),
  ...
  case POLICY_LOG:
    return true;
  case POLICY_KICK: {
    LOG_INFO("server", "AntiDOS: Player %s kicked!",
  ...
  case POLICY_BAN: {
    ...
    case 0: // Ban account
      ...
      sBan->BanAccount(nameOrIp, std::to_string(duration),
                       "DOS (Packet Flooding/Spoofing", "Server: AutoDOS");
      break;
    case 1: // Ban ip
      nameOrIp = Session->GetRemoteAddress();
      sBan->BanIP(nameOrIp, std::to_string(duration),
                  "DOS (Packet Flooding/Spoofing", "Server: AutoDOS");
      break;
    }

    LOG_INFO("server", "AntiDOS: Player automatically banned for %u seconds.",
             duration);
    return false;
  }
