...
public:
  ReactorRunnable() : m_Reactor(0), m_Connections(0), m_ThreadId(-1) {
...
WorldSocketMgr::WorldSocketMgr()
    : m_NetThreads(0), m_NetThreadsCount(0), m_SockOutKBuff(-1),
...
int WorldSocketMgr::StartReactiveIO(uint16 port, const char *address) {
  ...
  if (m_Acceptor->open(listen_addr, m_NetThreads[0].GetReactor(),
                       ACE_NONBLOCK) == -1) {
...
int WorldSocketMgr::OnSocketOpen(WorldSocket *sock) {
  ...
  if (m_SockOutKBuff >= 0) {
    if (sock->peer().set_option(SOL_SOCKET, SO_SNDBUF, (void *)&m_SockOutKBuff,
                                sizeof(int)) == -1 &&
        errno != ENOTSUP) {
  ...
  // Set TCP_NODELAY.
  if (m_UseNoDelay) {
    if (sock->peer().set_option(ACE_IPPROTO_TCP, TCP_NODELAY, (void *)&ndoption,
                                sizeof(int)) == -1) {
      LOG_ERROR("server",
                "WorldSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY "
                "errno = %s",
