...
public:
  CreatureTextBuilder(WorldObject *obj, uint8 gender, ChatMsg msgtype,
                      uint8 textGroup, uint32 id, uint32 language,
                      WorldObject const *target)
      : _source(obj), _gender(gender), _msgType(msgtype), _textGroup(textGroup),
        _textId(id), _language(language), _target(target) {}
...
public:
  PlayerTextBuilder(WorldObject *obj, WorldObject *speaker, uint8 gender,
                    ChatMsg msgtype, uint8 textGroup, uint32 id,
...
void CreatureTextMgr::LoadCreatureTexts() {
  ...
  do {
    ...
    if (!GetLanguageDescByID(temp.lang)) {
      LOG_ERROR("sql.sql",
                "CreatureTextMgr: Entry %u, Group %u in table `creature_texts` "
    ...
    if (temp.BroadcastTextId) {
      if (!sObjectMgr->GetBroadcastText(temp.BroadcastTextId)) {
        LOG_ERROR("sql.sql",
                  "CreatureTextMgr: Entry %u, Group %u, Id %u in table "
                  "`creature_text` has non-existing or incompatible "
                  "BroadcastTextId %u.",
                  temp.entry, temp.group, temp.id, temp.BroadcastTextId);
    ...
    if (temp.TextRange > TEXT_RANGE_WORLD) {
      LOG_ERROR("sql.sql",
                "CreatureTextMgr: Entry %u, Group %u, Id %u in table "
                "`creature_text` has incorrect TextRange %u.",
  ...
  LOG_INFO("server", ">> Loaded %u creature texts for %lu creatures in %u ms",
           textCount, mTextMap.size(), GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void CreatureTextMgr::LoadCreatureTextLocales() {
  ...
  LOG_INFO("server", ">> Loaded %u Creature Text Locale in %u ms",
           uint32(mLocaleTextMap.size()), GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
uint32 CreatureTextMgr::SendChat(
    ...
    CreatureTextRange range /*= TEXT_RANGE_NORMAL*/, uint32 sound /*= 0*/,
    TeamId teamId /*= TEAM_NEUTRAL*/, bool gmOnly /*= false*/,
  ...
  if (sList == mTextMap.end()) {
    LOG_ERROR("sql.sql",
              "CreatureTextMgr: Could not find Text for Creature(%s) Entry %u "
  ...
  if (itr == textHolder.end()) {
    LOG_ERROR("sql.sql",
              ...
              uint32(textGroup), source->GetName().c_str(),
              source->GetGUIDLow(), source->GetEntry());
    return 0;
  }
  ...
  CreatureTextGroup tempGroup; // will use this to talk after sorting
                               // repeatGroup

  for (CreatureTextGroup::const_iterator giter = textGroupContainer.begin();
       giter != textGroupContainer.end(); ++giter)
  ...
  if (finalSound)
    SendSound(source, finalSound, finalType, whisperTarget, range, teamId,
              gmOnly);
  ...
  if (srcPlr) {
    PlayerTextBuilder builder(source, finalSource, finalSource->getGender(),
                              finalType, iter->group, iter->id, finalLang,
                              whisperTarget);
    SendChatPacket(finalSource, builder, finalType, whisperTarget, range,
                   teamId, gmOnly);
  } else {
    CreatureTextBuilder builder(finalSource, finalSource->getGender(),
                                finalType, iter->group, iter->id, finalLang,
                                whisperTarget);
    SendChatPacket(finalSource, builder, finalType, whisperTarget, range,
                   teamId, gmOnly);
...
void CreatureTextMgr::SendSound(Creature *source, uint32 sound, ChatMsg msgType,
                                ...
                                CreatureTextRange range, TeamId teamId,
                                bool gmOnly) {
  if (!sound || !source)
    return;
  ...
  data << uint32(sound);
  SendNonChatPacket(source, &data, msgType, whisperTarget, range, teamId,
                    gmOnly);
}
...
void CreatureTextMgr::SetRepeatId(Creature *source, uint8 textGroup, uint8 id) {
  ...
  if (std::find(repeats.begin(), repeats.end(), id) == repeats.end())
    ...
    LOG_ERROR("sql.sql",
              ...
              uint32(textGroup), source->GetName().c_str(),
              source->GetGUIDLow(), source->GetEntry(), uint32(id));
}
...
std::string
CreatureTextMgr::GetLocalizedChatString(uint32 entry, uint8 gender,
                                        uint8 textGroup, uint32 id,
