...
bool Warden::IsValidCheckSum(uint32 checksum, const uint8 *data,
                             const uint16 length) {
  ...
  if (checkId) {
    if (Player const *plr = _session->GetPlayer()) {
      std::string const reportFormat =
          "Player %s (guid %u, account id: %u) failed warden %u check (%s). "
          "Action: %s";
      reportMsg = Warhead::StringFormat(
          reportFormat, plr->GetName().c_str(), plr->GetGUIDLow(),
          _session->GetAccountId(), checkId,
          ((checkData && !checkData->Comment.empty())
               ? checkData->Comment.c_str()
               : "<warden comment is not set>"),
          GetWardenActionStr(action).c_str());
    } else {
      std::string const reportFormat =
          "Account id: %u failed warden %u check. Action: %s";
      reportMsg =
          Warhead::StringFormat(reportFormat, _session->GetAccountId(), checkId,
                                GetWardenActionStr(action).c_str());
    }
  } else {
    if (Player const *plr = _session->GetPlayer()) {
      std::string const reportFormat =
          "Player %s (guid %u, account id: %u) triggered warden penalty by "
          "reason: %s. Action: %s";
      reportMsg = Warhead::StringFormat(
          reportFormat, plr->GetName().c_str(), plr->GetGUIDLow(),
      ...
      std::string const reportFormat =
          "Account id: %u failed warden %u check. Action: %s";
      reportMsg = Warhead::StringFormat(reportFormat, _session->GetAccountId(),
                                        causeMsg.c_str(),
                                        GetWardenActionStr(action).c_str());
    }
  }
...
void WorldSession::HandleWardenDataOpcode(WorldPacket &recvData) {
  ...
  recvData >> opcode;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("warden", "Got packet, opcode %02X, size %u", opcode,
  ...
  default:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("warden", "Got unknown warden opcode %02X of size %u.", opcode,
