...
static uint16 GetCheckPacketSize(WardenCheck const *check) {
  ...
  if (!check->Str.empty()) {
    size +=
        (static_cast<uint16>(check->Str.length()) + 1); // 1 byte string length
  }
...
void WardenWin::Init(WorldSession *session, SessionKey const &k) {
  ...
  LOG_DEBUG("warden", "Server side warden for client %u initializing...",
            session->GetAccountId());
  LOG_DEBUG("warden", "C->S Key: %s",
            Warhead::Impl::ByteArrayToHexStr(_inputKey).c_str());
  LOG_DEBUG("warden", "S->C Key: %s",
            Warhead::Impl::ByteArrayToHexStr(_outputKey).c_str());
  LOG_DEBUG("warden", "  Seed: %s",
            Warhead::Impl::ByteArrayToHexStr(_seed).c_str());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("warden", "Module Key: %s",
            Warhead::Impl::ByteArrayToHexStr(_module->Key).c_str());
  LOG_DEBUG("warden", "Module ID: %s",
            Warhead::Impl::ByteArrayToHexStr(_module->Id).c_str());
#endif
  RequestModule();
}
...
void WardenWin::InitializeModule() {
  ...
  Request.CheckSumm1 = BuildChecksum(
      &Request.Unk1, Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES);
...
void WardenWin::HandleHashResult(ByteBuffer &buff) {
  ...
  if (memcmp(buff.contents() + 1, Module.ClientKeySeedHash,
             Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES) != 0) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
...
void WardenWin::RequestChecks() {
  ...
  if (_PendingChecks.empty()) {
    ...
    if (!hasLuaChecks) {
      for (uint32 i = 0; i < sWorld->getIntConfig(GetMaxWardenChecksForType(
  ...
  Warhead::Containers::EraseIf(
      _CurrentChecks, [this, &expectedSize](uint16 id) {
        uint16 const thisSize =
            GetCheckPacketSize(sWardenCheckMgr->GetWardenDataById(id));
...
void WardenWin::HandleData(ByteBuffer &buff) {
  ...
  for (uint16 const checkId : _CurrentChecks) {
    ...
    case MEM_CHECK: {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("warden",
                  "RESULT MEM_CHECK not 0x00, CheckId %u account Id %u",
      ...
      if (memcmp(buff.contents() + buff.rpos(), result.data(), rd->Length) !=
          0) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("warden", "RESULT MEM_CHECK fail CheckId %u account Id %u",
                  checkId, _session->GetAccountId());
#endif
        checkFailed = checkId;
      ...
      buff.rpos(buff.rpos() + rd->Length);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("warden", "RESULT MEM_CHECK passed CheckId %u account Id %u",
                checkId, _session->GetAccountId());
#endif
      break;
    }
    ...
    case MODULE_CHECK: {
      ...
      if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) != 0) {
        ...
        if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
          LOG_DEBUG("warden",
                    "RESULT PAGE_CHECK fail, CheckId %u account Id %u", checkId,
                    _session->GetAccountId());

        if (type == MODULE_CHECK)
          LOG_DEBUG("warden",
                    "RESULT MODULE_CHECK fail, CheckId %u account Id %u",
                    checkId, _session->GetAccountId());

        if (type == DRIVER_CHECK)
          LOG_DEBUG("warden",
                    "RESULT DRIVER_CHECK fail, CheckId %u account Id %u",
                    checkId, _session->GetAccountId());
#endif
        checkFailed = checkId;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
        LOG_DEBUG("warden", "RESULT PAGE_CHECK passed CheckId %u account Id %u",
                  checkId, _session->GetAccountId());
      else if (type == MODULE_CHECK)
        LOG_DEBUG("warden",
                  "RESULT MODULE_CHECK passed CheckId %u account Id %u",
                  checkId, _session->GetAccountId());
      else if (type == DRIVER_CHECK)
        LOG_DEBUG("warden",
                  "RESULT DRIVER_CHECK passed CheckId %u account Id %u",
                  checkId, _session->GetAccountId());
#endif
      break;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("warden",
                "LUA_EVAL_CHECK CheckId %u account Id %u got in-warden dummy "
                "response",
                checkId, _session->GetAccountId() /* , result */);
#endif
      break;
    }
    case MPQ_CHECK: {
      ...
      if (Mpq_Result != 0) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("warden", "RESULT MPQ_CHECK not 0x00 account id %u",
                  _session->GetAccountId());
#endif
        checkFailed = checkId;
        continue;
      ...
      if (memcmp(buff.contents() + buff.rpos(),
                 ...
                 Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES) !=
          0) // SHA1
      {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("warden", "RESULT MPQ_CHECK fail, CheckId %u account Id %u",
                  checkId, _session->GetAccountId());
#endif
        checkFailed = checkId;
        buff.rpos(
            buff.rpos() +
            Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES); // 20 bytes
                                                                   // SHA1
        continue;
      }
      ...
      buff.rpos(
          ...
          Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES); // 20 bytes
                                                                 // SHA1
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("warden", "RESULT MPQ_CHECK passed, CheckId %u account Id %u",
                checkId, _session->GetAccountId());
#endif
      break;
    }
