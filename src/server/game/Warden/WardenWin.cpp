...
void WardenWin::Init(WorldSession* session, SessionKey const& k)
{
    ...
    LOG_DEBUG("warden", "C->S Key: %s", Warhead::Impl::ByteArrayToHexStr(_inputKey, false).c_str());
    LOG_DEBUG("warden", "S->C Key: %s", Warhead::Impl::ByteArrayToHexStr(_outputKey,false).c_str());
    LOG_DEBUG("warden", "  Seed: %s", Warhead::Impl::ByteArrayToHexStr(_seed, false).c_str());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("warden", "Module Key: %s", Warhead::Impl::ByteArrayToHexStr(_module->Key, false).c_str());
    LOG_DEBUG("warden", "Module ID: %s", Warhead::Impl::ByteArrayToHexStr(_module->Id, false).c_str());
#endif
    RequestModule();
}
...
void WardenWin::InitializeModule()
{
    ...
    Request.CheckSumm1 = BuildChecksum(&Request.Unk1, Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES);
...
void WardenWin::HandleHashResult(ByteBuffer& buff)
{
    ...
    // Verify key
    if (memcmp(buff.contents() + 1, Module.ClientKeySeedHash, Warhead::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES) != 0)
    {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
