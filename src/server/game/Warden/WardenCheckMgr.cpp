...
void WardenCheckMgr::LoadWardenChecks() {
  ...
  do {
    ...
    if (checkType == LUA_EVAL_CHECK && id > 9999) {
      LOG_ERROR(
          "server",
          "sql.sql: Warden Lua check with id %u found in `warden_checks`. Lua "
          "checks may have four-digit IDs at most. Skipped.",
          id);
      continue;
    }
    ...
    case LUA_EVAL_CHECK: {
      if (wardenCheck.Length > WARDEN_MAX_LUA_CHECK_LENGTH) {
        LOG_ERROR("server",
                  "sql.sql: Found over-long Lua check for Warden check with id "
                  "%u in `warden_checks`. Max length is %u. Skipped.",
                  id, WARDEN_MAX_LUA_CHECK_LENGTH);
        continue;
      }
...
void WardenCheckMgr::LoadWardenOverrides() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 Warden action overrides. DB table "
  ...
  do {
    ...
    // Check if check actually exists before accessing the CheckStore vector
      ...
      LOG_ERROR("server",
                "Warden check action override for non-existing check (ID: %u, "
                "action: %u), skipped",
                checkId, action);
    else {
