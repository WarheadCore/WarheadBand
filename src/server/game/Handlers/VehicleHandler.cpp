...
void WorldSession::HandleChangeSeatsOnControlledVehicle(WorldPacket &recvData) {
  ...
  if (!seat->CanSwitchFromSeat()) {
    ...
    LOG_ERROR(
        "server",
        "HandleChangeSeatsOnControlledVehicle, Opcode: %u, Player %u tried to "
        "switch seats but current seatflags %u don't permit that.",
...
void WorldSession::HandleEjectPassenger(WorldPacket &data) {
  ...
  if (!vehicle) {
    ...
    LOG_ERROR("server", "HandleEjectPassenger: Player %u is not in a vehicle!",
  ...
  if (IS_PLAYER_GUID(guid)) {
    ...
    if (!player) {
      LOG_ERROR("server",
                "Player %u tried to eject player %u from vehicle, but the "
                "latter was not found in world!",
    ...
    if (!player->IsOnVehicle(vehicle->GetBase())) {
      LOG_ERROR("server",
                "Player %u tried to eject player %u, but they are not in the "
                "same vehicle",
                GetPlayer()->GetGUIDLow(), GUID_LOPART(guid));
      return;
    }
    ...
    {
      LOG_ERROR("server",
                "Player %u tried to eject creature guid %u from vehicle, but "
    ...
    if (!unit->IsOnVehicle(vehicle->GetBase())) {
      LOG_ERROR("server",
                "Player %u tried to eject unit %u, but they are not in the "
                "same vehicle",
                GetPlayer()->GetGUIDLow(), GUID_LOPART(guid));
      return;
    }
    ...
    if (seat->IsEjectable()) {
      ...
      LOG_ERROR("server",
                "Player %u attempted to eject creature GUID %u from "
                "non-ejectable seat.",
                GetPlayer()->GetGUIDLow(), GUID_LOPART(guid));
  } else
    LOG_ERROR(
  ...
  if (Vehicle *vehicle = GetPlayer()->GetVehicle()) {
    if (VehicleSeatEntry const *seat =
      ...
      if (seat->CanEnterOrExit())
        ...
        LOG_ERROR("server",
                  "Player %u tried to exit vehicle, but seatflags %u (ID: %u) "
                  "don't permit that.",
