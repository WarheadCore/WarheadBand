  ...
  if (info) {
    ...
    if (localeConstant >= LOCALE_enUS)
      if (GameObjectLocale const *gameObjectLocale =
        ...
        ObjectMgr::GetLocaleString(gameObjectLocale->Name, localeConstant,
                                   Name);
        ObjectMgr::GetLocaleString(gameObjectLocale->CastBarCaption,
                                   localeConstant, CastBarCaption);
      }

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "WORLD: CMSG_GAMEOBJECT_QUERY '%s' - Entry: %u. ",
              info->name.c_str(), entry);
#endif
    ...
    data << IconName; // 2.0.3, string. Icon name to use instead of default icon
                      // for go's (ex: "Attack" makes sword)
    data << CastBarCaption; // 2.0.3, string. Text will appear in Cast Bar when
                            // using GO (ex: "Collecting")
    ...
    if (MapEntry const *corpseMapEntry = sMapStore.LookupEntry(mapid)) {
      if (corpseMapEntry->IsDungeon() && corpseMapEntry->entrance_map >= 0) {
        ...
        if (Map const *entranceMap =
          ...
          z = entranceMap->GetHeight(GetPlayer()->GetPhaseMask(), x, y,
                                     MAX_HEIGHT);
        }
      }
    }
  ...
  if (!gossip) {
    ...
    for (uint8 i = 0; i < MAX_GOSSIP_TEXT_OPTIONS; ++i) {
      BroadcastText const *bct =
          sObjectMgr->GetBroadcastText(gossip->Options[i].BroadcastTextID);
      if (bct) {
      ...
      if (locale != DEFAULT_LOCALE && !bct) {
        if (NpcTextLocale const *npcTextLocale =
          ...
          ObjectMgr::GetLocaleString(npcTextLocale->Text_0[i], locale,
                                     text0[i]);
          ObjectMgr::GetLocaleString(npcTextLocale->Text_1[i], locale,
                                     text1[i]);
        }
      }
...
void WorldSession::HandleQuestPOIQuery(WorldPacket &recvData) {
  ...
  // Read quest ids and add the in a unordered_set so we don't send POIs for the
  // same quest multiple times
