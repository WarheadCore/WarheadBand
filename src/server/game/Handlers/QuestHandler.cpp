...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket &recvData) {
  ...
  default:
    LOG_ERROR("server", "QuestGiver called for unexpected type %u",
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_HELLO npc = %u",
            GUID_LOPART(guid));
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleQuestgiverHelloOpcode - Unit (GUID: %u) not found "
  ...
  // if (!creature->GetTransport()) // pussywizard: reverted with new spline
  // (old: without this check, npc would stay in place and the transport would
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST npc = %u, quest = "
            "%u, unk1 = %u",
            uint32(GUID_LOPART(guid)), questId, unk1);
#endif
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WORLD: Received CMSG_QUESTGIVER_QUERY_QUEST npc = %u, quest = %u, "
            "unk1 = %u",
            uint32(GUID_LOPART(guid)), questId, unk1);
#endif
  ...
  if (Quest const *quest = sObjectMgr->GetQuestTemplate(questId)) {
    ...
    // if this breaks them, add && object->GetTypeId() == TYPEID_ITEM to this
    // check item-started quests never have that flag
    ...
    if (quest->HasFlag(QUEST_FLAGS_AUTOCOMPLETE))
      _player->PlayerTalkClass->SendQuestGiverRequestItems(
          quest, object->GetGUID(),
          _player->CanCompleteQuest(quest->GetQuestId()), true);
    else
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket &recvData) {
  ...
  if (reward >= QUEST_REWARD_CHOICES_COUNT) {
    LOG_ERROR("server",
              "Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player %s (guid %d) "
              "tried to get invalid reward (%u) (probably packet hacking)",
              _player->GetName().c_str(), _player->GetGUIDLow(), reward);
    return;
  }

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc = %u, quest = "
            "%u, reward = %u",
            uint32(GUID_LOPART(guid)), questId, reward);
#endif
  ...
  if (Quest const *quest = sObjectMgr->GetQuestTemplate(questId)) {
    if ((!_player->CanSeeStartQuest(quest) &&
      ...
      LOG_ERROR("server",
                "HACK ALERT: Player %s (guid: %u) is trying to complete quest "
                "(id: %u) but he has no right to do it!",
        ...
        if (!sScriptMgr->OnQuestReward(_player, questgiver, quest, reward)) {
          ...
          if (Quest const *nextQuest = _player->GetNextQuest(guid, quest)) {
            if (_player->CanAddQuest(nextQuest, false) &&
              ...
              _player->PlayerTalkClass->SendQuestGiverQuestDetails(nextQuest,
                                                                   guid, true);
            }
          }
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket &recvData) {
  ...
  LOG_DEBUG(
      ...
      uint32(GUID_LOPART(guid)), questId);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Received CMSG_QUESTLOG_REMOVE_QUEST slot = %u",
            slot);
#endif
      ...
      if (Quest const *quest = sObjectMgr->GetQuestTemplate(questId)) {
        ...
        if (quest->HasFlag(QUEST_FLAGS_FLAGS_PVP)) {
          _player->pvpInfo.IsHostile =
              _player->pvpInfo.IsInHostileArea || _player->HasPvPForcingQuest();
          _player->UpdatePvPState();
        }
      }

      _player->TakeQuestSourceItem(questId,
                                   true); // remove quest src item from player
      _player->AbandonQuest(questId); // remove all quest items player received
                                      // before abandoning quest.
...
#endif
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "Player %u abandoned quest %u", _player->GetGUIDLow(),
                questId);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_CONFIRM_ACCEPT quest = %u",
            questId);
#endif
  ...
  if (Quest const *quest = sObjectMgr->GetQuestTemplate(questId)) {
    ...
    if (_player->CanAddQuest(quest, true))
      _player->AddQuestAndCheckCompletion(
          quest,
          nullptr); // nullptr, this prevent DB script from duplicate running
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket &recvData) {
  ...
  LOG_DEBUG(
      "network",
      "WORLD: Received CMSG_QUESTGIVER_COMPLETE_QUEST npc = %u, quest = %u",
      uint32(GUID_LOPART(guid)), questId);
#endif
  ...
  if (Quest const *quest = sObjectMgr->GetQuestTemplate(questId)) {
    if (!_player->CanSeeStartQuest(quest) &&
      ...
      LOG_ERROR(
          "server",
          "Possible hacking attempt: Player %s [guid: %u] tried to complete "
          "quest [entry: %u] without being in possession of the quest!",
          _player->GetName().c_str(), _player->GetGUIDLow(), questId);
      return;
    }
...
void WorldSession::HandleQuestgiverQuestAutoLaunch(
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Received CMSG_PUSHQUESTTOPARTY quest = %u",
            questId);
#endif
      ...
      for (GroupReference *itr = group->GetFirstMember(); itr != nullptr;
        ...
        if ((quest->IsAutoComplete() && quest->IsRepeatable() &&
          ...
          player->PlayerTalkClass->SendQuestGiverRequestItems(
              quest, _player->GetGUID(),
              player->CanCompleteRepeatableQuest(quest), true);
        else {
...
void WorldSession::HandleQuestgiverStatusMultipleQuery(
  ...
  for (Player::ClientGUIDs::const_iterator itr = _player->m_clientGUIDs.begin();
       itr != _player->m_clientGUIDs.end(); ++itr) {
    uint32 questStatus = DIALOG_STATUS_NONE;
