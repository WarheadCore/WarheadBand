...
void WorldSession::HandleUseItemOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("network",
            "WORLD: CMSG_USE_ITEM packet, bagIndex: %u, slot: %u, castCount: "
            "%u, spellId: %u, Item: %u, glyphIndex: %u, data length = %i",
            bagIndex, slot, castCount, spellId, pItem->GetEntry(), glyphIndex,
  ...
  // only allow conjured consumable, bandage, poisons (all should have the 2^21
  // item flag set in DB)
...
void WorldSession::HandleOpenItemOpcode(WorldPacket &recvPacket) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: CMSG_OPEN_ITEM packet, data length = %i",
            (uint32)recvPacket.size());
#endif
  ...
  if (!(proto->Flags & ITEM_FLAG_HAS_LOOT) &&
    ...
    LOG_ERROR("server",
              "Possible hacking attempt: Player %s [guid: %u] tried to open "
              "item [guid: %u, entry: %u] which is not openable!",
              pUser->GetName().c_str(), pUser->GetGUIDLow(), item->GetGUIDLow(),
              proto->ItemId);
    return;
  }
  ...
  if (lockId) {
    ...
    if (!lockInfo) {
      ...
      LOG_ERROR("server",
                "WORLD::OpenItem: item [guid = %u] has an unknown lockId: %u!",
                item->GetGUIDLow(), lockId);
      return;
    }
  ...
  if (item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED)) // wrapped?
  {
    ...
    _openWrappedItemCallback.SetFutureResult(
        CharacterDatabase.AsyncQuery(stmt));
  } else
    pUser->SendLoot(item->GetGUID(), LOOT_CORPSE);
}

void WorldSession::HandleOpenWrappedItemCallback(PreparedQueryResult result,
                                                 uint8 bagIndex, uint8 slot,
                                                 uint32 itemLowGUID) {
  if (!GetPlayer())
    return;
  ...
  if (item->GetGUIDLow() != itemLowGUID ||
      !item->HasFlag(ITEM_FIELD_FLAGS,
                     ITEM_FIELD_FLAG_WRAPPED)) // during getting result, gift
                                               // was swapped with another item
    return;

  if (!result) {
    LOG_ERROR("server",
              "Wrapped item %u don't have record in character_gifts table and "
              "will deleted",
              item->GetGUIDLow());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Recvd CMSG_GAMEOBJ_USE Message [guid=%u]",
            GUID_LOPART(guid));
#endif
...
void WorldSession::HandleCastSpellOpcode(WorldPacket &recvPacket) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WORLD: got cast spell packet, castCount: %u, spellId: %u, "
            "castFlags: %u, data length = %u",
            castCount, spellId, castFlags, (uint32)recvPacket.size());
#endif
  ...
  if (mover->GetTypeId() == TYPEID_PLAYER) {
    ...
    if (Vehicle *veh = mover->GetVehicleKit())
      if (const VehicleSeatEntry *seat = veh->GetSeatForPassenger(_player))
        if (seat->m_flags & VEHICLE_SEAT_FLAG_CAN_ATTACK || spellInfo
                                                                    ->Effects
                                                                        [EFFECT_0]
                                                                    .Effect ==
          ...
          if ((mover->GetTypeId() == TYPEID_UNIT &&
              ...
              spellInfo->IsPassive()) // the creature can't cast that spell,
                                      // check player instead
          {
            if (!(spellInfo->Targets & TARGET_FLAG_GAMEOBJECT_ITEM) &&
            ...
            // swapping the mover will stop the check below at == TYPEID_UNIT,
  ...
  // Client is resending autoshot cast opcode when other spell is casted during
  // shoot rotation Skip it to prevent "interrupt" message
...
void WorldSession::HandleCancelAuraOpcode(WorldPacket &recvPacket) {
  ...
  // not allow remove non positive spells and spells with attr
  // SPELL_ATTR0_CANT_CANCEL
  if ((!spellInfo->IsPositive() ||
       spellInfo->HasAttribute(SPELL_ATTR0_CANT_CANCEL) ||
       spellInfo->IsPassive()) &&
      spellId != 605)
    return;
...
void WorldSession::HandlePetCancelAuraOpcode(WorldPacket &recvPacket) {
  ...
  if (!pet) {
    LOG_ERROR("server",
              "HandlePetCancelAura: Attempt to cancel an aura for non-existant "
              "pet %u by player '%s'",
  ...
  if (pet != GetPlayer()->GetGuardianPet() && pet != GetPlayer()->GetCharm()) {
    LOG_ERROR("server",
              "HandlePetCancelAura: Pet %u is not a pet of player '%s'",
              uint32(GUID_LOPART(guid)), GetPlayer()->GetName().c_str());
    return;
  }
...
void WorldSession::HandleSelfResOpcode(WorldPacket & /*recvData*/) {
  ...
  if (_player->HasAuraType(SPELL_AURA_PREVENT_RESURRECTION))
    return; // silent return, client should display error by itself and not send
            // this opcode

  if (_player->GetUInt32Value(PLAYER_SELF_RES_SPELL)) {
    SpellInfo const *spellInfo =
        sSpellMgr->GetSpellInfo(_player->GetUInt32Value(PLAYER_SELF_RES_SPELL));
    if (spellInfo) {
...
void WorldSession::HandleSpellClick(WorldPacket &recvData) {
  ...
  // TODO: Unit::SetCharmedBy: 28782 is not in world but 0 is trying to charm
  // it! -> crash
  if (!unit->IsInWorld())
    return;
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket &recvData) {
  ...
  Unit *creator =
      unit->GetAuraEffectsByType(SPELL_AURA_CLONE_CASTER).front()->GetCaster();
  if (!creator)
    return;
  ...
  if (creator->GetTypeId() == TYPEID_PLAYER) {
    ...
    static EquipmentSlots const itemSlots[] = {
        ...
        EQUIPMENT_SLOT_BACK,  EQUIPMENT_SLOT_TABARD,    EQUIPMENT_SLOT_END};

    // Display items in visible slots
    for (EquipmentSlots const *itr = &itemSlots[0]; *itr != EQUIPMENT_SLOT_END;
         ++itr) {
      if (*itr == EQUIPMENT_SLOT_HEAD &&
          player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_HELM))
        data << uint32(0);
