...
void WorldSession::HandleCalendarGetCalendar(WorldPacket & /*recvData*/) {
  ...
  for (uint8 i = 0; i < MAX_DIFFICULTY; ++i) {
    BoundInstancesMap const &m_boundInstances =
        sInstanceSaveMgr->PlayerGetBoundInstances(_player->GetGUIDLow(),
                                                  Difficulty(i));
    for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
      ...
      if (itr->second.perm) {
        ...
        dataBuffer << uint64(MAKE_NEW_GUID(
            save->GetInstanceId(), 0,
            HIGHGUID_INSTANCE)); // instance save id as unique instance copy id
        ++boundCounter;
      }
    }
  }
...
void WorldSession::HandleCalendarGetEvent(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_GET_EVENT. Player [" UI64FMTD "] Event [" UI64FMTD
            "]",
            _player->GetGUID(), eventId);

  if (CalendarEvent *calendarEvent = sCalendarMgr->GetEvent(eventId))
    sCalendarMgr->SendCalendarEvent(_player->GetGUID(), *calendarEvent,
...
void WorldSession::HandleCalendarGuildFilter(WorldPacket &recvData) {
  LOG_DEBUG("network", "CMSG_CALENDAR_GUILD_FILTER [" UI64FMTD "]",
            _player->GetGUID());
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_GUILD_FILTER: Min level [%d], Max level [%d], Min "
            "rank [%d]",
            minLevel, maxLevel, minRank);
}

void WorldSession::HandleCalendarArenaTeam(WorldPacket &recvData) {
  LOG_DEBUG("network", "CMSG_CALENDAR_ARENA_TEAM [" UI64FMTD "]",
            _player->GetGUID());
...
bool validUtf8String(WorldPacket &recvData, std::string &s, std::string action,
                     uint64 playerGUID) {
  if (!utf8::is_valid(s.begin(), s.end())) {
    LOG_INFO("server",
             "CalendarHandler: Player with guid %llu attempt to %s an event "
             "with invalid name or description (packet modification)",
             (unsigned long long)playerGUID, action.c_str());
    recvData.rfinish();
    return false;
  }
  return true;
...
void WorldSession::HandleCalendarAddEvent(WorldPacket &recvData) {
  ...
  SetCalendarEventCreationCooldown(time(nullptr) +
                                   CALENDAR_CREATE_EVENT_COOLDOWN);

  CalendarEvent *calendarEvent = new CalendarEvent(
      sCalendarMgr->GetFreeEventId(), guid, 0, CalendarEventType(type),
      dungeonId, time_t(eventPackedTime), flags, time_t(unkPackedTime), title,
      description);
  ...
  if (calendarEvent->IsGuildAnnouncement()) {
    ...
    CalendarInvite *invite = new CalendarInvite(
        0, calendarEvent->GetEventId(), 0, guid, 946684800,
        CALENDAR_STATUS_NOT_SIGNED_UP, CALENDAR_RANK_PLAYER, "");
    ...
    for (uint32 i = 0; i < inviteCount && i < CALENDAR_MAX_INVITES; ++i) {
      // 946684800 is 01/01/2000 00:00:00 - default response time
      CalendarInvite *invite = new CalendarInvite(
          sCalendarMgr->GetFreeInviteId(), calendarEvent->GetEventId(),
          invitee[i], guid, 946684800, CalendarInviteStatus(status[i]),
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_UPDATE_EVENT [" UI64FMTD "] EventId [" UI64FMTD
            "], InviteId [" UI64FMTD "] Title %s, Description %s, type %u "
            "Repeatable %u, MaxInvites %u, Dungeon ID %d, Time %u "
            "Time2 %u, Flags %u",
            guid, eventId, inviteId, title.c_str(), description.c_str(), type,
            repetitionType, maxInvites, dungeonId, eventPackedTime,
            timeZoneTime, flags);
...
void WorldSession::HandleCalendarCopyEvent(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_COPY_EVENT [" UI64FMTD "], EventId [" UI64FMTD
            "] inviteId [" UI64FMTD "] Time: %u",
            guid, eventId, inviteId, eventTime);
  ...
  if (CalendarEvent *oldEvent = sCalendarMgr->GetEvent(eventId)) {
    ...
    for (CalendarInviteStore::const_iterator itr = invites.begin();
      ...
      sCalendarMgr->AddInvite(
          newEvent,
          new CalendarInvite(**itr, sCalendarMgr->GetFreeInviteId(),
                             newEvent->GetEventId()),
          trans);
...
void WorldSession::HandleCalendarEventInvite(WorldPacket &recvData) {
  ...
  if (QueryResult result = CharacterDatabase.PQuery(
          "SELECT flags FROM character_social WHERE guid = " UI64FMTD
          " AND friend = " UI64FMTD,
  ...
  if (!isPreInvite) {
    if (CalendarEvent *calendarEvent = sCalendarMgr->GetEvent(eventId)) {
      ...
      // 946684800 is 01/01/2000 00:00:00 - default response time
      CalendarInvite *invite = new CalendarInvite(
          sCalendarMgr->GetFreeInviteId(), eventId, inviteeGuid, playerGuid,
          946684800, CALENDAR_STATUS_INVITED, CALENDAR_RANK_PLAYER, "");
      sCalendarMgr->AddInvite(calendarEvent, invite);
    } else
      sCalendarMgr->SendCalendarCommandResult(playerGuid,
                                              CALENDAR_ERROR_EVENT_INVALID);
    ...
    CalendarInvite *invite =
        new CalendarInvite(inviteId, 0, inviteeGuid, playerGuid, 946684800,
                           CALENDAR_STATUS_INVITED, CALENDAR_RANK_PLAYER, "");
...
void WorldSession::HandleCalendarEventSignup(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_EVENT_SIGNUP [" UI64FMTD "] EventId [" UI64FMTD
            "] Tentative %u",
            guid, eventId, tentative);
  ...
  if (CalendarEvent *calendarEvent = sCalendarMgr->GetEvent(eventId)) {
    ...
    CalendarInvite *invite =
        new CalendarInvite(sCalendarMgr->GetFreeInviteId(), eventId, guid, guid,
                           time(nullptr), status, CALENDAR_RANK_PLAYER, "");
    sCalendarMgr->AddInvite(calendarEvent, invite);
...
void WorldSession::HandleCalendarEventRsvp(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_EVENT_RSVP [" UI64FMTD "] EventId [" UI64FMTD
            "], InviteId [" UI64FMTD "], status %u",
...
void WorldSession::HandleCalendarEventRemoveInvite(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_EVENT_REMOVE_INVITE [" UI64FMTD
            "] EventId [" UI64FMTD "], ownerInviteId [" UI64FMTD
            "], Invitee ([" UI64FMTD "] id: [" UI64FMTD "])",
...
void WorldSession::HandleCalendarEventStatus(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_EVENT_STATUS [" UI64FMTD "] EventId [" UI64FMTD
            "] ownerInviteId [" UI64FMTD "], Invitee ([" UI64FMTD
            "] id: [" UI64FMTD "], status %u",
            guid, eventId, ownerInviteId, invitee, inviteId, status);

  if (CalendarEvent *calendarEvent = sCalendarMgr->GetEvent(eventId)) {
    if (CalendarInvite *invite = sCalendarMgr->GetInvite(inviteId)) {
      invite->SetStatus((CalendarInviteStatus)status);
      // not sure if we should set response time when moderator changes invite
      // status
      // invite->SetStatusTime(time(nullptr));
...
void WorldSession::HandleCalendarEventModeratorStatus(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_EVENT_MODERATOR_STATUS [" UI64FMTD
            "] EventId [" UI64FMTD "] ownerInviteId [" UI64FMTD
            "], Invitee ([" UI64FMTD "] id: [" UI64FMTD "], rank %u",
...
void WorldSession::HandleCalendarComplain(WorldPacket &recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_COMPLAIN [" UI64FMTD "] EventId [" UI64FMTD
            "] guid [" UI64FMTD "]",
...
void WorldSession::HandleCalendarGetNumPending(WorldPacket & /*recvData*/) {
  ...
  LOG_DEBUG("network",
            "CMSG_CALENDAR_GET_NUM_PENDING: [" UI64FMTD "] Pending: %u", guid,
            pending);
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket &recvData) {
  ...
  InstancePlayerBind *instanceBind = sInstanceSaveMgr->PlayerGetBoundInstance(
      GetPlayer()->GetGUIDLow(), mapId, Difficulty(difficulty));
