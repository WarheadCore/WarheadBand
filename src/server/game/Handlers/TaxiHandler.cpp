...
void WorldSession::SendTaxiStatus(uint64 guid) {
  ...
  uint32 curloc = sObjectMgr->GetNearestTaxiNode(
      ...
      unit->GetMapId(), GetPlayer()->GetTeamId());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleTaxiQueryAvailableNodes - Unit (GUID: %u) not "
  ...
  if (unit->GetEntry() == 29480)
    GetPlayer()->SetTaxiCheater(
        true); // Grimwing in Ebon Hold, special case. NOTE: Not perfect,
               // Zul'Aman should not be included according to WoWhead, and I
               // think taxicheat includes it.
...
bool WorldSession::SendLearnNewTaxiNode(Creature *unit) {
  ...
  if (curloc == 0)
    return true; // `true` send to avoid WorldSession::SendTaxiMenu call with
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleActivateTaxiExpressOpcode - Unit (GUID: %u) not "
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATETAXIEXPRESS from %d to %d",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Received CMSG_ACTIVATETAXI from %d to %d",
            nodes[0], nodes[1]);
#endif
  Creature *npc =
      GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_FLIGHTMASTER);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleActivateTaxiOpcode - Unit (GUID: %u) not found or "
