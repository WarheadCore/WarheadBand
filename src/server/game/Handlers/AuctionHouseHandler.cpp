...
void WorldSession::HandleAuctionHelloOpcode(WorldPacket &recvData) {
  ...
  if (!unit) {
    ...
    LOG_DEBUG("network",
              "WORLD: HandleAuctionHelloOpcode - Unit (GUID: %u) not found or "
...
void WorldSession::SendAuctionCommandResult(uint32 auctionId, uint32 Action,
                                            uint32 ErrorCode, uint32 bidError) {
...
void WorldSession::HandleAuctionSellItem(WorldPacket &recvData) {
  ...
  uint64 itemGUIDs[MAX_AUCTION_ITEMS]; // 160 slot = 4x 36 slot bag + backpack
                                       // 16 slot
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleAuctionSellItem - Player %s (GUID %u) attempted to "
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleAuctionSellItem - Unit (GUID: %u) not found or you "
  ...
  for (uint32 i = 0; i < itemsCount; ++i) {
    ...
    if (sAuctionMgr->GetAItem(item->GetGUIDLow()) || !item->CanBeTraded() ||
        item->IsNotEmptyBag() ||
  ...
  for (uint32 i = 0; i < itemsCount; ++i) {
    ...
    // Required stack size of auction matches to current item stack size, just
    // move item to auctionhouse
    if (itemsCount == 1 && item->GetCount() == count[i]) {
      ...
      LOG_DEBUG(
          "server",
          "CMSG_AUCTION_SELL_ITEM: Player %s (guid %d) is selling item %s "
          "entry %u (guid %d) to auctioneer %u with count %u with initial bid "
          "%u with buyout %u and with time %u (in sec) in auctionhouse %u",
          _player->GetName().c_str(), _player->GetGUIDLow(),
          item->GetTemplate()->Name1.c_str(), item->GetEntry(),
          item->GetGUIDLow(), AH->auctioneer, item->GetCount(), bid, buyout,
           ...
           // stack size, clone item and set correct stack size
    {
      ...
      if (!newItem) {
        LOG_ERROR("server",
                  "CMSG_AUCTION_SELL_ITEM: Could not create clone of item %u",
                  item->GetEntry());
        SendAuctionCommandResult(0, AUCTION_SELL_ITEM,
                                 ERR_AUCTION_DATABASE_ERROR);
        return;
      ...
      LOG_DEBUG(
          ...
          "%u with buyout %u and with time %u (in sec) in auctionhouse %u",
          _player->GetName().c_str(), _player->GetGUIDLow(),
          newItem->GetTemplate()->Name1.c_str(), newItem->GetEntry(),
          newItem->GetGUIDLow(), AH->auctioneer, newItem->GetCount(), bid,
          buyout, auctionTime, AH->GetHouseId());
#endif
      ...
      for (uint32 j = 0; j < itemsCount; ++j) {
        ...
        // Item stack count equals required count, ready to delete item - cloned
        // item will be used for auction
        if (item2->GetCount() == count[j]) {
          _player->MoveItemFromInventory(item2->GetBagSlot(), item2->GetSlot(),
                                         true);
          ...
          CharacterDatabase.CommitTransaction(trans);
          delete item2;
        } else // Item stack count is bigger than required count, update item
               // stack count and save to database - cloned item will be used
               // for auction
        {
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleAuctionPlaceBid - Unit (GUID: %u) not found or you "
              "can't interact with him.",
  ...
  // impossible have online own another character (use this for speedup check in
  // case online owner)
  Player *auction_owner = ObjectAccessor::FindPlayerInOrOutOfWorld(
      MAKE_NEW_GUID(auction->owner, 0, HIGHGUID_PLAYER));
  if (!auction_owner &&
      sObjectMgr->GetPlayerAccountIdByGUID(MAKE_NEW_GUID(
          auction->owner, 0, HIGHGUID_PLAYER)) == GetAccountId()) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleAuctionRemoveItem - Unit (GUID: %u) not found or "
              "you can't interact with him.",
              uint32(GUID_LOPART(auctioneer)));
#endif
    return;
  ...
  if (auction && auction->owner == player->GetGUIDLow()) {
    ...
    if (pItem) {
          ...
          0) // If we have a bidder, we have to send him the money he paid
      {
        ...
        if (!player->HasEnoughMoney(auctionCut)) // player doesn't have enough
                                                 // money, maybe message needed
          return;
        // some auctionBidderNotification would be needed, but don't know that
        // parts..
    ...
    LOG_ERROR("server",
              ...
              "player, or auction is nullptr",
              player->GetGUIDLow(), auctionId);
    return;
  }
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket &recvData) {
  ...
  if (recvData.size() != (16 + outbiddedCount * 4)) {
    LOG_ERROR(
        ...
        outbiddedCount, (unsigned long)recvData.size(),
        (16 + outbiddedCount * 4));
    outbiddedCount = 0;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleAuctionListBidderItems - Unit (GUID: %u) not found "
...
void WorldSession::HandleAuctionListOwnerItems(WorldPacket &recvData) {
  ...
  _lastAuctionListOwnerItemsMSTime =
      now + delay; // set longest possible here, actual exectuing will change
                   // this to getMSTime of that moment
  _player->m_Events.AddEvent(
      new AuctionListOwnerItemsDelayEvent(guid, _player->GetGUID(), true),
      _player->m_Events.CalculateTime(delay - diff));
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleAuctionListOwnerItems - Unit (GUID: %u) not found "
              "or you can't interact with him.",
              uint32(GUID_LOPART(creatureGuid)));
#endif
    return;
  }
...
void WorldSession::HandleAuctionListItems(WorldPacket &recvData) {
  ...
  AsyncAuctionListingMgr::GetTempList().push_back(AuctionListItemsDelayEvent(
      delay - diff, _player->GetGUID(), guid, searchedname, listfrom, levelmin,
      levelmax, usable, auctionSlotID, auctionMainCategory, auctionSubCategory,
      quality, getAll));
}
