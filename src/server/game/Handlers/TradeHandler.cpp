...
void WorldSession::SendUpdateTrade(bool trader_data /*= true*/) {
  ...
  data << uint32(0); // added in 2.4.0, this value must be equal to value from
                     // TRADE_STATUS_OPEN_WINDOW status packet (different value
                     // for different players to block multiple trades?)
  ...
  for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i) {
    ...
    if (Item *item = view_trade->GetItem(TradeSlots(i))) {
      ...
      data << uint32(
          item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED) ? 1 : 0);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "player trade item %u bag: %u slot: %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "partner trade item %u bag: %u slot: %u",
                item->GetGUIDLow(), item->GetBagSlot(), item->GetSlot());
#endif
      hisItems[i] = item;
  ...
  if (his_trade->IsAccepted()) {
    ...
    if (my_trade->GetMoney() >= 10 * GOLD) {
      CharacterDatabase.PExecute(
          ...
          "%u, \"<TRADE>\", NOW())",
          GetAccountId(), _player->GetGUIDLow(), _player->GetName().c_str(),
          GetRemoteAddress().c_str(), trader->GetSession()->GetAccountId(),
          trader->GetName().c_str(), my_trade->GetMoney());
    }
    if (his_trade->GetMoney() >= 10 * GOLD) {
      CharacterDatabase.PExecute(
          "INSERT INTO log_money VALUES(%u, %u, \"%s\", \"%s\", %u, \"%s\", "
          "%u, \"<TRADE>\", NOW())",
          trader->GetSession()->GetAccountId(), trader->GetGUIDLow(),
          trader->GetName().c_str(),
          trader->GetSession()->GetRemoteAddress().c_str(), GetAccountId(),
          _player->GetName().c_str(), his_trade->GetMoney());
    }
...
void WorldSession::HandleSetTradeItemOpcode(WorldPacket &recvPacket) {
  ...
  if (!item ||
      (tradeSlot != TRADE_SLOT_NONTRADED && !item->CanBeTraded(false, true))) {
    SendTradeStatus(TRADE_STATUS_TRADE_CANCELED);
    return;
  }
