...
void WorldSession::HandleInspectArenaTeamsOpcode(WorldPacket &recvData) {
  ...
  recvData >> guid;
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "Inspect Arena stats (GUID: %u TypeId: %u)",
...
void WorldSession::HandleArenaTeamInviteOpcode(WorldPacket &recvData) {
  ...
  if (player->GetArenaTeamId(arenaTeam->GetSlot())) {
    SendArenaTeamCommandResult(ERR_ARENA_TEAM_INVITE_SS, "", invitedName,
  ...
  if (arenaTeam->GetMembersSize() >= arenaTeam->GetType() * 2) {
    SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, arenaTeam->GetName(),
                               "", ERR_ARENA_TEAM_TOO_MANY_MEMBERS_S);
    return;
  }

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("bg.battleground", "Player %s Invited %s to Join his ArenaTeam",
            GetPlayer()->GetName().c_str(), invitedName.c_str());
#endif
...
void WorldSession::HandleArenaTeamAcceptOpcode(WorldPacket & /*recvData*/) {
  ...
  if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
      _player->GetTeamId() !=
          sObjectMgr->GetPlayerTeamIdByGUID(arenaTeam->GetCaptain())) {
    SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, "", "",
                               ERR_ARENA_TEAM_NOT_ALLIED);
    return;
  }
  ...
  arenaTeam->BroadcastEvent(ERR_ARENA_TEAM_JOIN_SS, _player->GetGUID(), 2,
                            _player->GetName().c_str(), arenaTeam->GetName(),
                            "");
}
