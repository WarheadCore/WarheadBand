...
void WorldSession::HandleJoinChannel(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("chat.system",
            "CMSG_JOIN_CHANNEL %s Channel: %u, unk1: %u, unk2: %u, channel: "
            "%s, password: %s",
            GetPlayerInfo().c_str(), channelId, unknown1, unknown2,
            channelName.c_str(), password.c_str());
#endif
  if (channelId) {
  ...
  // pussywizard: restrict allowed characters in channel name to avoid |0 and
  // possibly other exploits
  // if (!ObjectMgr::IsValidChannelName(channelName))
  if (channelName.find("|") != std::string::npos || channelName.size() >= 100 ||
      !utf8::is_valid(channelName.begin(), channelName.end()))
    return;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("chat.system",
            "CMSG_CHANNEL_UNMODERATOR %s Channel: %s, Target: %s",
            GetPlayerInfo().c_str(), channelName.c_str(), targetName.c_str());
#endif
  if (!normalizePlayerName(targetName))
    return;
...
void WorldSession::HandleGetChannelMemberCount(WorldPacket &recvPacket) {
  ...
  if (ChannelMgr *cMgr = ChannelMgr::forTeam(GetPlayer()->GetTeamId())) {
    if (Channel *channel = cMgr->GetChannel(channelName, GetPlayer())) {
      LOG_DEBUG("chat.system",
                "SMSG_CHANNEL_MEMBER_COUNT %s Channel: %s Count: %u",
                GetPlayerInfo().c_str(), channelName.c_str(),
