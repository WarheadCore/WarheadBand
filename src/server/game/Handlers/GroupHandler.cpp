...
void WorldSession::SendPartyResult(PartyOperation operation,
                                   const std::string &member, PartyResult res,
                                   uint32 val /* = 0 */) {
  ...
  data << uint32(res);
  data << uint32(
      val); // LFD cooldown related (used with ERR_PARTY_LFG_BOOT_COOLDOWN_S and
...
void WorldSession::HandleGroupInviteOpcode(WorldPacket &recvData) {
  ...
  if (player->GetInstanceId() != 0 &&
      player->GetDungeonDifficulty() != GetPlayer()->GetDungeonDifficulty()) {
  ...
  if (!player->GetSocial()->HasFriend(GetPlayer()->GetGUID()) &&
      GetPlayer()->getLevel() < sWorld->getIntConfig(CONFIG_PARTY_LEVEL_REQ)) {
  ...
  if (!group && GetPlayer()->GetGroupInvite() &&
      GetPlayer()->GetGroupInvite()->GetLeaderGUID() == GetPlayer()->GetGUID())
...
void WorldSession::HandleGroupAcceptOpcode(WorldPacket &recvData) {
  ...
  if (group->GetLeaderGUID() == GetPlayer()->GetGUID()) {
    LOG_ERROR("server",
              "HandleGroupAcceptOpcode: player %s(%d) tried to accept an "
  ...
  Player *leader =
      ObjectAccessor::FindPlayerInOrOutOfWorld(group->GetLeaderGUID());
...
void WorldSession::HandleGroupUninviteGuidOpcode(WorldPacket &recvData) {
  ...
  if (guid == GetPlayer()->GetGUID()) {
    LOG_ERROR("server",
              "WorldSession::HandleGroupUninviteGuidOpcode: leader %s(%d) "
  ...
  // Xinef: do not allow to kick with empty reason, this will resend packet with
  // given reason
...
void WorldSession::HandleGroupUninviteOpcode(WorldPacket &recvData) {
  ...
  if (GetPlayer()->GetName() == membername) {
    LOG_ERROR("server",
              "WorldSession::HandleGroupUninviteOpcode: leader %s(%d) tried to "
...
void WorldSession::HandleLootRoll(WorldPacket &recvData) {
  ...
  switch (rollType) {
  case ROLL_NEED:
    GetPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_ROLL_NEED,
                                           1);
    break;
  case ROLL_GREED:
    GetPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED,
                                           1);
    break;
  }
}
...
void WorldSession::HandlePartyAssignmentOpcode(WorldPacket &recvData) {
  ...
  case GROUP_ASSIGN_MAINTANK:
    group->RemoveUniqueGroupMemberFlag(
        MEMBER_FLAG_MAINTANK); // Remove main assist flag from current if any.
  ...
  *data << uint32(mask);
  ...
  if (mask & GROUP_UPDATE_FLAG_VEHICLE_SEAT) {
    if (Vehicle *veh = player->GetVehicle())
      *data << uint32(veh->GetVehicleInfo()
