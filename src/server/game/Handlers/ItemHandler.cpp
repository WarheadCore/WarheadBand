...
void WorldSession::HandleSwapInvItemOpcode(WorldPacket &recvData) {
  ...
  if (_player->IsBankPos(INVENTORY_SLOT_BAG_0, srcslot) && !CanUseBank()) {
    // TC_LOG_DEBUG("network", "WORLD: HandleSwapInvItemOpcode - Unit (GUID: %u)
    // not found or you can't interact with him.",
    // uint32(GUID_LOPART(m_currentBankerGUID)));
    return;
  }
...
void WorldSession::HandleSwapItem(WorldPacket &recvData) {
  ...
  if (_player->IsBankPos(srcbag, srcslot) && !CanUseBank()) {
    // TC_LOG_DEBUG("network", "WORLD: HandleSwapItem - Unit (GUID: %u) not
...
void ItemTemplate::InitializeQueryData() {
  ...
  queryData << Name1;
  queryData << uint8(0x00); // pProto->Name2; // blizz not send name there, just
  ...
  for (int s = 0; s < MAX_ITEM_PROTO_SPELLS; ++s) {
    ...
    // Spell::SendSpellCooldown use `item_template` or if not set then only use
    // spell cooldowns
    SpellInfo const *spell = sSpellMgr->GetSpellInfo(Spells[s].SpellId);
    if (spell) {
      bool db_data =
          Spells[s].SpellCooldown >= 0 || Spells[s].SpellCategoryCooldown >= 0;
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: CMSG_ITEM_QUERY_SINGLE - NO item INFO! (ENTRY: %u)",
              item);
#endif
...
void WorldSession::HandleSellItemOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleSellItemOpcode - Unit (GUID: %u) not found or you "
  ...
  if ((creature->GetCreatureTemplate()->flags_extra &
    ...
    _player->SendSellError(SELL_ERR_CANT_SELL_TO_THIS_MERCHANT, creature,
                           itemguid, 0);
    return;
  }
  ...
  if (pItem) {
    ...
    // this probably happens when right clicking a refundable item, the client
    // sends both CMSG_SELL_ITEM and CMSG_REFUND_ITEM (unverified)
    ...
    if (count == 0) {
      ...
      // prevent sell more items that exist in stack (possible only not from
      // client)
    ...
    if (pProto) {
      if (pProto->SellPrice > 0) {
        ...
        if (count < pItem->GetCount()) // need split items
        {
          ...
          if (!pNewItem) {
            LOG_ERROR("server",
                      "WORLD: HandleSellItemOpcode - could not create clone of "
                      "item %u; count = %u",
                      pItem->GetEntry(), count);
...
void WorldSession::HandleBuybackItem(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleBuybackItem - Unit (GUID: %u) not found or you can "
  ...
  if (pItem) {
    uint32 price = _player->GetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE_1 + slot -
                                           BUYBACK_SLOT_START);
    if (!_player->HasEnoughMoney(price)) {
      _player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, creature,
                            pItem->GetEntry(), 0);
      return;
    }
...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPacket &recvData) {
  ...
  GetPlayer()->BuyItemFromVendorSlot(vendorguid, slot, item, count, bag,
                                     bagslot);
}
...
void WorldSession::HandleBuyItemOpcode(WorldPacket &recvData) {
  ...
  GetPlayer()->BuyItemFromVendorSlot(vendorguid, slot, item, count, NULL_BAG,
                                     NULL_SLOT);
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: SendListInventory - Unit (GUID: %u) not found or you can "
              "not interact with him.",
              uint32(GUID_LOPART(vendorGuid)));
#endif
    _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, 0, 0);
    ...
    if (VendorItem const *item = items->GetItem(slot)) {
      if (ItemTemplate const *itemTemplate =
        ...
        ConditionList conditions =
            sConditionMgr->GetConditionsForNpcVendorEvent(vendor->GetEntry(),
                                                          item->item);
        if (!sConditionMgr->IsObjectMeetToConditions(_player, vendor,
                                                     conditions)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("network",
                    "SendListInventory: conditions not met for creature entry "
                    "%u item %u",
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket &recvPacket) {
  ...
  if (!CanUseBank(guid)) {
    // TC_LOG_DEBUG("network", "WORLD: HandleBuyBankSlotOpcode - Unit (GUID: %u)
    // not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
    return;
  }
...
void WorldSession::HandleAutoBankItemOpcode(WorldPacket &recvPacket) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "STORAGE: receive srcbag = %u, srcslot = %u", srcbag,
            srcslot);
#endif

  if (!CanUseBank()) {
    // TC_LOG_DEBUG("network", "WORLD: HandleAutoBankItemOpcode - Unit (GUID:
    // %u) not found or you can't interact with him.",
    // uint32(GUID_LOPART(m_currentBankerGUID)));
    return;
  }
  ...
  ItemPosCountVec dest;
  InventoryResult msg =
      _player->CanBankItem(NULL_BAG, NULL_SLOT, dest, pItem, false);
  if (msg != EQUIP_ERR_OK) {
    _player->SendEquipError(msg, pItem, nullptr);
    return;
  }
...
void WorldSession::HandleAutoStoreBankItemOpcode(WorldPacket &recvPacket) {
  ...
  if (!CanUseBank()) {
    // TC_LOG_DEBUG("network", "WORLD: HandleAutoStoreBankItemOpcode - Unit
...
void WorldSession::SendEnchantmentLog(uint64 target, uint64 caster,
                                      uint32 itemId, uint32 enchantId) {
...
void WorldSession::HandleWrapItemOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, "
            "item_slot = %u",
  ...
  if (item->GetUInt64Value(ITEM_FIELD_GIFTCREATOR)) // HasFlag(ITEM_FIELD_FLAGS,
                                                    // ITEM_FLAGS_WRAPPED);
  {
    _player->SendEquipError(EQUIP_ERR_WRAPPED_CANT_BE_WRAPPED, item, nullptr);
    return;
...
void WorldSession::HandleSocketOpcode(WorldPacket &recvData) {
  ...
  for (int i = 0; i < MAX_GEM_SOCKETS; ++i) {
    ...
    // continue check for case when attempt add 2 similar unique equipped gems
    // in one item.
      ...
      for (int j = 0; j < MAX_GEM_SOCKETS; ++j) {
        ...
        if (Gems[j]) {
          if (iGemProto->ItemId == Gems[j]->GetEntry()) {
            _player->SendEquipError(EQUIP_ERR_ITEM_UNIQUE_EQUIPPABLE_SOCKETED,
                                    itemTarget, nullptr);
            return;
          }
    ...
    if (iGemProto->ItemLimitCategory) {
      if (ItemLimitCategoryEntry const *limitEntry =
        ...
        // NOTE: limitEntry->mode is not checked because if item has limit then
        // it is applied in equip case
        for (int j = 0; j < MAX_GEM_SOCKETS; ++j) {
          if (Gems[j]) {
            // new gem
            if (iGemProto->ItemLimitCategory ==
  ...
  for (int i = 0; i < MAX_GEM_SOCKETS; ++i) {
    if (GemEnchants[i]) {
      itemTarget->SetEnchantment(EnchantmentSlot(SOCK_ENCHANTMENT_SLOT + i),
                                 GemEnchants[i], 0, 0, _player->GetGUID());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_ITEM_TEXT_QUERY item guid: %u",
...
bool WorldSession::CanUseBank(uint64 bankerGUID) const {
  ...
  if (!isUsingBankCommand) {
    Creature *creature =
        GetPlayer()->GetNPCIfCanInteractWith(bankerGUID, UNIT_NPC_FLAG_BANKER);
    if (!creature)
      return false;
  }
