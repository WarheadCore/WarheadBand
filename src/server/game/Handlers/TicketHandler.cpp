...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket &recvData) {
  // Don't accept tickets if the ticket queue is disabled. (Ticket UI is greyed
  // out but not fully dependable)
  ...
  if (!ticket || ticket->IsClosed()) {
    ...
    if (count && decompressedSize && decompressedSize < 0xFFFF) {
      ...
      if (uncompress(dest.contents(), &realSize, recvData.contents() + pos,
    ...
    sWorld->SendGMText(LANG_COMMAND_TICKETNEW, GetPlayer()->GetName().c_str(),
                       ticket->GetId());
...
void WorldSession::HandleGMTicketUpdateOpcode(WorldPacket &recv_data) {
  ...
  if (GmTicket *ticket =
    ...
    sWorld->SendGMText(LANG_COMMAND_TICKETUPDATED,
                       GetPlayer()->GetName().c_str(), ticket->GetId());
...
void WorldSession::HandleGMSurveySubmit(WorldPacket &recv_data) {
  ...
  for (uint8 i = 0; i < 10; i++) {
    uint32 subSurveyId; // ref to i'th GMSurveySurveys.dbc field (all fields in
