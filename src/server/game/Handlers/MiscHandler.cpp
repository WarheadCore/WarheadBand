...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket &recv_data) {
  ...
  if (IS_CRE_OR_VEH_GUID(guid)) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("network",
                "WORLD: HandleGossipSelectOptionOpcode - Unit (GUID: %u) not "
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("network",
                "WORLD: HandleGossipSelectOptionOpcode - GameObject (GUID: %u) "
                "not found.",
                uint32(GUID_LOPART(guid)));
#endif
      return;
    ...
    if (!item || _player->IsBankPos(item->GetPos())) {
      // TC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - %s not
      // found.", guid.ToString().c_str());
      return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleGossipSelectOptionOpcode - unsupported GUID type "
              "for highguid %u. lowpart %u.",
  ...
  if ((unit &&
       unit->GetCreatureTemplate()->ScriptID != unit->LastUsedScriptID) ||
...
void WorldSession::HandleWhoOpcode(WorldPacket &recvData) {
  ...
  recvData >> str_count; // user entered strings count, client limit=4 (checked
                         // on 2.0.10)
  ...
  LOG_DEBUG("network",
            "Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask "
            "%u, zones %u, strings %u",
  ...
  // client send in case not set max level value 100 but Trinity supports 255
  ...
  for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin();
    ...
    if (AreaTableEntry const *areaEntry =
            sAreaTableStore.LookupEntry(itr->second->GetZoneId()))
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far "
              "(trigger map: %u player map: %u), ignore Area Trigger ID: %u",
              player->GetName().c_str(), atEntry->map, player->GetMapId(),
              player->GetGUIDLow(), triggerId);
#endif
    return;
  }
...
void WorldSession::HandleUpdateAccountData(WorldPacket &recv_data) {
  ...
  if (decompressedSize > 0xFFFF) {
    ...
    LOG_ERROR("server", "UAD: Account data packet too big, size %u",
              decompressedSize);
    return;
  }
  ...
  if (!packetData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "MISC: Added Macro %u into button %u", action,
                button);
#endif
      break;
    case ACTION_BUTTON_EQSET:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "MISC: Added EquipmentSet %u into button %u", action,
                button);
#endif
      break;
    case ACTION_BUTTON_SPELL:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "MISC: Added Spell %u into button %u", action,
                button);
#endif
      break;
    ...
    default:
      LOG_ERROR("server",
                ...
                "%u for player %s (GUID: %u)",
                type, action, button, _player->GetName().c_str(),
                _player->GetGUIDLow());
      return;
    }
...
void WorldSession::HandleInspectOpcode(WorldPacket &recv_data) {
  ...
  if (!player) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "CMSG_INSPECT: No player found from GUID: " UI64FMTD,
              guid);
#endif
    return;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "MSG_INSPECT_HONOR_STATS: No player found from GUID: " UI64FMTD,
              guid);
#endif
    return;
  }
...
void WorldSession::HandleComplainOpcode(WorldPacket &recv_data) {
  ...
  case 1:
    ...
    recv_data >> description; // spam description string (messagetype, channel
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u, unk4 "
            "%u, message %s",
...
void WorldSession::HandleFarSightOpcode(WorldPacket &recvData) {
  ...
  if (apply) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "Added FarSight " UI64FMTD " to player %u",
              _player->GetUInt64Value(PLAYER_FARSIGHT), _player->GetGUIDLow());
#endif
    if (WorldObject *target = _player->GetViewpoint())
      _player->SetSeer(target);
    else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_ERROR("server", "Player %s requests non-existing seer " UI64FMTD,
...
void WorldSession::HandleTimeSyncResp(WorldPacket &recv_data) {
  ...
  // uint32 ourTicks = clientTicks + (World::GetGameTimeMS() -
  // _player->m_timeSyncServer);
...
void WorldSession::HandleSetDungeonDifficultyOpcode(WorldPacket &recv_data) {
  ...
  if (group) {
    ...
    Player::ResetInstances(_player->GetGUIDLow(),
                           INSTANCE_RESET_CHANGE_DIFFICULTY, false);
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket &recv_data) {
  ...
  if (group) {
    if (group->IsLeader(_player->GetGUID())) {
      ...
      if (uint32 preventionTime = group->GetDifficultyChangePreventionTime()) {
        ...
        case DIFFICULTY_PREVENTION_CHANGE_BOSS_KILLED:
          ChatHandler(this).PSendSysMessage(
              "Raid was in combat recently and may not change difficulty again "
              "for %u sec.",
              preventionTime);
          break;
        ...
        default:
          ChatHandler(this).PSendSysMessage(
              "Raid difficulty has changed recently, and may not change again "
              "for %u sec.",
              preventionTime);
          break;
        }
      ...
      for (GroupReference *itr = group->GetFirstMember(); itr != nullptr;
        ...
        if (IsSharedDifficultyMap(groupGuy->GetMap()->GetId()) &&
            (_player->GetRaidDifficulty() >= 0 &&
             uint32(mode % 2) == uint32(_player->GetRaidDifficulty() % 2)) &&
            group->isRaidGroup()) {
          if (!currMap)
          ...
          if (!groupGuy->IsAlive() || groupGuy->IsInCombat() ||
              groupGuy->GetVictim() || groupGuy->m_mover != groupGuy ||
              groupGuy->IsNonMeleeSpellCast(true) ||
              (!groupGuy->GetMotionMaster()->empty() &&
               groupGuy->GetMotionMaster()->GetCurrentMovementGeneratorType() !=
                   IDLE_MOTION_TYPE) ||
              ...
              !groupGuy->Satisfy(
                  sObjectMgr->GetAccessRequirement(groupGuy->GetMap()->GetId(),
                                                   Difficulty(mode)),
                  groupGuy->GetMap()->GetId(), false)) {
            _player->SendRaidDifficulty(group != nullptr);
            return;
          }
      ...
      if (currMap)
        for (Map::PlayerList::const_iterator itr =
             ...
             itr != currMap->GetPlayers().end(); ++itr)
          if (Player *p = itr->GetSource())
            if (p->GetGroup() != group) {
              if (!p->IsInWorld() || !p->IsAlive() || p->IsInCombat() ||
                  p->GetVictim() || p->m_mover != p ||
                  p->IsNonMeleeSpellCast(true) ||
                  (!p->GetMotionMaster()->empty() &&
                   p->GetMotionMaster()->GetCurrentMovementGeneratorType() !=
                       IDLE_MOTION_TYPE) ||
      ...
      for (std::map<Player *, Position>::iterator itr = playerTeleport.begin();
           itr != playerTeleport.end(); ++itr) {
        Player *p = itr->first;
        Map *oldMap = p->GetMap();
      ...
      for (std::map<Player *, Position>::iterator itr = playerTeleport.begin();
        ...
        if (!itr->first->TeleportTo(
                *(foundMaps.begin()), itr->second.GetPositionX(),
                itr->second.GetPositionY(), itr->second.GetPositionZ(),
    ...
    Player::ResetInstances(_player->GetGUIDLow(),
                           INSTANCE_RESET_CHANGE_DIFFICULTY, true);
