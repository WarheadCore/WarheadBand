...
void WorldSession::HandleCharEnum(PreparedQueryResult result) {
  ...
  if (result) {
    do {
      ...
      LOG_DEBUG("server", "Loading char guid %u from account %u.", guidlow,
                GetAccountId());
#endif
...
void WorldSession::HandleCharCreateOpcode(WorldPacket &recvData) {
  ...
  if (!classEntry) {
    ...
    SendPacket(&data);
    LOG_ERROR("server",
              "Class (%u) not found in DBC while creating new char for account "
              "(ID: %u): wrong DBC files or cheater?",
              class_, GetAccountId());
    return;
  }
  ...
  if (!raceEntry) {
    ...
    SendPacket(&data);
    LOG_ERROR("server",
              "Race (%u) not found in DBC while creating new char for account "
              "(ID: %u): wrong DBC files or cheater?",
              race_, GetAccountId());
    return;
  }
  ...
  delete _charCreateCallback.GetParam(); // Delete existing if any, to make the
                                         // callback chain reset to stage 0
  _charCreateCallback.SetParam(new CharacterCreateInfo(
      name, race_, class_, gender, skin, face, hairStyle, hairColor, facialHair,
      outfitId, recvData));
  PreparedStatement *stmt =
...
void WorldSession::HandleCharCreateCallback(PreparedQueryResult result,
  ...
  /** This is a series of callbacks executed consecutively as a result from the
     database becomes available. This is much more efficient than synchronous
     requests on packet handler, and much less DoS prone. It also prevents data
     syncrhonisation errors.
  */
  ...
  case 0: {
    ...
    PreparedStatement *stmt =
        LoginDatabase.GetPreparedStatement(LOGIN_SEL_SUM_REALM_CHARACTERS);
  ...
  case 2: {
    if (result) {
      ...
      createInfo->CharCount =
          uint8(fields[0].GetUInt64()); // SQL's COUNT() returns uint64 but it
    ...
    if (!allowTwoSideAccounts || skipCinematics == 1 ||
      ...
      PreparedStatement *stmt =
          CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_CREATE_INFO);
      stmt->setUInt32(0, GetAccountId());
      stmt->setUInt32(
          1, (skipCinematics == 1 || createInfo->Class == CLASS_DEATH_KNIGHT)
                 ? 10
                 : 1);
      _charCreateCallback.SetFutureResult(CharacterDatabase.AsyncQuery(stmt));
      _charCreateCallback.NextStage();
    ...
    _charCreateCallback.NextStage();
    HandleCharCreateCallback(PreparedQueryResult(nullptr),
                             createInfo); // Will jump to case 3
  } break;
  case 3: {
    ...
    if (result) {
      ...
      // TODO: check if cinematic already shown? (already logged in?; cinematic
      // field)
    ...
    if (AccountMgr::IsPlayerAccount(GetSecurity()) &&
        createInfo->Class == CLASS_DEATH_KNIGHT && !hasHeroicReqLevel) {
      WorldPacket data(SMSG_CHAR_CREATE, 1);
      data << uint8(CHAR_CREATE_LEVEL_REQUIREMENT);
      SendPacket(&data);
      delete createInfo;
    ...
    if (createInfo->Data.rpos() < createInfo->Data.wpos()) {
      ...
      LOG_DEBUG(
          ...
          createInfo->Name.c_str(), GetAccountId(), unk);
#endif
    }
    ...
    if (!newChar.Create(sObjectMgr->GenerateLowGuid(HIGHGUID_PLAYER),
                        createInfo)) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "Account: %d (IP: %s) Create Character:[%s] (GUID: %u)",
              GetAccountId(), IP_str.c_str(), createInfo->Name.c_str(),
              newChar.GetGUIDLow());
#endif
    ...
    sWorld->AddGlobalPlayerData(newChar.GetGUIDLow(), GetAccountId(),
                                newChar.GetName(), newChar.getGender(),
                                newChar.getRace(), newChar.getClass(),
                                newChar.getLevel(), 0, 0);
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket &recvData) {
  ...
  if (ObjectAccessor::FindPlayerInOrOutOfWorld(guid) ||
      sWorld->FindOfflineSessionForCharacterGUID(GUID_LOPART(guid))) {
  ...
  std::string IP_str = GetRemoteAddress();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Account: %d (IP: %s) Delete Character:[%s] (GUID: %u)",
  ...
  if (sLog->ShouldLog("entities.player.dump",
                      LogLevel::LOG_LEVEL_INFO)) // optimize GetPlayerDump call
  {
    std::string dump;

    if (PlayerDumpWriter().GetDump(GUID_LOPART(guid), dump))
      LOG_CHAR_DUMP(dump.c_str(), GetAccountId(), GUID_LOPART(guid),
                    name.c_str());
  }
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket &recvData) {
  if (PlayerLoading() || GetPlayer() != nullptr) {
    LOG_ERROR("server", "Player tries to login again, AccountId = %d",
              GetAccountId());
  ...
  if (!IsLegitCharacterForAccount(GUID_LOPART(playerGuid))) {
    LOG_ERROR("server", "Account (%u) can't login with that character (%u).",
              GetAccountId(), GUID_LOPART(playerGuid));
  ...
  if (WorldSession *sess =
          sWorld->FindOfflineSessionForCharacterGUID(GUID_LOPART(playerGuid)))
  ...
  if (WorldSession *sess = sWorld->FindOfflineSession(GetAccountId())) {
    ...
    if (p->GetGUID() != playerGuid)
      sess->KickPlayer(
          "No return, go to normal loading"); // no return, go to normal loading
    else {
      // pussywizard: players stay ingame no matter what (prevent abuse), but
      // allow to turn it off to stop crashing
      ...
      uint8
          ...
          limitB = 10,
          limitC =
              10; // pussywizard: this somehow froze (probably, ahh crash logs
...
void WorldSession::HandlePlayerLoginFromDB(LoginQueryHolder *holder) {
  ...
  if (!pCurrChar->LoadFromDB(GUID_LOPART(playerGuid), holder)) {
    SetPlayer(nullptr);
    KickPlayer("HandlePlayerLoginFromDB"); // disconnect client, player no set
                                           // to session and it will not deleted
                                           // or saved at kick
  ...
  if (uint32 guildId = Player::GetGuildIdFromStorage(pCurrChar->GetGUIDLow())) {
    ...
    if (member) {
      ...
      LOG_ERROR("server",
                "Player %s (GUID: %u) marked as member of not existing guild "
                "(id: %u), removing guild membership for player.",
                pCurrChar->GetName().c_str(), pCurrChar->GetGUIDLow(), guildId);
  ...
  // Reputations if "StartAllReputation" is enabled, -- TODO: Fix this in a
  // better way
  ...
  LOG_INFO("entities.player",
           ...
           pCurrChar->GetGUIDLow(), pCurrChar->getLevel());
  ...
  if (pCurrChar->IsInWorld()) {
    if (pCurrChar->GetMountBlockId() &&
      ...
      // Xinef: refresh this in case mount aura changes anything (eg no fly
      // zone)
  ...
  // pussywizard: on login it's not possible to go back to arena as a spectator,
  // HandleMoveWorldportAckOpcode is not sent, so call it here
...
void WorldSession::HandlePlayerLoginToCharInWorld(Player *pCurrChar) {
  ...
  for (uint16 Opcode = SMSG_SET_FLAT_SPELL_MODIFIER;
       Opcode <= SMSG_SET_PCT_SPELL_MODIFIER; ++Opcode) // PCT = FLAT+1
  {
    uint32 modType =
        (Opcode == SMSG_SET_FLAT_SPELL_MODIFIER) ? SPELLMOD_FLAT : SPELLMOD_PCT;
    for (uint32 opType = SPELLMOD_DAMAGE; opType < MAX_SPELLMOD; ++opType) {
      ...
      for (int32 eff = 0; eff < 96; ++eff) {
        ...
        int32 val = 0;
        for (SpellModList::const_iterator itr = spellMods.begin();
             itr != spellMods.end(); ++itr)
...
void WorldSession::HandleSetFactionCheat(WorldPacket & /*recvData*/) {
  LOG_ERROR("server", "WORLD SESSION: HandleSetFactionCheat, not expected "
                      "call, please report.");
...
void WorldSession::HandleCharRenameOpcode(WorldPacket &recvData) {
  ...
  // check name limitations
  if (AccountMgr::IsPlayerAccount(GetSecurity()) &&
      sObjectMgr->IsReservedName(newName)) {
    WorldPacket data(SMSG_CHAR_RENAME, 1);
    data << uint8(CHAR_NAME_RESERVED);
    SendPacket(&data);
    return;
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(
    PreparedQueryResult result, std::string const &newName) {
  if (!result) {
    WorldPacket data(SMSG_CHAR_RENAME, 1);
    data << uint8(CHAR_CREATE_ERROR);
    SendPacket(&data);
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_UPD_NAME);
  ...
  LOG_INFO(
      ...
      GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(), guidLow,
      newName.c_str());
...
void WorldSession::HandleCharCustomize(WorldPacket &recvData) {
  ...
  if (!IsLegitCharacterForAccount(GUID_LOPART(guid))) {
    LOG_ERROR("server",
              "Account %u, IP: %s tried to customise character %u, but it does "
              "not belong to their account!",
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_AT_LOGIN);
  ...
  GlobalPlayerData const *playerData =
      sWorld->GetGlobalPlayerData(GUID_LOPART(guid));
  if (!playerData) {
    WorldPacket data(SMSG_CHAR_CUSTOMIZE, 1);
    data << uint8(CHAR_CREATE_ERROR);
  ...
  LOG_INFO("entities.player",
           ...
           GetAccountId(), GetRemoteAddress().c_str(), playerData->name.c_str(),
  ...
  sWorld->UpdateGlobalPlayerData(
      GUID_LOPART(guid), PLAYER_UPDATE_DATA_NAME | PLAYER_UPDATE_DATA_GENDER,
      newName, 0, gender);
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket &recvData) {
  ...
  if (!IsLegitCharacterForAccount(GUID_LOPART(guid))) {
    LOG_ERROR("server",
              "Account %u, IP: %s tried to factionchange character %u, but it "
  ...
  if (!playerData) // pussywizard: restoring character via www spoils nameData
                   // (it's not restored so it may be null)
  {
    WorldPacket data(SMSG_CHAR_FACTION_CHANGE, 1);
    data << (uint8)CHAR_CREATE_ERROR;
    SendPacket(&data);
  ...
  if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE) {
    ...
    // check auctions, current packet is processed single-threaded way, so not a
    // problem
    ...
    for (uint8 i = 0; i < 2; ++i) // check both neutral and faction-specific AH
    {
      AuctionHouseObject *auctionHouse = sAuctionMgr->GetAuctionsMap(
          i == 0 ? 0
                 : (((1 << (playerData->race - 1)) & RACEMASK_ALLIANCE) ? 12
                                                                        : 29));
      AuctionHouseObject::AuctionEntryMap::const_iterator itr =
          auctionHouse->GetAuctionsBegin();
      AuctionHouseObject::AuctionEntryMap::const_iterator _end =
          auctionHouse->GetAuctionsEnd();
      for (; itr != _end; ++itr) {
  ...
  uint32 used_loginFlag = ((recvData.GetOpcode() == CMSG_CHAR_RACE_CHANGE)
  ...
  LOG_INFO("entities.player",
           "Account: %d (IP: %s), Character [%s] (guid: %u) Changed "
           "Race/Faction to: %s",
           GetAccountId(), GetRemoteAddress().c_str(), playerData->name.c_str(),
           lowGuid, newname.c_str());
  ...
  sWorld->UpdateGlobalPlayerData(GUID_LOPART(guid),
                                 PLAYER_UPDATE_DATA_NAME |
                                     ...
                                     PLAYER_UPDATE_DATA_GENDER,
                                 newname, 0, gender, race);
  ...
  if (oldRace != race) {
    ...
    if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE) {
      ...
      if (team == TEAM_ALLIANCE) {
        ...
        Player::SavePositionInDB(0, -8867.68f, 673.373f, 97.9034f, 0.0f, 1519,
                                 lowGuid);
      } else {
        stmt->setUInt16(1, 1);
        ...
        Player::SavePositionInDB(1, 1633.33f, -4439.11f, 15.7588f, 0.0f, 1637,
                                 lowGuid);
      }
      trans->Append(stmt);
      ...
      for (std::map<uint32, uint32>::const_iterator it =
        ...
        stmt->setUInt16(
            0, uint16(team == TEAM_ALLIANCE ? achiev_alliance : achiev_horde));
        stmt->setUInt32(1, lowGuid);
        trans->Append(stmt);
        ...
        stmt->setUInt16(
            0, uint16(team == TEAM_ALLIANCE ? achiev_alliance : achiev_horde));
        stmt->setUInt16(
            1, uint16(team == TEAM_ALLIANCE ? achiev_horde : achiev_alliance));
      ...
      for (std::map<uint32, uint32>::const_iterator it =
        ...
        stmt->setUInt32(0, lowGuid);
        stmt->setUInt32(1,
                        (team == TEAM_ALLIANCE ? quest_alliance : quest_horde));
        trans->Append(stmt);
        ...
        stmt->setUInt32(0,
                        (team == TEAM_ALLIANCE ? quest_alliance : quest_horde));
        stmt->setUInt32(1,
                        (team == TEAM_ALLIANCE ? quest_horde : quest_alliance));
        stmt->setUInt32(2, lowGuid);
        trans->Append(stmt);
      }
      ...
      {
        ObjectMgr::QuestMap const &questTemplates =
            sObjectMgr->GetQuestTemplates();
        for (ObjectMgr::QuestMap::const_iterator iter = questTemplates.begin();
             iter != questTemplates.end(); ++iter) {
          Quest const *quest = iter->second;
          uint32 newRaceMask =
              (team == TEAM_ALLIANCE) ? RACEMASK_ALLIANCE : RACEMASK_HORDE;
      ...
      for (std::map<uint32, uint32>::const_iterator it =
        ...
        stmt->setUInt32(0, lowGuid);
        stmt->setUInt32(1,
                        (team == TEAM_ALLIANCE ? spell_alliance : spell_horde));
        trans->Append(stmt);
        ...
        stmt->setUInt32(0,
                        (team == TEAM_ALLIANCE ? spell_alliance : spell_horde));
        stmt->setUInt32(1,
                        (team == TEAM_ALLIANCE ? spell_horde : spell_alliance));
        stmt->setUInt32(2, lowGuid);
        trans->Append(stmt);
      }
      ...
      for (std::map<uint32, uint32>::const_iterator it =
        ...
        uint32 newReputation =
            (team == TEAM_ALLIANCE) ? reputation_alliance : reputation_horde;
        uint32 oldReputation =
            (team == TEAM_ALLIANCE) ? reputation_horde : reputation_alliance;
        ...
        int32 newBaseRep = sObjectMgr->GetBaseReputationOf(
            sFactionStore.LookupEntry(newReputation), race, playerClass);
      ...
      if (knownTitlesStr) {
        ...
        // if (tokens.size() != ktcount) // pussywizard: checked at the
        // beginning
        //    return;
