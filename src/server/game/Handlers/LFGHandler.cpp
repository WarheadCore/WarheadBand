...
void WorldSession::HandleLfgJoinOpcode(WorldPacket &recvData) {
  ...
  if (!numDungeons) {
    ...
    LOG_DEBUG("network", "CMSG_LFG_JOIN [" UI64FMTD "] no dungeons selected",
              GetPlayer()->GetGUID());
#endif
    recvData.rfinish();
    return;
...
void WorldSession::HandleLfgLeaveOpcode(WorldPacket & /*recvData*/) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_LFG_LEAVE [" UI64FMTD "] in group: %u", guid,
            group ? 1 : 0);
#endif
  ...
  if (!group || group->GetLeaderGUID() == guid) {
    sLFGMgr->LeaveLfg(
        sLFGMgr->GetState(guid) == lfg::LFG_STATE_RAIDBROWSER ? guid : gguid);
...
void WorldSession::HandleLfgSetRolesOpcode(WorldPacket &recvData) {
  ...
  if (!group) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "CMSG_LFG_SET_ROLES [" UI64FMTD "] Not in group",
              guid);
#endif
    return;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "CMSG_LFG_SET_ROLES: Group [" UI64FMTD "], Player [" UI64FMTD
            "], Roles: %u",
            gguid, guid, roles);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  uint64 guid = GetPlayer()->GetGUID();
  LOG_DEBUG("network", "CMSG_LFG_SET_COMMENT [" UI64FMTD "] comment: %s", guid,
            comment.c_str());
#endif
...
void WorldSession::HandleLfgSetBootVoteOpcode(WorldPacket &recvData) {
  ...
  uint64 guid = GetPlayer()->GetGUID();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_LFG_SET_BOOT_VOTE [" UI64FMTD "] agree: %u", guid,
            agree ? 1 : 0);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_LFG_TELEPORT [" UI64FMTD "] out: %u",
...
void WorldSession::HandleLfgPlayerLockInfoRequestOpcode(
  ...
  uint64 guid = GetPlayer()->GetGUID();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_LFG_PLAYER_LOCK_INFO_REQUEST [" UI64FMTD "]",
            guid);
#endif
...
void WorldSession::SendLfgUpdatePlayer(lfg::LfgUpdateData const &updateData) {
  ...
  if (size) {
    ...
    data << uint8(size);
    for (lfg::LfgDungeonSet::const_iterator it = updateData.dungeons.begin();
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "SMSG_LFG_ROLE_CHOSEN [" UI64FMTD "] guid: [" UI64FMTD
            "] roles: %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "SMSG_LFG_ROLE_CHECK_UPDATE [" UI64FMTD "]",
            GetPlayer()->GetGUID());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "SMSG_LFG_JOIN_RESULT [" UI64FMTD
            "] checkResult: %u checkValue: %u",
...
void WorldSession::SendLfgQueueStatus(
    lfg::LfgQueueStatusData const &queueData) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "SMSG_LFG_QUEUE_STATUS [" UI64FMTD
            "] dungeon: %u - waitTime: %d - avgWaitTime: %d - waitTimeTanks: "
            "%d - waitTimeHealer: %d - waitTimeDps: %d - queuedTime: %u - "
            "tanks: %u - healers: %u - dps: %u",
            GetPlayer()->GetGUID(), queueData.dungeonId, queueData.waitTime,
            queueData.waitTimeAvg, queueData.waitTimeTank,
            queueData.waitTimeHealer, queueData.waitTimeDps,
            queueData.queuedTime, queueData.tanks, queueData.healers,
            queueData.dps);
#endif
...
void WorldSession::SendLfgPlayerReward(
  ...
  LOG_DEBUG("lfg",
            "SMSG_LFG_PLAYER_REWARD %s rdungeonEntry: %u, sdungeonEntry: %u, "
            "done: %u",
            GetPlayerInfo().c_str(), rewardData.rdungeonEntry,
            rewardData.sdungeonEntry, rewardData.done);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "SMSG_LFG_BOOT_PROPOSAL_UPDATE [" UI64FMTD
            "] inProgress: %u - didVote: %u - agree: %u - victim: [" UI64FMTD
            "] votes: %u - agrees: %u - left: %u - needed: %u - reason %s",
            guid, uint8(boot.inProgress),
            uint8(playerVote != lfg::LFG_ANSWER_PENDING),
            uint8(playerVote == lfg::LFG_ANSWER_AGREE), boot.victim, votesNum,
            agreeNum, secsleft, lfg::LFG_GROUP_KICK_VOTES_NEEDED,
            boot.reason.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "SMSG_LFG_PROPOSAL_UPDATE [" UI64FMTD "] state: %u",
            guid, proposal.state);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "SMSG_LFG_LFR_LIST [" UI64FMTD "] update: %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "SMSG_LFG_DISABLED [" UI64FMTD "]",
            GetPlayer()->GetGUID());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "SMSG_LFG_TELEPORT_DENIED [" UI64FMTD "] reason: %u",
