...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket &recvData) {
  ...
  Creature *unit =
      GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TABARDDESIGNER);
  if (!unit) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleTabardVendorActivateOpcode - Unit (GUID: %u) not "
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleBankerActivateOpcode - Unit (GUID: %u) not found "
              "or you can not interact with him.",
              uint32(GUID_LOPART(guid)));
#endif
    return;
  }
...
void WorldSession::SendTrainerList(uint64 guid, const std::string &strTitle) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: SendTrainerList - Unit (GUID: %u) not found or you can "
  ...
  if (!ci) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "WORLD: SendTrainerList - (GUID: %u) NO CREATUREINFO!",
              GUID_LOPART(guid));
#endif
    return;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: SendTrainerList - Training spells not found for creature "
              "(GUID: %u Entry: %u)",
  ...
  for (TrainerSpellMap::const_iterator itr = trainer_spells->spellList.begin();
    ...
    data << uint8(state == TRAINER_SPELL_GREEN_DISABLED ? TRAINER_SPELL_GREEN
                                                        : state);
    ...
    data << uint32(
        primary_prof_first_rank ? 1 : 0); // must be equal prev. field to have
                                          // learn button in enabled state
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      ...
      for (SpellsRequiringSpellMap::const_iterator itr2 = spellsRequired.first;
           itr2 != spellsRequired.second && maxReq < 3; ++itr2) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WORLD: Received CMSG_TRAINER_BUY_SPELL NpcGUID=%u, learn spell id "
            "is: %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleTrainerBuySpellOpcode - Unit (GUID: %u) not found "
              "or you can not interact with him.",
              uint32(GUID_LOPART(guid)));
#endif
    return;
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleGossipHelloOpcode - Unit (GUID: %u) not found or "
  ...
  if (unit->GetUInt32Value(UNIT_NPC_FLAGS) == UNIT_NPC_FLAG_GOSSIP &&
      unit->IsInCombat()) // should work on all flags?
    return;
  ...
  // xinef: and if he has pure gossip or is banker and moves or is tabard
  // designer?
  // if (unit->IsArmorer() || unit->IsCivilian() || unit->IsQuestGiver() ||
  // unit->IsServiceProvider() || unit->IsGuard())
  {
    // if (!unit->GetTransport()) // pussywizard: reverted with new spline (old:
    // without this check, npc would stay in place and the transport would
    // continue moving, so the npc falls off. NPCs on transports don't have
    // waypoints, so stopmoving is not needed)
    unit->StopMoving();
  }
...
Unit (GUID: %u) not found or you can't interact with him.",
uint32(GUID_LOPART(guid))); #endif return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleSpiritHealerActivateOpcode - Unit (GUID: %u) not "
              "found or you can not interact with him.",
              uint32(GUID_LOPART(guid)));
#endif
    return;
...
void WorldSession::SendSpiritResurrect() {
  ...
  if (corpse)
    corpseGrave = sGraveyard->GetClosestGraveyard(
        ...
        corpse->GetMapId(), _player->GetTeamId());
  ...
  // player ghost
  if (corpseGrave) {
    GraveyardStruct const *ghostGrave = sGraveyard->GetClosestGraveyard(
        _player->GetPositionX(), _player->GetPositionY(),
        _player->GetPositionZ(), _player->GetMapId(), _player->GetTeamId());

    if (corpseGrave != ghostGrave)
      _player->TeleportTo(corpseGrave->Map, corpseGrave->x, corpseGrave->y,
                          corpseGrave->z, _player->GetOrientation());
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleBinderActivateOpcode - Unit (GUID: %u) not found "
              "or you can not interact with him.",
...
void WorldSession::SendStablePet(uint64 guid) {
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_PET_SLOTS_DETAIL);
...
void WorldSession::SendStablePetCallback(PreparedQueryResult result,
  ...
  if (pet && pet->IsAlive() && pet->getPetType() == HUNTER_PET) {
    ...
    data << uint8(1); // 1 = current, 2/3 = in stable (any from 4, 5, ... create
                      // problems with proper show)
    ++num;
  } else if (_player->IsPetDismissed() ||
             _player->GetTemporaryUnsummonedPetNumber()) {
    PreparedStatement *stmt = CharacterDatabase.GetPreparedStatement(
        CHAR_SEL_CHAR_PET_BY_ENTRY_AND_SLOT);
    stmt->setUInt32(0, _player->GetGUIDLow());
    stmt->setUInt8(1, uint8(_player->GetTemporaryUnsummonedPetNumber()
                                ? PET_SAVE_AS_CURRENT
  ...
  if (result) {
    do {
      ...
      data << uint8(2); // 1 = current, 2/3 = in stable (any from 4, 5, ...
                        // create problems with proper show)
...
void WorldSession::HandleStablePet(WorldPacket &recvData) {
  ...
  if (pet) {
    ...
    SpellCastResult loadResult = Pet::TryLoadFromDB(
        _player, _player->GetTemporaryUnsummonedPetNumber() != 0, HUNTER_PET);
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_PET_SLOTS);
...
void WorldSession::HandleStablePetCallback(PreparedQueryResult result) {
  ...
  if (freeSlot > 0 && freeSlot <= GetPlayer()->m_stableSlots) {
    ...
    PreparedStatement *stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UDP_CHAR_PET_SLOT_BY_SLOT);
    stmt->setUInt8(0, freeSlot);
    stmt->setUInt32(1, _player->GetGUIDLow());
    stmt->setUInt8(2, uint8(_player->GetTemporaryUnsummonedPetNumber()
                                ? PET_SAVE_AS_CURRENT
                                : PET_SAVE_NOT_IN_SLOT));
    trans->Append(stmt);
...
void WorldSession::HandleUnstablePet(WorldPacket &recvData) {
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_PET_ENTRY);
...
void WorldSession::HandleUnstablePetCallback(PreparedQueryResult result,
  ...
  if (!Pet::LoadPetFromDB(_player, PET_LOAD_HANDLE_UNSTABLE_CALLBACK, petEntry,
                          petId)) {
    SendStableResult(STABLE_ERR_STABLE);
    return;
  }
...
void WorldSession::HandleStableSwapPet(WorldPacket &recvData) {
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_PET_SLOT_BY_ID);
...
void WorldSession::HandleStableSwapPetCallback(PreparedQueryResult result,
  ...
  if (pet)
    _player->RemovePet(pet, pet->IsAlive() ? PetSaveMode(slot)
                                           : PET_SAVE_AS_DELETED);
  else if (_player->IsPetDismissed() ||
           _player->GetTemporaryUnsummonedPetNumber()) {
    // change pet slot directly in database
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandleRepairItemOpcode - Unit (GUID: %u) not found or "
              "you can not interact with him.",
              uint32(GUID_LOPART(npcGUID)));
#endif
    return;
  ...
  if (itemGUID) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "ITEM: Repair item, itemGUID = %u, npcGUID = %u",
              GUID_LOPART(itemGUID), GUID_LOPART(npcGUID));
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "ITEM: Repair all items, npcGUID = %u",
              GUID_LOPART(npcGUID));
#endif
