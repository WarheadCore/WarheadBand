...
bool WorldSession::CanOpenMailBox(uint64 guid) {
  if (guid == _player->GetGUID()) {
    if (_player->GetSession()->GetSecurity() < SEC_MODERATOR) {
      LOG_ERROR("server", "%s attempt open mailbox in cheating way.",
...
void WorldSession::HandleSendMail(WorldPacket &recvData) {
  ...
  if (!rc) {
    ...
    LOG_DEBUG("server",
              "Player %u is sending mail to %s (GUID: not existed!) with "
              "subject %s and body %s includes %u items, %u copper and %u COD "
              "copper with unk1 = %u, unk2 = %u",
              player->GetGUIDLow(), receiver.c_str(), subject.c_str(),
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server",
            "Player %u is sending mail to %s (GUID: %u) with subject %s and "
            "body %s includes %u items, %u copper and %u COD copper with unk1 "
            "= %u, unk2 = %u",
            player->GetGUIDLow(), receiver.c_str(), GUID_LOPART(rc),
  ...
  if (receive) {
    ...
    if (GlobalPlayerData const *playerData =
            sWorld->GetGlobalPlayerData(GUID_LOPART(rc))) {
  ...
  uint32 rc_account = receive ? receive->GetSession()->GetAccountId()
  ...
  if (items_count > 0 || money > 0) {
    if (items_count > 0) {
      for (uint8 i = 0; i < items_count; ++i) {
        ...
        if (item->GetState() == ITEM_UNCHANGED)
          item->FSetState(
              ITEM_CHANGED); // pussywizard: so the item will be saved and owner
    ...
    if (money >= 10 * GOLD) {
      ...
      CharacterDatabase.PExecute(
          ...
          "%u, \"<MAIL> %s\", NOW())",
          GetAccountId(), player->GetGUIDLow(), player->GetName().c_str(),
          player->GetSession()->GetRemoteAddress().c_str(), rc_account,
          receiver.c_str(), money, subject.c_str());
    }
  }
  ...
  draft.AddMoney(money).AddCOD(COD).SendMailTo(
      trans, MailReceiver(receive, GUID_LOPART(rc)), MailSender(player),
  ...
  if (msg == EQUIP_ERR_OK) {
                    ...
                    // to sender by mail
    {
      ...
      if (sender || sender_accId) {
        MailDraft(m->subject, "")
            .AddMoney(m->COD)
            .SendMailTo(trans, MailReceiver(sender, m->sender),
        ...
        if (m->COD >= 10 * GOLD) {
          ...
          CharacterDatabase.PExecute(
              "INSERT INTO log_money VALUES(%u, %u, \"%s\", \"%s\", %u, "
              "\"%s\", %u, \"<COD> %s\", NOW())",
              GetAccountId(), player->GetGUIDLow(), player->GetName().c_str(),
              player->GetSession()->GetRemoteAddress().c_str(), sender_accId,
              senderName.c_str(), m->COD, subj.c_str());
        }
      }
...
void WorldSession::HandleGetMailList(WorldPacket &recvData) {
  ...
  uint32
      mailsCount = 0; // real send to client mails amount // real mails amount
  ...
  for (PlayerMails::iterator itr = player->GetMailBegin();
    ...
    uint8 item_count =
        uint8((*itr)->items.size()); // max count is MAX_MAIL_ITEMS (12)
    ...
    for (uint8 i = 0; i < item_count; ++i) {
      ...
      for (uint8 j = 0; j < MAX_INSPECTED_ENCHANTMENT_SLOT; ++j) {
        ...
        data << uint32(item ? item->GetEnchantmentDuration(EnchantmentSlot(j))
                            : 0);
        data << uint32(item ? item->GetEnchantmentCharges(EnchantmentSlot(j))
                            : 0);
      }
      // can be negative
  ...
  data.put<uint32>(
      0, player->totalMailCount); // this will display warning about undelivered
...
void WorldSession::HandleMailCreateTextItem(WorldPacket &recvData) {
  ...
  if (!m || (m->body.empty() && !m->mailTemplateId) ||
      m->state == MAIL_STATE_DELETED || m->deliver_time > time(nullptr) ||
  ...
  if (!bodyItem->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM),
...
void WorldSession::HandleQueryNextMailTime(WorldPacket & /*recvData*/) {
  ...
  if (_player->unReadMails > 0) {
    ...
    for (PlayerMails::iterator itr = _player->GetMailBegin();
      ...
      data << uint64(m->messageType == MAIL_NORMAL ? m->sender
                                                   : 0); // player guid
