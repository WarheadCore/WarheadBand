...
void WorldSession::HandleMoveWorldportAckOpcode() {
  ...
  // get the destination map entry, not the current one, this will fix homebind
  // and reset greeting
  ...
  if (GetPlayer()->IsInWorld()) {
    LOG_ERROR("server",
              "Player (Name %s) is still in world when teleported from map %u "
              "to new map %u",
  ...
  if (GetPlayer()->m_InstanceValid == false && !mInstance) {
    ...
    // pussywizard: m_InstanceValid can be false only by leaving a group in an
    // instance => so remove temp binds that could not be removed because player
    // was still on the map!
    if (!sInstanceSaveMgr->PlayerIsPermBoundToInstance(
            GetPlayer()->GetGUIDLow(), oldMap->GetId(),
            oldMap->GetDifficulty()))
      sInstanceSaveMgr->PlayerUnbindInstance(GetPlayer()->GetGUIDLow(),
                                             oldMap->GetId(),
                                             oldMap->GetDifficulty(), true);
  }
  ...
  if (!newMap || !newMap->CanEnter(GetPlayer(), false)) {
    LOG_ERROR(
        "server",
        "Map %d could not be created for player %d, porting player to homebind",
        loc.GetMapId(), GetPlayer()->GetGUIDLow());
    GetPlayer()->TeleportTo(GetPlayer()->m_homebindMapId,
                            GetPlayer()->m_homebindX, GetPlayer()->m_homebindY,
  ...
  GetPlayer()->Relocate(loc.GetPositionX(), loc.GetPositionY(), z,
                        loc.GetOrientation());
  ...
  if (!GetPlayer()->GetMap()->AddPlayerToMap(GetPlayer())) {
    LOG_ERROR("server",
              "WORLD: failed to teleport player %s (%d) to map %d because of "
              "unknown reason!",
  ...
  // pussywizard: transport teleport couldn't teleport us to the same map (some
  // other teleport pending, reqs not met, etc.), but we still have transport
  // set until player moves! clear it if map differs (crashfix)
  ...
  if (!_player->getHostileRefManager().isEmpty())
    _player->getHostileRefManager()
        .deleteReferences(); // pussywizard: multithreading crashfix

  CellCoord pair(Warhead::ComputeCellCoord(GetPlayer()->GetPositionX(),
  ...
  if (_player->InBattleground()) {
    ...
    // everything ok
      ...
      if (_player->IsInvitedForBattlegroundInstance()) // GMs are not invited,
  ...
  // pussywizard: arena spectator stuff
  {
    ...
    timeWhoCommandAllowed =
        time(nullptr) + sWorld->GetNextWhoListUpdateDelaySecs() +
        1; // after exiting arena Subscribe will scan for a player and cached
           // data says he is still in arena, so disallow until next update

    if (uint32 inviteInstanceId =
            _player->GetPendingSpectatorInviteInstanceId()) {
      if (Battleground *tbg =
              sBattlegroundMgr->GetBattleground(inviteInstanceId))
  ...
  // bg->AddPlayer(_player)!!!!
  CellCoord pair2(Warhead::ComputeCellCoord(GetPlayer()->GetPositionX(),
                                            GetPlayer()->GetPositionY()));
  Cell cell2(pair2);
  ...
  // resurrect character at enter into instance where his corpse exist after add
  // to map
  ...
  if (mInstance) {
    ...
    if (MapDifficulty const *mapDiff =
      ...
      if (mapDiff->resetTime)
        if (time_t timeReset =
                sInstanceSaveMgr->GetResetTimeFor(mEntry->MapID, diff)) {
  ...
  // in friendly area
           ...
           !GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_IN_PVP))
...
void WorldSession::HandleMoveTeleportAck(WorldPacket &recvData) {
  ...
  if (Pet *pet = plMover->GetPet()) {
    if (!pet->IsWithinDist3d(plMover,
                             plMover->GetMap()->GetVisibilityRange() - 5.0f))
      pet->NearTeleportTo(plMover->GetPositionX(), plMover->GetPositionY(),
                          plMover->GetPositionZ(), pet->GetOrientation());
  }
  ...
  if (oldPos.GetExactDist2d(plMover) > 100.0f) {
    ...
    if (old_zone != newzone) {
      ...
      // in friendly area
               ...
               !plMover->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_IN_PVP))
  ...
  if (plMover->HasUnitState(UNIT_STATE_FLEEING | UNIT_STATE_CONFUSED) ||
      plMover->IsCharmed()) // only in such cases SetClientControl(self, false)
                            // is sent
...
void WorldSession::HandleMovementOpcodes(WorldPacket &recvData) {
  ...
  if (movementInfo.flags & MOVEMENTFLAG_ONTRANSPORT) {
    ...
    // (also received at zeppelin leave by some reason with t_* as absolute in
    // continent coordinates, can be safely skipped)
    if (movementInfo.transport.pos.GetPositionX() > 75.0f ||
        movementInfo.transport.pos.GetPositionY() > 75.0f ||
        movementInfo.transport.pos.GetPositionZ() > 75.0f ||
        movementInfo.transport.pos.GetPositionX() < -75.0f ||
        movementInfo.transport.pos.GetPositionY() < -75.0f ||
        movementInfo.transport.pos.GetPositionZ() < -75.0f) {
      recvData.rfinish(); // prevent warnings spam
      return;
    ...
    if (plrMover) {
      if (!plrMover->GetTransport()) {
        if (Transport *transport =
                plrMover->GetMap()->GetTransport(movementInfo.transport.guid)) {
  ...
  if (plrMover && ((movementInfo.flags & MOVEMENTFLAG_SWIMMING) != 0) !=
    ...
    plrMover->SetInWater(
        !plrMover->IsInWater() ||
        plrMover->GetBaseMap()->IsUnderWater(movementInfo.pos.GetPositionX(),
                                             movementInfo.pos.GetPositionY(),
                                             movementInfo.pos.GetPositionZ()));
  }
  ...
  if (m_clientTimeDelay == 0)
    m_clientTimeDelay = mstime > movementInfo.time
                            ? std::min(mstime - movementInfo.time, (uint32)100)
                            : 0;
  ...
  if (mover->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE)) {
    ...
    if (movementInfo.HasMovementFlag(MOVEMENTFLAG_MASK_MOVING))
      return;
    movementInfo.pos.Relocate(mover->GetPositionX(), mover->GetPositionY(),
                              mover->GetPositionZ());
  ...
  movementInfo.time =
      mstime; // pussywizard: set to time of relocation (server time), constant
              // addition may smoothen movement clientside, but client sees
              // target on different position than the real serverside position
  ...
  // this is almost never true (pussywizard: only one packet when entering
  // vehicle), normally use mover->IsVehicle()
  if (mover->GetVehicle()) {
  ...
  // fall damage generation (ignore in flight case that can be triggered also at
  // lags in moment teleportation to another map). Xinef: moved it here,
  // previously StopMoving function called when player died relocated him to
  // last saved coordinates (which were in air)
  if (opcode == MSG_MOVE_FALL_LAND && plrMover && !plrMover->IsInFlight() &&
      (!plrMover->GetTransport() ||
       plrMover->GetTransport()->IsStaticTransport()))
    ...
    if (movementInfo.pos.GetPositionZ() <
      ...
      if (!plrMover->GetBattleground() ||
        ...
        if (plrMover->IsAlive()) {
          ...
          if (plrMover->IsAlive())
            plrMover->KillPlayer();
        } else if (!plrMover->HasFlag(PLAYER_FLAGS,
                                      PLAYER_FLAGS_IS_OUT_OF_BOUNDS)) {
          GraveyardStruct const *grave = sGraveyard->GetClosestGraveyard(
              plrMover->GetPositionX(), plrMover->GetPositionY(),
              plrMover->GetPositionZ(), plrMover->GetMapId(),
              plrMover->GetTeamId());
          if (grave) {
            plrMover->TeleportTo(grave->Map, grave->x, grave->y, grave->z,
                                 plrMover->GetOrientation());
            plrMover->Relocate(grave->x, grave->y, grave->z,
                               plrMover->GetOrientation());
          }
        }

        plrMover->StopMovingOnCurrentPos(); // pussywizard: moving corpse can't
                                            // release spirit
      }
  }
}
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Recvd %s (%u, 0x%X) opcode",
  ...
  static char const *move_type_name[MAX_MOVE_TYPE] = {
      "Walk",     "Run",    "RunBack",    "Swim",     "SwimBack",
      "TurnRate", "Flight", "FlightBack", "PitchRate"};
  ...
  if (!_player->GetTransport() &&
        ...
        newspeed) // must be greater - just correct
    {
      LOG_ERROR("server",
                ...
                move_type_name[move_type], _player->GetName().c_str(),
    ...
    } else // must be lesser - cheating
    {
      LOG_INFO("server",
               ...
               "be %f instead %f)",
               _player->GetName().c_str(), GetAccountId(),
               _player->GetSpeed(move_type), newspeed);
...
void WorldSession::HandleSetActiveMoverOpcode(WorldPacket &recvData) {
  ...
  if (GetPlayer()->IsInWorld() && _player->m_mover &&
    ...
    if (_player->m_mover->GetGUID() != guid)
      LOG_ERROR(
          "server",
          "HandleSetActiveMoverOpcode: incorrect mover guid: mover is " UI64FMTD
          " (%s - Entry: %u) and should be " UI64FMTD,
          guid, GetLogNameForGuid(guid), GUID_ENPART(guid),
...
void WorldSession::HandleMoveTimeSkippedOpcode(WorldPacket &recvData) {
  ...
  if (!mover) {
    LOG_ERROR("server",
              ...
              "from the unit moved by the player [" UI64FMTD "]",
              GetPlayer()->GetGUID());
    return;
  }
  ...
  if (guid != mover->GetGUID()) {
    LOG_ERROR("server",
              "WorldSession::HandleMoveTimeSkippedOpcode wrong guid from the "
              "unit moved by the player [" UI64FMTD "]",
              GetPlayer()->GetGUID());
    return;
  }
