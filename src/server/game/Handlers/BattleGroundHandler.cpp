...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket &recvData) {
  uint64 guid;
  uint32 bgTypeId_;
  uint32 instanceId; // sent to queue for particular bg from battlemaster's
  ...
  if (qSlot < PLAYER_MAX_BATTLEGROUND_QUEUES) {
    ...
    if (bgQueue.IsPlayerInvitedToRatedArena(_player->GetGUID())) {
      ...
      sBattlegroundMgr->BuildGroupJoinedBattlegroundPacket(
          &data, ERR_BATTLEGROUND_JOIN_FAILED);
      SendPacket(&data);
      return;
    }
  ...
  if (!sScriptMgr->CanJoinInBattlegroundQueue(_player, guid, bgTypeId,
                                              joinAsGroup, err) &&
      err <= 0) {
    WorldPacket data;
  ...
  if (!joinAsGroup) {
    if (GetPlayer()->InBattleground()) // currently in battleground
      ...
      err = ERR_GROUP_JOIN_BATTLEGROUND_DESERTERS;
                 ...
                 bgQueueTypeIdRandom)) // queued for random bg, so can't queue
                                       // for anything else
      err = ERR_IN_RANDOM_BG;
    else if (_player->InBattlegroundQueue() &&
             bgTypeId ==
                 BATTLEGROUND_RB) // already in queue, so can't queue for random
    ...
    // teleported yet
    else if (_player->getClass() == CLASS_DEATH_KNIGHT &&
    ...
    GroupQueueInfo *ginfo =
        bgQueue.AddGroup(_player, nullptr, bracketEntry, false, false, 0, 0, 0);
    ...
    sBattlegroundMgr->BuildBattlegroundStatusPacket(
        &data, bgt, queueSlot, STATUS_WAIT_QUEUE, avgWaitTime, 0, 0,
        TEAM_NEUTRAL);
    SendPacket(&data);
  ...
  // check if group can queue:
    ...
    // pussywizard: for party members - remove queues for which leader is not
    // queued to!
    ...
    for (GroupReference *itr = grp->GetFirstMember(); itr != nullptr;
      ...
      if (Player *member = itr->GetSource())
        for (uint32 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
          if (BattlegroundQueueTypeId mqtid =
                  member->GetBattlegroundQueueTypeId(i))
            if (leaderQueueTypeIds.count((uint32)mqtid) == 0) {
              BattlegroundQueue &bgQueue =
                  sBattlegroundMgr->GetBattlegroundQueue(mqtid);
    ...
    if (_player->InBattlegroundQueueForBattlegroundQueueType(
            bgQueueTypeIdRandom)) // queued for random bg, so can't queue for
                                  // anything else
      err = ERR_IN_RANDOM_BG;
    else if (_player->InBattlegroundQueue() &&
             bgTypeId ==
    ...
    if (err > 0)
      err = grp->CanJoinBattlegroundQueue(
          bgt, bgQueueTypeId, 0, bgt->GetMaxPlayersPerTeam(), false, 0);

    bool isPremade = (grp->GetMembersCount() >= bgt->GetMinPlayersPerTeam() &&
...
void WorldSession::HandleBattlefieldListOpcode(WorldPacket &recvData) {
  ...
  uint8 fromWhere;
  recvData >> fromWhere; // 0 - battlemaster (lua: ShowBattlefieldList), 1 - UI
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData) {
  ...
  Battleground *bg =
      sBattlegroundMgr->GetBattleground(ginfo.IsInvitedToBGInstanceGUID);
  ...
  case 1: // accept
  {
    ...
    for (uint32 qslot = 0; qslot < PLAYER_MAX_BATTLEGROUND_QUEUES; ++qslot)
      if (BattlegroundQueueTypeId q =
              _player->GetBattlegroundQueueTypeId(qslot)) {
    ...
    _player->SetBattlegroundId(bg->GetInstanceID(), bg->GetBgTypeID(),
                               queueSlot, true, bgTypeId == BATTLEGROUND_RB,
                               teamId);
...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket & /*recvData*/) {
  ...
  if (Battleground *bg = _player->GetBattleground())
    if (bg->GetPlayers().count(_player->GetGUID())) {
      sBattlegroundMgr->BuildBattlegroundStatusPacket(
          &data, bg, _player->GetCurrentBattlegroundQueueSlot(),
          STATUS_IN_PROGRESS, bg->GetEndTime(), bg->GetStartTime(),
          bg->GetArenaType(), _player->GetBgTeamId());
      SendPacket(&data);
    }
  ...
  for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i) {
    ...
    if (ginfo.IsInvitedToBGInstanceGUID) {
      ...
      uint32 remainingTime =
          (World::GetGameTimeMS() < ginfo.RemoveInviteTime
               ? getMSTimeDiff(World::GetGameTimeMS(), ginfo.RemoveInviteTime)
               : 1);
      sBattlegroundMgr->BuildBattlegroundStatusPacket(
          &data, bg, i, STATUS_WAIT_JOIN, remainingTime, 0, ginfo.ArenaType,
          TEAM_NEUTRAL, bg->isRated(), ginfo.BgTypeId);
      SendPacket(&data);
    }
    // if not invited - send STATUS_WAIT_QUEUE
      ...
      uint32 avgWaitTime = bgQueue.GetAverageQueueWaitTime(&ginfo);
      sBattlegroundMgr->BuildBattlegroundStatusPacket(
          &data, bgt, i, STATUS_WAIT_QUEUE, avgWaitTime,
          getMSTimeDiff(ginfo.JoinTime, World::GetGameTimeMS()),
          ginfo.ArenaType, TEAM_NEUTRAL, ginfo.IsRated);
      SendPacket(&data);
    }
  }
}
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket &recvData) {
  ...
  // arenas disabled
  if (DisableMgr::IsDisabledFor(DISABLE_TYPE_BATTLEGROUND, BATTLEGROUND_AA,
                                nullptr)) {
  ...
  // must have free queue slot
  // pussywizard: allow being queued only in one arena queue, and it even cannot
  // be together with bg queues
  if (_player->InBattlegroundQueue()) {
    WorldPacket data;
    sBattlegroundMgr->BuildGroupJoinedBattlegroundPacket(
  ...
  if (!asGroup) {
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(
        &data, bgt, queueSlot, STATUS_WAIT_QUEUE, avgWaitTime, 0, arenatype,
        TEAM_NEUTRAL);
    SendPacket(&data);
  ...
  // check if group can queue:
    ...
    err = grp->CanJoinBattlegroundQueue(bgt, bgQueueTypeId, arenatype,
                                        arenatype, (bool)isRated, arenaslot);
    ...
    if (err > 0) {
      BattlegroundQueue &bgQueue =
          sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId);
      GroupQueueInfo *ginfo =
          bgQueue.AddGroup(_player, grp, bracketEntry, isRated, false,
                           arenaRating, matchmakerRating, ateamId);
      avgWaitTime = bgQueue.GetAverageQueueWaitTime(ginfo);
    }
    ...
    if (ateamId)
      sBattlegroundMgr->ScheduleArenaQueueUpdate(ateamId, bgQueueTypeId,
