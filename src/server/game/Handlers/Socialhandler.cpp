...
void WorldSession::HandleAddFriendOpcode(WorldPacket &recv_data) {
  ...
  LOG_DEBUG("network", "WORLD: %s asked to add friend : '%s'",
  ...
  if (!AccountMgr::IsPlayerAccount(GetSecurity()) ||
      ...
      AccountMgr::IsPlayerAccount(
          AccountMgr::GetSecurity(friendAccountId, realmID))) {
    if (friendGuid) {
      if (friendGuid == GetPlayer()->GetGUID())
        ...
        if (GetPlayer()->GetSocial()->AddToSocialList(friendGuid,
                                                      SOCIAL_FLAG_FRIEND))
...
void WorldSession::HandleDelFriendOpcode(WorldPacket &recv_data) {
  ...
  _player->GetSocial()->RemoveFromSocialList(GUID_LOPART(FriendGUID),
                                             SOCIAL_FLAG_FRIEND);
...
void WorldSession::HandleAddIgnoreOpcode(WorldPacket &recv_data) {
  ...
  LOG_DEBUG("network", "WORLD: %s asked to Ignore: '%s'",
  ...
  if (IgnoreGuid == GetPlayer()->GetGUID()) // not add yourself
    ...
    // ignore list full
    if (!GetPlayer()->GetSocial()->AddToSocialList(lowGuid,
                                                   SOCIAL_FLAG_IGNORED))
...
void WorldSession::HandleDelIgnoreOpcode(WorldPacket &recv_data) {
  ...
  _player->GetSocial()->RemoveFromSocialList(GUID_LOPART(IgnoreGUID),
                                             SOCIAL_FLAG_IGNORED);
