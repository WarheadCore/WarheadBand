...
public:
  LoadPetFromDBQueryHolder(uint32 petNumber, bool current, uint32 diffTime,
                           std::string actionBar, uint32 health, uint32 mana)
      : m_petNumber(petNumber), m_current(current), m_diffTime(diffTime),
        m_actionBar(actionBar), m_savedHealth(health), m_savedMana(mana) {}
...
uint8 WorldSession::HandleLoadPetFromDBFirstCallback(PreparedQueryResult result,
  ...
  SpellInfo const *spellInfo =
      sSpellMgr->GetSpellInfo(summon_spell_id); // CANT BE nullptr
  ...
  if (pet_type == HUNTER_PET && savedhealth == 0 &&
  ...
  LoadPetFromDBQueryHolder *holder = new LoadPetFromDBQueryHolder(
      pet_number, current, uint32(time(nullptr) - fields[14].GetUInt32()),
      fields[13].GetString(), savedhealth, savedmana);
  ...
  float px, py, pz;
  owner->GetClosePoint(px, py, pz, pet->GetObjectSize(), PET_FOLLOW_DIST,
                       pet->GetFollowAngle());
  if (!pet->IsPositionValid()) {
    LOG_ERROR("server",
              "Pet (guidlow %d, entry %d) not loaded. Suggested coordinates "
              "isn't valid (X: %f Y: %f)",
  ...
  if (pet->getPetType() == HUNTER_PET ||
      pet->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON ||
      pet->GetCreatureTemplate()->type == CREATURE_TYPE_UNDEAD)
    pet->GetCharmInfo()->SetPetNumber(
        pet_number,
        pet->IsPermanentPetFor(owner)); // Show pet details tab (Shift+P) only
  ...
  case HUNTER_PET:
    ...
    pet->SetByteFlag(UNIT_FIELD_BYTES_2, 2,
                     fields[9].GetBool()
  ...
  default:
    if (!pet->IsPetGhoul())
      LOG_ERROR("server", "Pet have incorrect type (%u) for pet loading.",
                pet->getPetType());
    break;
  }

  pet->SetUInt32Value(UNIT_FIELD_PET_NAME_TIMESTAMP,
                      uint32(time(nullptr))); // cast can't be helped here
  ...
  // application for spells Example: 46584 - without this cooldown (which should
  // be set always when pet is loaded) isn't set clientside
  ...
  if (pet->getPetType() == SUMMON_PET &&
      !current) // all (?) summon pets come with full health when called, but
                // not when they are current
    pet->SetPower(POWER_MANA, pet->GetMaxPower(POWER_MANA));
  else {
    pet->SetHealth(savedhealth > pet->GetMaxHealth() ? pet->GetMaxHealth()
                                                     : savedhealth);
    pet->SetPower(POWER_MANA, savedmana > pet->GetMaxPower(POWER_MANA)
                                  ? pet->GetMaxPower(POWER_MANA)
                                  : savedmana);
  }
...
void WorldSession::HandleLoadPetFromDBSecondCallback(
    LoadPetFromDBQueryHolder *holder) {
  if (!GetPlayer())
    return;
  ...
  pet->_LoadAuras(holder->GetPreparedResult(PET_LOAD_QUERY_LOADAURAS),
                  holder->GetDiffTime());
  ...
  if (!is_temporary_summoned) {
    ...
    pet->_LoadSpellCooldowns(
        holder->GetPreparedResult(PET_LOAD_QUERY_LOADSPELLCOOLDOWN));
    pet->LearnPetPassives();
    pet->InitLevelupSpellsForLevel();
    pet->CastPetAuras(current);

    pet->GetCharmInfo()->LoadPetActionBar(
        holder->GetActionBar()); // action bar stored in already read string
  }
  ...
  if (owner->GetTypeId() == TYPEID_PLAYER && pet->isControlled() &&
      !pet->isTemporarySummoned() &&
      (pet->getPetType() == SUMMON_PET || pet->getPetType() == HUNTER_PET)) {
                ...
                // not when they are current
  {
    ...
    if (!holder->GetSavedHealth() && pet->getPetType() == HUNTER_PET &&
        pet->GetAsynchLoadType() != PET_LOAD_SUMMON_DEAD_PET)
      pet->setDeathState(JUST_DIED);
    else {
      pet->SetHealth(holder->GetSavedHealth() > pet->GetMaxHealth()
                         ? pet->GetMaxHealth()
                         : holder->GetSavedHealth());
      pet->SetPower(POWER_MANA,
                    holder->GetSavedMana() > pet->GetMaxPower(POWER_MANA)
                        ? pet->GetMaxPower(POWER_MANA)
                        : holder->GetSavedMana());
    }
  }
  ...
  owner->SetTemporaryUnsummonedPetNumber(
      0); // clear this only if pet is loaded successfuly
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "Vanitypet (guid: %u) does not exist - player '%s' (guid: %u / "
              "account: %u) attempted to dismiss it (possibly lagged out)",
...
void WorldSession::HandlePetAction(WorldPacket &recvData) {
  ...
  if (!pet) {
    LOG_ERROR("server",
              "HandlePetAction: Pet (GUID: %u) doesn't exist for player '%s'",
  ...
  if (pet != GetPlayer()->GetFirstControlled()) {
    LOG_ERROR("server",
              "HandlePetAction: Pet (GUID: %u) does not belong to player '%s'",
              uint32(GUID_LOPART(guid1)), GetPlayer()->GetName().c_str());
    return;
  }
  ...
  if (!pet->IsAlive()) {
    ...
    if ((flag != ACT_COMMAND || spellid != COMMAND_ABANDON) &&
        (!spell || !spell->HasAttribute(SPELL_ATTR0_CASTABLE_WHILE_DEAD)))
      return;
  }
  ...
  if (GetPlayer()->m_Controlled.size() == 1)
    ...
    for (Unit::ControlSet::iterator itr = GetPlayer()->m_Controlled.begin();
      ...
      else if ((*itr)->GetEntry() == NPC_MIRROR_IMAGE && flag == ACT_COMMAND &&
               spellid == COMMAND_FOLLOW) {
...
void WorldSession::HandlePetStopAttack(WorldPacket &recvData) {
  ...
  if (!pet) {
    LOG_ERROR("server", "HandlePetStopAttack: Pet %u does not exist",
  ...
  if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharm()) {
    LOG_ERROR("server",
              "HandlePetStopAttack: Pet GUID %u isn't a pet or charmed "
              "creature of player %s",
              uint32(GUID_LOPART(guid)), GetPlayer()->GetName().c_str());
    return;
  }
...
void WorldSession::HandlePetActionHelper(Unit *pet, uint64 guid1,
  ...
  if (!charmInfo) {
    LOG_ERROR("server",
              ...
              ", tagGuid: " UI64FMTD
              ", spellId: %u, flag: %u): object (entry: %u TypeId: %u) is "
  ...
  case ACT_COMMAND: // 0x07
    ...
    {
      bool controlledMotion = pet->GetMotionMaster()->GetMotionSlotType(
    ...
    case COMMAND_ATTACK: // spellid=1792  //ATTACK
    {
      ...
      if (Creature *creaturePet = pet->ToCreature())
        if (!creaturePet->_CanDetectFeignDeathOf(TargetUnit) ||
            !creaturePet->CanCreatureAttack(TargetUnit))
          return;
    ...
    case COMMAND_ABANDON: // abandon (hunter pet) or dismiss (summoned pet)
      if (pet->GetCharmerGUID() == GetPlayer()->GetGUID()) {
        ...
        if (pet->IsPet()) {
          if (pet->ToPet()->getPetType() == HUNTER_PET)
            ...
            // dismissing a summoned pet is like killing them (this prevents
            // returning a soulshard...)
            pet->setDeathState(CORPSE);
        } else if (pet->HasUnitTypeMask(UNIT_MASK_MINION | UNIT_MASK_SUMMON |
                                        UNIT_MASK_GUARDIAN |
      ...
      break;
    default:
      LOG_ERROR("server", "WORLD: unknown PET flag Action %i and spellid %i.",
                uint32(flag), spellid);
    }
    break;
  ...
  case ACT_ENABLED:  // 0xC1    spell
  {
    ...
    charmInfo->SetIsCommandAttack(
        !pet->ToCreature()->HasReactState(REACT_PASSIVE));
    charmInfo->SetIsReturning(false);
    charmInfo->SetIsFollowing(false);
    ...
    if (result == SPELL_CAST_OK) {
      ...
      // 10% chance to play special pet attack talk, else growl
      // actually this only seems to happen on special spells, fire shield for
      // imp, torment for voidwalker, but it's stupid to check every spell
      if (pet->IsPet() && (((Pet *)pet)->getPetType() == SUMMON_PET) &&
      ...
      if (!GetPlayer()->IsFriendlyTo(unit_target)) {
        ...
        if (pet->GetVictim() != TargetUnit ||
          ...
          pet->ToPet()->CastWhenWillAvailable(spellid, unit_target, nullptr,
                                              tempspellIsPositive);
        }
        ...
        if (pet->GetTypeId() != TYPEID_PLAYER && pet->ToCreature() &&
          ...
          pet->ToPet()->CastWhenWillAvailable(spellid, unit_target, victim,
                                              tempspellIsPositive);
        }
      }
    } else {
...
void WorldSession::HandlePetSetAction(WorldPacket &recvData) {
  ...
  for (uint8 i = 0; i < count; ++i) {
    ...
    // in the normal case, command and reaction buttons can only be moved, not
    // removed at moving count == 2, at removing count == 1 ignore attempt to
    // remove command|reaction buttons (not possible at normal case)
  ...
  // Xinef: loop all pets with same entry (fixes partial state change for feral
  // spirits)
  for (Unit::ControlSet::const_iterator itr = petsSet.begin();
       itr != petsSet.end(); ++itr) {
    Unit *pet = *itr;
    if (checkPet->GetEntry() == GUID_ENPART(petguid) &&
        pet->GetEntry() != GUID_ENPART(petguid))
      continue;
    ...
    if (!charmInfo) {
      LOG_ERROR("server",
                ...
                "%u) is considered pet-like but doesn't have a charminfo!",
          ...
          if (act_state == ACT_ENABLED) {
            if (pet->GetTypeId() == TYPEID_UNIT && pet->IsPet())
              ...
              for (Unit::ControlSet::iterator itr =
                   ...
                   itr != GetPlayer()->m_Controlled.end(); ++itr)
                if ((*itr)->GetEntry() == pet->GetEntry())
                  (*itr)->GetCharmInfo()->ToggleCreatureAutocast(spellInfo,
                                                                 true);
          }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "HandlePetAbandon. CMSG_PET_ABANDON pet guid is %u",
            GUID_LOPART(guid));
#endif
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket &recvPacket) {
  ...
  Creature *checkPet =
      ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, guid);
  if (!checkPet || (checkPet != _player->GetGuardianPet() &&
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket &recvPacket) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "WORLD: CMSG_PET_CAST_SPELL, guid: " UI64FMTD
            ", castCount: %u, spellId %u, castFlags %u",
  ...
  if (!caster ||
      (caster != _player->GetGuardianPet() && caster != _player->GetCharm())) {
    LOG_ERROR("server",
