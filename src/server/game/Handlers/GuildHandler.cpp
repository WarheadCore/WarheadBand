...
void WorldSession::HandleGuildQueryOpcode(WorldPacket &recvPacket) {
  ...
  LOG_DEBUG("guild", "CMSG_GUILD_QUERY [%s]: Guild: %u",
            GetPlayerInfo().c_str(), guildId);
#endif
  if (!guildId)
    return;
...
void WorldSession::HandleGuildCreateOpcode(WorldPacket &recvPacket) {
  ...
  LOG_ERROR("server",
            "CMSG_GUILD_CREATE: Possible hacking-attempt: %s tried to create a "
            "guild [Name: %s] using cheats",
            GetPlayerInfo().c_str(), name.c_str());
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_INVITE [%s]: Invited: %s",
            GetPlayerInfo().c_str(), invitedName.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_REMOVE [%s]: Target: %s",
            GetPlayerInfo().c_str(), playerName.c_str());
#endif
...
void WorldSession::HandleGuildRosterOpcode(WorldPacket & /*recvPacket*/) {
  ...
  if (Guild *guild = GetPlayer()->GetGuild())
    ...
    Guild::SendCommandResult(this, GUILD_COMMAND_ROSTER,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_PROMOTE [%s]: Target: %s",
            GetPlayerInfo().c_str(), playerName.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_DEMOTE [%s]: Target: %s",
            GetPlayerInfo().c_str(), playerName.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_LEADER [%s]: Target: %s",
            GetPlayerInfo().c_str(), name.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_MOTD [%s]: MOTD: %s", GetPlayerInfo().c_str(),
            motd.c_str());
#endif
...
void WorldSession::HandleGuildRankOpcode(WorldPacket &recvPacket) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_RANK [%s]: Rank: %s (%u)",
            GetPlayerInfo().c_str(), rankName.c_str(), rankId);
#endif
  ...
  guild->HandleSetRankInfo(this, rankId, rankName, rights, money,
                           rightsAndSlots);
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_ADD_RANK [%s]: Rank: %s",
            GetPlayerInfo().c_str(), rankName.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_INFO_TEXT [%s]: %s", GetPlayerInfo().c_str(),
            info.c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "MSG_SAVE_GUILD_EMBLEM [%s]: Guid: [" UI64FMTD
            "] Style: %d, Color: %d, BorderStyle: %d, BorderColor: %d, "
            "BackgroundColor: %d",
  ...
  if (GetPlayer()->GetNPCIfCanInteractWith(vendorGuid,
    ...
    if (Guild *guild = GetPlayer()->GetGuild())
      ...
      Guild::SendSaveEmblemResult(
          this, ERR_GUILDEMBLEM_NOGUILD); // "You are not part of a guild!";
  } else
    Guild::SendSaveEmblemResult(
        this, ERR_GUILDEMBLEM_INVALIDVENDOR); // "That's not an emblem vendor!"
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "MSG_GUILD_BANK_MONEY_WITHDRAWN [%s]",
            GetPlayerInfo().c_str());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANKER_ACTIVATE [%s]: Go: [" UI64FMTD "] AllSlots: %u",
            GetPlayerInfo().c_str(), guid, sendAllSlots);
#endif
  Guild *const guild = GetPlayer()->GetGuild();
  if (!guild) {
    Guild::SendCommandResult(this, GUILD_COMMAND_VIEW_TAB,
                             ERR_GUILD_PLAYER_NOT_IN_GUILD);
    return;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANK_QUERY_TAB [%s]: Go: [" UI64FMTD
            "], TabId: %u, ShowTabs: %u",
            GetPlayerInfo().c_str(), guid, tabId, full);
#endif
  if (GetPlayer()->GetGameObjectIfCanInteractWith(guid,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANK_DEPOSIT_MONEY [%s]: Go: [" UI64FMTD "], money: %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANK_WITHDRAW_MONEY [%s]: Go: [" UI64FMTD
            "], money: %u",
            GetPlayerInfo().c_str(), guid, money);
#endif
  if (money && GetPlayer()->GetGameObjectIfCanInteractWith(
                   guid, GAMEOBJECT_TYPE_GUILD_BANK))
    if (Guild *guild = GetPlayer()->GetGuild())
...
void WorldSession::HandleGuildBankSwapItems(WorldPacket &recvData) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_GUILD_BANK_SWAP_ITEMS [%s]",
            GetPlayerInfo().c_str());
#endif
  ...
  if (!GetPlayer()->GetGameObjectIfCanInteractWith(
          GoGuid, GAMEOBJECT_TYPE_GUILD_BANK)) {
  ...
  if (bankToBank) {
    ...
    guild->SwapItems(GetPlayer(), tabId, slotId, destTabId, destSlotId,
                     splitedAmount);
  } else {
    ...
    if (autoStore) {
      ...
      recvData.read_skip<uint8>();  // ToChar (?), always and expected to be 1
                                   // (autostore only triggered in Bank -> Char)
      recvData.read_skip<uint32>(); // Always 0
    } else {
      recvData >> playerBag;
    ...
    if (!Player::IsInventoryPos(playerBag, playerSlotId) &&
      ...
      guild->SwapItemsWithInventory(GetPlayer(), toChar, tabId, slotId,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANK_BUY_TAB [%s]: Go: [" UI64FMTD "], TabId: %u",
            GetPlayerInfo().c_str(), guid, tabId);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild",
            "CMSG_GUILD_BANK_UPDATE_TAB [%s]: Go: [" UI64FMTD
            "], TabId: %u, Name: %s, Icon: %s",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "MSG_GUILD_BANK_LOG_QUERY [%s]: TabId: %u",
            GetPlayerInfo().c_str(), tabId);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "MSG_QUERY_GUILD_BANK_TEXT [%s]: TabId: %u",
            GetPlayerInfo().c_str(), tabId);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("guild", "CMSG_SET_GUILD_BANK_TEXT [%s]: TabId: %u, Text: %s",
            GetPlayerInfo().c_str(), tabId, text.c_str());
#endif
