...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "Petitioner with GUID %u tried sell petition: name %s",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandlePetitionBuyOpcode - Unit (GUID: %u) not found or "
  ...
  if (creature->IsTabardDesigner()) {
    ...
    default:
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("network", "unknown selection at buy arena petition: %u",
                clientIndex);
#endif
      return;
    }
  ...
  if (type == GUILD_CHARTER_TYPE) {
    if (sGuildMgr->GetGuildByName(name)) {
      Guild::SendCommandResult(this, GUILD_COMMAND_CREATE,
                               ERR_GUILD_NAME_EXISTS_S, name);
      return;
    }
  ...
  InventoryResult msg = _player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest,
                                                 charterid, pProto->BuyCount);
  if (msg != EQUIP_ERR_OK) {
  ...
  Petition const *petition =
      sPetitionMgr->GetPetitionByOwnerWithType(_player->GetGUIDLow(), type);
  ...
  sPetitionMgr->AddPetition(charter->GetGUIDLow(), _player->GetGUIDLow(), name,
                            uint8(type));
}
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket &recvData) {
  ...
  // GUID_LOPART(petitionguid) at some GCC versions (wrong code optimization in
  // compiler?)
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_PETITION_SHOW_SIGNATURES petition entry: '%u'",
            petitionGuidLow);
#endif
  ...
  if (signs)
    for (SignatureMap::const_iterator itr = signatures->signatureMap.begin();
         itr != signatures->signatureMap.end(); ++itr) {
      data << uint64(
          MAKE_NEW_GUID(itr->first, 0, HIGHGUID_PLAYER)); // Player GUID
...
void WorldSession::HandlePetitionQueryOpcode(WorldPacket &recvData) {
  ...
  recvData >> petitionguid; // petition guid
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "CMSG_PETITION_QUERY Petition GUID %u Guild GUID %u",
...
void WorldSession::SendPetitionQueryOpcode(uint64 petitionguid) {
  Petition const *petition =
      sPetitionMgr->GetPetition(GUID_LOPART(petitionguid));
  if (!petition) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network", "CMSG_PETITION_QUERY failed for petition (GUID: %u)",
  ...
  data << uint32(
      GUID_LOPART(petitionguid)); // guild/team guid (in Trinity always same as
                                  // GUID_LOPART(petition guid)
  data << MAKE_NEW_GUID(petition->ownerGuid, 0,
                        HIGHGUID_PLAYER); // charter owner guid
  ...
  if (type == GUILD_CHARTER_TYPE) {
    ...
    data << uint32(needed);
    data << uint32(needed);
    data << uint32(0); // bypass client - side limitation, a different value is
                       // needed here for each petition
  } else {
    data << uint32(type - 1);
    data << uint32(type - 1);
    data << uint32(type); // bypass client - side limitation, a different value
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "Petition (GUID: %u) renamed to '%s'",
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket &recvData) {
  ...
  Petition const *petition =
      sPetitionMgr->GetPetition(GUID_LOPART(petitionGuid));
  if (!petition) {
    LOG_ERROR("server", "Petition %u is not found for player %u %s",
  ...
  Signatures const *signatures =
      sPetitionMgr->GetSignature(GUID_LOPART(petitionGuid));
  if (!signatures)
    return;
  ...
  if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GUILD) &&
      GetPlayer()->GetTeamId() !=
          sObjectMgr->GetPlayerTeamIdByGUID(ownerGuid)) {
    if (type != GUILD_CHARTER_TYPE)
  ...
  if (type != GUILD_CHARTER_TYPE) {
    ...
    if (_player->GetGuildId()) {
      Guild::SendCommandResult(this, GUILD_COMMAND_INVITE,
                               ERR_ALREADY_IN_GUILD_S, _player->GetName());
      return;
    }
  ...
  // xinef: fill petition store
  sPetitionMgr->AddSignature(GUID_LOPART(petitionGuid), GetAccountId(),
                             playerGuid);

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network",
            "PETITION SIGN: GUID %u by player: %s (GUID: %u Account: %u)",
            GUID_LOPART(petitionGuid), _player->GetName().c_str(), playerGuid,
            GetAccountId());
#endif
...
void WorldSession::HandlePetitionDeclineOpcode(WorldPacket &recvData) {
  ...
  recvData >> petitionguid; // petition guid
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "Petition %u declined by %u", GUID_LOPART(petitionguid),
            _player->GetGUIDLow());
#endif
  ...
  if (Player *owner = ObjectAccessor::FindPlayerInOrOutOfWorld(
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket &recvData) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "Petition %u turned in by %u", GUID_LOPART(petitionGuid),
            _player->GetGUIDLow());
#endif
  ...
  if (!petition) {
    LOG_ERROR("server",
              "Player %s (guid: %u) tried to turn in petition (guid: %u) that "
  ...
  if (type == GUILD_CHARTER_TYPE) {
    ...
    if (!arenaTeam->Create(_player->GetGUID(), type, name, background, icon,
                           iconcolor, border, bordercolor)) {
      delete arenaTeam;
      return;
    }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "PetitonsHandler: Arena team (guid: %u) added to ObjectMgr",
              arenaTeam->GetId());
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("network",
                  "PetitionsHandler: Adding arena team (guid: %u) member %u",
                  arenaTeam->GetId(), itr->first);
#endif
...
void WorldSession::SendPetitionShowList(uint64 guid) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: HandlePetitionShowListOpcode - Unit (GUID: %u) not found "
  ...
  if (creature->IsTabardDesigner()) {
    ...
    data << uint32(
        sWorld->getIntConfig(CONFIG_CHARTER_COST_GUILD)); // charter cost
    ...
    data << uint32(CHARTER_DISPLAY_ID);     // charter display id
    data << uint32(
        sWorld->getIntConfig(CONFIG_CHARTER_COST_ARENA_2v2)); // charter cost
    ...
    data << uint32(CHARTER_DISPLAY_ID);     // charter display id
    data << uint32(
        sWorld->getIntConfig(CONFIG_CHARTER_COST_ARENA_3v3)); // charter cost
    ...
    data << uint32(CHARTER_DISPLAY_ID);     // charter display id
    data << uint32(
        sWorld->getIntConfig(CONFIG_CHARTER_COST_ARENA_5v5)); // charter cost
