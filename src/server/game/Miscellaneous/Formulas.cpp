 ...
 * GPL v2 license:
 * https://github.com/azerothcore/azerothcore-wotlk/blob/master/LICENSE-GPL2
...
uint32 Warhead::XP::BaseGain(uint8 pl_level, uint8 mob_level,
...
uint32 Warhead::XP::Gain(Player *player, Unit *unit,
  ...
  if (!creature ||
    ...
    gain = BaseGain(
        player->getLevel(), unit->getLevel(),
        GetContentLevelsForMapAndZone(unit->GetMapId(), unit->GetZoneId()));

    if (gain && creature) {
      if (creature->isElite()) {
        // Elites in instances have a 2.75x XP bonus instead of the regular 2x
        // world bonus.
    ...
    xpMod *= isBattleGround ? sWorld->getRate(RATE_XP_BG_KILL)
