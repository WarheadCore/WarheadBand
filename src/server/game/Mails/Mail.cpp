...
void MailDraft::prepareItems(Player *receiver, SQLTransaction &trans) {
  ...
  for (uint32 i = 0; m_items.size() < MAX_MAIL_ITEMS && i < max_slot; ++i) {
    if (LootItem *lootitem = mailLoot.LootItemInSlot(i, receiver)) {
      if (Item *item =
              Item::CreateItem(lootitem->itemid, lootitem->count, receiver)) {
...
void MailDraft::SendReturnToSender(uint32 /*sender_acc*/, uint32 sender_guid,
  ...
  Player *receiver = ObjectAccessor::FindPlayerInOrOutOfWorld(
  ...
  SendMailTo(trans, MailReceiver(receiver, receiver_guid),
             MailSender(MAIL_NORMAL, sender_guid), MAIL_CHECK_MASK_RETURNED, 0);
}
...
void MailDraft::SendMailTo(SQLTransaction &trans, MailReceiver const &receiver,
  ...
  sScriptMgr->OnBeforeMailDraftSendMailTo(this, receiver, sender, checked,
  ...
  Player *pSender = ObjectAccessor::FindPlayerInOrOutOfWorld(
      MAKE_NEW_GUID(sender.GetSenderId(), 0, HIGHGUID_PLAYER));
  ...
  // sender is a game master)
    ...
    if (m_COD)
      ...
      expire_delay =
          pSender && pSender->GetSession()->GetSecurity() ? 90 * DAY : 30 * DAY;
  }
