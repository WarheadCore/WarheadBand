...
void CharacterDatabaseCleaner::CleanDatabase() {
  ...
  QueryResult result = CharacterDatabase.PQuery(
      "SELECT value FROM worldstates WHERE entry = %d", WS_CLEANING_FLAGS);
  if (!result)
    return;
  ...
  CharacterDatabase.DirectPExecute(
      "UPDATE worldstates SET value = %u WHERE entry = %d", flags,
      WS_CLEANING_FLAGS);

  sWorld->SetCleaningFlags(flags);

  LOG_INFO("server", ">> Cleaned character database in %u ms",
...
void CharacterDatabaseCleaner::CleanCharacterTalent() {
  CharacterDatabase.DirectPExecute(
      "DELETE FROM character_talent WHERE specMask >= %u",
      1 << MAX_TALENT_SPECS);
