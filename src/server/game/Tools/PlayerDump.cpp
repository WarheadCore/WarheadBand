...
bool PlayerDumpWriter::DumpTable(std::string &dump, uint32 guid,
                                 char const *tableFrom, char const *tableTo,
                                 DumpTableType type) {
  ...
  do {
    ...
    QueryResult result = CharacterDatabase.PQuery("SELECT * FROM %s WHERE %s",
                                                  tableFrom, wherestr.c_str());
    if (!result)
      return true;
    ...
    do {
      ...
      case DTT_INVENTORY:
        StoreGUID(result, 3,
                  items); // item guid collection (character_inventory.item)
        break;
      case DTT_PET:
        StoreGUID(result, 0,
                  pets); // pet petnumber collection (character_pet.id)
        break;
      case DTT_MAIL:
      ...
      case DTT_MAIL_ITEM:
        StoreGUID(result, 1,
                  items); // item guid collection (mail_items.item_guid)
        break;
      case DTT_CHARACTER: {
        if (result->GetFieldCount() <= 74) // avoid crashes on next check
          LOG_FATAL("server", "PlayerDumpWriter::DumpTable - Trying to access "
                              "non-existing or wrong positioned field "
                              "(`deleteInfos_Account`) in `characters` table.");

        if (result->Fetch()[74].GetUInt32()) // characters.deleteInfos_Account -
                                             // if filled error
          return false;
        break;
      }
      ...
      dump += "\n";
           ...
           guids_itr != guids->end()); // not set case iterate single time, set
                                       // case iterate for all guids
  return true;
}
...
DumpReturn PlayerDumpReader::LoadDump(const std::string &file, uint32 account,
  ...
  while (!feof(fin)) {
    ...
    // skip logfile-side dump start notice, the important notes and dump end
    // notices
    ...
    */
    ...
    if (tn.empty()) {
      LOG_ERROR("server",
                "LoadPlayerDump: Can't extract table name from line: '%s'!",
                line.c_str());
      ROLLBACK(DUMP_FILE_BROKEN);
    }
    ...
    case DTT_CHARACTER: {
      ...
      if (name == "") {
        ...
        if (result)
          if (!changenth(line, 38,
                         "1")) // characters.at_login set to "rename on login"
            ROLLBACK(DUMP_FILE_BROKEN);
    ...
    case DTT_EQSET_TABLE: {
      ...
      char newSetGuid[24];
      snprintf(newSetGuid, 24, UI64FMTD,
               sObjectMgr->GenerateEquipmentSetGuid());
    ...
    case DTT_PET: {
      // store a map of old pet id to new inserted pet id for use by type 5
      // tables
      ...
      std::map<uint32, uint32>::const_iterator petids_iter =
          petids.find(atoi(currpetid));
  ...
  sWorld->AddGlobalPlayerData(guid, account, name, gender, race, playerClass,
                              level, mails.size(), 0);
