...
bool BattlegroundSA::ResetObjs() {
  ...
  for (uint8 i = 0; i < BG_SA_BOAT_ONE; i++) {
    if (!AddObject(i, BG_SA_ObjEntries[i], BG_SA_ObjSpawnlocs[i][0],
                   BG_SA_ObjSpawnlocs[i][1], BG_SA_ObjSpawnlocs[i][2],
  ...
  for (uint8 i = 0; i < BG_SA_DEMOLISHER_5; i++) {
    if (!AddCreature(BG_SA_NpcEntries[i], i, BG_SA_NpcSpawnlocs[i][0],
                     BG_SA_NpcSpawnlocs[i][1], BG_SA_NpcSpawnlocs[i][2],
                     BG_SA_NpcSpawnlocs[i][3], 600))
      return false;
  }
  ...
  for (uint8 i = 0; i < BG_SA_MAX_GY; i++) {
    ...
    if (i == BG_SA_BEACH_GY) {
      ...
      AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z,
                     BG_SA_GYOrientation[i], Attackers);
    } else {
      GraveyardStatus[i] = GetOtherTeamId(Attackers);
      if (!AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z,
                          BG_SA_GYOrientation[i], GetOtherTeamId(Attackers)))
  ...
  for (uint8 i = BG_SA_CENTRAL_FLAG; i < BG_SA_PORTAL_DEFFENDER_BLUE; i++) {
    AddObject(i, (BG_SA_ObjEntries[i] - (Attackers == TEAM_ALLIANCE ? 1 : 0)),
              ...
              BG_SA_ObjSpawnlocs[i][2], BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0,
              RESPAWN_ONE_DAY);
    GetBGObject(i)->SetUInt32Value(GAMEOBJECT_FACTION, atF);
  }

  for (uint8 i = BG_SA_PORTAL_DEFFENDER_BLUE; i < BG_SA_BOMB; i++) {
    AddObject(i, BG_SA_ObjEntries[i], BG_SA_ObjSpawnlocs[i][0],
              BG_SA_ObjSpawnlocs[i][1], BG_SA_ObjSpawnlocs[i][2],
              BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0, RESPAWN_ONE_DAY);
  ...
  for (uint8 i = BG_SA_BOMB; i < BG_SA_MAXOBJ; i++) {
    AddObject(i, BG_SA_ObjEntries[BG_SA_BOMB], BG_SA_ObjSpawnlocs[i][0],
              BG_SA_ObjSpawnlocs[i][1], BG_SA_ObjSpawnlocs[i][2],
              BG_SA_ObjSpawnlocs[i][3], 0, 0, 0, 0, RESPAWN_ONE_DAY);
    GetBGObject(i)->SetUInt32Value(GAMEOBJECT_FACTION, atF);
  }
  ...
  UpdateWorldState(BG_SA_LEFT_GY_HORDE,
                   GraveyardStatus[BG_SA_LEFT_CAPTURABLE_GY] == TEAM_HORDE ? 1
                                                                           : 0);
  UpdateWorldState(
...
void BattlegroundSA::PostUpdateImpl(uint32 diff) {
  if (InitSecondRound) {
    if (UpdateWaitTimer < diff) {
      if (!SignaledRoundTwo) {
        ...
        InitSecondRound = false;
        SendMessageToAll(LANG_BG_SA_ROUND_TWO_ONE_MINUTE,
  ...
  if (Status == BG_SA_WARMUP) {
    ...
    if (TotalTime >= 60000) {
      ...
      // status was set to STATUS_WAIT_JOIN manually for Preparation, set it
      // back now
    ...
    if (TotalTime >= 30000) {
      if (!SignaledRoundTwoHalfMin) {
        ...
        SendMessageToAll(LANG_BG_SA_ROUND_TWO_START_HALF_MINUTE,
                         CHAT_MSG_BG_SYSTEM_NEUTRAL);
      }
    }
    StartShips();
    ...
    if (Status == BG_SA_ROUND_ONE) {
      if (TotalTime >= BG_SA_ROUNDLENGTH || _relicClicked) {
        if (_relicClicked) {
          ...
          // cast this spell only upon timer end, no other ability for defenders
          // to win :)
          for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin();
               itr != GetPlayers().end(); ++itr)
...
void BattlegroundSA::FillInitialWorldStates(WorldPacket &data) {
  ...
  data << uint32(BG_SA_RIGHT_GY_HORDE)
       << uint32(GraveyardStatus[BG_SA_RIGHT_CAPTURABLE_GY] == TEAM_HORDE ? 1
                                                                          : 0);
...
void BattlegroundSA::UpdatePlayerScore(Player *player, uint32 type,
...
void BattlegroundSA::DefendersPortalTeleport(GameObject *portal, Player *plr) {
  ...
  plr->TeleportTo(plr->GetMapId(), SOTADefPortalDest[portal_num][0],
                  SOTADefPortalDest[portal_num][1],
                  SOTADefPortalDest[portal_num][2],
                  SOTADefPortalDest[portal_num][3], TELE_TO_SPELL);
}

void BattlegroundSA::EventPlayerDamagedGO(Player * /*player*/, GameObject *go,
                                          uint32 eventType) {
  if (!go || !go->GetGOInfo())
    return;
...
void BattlegroundSA::DemolisherStartState(bool start) {
  ...
  for (uint8 i = BG_SA_DEMOLISHER_1; i <= BG_SA_DEMOLISHER_4; i++)
    if (Creature *dem = GetBGCreature(i)) {
      if (start)
        dem->SetFlag(UNIT_FIELD_FLAGS,
                     UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
      else
        dem->RemoveFlag(UNIT_FIELD_FLAGS,
                        UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
    }
  ...
  for (uint8 i = BG_SA_GUN_1; i <= BG_SA_GUN_10; i++)
    if (Creature *gun = GetBGCreature(i)) {
      if (start)
        gun->SetFlag(UNIT_FIELD_FLAGS,
                     UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
      else
        gun->RemoveFlag(UNIT_FIELD_FLAGS,
                        UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
    }
...
void BattlegroundSA::SendTime() {
  ...
  UpdateWorldState(BG_SA_TIMER_SEC_DECS,
                   ((end_of_round % 60000) % 10000) / 1000);
}
...
void BattlegroundSA::CaptureGraveyard(BG_SA_Graveyards i, Player *Source) {
  ...
  if (!sg) {
    LOG_ERROR("server",
              "BattlegroundSA::CaptureGraveyard: non-existant GY entry: %u",
              BG_SA_GYEntries[i]);
    return;
  }

  AddSpiritGuide(i + BG_SA_MAXNPC, sg->x, sg->y, sg->z, BG_SA_GYOrientation[i],
                 GraveyardStatus[i]);
  uint32 npc = 0;
  uint32 flag = 0;
  ...
  case BG_SA_LEFT_CAPTURABLE_GY:
    ...
    AddObject(flag,
              ...
              BG_SA_ObjSpawnlocs[flag][2], BG_SA_ObjSpawnlocs[flag][3], 0, 0, 0,
              0, RESPAWN_ONE_DAY);
    ...
    for (uint8 j = BG_SA_DEMOLISHER_7; j <= BG_SA_DEMOLISHER_8; j++) {
      AddCreature(BG_SA_NpcEntries[j], j, BG_SA_NpcSpawnlocs[j][0],
                  BG_SA_NpcSpawnlocs[j][1], BG_SA_NpcSpawnlocs[j][2],
                  BG_SA_NpcSpawnlocs[j][3], 600);
  ...
  case BG_SA_RIGHT_CAPTURABLE_GY:
    ...
    AddCreature(BG_SA_NpcEntries[npc], npc, BG_SA_NpcSpawnlocs[npc][0],
                BG_SA_NpcSpawnlocs[npc][1], BG_SA_NpcSpawnlocs[npc][2],
