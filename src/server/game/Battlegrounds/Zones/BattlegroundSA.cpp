    ...
    if (InitSecondRound)
    {
        if (UpdateWaitTimer < diff)
        {
            if (!SignaledRoundTwo)
            {
                ...
                InitSecondRound = false;
                Warhead::Text::SendBattlegroundMessageToAll(this, LANG_BG_SA_ROUND_TWO_ONE_MINUTE, CHAT_MSG_BG_SYSTEM_NEUTRAL);
            }
        }
        else
        {
    ...
    else if (Status == BG_SA_SECOND_WARMUP)
    {
        ...
        if (TotalTime >= 30000)
        {
            if (!SignaledRoundTwoHalfMin)
            {
                ...
                Warhead::Text::SendBattlegroundMessageToAll(this, LANG_BG_SA_ROUND_TWO_START_HALF_MINUTE, CHAT_MSG_BG_SYSTEM_NEUTRAL);
            }
        }
        StartShips();
        return;
...
void BattlegroundSA::EventPlayerDamagedGO(Player* /*player*/, GameObject* go, uint32 eventType)
{
    ...
    if (eventType == go->GetGOInfo()->building.destroyedEvent)
    {
        if (go->GetGOInfo()->building.destroyedEvent == 19837)
            ...
            Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_WAS_DESTROYED, go->GetGOInfo()->name.c_str());
    ...
    if (eventType == go->GetGOInfo()->building.damageEvent)
        Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_IS_UNDER_ATTACK, go->GetGOInfo()->name.c_str());
}
    ...
    switch(i)
    {
        case BG_SA_LEFT_CAPTURABLE_GY:
            ...
            if (Source->GetTeamId() == TEAM_ALLIANCE)
                Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_A_GY_WEST);
            else
                Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_H_GY_WEST);
            break;
        case BG_SA_RIGHT_CAPTURABLE_GY:
            ...
            if (Source->GetTeamId() == TEAM_ALLIANCE)
                Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_A_GY_EAST);
            else
                Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_H_GY_EAST);
            break;
        case BG_SA_CENTRAL_CAPTURABLE_GY:
            ...
            if (Source->GetTeamId() == TEAM_ALLIANCE)
                Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_A_GY_SOUTH);
            else
                Warhead::Text::SendBattlegroundWarningToAll(this, LANG_BG_SA_H_GY_SOUTH);
            break;
        default:
            ABORT();
            break;
...
void BattlegroundSA::EventPlayerUsedGO(Player* Source, GameObject* object)
{
    if (object->GetEntry() == BG_SA_ObjEntries[BG_SA_TITAN_RELIC] && CanInteractWithObject(BG_SA_TITAN_RELIC))
    {
        ...
        {
            if (Source->GetTeamId() == TEAM_ALLIANCE)
                Warhead::Text::SendBattlegroundMessageToAll(this, LANG_BG_SA_ALLIANCE_CAPTURED_RELIC, CHAT_MSG_BG_SYSTEM_NEUTRAL);
            else Warhead::Text::SendBattlegroundMessageToAll(this, LANG_BG_SA_HORDE_CAPTURED_RELIC, CHAT_MSG_BG_SYSTEM_NEUTRAL);
