  ...
  if (GetStatus() == STATUS_IN_PROGRESS) {
    ...
    while (uint32 eventId = _bgEvents.ExecuteEvent())
      ...
      case BG_AB_EVENT_CAPTURE_GOLDMINE: {
        ...
        _capturePointInfo[node]._state = teamId == TEAM_ALLIANCE
        ...
        SendMessage2ToAll(LANG_BG_AB_NODE_TAKEN,
                          teamId == TEAM_ALLIANCE ? CHAT_MSG_BG_SYSTEM_ALLIANCE
                                                  : CHAT_MSG_BG_SYSTEM_HORDE,
                          nullptr,
                          teamId == TEAM_ALLIANCE ? LANG_BG_AB_ALLY
                                                  : LANG_BG_AB_HORDE,
                          LANG_BG_AB_NODE_STABLES + node);
        PlaySoundToAll(teamId == TEAM_ALLIANCE
      ...
      case BG_AB_EVENT_HORDE_TICK: {
        ...
        auto information =
            uint8(m_TeamScores[teamId] / BG_AB_WARNING_NEAR_VICTORY_SCORE);
        ...
        if (reputationRewards < uint8(m_TeamScores[teamId] / _reputationTics))
          RewardReputationToTeam(teamId == TEAM_ALLIANCE ? 509 : 510, 10,
                                 teamId);
        if (information <
            uint8(m_TeamScores[teamId] / BG_AB_WARNING_NEAR_VICTORY_SCORE)) {
          SendMessageToAll(teamId == TEAM_ALLIANCE ? LANG_BG_AB_A_NEAR_VICTORY
                                                   : LANG_BG_AB_H_NEAR_VICTORY,
                           CHAT_MSG_BG_SYSTEM_NEUTRAL);
          PlaySoundToAll(BG_AB_SOUND_NEAR_VICTORY);
        }

        UpdateWorldState(teamId == TEAM_ALLIANCE ? BG_AB_OP_RESOURCES_ALLY
...
void BattlegroundAB::StartingEventCloseDoors() {
  for (uint32 obj = BG_AB_OBJECT_BANNER_NEUTRAL;
       obj < BG_AB_DYNAMIC_NODES_COUNT * BG_AB_OBJECTS_PER_NODE; ++obj)
...
void BattlegroundAB::StartingEventOpenDoors() {
  for (uint32 banner = BG_AB_OBJECT_BANNER_NEUTRAL, i = 0;
       i < BG_AB_DYNAMIC_NODES_COUNT; banner += BG_AB_OBJECTS_PER_NODE, ++i)
...
void BattlegroundAB::CreateBanner(uint8 node, bool delay) {
  ...
  SpawnBGObject(node * BG_AB_OBJECTS_PER_NODE + _capturePointInfo[node]._state,
                RESPAWN_IMMEDIATELY);
...
void BattlegroundAB::NodeOccupied(uint8 node) {
  ApplyPhaseMask();
  AddSpiritGuide(node, BG_AB_SpiritGuidePos[node][0],
  ...
  if (!trigger)
    trigger =
        AddCreature(WORLD_TRIGGER, BG_AB_ALL_NODES_COUNT + node,
                    BG_AB_NodePositions[node][0], BG_AB_NodePositions[node][1],
                    BG_AB_NodePositions[node][2], BG_AB_NodePositions[node][3]);

  if (trigger) {
    trigger->setFaction(
        _capturePointInfo[node]._ownerTeamId == TEAM_ALLIANCE ? 84 : 83);
...
void BattlegroundAB::EventPlayerClickedOnFlag(Player *player,
  ...
  if (_capturePointInfo[node]._state == BG_AB_NODE_STATE_NEUTRAL) {
    ...
    if (!_capturePointInfo[node]._captured) {
      ...
      _capturePointInfo[node]._state =
          BG_AB_NODE_STATE_ALLY_OCCUPIED + player->GetTeamId();
      _capturePointInfo[node]._ownerTeamId = player->GetTeamId();
  ...
  SendMessage2ToAll(message,
                    player->GetTeamId() == TEAM_ALLIANCE
                        ? CHAT_MSG_BG_SYSTEM_ALLIANCE
                        : CHAT_MSG_BG_SYSTEM_HORDE,
                    player, LANG_BG_AB_NODE_STABLES + node, message2);
}
...
bool BattlegroundAB::SetupBattleground() {
  ...
  AddObject(BG_AB_OBJECT_GATE_A, BG_AB_OBJECTID_GATE_A,
  ...
  for (uint32 i = 0; i < BG_AB_DYNAMIC_NODES_COUNT; ++i) {
    AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i, Buff_Entries[0],
    ...
    AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i + 1, Buff_Entries[1],
              BG_AB_BuffPositions[i][0], BG_AB_BuffPositions[i][1],
              BG_AB_BuffPositions[i][2], BG_AB_BuffPositions[i][3], 0, 0,
              sin(BG_AB_BuffPositions[i][3] / 2),
              cos(BG_AB_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY);
    AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i + 2, Buff_Entries[2],
              BG_AB_BuffPositions[i][0], BG_AB_BuffPositions[i][1],
              BG_AB_BuffPositions[i][2], BG_AB_BuffPositions[i][3], 0, 0,
              sin(BG_AB_BuffPositions[i][3] / 2),
              cos(BG_AB_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY);
  ...
  AddSpiritGuide(BG_AB_SPIRIT_ALIANCE,
                 ...
                 BG_AB_SpiritGuidePos[BG_AB_SPIRIT_ALIANCE][3], TEAM_ALLIANCE);
  AddSpiritGuide(BG_AB_SPIRIT_HORDE,
                 ...
                 BG_AB_SpiritGuidePos[BG_AB_SPIRIT_HORDE][3], TEAM_HORDE);

  for (uint32 i = BG_AB_OBJECT_BANNER_NEUTRAL; i < BG_AB_OBJECT_MAX; ++i)
    if (BgObjects[i] == 0) {
      LOG_ERROR("sql.sql", "BatteGroundAB: Failed to spawn some object "
  ...
  for (uint32 i = BG_AB_SPIRIT_ALIANCE; i <= BG_AB_SPIRIT_HORDE; ++i)
    if (BgCreatures[i] == 0) {
      LOG_ERROR("sql.sql", "BatteGroundAB: Failed to spawn spirit guides "
                           "Battleground not created!");
      return false;
    }
...
GraveyardStruct const *BattlegroundAB::GetClosestGraveyard(Player *player) {
  GraveyardStruct const *entry = sGraveyard->GetGraveyard(
      BG_AB_GraveyardIds[BG_AB_SPIRIT_ALIANCE + player->GetTeamId()]);
...
void BattlegroundAB::UpdatePlayerScore(Player *player, uint32 type,
