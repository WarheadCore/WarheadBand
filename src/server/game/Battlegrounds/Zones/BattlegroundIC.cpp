...
void BattlegroundIC::DoAction(uint32 action, uint64 guid) {
  ...
  player->TeleportTo(GetMapId(), x, y, z, player->GetOrientation(),
  ...
  for (uint8 i = NODE_TYPE_REFINERY; i < MAX_NODE_TYPES; ++i) {
    if (nodePoint[i].nodeType == NODE_TYPE_DOCKS) {
      if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
        ...
        if (nodePoint[i].timer <= diff) {
            ...
            if (Creature *catapult = GetBgMap()->GetCreature(BgCreatures[type]))
              if (!catapult->IsAlive()) {
                ...
                RespawnMap::iterator itr =
                    respawnMap.find(catapult->GetGUIDLow());
                if (itr == respawnMap.end() || time(nullptr) < itr->second)
                  continue;

                catapult->Relocate(
                    BG_IC_DocksVehiclesCatapults[j].GetPositionX(),
          ...
          // we need to confirm this is blizzlike, not sure if it is every 3
          // minutes
          for (uint8 j = 0; j < MAX_GLAIVE_THROWERS_SPAWNS_PER_FACTION; ++j) {
            ...
            if (Creature *glaiveThrower =
              ...
              if (!glaiveThrower->IsAlive()) {
                // Check if creature respawn time is properly saved
                RespawnMap::iterator itr =
                    respawnMap.find(glaiveThrower->GetGUIDLow());
                if (itr == respawnMap.end() || time(nullptr) < itr->second)
                  continue;

                glaiveThrower->Relocate(
                    BG_IC_DocksVehiclesGlaives[j].GetPositionX(),
    ...
    if (nodePoint[i].nodeType == NODE_TYPE_WORKSHOP) {
      if (nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A ||
        ...
        if (siegeEngineWorkshopTimer <= diff) {
          ...
          if (Creature *siege =
                  GetBGCreature(siegeType)) // this always should be true
            if (!siege->IsAlive()) {
              // Check if creature respawn time is properly saved
          ...
          for (uint8 u = 0; u < MAX_DEMOLISHERS_SPAWNS_PER_FACTION; ++u) {
            ...
            if (Creature *demolisher =
              ...
              if (!demolisher->IsAlive()) {
                // Check if creature respawn time is properly saved
                RespawnMap::iterator itr =
                    respawnMap.find(demolisher->GetGUIDLow());
                if (itr == respawnMap.end() || time(nullptr) < itr->second)
                  continue;
    ...
    if (nodePoint[i].needChange) {
      if (nodePoint[i].timer <= diff) {
        ...
        float cords[4] = {banner->GetPositionX(), banner->GetPositionY(),
                          banner->GetPositionZ(), banner->GetOrientation()};

        DelObject(nodePoint[i].gameobject_type);
        AddObject(nodePoint[i].gameobject_type, nodePoint[i].gameobject_entry,
                  cords[0], cords[1], cords[2], cords[3], 0, 0, 0, 0,
                  RESPAWN_ONE_DAY);
        ...
        SendMessage2ToAll(
            LANG_BG_IC_TEAM_HAS_TAKEN_NODE, CHAT_MSG_BG_SYSTEM_NEUTRAL, nullptr,
            (nodePoint[i].faction == TEAM_ALLIANCE ? LANG_BG_IC_ALLIANCE
                                                   : LANG_BG_IC_HORDE),
            nodePoint[i].string);
...
void BattlegroundIC::UpdatePlayerScore(Player *player, uint32 type,
...
bool BattlegroundIC::SetupBattleground() {
  for (uint8 i = 0; i < MAX_NORMAL_GAMEOBJECTS_SPAWNS; ++i) {
    if (!AddObject(BG_IC_ObjSpawnlocs[i].type, BG_IC_ObjSpawnlocs[i].entry,
                   ...
                   BG_IC_ObjSpawnlocs[i].z, BG_IC_ObjSpawnlocs[i].o, 0, 0, 0, 0,
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("server",
                "Isle of Conquest: There was an error spawning gameobject %u",
                BG_IC_ObjSpawnlocs[i].entry);
      return false;
    }
  ...
  for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTERS_SPAWNS; ++i) {
    if (!AddObject(BG_IC_Teleporters[i].type, BG_IC_Teleporters[i].entry,
                   ...
                   BG_IC_Teleporters[i].z, BG_IC_Teleporters[i].o, 0, 0, 0, 0,
                   RESPAWN_ONE_DAY)) {
      LOG_ERROR("server",
                "Isle of Conquest | Starting Event Open Doors: There was an "
                "error spawning gameobject %u",
  ...
  for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTER_EFFECTS_SPAWNS; ++i) {
    if (!AddObject(BG_IC_TeleporterEffects[i].type,
                   ...
                   BG_IC_TeleporterEffects[i].x, BG_IC_TeleporterEffects[i].y,
                   BG_IC_TeleporterEffects[i].z, BG_IC_TeleporterEffects[i].o,
  ...
  for (uint8 i = 0; i < MAX_NORMAL_NPCS_SPAWNS; ++i) {
    if (!AddCreature(BG_IC_NpcSpawnlocs[i].entry, BG_IC_NpcSpawnlocs[i].type,
                     ...
                     RESPAWN_ONE_DAY)) {
      LOG_ERROR("server",
                "Isle of Conquest: There was an error spawning creature %u",
                BG_IC_NpcSpawnlocs[i].entry);
      return false;
    }
  }

  if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 3, BG_IC_SpiritGuidePos[5][0],
                      BG_IC_SpiritGuidePos[5][1], BG_IC_SpiritGuidePos[5][2],
                      BG_IC_SpiritGuidePos[5][3], TEAM_ALLIANCE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 4, BG_IC_SpiritGuidePos[6][0],
                      BG_IC_SpiritGuidePos[6][1], BG_IC_SpiritGuidePos[6][2],
                      BG_IC_SpiritGuidePos[6][3], TEAM_HORDE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 5, BG_IC_SpiritGuidePos[7][0],
                      BG_IC_SpiritGuidePos[7][1], BG_IC_SpiritGuidePos[7][2],
                      BG_IC_SpiritGuidePos[7][3], TEAM_ALLIANCE) ||
      !AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + 6, BG_IC_SpiritGuidePos[8][0],
                      BG_IC_SpiritGuidePos[8][1], BG_IC_SpiritGuidePos[8][2],
                      BG_IC_SpiritGuidePos[8][3], TEAM_HORDE)) {
    LOG_ERROR("server",
  ...
  for (uint8 i = BG_IC_GO_HUGE_SEAFORIUM_BOMBS_A_1;
       i < BG_IC_GO_HUGE_SEAFORIUM_BOMBS_H_4; ++i)
...
void BattlegroundIC::EndBattleground(TeamId winnerTeamId) {
  SendMessage2ToAll(
      LANG_BG_IC_TEAM_WINS, CHAT_MSG_BG_SYSTEM_NEUTRAL, nullptr,
      (winnerTeamId == TEAM_ALLIANCE ? LANG_BG_IC_ALLIANCE : LANG_BG_IC_HORDE));
  ...
  for (uint8 i = 0; i < MAX_NODE_TYPES; ++i) {
    if (nodePoint[i].gameobject_entry == gameObject->GetEntry()) {
      ...
      if (nodePoint[i].banners[BANNER_A_CONTESTED] == nextBanner ||
        ...
        nodePoint[i].timer =
            BANNER_STATE_CHANGE_TIME; // 1 minute for last change (real faction
                                      // banner)
        nodePoint[i].needChange = true;

        RelocateDeadPlayers(
            BgCreatures[BG_IC_NPC_SPIRIT_GUIDE_1 + nodePoint[i].nodeType - 2]);

        // if we are here means that the point has been lost, or it is the first
        // capture

        if (nodePoint[i].nodeType != NODE_TYPE_REFINERY &&
            nodePoint[i].nodeType != NODE_TYPE_QUARRY)
          if (BgCreatures[BG_IC_NPC_SPIRIT_GUIDE_1 + (nodePoint[i].nodeType) -
                          2])
        ...
        SendMessage2ToAll(LANG_BG_IC_TEAM_ASSAULTED_NODE_1,
                          CHAT_MSG_BG_SYSTEM_NEUTRAL, player,
                          nodePoint[i].string);
        SendMessage2ToAll(
            LANG_BG_IC_TEAM_ASSAULTED_NODE_2, CHAT_MSG_BG_SYSTEM_NEUTRAL,
            player, nodePoint[i].string,
            (player->GetTeamId() == TEAM_ALLIANCE ? LANG_BG_IC_ALLIANCE
                                                  : LANG_BG_IC_HORDE));
      ...
      // if we are going to spawn the definitve faction banner, we dont need the
      // timer anymore
      {
        nodePoint[i].timer = BANNER_STATE_CHANGE_TIME;
        nodePoint[i].needChange = false;
        SendMessage2ToAll(LANG_BG_IC_TEAM_DEFENDED_NODE,
                          CHAT_MSG_BG_SYSTEM_NEUTRAL, player,
                          nodePoint[i].string);
      ...
      // If the needChange bool was set true, we will handle the rest in the
      // Update Map function.
      return;
    }
  }
...
uint32 BattlegroundIC::GetNextBanner(ICNodePoint *nodePoint, uint32 team,
                                     bool returnDefinitve) {
  // this is only used in the update map function
  if (returnDefinitve)
    // here is a special case, here we must return the definitve faction banner
    // after the grey banner was spawned 1 minute
    return nodePoint->banners[(team == TEAM_ALLIANCE ? BANNER_A_CONTROLLED
                                                     : BANNER_H_CONTROLLED)];

  // there were no changes, this point has never been captured by any faction or
  // at least clicked
...
void BattlegroundIC::HandleContestedNodes(ICNodePoint *nodePoint) {
  if (nodePoint->nodeType == NODE_TYPE_HANGAR) {
    if (gunshipAlliance && gunshipHorde)
      (nodePoint->faction == TEAM_ALLIANCE ? gunshipHorde : gunshipAlliance)
          ->EnableMovement(false);

    for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_1;
         u <= BG_IC_GO_HANGAR_TELEPORTER_3; ++u)
      DelObject(u);

    for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_EFFECT_1;
         u <= BG_IC_GO_HANGAR_TELEPORTER_EFFECT_3; ++u)
      DelObject(u);
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint *nodePoint,
  ...
  if (nodePoint->nodeType != NODE_TYPE_REFINERY &&
    ...
    if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + nodePoint->nodeType - 2,
                        ...
                        nodePoint->faction))
      LOG_ERROR("server",
                "Isle of Conquest: Failed to spawn spirit guide! point: %u, "
                "team: %u, ",
                nodePoint->nodeType, nodePoint->faction);
  }
  ...
  case BG_IC_GO_HANGAR_BANNER: {
    ...
    for (uint8 u = 0; u < MAX_HANGAR_TELEPORTERS_SPAWNS; ++u) {
      ...
      if (!AddObject(type,
                     ...
                     BG_IC_HangarTeleporters[u].GetOrientation(), 0, 0, 0, 0,
                     RESPAWN_ONE_DAY))
        LOG_ERROR("server",
                  "Isle of Conquest: There was an error spawning a gunship "
                  "portal. Type: %u",
    ...
    for (uint8 u = 0; u < MAX_HANGAR_TELEPORTER_EFFECTS_SPAWNS; ++u) {
      ...
      if (!AddObject(type,
                     ...
                     BG_IC_HangarTeleporterEffects[u].GetOrientation(), 0, 0, 0,
                     0, RESPAWN_ONE_DAY, GO_STATE_ACTIVE))
        LOG_ERROR("server",
                  "Isle of Conquest: There was an error spawning a gunship "
                  "portal effects. Type: %u",
                  BG_IC_GO_HANGAR_TELEPORTER_1 + u);
    }
    ...
    for (uint8 u = 0; u < MAX_TRIGGER_SPAWNS_PER_FACTION; ++u) {
      if (!AddCreature(NPC_WORLD_TRIGGER_NOT_FLOATING,
                       ...
                       nodePoint->faction == TEAM_ALLIANCE ? gunshipAlliance
                                                           : gunshipHorde))
        LOG_ERROR("server",
                  "Isle of Conquest: There was an error spawning a world "
                  "trigger. Type: %u",
                  BG_IC_NPC_WORLD_TRIGGER_NOT_FLOATING);
    }
    ...
    for (uint8 u = 0; u < MAX_CAPTAIN_SPAWNS_PER_FACTION; ++u) {
      ...
      if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_1)
        if (AddCreature(
                ...
                type,
                BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE ? 2
                                                                         : 0]
                    .GetPositionX(),
                BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE ? 2
                                                                         : 0]
      ...
      if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_2)
        if (!AddCreature(
                nodePoint->faction == TEAM_ALLIANCE
                    ...
                    : NPC_HORDE_GUNSHIP_CAPTAIN,
                type,
                BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE ? 3
                                                                         : 1]
                    .GetPositionX(),
                BG_IC_HangarCaptains[nodePoint->faction == TEAM_ALLIANCE ? 3
                                                                         : 1]
  ...
  case BG_IC_GO_DOCKS_BANNER:
    ...
    for (uint8 i = 0; i < MAX_GLAIVE_THROWERS_SPAWNS_PER_FACTION; ++i) {
      uint8 type =
          (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_GLAIVE_THROWER_1_A
                                               : BG_IC_NPC_GLAIVE_THROWER_1_H) +
          i;
      ...
      if (AddCreature(
              nodePoint->faction == TEAM_ALLIANCE ? NPC_GLAIVE_THROWER_A
                                                  : NPC_GLAIVE_THROWER_H,
              type, BG_IC_DocksVehiclesGlaives[i].GetPositionX(),
              BG_IC_DocksVehiclesGlaives[i].GetPositionY(),
              BG_IC_DocksVehiclesGlaives[i].GetPositionZ(),
              BG_IC_DocksVehiclesGlaives[i].GetOrientation(), RESPAWN_ONE_DAY))
    ...
    for (uint8 i = 0; i < MAX_CATAPULTS_SPAWNS_PER_FACTION; ++i) {
      ...
      if (AddCreature(NPC_CATAPULT, type,
        ...
        GetBGCreature(type)->setFaction(
            BG_IC_Factions[(nodePoint->faction == TEAM_ALLIANCE ? 0 : 1)]);
    }
    break;
  case BG_IC_GO_WORKSHOP_BANNER: {
    ...
    if (!recapture) {
      for (uint8 i = 0; i < MAX_DEMOLISHERS_SPAWNS_PER_FACTION; ++i) {
        uint8 type =
            (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_DEMOLISHER_1_A
                                                 : BG_IC_NPC_DEMOLISHER_1_H) +
            i;
        ...
        if (AddCreature(
                NPC_DEMOLISHER, type, BG_IC_WorkshopVehicles[i].GetPositionX(),
                BG_IC_WorkshopVehicles[i].GetPositionY(),
                BG_IC_WorkshopVehicles[i].GetPositionZ(),
                BG_IC_WorkshopVehicles[i].GetOrientation(), RESPAWN_ONE_DAY))
          GetBGCreature(type)->setFaction(
              BG_IC_Factions[(nodePoint->faction == TEAM_ALLIANCE ? 0 : 1)]);
      }

      uint8 siegeType =
          (nodePoint->faction == TEAM_ALLIANCE ? BG_IC_NPC_SIEGE_ENGINE_A
                                               : BG_IC_NPC_SIEGE_ENGINE_H);
      if (!GetBgMap()->GetCreature(BgCreatures[siegeType])) {
        AddCreature((nodePoint->faction == TEAM_ALLIANCE ? NPC_SIEGE_ENGINE_A
                                                         : NPC_SIEGE_ENGINE_H),
                    siegeType, BG_IC_WorkshopVehicles[4].GetPositionX(),
                    BG_IC_WorkshopVehicles[4].GetPositionY(),
                    BG_IC_WorkshopVehicles[4].GetPositionZ(),
                    BG_IC_WorkshopVehicles[4].GetOrientation(),
                    RESPAWN_ONE_DAY);
      ...
      if (Creature *siegeEngine =
        ...
        respawnMap[siegeEngine->GetGUIDLow()] =
            time(nullptr) + VEHICLE_RESPAWN_TIME;
      }
    }
    ...
    for (uint8 i = 0; i < MAX_WORKSHOP_BOMBS_SPAWNS_PER_FACTION; ++i) {
      AddObject(BG_IC_GO_SEAFORIUM_BOMBS_1 + i, GO_SEAFORIUM_BOMBS,
                ...
                workshopBombs[i].GetOrientation(), 0, 0, 0, 0, 10);
...
void BattlegroundIC::DestroyGate(Player * /*player*/, GameObject *go) {
  ...
  TurnBosses(true);
  SendMessage2ToAll(lang_entry, CHAT_MSG_BG_SYSTEM_NEUTRAL, nullptr,
                    (teamId == TEAM_ALLIANCE ? LANG_BG_IC_HORDE_KEEP
                                             : LANG_BG_IC_ALLIANCE_KEEP));
}

void BattlegroundIC::EventPlayerDamagedGO(Player * /*player*/,
                                          GameObject * /*go*/,
                                          uint32 /*eventType*/) {}
...
GraveyardStruct const *BattlegroundIC::GetClosestGraveyard(Player *player) {
  ...
  if (!good_entry)
    good_entry = sGraveyard->GetGraveyard(
        BG_IC_GraveyardIds[player->GetTeamId() + MAX_NODE_TYPES]);
