  ...
  */
  if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS]) {
    CastSpellOnTeam(23658,
                    TEAM_HORDE); // this is a spell which finishes a quest where
                                 // a player has to kill the boss
    ...
    CastSpellOnTeam(23658,
                    TEAM_ALLIANCE); // this is a spell which finishes a quest
    ...
    if (!m_CaptainAlive[0]) {
      LOG_ERROR("server", "Killed a Captain twice, please report this bug, if "
                          "you haven't done \".respawn\"");
      return;
    }
    ...
    if (creature)
      YellToAll(creature, GetAcoreString(LANG_BG_AV_A_CAPTAIN_DEAD),
                LANG_UNIVERSAL);
    ...
    Creature *creature = GetBGCreature(AV_CPLACE_HERALD);
    if (creature)
      YellToAll(creature, GetAcoreString(LANG_BG_AV_H_CAPTAIN_DEAD),
                LANG_UNIVERSAL);
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player *player) {
  if (GetStatus() != STATUS_IN_PROGRESS)
    return; // maybe we should log this, cause this must be a cheater or a big
            // bug
        ...
        m_Team_QuestStatus[teamId][0] == 1500) // 25, 50, 75 turn ins
    {
      ...
      for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION;
        ...
        if (m_Nodes[i].OwnerId == player->GetTeamId() &&
          ...
          DePopulateNode(i);
          PopulateNode(i);
          // maybe this is bad, because it will instantly respawn all creatures
          // on every grave..
        }
    }
    break;
  ...
  case AV_QUEST_H_NEAR_MINE:
    ...
    if (m_Team_QuestStatus[teamId][5] == 28) {
      ...
      if (m_Team_QuestStatus[teamId][6] == 7)
        LOG_DEBUG("bg.battleground",
                  "BG_AV Quest %i completed (need to implement some events "
...
void BattlegroundAV::UpdateScore(TeamId teamId, int16 points) {
  ...
  if (points < 0) {
    if (m_Team_Scores[teamId] < 1) {
      ...
      SendMessageToAll(teamId == TEAM_HORDE ? LANG_BG_AV_H_NEAR_LOSE
                                            : LANG_BG_AV_A_NEAR_LOSE,
                       teamId == TEAM_HORDE ? CHAT_MSG_BG_SYSTEM_HORDE
...
Creature *BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type) {
  ...
  if (type >= AV_CPLACE_MAX) // static
  {
    ...
    creature = AddCreature(
        BG_AV_StaticCreatureInfo[cinfoid], type + AV_CPLACE_MAX,
        BG_AV_StaticCreaturePos[type][0], BG_AV_StaticCreaturePos[type][1],
        BG_AV_StaticCreaturePos[type][2], BG_AV_StaticCreaturePos[type][3]);
    isStatic = true;
  } else {
    creature =
        AddCreature(BG_AV_CreatureInfo[cinfoid], type,
                    BG_AV_CreaturePos[type][0], BG_AV_CreaturePos[type][1],
                    BG_AV_CreaturePos[type][2], BG_AV_CreaturePos[type][3]);
  }
  if (!creature)
    return nullptr;
  if (creature->GetEntry() == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN] ||
    ...
    creature->SetRespawnDelay(
        RESPAWN_ONE_DAY); // TODO: look if this can be done by database + also
                          // add this for the wingcommanders
  ...
  if ((isStatic && cinfoid >= 10 && cinfoid <= 14) ||
    ...
    if (!isStatic && ((cinfoid >= AV_NPC_A_GRAVEDEFENSE0 &&
                      ...
                      (cinfoid >= AV_NPC_H_GRAVEDEFENSE0 &&
                       cinfoid <= AV_NPC_H_GRAVEDEFENSE3))) {
      CreatureData &data =
          sObjectMgr->NewOrExistCreatureData(creature->GetDBTableGUIDLow());
  ...
  if (triggerSpawnID && newFaction) {
    if (Creature *trigger = AddCreature(WORLD_TRIGGER, triggerSpawnID,
  ...
  if (GetStatus() == STATUS_IN_PROGRESS) {
      ...
      if (m_CaptainBuffTimer[i] > diff)
        ...
        if (i == 0) {
          ...
          Creature *creature = GetBGCreature(
              AV_CPLACE_MAX + 59); // TODO: make the captains a dynamic creature
          if (creature)
        ...
        m_CaptainBuffTimer[i] =
            120000 +
            urand(0, 4) *
                60000; // as far as i could see, the buff is randomly so i make
                       // 2minutes (thats the duration of the buff itself) +
    ...
    for (uint8 mine = 0; mine < 2; mine++) {
      if (m_Mine_Owner[mine] == TEAM_ALLIANCE ||
        ...
        else // we don't need to set this timer to 0 cause this codepart wont
             // get called when this thing is 0
        {
    ...
    if (m_Mine_Timer <= 0)
      m_Mine_Timer = AV_MINE_TICK_TIMER; // this is at the end, cause we need to
                                         // update both mines

    // looks for all timers of the nodes and destroy the building (for
    // graveyards the building wont get destroyed, it goes just to the other team
    for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
...
void BattlegroundAV::EndBattleground(TeamId winnerTeamId) {
  ...
  for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE;
    ...
    if (rep[iTeamId] != 0)
      RewardReputationToTeam(iTeamId == TEAM_ALLIANCE ? 730 : 729, rep[iTeamId],
                             iTeamId);
    if (kills[iTeamId] != 0)
...
void BattlegroundAV::UpdatePlayerScore(Player *player, uint32 type,
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node) {
  ...
  if (IsTower(node)) {
    ...
    UpdateScore((ownerId == TEAM_ALLIANCE) ? TEAM_HORDE : TEAM_ALLIANCE,
                -1 * BG_AV_RES_TOWER);
    RewardReputationToTeam(ownerId == TEAM_ALLIANCE ? 730 : 729,
                           BG_AV_REP_TOWER, ownerId);
    RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_TOWER), ownerId);

    SpawnBGObject(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH + ownerId + (2 * tmp),
                  RESPAWN_ONE_DAY);
    SpawnBGObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH + ownerId + (2 * tmp),
                  RESPAWN_ONE_DAY);
  } else {
    ...
    SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                  RESPAWN_ONE_DAY);
    SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + ownerId + 3 * node,
                  RESPAWN_IMMEDIATELY);
    PopulateNode(node);
  ...
  char buf[256];
  if (IsTower(node))
    sprintf(buf, GetAcoreString(LANG_BG_AV_TOWER_TAKEN), GetNodeName(node),
            (ownerId == TEAM_ALLIANCE) ? GetAcoreString(LANG_BG_AV_ALLY)
                                       : GetAcoreString(LANG_BG_AV_HORDE));
  else
    sprintf(buf, GetAcoreString(LANG_BG_AV_GRAVE_TAKEN), GetNodeName(node),
            (ownerId == TEAM_ALLIANCE) ? GetAcoreString(LANG_BG_AV_ALLY)
                                       : GetAcoreString(LANG_BG_AV_HORDE));
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, TeamId teamId, bool initial) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("bg.battleground", "bg_av populating mine %i (0=north, 1=south)",
            mine);
#endif
  uint16 miner;
  ...
  AddAVCreature(miner + 3, (mine == AV_NORTH_MINE) ? AV_CPLACE_MINE_N_3
                                                   : AV_CPLACE_MINE_S_3);

  if (teamId == TEAM_ALLIANCE || teamId == TEAM_HORDE) {
    m_Mine_Reclaim_Timer[mine] = AV_MINE_RECLAIM_TIMER;
    char buf[256];
    sprintf(buf, GetAcoreString(LANG_BG_AV_MINE_TAKEN),
            GetAcoreString((mine == AV_NORTH_MINE) ? LANG_BG_AV_MINE_NORTH
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node) {
  ...
  if (IsTower(node))
    ...
    if (!AddSpiritGuide(node, BG_AV_CreaturePos[node][0],
                        ...
                        BG_AV_CreaturePos[node][3], ownerId))
  ...
  if (node >= BG_AV_NODES_MAX) // fail safe
    return;
  Creature *trigger =
      GetBgMap()->GetCreature(BgCreatures[node + 302]); // 0-302 other creatures
  if (!trigger) {
    trigger = AddCreature(
        WORLD_TRIGGER, node + 302, BG_AV_CreaturePos[node + 302][0],
        BG_AV_CreaturePos[node + 302][1], BG_AV_CreaturePos[node + 302][2],
...
void BattlegroundAV::EventPlayerDefendsPoint(Player *player, uint32 object) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("bg.battleground", "player defends point object: %i node: %i",
            object, node);
#endif
  ...
  if (!IsTower(node)) {
    SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                  RESPAWN_ONE_DAY);
    SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + teamId + 3 * node,
                  RESPAWN_IMMEDIATELY);
  }
  // despawn old go
  SpawnBGObject(object, RESPAWN_ONE_DAY);
  ...
  } else if (node == BG_AV_NODES_SNOWFALL_GRAVE) // snowfall eyecandy
  {
    for (uint8 i = 0; i < 4; i++) {
      SpawnBGObject(((ownerId == TEAM_ALLIANCE)
                        ...
                        i,
                    RESPAWN_ONE_DAY);
      SpawnBGObject(((teamId == TEAM_ALLIANCE) ? BG_AV_OBJECT_SNOW_EYECANDY_A
                                               : BG_AV_OBJECT_SNOW_EYECANDY_H) +
                        i,
                    RESPAWN_IMMEDIATELY);
    }
  ...
  UpdatePlayerScore(
      player,
      (IsTower(node)) ? SCORE_TOWERS_DEFENDED : SCORE_GRAVEYARDS_DEFENDED, 1);
  if (IsTower(node))
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player *player, uint32 object) {
  ...
  TeamId teamId = player->GetTeamId();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("bg.battleground", "bg_av: player assaults point object %i node %i",
            object, node);
#endif
  ...
  if (m_Nodes[node].TotalOwnerId != TEAM_NEUTRAL) {
    ...
    if (IsTower(node)) {
      ...
      SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION + 3 * node,
                    RESPAWN_IMMEDIATELY); // neutral aura spawn
      SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION + prevOwnerId +
                        3 * node,
  ...
  UpdatePlayerScore(
      player,
      (IsTower(node)) ? SCORE_TOWERS_ASSAULTED : SCORE_GRAVEYARDS_ASSAULTED, 1);
...
void BattlegroundAV::FillInitialWorldStates(WorldPacket &data) {
  ...
  for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT;
    ...
    for (uint8 j = 1; j <= 3; j += 2) {
      ...
      data << uint32(
                  BG_AV_NodeWorldStates[i][GetWorldStateType(j, TEAM_ALLIANCE)])
  ...
  for (uint8 i = BG_AV_NODES_DUNBALDAR_SOUTH; i < BG_AV_NODES_MAX; ++i)
    for (uint8 j = 1; j <= 3; j += 2) {
      // j=1=assaulted j=3=controled //i dont have j=2=destroyed cause destroyed
      // is the same like enemy-team controll
      stateok = (m_Nodes[i].State == j ||
                   ...
                   // fit good into the worldstatesarray
{
  ...
  LOG_ERROR("server",
            "BG_AV: should update a strange worldstate state:%i team:%i", state,
            teamId);
...
void BattlegroundAV::SendMineWorldStates(uint32 mine) {
  ...
  for (uint8 i = 0; i < 3; ++i)
    UpdateWorldState(BG_AV_MineWorldStates[mine][i],
...
GraveyardStruct const *BattlegroundAV::GetClosestGraveyard(Player *player) {
  ...
  for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT;
    ...
    if (m_Nodes[i].OwnerId == player->GetTeamId() &&
      ...
      if (entry) {
        dist =
            (entry->x - x) * (entry->x - x) + (entry->y - y) * (entry->y - y);
        if (dist < minDist) {
          minDist = dist;
          pGraveyard = entry;
        }
...
bool BattlegroundAV::SetupBattleground() {
  ...
  for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i) {
    if (i <= BG_AV_NODES_FROSTWOLF_HUT) {
      if (!AddObject(i, BG_AV_OBJECTID_BANNER_A_B, BG_AV_ObjectPos[i][0],
          ...
          !AddObject(i + 11, BG_AV_OBJECTID_BANNER_CONT_A_B,
                     ...
                     sin(BG_AV_ObjectPos[i][3] / 2),
                     cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY) ||
          !AddObject(i + 33, BG_AV_OBJECTID_BANNER_H_B, BG_AV_ObjectPos[i][0],
                     BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
                     BG_AV_ObjectPos[i][3], 0, 0,
                     sin(BG_AV_ObjectPos[i][3] / 2),
                     cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY) ||
          ...
          !AddObject(BG_AV_OBJECT_AURA_H_FIRSTAID_STATION + i * 3,
                     ...
                     sin(BG_AV_ObjectPos[i][3] / 2),
                     cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY)) {
        LOG_ERROR("server", "BatteGroundAV: Failed to spawn some object "
    ...
    } else // towers
    {
      ...
      {
        if (!AddObject(i, BG_AV_OBJECTID_BANNER_A, BG_AV_ObjectPos[i][0],
                       BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
                       BG_AV_ObjectPos[i][3], 0, 0,
                       sin(BG_AV_ObjectPos[i][3] / 2),
                       cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY) ||
            ...
            !AddObject(BG_AV_OBJECT_TAURA_A_DUNBALDAR_SOUTH +
                       ...
                       BG_AV_OBJECTID_AURA_A, BG_AV_ObjectPos[i + 8][0],
                       BG_AV_ObjectPos[i + 8][1], BG_AV_ObjectPos[i + 8][2],
                       BG_AV_ObjectPos[i + 8][3], 0, 0,
                       sin(BG_AV_ObjectPos[i + 8][3] / 2),
                       cos(BG_AV_ObjectPos[i + 8][3] / 2), RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_AURA_N, BG_AV_ObjectPos[i + 8][0],
                       BG_AV_ObjectPos[i + 8][1], BG_AV_ObjectPos[i + 8][2],
                       BG_AV_ObjectPos[i + 8][3], 0, 0,
                       sin(BG_AV_ObjectPos[i + 8][3] / 2),
                       cos(BG_AV_ObjectPos[i + 8][3] / 2), RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TFLAG_A_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_TOWER_BANNER_A, BG_AV_ObjectPos[i + 8][0],
                       BG_AV_ObjectPos[i + 8][1], BG_AV_ObjectPos[i + 8][2],
                       BG_AV_ObjectPos[i + 8][3], 0, 0,
                       sin(BG_AV_ObjectPos[i + 8][3] / 2),
                       cos(BG_AV_ObjectPos[i + 8][3] / 2), RESPAWN_ONE_DAY) ||
            !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH +
                       ...
                       BG_AV_ObjectPos[i + 8][0], BG_AV_ObjectPos[i + 8][1],
                       BG_AV_ObjectPos[i + 8][2], BG_AV_ObjectPos[i + 8][3], 0,
                       0, sin(BG_AV_ObjectPos[i + 8][3] / 2),
                       cos(BG_AV_ObjectPos[i + 8][3] / 2), RESPAWN_ONE_DAY)) {
          LOG_ERROR("server", "BatteGroundAV: Failed to spawn some object "
      ...
      } else // horde towers
      {
        if (!AddObject(i + 7, BG_AV_OBJECTID_BANNER_CONT_A,
                       ...
                       sin(BG_AV_ObjectPos[i][3] / 2),
                       cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY) ||
            !AddObject(i + 29, BG_AV_OBJECTID_BANNER_H, BG_AV_ObjectPos[i][0],
                       BG_AV_ObjectPos[i][1], BG_AV_ObjectPos[i][2],
                       BG_AV_ObjectPos[i][3], 0, 0,
                       sin(BG_AV_ObjectPos[i][3] / 2),
                       cos(BG_AV_ObjectPos[i][3] / 2), RESPAWN_ONE_DAY) ||
            ...
            !AddObject(BG_AV_OBJECT_TAURA_H_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_AURA_H, BG_AV_ObjectPos[i + 8][0],
                       BG_AV_ObjectPos[i + 8][1], BG_AV_ObjectPos[i + 8][2],
                       BG_AV_ObjectPos[i + 8][3], 0, 0,
                       sin(BG_AV_ObjectPos[i + 8][3] / 2),
                       cos(BG_AV_ObjectPos[i + 8][3] / 2), RESPAWN_ONE_DAY) ||
            ...
            !AddObject(BG_AV_OBJECT_TFLAG_H_DUNBALDAR_SOUTH +
                           (2 * (i - BG_AV_NODES_DUNBALDAR_SOUTH)),
                       BG_AV_OBJECTID_TOWER_BANNER_H, BG_AV_ObjectPos[i + 8][0],
                       BG_AV_ObjectPos[i + 8][1], BG_AV_ObjectPos[i + 8][2],
                       BG_AV_ObjectPos[i + 8][3], 0, 0,
                       sin(BG_AV_ObjectPos[i + 8][3] / 2),
                       cos(BG_AV_ObjectPos[i + 8][3] / 2), RESPAWN_ONE_DAY)) {
      ...
      for (uint8 j = 0; j <= 9; j++) // burning aura
      {
        if (!AddObject(
                ...
                BG_AV_ObjectPos[AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                               ...
                               [3],
                0, 0,
                sin(BG_AV_ObjectPos[AV_OPLACE_BURN_DUNBALDAR_SOUTH +
                                    ((i - BG_AV_NODES_DUNBALDAR_SOUTH) * 10) +
                                    j][3] /
                    2),
    ...
    for (uint8 j = 0; j <= 9; j++) {
      if (j < 5) {
        if (!AddObject(
                ...
                BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) + j][3], 0,
                0,
                sin(BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) + j]
                                   [3] /
                    2),
                cos(BG_AV_ObjectPos[AV_OPLACE_BURN_BUILDING_A + (i * 10) + j]
                                   [3] /
                    2),
                RESPAWN_ONE_DAY)) {
          LOG_ERROR("server",
  ...
  for (uint16 i = 0;
       ...
       i++) {
    if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_N_MIN + i, BG_AV_OBJECTID_MINE_N,
      ...
      LOG_ERROR("server",
                ...
                "Battleground not created!7.5.%i",
                i);
      return false;
    }
  }
  for (uint16 i = 0;
       ...
       i++) {
    if (!AddObject(BG_AV_OBJECT_MINE_SUPPLY_S_MIN + i, BG_AV_OBJECTID_MINE_S,
      ...
      LOG_ERROR("server",
                "BatteGroundAV: Failed to spawn some mine supplies "
                "Battleground not created!7.6.%i",
                i);
      return false;
    }
  }
  ...
  if (IsHolidayActive(HOLIDAY_FEAST_OF_WINTER_VEIL))
    for (uint16 i = 0; i <= (BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MAX -
      ...
      if (!AddObject(
              ...
              BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][3], 0, 0,
              sin(BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][3] /
                  2),
              cos(BG_AV_ObjectPos[AV_OPLACE_HANDPACKED_SNOWDRIFT_MIN + i][3] /
                  2),
              RESPAWN_ONE_DAY))
        return false;
    }
  ...
  // Quest banners
  if (!AddObject(BG_AV_OBJECT_FROSTWOLF_BANNER, BG_AV_OBJECTID_FROSTWOLF_BANNER,
                 ...
                 BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][3], 0, 0,
                 sin(BG_AV_ObjectPos[AV_OPLACE_FROSTWOLF_BANNER][3] / 2),
  ...
  if (!AddObject(BG_AV_OBJECT_STORMPIKE_BANNER, BG_AV_OBJECTID_STORMPIKE_BANNER,
                 ...
                 BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][3], 0, 0,
                 sin(BG_AV_ObjectPos[AV_OPLACE_STORMPIKE_BANNER][3] / 2),
  ...
  if (IsHolidayActive(HOLIDAY_FEAST_OF_WINTER_VEIL))
    for (i = BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MIN;
         i <= BG_AV_OBJECT_HANDPACKED_SNOWDRIFT_MAX; i++)
      SpawnBGObject(i, RESPAWN_IMMEDIATELY);
  ...
  AddSpiritGuide(7, BG_AV_CreaturePos[7][0], BG_AV_CreaturePos[7][1],
                 BG_AV_CreaturePos[7][2], BG_AV_CreaturePos[7][3],
                 TEAM_ALLIANCE);
  AddSpiritGuide(8, BG_AV_CreaturePos[8][0], BG_AV_CreaturePos[8][1],
                 BG_AV_CreaturePos[8][2], BG_AV_CreaturePos[8][3], TEAM_HORDE);
  ...
  if (
    ...
    LOG_ERROR("sql.sql", "BatteGroundAV: Failed to spawn some object "
                         "Battleground not created!1");
    return false;
  }
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, TeamId teamId) {
  ...
  if (m_Nodes[node].State == POINT_ASSAULTED &&
      m_Nodes[node].TotalOwnerId != TEAM_NEUTRAL) // only assault an assaulted
                                                  // node if no totalowner exists
  {
...
void BattlegroundAV::ResetBGSubclass() {
  for (uint8 i = 0; i < 2;
       i++) // forloop for both teams (it just make 0 == alliance and 1 == horde
            // also for both mines 0=north 1=south
  {
    for (uint8 j = 0; j < 9; j++)
      m_Team_QuestStatus[i][j] = 0;
    ...
    m_CaptainBuffTimer[i] =
        120000 +
        urand(0, 4) * 60; // as far as i could see, the buff is randomly so i
                          // make 2minutes (thats the duration of the buff
  ...
  InitNode(BG_AV_NODES_SNOWFALL_GRAVE, TEAM_NEUTRAL,
...
bool BattlegroundAV::IsAllTowersControlledAndCaptainAlive(TeamId teamId) const {
  if (teamId == TEAM_ALLIANCE) {
    for (BG_AV_Nodes i = BG_AV_NODES_DUNBALDAR_SOUTH;
         i <= BG_AV_NODES_STONEHEART_BUNKER; ++i) // alliance towers controlled
    {
      if (m_Nodes[i].State == POINT_CONTROLED) {
        if (m_Nodes[i].OwnerId != TEAM_ALLIANCE)
          return false;
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_ICEBLOOD_TOWER;
         i <= BG_AV_NODES_FROSTWOLF_WTOWER; ++i) // horde towers destroyed
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_ICEBLOOD_TOWER;
         i <= BG_AV_NODES_FROSTWOLF_WTOWER; ++i) // horde towers controlled
    {
      if (m_Nodes[i].State == POINT_CONTROLED) {
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_DUNBALDAR_SOUTH;
         i <= BG_AV_NODES_STONEHEART_BUNKER; ++i) // alliance towers destroyed
      if (m_Nodes[i].State != POINT_DESTROYED)
        return false;
