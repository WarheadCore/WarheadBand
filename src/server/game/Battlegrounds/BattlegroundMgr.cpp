...
BattlegroundMgr::BattlegroundMgr()
    : m_ArenaTesting(false), m_Testing(false), m_lastClientVisibleInstanceId(0),
      m_NextAutoDistributionTime(0), m_AutoDistributionTimeChecker(0),
      m_NextPeriodicQueueUpdateTime(5 * IN_MILLISECONDS) {
  for (uint32 qtype = BATTLEGROUND_QUEUE_NONE;
       qtype < MAX_BATTLEGROUND_QUEUE_TYPES; ++qtype)
    m_BattlegroundQueues[qtype].SetBgTypeIdAndArenaType(
        BGTemplateId(BattlegroundQueueTypeId(qtype)),
        BGArenaType(BattlegroundQueueTypeId(qtype)));
}
...
void BattlegroundMgr::Update(uint32 diff) {
  ...
  for (BattlegroundContainer::iterator itr = m_Battlegrounds.begin(), itrDelete;
       itr != m_Battlegrounds.end();) {
    itrDelete = itr++;
  ...
  // update events
  for (int qtype = BATTLEGROUND_QUEUE_NONE;
       qtype < MAX_BATTLEGROUND_QUEUE_TYPES; ++qtype)
    m_BattlegroundQueues[qtype].UpdateEvents(diff);

  // update using scheduled tasks (used only for rated arenas, initial opponent
  // search works differently than periodic queue update)
  if (!m_ArenaQueueUpdateScheduler.empty()) {
    ...
    for (uint8 i = 0; i < scheduled.size(); i++) {
      ...
      m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(
          bracket_id, true, arenaRatedTeamId); // pussywizard: looking for
                                               // opponents only for this team
    }
  }
  ...
  if (m_NextPeriodicQueueUpdateTime < diff) {
    ...
    for (uint32 qtype = BATTLEGROUND_QUEUE_2v2;
      ...
      for (uint32 bracket = BG_BRACKET_ID_FIRST;
           bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
        m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(
            BattlegroundBracketId(bracket), true,
            0); // pussywizard: 0 for rated means looking for opponents for
                // every team

    // for battlegrounds and not rated arenas
    // in first loop try to fill already running battlegrounds, then in a second
    // loop try to create new battlegrounds
    for (uint32 qtype = BATTLEGROUND_QUEUE_AV;
         qtype < MAX_BATTLEGROUND_QUEUE_TYPES; ++qtype)
      for (uint32 bracket = BG_BRACKET_ID_FIRST;
           bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
        m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(
            BattlegroundBracketId(bracket), false, 0);
...
void BattlegroundMgr::BuildBattlegroundStatusPacket(
    WorldPacket *data, Battleground *bg, uint8 QueueSlot, uint8 StatusID,
    uint32 Time1, uint32 Time2, uint8 arenatype, TeamId teamId, bool isRated,
  ...
  *data << uint32(forceBgTypeId != BATTLEGROUND_TYPE_NONE ? forceBgTypeId
                                                          : bg->GetBgTypeID());
  *data << uint16(0x1F90);
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket *data,
                                            Battleground *bg) {
  uint8 type = (bg->isArena() ? 1 : 0);

  data->Initialize(MSG_PVP_LOG_DATA,
                   (1 + 1 + 4 + 40 * bg->GetPlayerScoresSize()));
  *data << uint8(type); // type (battleground=0/arena=1)
  ...
  if (type) // arena
  {
    ...
    for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE;
      ...
      // Xinef: oryginally this was looping in reverse order, loop order was
      // changed so we have to change checked teamId
      int32 rating_change = bg->GetArenaTeamRatingChangeForTeam(
          Battleground::GetOtherTeamId(iTeamId));

      uint32 pointsLost = rating_change < 0 ? -rating_change : 0;
      uint32 pointsGained = rating_change > 0 ? rating_change : 0;
      uint32 MatchmakerRating =
          bg->GetArenaMatchmakerRating(Battleground::GetOtherTeamId(iTeamId));
    ...
    for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE;
      ...
      if (ArenaTeam *at = sArenaTeamMgr->GetArenaTeamById(
              bg->GetArenaTeamIdForTeam(Battleground::GetOtherTeamId(iTeamId))))
        *data << at->GetName();
      else
        *data << uint8(0);
    }
  ...
  *data << uint32(scoreCount); // placeholder
  ...
  Battleground::BattlegroundScoreMap::const_iterator itr2 =
      bg->GetPlayerScoresBegin();
  for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2;
       itr != bg->GetPlayerScoresEnd();) {
    itr2 = itr++;
    if (!bg->IsPlayerInBattleground(itr2->first)) {
      LOG_ERROR("server",
                "Player " UI64FMTD " has scoreboard entry for battleground %u "
                                   "but is not in battleground!",
                itr->first, bg->GetBgTypeID());
      continue;
    }
    ...
    *data << uint32(itr2->second->KillingBlows);
    if (type == 0) {
      ...
      *data << uint8(itr2->second->player->GetBgTeamId() == TEAM_ALLIANCE
                         ? 1
                         : 0); // green or yellow
    }
    ...
    *data << uint32(itr2->second->HealingDone); // healing done
    ...
    case BATTLEGROUND_RB:
      ...
      case 489:
        ...
        *data << uint32(((BattlegroundWGScore *)itr2->second)
                            ->FlagCaptures); // flag captures
        *data << uint32(
            ((BattlegroundWGScore *)itr2->second)->FlagReturns); // flag returns
        break;
      case 566:
        *data << uint32(0x00000001); // count of next fields
        *data << uint32(((BattlegroundEYScore *)itr2->second)
                            ->FlagCaptures); // flag captures
        break;
      case 529:
        *data << uint32(0x00000002); // count of next fields
        *data << uint32(((BattlegroundABScore *)itr2->second)
      ...
      case 30:
        ...
        *data << uint32(((BattlegroundAVScore *)itr2->second)
        ...
        *data << uint32(((BattlegroundAVScore *)itr2->second)
                            ->MinesCaptured); // MinesCaptured
        break;
      case 607:
        *data << uint32(0x00000002); // count of next fields
        *data << uint32(
            ((BattlegroundSAScore *)itr2->second)->demolishers_destroyed);
        *data << uint32(((BattlegroundSAScore *)itr2->second)->gates_destroyed);
        break;
      case 628:                      // IC
        *data << uint32(0x00000002); // count of next fields
        *data << uint32(((BattlegroundICScore *)itr2->second)
                            ->BasesAssaulted); // bases asssulted
        *data << uint32(((BattlegroundICScore *)itr2->second)
                            ->BasesDefended); // bases defended
      ...
      default:
        if (BattlegroundMgr::getBgFromMap.find(bg->GetMapId()) !=
            BattlegroundMgr::getBgFromMap.end())
    ...
    case BATTLEGROUND_WS:
      *data << uint32(0x00000002); // count of next fields
      *data << uint32(
          ((BattlegroundWGScore *)itr2->second)->FlagCaptures); // flag captures
      *data << uint32(
          ((BattlegroundWGScore *)itr2->second)->FlagReturns); // flag returns
      break;
    ...
    case BATTLEGROUND_EY:
      *data << uint32(0x00000001); // count of next fields
      *data << uint32(
          ((BattlegroundEYScore *)itr2->second)->FlagCaptures); // flag captures
      break;
    ...
    case BATTLEGROUND_RV:
      *data << uint32(0);
      break;
    default:
      if (BattlegroundMgr::getBgFromTypeID.find(bgTypeId) !=
          BattlegroundMgr::getBgFromTypeID.end())
    ...
    // should never happen
    if (++scoreCount >= bg->GetMaxPlayersPerTeam() * 2 &&
        itr != bg->GetPlayerScoresEnd()) {
      LOG_INFO("misc",
               "Battleground %u scoreboard has more entries (%u) than allowed "
               "players in this bg (%u)",
               bgTypeId, bg->GetPlayerScoresSize(),
               bg->GetMaxPlayersPerTeam() * 2);
      break;
    }
  }
...
BattlegroundMgr::GetBattlegroundTemplate(BattlegroundTypeId bgTypeId) {
  BattlegroundTemplateContainer::const_iterator itr =
      m_BattlegroundTemplates.find(bgTypeId);
...
BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId,
  ...
  bg->SetClientInstanceID(
      IsArenaType(originalBgTypeId) ? 0 : GetNextClientVisibleInstanceId());
  bg->Init();
...
bool BattlegroundMgr::CreateBattleground(CreateBattlegroundData &data) {
  ...
  bg->SetTeamStartLoc(TEAM_ALLIANCE, data.Team1StartLocX, data.Team1StartLocY,
                      data.Team1StartLocZ, data.Team1StartLocO);
  bg->SetTeamStartLoc(TEAM_HORDE, data.Team2StartLocX, data.Team2StartLocY,
                      data.Team2StartLocZ, data.Team2StartLocO);
...
void BattlegroundMgr::CreateInitialBattlegrounds() {
  ...
  if (!result) {
    LOG_ERROR("server", ">> Loaded 0 battlegrounds. DB table "
                        "`battleground_template` is empty.");
    return;
  }
  ...
  do {
    ...
    if (!bl) {
      LOG_ERROR("server",
                "Battleground ID %u not found in BattlemasterList.dbc. "
    ...
    if (data.MaxPlayersPerTeam == 0 ||
        data.MinPlayersPerTeam > data.MaxPlayersPerTeam) {
      LOG_ERROR("server",
                "Table `battleground_template` for id %u has bad values for "
                "MinPlayersPerTeam (%u) and MaxPlayersPerTeam(%u)",
                data.bgTypeId, data.MinPlayersPerTeam, data.MaxPlayersPerTeam);
      continue;
    }

    if (data.LevelMin == 0 || data.LevelMax == 0 ||
        data.LevelMin > data.LevelMax) {
      LOG_ERROR("server",
                "Table `battleground_template` for id %u has bad values for "
                "LevelMin (%u) and LevelMax(%u)",
    ...
    if (data.bgTypeId == BATTLEGROUND_AA || data.bgTypeId == BATTLEGROUND_RB) {
      ...
      if (GraveyardStruct const *start = sGraveyard->GetGraveyard(startId)) {
        ...
        LOG_ERROR("server",
                  "Table `battleground_template` for id %u have non-existed "
                  "`game_graveyard` table id %u in field `AllianceStartLoc`. "
                  "BG not created.",
                  data.bgTypeId, startId);
        continue;
      }
      ...
      if (GraveyardStruct const *start = sGraveyard->GetGraveyard(startId)) {
        ...
        LOG_ERROR("server",
                  "Table `battleground_template` for id %u have non-existed "
                  "`game_graveyard` table id %u in field `HordeStartLoc`. BG "
                  "not created.",
                  data.bgTypeId, startId);
        continue;
      }
    ...
    if (bl->mapid[1] == -1) // in this case we have only one mapId
      _battlegroundMapTemplates[bl->mapid[0]] =
          &_battlegroundTemplates[BattlegroundTypeId(bgTypeId)];

    ++count;
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %u battlegrounds in %u ms", count,
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution() {
  ...
  LOG_INFO("server", "AzerothCore Battleground: Initializing Automatic Arena "
                     "Point Distribution");
  if (wstime < curtime) {
    m_NextAutoDistributionTime =
        curtime; // reset will be called in the next update
    LOG_INFO("server", "AzerothCore Battleground: Next arena point "
                       "distribution time in the past, reseting it now.");
  } else
    m_NextAutoDistributionTime = wstime;
  LOG_INFO("server", "AzerothCore Battleground: Automatic Arena Point "
                     "Distribution initialized.");
}

void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket *data,
                                                  uint64 guid, Player *player,
  ...
  winner_kills = Warhead::Honor::hk_honor_at_level(player->getLevel(),
                                                   float(winner_kills));
  loser_kills =
      Warhead::Honor::hk_honor_at_level(player->getLevel(), float(loser_kills));
    ...
    *data << uint32(0); // number of bg instances
    ...
    if (Battleground *bgt = GetBattlegroundTemplate(bgTypeId))
      if (GetBattlegroundBracketByLevel(bgt->GetMapId(), player->getLevel())) {
        uint32 count = 0;
        /*for (BattlegroundClientIdsContainer::const_iterator itr =
        clientIds.begin(); itr != clientIds.end(); ++itr)
        {
            *data << uint32(*itr);
            ++count;
        }*/
...
void BattlegroundMgr::SendToBattleground(Player *player, uint32 instanceId,
                                         BattlegroundTypeId /*bgTypeId*/) {
...
void BattlegroundMgr::ScheduleArenaQueueUpdate(
    uint32 arenaRatedTeamId, BattlegroundQueueTypeId bgQueueTypeId,
    BattlegroundBracketId bracket_id) {
  uint64 const scheduleId =
      ((uint64)arenaRatedTeamId << 32) | (bgQueueTypeId << 16) | bracket_id;
  if (std::find(m_ArenaQueueUpdateScheduler.begin(),
                m_ArenaQueueUpdateScheduler.end(),
                scheduleId) == m_ArenaQueueUpdateScheduler.end())
...
void BattlegroundMgr::LoadBattleMastersEntry() {
  ...
  if (!result) {
    LOG_INFO("server", ">> Loaded 0 battlemaster entries. DB table "
  ...
  do {
    ...
    if (CreatureTemplate const *cInfo =
      ...
      if ((cInfo->npcflag & UNIT_NPC_FLAG_BATTLEMASTER) == 0)
        LOG_ERROR("sql.sql",
                  "Creature (Entry: %u) listed in `battlemaster_entry` is not "
                  "a battlemaster.",
                  entry);
    } else {
      LOG_ERROR(
          "sql.sql",
          "Creature (Entry: %u) listed in `battlemaster_entry` does not exist.",
          entry);
      continue;
    }
    ...
    if (!sBattlemasterListStore.LookupEntry(bgTypeId)) {
      LOG_ERROR("sql.sql",
                "Table `battlemaster_entry` contain entry %u for not existed "
  ...
  LOG_INFO("server", ">> Loaded %u battlemaster entries in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
...
void BattlegroundMgr::CheckBattleMasters() {
  ...
  for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
    ...
    if ((itr->second.npcflag & UNIT_NPC_FLAG_BATTLEMASTER) &&
      ...
      LOG_ERROR("sql.sql",
                "CreatureTemplate (Entry: %u) has UNIT_NPC_FLAG_BATTLEMASTER "
...
BattlegroundTypeId
BattlegroundMgr::WeekendHolidayIdToBGType(HolidayIds holiday) {
  switch (holiday) {
...
BattlegroundTypeId BattlegroundMgr::GetRandomBG(BattlegroundTypeId bgTypeId) {
  if (GetBattlegroundTemplateByTypeId(bgTypeId)) {
    ...
    for (int32 mapId : bl->mapid) {
      ...
      if (CreateBattlegroundData const *bg =
    ...
    return Warhead::Containers::SelectRandomWeightedContainerElement(ids,
                                                                     weights);
  }
...
void BattlegroundMgr::InviteGroupToBG(GroupQueueInfo *ginfo, Battleground *bg,
                                      TeamId teamId) {
  ...
  for (auto itr : ginfo->Players) {
    ...
    BGQueueInviteEvent *inviteEvent = new BGQueueInviteEvent(
        player->GetGUID(), ginfo->IsInvitedToBGInstanceGUID, ginfo->BgTypeId,
    ...
    BGQueueRemoveEvent *removeEvent = new BGQueueRemoveEvent(
        player->GetGUID(), ginfo->IsInvitedToBGInstanceGUID, bgQueueTypeId,
    ...
    sBattlegroundMgr->BuildBattlegroundStatusPacket(
        ...
        ginfo->ArenaType, TEAM_NEUTRAL, bg->isRated(), ginfo->BgTypeId);
    player->GetSession()->SendPacket(&data);
    ...
    if (bg->isArena() && bg->isRated())
      bg->ArenaLogEntries[player->GetGUID()].Fill(
          ...
          player->GetSession()->GetAccountId(), ginfo->ArenaTeamId,
