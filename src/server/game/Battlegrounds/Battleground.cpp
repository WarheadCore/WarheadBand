...
public:
  BattlegroundChatBuilder(ChatMsg msgtype, uint32 textId, Player const *source,
...
private:
  void do_helper(WorldPacket &data, char const *text) {
    ChatHandler::BuildChatPacket(data, _msgtype, LANG_UNIVERSAL, _source,
                                 _source, text);
  }
...
public:
  Battleground2ChatBuilder(ChatMsg msgtype, uint32 textId, Player const *source,
...
Battleground::~Battleground() {
  ...
  for (BattlegroundScoreMap::const_iterator itr = PlayerScores.begin();
       itr != PlayerScores.end(); ++itr)
    delete itr->second;
}
...
void Battleground::Update(uint32 diff) {
  ...
  case STATUS_IN_PROGRESS:
    if (isArena()) {
      if (GetStartTime() >=
          46 * MINUTE * IN_MILLISECONDS) // pussywizard: 1min startup + 45min
                                         // allowed duration
      {
    ...
    for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin();
      ...
      if (pos.GetExactDistSq(x, y, z) > maxDist) {
        ...
        LOG_DEBUG("bg.battleground",
                  "BATTLEGROUND: Sending %s back to start location (map: %u) "
                  "(possible exploit)",
                  itr->second->GetName().c_str(), GetMapId());
#endif
...
inline void Battleground::_ProcessResurrect(uint32 diff) {
  ...
  if (m_LastResurrectTime >= RESURRECTION_INTERVAL) {
    if (GetReviveQueueSize()) {
      for (std::map<uint64, std::vector<uint64>>::iterator itr =
               m_ReviveQueue.begin();
           itr != m_ReviveQueue.end(); ++itr) {
        Creature *sh = nullptr;
        for (std::vector<uint64>::const_iterator itr2 = (itr->second).begin();
      ...
      m_LastResurrectTime = 0;
             ...
             500) // Resurrect players only half a second later, to see spirit
                  // heal effect on NPC
  {
    for (std::vector<uint64>::const_iterator itr = m_ResurrectQueue.begin();
         itr != m_ResurrectQueue.end(); ++itr) {
  ...
  if (!m_PrematureCountDown) {
    ...
    if (newtime > (MINUTE * IN_MILLISECONDS)) {
      if (newtime / (MINUTE * IN_MILLISECONDS) !=
        ...
        PSendMessageToAll(
            LANG_BATTLEGROUND_PREMATURE_FINISH_WARNING, CHAT_MSG_SYSTEM,
            nullptr,
      ...
      if (newtime / (15 * IN_MILLISECONDS) !=
        ...
        PSendMessageToAll(
            LANG_BATTLEGROUND_PREMATURE_FINISH_WARNING_SECS, CHAT_MSG_SYSTEM,
            nullptr, (uint32)(m_PrematureCountDownTimer / IN_MILLISECONDS));
    }
...
inline void Battleground::_ProcessJoin(uint32 diff) {
  ...
  if (!(m_Events & BG_STARTING_EVENT_1)) {
    ...
    if (!FindBgMap()) {
      LOG_ERROR("server",
                "Battleground::_ProcessJoin: map (map id: %u, instance id: %u) "
                "is not created!",
    ...
    SendMessageToAll(StartMessageIds[BG_STARTING_EVENT_FIRST],
  ...
  // After 1 minute or 30 seconds, warning is signaled
    ...
    m_Events |= BG_STARTING_EVENT_2;
    SendMessageToAll(StartMessageIds[BG_STARTING_EVENT_SECOND],
                     CHAT_MSG_BG_SYSTEM_NEUTRAL);
  }
  // After 30 or 15 seconds, warning is signaled
    ...
    m_Events |= BG_STARTING_EVENT_3;
    SendMessageToAll(StartMessageIds[BG_STARTING_EVENT_THIRD],
                     CHAT_MSG_BG_SYSTEM_NEUTRAL);
  ...
  // Delay expired (after 2 or 1 minute)
    ...
    // Remove preparation
    if (isArena()) {
      // pussywizard: initial visibility range is 30yd, set it to a proper value
      // now:
      ...
      for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin();
        ...
        if (Player *player = itr->second) {
          ...
          sBattlegroundMgr->BuildBattlegroundStatusPacket(
              &status, this, player->GetCurrentBattlegroundQueueSlot(),
              STATUS_IN_PROGRESS, 0, GetStartTime(), GetArenaType(),
              player->GetBgTeamId());
          ...
          for (Unit::AuraApplicationMap::iterator iter = auraMap.begin();
               iter != auraMap.end();) {
            ...
            if (!aura->IsPermanent() &&
                ...
                // Xinef: bullshit condition, ALL buffs should be removed
      ...
      if (GetStatus() == STATUS_IN_PROGRESS) {
        for (ToBeTeleportedMap::const_iterator itr = m_ToBeTeleported.begin();
             itr != m_ToBeTeleported.end(); ++itr)
          if (Player *p = ObjectAccessor::GetObjectInOrOutOfWorld(
                  itr->first, (Player *)nullptr))
            if (Player *t = ObjectAccessor::FindPlayer(itr->second)) {
              if (!t->FindMap() || t->FindMap() != GetBgMap())
                continue;

              p->SetSummonPoint(t->GetMapId(), t->GetPositionX(),
                                t->GetPositionY(), t->GetPositionZ(), 15, true);
...
void Battleground::SetTeamStartLoc(TeamId teamId, float X, float Y, float Z,
                                   float O) {
...
void Battleground::SendPacketToTeam(TeamId teamId, WorldPacket *packet,
                                    Player *sender, bool self) {
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
       itr != m_Players.end(); ++itr)
...
void Battleground::YellToAll(Creature *creature, char const *text,
                             uint32 language) {
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
       itr != m_Players.end(); ++itr) {
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_YELL,
                                 Language(language), creature, itr->second,
                                 text);
...
void Battleground::RewardReputationToTeam(uint32 factionId, uint32 reputation,
                                          TeamId teamId) {
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
       itr != m_Players.end(); ++itr)
    if (itr->second->GetBgTeamId() == teamId) {
      ...
      AddPct(repGain,
             itr->second->GetTotalAuraModifier(SPELL_AURA_MOD_REPUTATION_GAIN));
      AddPct(repGain,
...
void Battleground::UpdateWorldStateForPlayer(uint32 Field, uint32 Value,
                                             Player *player) {
  WorldPacket data;
  sBattlegroundMgr->BuildUpdateWorldStatePacket(&data, Field, Value);
...
void Battleground::EndBattleground(TeamId winnerTeamId) {
  ...
  bool bValidArena =
      isArena() && isRated() && GetStatus() == STATUS_IN_PROGRESS &&
      GetStartTime() >=
          startDelay +
  ...
  if (isBattleground() &&
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_PVPSTATS_BATTLEGROUND);
  ...
  if (isArena() && isRated()) {
    winnerArenaTeam = sArenaTeamMgr->GetArenaTeamById(GetArenaTeamIdForTeam(
        winnerTeamId == TEAM_NEUTRAL ? TEAM_HORDE : winnerTeamId));
    loserArenaTeam = sArenaTeamMgr->GetArenaTeamById(GetArenaTeamIdForTeam(
    ...
    if (winnerArenaTeam && loserArenaTeam &&
      ...
      if (winnerTeamId != TEAM_NEUTRAL) {
        ...
        winnerMatchmakerChange =
            bValidArena ? winnerArenaTeam->WonAgainst(winnerMatchmakerRating,
                                                      loserMatchmakerRating,
                                                      winnerChange, GetBgMap())
                        : 0;
        loserMatchmakerChange = loserArenaTeam->LostAgainst(
            loserMatchmakerRating, winnerMatchmakerRating, loserChange,
            GetBgMap());

        sScriptMgr->OnAfterArenaRatingCalculation(this, winnerMatchmakerChange,
                                                  loserMatchmakerChange,
                                                  winnerChange, loserChange);

        SetArenaMatchmakerRating(winnerTeamId, winnerMatchmakerRating +
                                                   winnerMatchmakerChange);
        SetArenaMatchmakerRating(GetOtherTeamId(winnerTeamId),
                                 loserMatchmakerRating + loserMatchmakerChange);
        SetArenaTeamRatingChangeForTeam(winnerTeamId, winnerChange);
        SetArenaTeamRatingChangeForTeam(GetOtherTeamId(winnerTeamId),
                                        loserChange);
        ...
        stmt->setUInt32(
            2,
            ((GetStartTime() <= startDelay ? 0 : GetStartTime() - startDelay) /
             1000));
        ...
        uint8 memberId = 0;
        for (Battleground::ArenaLogEntryDataMap::const_iterator itr =
                 ArenaLogEntries.begin();
             itr != ArenaLogEntries.end(); ++itr) {
      ...
      // Deduct 16 points from each teams arena-rating if there are no winners
      // after 45+2 minutes
      else {
        ...
        SetArenaTeamRatingChangeForTeam(TEAM_ALLIANCE,
                                        ARENA_TIMELIMIT_POINTS_LOSS);
        SetArenaTeamRatingChangeForTeam(TEAM_HORDE,
                                        ARENA_TIMELIMIT_POINTS_LOSS);
  ...
  for (BattlegroundPlayerMap::iterator itr = m_Players.begin();
       itr != m_Players.end(); ++itr) {
    ...
    if (bgTeamId == winnerTeamId && isArena() && isRated() &&
        GetArenaType() == ARENA_TYPE_5v5 && aliveWinners == 1 &&
        player->IsAlive() && bValidArena)
    ...
    if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam &&
        winnerArenaTeam != loserArenaTeam) {
      if (bgTeamId == winnerTeamId) {
        if (bValidArena) {
          // update achievement BEFORE personal rating update
          uint32 rating =
              player->GetArenaPersonalRating(winnerArenaTeam->GetSlot());
          player->UpdateAchievementCriteria(
              ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA, rating ? rating : 1);
          player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA,
                                            GetMapId());

          winnerArenaTeam->MemberWon(player, loserMatchmakerRating,
                                     winnerMatchmakerChange);
        }
      } else {
        loserArenaTeam->MemberLost(player, winnerMatchmakerRating,
                                   loserMatchmakerChange);

        // Arena lost => reset the win_rated_arena having the "no_lose"
        // condition
        player->ResetAchievementCriteria(ACHIEVEMENT_CRITERIA_CONDITION_NO_LOSE,
                                         0);
      }

      player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_PLAY_ARENA,
                                        GetMapId());
    }
    ...
    if (bgTeamId == winnerTeamId) {
      if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetBgTypeID(true))) {
        UpdatePlayerScore(player, SCORE_BONUS_HONOR,
    ...
    if (isBattleground() &&
      ...
      BattlegroundScoreMap::const_iterator score =
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildBattlegroundStatusPacket(
        &data, this, player->GetCurrentBattlegroundQueueSlot(),
        STATUS_IN_PROGRESS, TIME_TO_AUTOREMOVE, GetStartTime(), GetArenaType(),
        player->GetBgTeamId());
    player->GetSession()->SendPacket(&data);

    player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_BATTLEGROUND, player->GetMapId());
  }
  ...
  if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam &&
    ...
    // this way all arena team members will get notified, not only the ones who
...
void Battleground::BlockMovement(Player *player) {
  player->SetClientControl(
      player,
      0); // movement disabled NOTE: the effect will be automatically removed by
          // client when the player is teleported from the battleground, so no
          // need to send with uint8(1) in RemovePlayerAtLeave()
}

void Battleground::RemovePlayerAtLeave(Player *player) {
  TeamId teamId = player->GetBgTeamId();

  // check if the player was a participant of the match, or only entered through
  // gm command
  ...
  if (participant) {
    ...
    sBattlegroundMgr->BuildBattlegroundStatusPacket(
        &data, this, player->GetCurrentBattlegroundQueueSlot(), STATUS_NONE, 0,
        0, 0, TEAM_NEUTRAL);
    player->GetSession()->SendPacket(&data);
  ...
  player->SetBattlegroundId(0, BATTLEGROUND_TYPE_NONE,
                            PLAYER_MAX_BATTLEGROUND_QUEUES, false, false,
                            TEAM_NEUTRAL);

  // Xinef: remove all criterias on bg leave
  player->ResetAchievementCriteria(ACHIEVEMENT_CRITERIA_CONDITION_BG_MAP,
                                   GetMapId(), true);
...
void Battleground::Init() {
  ...
  if (m_BgInvitedPlayers[TEAM_ALLIANCE] > 0 ||
    ...
    LOG_ERROR("server",
              ...
              "%u, horde: %u) for BG (map: %u, instance id: %u)!",
              m_BgInvitedPlayers[TEAM_ALLIANCE], m_BgInvitedPlayers[TEAM_HORDE],
              m_MapId, m_InstanceID);
    ABORT();
  }
...
void Battleground::AddPlayer(Player *player) {
  ...
  if (isArena()) {
    ...
    if (GetStatus() == STATUS_WAIT_JOIN) // not started yet
      player->CastSpell(player, SPELL_PREPARATION,
  ...
  // Log
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "BATTLEGROUND: Player %s joined the battle.",
...
void Battleground::AddOrSetPlayerToCorrectBgGroup(Player *player,
  ...
  if (player->GetGroup() &&
    ...
    LOG_INFO("misc",
             "Battleground::AddOrSetPlayerToCorrectBgGroup - player is already "
             "in %s group!",
...
uint32 Battleground::GetFreeSlotsForTeam(TeamId teamId) const {
  ...
  // BG_QUEUE_INVITATION_TYPE_NO_BALANCE, invite everyone unless the BG is full
  ...
  // BG_QUEUE_INVITATION_TYPE_NO_BALANCE, do not allow to join too many players
  // of one faction
...
void Battleground::ReadyMarkerClicked(Player *p) {
  ...
  p->GetSession()->SendNotification("You are marked as ready %u/%u", count,
                                    req);
  if (count == req) {
    m_Events |= BG_STARTING_EVENT_2;
...
void Battleground::UpdatePlayerScore(Player *player, uint32 type, uint32 value,
  ...
  case SCORE_KILLING_BLOWS: // Killing blows
    ...
    if (isArena() && isRated()) {
      ArenaLogEntryDataMap::iterator itr2 =
          ArenaLogEntries.find(player->GetGUID());
  ...
  case SCORE_BONUS_HONOR: // Honor bonus
    ...
    if (isBattleground()) {
      ...
      if (doAddHonor)
        player->RewardHonor(nullptr, 1,
                            value); // RewardHonor calls UpdatePlayerScore with
                                    // doAddHonor = false
      else
  ...
  default:
    LOG_ERROR("server",
              "Battleground::UpdatePlayerScore: unknown score type (%u) for BG "
              "(map: %u, instance id: %u)!",
...
void Battleground::RemovePlayerFromResurrectQueue(Player *player) {
  for (std::map<uint64, std::vector<uint64>>::iterator itr =
           m_ReviveQueue.begin();
       itr != m_ReviveQueue.end(); ++itr)
    for (std::vector<uint64>::iterator itr2 = (itr->second).begin();
         itr2 != (itr->second).end(); ++itr2)
...
void Battleground::RelocateDeadPlayers(uint64 queueIndex) {
  ...
  if (!ghostList.empty()) {
    ...
    for (std::vector<uint64>::const_iterator itr = ghostList.begin();
         itr != ghostList.end(); ++itr) {
      Player *player = ObjectAccessor::FindPlayer(*itr);
      if (!player)
        continue;
      ...
      if (closestGrave)
        player->TeleportTo(GetMapId(), closestGrave->x, closestGrave->y,
                           closestGrave->z, player->GetOrientation());
    }
    ghostList.clear();
  }
}
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y,
                             float z, float o, float rotation0, float rotation1,
                             float rotation2, float rotation3,
                             uint32 /*respawnTime*/, GOState goState) {
  ...
  if (!map)
    return false;
  // Must be created this way, adding to godatamap would add it to the base map
  // of the instance and when loading it (in go::LoadFromDB()), a new guid would
  // be assigned to the object, and a new object would be created So we must
  // create it specific for this instance
  ...
  if (!go->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), entry,
                  GetBgMap(), PHASEMASK_NORMAL, x, y, z, o,
                  G3D::Quat(rotation0, rotation1, rotation2, rotation3), 100,
                  goState)) {
    LOG_ERROR("sql.sql",
              "Battleground::AddObject: cannot create gameobject (entry: %u) "
              "for BG (map: %u, instance id: %u)!",
  ...
  /*
     ...
     if that was the only function using godata from GameObject without checking
     if it existed GameObjectData& data = sObjectMgr->NewGOData(guid);
...
void Battleground::DoorClose(uint32 type) {
  if (GameObject *obj = GetBgMap()->GetGameObject(BgObjects[type])) {
    ...
    LOG_ERROR("server",
              "Battleground::DoorClose: door gameobject (type: %u, GUID: %u) "
              "not found for BG (map: %u, instance id: %u)!",
...
void Battleground::DoorOpen(uint32 type) {
  if (GameObject *obj = GetBgMap()->GetGameObject(BgObjects[type])) {
    ...
    LOG_ERROR("server",
              "Battleground::DoorOpen: door gameobject (type: %u, GUID: %u) "
              "not found for BG (map: %u, instance id: %u)!",
              type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
}
...
GameObject *Battleground::GetBGObject(uint32 type) {
  ...
  if (!obj)
    LOG_ERROR("server",
              "Battleground::GetBGObject: gameobject (type: %u, GUID: %u) not "
              "found for BG (map: %u, instance id: %u)!",
              type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
  return obj;
}
...
Creature *Battleground::GetBGCreature(uint32 type) {
  ...
  if (!creature)
    LOG_ERROR("server",
              "Battleground::GetBGCreature: creature (type: %u, GUID: %u) not "
              "found for BG (map: %u, instance id: %u)!",
...
Creature *Battleground::AddCreature(uint32 entry, uint32 type, float x, float y,
                                    float z, float o, uint32 respawntime,
  ...
  if (!creature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), map,
                        PHASEMASK_NORMAL, entry, 0, x, y, z, o)) {
    LOG_ERROR("server",
              "Battleground::AddCreature: cannot create creature (entry: %u) "
              "for BG (map: %u, instance id: %u)!",
              entry, m_MapId, m_InstanceID);
    delete creature;
    return nullptr;
  ...
  if (!cinfo) {
    LOG_ERROR("server",
              "Battleground::AddCreature: creature template (entry: %u) does "
              "not exist for BG (map: %u, instance id: %u)!",
              entry, m_MapId, m_InstanceID);
    delete creature;
    return nullptr;
  }
...
bool Battleground::DelCreature(uint32 type) {
  ...
  LOG_ERROR("server",
            "Battleground::DelCreature: creature (type: %u, GUID: %u) not "
            "found for BG (map: %u, instance id: %u)!",
            type, GUID_LOPART(BgCreatures[type]), m_MapId, m_InstanceID);
  BgCreatures[type] = 0;
  return false;
...
bool Battleground::DelObject(uint32 type) {
  ...
  LOG_ERROR("server",
            "Battleground::DelObject: gameobject (type: %u, GUID: %u) not "
            "found for BG (map: %u, instance id: %u)!",
            type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
  BgObjects[type] = 0;
  return false;
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z,
                                  float o, TeamId teamId) {
  ...
  LOG_ERROR("server",
            ...
            "%u, entry: %u) for BG (map: %u, instance id: %u)!",
            type, entry, m_MapId, m_InstanceID);
  EndNow();
  return false;
}

void Battleground::SendMessageToAll(uint32 entry, ChatMsg type,
                                    Player const *source) {
  if (!entry)
    return;

  Warhead::BattlegroundChatBuilder bg_builder(type, entry, source);
  Warhead::LocalizedPacketDo<Warhead::BattlegroundChatBuilder> bg_do(
      bg_builder);
  BroadcastWorker(bg_do);
}

void Battleground::PSendMessageToAll(uint32 entry, ChatMsg type,
                                     Player const *source, ...) {
  if (!entry)
    return;
  ...
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
    ...
    if (localizedPackets.find(
      ...
      ChatHandler::BuildChatPacket(
          ...
          CHAT_MSG_RAID_BOSS_EMOTE, LANG_UNIVERSAL, nullptr, nullptr, str);
    }
...
void Battleground::SendMessage2ToAll(uint32 entry, ChatMsg type,
                                     Player const *source, uint32 arg1,
                                     uint32 arg2) {
  Warhead::Battleground2ChatBuilder bg_builder(type, entry, source, arg1, arg2);
  Warhead::LocalizedPacketDo<Warhead::Battleground2ChatBuilder> bg_do(
      bg_builder);
  BroadcastWorker(bg_do);
}
...
void Battleground::HandleTriggerBuff(GameObject *gameObject) {
  ...
  uint32 index = 0;
  for (; index < BgObjects.size() && BgObjects[index] != gameObject->GetGUID();
       ++index)
    ;
...
uint32 Battleground::GetAlivePlayersCountByTeam(TeamId teamId) const {
  ...
  for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin();
    ...
    if (itr->second->IsAlive() &&
        !itr->second->HasByteFlag(UNIT_FIELD_BYTES_2, 3,
...
int32 Battleground::GetObjectType(uint64 guid) {
  ...
  LOG_ERROR(
      "server",
      "Battleground::GetObjectType: player used gameobject (GUID: %u) which is "
      "not in internal data for BG (map: %u, instance id: %u), cheating?",
...
GraveyardStruct const *Battleground::GetClosestGraveyard(Player *player) {
  return sGraveyard->GetClosestGraveyard(
      player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(),
      player->GetMapId(), player->GetBgTeamId());
}
...
uint32 Battleground::GetTeamScore(TeamId teamId) const {
  ...
  LOG_ERROR("server", "GetTeamScore with wrong Team %u for BG %u", teamId,
            GetBgTypeID());
  return 0;
}
...
uint8 Battleground::GetUniqueBracketId() const { return GetMinLevel() / 10; }
