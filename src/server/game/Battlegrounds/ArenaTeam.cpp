...
ArenaTeam::ArenaTeam()
    : TeamId(0), Type(0), TeamName(), CaptainGuid(0), BackgroundColor(0),
      EmblemStyle(0), EmblemColor(0), BorderStyle(0), BorderColor(0) {
  Stats.WeekGames = 0;
  Stats.SeasonGames = 0;
  Stats.Rank = 0;
...
bool ArenaTeam::Create(uint64 captainGuid, uint8 type,
                       std::string const &teamName, uint32 backgroundColor,
                       uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle,
                       uint32 borderColor) {
...
bool ArenaTeam::AddMember(uint64 playerGuid) {
  ...
  if (player) {
    ...
    GlobalPlayerData const *playerData =
        sWorld->GetGlobalPlayerData(GUID_LOPART(playerGuid));
    if (!playerData)
      return false;
  ...
  if ((player && player->GetArenaTeamId(GetSlot())) ||
      Player::GetArenaTeamIdFromStorage(GUID_LOPART(playerGuid), GetSlot()) !=
          0) {
    LOG_ERROR(
        "server",
        "Arena: Player %s (guid: %u) already has an arena team of type %u",
        playerName.c_str(), GUID_LOPART(playerGuid), GetType());
    return false;
  }
  ...
  sWorld->UpdateGlobalPlayerArenaTeam(GUID_LOPART(playerGuid), GetSlot(),
                                      GetId());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result) {
  ...
  do {
    ...
    newMember.MatchMakerRating =
        fields[9].GetUInt16() > 0
            ? fields[9].GetUInt16()
            : sWorld->getIntConfig(CONFIG_ARENA_START_MATCHMAKER_RATING);
    ;
    newMember.MaxMMR =
        std::max(fields[10].GetUInt16(), newMember.MatchMakerRating);

    // Delete member if character information is missing
    if (fields[6].GetString().empty()) {
      LOG_ERROR("sql.sql",
                "ArenaTeam %u has member with empty name - probably player %u "
                "doesn't exist, deleting him from memberlist!",
                arenaTeamId, GUID_LOPART(newMember.Guid));
    ...
    Members.push_back(newMember);
    sWorld->UpdateGlobalPlayerArenaTeam(GUID_LOPART(newMember.Guid), GetSlot(),
                                        GetId());
...
void ArenaTeam::SetCaptain(uint64 guid) {
  ...
  if (Player *newCaptain = ObjectAccessor::FindPlayerInOrOutOfWorld(guid)) {
    ...
    /*if (oldCaptain)
    {
        LOG_DEBUG("bg.battleground", "Player: %s [GUID: %u] promoted player: %s
    [GUID: %u] to leader of arena team [Id: %u] [Type: %u].",
            oldCaptain->GetName().c_str(), oldCaptain->GetGUIDLow(),
    newCaptain->GetName().c_str(), newCaptain->GetGUIDLow(), GetId(),
    GetType());
    }*/
  }
}
...
void ArenaTeam::DelMember(uint64 guid, bool cleanDb) {
          ...
          if (queue.GetPlayerGroupInfoData(playerMember->GetGUID(), &ginfo)) {
            if (!ginfo.IsInvitedToBGInstanceGUID) {
              ...
              sBattlegroundMgr->BuildBattlegroundStatusPacket(
                  &data, nullptr,
                  playerMember->GetBattlegroundQueueIndex(bgQueue), STATUS_NONE,
                  0, 0, 0, TEAM_NEUTRAL);
  ...
  if (player) {
    player->GetSession()->SendArenaTeamCommandResult(ERR_ARENA_TEAM_QUIT_S,
                                                     GetName(), "", 0);
...
void ArenaTeam::Disband(WorldSession *session) {
  ...
  // Broadcast update
  if (session) {
    BroadcastEvent(ERR_ARENA_TEAM_DISBANDED_S, 0, 2, session->GetPlayerName(),
                   GetName(), "");
  }
...
void ArenaTeam::Roster(WorldSession *session) {
  ...
  for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
    ...
    data << uint32(
        (itr->Guid == GetCaptain() ? 0 : 1)); // captain flag 0 captain 1 member
...
void ArenaTeamMember::ModifyPersonalRating(Player *player, int32 mod,
                                           uint32 type) {
  ...
  if (player && ArenaTeam::GetSlotByType(type) < 3) {
    player->SetArenaTeamInfoField(ArenaTeam::GetSlotByType(type),
                                  ARENA_TEAM_PERSONAL_RATING, PersonalRating);
    player->UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_PERSONAL_RATING, PersonalRating,
        type);
  }
}
...
void ArenaTeamMember::ModifyMatchmakerRating(int32 mod, uint32 /*slot*/) {
  if (mod < 0) {
    ...
    mod = std::min<int32>(
        std::max<int32>(
            -((int32)MatchMakerRating - (int32)MaxMMR + maxAllowedDrop), mod),
        0);
  }
...
void ArenaTeam::BroadcastEvent(ArenaTeamEvents event, uint64 guid,
                               uint8 strCount, std::string const &str1,
  ...
  case 3:
    ...
    break;
  default:
    LOG_ERROR("server", "Unhandled strCount %u in ArenaTeam::BroadcastEvent",
              strCount);
    return;
  }
...
uint8 ArenaTeam::GetSlotByType(uint32 type) {
  ...
  LOG_ERROR("server", "FATAL: Unknown arena team type %u for some arena team",
            type);
  return 0xFF;
}
...
float ArenaTeam::GetChanceAgainst(uint32 ownRating, uint32 opponentRating) {
  // Returns the chance to win against a team with the given rating, used in the
  // rating adjustment calculation ELO system
  return 1.0f / (1.0f + exp(log(10.0f) *
    ...
    if (ownRating < 1300) {
      ...
      if (ownRating < 1000)
        ...
        mod = ((win_rating_modifier1 / 2.0f) +
               ((win_rating_modifier1 / 2.0f) * (1300.0f - float(ownRating)) /
                300.0f)) *
              (1.0f - chance);
    } else
...
void ArenaTeam::FinishGame(int32 mod, const Map *bgMap) {
      ...
      if (Player *member = ObjectAccessor::FindPlayerInOrOutOfWorld(itr->Guid))
        if (member->FindMap() == bgMap)
          member->UpdateAchievementCriteria(
              ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_TEAM_RATING, Stats.Rating,
              Type);
  }
  ...
  // Update team's rank, start with rank 1 and increase until no team with more
  // rating was found
  Stats.Rank = 1;
...
int32 ArenaTeam::WonAgainst(uint32 Own_MMRating, uint32 Opponent_MMRating,
                            int32 &rating_change, const Map *bgMap) {
...
void ArenaTeam::MemberLost(Player *player, uint32 againstMatchmakerRating,
  ...
  for (MemberList::iterator itr = Members.begin(); itr != Members.end();
    ...
    if (itr->Guid == player->GetGUID()) {
      ...
      int32 mod =
          GetRatingMod(itr->PersonalRating, againstMatchmakerRating, false);
      ...
      player->SetArenaTeamInfoField(GetSlot(), ARENA_TEAM_GAMES_WEEK,
                                    itr->WeekGames);
      player->SetArenaTeamInfoField(GetSlot(), ARENA_TEAM_GAMES_SEASON,
                                    itr->SeasonGames);
      return;
    }
  }
...
void ArenaTeam::MemberWon(Player *player, uint32 againstMatchmakerRating,
                          int32 MatchmakerRatingChange) {
  ...
  for (MemberList::iterator itr = Members.begin(); itr != Members.end();
    ...
    if (itr->Guid == player->GetGUID()) {
      // update personal rating
      int32 mod =
          GetRatingMod(itr->PersonalRating, againstMatchmakerRating, true);
      itr->ModifyPersonalRating(player, mod, GetType());

      // update matchmaker rating (pussywizard: but don't allow it to go over
      // team rating)
...
void ArenaTeam::UpdateArenaPointsHelper(
  ...
  // Helper function for arena point distribution (this way, when distributing,
  // no actual calculation is required, just a few comparisons) 10 played games
  // per week is a minimum
  ...
  for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
    ...
    std::map<uint32, uint32>::iterator plr_itr =
        playerPoints.find(GUID_LOPART(itr->Guid));
...
void ArenaTeam::SaveToDB() {
  ...
  for (MemberList::const_iterator itr = Members.begin(); itr != Members.end();
    ...
    stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_REP_CHARACTER_ARENA_STATS);
