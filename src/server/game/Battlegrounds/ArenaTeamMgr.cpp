...
ArenaTeam *ArenaTeamMgr::GetArenaTeamByCaptain(uint64 guid,
                                               const uint32 type) const {
  for (ArenaTeamContainer::const_iterator itr = ArenaTeamStore.begin();
       itr != ArenaTeamStore.end(); ++itr) {
...
void ArenaTeamMgr::LoadArenaTeams() {
  ...
  LOG_INFO("server", ">> Loaded %u arena teams in %u ms", count,
...
void ArenaTeamMgr::DistributeArenaPoints() {
  ...
  for (ArenaTeamContainer::iterator teamItr = GetArenaTeamMapBegin();
       teamItr != GetArenaTeamMapEnd(); ++teamItr)
  ...
  for (std::map<uint32, uint32>::iterator playerItr = PlayerPoints.begin();
       playerItr != PlayerPoints.end(); ++playerItr) {
