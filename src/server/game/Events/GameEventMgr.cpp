    ...
    for (std::set<uint16>::const_iterator itr =
      ...
      if ((mGameEvent[*itr].state != GAMEEVENT_WORLD_NEXTPHASE &&
           mGameEvent[*itr].state !=
               GAMEEVENT_WORLD_FINISHED) || // if prereq not in nextphase or
                                            // finished state, then can't start
                                            // this one
          mGameEvent[*itr].nextstart >
              currenttime) // if not in nextphase state for long enough, can't
                           // start this one
        return false;
    }
    // all prerequisite events are met
    // but if there are no prerequisites, this can be only activated through gm
    // command
...
uint32 GameEventMgr::NextCheck(uint16 entry) const {
  ...
  // for NEXTPHASE state world events, return the delay to start the next event,
  // so the followup event will be checked correctly
  ...
  // for CONDITIONS state world events, return the length of the wait period, so
  // if the conditions are met, this check will be called again to set the timer
  // as NEXTPHASE event
  ...
  if ((((currenttime - mGameEvent[entry].start) %
        (mGameEvent[entry].occurence * 60)) < (mGameEvent[entry].length * 60)))
...
bool GameEventMgr::StartEvent(uint16 event_id, bool overwrite) {
  ...
  if (data.state == GAMEEVENT_NORMAL || data.state == GAMEEVENT_INTERNAL) {
    ...
    // force game event update to set the update timer if conditions were met
    // from a command this update is needed to possibly start events dependent
    // on the started one or to scedule another update where the next event will
    // be started
...
void GameEventMgr::LoadFromDB() {
  {
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "UNIX_TIMESTAMP(end_time), occurence, length, holiday, holidayStage, "
        "description, world_event, announce FROM game_event");
    if (!result) {
      mGameEvent.clear();
      LOG_ERROR("sql.sql",
    ...
    do {
      ...
      if (pGameEvent.length == 0 &&
          pGameEvent.state == GAMEEVENT_NORMAL) // length>0 is validity check
      {
        LOG_ERROR("sql.sql",
                  "`game_event` game event id (%i) isn't a world event and has "
                  "length = 0, thus it can't be used.",
                  event_id);
        continue;
      }
    ...
    LOG_INFO("server", ">> Loaded %u game events in %u ms", count,
  ...
  LOG_INFO("server", "Loading Game Event Saves Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 game event saves in game events. DB "
      ...
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_save` game event entry (%i) is out of range "
                    "compared to max event entry in `game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server",
               ">> Loaded %u game event saves in game events in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event Prerequisite Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 game event prerequisites in game events. "
                         "DB table `game_event_prerequisite` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_prerequisite` game event id (%i) is out of "
        ...
        if (mGameEvent[event_id].state != GAMEEVENT_NORMAL &&
          ...
          if (prerequisite_event >= mGameEvent.size()) {
            LOG_ERROR(
                "sql.sql",
                "`game_event_prerequisite` game event prerequisite id (%i) is "
          ...
          LOG_ERROR("sql.sql",
                    "game_event_prerequisiste includes event entry for "
                    "non-worldevent id %u",
                    event_id);
          continue;
        }
  ...
  LOG_INFO("server", "Loading Game Event Creature Data...");
  {
    ...
    //                                                       1                2
    QueryResult result = WorldDatabase.Query(
        "SELECT creature.guid, game_event_creature.eventEntry FROM creature"
        " JOIN game_event_creature ON creature.guid = "
        "game_event_creature.guid");

    if (!result) {
      LOG_INFO("server", ">> Loaded 0 creatures in game events. DB table "
      ...
      do {
        ...
        if (internal_event_id < 0 ||
          ...
          LOG_ERROR("sql.sql",
                    "`game_event_creature` game event id (%i) is out of range "
      ...
      LOG_INFO("server", ">> Loaded %u creatures in game events in %u ms",
               count, GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event GO Data...");
  {
    ...
    //                                                      0                1
    QueryResult result =
        WorldDatabase.Query("SELECT gameobject.guid, "
                            "game_event_gameobject.eventEntry FROM gameobject"
                            " JOIN game_event_gameobject ON "
                            "gameobject.guid=game_event_gameobject.guid");

    if (!result) {
      LOG_INFO("server", ">> Loaded 0 gameobjects in game events. DB table "
      ...
      do {
        ...
        if (internal_event_id < 0 ||
          ...
          LOG_ERROR("sql.sql",
                    "`game_event_gameobject` game event id (%i) is out of "
                    "range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u gameobjects in game events in %u ms",
               count, GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event Model/Equipment Change Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 model/equipment changes in game events. "
                         "DB table `game_event_model_equip` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (event_id >= mGameEventModelEquip.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_model_equip` game event id (%u) is out of "
                    "range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
        ...
        if (newModelEquipSet.equipment_id > 0) {
          ...
          if (!sObjectMgr->GetEquipmentInfo(entry, equipId)) {
            LOG_ERROR("sql.sql",
                      "Table `game_event_model_equip` have creature (Guid: %u) "
  ...
  LOG_INFO("server", "Loading Game Event Quest Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 quests additions in game events. DB "
                         "table `game_event_creature_quest` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (event_id >= mGameEventCreatureQuests.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_creature_quest` game event id (%u) is out of "
                    "range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server",
               ">> Loaded %u quests additions in game events in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event GO Quest Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 go quests additions in game events. DB "
                         "table `game_event_gameobject_quest` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (event_id >= mGameEventGameObjectQuests.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_gameobject_quest` game event id (%u) is out "
  ...
  LOG_INFO("server", "Loading Game Event Quest Condition Data...");
  {
    ...
    //                                                 0       1         2 3
    QueryResult result =
        WorldDatabase.Query("SELECT quest, eventEntry, condition_id, num FROM "
                            "game_event_quest_condition");

    if (!result) {
      LOG_INFO("server", ">> Loaded 0 quest event conditions in game events. "
                         "DB table `game_event_quest_condition` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_quest_condition` game event id (%u) is out of "
                    "range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
  ...
  LOG_INFO("server", "Loading Game Event Condition Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 conditions in game events. DB table "
      ...
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_condition` game event id (%u) is out of range "
                    "compared to max event id in `game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u conditions in game events in %u ms",
               count, GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event Condition Save Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 condition saves in game events. DB table "
      ...
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_condition_save` game event id (%u) is out of "
                    "range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
        ...
        if (itr != mGameEvent[event_id].conditions.end()) {
          ...
          LOG_ERROR("sql.sql",
                    "game_event_condition_save contains not present condition "
                    "evt id %u cond id %u",
                    event_id, condition);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u condition saves in game events in %u ms",
               count, GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event NPCflag Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 npcflags in game events. DB table "
      ...
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_npcflag` game event id (%u) is out of range "
                    "compared to max event id in `game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u npcflags in game events in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event Seasonal Quest Relations...");
  {
    ...
    if (!result) {
      LOG_INFO("server",
               ">> Loaded 0 seasonal quests additions in game events. DB table "
               "`game_event_seasonal_questrelation` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (!questTemplate) {
          LOG_ERROR("sql.sql",
                    "`game_event_seasonal_questrelation` quest id (%u) does "
                    "not exist in `quest_template`",
                    questId);
          continue;
        }
        ...
        if (eventEntry >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_seasonal_questrelation` event id (%u) is out "
                    "of range compared to max event in `game_event`",
                    eventEntry);
          continue;
        }
  ...
  LOG_INFO("server", "Loading Game Event Vendor Additions Data...");
  {
    ...
    //                                                    0        1    2 3 4 5
    QueryResult result = WorldDatabase.Query(
        "SELECT eventEntry, guid, item, maxcount, incrtime, ExtendedCost FROM "
        "game_event_npc_vendor ORDER BY guid, slot ASC");

    if (!result) {
      LOG_INFO("server", ">> Loaded 0 vendor additions in game events. DB "
                         "table `game_event_npc_vendor` is empty.");
      LOG_INFO("server", " ");
    } else {
      uint32 count = 0;
      do {
        ...
        if (event_id >= mGameEventVendors.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_npc_vendor` game event id (%u) is out of "
                    "range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
        ...
        // get the event npc flag for checking if the npc will be vendor during
        // the event or not
        ...
        if (!sObjectMgr->IsVendorItemValid(newEntry.entry, newEntry.item,
                                           newEntry.maxcount, newEntry.incrtime,
                                           newEntry.ExtendedCost, nullptr,
      ...
      LOG_INFO("server",
               ">> Loaded %u vendor additions in game events in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
  ...
  LOG_INFO("server", "Loading Game Event Battleground Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 battleground holidays in game events. DB "
      ...
      do {
        ...
        if (event_id >= mGameEvent.size()) {
          LOG_ERROR("sql.sql",
                    "`game_event_battleground_holiday` game event id (%u) is "
                    "out of range compared to max event id in `game_event`",
                    event_id);
          continue;
        }
  ...
  LOG_INFO("server", "Loading Game Event Pool Data...");
  {
    ...
    if (!result) {
      LOG_INFO("server", ">> Loaded 0 pools for game events. DB table "
      ...
      do {
        ...
        if (internal_event_id < 0 ||
          ...
          LOG_ERROR("sql.sql",
                    "`game_event_pool` game event id (%i) is out of range "
                    "compared to max event id in `game_event`",
                    event_id);
          continue;
        }
      ...
      LOG_INFO("server", ">> Loaded %u pools for game events in %u ms", count,
               GetMSTimeDiffToNow(oldMSTime));
      LOG_INFO("server", " ");
    }
  }
...
void GameEventMgr::LoadHolidayDates() {
  ...
  do {
    ...
    if (!entry) {
      LOG_ERROR("sql.sql", "holiday_dates entry has invalid holiday id %u.",
                holidayId);
      continue;
    }
    ...
    if (dateId >= MAX_HOLIDAY_DATES) {
      LOG_ERROR("sql.sql", "holiday_dates entry has out of range date_id %u.",
                dateId);
      continue;
    }
    ...
    auto itr = std::lower_bound(modifiedHolidays.begin(),
                                modifiedHolidays.end(), entry->Id);
  ...
  LOG_INFO("server", ">> Loaded %u holiday dates in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
}
...
void GameEventMgr::StartArenaSeason() {
  ...
  if (!result) {
    LOG_ERROR("server", "ArenaSeason (%u) must be an existant Arena Season",
              season);
    return;
  }
  ...
  if (eventId >= mGameEvent.size()) {
    LOG_ERROR("server", "EventEntry %u for ArenaSeason (%u) does not exists",
              eventId, season);
    return;
  }
...
uint32 GameEventMgr::Update() // return the next event delay in ms
{
  ...
  for (uint16 itr = 1; itr < mGameEvent.size(); ++itr) {
    ...
    if (CheckOneGameEvent(itr)) {
      // if the world event is in NEXTPHASE state, and the time has passed to
      // finish this event, then do so
      if (mGameEvent[itr].state == GAMEEVENT_WORLD_NEXTPHASE &&
        ...
        // changed, save to DB the gameevent state, will be updated in next
        // update cycle
        SaveWorldEventStateToDB(itr);
  ...
  for (std::set<uint16>::iterator itr = activate.begin(); itr != activate.end();
       ++itr)
    // start the event
  ...
  LOG_DEBUG("server", "Next game event check in %u seconds.",
            nextEventDelay + 1);
#endif
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "GameEvent %u \"%s\" removed.", event_id,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "GameEvent %u \"%s\" started.", event_id,
            mGameEvent[event_id].description.c_str());
#endif
...
void GameEventMgr::UpdateEventNPCFlags(uint16 event_id) {
  ...
  for (NPCFlagList::iterator itr = mGameEventNPCFlags[event_id].begin();
    ...
    // get the creature data from the low guid to get the entry, to be able to
    // find out the whole guid
    if (CreatureData const *data = sObjectMgr->GetCreatureData(itr->first)) {
      ...
      if (cr) {
        ...
        // reset gossip options, since the flag change might have added /
        // removed some
        // cr->ResetGossipOptions();
      }
      // if we didn't find it, then the npcflag will be updated when the
      // creature is loaded
    }
  }
}
...
void GameEventMgr::GameEventSpawn(int16 event_id) {
  ...
  if (internal_event_id < 0 ||
    ...
    LOG_ERROR("server",
              ...
              "mGameEventCreatureGuids element %i (size: " SZFMTD ")",
  ...
  if (internal_event_id >= int32(mGameEventGameobjectGuids.size())) {
    LOG_ERROR("server",
              "GameEventMgr::GameEventSpawn attempt access to out of range "
              "mGameEventGameobjectGuids element %i (size: " SZFMTD ")",
  ...
  if (internal_event_id >= int32(mGameEventPoolIds.size())) {
    LOG_ERROR("server",
              "GameEventMgr::GameEventSpawn attempt access to out of range "
              "mGameEventPoolIds element %u (size: " SZFMTD ")",
...
void GameEventMgr::ChangeEquipOrModel(int16 event_id, bool activate) {
  for (ModelEquipList::iterator itr = mGameEventModelEquip[event_id].begin();
    ...
    // Update if spawned
    Creature *creature = ObjectAccessor::GetObjectInWorld(
        MAKE_NEW_GUID(itr->first, data->id, HIGHGUID_UNIT),
        (Creature *)nullptr);
    if (creature) {
      if (activate) {
    ...
    } else // If not spawned
    {
      ...
      if (data2 && activate) {
        CreatureTemplate const *cinfo =
            sObjectMgr->GetCreatureTemplate(data2->id);
  ...
  if (event.holiday_id != HOLIDAY_NONE) {
    ...
    if (bgTypeId != BATTLEGROUND_TYPE_NONE) {
      ...
      if (bl && bl->HolidayWorldStateId) {
        WorldPacket data;
        sBattlegroundMgr->BuildUpdateWorldStatePacket(
            &data, bl->HolidayWorldStateId, Activate ? 1 : 0);
...
void GameEventMgr::SaveWorldEventStateToDB(uint16 event_id) {
  ...
  stmt->setUInt32(2, mGameEvent[event_id].nextstart
...
void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate) {
  ...
  {
    ACORE_READ_GUARD(HashMapHolder<Creature>::LockType,
                     *HashMapHolder<Creature>::GetLock());
    HashMapHolder<Creature>::MapType const &m = ObjectAccessor::GetCreatures();
    for (HashMapHolder<Creature>::MapType::const_iterator iter = m.begin();
         iter != m.end(); ++iter)
      if (iter->second->IsInWorld() &&
          !iter->second->IsDuringRemoveFromWorld() && iter->second->FindMap() &&
          iter->second->IsAIEnabled && iter->second->AI())
        iter->second->AI()->sOnGameEvent(activate, event_id);
  }
  {
    ACORE_READ_GUARD(HashMapHolder<GameObject>::LockType,
                     *HashMapHolder<GameObject>::GetLock());
    HashMapHolder<GameObject>::MapType const &m =
        ObjectAccessor::GetGameObjects();
    for (HashMapHolder<GameObject>::MapType::const_iterator iter = m.begin();
         iter != m.end(); ++iter)
      if (iter->second->IsInWorld() && iter->second->FindMap() &&
          iter->second->AI())
...
void GameEventMgr::SetHolidayEventTime(GameEventData &event) {
  ...
  {
    LOG_ERROR("sql.sql", "Missing date or duration for holiday %u.",
              event.holiday_id);
    return;
  }
...

  {
    ...
    if (curTime < startTime + event.length * MINUTE) {
      ...
      // (modified by holiday_dates) if none is found we don't modify start date
      // and use the one in game_event
    }
  }
}
