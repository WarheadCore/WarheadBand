...
void LootItemStorage::LoadStorageFromDB() {
  ...
  do {
    ...
    itemList.push_back(
        StoredLootItem(fields[1].GetUInt32(), fields[2].GetUInt32(),
                       fields[3].GetInt32(), fields[4].GetUInt32()));

    ++count;
  } while (result->NextRow());

  LOG_INFO("server", ">> Loaded %d stored items in %u ms", count,
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}

void LootItemStorage::RemoveEntryFromDB(uint32 containerId, uint32 itemid,
                                        uint32 count) {
...
void LootItemStorage::AddNewStoredLoot(Loot *loot, Player * /*player*/) {
  if (lootItemStore.find(loot->containerId) != lootItemStore.end()) {
    LOG_INFO("misc", "LootItemStorage::AddNewStoredLoot (A1) - %u!",
             loot->containerId);
    return;
  }
  ...
  if (!loot->isLooted())
    for (LootItemList::const_iterator li = loot->items.begin();
         li != loot->items.end(); li++) {
      // Even if an item is not available for a specific player, it doesn't mean
      // that we are not able to trade this container to another player that is
      // able to loot that item if we don't save it then the item will be lost
      // at player re-login.
      ...
      itemList.push_back(StoredLootItem(
          li->itemid, li->count, li->randomPropertyId, li->randomSuffix));
...
void LootItemStorage::RemoveStoredLootItem(uint32 containerId, uint32 itemid,
                                           uint32 count, Loot *loot) {
...
void LootItemStorage::RemoveStoredLoot(uint32 containerId) {
  ...
  SQLTransaction trans = CharacterDatabase.BeginTransaction();
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_DEL_ITEMCONTAINER_CONTAINER);
  stmt->setUInt32(0, containerId);
  trans->Append(stmt);
