...
struct LootGroupInvalidSelector
    : public Warhead::unary_function<LootStoreItem *, bool> {
  ...
  bool operator()(LootStoreItem *item) const {
    ...
    for (std::vector<LootItem>::const_iterator itr = _loot.items.begin();
         itr != _loot.items.end(); ++itr)
      if (itr->itemid == item->itemid) {
        ++foundDuplicates;
        if (_proto->InventoryType == 0 && foundDuplicates == 3 &&
            _proto->ItemId !=
                47242 /*Trophy of the Crusade*/) // Non-equippable items are
                                                 // limited to 3 drops
          return true;
...
public:
  ...
  void Process(Loot &loot, Player const *player, LootStore const &lootstore,
               uint16 lootMode) const; // Rolls an item from the group (if any)
                                       // and adds the item to the loot
...
private:
  ...
  LootStoreItem const *
  Roll(Loot &loot, Player const *player, LootStore const &store,
       uint16 lootMode) const; // Rolls an item from the group, returns nullptr
                               // if all miss their chances
...
uint32 LootStore::LoadLootTable() {
  ...
  QueryResult result = WorldDatabase.PQuery(
      ...
      "GroupId, MinCount, MaxCount FROM %s",
      GetName());
  ...
  do {
    ...
    if (maxcount > std::numeric_limits<uint8>::max()) {
      LOG_ERROR("sql.sql",
                "Table '%s' Entry %d Item %d: MaxCount value (%u) to large. "
                "must be less %u - skipped",
    ...
    if (lootmode == 0) {
      LOG_ERROR("server",
                "Table '%s' Entry %d Item %d: LootMode is equal to 0, item "
                "will never drop - setting mode 1",
                GetName(), entry, item);
      lootmode = 1;
    }

    LootStoreItem *storeitem =
        new LootStoreItem(item, reference, chance, needsquest, lootmode,
...
void LootStore::ReportNonExistingId(uint32 lootId, const char *ownerType,
                                    uint32 ownerId) const {
  LOG_ERROR("sql.sql",
            "Table '%s' Entry %d does not exist but it is used by %s %d",
...
bool LootStoreItem::Roll(bool rate, Player const *player, Loot &loot,
  ...
  if (reference > 0) // reference case
    return roll_chance_f(
        _chance * (rate ? sWorld->getRate(RATE_DROP_ITEM_REFERENCED) : 1.0f));
...
bool LootStoreItem::IsValid(LootStore const &store, uint32 entry) const {
  if (groupid >= 1 << 7) // it stored in 7 bit field
  {
    LOG_ERROR(
        ...
        store.GetName(), entry, itemid, groupid, 1 << 7);
    return false;
  }

  if (mincount == 0) {
    LOG_ERROR("sql.sql",
              "Table '%s' Entry %d Item %d: wrong MinCount (%d) - skipped",
              store.GetName(), entry, itemid, mincount);
    return false;
  }
  ...
  if (reference == 0) // item (quest or non-quest) entry, maybe grouped
  {
    ...
    {
      LOG_ERROR("sql.sql",
                "Table '%s' Entry %d Item %d: MaxCount (%u) less that MinCount "
                "(%i) - skipped",
  ...
  } else // if reference loot
  {
    if (needs_quest)
      LOG_ERROR("sql.sql",
                "Table '%s' Entry %d Item %d: quest required will be ignored",
                store.GetName(), entry, itemid);
...
bool LootItem::AllowedForPlayer(Player const *player) const {
  ...
  // not show loot for players without profession or those who already know the
  // recipe
...
bool Loot::FillLoot(uint32 lootId, LootStore const &store, Player *lootOwner,
  ...
  tab->Process(
      *this, store, lootMode,
      lootOwner); // Processing is done there, callback via Loot::AddItem()
  ...
  if (!personal && group) {
    ...
    for (GroupReference *itr = group->GetFirstMember(); itr != nullptr;
         itr = itr->next())
      if (Player *player =
              itr->GetSource()) // should actually be looted object instead of
                                // lootOwner but looter has to be really close
                                // so doesnt really matter
        if (player->IsInMap(lootOwner)) // pussywizard: multithreading crashfix
          FillNotNormalLootFor(player,
                               player->IsAtGroupRewardDistance(lootOwner));
  ...
  for (uint8 i = 0; i < quest_items.size(); ++i) {
    ...
    if (!item.is_looted &&
      ...
      if (item.freeforall || !item.is_blocked)
        ++unlootedCount;
      if (!player->GetGroup() ||
          (player->GetGroup()->GetLootMethod() != GROUP_LOOT &&
           player->GetGroup()->GetLootMethod() != ROUND_ROBIN))
  ...
  for (PlayersLootingSet::iterator i = PlayersLooting.begin();
    ...
    if (Player *player = ObjectAccessor::FindPlayer(*i)) {
      QuestItemMap::const_iterator pq =
          PlayerQuestItems.find(player->GetGUIDLow());
...
LootItem *Loot::LootItemInSlot(uint32 lootSlot, Player *player,
                               QuestItem **qitem, QuestItem **ffaitem,
                               QuestItem **conditem) {
  ...
  if (lootSlot >= items.size()) {
    ...
    if (itr != PlayerQuestItems.end() && questSlot < itr->second->size()) {
      ...
      if (item->follow_loot_rules &&
          !item->AllowedForPlayer(
              player)) // pussywizard: such items (follow_loot_rules) are added
                       // to every player, but not everyone is allowed, check it
                       // here
        return nullptr;
    ...
    if (item->freeforall) {
      QuestItemMap::const_iterator itr =
          PlayerFFAItems.find(player->GetGUIDLow());
      ...
      QuestItemMap::const_iterator itr =
          PlayerNonQuestNonFFAConditionalItems.find(player->GetGUIDLow());
...
bool Loot::hasItemFor(Player *player) const {
  ...
  QuestItemMap::const_iterator q_itr =
      lootPlayerQuestItems.find(player->GetGUIDLow());
  ...
  QuestItemMap::const_iterator ffa_itr =
      lootPlayerFFAItems.find(player->GetGUIDLow());
...
ByteBuffer &operator<<(ByteBuffer &b, LootView const &lv) {
  ...
  QuestItemMap::const_iterator q_itr =
      lootPlayerQuestItems.find(lv.viewer->GetGUIDLow());
  if (q_itr != lootPlayerQuestItems.end()) {
    QuestItemList *q_list = q_itr->second;
    for (QuestItemList::const_iterator qi = q_list->begin();
         qi != q_list->end(); ++qi) {
  ...
  QuestItemMap::const_iterator ffa_itr =
      lootPlayerFFAItems.find(lv.viewer->GetGUIDLow());
  if (ffa_itr != lootPlayerFFAItems.end()) {
    QuestItemList *ffa_list = ffa_itr->second;
    for (QuestItemList::const_iterator fi = ffa_list->begin();
         fi != ffa_list->end(); ++fi) {
...
LootStoreItem const *LootTemplate::LootGroup::Roll(Loot &loot,
  ...
  possibleLoot.remove_if(LootGroupInvalidSelector(loot, lootMode));
  if (!possibleLoot.empty()) // If nothing selected yet - an item is taken from
                             // equal-chanced part
...
// the loot
void LootTemplate::LootGroup::Process(Loot &loot, Player const *player,
                                      LootStore const &store,
                                      uint16 lootMode) const {
...
void LootTemplate::LootGroup::Verify(LootStore const &lootstore, uint32 id,
  ...
  if (chance >= 100.0f && !EqualChanced.empty()) {
    LOG_ERROR("sql.sql",
              "Table '%s' entry %u group %d has items with chance=0%% but "
              "group total chance >= 100%% (%f)",
              lootstore.GetName(), id, group_id, chance);
  }
}
  ...
  for (LootStoreItemList::const_iterator i = Entries.begin();
    ...
    if (item->reference > 0) // References processing
    {
      LootTemplate const *Referenced =
          LootTemplates_Reference.GetLootFor(item->reference);
      if (!Referenced)
...
void LootTemplate::CheckLootRefs(LootTemplateMap const &store,
  ...
  for (LootGroups::const_iterator grItr = Groups.begin(); grItr != Groups.end();
       ++grItr)
...
void LoadLootTemplates_Creature() {
  ...
  for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
    ...
    if (uint32 lootid = itr->second.lootid) {
      if (lootIdSet.find(lootid) == lootIdSet.end())
        LootTemplates_Creature.ReportNonExistingId(lootid, "Creature",
                                                   itr->second.Entry);
      else
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u creature loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 creature loot templates. DB table "
...
void LoadLootTemplates_Disenchant() {
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u disenchanting loot templates in %u ms",
             count, GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 disenchanting loot templates. DB table "
...
void LoadLootTemplates_Fishing() {
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u fishing loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 fishing loot templates. DB table "
...
void LoadLootTemplates_Gameobject() {
  ...
  for (GameObjectTemplateContainer::const_iterator itr = gotc->begin();
    ...
    if (uint32 lootid = itr->second.GetLootId()) {
      if (lootIdSet.find(lootid) == lootIdSet.end())
        LootTemplates_Gameobject.ReportNonExistingId(lootid, "Gameobject",
                                                     itr->second.entry);
      else
        lootIdSetUsed.insert(lootid);
    }
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u gameobject loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 gameobject loot templates. DB table "
...
void LoadLootTemplates_Item() {
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u item loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 item loot templates. DB table "
...
void LoadLootTemplates_Milling() {
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u milling loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 milling loot templates. DB table "
...
void LoadLootTemplates_Pickpocketing() {
  ...
  for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
    ...
    if (uint32 lootid = itr->second.pickpocketLootId) {
      if (lootIdSet.find(lootid) == lootIdSet.end())
        LootTemplates_Pickpocketing.ReportNonExistingId(lootid, "Creature",
                                                        itr->second.Entry);
      else
        lootIdSetUsed.insert(lootid);
    }
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u pickpocketing loot templates in %u ms",
             count, GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 pickpocketing loot templates. DB table "
...
void LoadLootTemplates_Prospecting() {
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u prospecting loot templates in %u ms",
             count, GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 prospecting loot templates. DB table "
...
void LoadLootTemplates_Mail() {
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u mail loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 mail loot templates. DB table "
...
void LoadLootTemplates_Skinning() {
  ...
  for (CreatureTemplateContainer::const_iterator itr = ctc->begin();
    ...
    if (uint32 lootid = itr->second.SkinLootId) {
      if (lootIdSet.find(lootid) == lootIdSet.end())
        LootTemplates_Skinning.ReportNonExistingId(lootid, "Creature",
                                                   itr->second.Entry);
      else
        lootIdSetUsed.insert(lootid);
    }
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u skinning loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 skinning loot templates. DB table "
...
void LoadLootTemplates_Spell() {
  ...
  for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize();
    ...
    if (lootIdSet.find(spell_id) == lootIdSet.end()) {
      ...
      // ignore 61756 (Northrend Inscription Research (FAST QA VERSION) for
      // example
      if (!spellInfo->HasAttribute(SPELL_ATTR0_NOT_SHAPESHIFT) ||
          spellInfo->HasAttribute(SPELL_ATTR0_TRADESPELL)) {
        LootTemplates_Spell.ReportNonExistingId(spell_id, "Spell",
                                                spellInfo->Id);
      }
    } else
  ...
  if (count)
    LOG_INFO("server", ">> Loaded %u spell loot templates in %u ms", count,
             GetMSTimeDiffToNow(oldMSTime));
  else
    LOG_ERROR("sql.sql", ">> Loaded 0 spell loot templates. DB table "
...
void LoadLootTemplates_Reference() {
  ...
  LOG_INFO("server", ">> Loaded refence loot templates in %u ms",
           GetMSTimeDiffToNow(oldMSTime));
  LOG_INFO("server", " ");
}
