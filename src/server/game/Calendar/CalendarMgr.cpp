...
CalendarMgr::~CalendarMgr() {
  for (CalendarEventStore::iterator itr = _events.begin(); itr != _events.end();
       ++itr)
    delete *itr;

  for (CalendarEventInviteStore::iterator itr = _invites.begin();
       itr != _invites.end(); ++itr)
...
void CalendarMgr::LoadFromDB() {
  ...
  if (QueryResult result = CharacterDatabase.Query(
    ...
    do {
      ...
      CalendarEvent *calendarEvent = new CalendarEvent(
          eventId, creatorGUID, guildId, type, dungeonId, time_t(eventTime),
          flags, time_t(timezoneTime), title, description);
  ...
  //                                                       4       5 6      7
  if (QueryResult result = CharacterDatabase.Query(
          "SELECT id, event, invitee, sender, status, statustime, `rank`, text "
    ...
    do {
      ...
      uint64 senderGUID =
          MAKE_NEW_GUID(fields[3].GetUInt32(), 0, HIGHGUID_PLAYER);
      ...
      CalendarModerationRank rank =
          CalendarModerationRank(fields[6].GetUInt8());
      std::string text = fields[7].GetString();

      CalendarInvite *invite =
          new CalendarInvite(inviteId, eventId, invitee, senderGUID,
                             time_t(statusTime), status, rank, text);
...
void CalendarMgr::AddInvite(CalendarEvent *calendarEvent,
                            CalendarInvite *invite, SQLTransaction &trans) {
...
void CalendarMgr::RemoveEvent(CalendarEvent *calendarEvent, uint64 remover) {
  ...
  for (size_t i = 0; i < eventInvites.size(); ++i) {
    ...
    if (remover && invite->GetInviteeGUID() != remover)
      mail.SendMailTo(trans, MailReceiver(invite->GetInviteeGUID()),
...
void CalendarMgr::RemoveInvite(uint64 inviteId, uint64 eventId,
                               uint64 /*remover*/) {
  CalendarEvent *calendarEvent = GetEvent(eventId);
  ...
  if (!calendarEvent->IsGuildEvent())
    SendCalendarEventInviteRemoveAlert((*itr)->GetInviteeGUID(), *calendarEvent,
                                       CALENDAR_STATUS_REMOVED);

  SendCalendarEventInviteRemove(*calendarEvent, **itr,
                                calendarEvent->GetFlags());
...
CalendarInvite *CalendarMgr::GetInvite(uint64 inviteId) const {
  ...
  LOG_DEBUG("entities.unit",
            "CalendarMgr::GetInvite: [" UI64FMTD "] not found!", inviteId);
#endif
  return nullptr;
}
...
CalendarEventStore CalendarMgr::GetEventsCreatedBy(uint64 guid,
  ...
  for (CalendarEventInviteStore::const_iterator itr = _invites.begin();
    ...
    for (CalendarInviteStore::const_iterator itr2 = itr->second.begin();
      ...
      if ((*itr2)->GetInviteeGUID() == guid)
        if (CalendarEvent *event = GetEvent(
                itr->first)) // nullptr check added as attempt to fix #11512
          events.insert(event);
...
void CalendarMgr::SendCalendarEventInvite(CalendarInvite const &invite) {
  ...
  uint8 level =
      player ? player->getLevel() : Player::GetLevelFromStorage(invitee);
  ...
  data << uint8(invite.GetSenderGUID() !=
                invite.GetInviteeGUID()); // false only if the invite is sign-up

  if (!calendarEvent) // Pre-invite
  {
    if (Player *player =
            ObjectAccessor::FindPlayerInOrOutOfWorld(invite.GetSenderGUID()))
...
void CalendarMgr::SendCalendarEventUpdateAlert(
  ...
  WorldPacket data(SMSG_CALENDAR_EVENT_UPDATED_ALERT,
                   1 + 8 + 4 + 4 + 4 + 1 + 4 + calendarEvent.GetTitle().size() +
...
void CalendarMgr::SendCalendarEventRemovedAlert(
  ...
  data << uint8(
      1); // FIXME: If true does not SignalEvent(EVENT_CALENDAR_ACTION_PENDING)
  data << uint64(calendarEvent.GetEventId());
  data.AppendPackedTime(calendarEvent.GetEventTime());
  ...
  if (calendarEvent.IsGuildEvent() || calendarEvent.IsGuildAnnouncement()) {
    if (Guild *guild = sGuildMgr->GetGuildById(calendarEvent.GetGuildId()))
      guild->BroadcastPacket(&data);
                 ...
                 invite.GetInviteeGUID()))
    player->SendDirectMessage(&data);
}

void CalendarMgr::SendCalendarEvent(uint64 guid,
                                    CalendarEvent const &calendarEvent,
                                    CalendarSendEventType sendType) {
  ...
  for (CalendarInviteStore::const_iterator itr = eventInviteeList.begin();
    ...
    uint8 inviteeLevel = invitee ? invitee->getLevel()
                                 : Player::GetLevelFromStorage(inviteeGuid);
    uint32 inviteeGuildId =
        invitee ? invitee->GetGuildId()
                : Player::GetGuildIdFromStorage(GUID_LOPART(inviteeGuid));
...
void CalendarMgr::SendCalendarEventInviteRemoveAlert(
    uint64 guid, CalendarEvent const &calendarEvent,
...
void CalendarMgr::SendCalendarCommandResult(uint64 guid, CalendarError err,
...
void CalendarMgr::SendPacketToAllEventRelatives(
  ...
  for (CalendarInviteStore::iterator itr = invites.begin();
       itr != invites.end(); ++itr)
    if (Player *player =
            ObjectAccessor::FindPlayerInOrOutOfWorld((*itr)->GetInviteeGUID()))
      if (!calendarEvent.IsGuildEvent() ||
