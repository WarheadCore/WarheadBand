...
void LFGPlayerScript::OnLogout(Player *player) {
  ...
  if (!player->GetGroup() || !player->GetGroup()->isLFGGroup()) {
    ...
    sLFGMgr->LeaveAllLfgQueues(
        player->GetGUID(), true,
        player->GetGroup() ? player->GetGroup()->GetGUID() : 0);
...
void LFGPlayerScript::OnLogin(Player *player) {
  ...
  if (Group const *group = player->GetGroup()) {
    ...
    if (gguid != gguid2) {
      // LOG_ERROR("server", "%s on group %u but LFG has group %u saved...
      // Fixing.",
      //    player->GetSession()->GetPlayerInfo().c_str(), GUID_LOPART(gguid2),
      //    GUID_LOPART(gguid));
  ...
  // TODO - Restore LfgPlayerData and send proper status to player if it was in
  // a group
}

void LFGPlayerScript::OnBindToInstance(Player *player, Difficulty difficulty,
...
void LFGPlayerScript::OnMapChanged(Player *player) {
  ...
  if (sLFGMgr->inLfgDungeonMap(player->GetGUID(), map->GetId(),
                               map->GetDifficulty())) {
    Group *group = player->GetGroup();
    // This function is also called when players log in
    // if for some reason the LFG system recognises the player as being in a LFG
    // dungeon, but the player was loaded without a valid group, we'll teleport
    // to homebind to prevent crashes or other undefined behaviour
    if (!group) {
      sLFGMgr->LeaveLfg(player->GetGUID());
      ...
      LOG_DEBUG(
          "lfg",
          "LFGPlayerScript::OnMapChanged, Player %s (%u) is in LFG dungeon map "
          "but does not have a valid group! Teleporting to homebind.",
...
void LFGGroupScript::OnAddMember(Group *group, uint64 guid) {
  ...
  if (leader == guid) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("lfg",
              "LFGScripts::OnAddMember [" UI64FMTD "]: added [" UI64FMTD
              "] leader " UI64FMTD "]",
              gguid, guid, leader);
#endif
    ...
    LOG_DEBUG("lfg",
              "LFGScripts::OnAddMember [" UI64FMTD "]: added [" UI64FMTD
              "] leader " UI64FMTD "] gstate: %u, state: %u",
  ...
  if (!group->isLFGGroup()) {
    sLFGMgr->LeaveAllLfgQueues(
        leader, true,
        gguid); // pussywizard: invited, queued, party formed, neither party nor
                // new member are queued, but leader is in queue solo!
...
void LFGGroupScript::OnRemoveMember(Group *group, uint64 guid,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("lfg",
            "LFGScripts::OnRemoveMember [" UI64FMTD "]: remove [" UI64FMTD
            "] Method: %d Kicker: [" UI64FMTD "] Reason: %s",
  ...
  if (state !=
      LFG_STATE_FINISHED_DUNGEON) // Need more players to finish the dungeon
    if (Player *leader =
            ObjectAccessor::FindPlayerInOrOutOfWorld(sLFGMgr->GetLeader(gguid)))
...
void LFGGroupScript::OnChangeLeader(Group *group, uint64 newLeaderGuid,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("lfg",
            "LFGScripts::OnChangeLeader [" UI64FMTD "]: old [" UI64FMTD
            "] new [" UI64FMTD "]",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("lfg",
            "LFGScripts::OnInviteMember [" UI64FMTD "]: invite [" UI64FMTD
            "] leader [" UI64FMTD "]",
            gguid, guid, leader);
#endif
