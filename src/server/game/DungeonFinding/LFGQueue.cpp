...
void LFGQueue::AddToQueue(uint64 guid, bool failedProposal) {
  // LOG_INFO("server", "ADD AddToQueue: %u, failed proposal: %u",
  // GUID_LOPART(guid), failedProposal ? 1 : 0);
  LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
  if (itQueue == QueueDataStore.end()) {
    LOG_ERROR("server",
              "LFGQueue::AddToQueue: Queue data not found for [" UI64FMTD "]",
              guid);
    return;
  }
...
void LFGQueue::RemoveFromQueue(uint64 guid, bool partial) {
  // LOG_INFO("server", "REMOVE RemoveFromQueue: %u, partial: %u",
  // GUID_LOPART(guid), partial ? 1 : 0);
  ...
  for (LfgQueueDataContainer::iterator itr = QueueDataStore.begin();
       itr != QueueDataStore.end(); ++itr) {
    if (itr->first != guid) {
      if (itr->second.bestCompatible.hasGuid(guid)) {
        // LOG_INFO("server", "CLEAR bestCompatible: %s, because of guid: %u",
        // itr->second.bestCompatible.toString().c_str(), GUID_LOPART(guid));
        itr->second.bestCompatible.clear();
      }
    } else {
      // LOG_INFO("server", "CLEAR bestCompatible SELF: %s, because of guid:
      // %u", itr->second.bestCompatible.toString().c_str(), GUID_LOPART(guid));
  ...
  if (!partial && itDelete != QueueDataStore.end()) {
    ...
    // LOG_INFO("server", "ERASE QueueDataStore for: %u, itDelete: %u,%u,%u",
    // GUID_LOPART(guid), itDelete->second.dps, itDelete->second.healers,
    // itDelete->second.tanks);
    QueueDataStore.erase(itDelete);
    // LOG_INFO("server", "ERASE QueueDataStore for: %u SUCCESS",
    // GUID_LOPART(guid));
  }
}
...
void LFGQueue::AddToNewQueue(uint64 guid, bool front) {
  if (front) {
    ...
    // LOG_INFO("server", "ADD AddToNewQueue at the END: %u",
    // GUID_LOPART(guid));
...
void LFGQueue::AddQueueData(uint64 guid, time_t joinTime,
...
void LFGQueue::RemoveFromCompatibles(uint64 guid) {
  ...
  for (LfgCompatibleContainer::iterator it = CompatibleList.begin();
       it != CompatibleList.end(); ++it)
    if (it->hasGuid(guid)) {
      // LOG_INFO("server", "Removed Compatible: %s, because of guid: %u",
      // it->toString().c_str(), GUID_LOPART(guid));
  ...
  for (LfgCompatibleContainer::iterator itr = CompatibleTempList.begin();
    ...
    if (it->hasGuid(guid)) {
      // LOG_INFO("server", "Erased Temp Compatible: %s, because of guid: %u",
      // it->toString().c_str(), GUID_LOPART(guid));
...
uint8 LFGQueue::FindGroups() {
  ...
  if (!newToQueueStore.empty()) {
    ...
    bool pushCompatiblesToFront =
        (std::find(restoredAfterProposal.begin(), restoredAfterProposal.end(),
                   newGuid) != restoredAfterProposal.end());
    // LOG_INFO("server", "newToQueueStore guid: %u, front: %u",
    // GUID_LOPART(newGuid), pushCompatiblesToFront ? 1 : 0);
    ...
    return newGroupsProcessed; // pussywizard: only one per update, shouldn't be
                               // a problem
  }
...
LfgCompatibility LFGQueue::FindNewGroups(const uint64 &newGuid) {
  ...
  // combination was found once, second 16 bits for marking second occurence of
  // that combination, etc
  uint64 foundMask = 0;
  uint32 foundCount = 0;
  ...
  if (currentCompatibles.empty()) {
    selfCompatibility = CheckCompatibility(Lfg5Guids(), newGuid, foundMask,
                                           foundCount, currentCompatibles);
  ...
  for (Lfg5GuidsList::iterator it = CompatibleList.begin();
    ...
    if ((foundMask & 0x3FFF3FFF3FFF3FFF) ==
        0x3FFF3FFF3FFF3FFF) // each combination of dps+heal+tank already found 4
                            // times
      break;
  }
...
LfgCompatibility
LFGQueue::CheckCompatibility(Lfg5Guids const &checkWith, const uint64 &newGuid,
                             uint64 &foundMask, uint32 &foundCount,
                             const std::set<Lfg5Guids> &currentCompatibles) {
  // LOG_INFO("server", "CHECK CheckCompatibility: %s, new guid: %u",
  // checkWith.toString().c_str(), GUID_LOPART(newGuid));
  ...
  for (uint8 i = 0; i < 5 && (guid = check.guid[i]) != 0 && numLfgGroups < 2 &&
                    numPlayers <= MAXGROUPSIZE;
       ++i) {
    LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
    if (itQueue == QueueDataStore.end()) {
      LOG_ERROR("server",
                "LFGQueue::CheckCompatibility: [" UI64FMTD
                "] is not queued but listed as queued!",
                guid);
      RemoveFromQueue(guid);
      return LFG_COMPATIBILITY_PENDING;
    }

    // Store group so we don't need to call Mgr to get it later (if it's player
    // group will be 0 otherwise would have joined as group)
    for (LfgRolesMap::const_iterator it2 = itQueue->second.roles.begin();
         it2 != itQueue->second.roles.end(); ++it2)
  ...
  if (check.size() > 1) {
    for (uint8 i = 0; i < 5 && check.guid[i]; ++i) {
      ...
      for (LfgRolesMap::const_iterator itRoles = roles.begin();
           itRoles != roles.end(); ++itRoles) {
        LfgRolesMap::const_iterator itPlayer;
        for (itPlayer = proposalRoles.begin(); itPlayer != proposalRoles.end();
             ++itPlayer) {
          if (itRoles->first == itPlayer->first) {
            // pussywizard: LFG ZOMG! this means that this player was in two
            // different LfgQueueData (in QueueDataStore), and at least one of
            // them is a group guid, because we do checks so there aren't 2 same
            // guids in current CHECK
            // LOG_ERROR("server", "LFGQueue::CheckCompatibility: ERROR! Player
            // multiple times in queue! [" UI64FMTD "]", itRoles->first);
            break;
      ...
      if (foundMask & (((uint64)1) << (16 + roleCheckResult - 1))) {
        if (foundMask & (((uint64)1) << (32 + roleCheckResult - 1))) {
          if (foundMask & (((uint64)1) << (48 + roleCheckResult - 1))) {
            if (foundCount >=
                10) // but only after finding at least 10 compatibles (this
                    // helps when there are few groups)
              return LFG_INCOMPATIBLES_NO_ROLES;
          } else
    ...
    for (uint8 i = 1; i < 5 && check.guid[i]; ++i) {
      ...
      std::set_intersection(proposalDungeons.begin(), proposalDungeons.end(),
  ...
  if (numPlayers != MAXGROUPSIZE) {
    ...
    for (uint8 i = 0; i < 5 && check.guid[i]; ++i) {
      ...
      if (!itr->second.bestCompatible
               .empty()) // update if groups don't have it empty (for empty it
                         // will be generated in UpdateQueueTimers)
  ...
  proposal.isNew =
      numLfgGroups != 1 || sLFGMgr->GetOldState(gguid) != LFG_STATE_DUNGEON;
  ...
  proposal.leader = 0;
  proposal.dungeonId =
      Warhead::Containers::SelectRandomContainerElement(proposalDungeons);

  bool leader = false;
  for (LfgRolesMap::const_iterator itRoles = proposalRoles.begin();
       itRoles != proposalRoles.end(); ++itRoles) {
    // Assing new leader
...
void LFGQueue::UpdateQueueTimers(uint32 diff) {
  ...
  if (!sendQueueStatus) {
    for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin();
      ...
      if (itQueue->second.bestCompatible.empty()) {
        ...
        if (numOfCompatibles /*must be positive, because proposals don't delete
                                QueueQueueData*/
            && currTime - itQueue->second.lastRefreshTime >= 60 &&
            numOfCompatibles <
                (5 - itQueue->second.bestCompatible.roles->size()) * 25) {
  ...
  for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin();
       itQueue != QueueDataStore.end(); ++itQueue) {
    ...
    for (LfgRolesMap::const_iterator itPlayer = queueinfo.roles.begin();
         itPlayer != queueinfo.roles.end(); ++itPlayer)
    ...
    LfgQueueStatusData queueData(dungeonId, waitTime, wtAvg, wtTank, wtHealer,
                                 wtDps, queuedTime, queueinfo.tanks,
                                 queueinfo.healers, queueinfo.dps);
    for (LfgRolesMap::const_iterator itPlayer = queueinfo.roles.begin();
         itPlayer != queueinfo.roles.end(); ++itPlayer) {
...
void LFGQueue::UpdateBestCompatibleInQueue(
  ...
  // LOG_INFO("server", "UpdateBestCompatibleInQueue: %s",
  // key.toString().c_str());
