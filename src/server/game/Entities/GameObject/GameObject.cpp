...
GameObject::~GameObject() {
  delete m_AI;
  delete m_model;
  // if (m_uint32Values)                                      // field array can
  // be not exist if GameOBject not loaded
...
void GameObject::RemoveFromOwner() {
  ...
  LOG_FATAL("server", "Delete GameObject (GUID: %u Entry: %u SpellId %u LinkedGO
  %u) that lost references to owner (GUID %u Type '%s') GO list. Crash possible
  later.", GetGUIDLow(), GetGOInfo()->entry, m_spellId,
  GetGOInfo()->GetLinkedGameObjectEntry(), GUID_LOPART(ownerGUID), ownerType);*/
  SetOwnerGUID(0);
}
...
void GameObject::CheckRitualList() {
  if (m_unique_users.empty())
    return;
  for (std::set<uint64>::iterator itr = m_unique_users.begin();
...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map *map,
                        uint32 phaseMask, float x, float y, float z, float ang,
                        G3D::Quat const &rotation, uint32 animprogress,
  ...
  if (!goinfo) {
    LOG_ERROR("sql.sql",
              "Gameobject (GUID: %u Entry: %u) not created: non-existing entry "
              "in `gameobject_template`. Map: %u (X: %f Y: %f Z: %f)",
              guidlow, name_id, map->GetId(), x, y, z);
    return false;
  }
  ...
  if (goinfo->type >= MAX_GAMEOBJECT_TYPE) {
    LOG_ERROR(
        "sql.sql",
        "Gameobject (GUID: %u Entry: %u) not created: non-existing GO type "
        "'%u' in `gameobject_template`. It will crash client if created.",
  ...
  default:
    ...
    // (using special gameobject addon data) pussywizard: temporarily calculate
    // WorldRotation from orientation, do so until values in db are correct
    if (addon && addon->invisibilityType == INVISIBILITY_GENERAL &&
        addon->InvisibilityValue == 0)
      SetWorldRotation(rotation);
    else
      SetWorldRotationAngles(NormalizeOrientation(GetOrientation()), 0.0f,
                             0.0f);
    break;
  }
...
void GameObject::Update(uint32 diff) {
  ...
  case GO_NOT_READY: {
    ...
    case GAMEOBJECT_TYPE_TRAP: {
      ...
      if (goInfo->trap.type == 2)
        m_cooldownTime = World::GetGameTimeMS() +
                         10 * IN_MILLISECONDS; // Hardcoded tooltip value
      else if (GetOwner())
        m_cooldownTime =
            World::GetGameTimeMS() + goInfo->trap.startDelay * IN_MILLISECONDS;
    ...
    case GAMEOBJECT_TYPE_SUMMONING_RITUAL: {
      ...
      if (info->summoningRitual.animSpell) {
        // xinef: if ritual requires animation, ensure that all users performs
        // channel
        CheckRitualList();
      }
      ...
      Unit *owner = GetOwner();
      Unit *spellCaster = owner ? owner
                                : ObjectAccessor::GetPlayer(
                                      *this, MAKE_NEW_GUID(m_ritualOwnerGUIDLow,
                                                           0, HIGHGUID_PLAYER));
      if (!spellCaster) {
      ...
      // on the current DB there is only one gameobject that uses this (Ritual
      // of Doom) and its required target number is 1 (outter for loop will run
      // once)
      if (info->summoningRitual.casterTargetSpell &&
        ...
        for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets;
             i++)
          // m_unique_users can contain only player GUIDs
          if (Player *target = ObjectAccessor::GetPlayer(
                  *this, Warhead::Containers::SelectRandomContainerElement(
                             m_unique_users)))
            spellCaster->CastSpell(
                target, info->summoningRitual.casterTargetSpell, true);
    ...
    if (m_respawnTime > 0) // timer on
    {
      ...
      if (m_respawnTime <= now) // timer expired
      {
        ...
        case GAMEOBJECT_TYPE_BUTTON:
          // we need to open doors if they are closed (add there another
          // condition if this code breaks some usage, but it need to be here for
          // battlegrounds)
        ...
        // Xinef: Call AI Reset (required for example in SmartAI to clear one
        // time events)
        if (AI())
          AI()->Reset();
      ...
      if (goInfo->type == GAMEOBJECT_TYPE_TRAP) {
        ...
        if (goInfo->trap.type == 2) {
          if (goInfo->trap.spellId)
            CastSpell(nullptr,
                      goInfo->trap.spellId); // FIXME: null target won't work
                                             // for target type 1
          SetLootState(GO_JUST_DEACTIVATED);
          break;
        }
        ...
        /// @todo This is activation radius. Casting radius must be selected
        /// from spell data.
        /// @todo Move activated state code to GO_ACTIVATED, in this place just
        /// check for activation and set state.
        ...
        // Type 0 and 1 - trap (type 0 will not get removed after casting a
        // spell)
        Unit *owner = GetOwner();
        ...
        {
          Warhead::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(
              this, owner, radius);
          Warhead::UnitSearcher<
              Warhead::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>
        ...
        {
          // environmental damage spells already have around enemies targeting
          // but this not help in case not existed GO casting support affect
          // only players
          Player *player = nullptr;
          Warhead::AnyPlayerInObjectRangeCheck checker(this, radius, true,
                                                       true);
  ...
  case GO_ACTIVATED: {
    ...
    case GAMEOBJECT_TYPE_BUTTON:
      if (GetGOInfo()->GetAutoCloseTime() &&
          World::GetGameTimeMS() >= m_cooldownTime)
        ResetDoorOrButton();
      break;
  ...
  case GO_JUST_DEACTIVATED: {
    ...
    if (GetGoType() == GAMEOBJECT_TYPE_GOOBER) {
      ...
      // Xinef: Do not return here for summoned gos that should be deleted few
      // lines below Xinef: Battleground objects are treated as spawned by
      // default
      if (GameObjectTemplateAddon const *addon = GetTemplateAddon())
    ...
    //! If this is summoned by a spell with ie. SPELL_EFFECT_SUMMON_OBJECT_WILD,
    //! with or without owner, we check respawn criteria based on spell The
    //! GetOwnerGUID() check is mostly for compatibility with hacky scripts -
    //! 99% of the time summoning should be done trough spells.
    ...
    // burning flags in some battlegrounds, if you find better condition, just
    // add it
...
void GameObject::getFishLoot(Loot *fishloot, Player *loot_owner) {
                         ...
                         // fishloot->FillLoot return true. it wrong.
  {
    ...
    // use zone 1 as default, somewhere fishing got nothing,becase subzone and
    // zone not set, like Off the coast of Storm Peaks.
    if (fishloot->empty())
      fishloot->FillLoot(defaultzone, LootTemplates_Fishing, loot_owner, true,
                         true);
  }
}
...
bool GameObject::LoadGameObjectFromDB(uint32 guid, Map *map, bool addToMap) {
  ...
  if (!Create(guid, entry, map, phaseMask, x, y, z, ang, data->rotation,
              animprogress, go_state, artKit))
    return false;
...
void GameObject::SaveRespawnTime() {
  if (m_goData && m_goData->dbData && m_respawnTime > time(nullptr) &&
      m_spawnedByDefault)
...
bool GameObject::ActivateToQuest(Player *target) const {
  ...
  case GAMEOBJECT_TYPE_CHEST: {
    ...
    if (LootTemplates_Gameobject.HaveQuestLootForPlayer(
      ...
      // TODO: fix this hack
      // look for battlegroundAV for some objects which are only activated after
      // mine gots captured by own team
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit *target) {
  ...
  GameObject *trapGO = nullptr;
  {
    ...
    Warhead::NearestGameObjectEntryInObjectRangeCheck go_check(
        *target, trapEntry, range);
    Warhead::GameObjectLastSearcher<
        Warhead::NearestGameObjectEntryInObjectRangeCheck>
        checker(this, trapGO, go_check);

    TypeContainerVisitor<Warhead::GameObjectLastSearcher<
                             Warhead::NearestGameObjectEntryInObjectRangeCheck>,
...
GameObject *GameObject::LookupFishingHoleAround(float range) {
  ...
  Warhead::GameObjectSearcher<Warhead::NearestGameObjectFishingHole> checker(
      this, ok, u_check);

  TypeContainerVisitor<
      Warhead::GameObjectSearcher<Warhead::NearestGameObjectFishingHole>,
      GridTypeMapContainer>
...
void GameObject::SetGoArtKit(uint8 kit) {
  ...
  GameObjectData *data =
      const_cast<GameObjectData *>(sObjectMgr->GetGOData(m_DBTableGuid));
  if (data)
    data->artKit = kit;
}
...
void GameObject::Use(Unit *user) {
  ...
  case GAMEOBJECT_TYPE_CHAIR: // 7
  {
    ...
    {
      if (info->chair.slots > 0) // sometimes chairs in DB have error in fields
                                 // and we dont know number of slots
        for (uint32 i = 0; i < info->chair.slots; ++i)
          ChairListSlots[i] =
              0; // Last user of current slot set to 0 (none sit here yet)
      else
    ...
    for (ChairSlotAndUser::iterator itr = ChairListSlots.begin();
      ...
      // the distance between this slot and the center of the go - imagine a 1D
      // space
      ...
      float y_i =
          GetPositionY() + relativeDistance * sin(orthogonalOrientation);
      ...
      if (itr->second) {
        if (Player *ChairUser = ObjectAccessor::GetPlayer(*this, itr->second)) {
          if (ChairUser->IsSitState() &&
            ...
            continue; // This seat is already occupied by ChairUser. NOTE: Not
                      // sure if the ChairUser->getStandState() !=
                      // UNIT_STAND_STATE_SIT check is required.
          else
            itr->second = 0; // This seat is unoccupied.
        } else
          itr->second =
              0; // The seat may of had an occupant, but they're offline.
      }
    ...
    if (found_free_slot) {
      ...
      if (itr != ChairListSlots.end()) {
        ...
        player->TeleportTo(
            GetMapId(), x_lowest, y_lowest, GetPositionZ(), GetOrientation(),
...
#endif
        GetMap()->ScriptsStart(sEventScripts, info->goober.eventId, player,
                               this);
  ...
  case GAMEOBJECT_TYPE_FISHINGNODE: // 17
  {
    ...
    case GO_READY: // ready for loot
    {
      ...
      // but you will likely cause junk in areas that require a high fishing
      ...
      if (chance >= roll) {
        ...
        SetSpellId(
            0); // prevent removing unintended auras at Unit::RemoveGameObject
  ...
  case GAMEOBJECT_TYPE_SUMMONING_RITUAL: // 18
  {
    ...
    if (owner) {
      ...
      // accept only use by player from same group as owner, excluding owner
      // itself (unique use already added in spell effect)
      ...
      Player *ritualOwner = ObjectAccessor::GetPlayer(
          *this, MAKE_NEW_GUID(m_ritualOwnerGUIDLow, 0, HIGHGUID_PLAYER));
      if (!ritualOwner)
        return;
      if (player != ritualOwner && (info->summoningRitual.castersGrouped &&
  ...
  case GAMEOBJECT_TYPE_SPELLCASTER: // 22
  {
    ...
    if (info->spellcaster.partyOnly) {
      Player const *caster = ObjectAccessor::GetObjectInOrOutOfWorld(
          GetOwnerGUID(), (Player *)nullptr);
  ...
  case GAMEOBJECT_TYPE_BARBER_CHAIR: // 32
  {
    ...
    player->TeleportTo(GetMapId(), GetPositionX(), GetPositionY(),
  ...
  default:
    if (GetGoType() >= MAX_GAMEOBJECT_TYPE)
      LOG_ERROR(
          "server",
          "GameObject::Use(): unit (type: %u, guid: %u, name: %s) tries to use "
          "object (guid: %u, entry: %u, name: %s) of unknown type (%u)",
          user->GetTypeId(), user->GetGUIDLow(), user->GetName().c_str(),
          GetGUIDLow(), GetEntry(), GetGOInfo()->name.c_str(), GetGoType());
    break;
  }
...
void GameObject::CastSpell(Unit *target, uint32 spellId) {
  ...
  Creature *trigger =
      SummonTrigger(GetPositionX(), GetPositionY(), GetPositionZ(), 0,
                    spellInfo->CalcCastTime() + 2000, true);
  if (!trigger)
    return;
...
bool GameObject::IsInRange(float x, float y, float z, float radius) const {
  ...
  //! objects are on the same position. The code below this check wont crash if
  //! dist is 0 because 0/0 in float operations is valid, and returns infinite
...
// pussywizard!
void GameObject::SendMessageToSetInRange(WorldPacket *data, float dist,
  ...
  if (includeMargin)
    dist +=
        ...
        2.0f; // pussywizard: to ensure everyone receives all important packets
...
std::string const &
GameObject::GetNameForLocaleIdx(LocaleConstant loc_idx) const {
...
void GameObject::SetTransportPathRotation(float qx, float qy, float qz,
                                          float qw) {
...
void GameObject::ModifyHealth(int32 change,
  ...
  if (player) {
    ...
    data << uint32(
        -change); // change  < 0 triggers SPELL_BUILDING_HEAL combat log event
  ...
  case GO_DESTRUCTIBLE_DAMAGED: {
    ...
    if (BattlegroundMap *bgMap = GetMap()->ToBattlegroundMap())
      if (Battleground *bg = bgMap->GetBG())
        bg->EventPlayerDamagedGO(eventInvoker, this,
...
void GameObject::SetLootState(LootState state, Unit *unit) {
  ...
  GetGoType() == GAMEOBJECT_TYPE_BUTTON ? GetGOInfo()->door.startOpen : false);
...
void GameObject::SetGoState(GOState state) {
  ...
  if (m_model) {
    ...
    // pussywizard: this startOpen is unneeded here, collision depends entirely
    // on current GOState
...
bool GameObject::IsLootAllowedFor(Player const *player) const {
  ...
  if (!playerGroup ||
      playerGroup != GetLootRecipientGroup()) // if we dont have a group we
                                              // arent the recipient
    return false; // if go doesnt have group bound it means it was solo killed
                  // by someone else

  return true;
}

void GameObject::BuildValuesUpdate(uint8 updateType, ByteBuffer *data,
                                   Player *target) const {
  if (!target)
    return;
            ...
            if (t->GetPauseTime()) {
              if (GetGoState() == GO_STATE_READY) {
                if (t->GetPathProgress() >=
                    t->GetPauseTime()) // if not, send 100% progress
                  pathProgress = int16(
                      float(t->GetPathProgress() - t->GetPauseTime()) /
...
void GameObject::GetRespawnPosition(float &x, float &y, float &z,
                                    float *ori /* = nullptr*/) const {
  if (m_DBTableGuid) {
    if (GameObjectData const *data =
            sObjectMgr->GetGOData(GetDBTableGUIDLow())) {
      x = data->posX;
      y = data->posY;
      z = data->posZ;
      if (ori)
...
float GameObject::GetInteractionDistance() {
  switch (GetGoType()) {
  /// @todo find out how the client calculates the maximal usage distance to
  /// spellless working
  // gameobjects like guildbanks and mailboxes - 10.0 is a just an abitrary
  // choosen number
