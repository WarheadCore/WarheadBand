...
void LoadRandomEnchantmentsTable() {
  ...
  if (result) {
    ...
    LOG_INFO("server", ">> Loaded %u Item Enchantment definitions in %u ms",
             count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server", " ");
  } else {
    LOG_ERROR("sql.sql", ">> Loaded 0 Item Enchantment definitions. DB table "
...
uint32 GetItemEnchantMod(int32 entry) {
  ...
  if (tab == RandomItemEnch.end()) {
    LOG_ERROR(
        "sql.sql",
        "Item RandomProperty / RandomSuffix id #%u used in `item_template` but "
        "it does not have records in `item_enchantment_template` table.",
        entry);
    return 0;
  }
