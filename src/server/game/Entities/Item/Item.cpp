  ...
  for (uint32 x = 0; x < MAX_ITEM_SET_SPELLS; ++x) {
    ...
    for (uint32 y = 0; y < MAX_ITEM_SET_SPELLS; ++y) {
      if (!eff->spells[y]) // free slot
      {
        ...
        // spell casted only if fit form requirement, in other case will casted
        // at form change
...
bool Item::LoadFromDB(uint32 guid, uint64 owner_guid, Field *fields,
  ...
  //                                                    4      5             6
  //                                                    7           8 9    10
  // result = CharacterDatabase.PQuery("SELECT creatorGuid, giftCreatorGuid,
  // count, duration, charges, flags, enchantments, randomPropertyId, durability,
  // playedTime, text FROM item_instance WHERE guid = '%u'", guid);

  // create item before any checks for store correct guid
  // and allow use "FSetState(ITEM_REMOVED); SaveToDB();" for deleting item from
  // DB
  ...
  _LoadIntoDataField(enchants.c_str(), ITEM_FIELD_ENCHANTMENT_1_1,
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id) {
  ...
  if ((itemProto->RandomProperty) && (itemProto->RandomSuffix)) {
    LOG_ERROR("sql.sql",
              "Item template %u have RandomProperty == %u and RandomSuffix == "
              "%u, but must have one from field =0",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("entities.player.items",
              "Item::AddToUpdateQueueOf - Owner's guid (%u) and player's guid "
              "(%u) don't match!",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("entities.player.items",
              "Item::RemoveFromUpdateQueueOf - Owner's guid (%u) and player's "
              "guid (%u) don't match!",
              GUID_LOPART(GetOwnerGUID()), player->GetGUIDLow());
#endif
    return;
...
void Item::SetEnchantment(EnchantmentSlot slot, uint32 id, uint32 duration,
                          uint32 charges, uint64 caster /*= 0*/) {
  ...
  if (slot < MAX_INSPECTED_ENCHANTMENT_SLOT) {
    if (uint32 oldEnchant = GetEnchantmentId(slot))
      owner->GetSession()->SendEnchantmentLog(GetOwnerGUID(), 0, GetEntry(),
                                              oldEnchant);

    if (id)
      owner->GetSession()->SendEnchantmentLog(GetOwnerGUID(), caster,
                                              GetEntry(), id);
  }
...
void Item::SetEnchantmentDuration(EnchantmentSlot slot, uint32 duration,
  ...
  SetUInt32Value(ITEM_FIELD_ENCHANTMENT_1_1 + slot * MAX_ENCHANTMENT_OFFSET +
                 ...
                 duration);
  SetState(ITEM_CHANGED, owner);
  // Cannot use GetOwner() here, has to be passed as an argument to avoid freeze
  // due to hashtable locking
}
...
Item *Item::CreateItem(uint32 item, uint32 count, Player const *player,
                       bool clone, uint32 randomPropertyId) {
  if (count < 1)
  ...
  if (pProto) {
    ...
    if (pItem->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM), item,
                      player)) {
...
Item *Item::CloneItem(uint32 count, Player const *player) const {
  ...
  Item *newItem = CreateItem(GetEntry(), count, player, true,
