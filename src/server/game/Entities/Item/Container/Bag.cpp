...
Bag::~Bag() {
  for (uint8 i = 0; i < MAX_BAG_SIZE; ++i)
    if (Item *item = m_bagslot[i]) {
      if (item->IsInWorld()) {
        LOG_FATAL("server",
                  "Item %u (slot %u, bag slot %u) in bag %u (slot %u, bag slot "
                  "%u, m_bagslot %u) is to be deleted but is still in world.",
                  item->GetEntry(), (uint32)item->GetSlot(),
                  (uint32)item->GetBagSlot(), GetEntry(), (uint32)GetSlot(),
                  (uint32)GetBagSlot(), (uint32)i);
...
void Bag::SaveToDB(SQLTransaction &trans) { Item::SaveToDB(trans); }
...
bool Bag::LoadFromDB(uint32 guid, uint64 owner_guid, Field *fields,
  ...
  SetUInt32Value(CONTAINER_FIELD_NUM_SLOTS, itemProto->ContainerSlots);
  // cleanup bag content related item value fields (its will be filled correctly
