...
bool Corpse::Create(uint32 guidlow, Player *owner) {
  ...
  if (!IsPositionValid()) {
    LOG_ERROR("server",
              "Corpse (guidlow %d, owner %s) not created. Suggested "
              "coordinates isn't valid (X: %f Y: %f)",
              guidlow, owner->GetName().c_str(), owner->GetPositionX(),
              owner->GetPositionY());
    return false;
  }
...
void Corpse::SaveToDB() {
  ...
  stmt->setString(
      8, _ConcatFields(CORPSE_FIELD_ITEM, EQUIPMENT_SLOT_END));   // itemCache
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field *fields) {
  ...
  // SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, bytes1,
  // bytes2, guildId, flags, dynFlags, time, corpseType, instanceId, phaseMask,
  // corpseGuid, guid FROM corpse WHERE corpseType <> 0
