...
Pet::Pet(Player *owner, PetType type)
    : Guardian(nullptr, owner ? owner->GetGUID() : 0, true),
      ...
      m_petRegenTimer(PET_FOCUS_REGEN_INTERVAL), m_declinedname(nullptr),
      m_tempspellTarget(nullptr), m_tempoldTarget(nullptr),
      m_tempspellIsPositive(false), m_tempspell(0),
...
void Pet::AddToWorld() {
  ...
  if (IS_PLAYER_GUID(GetOwnerGUID()) && GetMapId() == 631 && FindMap() &&
...
SpellCastResult
Pet::TryLoadFromDB(Player *owner, bool current /*= false*/,
  ...
  stmt->setUInt8(1,
                 uint8(current ? PET_SAVE_AS_CURRENT : PET_SAVE_NOT_IN_SLOT));
  ...
  CreatureTemplate const *creatureInfo =
      sObjectMgr->GetCreatureTemplate(petentry);
  if (!creatureInfo) {
    LOG_ERROR("server",
              "Pet entry %u does not exist but used at pet load (owner: %s).",
              petentry, owner->GetName().c_str());
    return SPELL_FAILED_NO_PET;
  }
...
bool Pet::LoadPetFromDB(Player *owner, uint8 asynchLoadType, uint32 petentry,
                        uint32 petnumber, bool current, AsynchPetSummon *info) {
  ...
  sScriptMgr->OnBeforeLoadPetFromDB(owner, petentry, petnumber, current,
                                    forceLoadFromDB);
  ...
  if (petnumber) {
    ...
    // known petentry entry (unique for summoned pet, but non unique for hunter
    // pet (only from current or not stabled pets)
    stmt = CharacterDatabase.GetPreparedStatement(
  ...
  if (AsynchPetSummon *info =
          owner->GetSession()->_loadPetFromDBFirstCallback.GetSecondParam())
    delete info;
  owner->GetSession()->_loadPetFromDBFirstCallback.Reset();
  owner->GetSession()->_loadPetFromDBFirstCallback.SetFirstParam(
      asynchLoadType);
  owner->GetSession()->_loadPetFromDBFirstCallback.SetSecondParam(info);
  owner->GetSession()->_loadPetFromDBFirstCallback.SetFutureResult(
...
void Pet::SavePetToDB(PetSaveMode mode, bool logout) {
  ...
  if (mode >= PET_SAVE_AS_CURRENT) {
    ...
    stmt->setUInt8(
        10,
        uint8(HasByteFlag(UNIT_FIELD_BYTES_2, 2, UNIT_CAN_BE_RENAMED) ? 0 : 1));
...
void Pet::Update(uint32 diff) {
  ...
  case CORPSE: {
    if (getPetType() != HUNTER_PET || m_corpseRemoveTime <= time(nullptr)) {
      Remove(PET_SAVE_NOT_IN_SLOT); // hunters' pets never get removed because
                                    // of death, NEVER!
      return;
    }
    break;
  ...
  case ALIVE: {
    ...
    if (isControlled()) {
      if (owner->GetPetGUID() != GetGUID()) {
        LOG_ERROR("server", "Pet %u is not pet of owner %s, removed",
                  GetEntry(), m_owner->GetName().c_str());
        Remove(getPetType() == HUNTER_PET ? PET_SAVE_AS_DELETED
                                          : PET_SAVE_NOT_IN_SLOT);
        return;
      }
    }
    ...
    if (m_tempspell != 0) {
      ...
      if (tempspellTarget && tempspellTarget->IsAlive()) {
        ...
        if (IsWithinLOSInMap(tempspellTarget) &&
          ...
          if (!GetCharmInfo()->GetGlobalCooldownMgr().HasGlobalCooldown(
                  spellInfo) &&
...
void Pet::GivePetXP(uint32 xp) {
  ...
  // If pet is detected to be at, or above(?) the players level, don't hand out
  // XP
...
bool Pet::CreateBaseAtCreature(Creature *creature) {
  ...
  if (!IsPositionValid()) {
    LOG_ERROR("server",
              "Pet (guidlow %d, entry %d) not created base at creature. "
              "Suggested coordinates isn't valid (X: %f Y: %f)",
...
bool Pet::CreateBaseAtCreatureInfo(CreatureTemplate const *cinfo, Unit *owner) {
  ...
  Relocate(owner->GetPositionX(), owner->GetPositionY(), owner->GetPositionZ(),
...
bool Guardian::InitStatsForLevel(uint8 petlevel) {
  ...
  if (owner->GetTypeId() == TYPEID_PLAYER) {
    sScriptMgr->OnBeforeGuardianInitStatsForLevel(owner->ToPlayer(), this,
                                                  cinfo, petType);

    if (IsPet()) {
      if (petType == MAX_PET_TYPE) {
        // The petType was not overwritten by the hook, continue with default
        // initialization
      ...
      if (petType == HUNTER_PET)
        ...
        LOG_ERROR("server",
                  "Unknown type pet %u is summoned by player class %u",
                  GetEntry(), owner->getClass());
    }
  }
  ...
  if (cFamily && cFamily->minScale > 0.0f && petType == HUNTER_PET) {
    ...
    if (getLevel() >= cFamily->maxScaleLevel)
      ...
      scale = 0.5f + 0.5f * float(getLevel() - cFamily->minScaleLevel) /
                         float(cFamily->maxScaleLevel - cFamily->minScaleLevel);
  ...
  } else // not exist in DB, use some default fake data
  {
    ...
    SetCreateHealth(std::max<uint32>(1, stats->BaseHealth[cinfo->expansion] *
                                            factorHealth));
  ...
  case SUMMON_PET: {
    ...
    case NPC_FELGUARD: {
      ...
      if (AuraEffect *aurEff =
        ...
        SetModifierValue(UNIT_MOD_ATTACK_POWER, TOTAL_PCT,
                         1.0f + float(aurEff->GetAmount() / 100.0f));
  ...
  if (GetEntry() == NPC_RISEN_GHOUL) {
    ...
    if (owner->getRace() == RACE_ORC)
      CastSpell(this, SPELL_ORC_RACIAL_COMMAND, true, nullptr, nullptr,
                owner->GetGUID());
    ...
    if (Aura *aur = AddAura(SPELL_NIGHT_OF_THE_DEAD_AVOIDANCE, this))
      if (AuraEffect *aurEff = owner->GetAuraEffect(
              SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_DEATHKNIGHT, 2718, 0))
        if (aur->GetEffect(0))
...
uint32 Pet::GetCurrentFoodBenefitLevel(uint32 itemlevel) const {
  ...
  // -14..-11
           ...
           itemlevel + 14) // level 55 food gets green on 70, makes sense to me
    return 8000;
  // -15 or less
  else
...
void Pet::_SaveSpellCooldowns(SQLTransaction &trans, bool logout) {
  ...
  for (itr = m_CreatureSpellCooldowns.begin();
    ...
    if (itr2->second <= World::GetGameTimeMS() + 1000)
      ...
      uint32 cooldown =
          ((itr2->second - World::GetGameTimeMS()) / IN_MILLISECONDS) + curTime;
      stmt =
          CharacterDatabase.GetPreparedStatement(CHAR_INS_PET_SPELL_COOLDOWN);
      stmt->setUInt32(0, m_charmInfo->GetPetNumber());
...
void Pet::_SaveSpells(SQLTransaction &trans) {
  for (PetSpellMap::iterator itr = m_spells.begin(), next = m_spells.begin();
    ...
    case PETSPELL_REMOVED:
      stmt =
          CharacterDatabase.GetPreparedStatement(CHAR_DEL_PET_SPELL_BY_SPELL);
      stmt->setUInt32(0, m_charmInfo->GetPetNumber());
...
void Pet::_SaveAuras(SQLTransaction &trans, bool logout) {
  ...
  for (AuraMap::const_iterator itr = m_ownedAuras.begin();
    ...
    // pussywizard: don't save auras that cannot be cancelled (needed for ICC
    // buff on pets/summons)
    ...
    // don't save guid of caster in case we are caster of the spell - guid for
    // pet is generated every pet load, so it won't match saved guid anyways
    uint64 casterGUID = (itr->second->GetCasterGUID() == GetGUID())
                            ? 0
...
void Pet::InitLevelupSpellsForLevel() {
  ...
  if (PetLevelupSpellSet const *levelupSpells =
          GetCreatureTemplate()->family
              ? sSpellMgr->GetPetLevelupSpellList(GetCreatureTemplate()->family)
              : nullptr) {
    // PetLevelupSpellSet ordered by levels, process in reversed order
    for (PetLevelupSpellSet::const_reverse_iterator itr =
             levelupSpells->rbegin();
         itr != levelupSpells->rend(); ++itr) {
  ...
  if (PetDefaultSpellsEntry const *defSpells =
    ...
    for (uint8 i = 0; i < MAX_CREATURE_SPELL_DATA_SLOT; ++i) {
      SpellInfo const *spellEntry =
          sSpellMgr->GetSpellInfo(defSpells->spellid[i]);
      if (!spellEntry)
        continue;
...
void Pet::resetTalentsForAllPetsOf(Player *owner,
  ...
  PreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_PET);
  stmt->setUInt32(0, owner->GetGUIDLow());
...
void Pet::LearnPetPassives() {
  ...
  if (petStore != sPetFamilySpellsStore.end()) {
    ...
    for (PetFamilySpellsSet::const_iterator petSet = petStore->second.begin();
         petSet != petStore->second.end(); ++petSet)
...
void Pet::HandleAsynchLoadSucceed() {
  ...
  } else // if (GetAsynchLoadType() == PET_LOAD_BG_RESURRECT ||
         // GetAsynchLoadType() == PET_LOAD_SUMMON_PET || GetAsynchLoadType() ==
         // PET_LOAD_SUMMON_DEAD_PET)
  {
    // Remove Demonic Sacrifice auras (known pet)
    Unit::AuraEffectList const &auraClassScripts =
        owner->GetAuraEffectsByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
    for (Unit::AuraEffectList::const_iterator itr = auraClassScripts.begin();
    ...
    if (GetAsynchLoadType() == PET_LOAD_SUMMON_DEAD_PET) {
      setDeathState(ALIVE);
      ClearUnitState(
          uint32(UNIT_STATE_ALL_STATE &
                 ~(UNIT_STATE_POSSESSED))); // xinef: added just in case... some
                                            // linked auras and so on
      SetHealth(
          CountPctFromMaxHealth(15)); // Xinef: well, only two pet reviving
                                      // spells exist and both revive with 15%
  ...
  if (owner->NeedSendSpectatorData() && GetCreatureTemplate()->family) {
    ArenaSpectator::SendCommand_UInt32Value(owner->FindMap(), owner->GetGUID(),
                                            "PHP", (uint32)GetHealthPct());
    ArenaSpectator::SendCommand_UInt32Value(owner->FindMap(), owner->GetGUID(),
                                            "PET",
                                            GetCreatureTemplate()->family);
  }
...
void Pet::HandleAsynchLoadFailed(AsynchPetSummon *info, Player *player,
                                 uint8 asynchLoadType, uint8 loadResult) {
  if (loadResult == PET_LOAD_ERROR &&
      asynchLoadType == PET_LOAD_HANDLE_UNSTABLE_CALLBACK) {
    player->GetSession()->SendStableResult(0x06 /*STABLE_ERR_STABLE*/);
  } else if (loadResult == PET_LOAD_NO_RESULT && info &&
             (asynchLoadType == PET_LOAD_SUMMON_PET ||
              asynchLoadType == PET_LOAD_SUMMON_DEAD_PET)) {
    ...
    uint32 pet_number = sObjectMgr->GeneratePetNumber();
    if (!pet->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_PET), map,
                     player->GetPhaseMask(), info->m_entry, pet_number)) {
    ...
    if (info->m_petType == SUMMON_PET) {
      if (pet->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON ||
        ...
        pet->GetCharmInfo()->SetPetNumber(
            pet_number,
            true); // Show pet details tab (Shift+P) only for demons & undead
      else
      ...
      pet->SetUInt32Value(
          ...
          uint32(time(nullptr))); // cast can't be helped in this case
    }
    ...
    if (asynchLoadType == PET_LOAD_SUMMON_PET) {
      Unit *caster = ObjectAccessor::GetObjectInMap(info->m_casterGUID, map,
                                                    (Unit *)nullptr);
      if (!caster)
        caster = player;
      ...
      // in this function old pet is saved with current health eg. 20% and new
      // one is loaded from db with same amount pet should have full health
    ...
    } else // if GetAsynchLoad() == PET_LOAD_SUMMON_DEAD_PET
    {
      ...
      pet->SetHealth(pet->CountPctFromMaxHealth(uint32(
          info->m_casterGUID))); // damage for this effect is saved in caster
                                 // guid, dont create next variable...
...
void Pet::CastWhenWillAvailable(uint32 spellid, Unit *spellTarget,
                                Unit *oldTarget, bool spellIsPositive) {
  if (!spellid)
    return;
