...
bool MotionTransport::CreateMoTrans(uint32 guidlow, uint32 entry, uint32 mapid,
  ...
  if (!goinfo) {
    LOG_ERROR("server",
              ...
              "found, guidlow: %u map: %u  (X: %f Y: %f Z: %f) ang: %f",
...
void MotionTransport::Update(uint32 diff) {
  ...
  for (;;) {
    ...
    sScriptMgr->OnRelocate(this, _currentFrame->Node->index,
                           _currentFrame->Node->mapid, _currentFrame->Node->x,
                           _currentFrame->Node->y, _currentFrame->Node->z);

    // TC_LOG_DEBUG("entities.transport", "Transport %u (%s) moved to node %u %u
    // %f %f %f", GetEntry(), GetName().c_str(), _currentFrame->Node->index,
    // _currentFrame->Node->mapid, _currentFrame->Node->x,
    // _currentFrame->Node->y, _currentFrame->Node->z);
  ...
  if (_positionChangeTimer.Passed()) {
    ...
    if (IsMoving()) {
      ...
      UpdatePosition(pos.x, pos.y, pos.z,
                     NormalizeOrientation(atan2(dir.y, dir.x) + M_PI));
    } else {
...
Creature *MotionTransport::CreateNPCPassenger(uint32 guid,
  ...
  if (!creature->IsPositionValid()) {
    LOG_ERROR("server",
              "Creature (guidlow %d, entry %d) not created. Suggested "
...
GameObject *MotionTransport::CreateGOPassenger(uint32 guid,
  ...
  if (!go->IsPositionValid()) {
    LOG_ERROR("server",
              "GameObject (guidlow %d, entry %d) not created. Suggested "
              "coordinates aren't valid (X: %f Y: %f)",
              go->GetGUIDLow(), go->GetEntry(), go->GetPositionX(),
              go->GetPositionY());
    delete go;
    return nullptr;
  }
...
bool MotionTransport::TeleportTransport(uint32 newMapid, float x, float y,
                                        float z, float o) {
  ...
  if (oldMap->GetId() != newMapid) {
    ...
    for (PassengerSet::iterator itr = _passengers.begin();
         itr != _passengers.end(); ++itr) {
      if ((*itr)->GetTypeId() == TYPEID_PLAYER) {
        float destX, destY, destZ, destO;
        (*itr)->m_movementInfo.transport.pos.GetPosition(destX, destY, destZ,
                                                         destO);
        TransportBase::CalculatePassengerPosition(destX, destY, destZ, &destO,
                                                  x, y, z, o);
...
void MotionTransport::DelayedTeleportTransport() {
  ...
  for (PassengerSet::iterator itr = _passengersCopy.begin();
    ...
    case TYPEID_PLAYER: {
      ...
      TransportBase::CalculatePassengerPosition(destX, destY, destZ, &destO, x,
                                                y, z, o);
      if (!obj->ToPlayer()->TeleportTo(newMapId, destX, destY, destZ, destO,
...
void MotionTransport::UpdatePassengerPositions(PassengerSet &passengers) {
  for (PassengerSet::iterator itr = passengers.begin(); itr != passengers.end();
       ++itr) {
    ...
    // Do not use Unit::UpdatePosition here, we don't want to remove auras as if
    // regular movement occurred
    float x, y, z, o;
...
void MotionTransport::DoEventIfAny(KeyFrame const &node, bool departure) {
  if (uint32 eventid =
    ...
    // TC_LOG_DEBUG("maps.script", "Taxi %s event %u of node %u of %s path",
...
bool StaticTransport::Create(uint32 guidlow, uint32 name_id, Map *map,
                             uint32 phaseMask, float x, float y, float z,
                             float ang, G3D::Quat const &rotation,
                             uint32 animprogress, GOState go_state,
                             uint32 artKit) {
  ASSERT(map);
  SetMap(map);
  ...
  // SetWorldRotation( /*for StaticTransport we need 2 rotation Quats in db for
  // World- and Path- Rotation*/ );
...
void StaticTransport::Update(uint32 diff) {
  ...
  if (GetPauseTime()) {
    if (GetGoState() == GO_STATE_READY) {
      ...
      if (GetPathProgress() == GetPauseTime()) // waiting at it's destination
                                               // for state change, do nothing
        return;
...
void StaticTransport::RelocateToProgress(uint32 progress) {
  ...
  float percPos;
  if (m_goValue.Transport.AnimationInfo->GetAnimNode(progress, curr, next,
                                                     percPos)) {
    // curr node offset
    ...
    // pussywizard: PathRotation in db is only simple orientation rotation, so
    // don't use sophisticated and not working code reminder: WorldRotation only
    // influences model rotation, not the path
    float sign =
    ...
    // pussywizard: AnimRotation in dbc is only simple orientation rotation, so
    // don't use sophisticated and not working code
    G3D::Quat currRot, nextRot;
    float percRot;
    m_goValue.Transport.AnimationInfo->GetAnimRotation(progress, currRot,
                                                       nextRot, percRot);
...
void StaticTransport::UpdatePosition(float x, float y, float z, float o) {
  ...
  GetMap()->GameObjectRelocation(this, x, y, z,
                                 o); // this also relocates the model
