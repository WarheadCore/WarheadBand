...
DamageInfo::DamageInfo(Unit *_attacker, Unit *_victim, uint32 _damage,
                       SpellInfo const *_spellInfo, SpellSchoolMask _schoolMask,
...
DamageInfo::DamageInfo(CalcDamageInfo &dmgInfo)
    : m_attacker(dmgInfo.attacker), m_victim(dmgInfo.target),
      m_damage(dmgInfo.damage), m_spellInfo(nullptr),
...
ProcEventInfo::ProcEventInfo(Unit *actor, Unit *actionTarget, Unit *procTarget,
    ...
    : _actor(actor), _actionTarget(actionTarget), _procTarget(procTarget),
      ...
      _spellPhaseMask(spellPhaseMask), _hitMask(hitMask),
...
Unit::Unit(bool isWorldObject)
    : WorldObject(isWorldObject), m_movedByPlayer(nullptr),
      ...
      m_ThreatManager(this), m_vehicle(nullptr), m_vehicleKit(nullptr),
      m_unitTypeMask(UNIT_MASK_NONE), m_HostileRefManager(this),
...
bool GlobalCooldownMgr::HasGlobalCooldown(SpellInfo const *spellInfo) const {
  ...
  return itr != m_GlobalCooldowns.end() && itr->second.duration &&
         getMSTimeDiff(itr->second.cast_time, World::GetGameTimeMS()) <
             itr->second.duration;
}
...
void GlobalCooldownMgr::AddGlobalCooldown(SpellInfo const *spellInfo,
  ...
  m_GlobalCooldowns[spellInfo->StartRecoveryCategory] =
      GlobalCooldown(gcd, World::GetGameTimeMS());
}
...
void Unit::Update(uint32 p_time) {
  ...
  if (IsInCombat() &&
    ...
    // without stopping half way there and running off. These flags are reset
    // after target dies or another command is given.
  ...
  if (int32 ranged_attack = getAttackTimer(RANGED_ATTACK))
    setAttackTimer(RANGED_ATTACK,
                   ranged_attack > 0 ? ranged_attack - (int32)p_time : 0);
...
void Unit::SendMonsterMove(float NewPosX, float NewPosY, float NewPosZ,
                           uint32 TransitTime, SplineFlags sf) {
...
void Unit::UpdateSplinePosition() {
  ...
  if (movespline->onTransport) {
    ...
    if (TransportBase *transport = GetDirectTransport())
      transport->CalculatePassengerPosition(loc.x, loc.y, loc.z,
                                            &loc.orientation);
  }

  // Xinef: this is bullcrap, if we had spline running update orientation along
  // with position
...
void Unit::DisableSpline() {
  m_movementInfo.RemoveMovementFlag(MovementFlags(MOVEMENTFLAG_SPLINE_ENABLED |
...
bool Unit::GetRandomContactPoint(const Unit *obj, float &x, float &y, float &z,
                                 bool force) const {
  ...
  if (c)
    if (c->isWorldBoss() || c->IsDungeonBoss() ||
      ...
      attacker_number = 0; // pussywizard: pets and bosses just come to target
                           // from their angle

  GetNearPoint(
      obj, x, y, z,
      isMoving() ? (obj->GetCombatReach() > 7.75f ? obj->GetCombatReach() - 7.5f
                                                  : 0.25f)
                 : obj->GetCombatReach(),
      0.0f,
  ...
  // pussywizard
  if (fabs(this->GetPositionZ() - z) > this->GetCollisionHeight() ||
      !IsWithinLOS(x, y, z)) {
    x = this->GetPositionX();
    y = this->GetPositionY();
    z = this->GetPositionZ();
...
bool Unit::HasBreakableByDamageCrowdControlAura(
  ...
  if (Spell *currentChanneledSpell =
    ...
    excludeAura = currentChanneledSpell->GetSpellInfo()
                      ->Id; // Avoid self interrupt of channeled Crowd Control
                            // spells like Seduction
...
uint32 Unit::DealDamage(Unit *attacker, Unit *victim, uint32 damage,
  ...
  // if (attacker)
  {
    if (victim->IsAIEnabled)
      victim->GetAI()->DamageTaken(attacker, damage, damagetype,
                                   damageSchoolMask);
  ...
  if (damagetype != NODAMAGE) {
    ...
    for (AuraEffectList::iterator i = vCopyDamageCopy.begin();
      ...
      if (shareDamageTarget->IsImmunedToDamageOrSchool(damageSchoolMask)) {
        ...
        Unit::CalcAbsorbResist(attacker, shareDamageTarget, damageSchoolMask,
                               damagetype, shareDamage, &shareAbsorb,
                               &shareResist, spellProto, true);
        shareDamage -= std::min(shareAbsorb, shareDamage);
      }

      if (attacker && shareDamageTarget->GetTypeId() == TYPEID_PLAYER)
        attacker->SendSpellNonMeleeDamageLog(
            shareDamageTarget, spell->Id,
            shareDamage + shareAbsorb + shareResist, damageSchoolMask,
            shareAbsorb, shareResist, damagetype == DIRECT_DAMAGE, 0, false);

      Unit::DealDamage(attacker, shareDamageTarget, shareDamage, cleanDamage,
                       NODAMAGE, damageSchoolMask, spellProto, false);
    }
  }

  // Rage from Damage made (only from direct weapon damage)
  if (attacker && cleanDamage && damagetype == DIRECT_DAMAGE &&
      attacker != victim && attacker->getPowerType() == POWER_RAGE) {
    ...
    case OFF_ATTACK: {
      weaponSpeedHitFactor =
          uint32(attacker->GetAttackTime(cleanDamage->attackType) / 1000.0f *
  ...
  if (victim->GetTypeId() == TYPEID_PLAYER && victim->ToPlayer()->duel &&
    ...
    // prevent kill only if killed in duel and killed by opponent or opponent
    // controlled creature
  ...
  if (attacker && attacker != victim)
    if (Player *killer = attacker->GetCharmerOrOwnerPlayerOrPlayerItself()) {
      // pussywizard: don't allow GMs to deal damage in normal way (this leaves
      // no evidence in logs!), they have commands to do so
      ...
      if (Battleground *bg = killer->GetBattleground()) {
        ...
        killer->UpdateAchievementCriteria(
            ACHIEVEMENT_CRITERIA_TYPE_DAMAGE_DONE, damage, 0,
  ...
  if (victim->GetTypeId() == TYPEID_PLAYER)
    ...
    if (!attacker || attacker->IsControlledByPlayer() ||
      ...
      victim->ToCreature()->LowerPlayerDamageReq(health < damage ? health
                                                                 : damage);
  }
  ...
  if (health <= damage) {
    ...
    Unit::Kill(attacker, victim, durabilityLoss,
               cleanDamage ? cleanDamage->attackType : BASE_ATTACK, spellProto);
  } else {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    ...
    if (damagetype == DIRECT_DAMAGE || damagetype == SPELL_DIRECT_DAMAGE)
      victim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DIRECT_DAMAGE,
                                            spellProto ? spellProto->Id : 0);

    if (victim->GetTypeId() != TYPEID_PLAYER) {
      // Part of Evade mechanics. DoT's and Thorns / Retribution Aura do not
      // contribute to this
      if (damagetype != DOT && damage > 0 &&
          !IS_PLAYER_GUID(victim->GetOwnerGUID()) &&
          (!spellProto || !spellProto->HasAura(SPELL_AURA_DAMAGE_SHIELD)))
        victim->ToCreature()->SetLastDamagedTime(sWorld->GetGameTime() +
                                                 MAX_AGGRO_RESET_TIME);

      if (attacker)
        victim->AddThreat(attacker, float(damage), damageSchoolMask,
                          spellProto);
...
SpellCastResult Unit::CastSpell(SpellCastTargets const &targets,
                                ...
                                TriggerCastFlags triggerFlags, Item *castItem,
                                AuraEffect const *triggeredByAura,
                                uint64 originalCaster) {
  if (!spellInfo) {
    LOG_ERROR(
        "server", "CastSpell: unknown spell by caster: %s %u)",
...
SpellCastResult Unit::CastSpell(Unit *victim, uint32 spellId, bool triggered,
                                Item *castItem,
                                AuraEffect const *triggeredByAura,
                                uint64 originalCaster) {
  return CastSpell(victim, spellId,
                   triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem,
                   triggeredByAura, originalCaster);
}

SpellCastResult
Unit::CastSpell(Unit *victim, uint32 spellId,
  ...
  if (!spellInfo) {
    LOG_ERROR(
        "server", "CastSpell: unknown spell id %u by caster: %s %u)", spellId,
        (GetTypeId() == TYPEID_PLAYER ? "player (GUID:" : "creature (Entry:"),
        (GetTypeId() == TYPEID_PLAYER ? GetGUIDLow() : GetEntry()));
    return SPELL_FAILED_SPELL_UNAVAILABLE;
  }

  return CastSpell(victim, spellInfo, triggerFlags, castItem, triggeredByAura,
                   originalCaster);
}

SpellCastResult Unit::CastSpell(Unit *victim, SpellInfo const *spellInfo,
                                bool triggered, Item *castItem /*= nullptr*/,
                                AuraEffect const *triggeredByAura /*= nullptr*/,
                                uint64 originalCaster /*= 0*/) {
  return CastSpell(victim, spellInfo,
                   triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem,
                   triggeredByAura, originalCaster);
}
...
SpellCastResult Unit::CastSpell(Unit *victim, SpellInfo const *spellInfo,
  ...
  return CastSpell(targets, spellInfo, nullptr, triggerFlags, castItem,
                   triggeredByAura, originalCaster);
}

SpellCastResult Unit::CastCustomSpell(Unit *target, uint32 spellId,
                                      int32 const *bp0, int32 const *bp1,
                                      int32 const *bp2, bool triggered,
                                      Item *castItem,
                                      AuraEffect const *triggeredByAura,
                                      uint64 originalCaster) {
  ...
  return CastCustomSpell(spellId, values, target,
                         triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE,
                         castItem, triggeredByAura, originalCaster);
}

SpellCastResult Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod,
                                      int32 value, Unit *target, bool triggered,
                                      Item *castItem,
                                      AuraEffect const *triggeredByAura,
                                      uint64 originalCaster) {
  CustomSpellValues values;
...
SpellCastResult Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod,
  ...
  CustomSpellValues values;
  values.AddSpellMod(mod, value);
  return CastCustomSpell(spellId, values, target, triggerFlags, castItem,
                         triggeredByAura, originalCaster);
}

SpellCastResult
Unit::CastCustomSpell(uint32 spellId, CustomSpellValues const &value,
                      Unit *victim, TriggerCastFlags triggerFlags,
  ...
  return CastSpell(targets, spellInfo, &value, triggerFlags, castItem,
                   triggeredByAura, originalCaster);
}

SpellCastResult Unit::CastSpell(float x, float y, float z, uint32 spellId,
                                bool triggered, Item *castItem,
                                AuraEffect const *triggeredByAura,
                                uint64 originalCaster) {
  SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(spellId);
  if (!spellInfo) {
  ...
  return CastSpell(targets, spellInfo, nullptr,
                   triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem,
                   triggeredByAura, originalCaster);
}

SpellCastResult Unit::CastSpell(GameObject *go, uint32 spellId, bool triggered,
                                Item *castItem, AuraEffect *triggeredByAura,
                                uint64 originalCaster) {
  SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(spellId);
  if (!spellInfo) {
    LOG_ERROR(
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage *damageInfo,
                                     ...
                                     WeaponAttackType attackType, bool crit) {
  if (damage < 0)
    return;
  ...
  if (Unit::IsDamageReducedByArmor(damageSchoolMask, spellInfo))
    damage = Unit::CalcArmorReducedDamage(this, victim, damage, spellInfo, 0,
                                          attackType);
  ...
  case SPELL_DAMAGE_CLASS_MELEE: {
    ...
    if (crit) {
      ...
      if (Player *modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_CRIT_DAMAGE_BONUS,
                                crit_bonus);
      damage += crit_bonus;
    ...
    if (CanApplyResilience()) {
      if (attackType != RANGED_ATTACK)
        Unit::ApplyResilience(victim, nullptr, &damage, crit,
                              CR_CRIT_TAKEN_MELEE);
      else
        Unit::ApplyResilience(victim, nullptr, &damage, crit,
  ...
  sScriptMgr->ModifySpellDamageTaken(damageInfo->target, damageInfo->attacker,
                                     damage);

  // Calculate absorb resist
  if (damage > 0) {
    Unit::CalcAbsorbResist(this, victim, damageSchoolMask, SPELL_DIRECT_DAMAGE,
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo,
  ...
  if (spellProto == nullptr) {
    ...
    LOG_DEBUG("entities.unit",
              "Unit::DealSpellDamage has wrong damageInfo->SpellID: %u",
              damageInfo->SpellID);
#endif
    return;
  }
  ...
  Unit::DealDamage(this, victim, damageInfo->damage, &cleanDamage,
                   SPELL_DIRECT_DAMAGE, SpellSchoolMask(damageInfo->schoolMask),
                   spellProto, durabilityLoss);
}

// TODO for melee need create structure as in
void Unit::CalculateMeleeDamage(Unit *victim, uint32 damage,
  ...
  damage =
      MeleeDamageBonusDone(damageInfo->target, damage, damageInfo->attackType);
  damage = damageInfo->target->MeleeDamageBonusTaken(this, damage,
                                                     damageInfo->attackType);

  // Script Hook For CalculateMeleeDamage -- Allow scripts to change the Damage
  // pre class mitigation calculations
  sScriptMgr->ModifyMeleeDamage(damageInfo->target, damageInfo->attacker,
                                damage);

  // Calculate armor reduction
  if (IsDamageReducedByArmor((SpellSchoolMask)(damageInfo->damageSchoolMask))) {
    damageInfo->damage = Unit::CalcArmorReducedDamage(
        this, damageInfo->target, damage, nullptr, 0, damageInfo->attackType);
  ...
  case MELEE_HIT_CRIT: {
    ...
    // Increase crit damage from SPELL_AURA_MOD_CRIT_PERCENT_VERSUS
    mod += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS,
                                          crTypeMask);
    if (mod != 0)
  ...
  // attackType is checked already for BASE_ATTACK or OFF_ATTACK so it can't be
  // RANGED_ATTACK here
  if (CanApplyResilience())
    Unit::ApplyResilience(victim, nullptr, &resilienceReduction,
  ...
  if (int32(damageInfo->damage) > 0) {
    ...
    Unit::CalcAbsorbResist(this, damageInfo->target,
                           SpellSchoolMask(damageInfo->damageSchoolMask),
                           DIRECT_DAMAGE, damageInfo->damage,
                           &damageInfo->absorb, &damageInfo->resist);

    if (damageInfo->absorb) {
      damageInfo->HitInfo |= (damageInfo->damage - damageInfo->absorb == 0
                                  ? HITINFO_FULL_ABSORB
    ...
    if (damageInfo->resist)
      damageInfo->HitInfo |= (damageInfo->damage - damageInfo->resist == 0
                                  ? HITINFO_FULL_RESIST
...
void Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss) {
  ...
  Unit::DealDamage(this, victim, damageInfo->damage, &cleanDamage,
                   DIRECT_DAMAGE, SpellSchoolMask(damageInfo->damageSchoolMask),
  ...
  if (damageInfo->damage &&
      ((damageInfo->hitOutCome == MELEE_HIT_CRIT ||
       ...
       GetTypeId() != TYPEID_PLAYER && !ToCreature()->IsControlledByPlayer() &&
    ...
    // there is a newbie protection, at level 10 just 7% base chance; assuming
    // linear function
  ...
  if (GetTypeId() == TYPEID_PLAYER)
    ToPlayer()->CastItemCombatSpell(victim, damageInfo->attackType,
  ...
  if (damageInfo->damage) {
    ...
    for (AuraEffectList::const_iterator dmgShieldItr =
      ...
      uint32 damage = uint32(
          std::max(0, (*dmgShieldItr)->GetAmount())); // xinef: done calculated
                                                      // at amount calculation

      if (Unit *caster = (*dmgShieldItr)->GetCaster())
        damage = this->SpellDamageBonusTaken(caster, i_spellProto, damage,
                                             SPELL_DIRECT_DAMAGE);

      // No Unit::CalcAbsorbResist here - opcode doesn't send that data - this
      // damage is probably not affected by that
      ...
      Unit::DealDamage(victim, this, damage, 0, SPELL_DIRECT_DAMAGE,
                       i_spellProto->GetSchoolMask(), i_spellProto, true);
    }
  }
}
...
bool Unit::IsDamageReducedByArmor(SpellSchoolMask schoolMask,
                                  SpellInfo const *spellInfo, uint8 effIndex) {
  // only physical spells damage gets reduced by armor
  if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0)
    return false;
  if (spellInfo) {
    // there are spells with no specific attribute but they have "ignores armor"
    // in tooltip
...
uint32 Unit::CalcArmorReducedDamage(Unit const *attacker, Unit const *victim,
                                    const uint32 damage,
                                    SpellInfo const *spellInfo,
                                    uint8 attackerLevel,
                                    WeaponAttackType /*attackType*/) {
  ...
  if (attacker) {
    ...
    if (attacker->GetTypeId() == TYPEID_PLAYER) {
      ...
      AuraEffectList const &armorPenAuras =
          attacker->GetAuraEffectsByType(SPELL_AURA_MOD_ARMOR_PENETRATION_PCT);
      for (AuraEffectList::const_iterator itr = armorPenAuras.begin();
           itr != armorPenAuras.end(); ++itr) {
      ...
      float armorPen = CalculatePct(
          maxArmorPen, bonusPct + attacker->ToPlayer()->GetRatingBonusValue(
...
float Unit::GetEffectiveResistChance(Unit const *owner,
  ...
  if (spellInfo && spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK &&
      spellInfo->SpellIconID == 3178)
    victimResistance = 0;

  victimResistance = std::max(victimResistance, 0.0f);
  if (owner)
    victimResistance += std::max(
        (float(victim->getLevel()) - float(owner->getLevel())) * 5.0f, 0.0f);
...
void Unit::CalcAbsorbResist(Unit *attacker, Unit *victim,
                            SpellSchoolMask schoolMask,
                            DamageEffectType damagetype, const uint32 damage,
                            uint32 *absorb, uint32 *resist,
                            SpellInfo const *spellInfo, bool Splited) {
  if (!victim || !victim->IsAlive() || !damage)
    return;

  DamageInfo dmgInfo =
      DamageInfo(attacker, victim, damage, spellInfo, schoolMask, damagetype);
  ...
  if (!(schoolMask & SPELL_SCHOOL_MASK_NORMAL) &&
      (!(schoolMask & SPELL_SCHOOL_MASK_HOLY) ||
       victim->GetTypeId() == TYPEID_UNIT) &&
      (!spellInfo ||
       (!spellInfo->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) &&
        !spellInfo->HasAttribute(SPELL_ATTR4_IGNORE_RESISTANCES)))) {
    float averageResist =
        Unit::GetEffectiveResistChance(attacker, schoolMask, victim, spellInfo);

    float discreteResistProbability[11];
    for (uint32 i = 0; i < 11; ++i) {
      discreteResistProbability[i] =
          0.5f - 2.5f * fabs(0.1f * i - averageResist);
    ...
    if (damageResisted) // if equal to 0, checking these is pointless
    {
      ...
      if (spellInfo && spellInfo->HasAttribute(
        ...
        if (armorReduction <
            damageResisted) // pick the lower one, the weakest resistance counts
  ...
  // We're going to call functions which can modify content of the list during
  // iteration over it's elements Let's copy the list so we can prevent iterator
  // invalidation
  ...
  for (AuraEffectList::iterator itr = vSchoolAbsorbCopy.begin();
       (itr != vSchoolAbsorbCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr) {
  ...
  if (/*victim != attacker &&*/ !Splited) {
    ...
    for (AuraEffectList::iterator itr = vSplitDamageFlatCopy.begin();
      ...
      if (caster->IsImmunedToDamageOrSchool(schoolMask)) {
        ...
        Unit::CalcAbsorbResist(attacker, caster, schoolMask, damagetype,
                               splitted, &splitted_absorb, &splitted_resist,
                               spellInfo, true);
      ...
      caster->ProcDamageAndSpellFor(true, attacker, procVictim, procEx,
                                    BASE_ATTACK, spellInfo, splitted, nullptr);

      if (attacker)
        attacker->SendSpellNonMeleeDamageLog(
            caster, (*itr)->GetSpellInfo()->Id, splitted + splitted_absorb,
            schoolMask, splitted_absorb, 0, false, 0, false);

      CleanDamage cleanDamage =
          CleanDamage(splitted, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
      Unit::DealDamage(attacker, caster, splitted, &cleanDamage, DIRECT_DAMAGE,
                       schoolMask, (*itr)->GetSpellInfo(), false);
    }
    ...
    for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(), next;
      ...
      if (!splitSpellInfo->Effects[(*itr)->GetEffIndex()].IsAreaAuraEffect() &&
          !splitSpellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS))
        if (!caster->IsWithinLOSInMap(victim) ||
            !caster->IsWithinDist(victim,
      ...
      (*itr)->GetBase()->CallScriptEffectSplitHandlers(*itr, aurApp, dmgInfo,
                                                       splitDamage);

      // absorb must be smaller than the damage itself
      splitDamage =
          RoundToInterval(splitDamage, uint32(0), uint32(dmgInfo.GetDamage()));
      ...
      if (caster->IsImmunedToDamageOrSchool(schoolMask)) {
        ...
        Unit::DealDamageMods(caster, splitted, &splitted_absorb);
        Unit::CalcAbsorbResist(attacker, caster, splitSchoolMask, damagetype,
                               splitted, &splitted_absorb, &splitted_resist,
                               spellInfo, true);
        splitted -= std::min(splitted_absorb, splitted);
      }
      ...
      if (attacker)
        attacker->SendSpellNonMeleeDamageLog(
            caster, splitSpellInfo->Id, splitted + splitted_absorb,
            splitSchoolMask, splitted_absorb, 0, false, 0, false);

      CleanDamage cleanDamage =
          CleanDamage(splitted, 0, BASE_ATTACK, MELEE_HIT_NORMAL);
      Unit::DealDamage(attacker, caster, splitted, &cleanDamage, DIRECT_DAMAGE,
                       splitSchoolMask, splitSpellInfo, false);
    }
  }
...
void Unit::CalcHealAbsorb(Unit const *victim, const SpellInfo *healSpell,
  ...
  AuraEffectList const &vHealAbsorb =
      victim->GetAuraEffectsByType(SPELL_AURA_SCHOOL_HEAL_ABSORB);
...
void Unit::AttackerStateUpdate(Unit *victim, WeaponAttackType attType,
                               bool extra) {
  if (HasUnitState(UNIT_STATE_CANNOT_AUTOATTACK) ||
  ...
  if (attType == BASE_ATTACK && m_currentSpells[CURRENT_MELEE_SPELL] && !extra)
    ...
    ProcDamageAndSpell(damageInfo.target, damageInfo.procAttacker,
                       damageInfo.procVictim, damageInfo.procEx,
                       damageInfo.damage, damageInfo.attackType);

#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    if (GetTypeId() == TYPEID_PLAYER)
      LOG_DEBUG("server",
                "AttackerStateUpdate: (Player) %u attacked %u (TypeId: %u) for "
                "%u dmg, absorbed %u, blocked %u, resisted %u.",
                GetGUIDLow(), victim->GetGUIDLow(), victim->GetTypeId(),
                damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount,
                damageInfo.resist);
    else
      LOG_DEBUG("server",
                "AttackerStateUpdate: (NPC)    %u attacked %u (TypeId: %u) for "
                "%u dmg, absorbed %u, blocked %u, resisted %u.",
                GetGUIDLow(), victim->GetGUIDLow(), victim->GetTypeId(),
                damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount,
                damageInfo.resist);
...
Position *Unit::GetMeleeAttackPoint(Unit *attacker) {
  ...
  if (!GetMap()->CanReachPositionAndGetValidCoords(this, x, y, z, true, true)) {
    GetNearPoint(attacker, x, y, z, distance, 0.0f,
                 absAngle * -1); // try the other side

    if (!GetMap()->CanReachPositionAndGetValidCoords(this, x, y, z, true,
                                                     true)) {
      return nullptr;
    }
  }
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit *victim,
  ...
  float miss_chance =
      MeleeSpellMissChance(victim, attType,
  ...
  // removed
  // LOG_DEBUG("server", "MELEE OUTCOME: miss %f crit %f dodge %f parry %f block
  // %f", miss_chance, crit_chance, dodge_chance, parry_chance, block_chance);

  return RollMeleeOutcomeAgainst(
      victim, attType, int32(crit_chance * 100), int32(miss_chance * 100),
      int32(dodge_chance * 100), int32(parry_chance * 100),
      int32(block_chance * 100));
}

MeleeHitOutcome
Unit::RollMeleeOutcomeAgainst(const Unit *victim, WeaponAttackType attType,
                              int32 crit_chance, int32 miss_chance,
                              int32 dodge_chance, int32 parry_chance,
  ...
  sScriptMgr->OnBeforeRollMeleeOutcomeAgainst(
      this, victim, attType, attackerMaxSkillValueForLevel,
      victimMaxSkillValueForLevel, attackerWeaponSkill, victimDefenseSkill,
      crit_chance, miss_chance, dodge_chance, parry_chance, block_chance);
  ...
  // LOG_DEBUG("server", "RollMeleeOutcomeAgainst: skill bonus of %d for
  // attacker", skillBonus); LOG_DEBUG("server", "RollMeleeOutcomeAgainst: rolled
  // %d, miss %d, dodge %d, parry %d, block %d, crit %d",
  ...
  if (victim->GetTypeId() == TYPEID_PLAYER && crit_chance > 0 &&
      !victim->IsStandState()) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  ...
  if (victim->GetTypeId() == TYPEID_PLAYER && !victim->HasInArc(M_PI, this) &&
    ...
    // LOG_DEBUG("server", "RollMeleeOutcomeAgainst: attack came from behind and
    // victim was a player.");
  }
  // Xinef: do not allow to dodge with CREATURE_FLAG_EXTRA_NO_DODGE flag
    ...
    if (GetTypeId() == TYPEID_PLAYER)
      dodge_chance -=
          int32(ToPlayer()->GetExpertiseDodgeOrParryReduction(attType) * 100);
    else
    ...
    dodge_chance +=
        GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_COMBAT_RESULT_CHANCE,
                                        VICTIMSTATE_DODGE) *
        100;
    dodge_chance = int32(float(dodge_chance) *
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "RollMeleeOutcomeAgainst: DODGE <%d, %d)", sum - tmp,
                sum);
#endif
  ...
  // check if attack comes from behind, nobody can parry or block if attacker is
  // behind
  if (!victim->HasInArc(M_PI, this) &&
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER ||
      ...
      if (tmp > 0 // check if unit _can_ parry
          && (tmp -= skillBonus) > 0 && roll < (sum += tmp)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "RollMeleeOutcomeAgainst: PARRY <%d, %d)",
                  sum - tmp, sum);
#endif
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER ||
      ...
      if (tmp > 0 // check if unit _can_ block
          && (tmp -= skillBonus) > 0 && roll < (sum += tmp)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "RollMeleeOutcomeAgainst: BLOCK <%d, %d)",
                  sum - tmp, sum);
#endif
  ...
  if (attType != RANGED_ATTACK && (GetTypeId() == TYPEID_PLAYER || IsPet()) &&
    ...
    if (roll < (sum += tmp)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "RollMeleeOutcomeAgainst: GLANCING <%d, %d)",
                sum - 4000, sum);
#endif
  ...
  if (getLevelForTarget(victim) >= victim->getLevelForTarget(this) + 4 &&
    ...
    if (tmp >= 15) {
      ...
      if (roll < (sum += tmp)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "RollMeleeOutcomeAgainst: CRUSHING <%d, %d)",
                  sum - tmp, sum);
#endif
  ...
  if (tmp > 0 && roll < (sum += tmp)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "RollMeleeOutcomeAgainst: CRIT <%d, %d)", sum - tmp,
              sum);
#endif
...
uint32 Unit::CalculateDamage(WeaponAttackType attType, bool normalized,
  ...
  if (normalized || !addTotalPct)
    CalculateMinMaxDamage(attType, normalized, addTotalPct, minDamage,
                          maxDamage);
  else {
    switch (attType) {
    case RANGED_ATTACK:
    ...
    // pussywizard: this check only in "else" section, proper check is also in
...
void Unit::SendMeleeAttackStop(Unit *victim) {
  ...
  // clearing here ClearUnitState(UNIT_STATE_MELEE_ATTACKING); // ZOMG!
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *victim, SpellInfo const *spell) {
  ...
  uint32 missChance = uint32(
      MeleeSpellMissChance(victim, attType, skillDiff, spell->Id) * 100.0f);
  // Roll miss
  ...
  bool canBlock = spell->HasAttribute(SPELL_ATTR3_BLOCKABLE_SPELL) &&
                  !spell->HasAttribute(SPELL_ATTR0_CU_DIRECT_DAMAGE);
  ...
  if (!victim->HasInArc(M_PI, this) ||
      spell->HasAttribute(SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET)) {
    if (!victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION)) {
  ...
  for (AuraEffectList::const_iterator i = ignore.begin(); i != ignore.end();
    ...
    default:
      ...
      LOG_DEBUG(
          "server",
          "Spell %u SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state %d",
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *victim, SpellInfo const *spell) {
  ...
  // Spells with SPELL_ATTR3_IGNORE_HIT_RESULT will ignore target's avoidance
  // effects xinef: imo it should completly ignore all calculations, eg: 14792.
  // Hits 80 level players on blizz without any problems
  // if (!spell->HasAttribute(SPELL_ATTR3_IGNORE_HIT_RESULT))
  {
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
      modHitChance -=
          int32(victim->ToPlayer()->GetRatingBonusValue(CR_HIT_TAKEN_SPELL));
  }
  ...
  int32 rand = irand(1, 10000); // Needs to be  1 to 10000 to avoid the 1/10000
                                // chance to miss on 100% hit rating
  ...
  if (!spell->IsPositive() &&
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      ...
      if (spell->Effects[i].IsEffect() &&
          spell->Effects[i].ApplyAuraName == 0) {
        bNegativeAura = false;
        break;
      }
    }
    ...
    if (spell->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) &&
        (spell->GetSchoolMask() &
         (SPELL_SCHOOL_MASK_NORMAL | SPELL_SCHOOL_MASK_HOLY)) == 0)
      tmp += int32(Unit::GetEffectiveResistChance(this, spell->GetSchoolMask(),
                                                  victim, spell) *
                   10000.0f); // 100 for spell calculations, and 100 for return
                              // value percentage
  }
...
// For spells
//   Resist
SpellMissInfo Unit::SpellHitResult(Unit *victim, SpellInfo const *spell,
                                   bool CanReflect) {
  // Check for immune
  ...
  // TODO: client not show miss log for this spells - so need find info for this
  // in dbc and use it!
  ...
  if (CanReflect) {
    ...
    for (Unit::AuraEffectList::const_iterator i = mReflectSpellsSchool.begin();
         i != mReflectSpellsSchool.end(); ++i)
      if ((*i)->GetMiscValue() & spell->GetSchoolMask())
        reflectchance += (*i)->GetAmount();
    if (reflectchance > 0 && roll_chance_i(reflectchance)) {
      // Start triggers for remove charges if need (trigger only for victim, and
      // mark as active spell)
  ...
  case SPELL_DAMAGE_CLASS_NONE: {
    ...
    // Xinef: apply DAMAGE_CLASS_MAGIC conditions to damaging DAMAGE_CLASS_NONE
    // spells
  ...
  if (GetTypeId() == TYPEID_PLAYER)
    ...
    if (ToCreature()->IsTotem())
      ...
      float dodge = ToCreature()->isWorldBoss()
                        ? 5.85f
                        : 5.0f; // Xinef: bosses should have 6.5% dodge (5.9 +
                                // 0.6 from defense skill difference)
...
float Unit::GetUnitParryChance() const {
  ...
  if (Player const *player = ToPlayer()) {
    if (player->CanParry()) {
      ...
      if (tmpitem)
        chance =
            player->GetRealParry(); // GetFloatValue(PLAYER_PARRY_PERCENTAGE);
    }
...
float Unit::GetUnitCriticalChance(WeaponAttackType attackType,
  ...
  if (attackType == RANGED_ATTACK)
    ...
    crit +=
        victim->GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_CHANCE);
...
uint32 Unit::GetWeaponSkillValue(WeaponAttackType attType,
  ...
  if (Player const *player = ToPlayer()) {
    ...
    if (IsInFeralForm())
      return GetMaxSkillValueForLevel(); // always maximized SKILL_FERAL_COMBAT
                                         // in fact
...
void Unit::_UpdateSpells(uint32 time) {
  ...
  // m_auraUpdateIterator can be updated in indirect called code at aura remove
  // to skip next planned to update but removed auras
  for (m_auraUpdateIterator = m_ownedAuras.begin();
       m_auraUpdateIterator != m_ownedAuras.end();) {
    Aura *i_aura = m_auraUpdateIterator->second;
    ++m_auraUpdateIterator; // need shift to next for allow update if need into
                            // aura update
...
void Unit::_UpdateAutoRepeatSpell() {
  ...
  if ((GetTypeId() == TYPEID_PLAYER && ToPlayer()->isMoving() &&
      ...
      IsNonMeleeSpellCast(false, false, true,
                          spellProto->Id == HUNTER_AUTOSHOOT)) {
...
void Unit::SetCurrentCastedSpell(Spell *pSpell) {
  ...
  case CURRENT_GENERIC_SPELL: {
    ...
    if (Spell *s = GetCurrentSpell(CURRENT_CHANNELED_SPELL))
      if (s->GetSpellInfo()->Id !=
          69051) // pussywizard: FoS, boss Devourer of Souls, Mirrored Soul,
                 // does not have any special attribute
  ...
  case CURRENT_AUTOREPEAT_SPELL: {
    ...
    if (pSpell->m_spellInfo->Id != 75) {
      // generic autorepeats break generic non-delayed and channeled non-delayed
      // spells
      InterruptSpell(CURRENT_GENERIC_SPELL, false);
      InterruptSpell(CURRENT_CHANNELED_SPELL, false);
    }
...
bool Unit::IsNonMeleeSpellCast(bool withDelayed, bool skipChanneled,
                               bool skipAutorepeat, bool isAutoshoot,
  ...
  if (m_currentSpells[CURRENT_GENERIC_SPELL] &&
    ...
    if (!skipInstant || m_currentSpells[CURRENT_GENERIC_SPELL]->GetCastTime()) {
      if (!isAutoshoot ||
          !m_currentSpells[CURRENT_GENERIC_SPELL]->m_spellInfo->HasAttribute(
  ...
  if (!skipChanneled && m_currentSpells[CURRENT_CHANNELED_SPELL] &&
      (m_currentSpells[CURRENT_CHANNELED_SPELL]->getState() !=
       SPELL_STATE_FINISHED)) {
    if (!isAutoshoot ||
        !m_currentSpells[CURRENT_CHANNELED_SPELL]->m_spellInfo->HasAttribute(
            SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS))
      return true;
  }
  // autorepeat spells may be finished or delayed, but they are still considered
  // cast
...
void Unit::InterruptNonMeleeSpells(bool withDelayed, uint32 spell_id,
  ...
  // channeled spells are interrupted if they are not finished, even if they are
  // delayed
...
bool Unit::CanMoveDuringChannel() const {
  if (Spell *spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
    if (spell->getState() != SPELL_STATE_FINISHED)
      return spell->GetSpellInfo()->HasAttribute(
...
bool Unit::isInAccessiblePlaceFor(Creature const *c) const {
  if (c->GetMapId() == 618) // Ring of Valor
  {
    ...
    if (BattlegroundMap *bgMap = c->GetMap()->ToBattlegroundMap())
      if (Battleground *bg = bgMap->GetBG())
        if (bg->GetStartTime() <
            80133) // 60000ms preparation time + 20133ms elevator rise time
          return false;
  } else if (c->GetMapId() == 631) // Icecrown Citadel
  {
    ...
    // pussywizard: prevent any bugs by passengers exiting transports or normal
    // creatures flying away
    if (c->GetTransport() != this->GetTransport())
      return false;
  }

  if (IsInWater())
    return IsUnderWater() ? c->CanEnterWater()
                          : (c->CanEnterWater() || c->CanFly());
  else
...
void Unit::UpdateEnvironmentIfNeeded(const uint8 option) {
  ...
  if (GetTypeId() != TYPEID_UNIT || !IsAlive() ||
      (!IsInWorld() && option != 3) || !FindMap() ||
      IsDuringRemoveFromWorld() || !IsPositionValid())
    return;
  ...
  if (option <= 1 &&
      GetExactDistSq(&m_last_environment_position) < radiusAvg * radiusAvg)
    return;

  m_last_environment_position.Relocate(GetPositionX(), GetPositionY(),
                                       GetPositionZ());
  m_staticFloorZ = GetMap()->GetHeight(GetPhaseMask(), GetPositionX(),
  ...
  bool canChangeFlying =
      option == 3 || GetMotionMaster()->GetCurrentMovementGeneratorType() !=
                         WAYPOINT_MOTION_TYPE;
  bool canFallGround =
      option == 0 && canChangeFlying && GetInstanceId() == 0 && !IsInCombat() &&
      !GetVehicle() && !GetTransport() &&
      !HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && !c->IsTrigger() &&
      !c->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE) &&
      GetMotionMaster()->GetCurrentMovementGeneratorType() <=
          RANDOM_MOTION_TYPE &&
      !HasUnitState(UNIT_STATE_EVADE) && !IsControlledByPlayer();
  ...
  ZLiquidStatus liquidStatus =
      baseMap->getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS, &liquidData);
  // IsInWater
  bool enoughWater = baseMap->HasEnoughWater(this, liquidData);
  m_last_isinwater_status =
      (liquidStatus & (LIQUID_MAP_IN_WATER | LIQUID_MAP_UNDER_WATER)) &&
      enoughWater;
  m_last_islittleabovewater_status =
      (liquidData.level > INVALID_HEIGHT &&
       liquidData.level > liquidData.depth_level &&
       liquidData.level <= z + 3.0f && liquidData.level > z - 1.0f);

  // IsUnderWater
  m_last_isunderwater_status =
      (liquidStatus & LIQUID_MAP_UNDER_WATER) && enoughWater;

  // UpdateUnderwaterState
  if (IsPet() || IsVehicle()) {
    if (option == 1) // Unit::IsInWater, Unit::IsUnderwater, adding/removing
                     // auras can cause crashes (eg threat change while
                     // iterating threat table), so skip
      m_last_environment_position.Relocate(-5000.0f, -5000.0f, -5000.0f, 0.0f);
    else {
      if (!liquidStatus) {
  ...
  if (m_last_isinwater_status) {
    if (!c->CanFly() || enoughWater) {
      if (!HasUnitState(UNIT_STATE_NO_ENVIRONMENT_UPD) && c->CanSwim() &&
          (!HasUnitMovementFlag(MOVEMENTFLAG_SWIMMING) ||
           !HasUnitMovementFlag(MOVEMENTFLAG_DISABLE_GRAVITY))) {
        SetSwim(true);
  ...
  if (!m_last_isinwater_status) {
    if (!HasUnitState(UNIT_STATE_NO_ENVIRONMENT_UPD) && c->CanWalk() &&
        HasUnitMovementFlag(MOVEMENTFLAG_SWIMMING)) {
      SetSwim(false);
  ...
  if (isInAir) {
    if (GetMap()->GetGrid(x, y)) {
      ...
      if (temp > INVALID_HEIGHT) {
        ground_z = (c->CanSwim() && liquidData.level > INVALID_HEIGHT)
                       ? liquidData.level
                       : temp;
  ...
  if (!HasUnitState(UNIT_STATE_NO_ENVIRONMENT_UPD) && canChangeFlying) {
    // xinef: summoned vehicles are treated as always in air, fixes flying on
    // such units
    ...
    if (IS_PLAYER_GUID(c->GetOwnerGUID()) && c->CanFly() && IsVehicle() &&
        !c->GetDBTableGUIDLow()) // mainly for oculus drakes
    {
      ...
      if (!IsHovering() && IsAlive() &&
          (c->CanHover() || HasAuraType(SPELL_AURA_HOVER))) {
        SetHover(true);
        changed = true;
      }
  ...
  if (!HasUnitState(UNIT_STATE_NO_ENVIRONMENT_UPD) && canFallGround &&
      !c->CanFly() && !c->IsFalling() && !m_last_isinwater_status &&
      (c->GetUnitMovementFlags() &
       (MOVEMENTFLAG_CAN_FLY | MOVEMENTFLAG_DISABLE_GRAVITY |
        MOVEMENTFLAG_HOVER | MOVEMENTFLAG_SWIMMING)) == 0 &&
      z - ground_z > 5.0f && z - ground_z < 80.0f)
...
void SafeUnitPointer::UnitDeleted() {
  ...
  if (defaultValue) {
    if (Player *p = defaultValue->ToPlayer()) {
      LOG_INFO(
          "misc",
          "SafeUnitPointer::UnitDeleted (A1) - %u, %u, %u, %u, %u, %u, %u, %u",
          p->GetGUIDLow(), p->GetMapId(), p->GetInstanceId(),
          p->FindMap()->GetId(), p->IsInWorld() ? 1 : 0,
          p->IsDuringRemoveFromWorld() ? 1 : 0, p->IsBeingTeleported() ? 1 : 0,
          p->isBeingLoaded() ? 1 : 0);
      if (ptr)
...
bool Unit::IsInWater(bool allowAbove) const {
  ...
  return m_last_isinwater_status ||
         (allowAbove && m_last_islittleabovewater_status);
}
...
void Unit::UpdateUnderwaterState(Map * /*m*/, float /*x*/, float /*y*/,
                                 float /*z*/) {}

void Unit::DeMorph() { SetDisplayId(GetNativeDisplayId()); }

Aura *Unit::_TryStackingOrRefreshingExistingAura(
    SpellInfo const *newAura, uint8 effMask, Unit *caster,
    int32 *baseAmount /*= nullptr*/, Item *castItem /*= nullptr*/,
    uint64 casterGUID /*= 0*/, bool periodicReset /*= false*/) {
  ...
  if (!newAura->IsMultiSlotAura()) {
    ...
    // find current aura from spell and change it's stackamount, or refresh it's
    // duration
    if (Aura *foundAura = GetOwnedAura(
            ...
            newAura->HasAttribute(SPELL_ATTR0_CU_SINGLE_AURA_STACK)
                ? 0
                : casterGUID,
            newAura->HasAttribute(SPELL_ATTR0_CU_ENCHANT_PROC) ? castItemGUID
                                                               : 0,
            0)) {
      ...
      // update basepoints with new values - effect amount will be recalculated
      // in ModStackAmount
      for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
        ...
        int32 *oldBP =
            const_cast<int32 *>(&(foundAura->GetEffect(i)->m_baseAmount));
        *oldBP = bp;
      }
...
void Unit::_AddAura(UnitAura *aura, Unit *caster) {
  ...
  if (aura->IsSingleTarget()) {
     ...
     *        but may be created as a result of aura links (player mounts with
     * passengers)
     */
...
void Unit::_ApplyAura(AuraApplication *aurApp, uint8 effMask) {
  ...
  if (AuraStateType aState = spellInfo->GetAuraState()) {
    if (aState != AURA_STATE_CONFLAGRATE) {
      ...
      ConflagrateAuraStateDelayEvent *pEvent =
          new ConflagrateAuraStateDelayEvent(GetGUID(), caster->GetGUID());
      m_Events.AddEvent(
          pEvent,
          m_Events.CalculateTime(
              700)); // intended 700ms delay before allowing to cast conflagrate
    }
  }
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i,
  ...
  Aura *aura = aurApp->GetBase();
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("spells.aura", "Aura %u now is remove mode %d", aura->GetId(),
            removeMode);
#endif
...
void Unit::RemoveOwnedAura(AuraMap::iterator &i, AuraRemoveMode removeMode) {
  ...
  // if unit currently update aura list then make safe update iterator shift to
  // next
...
void Unit::RemoveOwnedAura(uint32 spellId, uint64 casterGUID, uint8 reqEffMask,
                           AuraRemoveMode removeMode) {
...
void Unit::RemoveAurasDueToSpell(uint32 spellId, uint64 casterGUID,
...
void Unit::RemoveAuraFromStack(uint32 spellId, uint64 casterGUID,
                               AuraRemoveMode removeMode) {
  AuraMapBoundsNonConst range = m_ownedAuras.equal_range(spellId);
...
void Unit::RemoveAurasDueToSpellByDispel(uint32 spellId,
                                         ...
                                         uint64 casterGUID, Unit *dispeller,
    ...
    if (aura->GetCasterGUID() == casterGUID) {
        ...
        if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x1000) {
          if (Unit *caster = aura->GetCaster()) {
            if (AuraEffect *aureff =
                    caster->GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS,
                                          SPELLFAMILY_HUNTER, 3521, 1)) {
              if (Aura *noxious = Aura::TryCreate(aura->GetSpellInfo(),
                                                  aura->GetEffectMask(),
                                                  dispeller, caster)) {
                noxious->SetDuration(aura->GetDuration() * aureff->GetAmount() /
                                     100);
      ...
      case SPELLFAMILY_DEATHKNIGHT: {
        ...
        if (aura->GetSpellInfo()->SpellFamilyFlags[1] & 0x4000000) {
          if (AuraEffect *aureff = GetAuraEffect(SPELL_AURA_MOD_DECREASE_SPEED,
                                                 SPELLFAMILY_DEATHKNIGHT, 0,
                                                 0x40000, 0, casterGUID))
...
void Unit::RemoveAurasDueToSpellBySteal(uint32 spellId, uint64 casterGUID,
  ...
  for (AuraMap::iterator iter = range.first; iter != range.second;) {
    ...
    if (aura->GetCasterGUID() == casterGUID) {
      ...
      // Cast duration to unsigned to prevent permanent aura's such as Righteous
      // Fury being permanently added to caster
      uint32 dur =
          std::min(2u * MINUTE * IN_MILLISECONDS, uint32(aura->GetDuration()));
      ...
      if (Aura *oldAura =
        ...
        // single target state must be removed before aura creation to preserve
        // existing single target aura
        if (aura->IsSingleTarget())
          aura->UnregisterSingleTarget();
        ...
        if (!curAura ||
            (!curAura->IsPermanent() && curAura->GetDuration() < (int32)dur))
          if (Aura *newAura = Aura::TryRefreshStackOrCreate(
                  aura->GetSpellInfo(), effMask, stealer, nullptr,
                  &baseDamage[0], nullptr, aura->GetCasterGUID())) {
            // created aura must not be single target aura,, so stealer won't
            // loose it on recast
            ...
            // FIXME: using aura->GetMaxDuration() maybe not blizzlike but it
            // fixes stealing of spells like Innervate
            newAura->SetLoadedState(aura->GetMaxDuration(), int32(dur),
...
void Unit::RemoveNotOwnSingleTargetAuras() {
  ...
  // before first Unit::Update after adding it to m_ownedAuras. Quickly removing
  // such an aura will lead to it not being unregistered from caster's single
  // cast auras container leading to assertion failures if the aura was cast on
  // a player that can (and is changing map at the point where this function is
  // called). Such situation occurs when player is logging in inside an instance
  // and fails the entry check for any reason. The aura that was loaded from db
  // (indirectly, via linked casts) gets removed before it has a chance to
  // register in m_appliedAuras
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1,
                                 uint32 familyFlag2, uint32 familyFlag3,
                                 uint64 casterGUID) {
  for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
       iter != m_appliedAuras.end();) {
    Aura const *aura = iter->second->GetBase();
...
void Unit::RemoveAurasWithMechanic(uint32 mechanic_mask,
                                   AuraRemoveMode removemode, uint32 except) {
  for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
       iter != m_appliedAuras.end();) {
    Aura const *aura = iter->second->GetBase();
...
void Unit::RemoveAreaAurasDueToLeaveWorld() {
  ...
  for (AuraMap::iterator iter = m_ownedAuras.begin();
    ...
    for (Aura::ApplicationMap::const_iterator itr = appMap.begin();
      ...
      // things linked on aura remove may apply new area aura - so start from
      // the beginning
...
void Unit::RemoveArenaAuras() {
  ...
  for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
    ...
    if (!aura->GetSpellInfo()->HasAttribute(
            SPELL_ATTR4_DONT_REMOVE_IN_ARENA) // don't remove stances,
                                              // shadowform, pally/hunter auras
        && !aura->IsPassive()                 // don't remove passive auras
        && !aura->IsArea() // don't remove area auras, eg pet talents affecting
                           // owner
        && (aurApp->IsPositive() || IsPet() ||
            !aura->GetSpellInfo()->HasAttribute(
                SPELL_ATTR3_DEATH_PERSISTENT))) // not negative death persistent
                                                // auras
    {
      RemoveAura(iter);
    }
    // xinef: special marker, 95% sure
    else if (aura->GetSpellInfo()->HasAttribute(
...
void Unit::RemoveEvadeAuras() {
  for (AuraApplicationMap::iterator iter = m_appliedAuras.begin();
    ...
    if (aura->GetSpellInfo()->HasAura(SPELL_AURA_CONTROL_VEHICLE) ||
        ...
        (aura->IsPassive() && IS_PLAYER_GUID(GetOwnerGUID())))
      ++iter;
    else
      _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
  }
...
void Unit::_RemoveAllAuraStatMods() {
  for (AuraApplicationMap::iterator i = m_appliedAuras.begin();
       i != m_appliedAuras.end(); ++i)
    (*i).second->GetBase()->HandleAllEffects(i->second, AURA_EFFECT_HANDLE_STAT,
                                             false);
}
...
AuraEffect *Unit::GetAuraEffect(AuraType type, SpellFamilyNames family,
                                uint32 familyFlag1, uint32 familyFlag2,
                                uint32 familyFlag3, uint64 casterGUID) const {
  AuraEffectList const &auras = GetAuraEffectsByType(type);
...
AuraApplication *
Unit::GetAuraApplicationOfRankedSpell(uint32 spellId, uint64 casterGUID,
                                      uint64 itemCasterGUID, uint8 reqEffMask,
                                      AuraApplication *except) const {
  uint32 rankSpell = sSpellMgr->GetFirstSpellInChain(spellId);
  while (rankSpell) {
...
void Unit::GetDispellableAuraList(Unit *caster, uint32 dispelMask,
        ...
        // used for glyph of scourge strike
          ...
          if (aura && !aura->IsRemoved() && aura->GetDuration() > 0)
            if ((aura->GetApplyTime() + aura->GetMaxDuration() / 1000 + 8) >
                (time(nullptr) + aura->GetDuration() / 1000))
...
int32 Unit::GetMaxPositiveAuraModifierByMiscMask(
    AuraType auratype, uint32 misc_mask, const AuraEffect *except) const {
  int32 modifier = 0;
...
DynamicObject *Unit::GetDynObject(uint32 spellId) {
  if (m_dynObj.empty())
    return nullptr;
  for (DynObjectList::const_iterator i = m_dynObj.begin(); i != m_dynObj.end();
       ++i) {
...
void Unit::AddGameObject(GameObject *gameObj) {
  ...
  if (GetTypeId() == TYPEID_PLAYER && gameObj->GetSpellId()) {
    ...
    if (createBySpell && createBySpell->IsCooldownStartedOnEvent())
      // note: item based cooldowns and cooldown spell mods with charges ignored
      // (unknown existing cases)
...
void Unit::SendSpellNonMeleeDamageLog(Unit *target, uint32 SpellID,
                                      uint32 Damage,
                                      SpellSchoolMask damageSchoolMask,
                                      uint32 AbsorbedDamage, uint32 Resist,
                                      bool PhysicalDamage, uint32 Blocked,
                                      bool CriticalHit) {
...
void Unit::ProcDamageAndSpell(Unit *victim, uint32 procAttacker,
                              uint32 procVictim, uint32 procExtra,
                              uint32 amount, WeaponAttackType attType,
                              SpellInfo const *procSpell,
                              SpellInfo const *procAura) {
  // Not much to do if no flags are set.
  if (procAttacker)
    ProcDamageAndSpellFor(false, victim, procAttacker, procExtra, attType,
                          procSpell, amount, procAura);
  // Now go on with a victim's events'n'auras
  // Not much to do if no flags are set or there is no victim
  if (victim && victim->IsAlive() && procVictim)
    victim->ProcDamageAndSpellFor(true, this, procVictim, procExtra, attType,
                                  procSpell, amount, procAura);
}
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo *pInfo) {
  ...
  case SPELL_AURA_PERIODIC_MANA_LEECH:
    ...
    break;
  default:
    LOG_ERROR("server", "Unit::SendPeriodicAuraLog: unknown aura %u",
...
void Unit::SendAttackStateUpdate(CalcDamageInfo *damageInfo) {
  ...
  uint32 count = 1;
  size_t maxsize =
      4 + 5 + 5 + 4 + 4 + 1 + 4 + 4 + 4 + 4 + 4 + 1 + 4 + 4 + 4 + 4 + 4 * 12;
...
void Unit::SendAttackStateUpdate(uint32 HitInfo, Unit *target,
                                 uint8 /*SwingType*/,
                                 SpellSchoolMask damageSchoolMask,
                                 uint32 Damage, uint32 AbsorbDamage,
                                 uint32 Resist, VictimState TargetState,
...
bool Unit::HandleDummyAuraProc(Unit *victim, uint32 damage,
                               AuraEffect *triggeredByAura,
                               SpellInfo const *procSpell, uint32 procFlag,
                               uint32 procEx, uint32 cooldown) {
  ...
  uint32 cooldown_spell_id = 0; // for random trigger, will be one of the
                                // triggered spell to avoid repeatable triggers
  ...
  case SPELLFAMILY_GENERIC: {
    ...
    // Druid) 40998 Kleptomania: +35 agility (Warrior, Rogue, Paladin, Hunter,
    // Druid) 40999 Megalomania: +41 damage/healing (Druid, Shaman, Priest,
    // Warlock, Mage, Paladin) 41002 Paranoia: +35 spell/melee/ranged crit
    // strike rating (All classes) 41005 Manic: +35 haste (spell, melee and
    // ranged) (All classes) 41009 Narcissism: +35 intellect (Druid, Shaman,
    // Priest, Warlock, Mage, Paladin, Hunter) 41011 Martyr Complex: +35 stamina
    // (All classes) 41406 Dementia: Every 5 seconds either gives you +5%
    // damage/healing. (Druid, Shaman, Priest, Warlock, Mage, Paladin) 41409
    // Dementia: Every 5 seconds either gives you -5% damage/healing. (Druid,
    // Shaman, Priest, Warlock, Mage, Paladin)
    case 39446: {
      ...
      switch (getClass()) {
      case CLASS_PALADIN: // 39511, 40997, 40998, 40999, 41002, 41005, 41009,
                          // 41011, 41409
      case CLASS_DRUID:   // 39511, 40997, 40998, 40999, 41002, 41005, 41009,
                          // 41011, 41409
        triggered_spell_id =
      ...
      case CLASS_WARLOCK: // 40999, 41002, 41005, 41009, 41011, 41406, 41409
        ...
        break;
      case CLASS_HUNTER: // 40997, 40999, 41002, 41005, 41009, 41011, 41406,
                         // 41409
        triggered_spell_id =
      ...
      if (roll_chance_i(10))
        ToPlayer()->Say("This is Madness!",
                        LANG_UNIVERSAL); // TODO: It should be moved to
    ...
    case 45481: {
      ...
      if (player->GetReputationRank(934) == REP_EXALTED) {
        // triggered at positive/self casts also, current attack target used
        // then
    ...
    case 67534: {
      ...
      CastCustomSpell(this, 67535, &dmg, nullptr, nullptr, true, 0,
                      triggeredByAura, triggeredByAura->GetCasterGUID());
      return true;
    }
    ...
    case 69172: // everything except for DoTs
    {
      ...
      if (Unit *caster = triggeredByAura->GetCaster()) {
        if (procFlag & (PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS |
          ...
          if (caster->GetVictim()) {
            int32 dmg = damage;
            target->CastCustomSpell(caster->GetVictim(), 69189, &dmg, 0, 0,
                                    true);
          }
        }
      }
  ...
  case SPELLFAMILY_WARRIOR: {
    ...
    if (dummySpell->SpellIconID == 1697) {
      // only for spells and hit/crit (trigger start always) and not start from
      // self casted spells (5530 Mace Stun Effect for example)
      if (procSpell == 0 ||
  ...
  case SPELLFAMILY_WARLOCK: {
    ...
    case 30296: {
      ...
      for (Unit::AuraEffectList::const_iterator i = SoulLeechAuras.begin();
           i != SoulLeechAuras.end(); ++i) {
        if ((*i)->GetId() == 54117 || (*i)->GetId() == 54118) {
          ...
          if (target) {
            // regen mana for pet
            CastCustomSpell(target, 54607, &basepoints0, nullptr, nullptr, true,
                            castItem, triggeredByAura);
          }
          // regen mana for caster
          CastCustomSpell(this, 59117, &basepoints0, nullptr, nullptr, true,
                          castItem, triggeredByAura);
  ...
  case SPELLFAMILY_PRIEST: {
    ...
    case 15286: {
      ...
      CastCustomSpell(this, 15290, &team, &self, nullptr, true, castItem,
                      triggeredByAura);
      return true; // no hidden cooldown
    }
  ...
  case SPELLFAMILY_DRUID: {
    ...
    case 54832: {
      ...
      int32 mana_perc = triggeredByAura->GetSpellInfo()
                            ...
                            .CalcValue();
      basepoints0 =
          int32(CalculatePct(GetCreatePowers(POWER_MANA), mana_perc) / 10);
    ...
    case 54815: {
      ...
      if (AuraEffect const *AurEff = target->GetAuraEffect(
              SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x00800000, 0x0,
              0x0, GetGUID())) {
        // Rip's max duration, note: spells which modifies Rip's duration also
        // counted like Glyph of Rip
        ...
        CountMax +=
            3 * triggerAmount * IN_MILLISECONDS; // Glyph of Shred -> +6 seconds
        CountMax += HasAura(54818)
                        ? 4 * IN_MILLISECONDS
                        : 0; // Glyph of Rip                 -> +4 seconds
        CountMax += HasAura(60141)
                        ? 4 * IN_MILLISECONDS
        ...
        if (CountMin < CountMax) {
          AurEff->GetBase()->SetDuration(AurEff->GetBase()->GetDuration() +
                                         triggerAmount * IN_MILLISECONDS);
          AurEff->GetBase()->SetMaxDuration(CountMin +
                                            triggerAmount * IN_MILLISECONDS);
          return true;
        }
      }
    ...
    case 24932: {
      ...
      if (triggeredByAura->GetCasterGUID() != GetGUID())
        break;
      int32 basepoints1 =
          CalculatePct(GetMaxPower(Powers(POWER_MANA)), triggerAmount * 2);
    ...
    [[fallthrough]]; // TODO: Not sure whether the fallthrough was a mistake
                     // (forgetting a break) or intended. This should be
                     // double-checked.
  }
  case SPELLFAMILY_ROGUE: {
    ...
    case 56800: {
      if (victim)
        if (AuraEffect *aurEff = victim->GetAuraEffect(
                SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x100000, 0, 0,
                GetGUID()))
    ...
    case 32748: {
      // Prevent cast Deadly Throw Interrupt on self from last effect (apply
      // dummy) of Deadly Throw
      if (this == victim)
        return false;
    ...
    if (dummySpell->SpellIconID == 2909) {
      ...
      if (AuraEffect const *aur = GetAuraEffect(
              SPELL_AURA_MOD_MELEE_HASTE, SPELLFAMILY_ROGUE, 0x40000, 0, 0)) {
        aur->GetBase()->SetDuration(aur->GetSpellInfo()->GetMaxDuration(),
                                    true);
        return true;
      }
      return false;
  ...
  case SPELLFAMILY_HUNTER: {
    ...
    case 2236: // Thrill of the Hunt
    {
      ...
      if (procSpell->SpellFamilyFlags[2] & 0x200) {
        ...
        if (AuraEffect const *pEff = victim->GetAuraEffect(
                SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_HUNTER, 0x0, 0x80000000,
                0x0, GetGUID()))
          basepoints0 =
        ...
        basepoints0 = procSpell->CalcPowerCost(
    ...
    {
      // Proc only from periodic (from trap activation proc another aura of this
      // spell)
  ...
  case SPELLFAMILY_PALADIN: {
    ...
    if (dummySpell->Id == 53651) {
      ...
      triggered_spell_id =
          procSpell->IsRankOf(sSpellMgr->GetSpellInfo(635)) ? 53652 : 53654;

      victim->CastCustomSpell(beaconTarget, triggered_spell_id, &basepoints0,
                              nullptr, nullptr, true, 0, triggeredByAura,
                              victim->GetGUID());
      return true;
    }
    ...
    case 20185: {
      ...
      victim->CastCustomSpell(victim, 20267, &basepoints0, 0, 0, true, 0,
                              triggeredByAura);
    ...
    case 20186: {
      if (!victim || !victim->IsAlive() ||
          victim->getPowerType() != POWER_MANA ||
          victim->HasSpellCooldown(20268))
        return false;

      // 2% of base mana
      basepoints0 = int32(CalculatePct(victim->GetCreateMana(), 2));
      victim->CastCustomSpell(victim, 20268, &basepoints0, nullptr, nullptr,
    ...
    case 28789: {
      ...
      case CLASS_SHAMAN:
      case CLASS_DRUID:
        triggered_spell_id = 28795; // Increases the friendly target's mana
                                    // regeneration by $s1 per 5 sec. for $d.
        break;
      case CLASS_MAGE:
      case CLASS_WARLOCK:
        triggered_spell_id = 28793; // Increases the friendly target's spell
                                    // damage and healing by up to $s1 for $d.
        break;
      case CLASS_HUNTER:
      case CLASS_ROGUE:
        triggered_spell_id = 28791; // Increases the friendly target's attack
                                    // power by $s1 for $d.
        break;
      case CLASS_WARRIOR:
    ...
    case 31801: {
      ...
      // At melee attack or Hammer of the Righteous spell damage considered as
      // melee attack
    ...
    case 33776: {
      ...
      // heal amount
      basepoints0 = int32(CalculatePct(
          std::min(damage, GetMaxHealth() - GetHealth()), triggerAmount));
      target = this;
  ...
  case SPELLFAMILY_SHAMAN: {
    ...
    case 28823: {
      ...
      case CLASS_SHAMAN:
      case CLASS_DRUID:
        triggered_spell_id = 28824; // Increases the friendly target's mana
                                    // regeneration by $s1 per 5 sec. for $d.
        break;
      case CLASS_MAGE:
      case CLASS_WARLOCK:
        triggered_spell_id = 28825; // Increases the friendly target's spell
                                    // damage and healing by up to $s1 for $d.
        break;
      case CLASS_HUNTER:
      case CLASS_ROGUE:
        triggered_spell_id = 28826; // Increases the friendly target's attack
                                    // power by $s1 for $d.
        break;
      case CLASS_WARRIOR:
    ...
    case 33757: {
      ...
      default: {
        LOG_ERROR(
            "server",
            "Unit::HandleDummyAuraProc: non handled item enchantment (rank?) "
            "%u for spell id: %u (Windfury)",
            castItem->GetEnchantmentId(EnchantmentSlot(TEMP_ENCHANTMENT_SLOT)),
            dummySpell->Id);
        return false;
      }
      ...
      if (!windfurySpellInfo) {
        LOG_ERROR(
            "server",
            "Unit::HandleDummyAuraProc: non-existing spell id: %u (Windfury)",
            spellId);
        return false;
      }
      ...
      basepoints0 =
          int32(extra_attack_power / 14.0f * GetAttackTime(attType) / 1000);
      ...
      for (uint32 i = 0; i < 2; ++i)
        CastCustomSpell(victim, triggered_spell_id, &basepoints0, nullptr,
    ...
    case 58877: {
      ...
      // Heal wolf
      CastCustomSpell(this, triggered_spell_id, &basepoints0, nullptr, nullptr,
    ...
    case 70817: {
      ...
      if (AuraEffect const *aurEff = target->GetAuraEffect(
              SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0x0,
              0x0, GetGUID())) {
        Aura *flameShock = aurEff->GetBase();
        int32 maxDuration = flameShock->GetMaxDuration();
        int32 newDuration =
            flameShock->GetDuration() + 2 * aurEff->GetAmplitude();
    ...
    if (dummySpell->SpellFamilyFlags[0] & 0x200000) {
      ...
      float fire_onhit =
          float(CalculatePct(dummySpell->Effects[EFFECT_0].CalcValue(), 1.0f));
      ...
      if (castItem->GetSlot() == EQUIPMENT_SLOT_OFFHAND &&
        ...
        // Value1: add the tooltip damage by swingspeed + Value2: add spelldmg
        // by swingspeed
    ...
    if (dummySpell->SpellIconID == 2287) {
      ...
      if (AuraEffect const *aurEff =
              GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_SHAMAN,
                            0, 0x00000020, 0)) {
        uint32 spell =
            aurEff->GetSpellInfo()->Effects[aurEff->GetEffIndex()].TriggerSpell;
    ...
    // Lightning Overload
    if (dummySpell->SpellIconID ==
        2018) // only this spell have SpellFamily Shaman SpellIconID == 2018 and
              // dummy aura
    {
      ...
      do {
        ...
        // Cast
        CastCustomSpell(victim, spell, &dmg, 0, 0, true, castItem,
                        triggeredByAura);
    ...
    if (dummySpell->SpellIconID == 3059) {
      // Lightning Shield
      if (AuraEffect const *aurEff = GetAuraEffect(
              SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_SHAMAN, 0x400, 0, 0)) {
  ...
  case SPELLFAMILY_DEATHKNIGHT: {
    ...
    if (dummySpell->SpellIconID == 2023) {
      ...
      ToPlayer()->RestoreSpellMods(m_currentSpells[CURRENT_GENERIC_SPELL],
                                   51124, nullptr); // Killing Machine
      ToPlayer()->RestoreSpellMods(m_currentSpells[CURRENT_GENERIC_SPELL],
                                   49796, nullptr); // Deathchill

      // Xinef: Somehow basepoints are divided by 2 which is later divided by 2
      // (offhand multiplier)
    ...
    if (dummySpell->Id == 17619) {
      if (procSpell->SpellFamilyName == SPELLFAMILY_POTION) {
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++) {
          ...
          CastCustomSpell(this, triggered_spell_id, &basepoints0, nullptr,
                          nullptr, true, nullptr, triggeredByAura);
        }
        return true;
      }
    }
  ...
  case SPELLFAMILY_PET: {
    ...
    case 201: {
      ...
      target = this;
      float addThreat = float(
          CalculatePct(procSpell->Effects[0].CalcValue(this), triggerAmount));
  ...
  if (!triggerEntry) {
    LOG_ERROR("server",
              "Unit::HandleDummyAuraProc: Spell %u has non-existing triggered "
              "spell %u",
  ...
  if (basepoints0)
    CastCustomSpell(target, triggered_spell_id, &basepoints0, nullptr, nullptr,
                    true, castItem, triggeredByAura, originalCaster);
  else
    CastSpell(target, triggered_spell_id, true, castItem, triggeredByAura,
              originalCaster);
...
bool Unit::HandleAuraProc(Unit *victim, uint32 damage, Aura *triggeredByAura,
                          SpellInfo const * /*procSpell*/, uint32 /*procFlag*/,
                          uint32 procEx, uint32 /*cooldown*/, bool *handled) {
  SpellInfo const *dummySpell = triggeredByAura->GetSpellInfo();
  ...
  case SPELLFAMILY_GENERIC:
    ...
    case 59915: {
      CastSpell(this, 59914, true); // 59914 already has correct basepoints in
  ...
  case SPELLFAMILY_MAGE: {
    ...
    case 31658: {
      *handled = true;
      ...
      CastCustomSpell(this, 67545, &bp0, nullptr, nullptr, true, nullptr,
                      triggeredByAura->GetEffect(EFFECT_0), GetGUID());
      return true;
    }
    }
    break;
...
bool Unit::HandleProcTriggerSpell(Unit *victim, uint32 damage,
                                  AuraEffect *triggeredByAura,
                                  SpellInfo const *procSpell, uint32 procFlags,
                                  uint32 procEx, uint32 cooldown) {
  ...
  // if (sSpellMgr->GetSpellInfo(trigger_spell_id) == nullptr)
  {
    ...
    case SPELLFAMILY_GENERIC:
      ...
      case 43820: // Charm of the Witch Doctor (Amani Charm of the Witch Doctor
                  // trinket)
      ...
      case 67771: // Death's Choice (heroic), Item - Coliseum 25 Heroic Melee
                  // Trinket
      {
        if (!damage)
          return false;
      ...
      case 27522:
      case 40336: {
        // On successful melee or ranged attack gain $29471s1 mana and if
        // possible drain $27526s1 mana from the target.
        if (IsAlive())
    ...
    case SPELLFAMILY_MAGE:
      if (auraSpellInfo->SpellIconID == 2127) // Blazing Speed
      {
        ...
        default:
          LOG_ERROR("server",
                    "Unit::HandleProcTriggerSpell: Spell %u miss posibly "
                    "Blazing Speed",
                    auraSpellInfo->Id);
          return false;
        }
    ...
    case SPELLFAMILY_PRIEST: {
      ...
      if (auraSpellInfo->SpellIconID == 1875) {
        ...
        CastDelayedSpellWithPeriodicAmount(
            this, trigger_spell_id, SPELL_AURA_PERIODIC_HEAL, basepoints0);
        return true;
      }
      break;
    }
      ...
      default:
        ...
        if (auraSpellInfo->SpellIconID == 241) {
          ...
          if (procSpell->SpellFamilyFlags[1] & 0x00010000) {
            ...
            default:
              LOG_ERROR("server",
                        "Unit::HandleProcTriggerSpell: Spell %u not handled in "
                        "HShock",
                        procSpell->Id);
              return false;
            }
          ...
          if (!originalSpell) {
            LOG_ERROR("server",
                      "Unit::HandleProcTriggerSpell: Spell %u unknown but "
                      "selected as original in Illu",
                      originalSpellId);
            return false;
          }
          // percent stored in effect 1 (class scripts) base points
          int32 cost = int32(
              originalSpell->ManaCost +
              CalculatePct(GetCreateMana(), originalSpell->ManaCostPercentage));
          basepoints0 =
              CalculatePct(cost, auraSpellInfo->Effects[1].CalcValue());
    ...
    case SPELLFAMILY_SHAMAN: {
      ...
      if (auraSpellInfo->SpellFamilyFlags[0] & 0x400) {
        trigger_spell_id =
            sSpellMgr->GetSpellWithRank(26364, auraSpellInfo->GetRank());
      }
      // Nature's Guardian
      else if (auraSpellInfo->SpellIconID == 2013) {
        // Check health condition - should drop to less 30% (damage deal after
        // this!)
  ...
  if (triggerEntry == nullptr) {
    ...
    // LOG_ERROR("server", "Unit::HandleProcTriggerSpell: Spell %u has 0 in
    // EffectTriggered[%d]. Unhandled custom case?", auraSpellInfo->Id,
  ...
  case 62821:
    if (this->GetTypeId() != TYPEID_PLAYER) // spell has Attribute, but
                                            // persistent area auras ignore it
      return false;
    break;
  ...
  case 71770:
    if (victim)
      victim->CastSpell(victim, trigger_spell_id,
                        true); // EffectImplicitTarget is self
    return true;
  ...
  if (auraSpellInfo->SpellFamilyName == SPELLFAMILY_GENERIC) {
    ...
    if (triggerEntry->SpellIconID == 20) {
      ...
      if (AuraEffect *aurEff = victim->GetAuraEffect(SPELL_AURA_MOD_ROOT,
                                                     SPELLFAMILY_DRUID, 20, 0))
        if ((aurEff->GetBase()->GetMaxDuration() -
             aurEff->GetBase()->GetDuration()) < 1000)
          return false;
  ...
  case 48090: {
    ...
    if (IsPet())
      if (Unit *owner = GetOwner()) {
        ...
        if (AuraEffect *aurEff =
                owner->GetDummyAuraEffect(SPELLFAMILY_WARLOCK, 3220, 0)) {
          basepoints0 =
              int32((aurEff->GetAmount() *
                         owner->SpellBaseDamageBonusDone(
                             SpellSchoolMask(SPELL_SCHOOL_MASK_MAGIC)) +
                     100.0f) /
                    100.0f); // TODO: Is it right?
          CastCustomSpell(this, trigger_spell_id, &basepoints0, &basepoints0,
                          nullptr, true, castItem, triggeredByAura);
          return true;
        }
      }
  ...
  case 52437: // Sudden Death
  {
    ...
    if (AuraEffect const *aurEff = GetAuraEffect(70847, 0)) {
      ...
      if (trigger_spell_id == 46916)
        CastSpell(this, 71072, true, castItem,
                  triggeredByAura); // Slam GCD Reduced
      else
        CastSpell(this, 71069, true, castItem,
  ...
  case 53817: {
    ...
    if (AuraEffect const *aurEff = GetAuraEffect(70832, 0))
      if (Aura const *maelstrom = GetAura(53817))
        // xinef: we have 4 charges and all proc conditions are met - aura
        // reaches 5 charges
  ...
  // Lock and Load
  case 56453: {
    // Proc only from Frost/Freezing trap activation or from Freezing Arrow (the
    // periodic dmg proc handled elsewhere)
  ...
  case 70893: {
    ...
    if (!(procSpell->SpellFamilyFlags[1] & 0x10000000) &&
      ...
      return false;
    // check if we're procced by Claw, Bite or Smack (need to use the spell icon
    // ID to detect it)
  ...
  case 44544: {
    // Find Frostbite
    if (AuraEffect *aurEff = this->GetAuraEffect(
            SPELL_AURA_ADD_TARGET_TRIGGER, SPELLFAMILY_MAGE, 119, EFFECT_0)) {
      if (!victim)
        return false;
      ...
      if (roll_chance_i(chance))
        CastSpell(victim,
                  aurEff->GetSpellInfo()->Effects[EFFECT_0].TriggerSpell, true);
    }
    break;
  }
  }
  ...
  if (basepoints0)
    CastCustomSpell(target, triggerEntry->Id, &basepoints0, nullptr, nullptr,
...
bool Unit::HandleOverrideClassScriptAuraProc(Unit *victim, uint32 /*damage*/,
                                             AuraEffect *triggeredByAura,
                                             SpellInfo const *procSpell,
                                             uint32 cooldown) {
  ...
  if (!triggerEntry) {
    LOG_ERROR("server",
              "Unit::HandleOverrideClassScriptAuraProc: Spell %u triggering "
              "for class script id %u",
...
void Unit::setPowerType(Powers new_powertype) {
  ...
  case POWER_MANA:
    break;
  case POWER_RAGE:
    SetMaxPower(POWER_RAGE, uint32(std::ceil(GetCreatePowers(POWER_RAGE) *
                                             powerMultiplier)));
    SetPower(POWER_RAGE, 0);
    break;
  case POWER_FOCUS:
    SetMaxPower(POWER_FOCUS, uint32(std::ceil(GetCreatePowers(POWER_FOCUS) *
                                              powerMultiplier)));
    SetPower(POWER_FOCUS,
             uint32(std::ceil(GetCreatePowers(POWER_FOCUS) * powerMultiplier)));
    break;
  case POWER_ENERGY:
    SetMaxPower(POWER_ENERGY, uint32(std::ceil(GetCreatePowers(POWER_ENERGY) *
                                               powerMultiplier)));
    break;
  case POWER_HAPPINESS:
    SetMaxPower(
        POWER_HAPPINESS,
        uint32(std::ceil(GetCreatePowers(POWER_HAPPINESS) * powerMultiplier)));
    SetPower(
        POWER_HAPPINESS,
        uint32(std::ceil(GetCreatePowers(POWER_HAPPINESS) * powerMultiplier)));
    break;
  ...
  if (const Player *player = ToPlayer())
    if (player->NeedSendSpectatorData()) {
      ArenaSpectator::SendCommand_UInt32Value(FindMap(), GetGUID(), "PWT",
                                              new_powertype);
      ArenaSpectator::SendCommand_UInt32Value(
          FindMap(), GetGUID(), "MPW",
          new_powertype == POWER_RAGE || new_powertype == POWER_RUNIC_POWER
    ...
    if (GetGUID() != guid) {
      if (Player const *player = ToPlayer())
        LOG_ERROR("server",
                  ...
                  player->GetName().c_str(), getFaction());
      else if (Creature const *creature = ToCreature())
        LOG_ERROR("server",
                  "Creature (template id: %u) has invalid faction (faction "
                  "template id) #%u",
                  creature->GetCreatureTemplate()->Entry, getFaction());
      else
        LOG_ERROR("server",
                  "Unit (name=%s, type=%u) has invalid faction (faction "
                  "template id) #%u",
                  GetName().c_str(), uint32(GetTypeId()), getFaction());
  ...
  if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)) {
    if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)) {
      if (selfPlayerOwner && targetPlayerOwner) {
        // always friendly to other unit controlled by player, or to the player
        // himself
        if (selfPlayerOwner == targetPlayerOwner)
          return REP_FRIENDLY;
        ...
        // same group - checks dependant only on our faction - skip FFA_PVP for
        // example
        if (selfPlayerOwner->IsInRaidWith(targetPlayerOwner))
          return REP_FRIENDLY; // return true to allow config option
                               // AllowTwoSide.Interaction.Group to work
        // however client seems to allow mixed group parties, because in 13850
        // client it works like: return
          ...
          if (!selfPlayerOwner->HasFlag(UNIT_FIELD_FLAGS_2,
            ...
            if (FactionEntry const *targetFactionEntry =
              ...
              if (targetFactionEntry->CanHaveReputation()) {
                ...
                // if faction has reputation, hostile state depends only from
                // AtWar state
    ...
    if (!target->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_IGNORE_REPUTATION)) {
      if (FactionEntry const *factionEntry =
        ...
        if (factionEntry->CanHaveReputation()) {
          // CvP case - check reputation, don't allow state higher than neutral
          // when at war
          ReputationRank repRank =
              targetPlayerOwner->GetReputationMgr().GetRank(factionEntry);
...
bool Unit::Attack(Unit *victim, bool meleeAttack) {
  ...
  // if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED)) // pussywizard: wtf? why
  // having this flag prevents from entering combat? it should just prevent melee
  // attack
  //    return false;
  ...
  // Let the pet know we've started attacking someting. Handles melee attacks
  // only Spells such as auto-shot and others handled in
...
void Unit::CombatStop(bool includingCast) {
  ...
  RemoveAllAttackers();
  if (GetTypeId() == TYPEID_PLAYER)
    ToPlayer()->SendAttackSwingCancelAttack(); // melee and ranged forced attack
                                               // cancel
  ClearInCombat();
...
void Unit::ModifyAuraState(AuraStateType flag, bool apply) {
  if (apply) {
      ...
      for (Unit::AuraMap::iterator itr = tAuras.begin(); itr != tAuras.end();
        ...
        if ((*itr).second->GetSpellInfo()->CasterAuraState == flag)
          if (AuraApplication *aurApp =
            ...
            (*itr).second->HandleAllEffects(aurApp, AURA_EFFECT_HANDLE_REAL,
                                            true);
      }
    }
  } else {
    if (HasFlag(UNIT_FIELD_AURASTATE, 1 << (flag - 1))) {
      ...
      if (flag !=
          AURA_STATE_ENRAGE) // enrage aura state triggering continues auras
      {
        Unit::AuraMap &tAuras = GetOwnedAuras();
        for (Unit::AuraMap::iterator itr = tAuras.begin(); itr != tAuras.end();
             ++itr) {
...
bool Unit::HasAuraState(AuraStateType flag, SpellInfo const *spellProto,
                        Unit const *Caster) const {
  if (Caster) {
    if (spellProto) {
      AuraEffectList const &stateAuras =
          Caster->GetAuraEffectsByType(SPELL_AURA_ABILITY_IGNORE_AURASTATE);
...
Minion *Unit::GetFirstMinion() const {
  if (uint64 pet_guid = GetMinionGUID()) {
    ...
    LOG_ERROR("server", "Unit::GetFirstMinion: Minion %u not exist.",
...
Guardian *Unit::GetGuardianPet() const {
  if (uint64 pet_guid = GetPetGUID()) {
    ...
    LOG_FATAL("server",
              "Unit::GetGuardianPet: Guardian " UI64FMTD " not exist.",
              pet_guid);
...
Unit *Unit::GetCharm() const {
  if (uint64 charm_guid = GetCharmGUID()) {
    ...
    LOG_ERROR("server", "Unit::GetCharm: Charmed creature %u not exist.",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("entities.unit", "SetMinion %u for %u, apply %u",
            minion->GetEntry(), GetEntry(), apply);
#endif

  if (apply) {
    if (minion->GetOwnerGUID()) {
      LOG_FATAL("server", "SetMinion: Minion %u is not the minion of owner %u",
    ...
    // Ghoul pets have energy instead of mana (is anywhere better place for this
    // code?)
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
      // Send infinity cooldown - client does that automatically but after relog
      // cooldown needs to be set again
...
void Unit::RemoveAllMinionsByEntry(uint32 entry) {
  for (Unit::ControlSet::iterator itr = m_Controlled.begin();
    ...
    // i think this is safe because i have never heard that a despawned minion
    // will trigger a same minion
  }
}
  ...
  if (apply) {
    if (GetTypeId() == TYPEID_PLAYER) {
      if (!AddUInt64Value(UNIT_FIELD_CHARM, charm->GetGUID()))
        LOG_FATAL("server",
                  "Player %s is trying to charm unit %u, but it already has a "
                  "charmed unit " UI64FMTD "",
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
      if (!RemoveUInt64Value(UNIT_FIELD_CHARM, charm->GetGUID()))
        LOG_FATAL("server",
                  "Player %s is trying to uncharm unit %u, but it has another "
                  "charmed unit " UI64FMTD "",
                  GetName().c_str(), charm->GetEntry(), GetCharmGUID());
    }
...
int32 Unit::DealHeal(Unit *healer, Unit *victim, uint32 addhealth) {
  ...
  if (Player *player = unit->ToPlayer()) {
    ...
    // use the actual gain, as the overheal shall not be counted, skip gain 0
    // (it ignored anyway in to criteria)
    if (gain &&
        player->InBattleground()) // pussywizard: InBattleground() optimization
      player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HEALING_DONE,
                                        gain, 0, victim);
...
Unit *Unit::GetMagicHitRedirectTarget(Unit *victim,
  ...
  if (spellInfo->HasAttribute(SPELL_ATTR0_ABILITY) ||
      spellInfo->HasAttribute(SPELL_ATTR1_CANT_BE_REDIRECTED) ||
      spellInfo->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY))
    return victim;
  ...
  for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin();
    ...
    if (Unit *magnet = (*itr)->GetBase()->GetUnitOwner())
      if (spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK
        ...
        // Xinef: We should choose minimum between flight time and queue time as
        // in reflect, however we dont know flight time at this point, use
        ...
        if (magnet->IsTotem()) {
          ...
          if (spellInfo->Speed > 0.0f) {
            float dist =
                GetDistance(magnet->GetPositionX(), magnet->GetPositionY(),
                            magnet->GetPositionZ());
            if (dist < 5.0f)
              dist = 5.0f;
...
void Unit::RemoveAllControlled() {
  ...
  while (!m_Controlled.empty()) {
    ...
    if (target->GetCharmerGUID() == GetGUID())
      ...
      LOG_ERROR("server",
                "Unit %u is trying to release unit %u which is neither charmed "
                "nor owned by it",
                GetEntry(), target->GetEntry());
  }
  if (GetPetGUID())
    LOG_FATAL("server", "Unit %u is not able to release its pet " UI64FMTD,
              GetEntry(), GetPetGUID());
  if (GetMinionGUID())
    LOG_FATAL("server", "Unit %u is not able to release its minion " UI64FMTD,
              GetEntry(), GetMinionGUID());
  if (GetCharmGUID())
    LOG_FATAL("server", "Unit %u is not able to release its charm " UI64FMTD,
...
Unit *Unit::GetNextRandomRaidMemberOrPet(float radius) {
  ...
  // reserve place for players and pets because resizing vector every unit push
  // is unefficient (vector is reallocated then)
...
void Unit::SendHealSpellLog(Unit *victim, uint32 SpellID, uint32 Damage,
...
int32 Unit::HealBySpell(Unit *victim, SpellInfo const *spellInfo,
  ...
  SendHealSpellLog(victim, spellInfo->Id, addHealth, uint32(addHealth - gain),
                   absorb, critical);
  return gain;
}

void Unit::SendEnergizeSpellLog(Unit *victim, uint32 spellID, uint32 damage,
                                Powers powerType) {
...
void Unit::EnergizeBySpell(Unit *victim, uint32 spellID, uint32 damage,
                           Powers powerType) {
  ...
  SpellInfo const *spellInfo = sSpellMgr->GetSpellInfo(spellID);
  victim->getHostileRefManager().threatAssist(this, float(damage) * 0.5f,
                                              spellInfo);
}

float Unit::SpellPctDamageModsDone(Unit *victim, SpellInfo const *spellProto,
                                   DamageEffectType damagetype) {
  ...
  for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin();
    ...
    if (spellProto->EquippedItemClass == -1 &&
        ...
        (*i)->GetMiscValue() ==
            SPELL_SCHOOL_MASK_NORMAL) // prevent apply mods from weapon specific
                                      // case to non weapon specific spells
                                      // (Example: thunder clap and two-handed
                                      // weapon specialization)
      continue;

    if (!spellProto->ValidateAttribute6SpellDamageMods(this, *i,
                                                       damagetype == DOT))
      continue;
  ...
  for (AuraEffectList::const_iterator i = mDamageDoneVersus.begin();
    ...
    if ((creatureTypeMask & uint32((*i)->GetMiscValue())) &&
        spellProto->ValidateAttribute6SpellDamageMods(this, *i,
                                                      damagetype == DOT))
      AddPct(DoneTotalMod, (*i)->GetAmount());
  ...
  for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
    ...
    case 4919:
      if (victim->HasAuraState(AURA_STATE_HEALTHLESS_35_PERCENT, spellProto,
                               this))
        AddPct(DoneTotalMod, (*i)->GetAmount());
      break;
    }
    ...
    {
      if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID,
                                0x200002, 0, 0))
        AddPct(DoneTotalMod, (*i)->GetAmount());
      break;
    }
    ...
    // Twisted Faith
    case 7377: {
      if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST,
                                0x8000, 0, 0, GetGUID()))
        AddPct(DoneTotalMod, (*i)->GetAmount());
      break;
    }
    ...
    case 7601:
    case 7602: {
      if (victim->GetAuraEffect(SPELL_AURA_MOD_STALKED, SPELLFAMILY_HUNTER,
                                0x400, 0, 0))
        AddPct(DoneTotalMod, (*i)->GetAmount());
      break;
    }
  ...
  case SPELLFAMILY_PRIEST:
    ...
    if (spellProto->SpellFamilyFlags[0] & 0x800000) {
      ...
      if (AuraEffect *aurEff = GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS,
                                             SPELLFAMILY_PRIEST, 2848, 1))
        // Increase Mind Flay damage if Shadow Word: Pain present on target
        if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                  SPELLFAMILY_PRIEST, 0x8000, 0, 0, GetGUID()))
          AddPct(DoneTotalMod, aurEff->GetAmount());
    ...
    // Smite
      ...
      if (AuraEffect *aurEff = GetAuraEffect(55692, 0))
        if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                  SPELLFAMILY_PRIEST, 0x100000, 0, 0,
                                  GetGUID()))
          AddPct(DoneTotalMod, aurEff->GetAmount());
    }
    // Shadow Word: Death
  ...
  case SPELLFAMILY_PALADIN:
    ...
    if ((spellProto->SpellFamilyFlags[1] & 0x400000) &&
      ...
      // Get stack of Holy Vengeance/Blood Corruption on the target added by
      // caster
      uint32 stacks = 0;
      ...
      // + 10% for each application of Holy Vengeance/Blood Corruption on the
      // target
      if (stacks)
  ...
  case SPELLFAMILY_HUNTER:
    ...
    if (spellProto->SpellFamilyFlags[1] & 0x1)
      if (AuraEffect *aurEff = GetAuraEffect(56826, 0)) // Glyph of Steady Shot
        if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                  SPELLFAMILY_HUNTER, 0x00004000, 0, 0,
                                  GetGUID()))
          AddPct(DoneTotalMod, aurEff->GetAmount());
    break;
  case SPELLFAMILY_DEATHKNIGHT:
...
uint32 Unit::SpellDamageBonusDone(Unit *victim, SpellInfo const *spellProto,
                                  uint32 pdamage, DamageEffectType damagetype,
                                  float TotalMod, uint32 stack) {
  if (!spellProto || !victim || damagetype == DIRECT_DAMAGE)
    return pdamage;
  ...
  if (GetTypeId() == TYPEID_UNIT) {
    if (IsTotem()) {
      if (Unit *owner = GetOwner())
        return owner->SpellDamageBonusDone(victim, spellProto, pdamage,
                                           damagetype, TotalMod, stack);
    }
    // Dancing Rune Weapon...
    else if (GetEntry() == 27893) {
      if (Unit *owner = GetOwner())
  ...
  if (Player *modOwner = GetSpellModOwner())
    modOwner->ApplySpellMod(spellProto->Id,
                            damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE,
                            tmpDamage);

  return uint32(std::max(tmpDamage, 0.0f));
}

uint32 Unit::SpellDamageBonusTaken(Unit *caster, SpellInfo const *spellProto,
                                   uint32 pdamage, DamageEffectType damagetype,
                                   uint32 stack) {
  ...
  for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
       i != mTotalAuraList.end(); ++i)
    if (((*i)->GetMiscValue() & spellProto->GetSchoolMask()))
      if (spellProto->ValidateAttribute6SpellDamageMods(caster, *i,
                                                        damagetype == DOT))
  ...
  for (AuraEffectList::const_iterator i = mDummyAuras.begin();
    ...
    case 2109:
      if ((*i)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) {
        // Patch 2.4.3: The resilience required to reach the 90% damage
        // reduction cap
  ...
  int32 TakenAdvertisedBenefit =
      SpellBaseDamageBonusTaken(spellProto->GetSchoolMask(), damagetype == DOT);
  ...
  if (TakenAdvertisedBenefit) {
    if (coeff <= 0.0f) {
      if (caster)
        coeff = caster->CalculateDefaultCoefficient(spellProto, damagetype) *
                int32(stack);
      else
        coeff =
            CalculateDefaultCoefficient(spellProto, damagetype) * int32(stack);
    }
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) {
  ...
  if (GetTypeId() == TYPEID_PLAYER) {
    ...
    for (AuraEffectList::const_iterator i = mDamageDoneOfStatPercent.begin();
...
int32 Unit::SpellBaseDamageBonusTaken(SpellSchoolMask schoolMask, bool isDoT) {
  ...
  for (AuraEffectList::const_iterator i = mDamageTaken.begin();
    ...
    if (((*i)->GetMiscValue() & schoolMask) != 0) {
      // Xinef: if we have DoT damage type and aura has charges, check if it
      // affects DoTs Xinef: required for hemorrhage & rupture / garrote
...
float Unit::SpellTakenCritChance(Unit const *caster,
                                 SpellInfo const *spellProto,
                                 SpellSchoolMask schoolMask, float doneChance,
                                 WeaponAttackType attackType,
                                 bool skipEffectCheck) const {
  // not critting spell
  if (spellProto->HasAttribute(SPELL_ATTR2_CANT_CRIT))
  ...
  case SPELL_DAMAGE_CLASS_MAGIC: {
    if (!spellProto->IsPositive()) {
      ...
      // SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE xinef: apply max and min only
      ...
      Unit::ApplyResilience(this, &crit_chance, nullptr, false,
                            CR_CRIT_TAKEN_SPELL);
    }
    ...
    if (caster) {
      ...
      for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
        ...
        case 7917: // Glyph of Shadowburn
          if (HasAuraState(AURA_STATE_HEALTHLESS_35_PERCENT, spellProto,
                           caster))
      ...
      case SPELLFAMILY_DRUID:
        ...
        if (HasAuraState(AURA_STATE_FAERIE_FIRE))
          if (AuraEffect const *aurEff =
                  caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 109, 0))
            crit_chance += aurEff->GetAmount();
        ...
        if (spellProto->SpellFamilyFlags[0] & 0x4 &&
          ...
          if (AuraEffect const *aurEff =
                  caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 1771, 0))
            if (GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID,
                              0x00000002, 0, 0))
              crit_chance += aurEff->GetAmount();
          break;
        }
      ...
      case SPELLFAMILY_SHAMAN:
        ...
        if (spellProto->SpellFamilyFlags[1] & 0x00001000) {
          if (GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN,
  ...
  case SPELL_DAMAGE_CLASS_MELEE:
    ...
    if (caster)
      switch (spellProto->SpellFamilyName) {
      case SPELLFAMILY_DRUID:
        // Rend and Tear - bonus crit chance for Ferocious Bite on bleeding
        // targets
        if (spellProto->SpellFamilyFlags[0] & 0x00800000 &&
          ...
          if (AuraEffect const *rendAndTear =
                  caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 2859, 1))
  ...
  case SPELL_DAMAGE_CLASS_RANGED: {
    ...
    // reduce crit chance from Rating for players
    if (attackType != RANGED_ATTACK) {
      // xinef: little hack, crit chance dont require caster to calculate, pass
      // victim
      Unit::ApplyResilience(this, &crit_chance, nullptr, false,
                            CR_CRIT_TAKEN_MELEE);
    } else
...
uint32 Unit::SpellCriticalDamageBonus(Unit const *caster,
                                      SpellInfo const *spellProto,
                                      uint32 damage, Unit const *victim) {
...
uint32 Unit::SpellCriticalHealingBonus(Unit const *caster,
  ...
  if (caster)
    damage = int32(float(damage) * caster->GetTotalAuraMultiplier(
                                       SPELL_AURA_MOD_CRITICAL_HEALING_AMOUNT));

  return damage;
}

float Unit::SpellPctHealingModsDone(Unit *victim, SpellInfo const *spellProto,
                                    DamageEffectType damagetype) {
  ...
  for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
    ...
    {
      if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_DRUID,
                                0x40, 0, 0))
        AddPct(DoneTotalMod, (*i)->GetAmount());
      break;
    }
    ...
    case 7871: // Glyph of Lesser Healing Wave
    {
      ...
      if (victim->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, 0,
                                0x00000400, 0))
        AddPct(DoneTotalMod, (*i)->GetAmount());
      break;
    }
...
uint32 Unit::SpellHealingBonusDone(Unit *victim, SpellInfo const *spellProto,
                                   uint32 healamount,
                                   DamageEffectType damagetype, float TotalMod,
                                   uint32 stack) {
  // For totems get healing bonus from owner (statue isn't totem in fact)
  if (GetTypeId() == TYPEID_UNIT && IsTotem())
    if (Unit *owner = GetOwner())
      return owner->SpellHealingBonusDone(victim, spellProto, healamount,
                                          damagetype, TotalMod, stack);
  ...
  for (AuraEffectList::const_iterator i = mOverrideClassScript.begin();
    ...
    case 3736: // Hateful Totem of the Third Wind / Increased Lesser Healing
               // Wave / LK Arena (4/5/6) Totem of the Third Wind / Savage Totem
               // of the Third Wind
      DoneTotal += (*i)->GetAmount();
      break;
    }
...
uint32 Unit::SpellHealingBonusTaken(Unit *caster, SpellInfo const *spellProto,
                                    uint32 healamount,
  ...
  if (TakenAdvertisedBenefit) {
    ...
    if (coeff <= 0)
      coeff = CalculateDefaultCoefficient(spellProto, damagetype) *
              int32(stack) * 1.88f; // As wowwiki says: C = (Cast Time / 3.5)
                                    // * 1.88 (for healing spells)
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask) {
  ...
  if (GetTypeId() == TYPEID_PLAYER) {
    ...
    for (AuraEffectList::const_iterator i = mHealingDoneOfStatPercent.begin();
...
bool Unit::IsImmunedToSpell(SpellInfo const *spellInfo) {
  ...
  // xinef: my special immunity, if spellid is not on this list it means npc is
  // immune
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const *spellInfo,
  ...
  if (uint32 aura = spellInfo->Effects[index].ApplyAuraName) {
    ...
    for (SpellImmuneList::const_iterator itr = list.begin(); itr != list.end();
         ++itr)
      if (itr->type == aura &&
          (itr->spellId != 64848 ||
           spellInfo->Effects[index].MiscValue == POWER_MANA))
...
uint32 Unit::MeleeDamageBonusDone(Unit *victim, uint32 pdamage,
                                  WeaponAttackType attType,
                                  SpellInfo const *spellProto) {
  if (!victim || pdamage == 0)
    return 0;
  ...
  if (GetTypeId() == TYPEID_UNIT) {
    ...
    if (GetEntry() == 27893) {
      if (Unit *owner = GetOwner())
        return owner->MeleeDamageBonusDone(victim, pdamage, attType,
                                           spellProto) /
               2;
    }
  }
  ...
  // ..done (base at attack power for marked target and base at attack power for
  // creature type)
  int32 APbonus = 0;
...
uint32 Unit::MeleeDamageBonusTaken(Unit *attacker, uint32 pdamage,
                                   WeaponAttackType attType,
                                   SpellInfo const *spellProto) {
  if (pdamage == 0)
    return 0;
  ...
  for (AuraEffectList::const_iterator i = mDamageTaken.begin();
       i != mDamageTaken.end(); ++i)
    if ((*i)->GetMiscValue() &
        (spellProto ? spellProto->GetSchoolMask()
  ...
  if (spellProto) {
    ...
    // Shred, Maul - "Effects which increase Bleed damage also increase Shred
    // damage"
    if (spellProto->SpellFamilyName == SPELLFAMILY_DRUID &&
  ...
  if (TakenTotalMod < 1.0f &&
    ...
    for (AuraEffectList::const_iterator j = ResIgnoreAuras.begin();
         j != ResIgnoreAuras.end(); ++j)
      if ((*j)->GetMiscValue() & (spellProto ? spellProto->GetSchoolMask()
...
void Unit::ApplySpellImmune(uint32 spellId, uint32 op, uint32 type, bool apply,
  ...
  if (apply) {
    ...
    if (spellId == 0 &&
        std::find_if(m_spellImmune[op].begin(), m_spellImmune[op].end(),
                     spellIdImmunityPredicate(type)) != m_spellImmune[op].end())
      return;
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry) {
  ...
  if (Player *player = ToPlayer()) {
    ...
    if (pet) {
      ...
      // don't unsummon pet in arena but SetFlag UNIT_FLAG_STUNNED to disable
      // pet's interface
      if (bg && bg->isArena())
...
void Unit::Dismount() {
  ...
  // this prevents adding a pet to a not created map which would otherwise cause
  // a crash (it could probably happen when logging in after a previous crash)
  if (Player *player = ToPlayer()) {
    if (Pet *pPet = player->GetPet()) {
      if (pPet->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_STUNNED) &&
...
void Unit::CombatStart(Unit *target, bool initialAggro) {
  ...
  if (initialAggro) {
    ...
    if (!target->IsInCombat() && target->GetTypeId() != TYPEID_PLAYER &&
      ...
      if (target->IsPet())
        target->ToCreature()->AI()->AttackedBy(
...
void Unit::SetInCombatState(bool PvP, Unit *enemy, uint32 duration) {
  ...
  if (PvP)
    m_CombatTimer =
        std::max<uint32>(GetCombatTimer(), std::max<uint32>(5500, duration));
  else if (duration)
  ...
  // xinef: if we somehow engage in combat (scripts, dunno) with player, remove
  // this flag so he can fight back
  if (GetTypeId() == TYPEID_UNIT && enemy &&
      HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC) &&
      enemy->GetCharmerOrOwnerPlayerOrPlayerItself())
    RemoveFlag(UNIT_FIELD_FLAGS,
               UNIT_FLAG_IMMUNE_TO_PC); // unit has engaged in combat, remove
                                        // immunity so players can fight back
  ...
  if (Creature *creature = ToCreature()) {
    ...
    if ((IsAIEnabled && creature->AI()->IsEscorted()) ||
        ...
        GetMotionMaster()->GetCurrentMovementGeneratorType() ==
            ESCORT_MOTION_TYPE)
      creature->SetHomePosition(GetPositionX(), GetPositionY(), GetPositionZ(),
                                GetOrientation());
...
void Unit::ClearInCombat() {
  ...
  if (Creature *creature = ToCreature()) {
    if (creature->GetCreatureTemplate() &&
        creature->GetCreatureTemplate()->unit_flags & UNIT_FLAG_IMMUNE_TO_PC)
      SetFlag(UNIT_FIELD_FLAGS,
              UNIT_FLAG_IMMUNE_TO_PC); // set immunity state to the one from db
                                       // on evade
...
bool Unit::isTargetableForAttack(bool checkFakeDeath, Unit const *byWho) const {
  ...
  if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC) && byWho &&
...
bool Unit::_IsValidAttackTarget(Unit const *target, SpellInfo const *bySpell,
  ...
  if (m_vehicle)
    if (IsOnVehicle(target) || m_vehicle->GetBase()->IsOnVehicle(target))
      if (!IsHostileTo(target)) // pussywizard: actually can attack own vehicle
                                // or passenger if it's hostile to us - needed
                                // for snobold in Gormok encounter
        return false;
  ...
  if ((!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_CAN_TARGET_INVISIBLE)) &&
      (obj ? !obj->CanSeeOrDetect(target, bySpell && bySpell->IsAffectingArea())
           : !CanSeeOrDetect(
                 target, (bySpell && bySpell->IsAffectingArea()) ||
                             (target->GetTypeId() == TYPEID_PLAYER &&
  ...
  if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE |
      ...
      (!target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) &&
       HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC)) ||
      (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) &&
       target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC))
      // check if this is a world trigger cast - GOs are using world triggers to
      // cast their spells, so we need to ignore their immunity flag here, this
      // is a temp workaround, needs removal when go cast is implemented
      // properly
        ...
        if (!(player->GetReputationMgr().GetForcedRankIfAny(factionTemplate)))
          if (FactionEntry const *factionEntry =
            ...
            if (FactionState const *repState =
                    player->GetReputationMgr().GetState(factionEntry))
  ...
  if (creatureAttacker && creatureAttacker->GetCreatureTemplate()->type_flags &
                              CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT)
    return false;

  Player const *playerAffectingAttacker =
      HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) ? GetAffectingPlayer()
                                                          : nullptr;
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced) {
  ...
  case MOVE_RUN: {
    if (IsMounted()) // Use on mount auras
    {
      ...
      non_stack_bonus +=
          GetMaxPositiveAuraModifier(SPELL_AURA_MOD_SPEED_NOT_STACK) / 100.0f;
    }
    break;
  }
  case MOVE_SWIM: {
  ...
  case MOVE_FLIGHT: {
        ...
        IsControlledByPlayer()) // not sure if good for pet
    {
      ...
      stack_bonus =
          GetTotalAuraMultiplier(SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS);
  ...
  case MOVE_FLIGHT: {
    if (GetTypeId() == TYPEID_UNIT) {
      ...
      if (pOwner && !IsInCombat() && !IsVehicle() &&
          !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
          (IsPet() || IsGuardian() || GetGUID() == pOwner->GetCritterGUID() ||
           GetCharmerGUID() == pOwner->GetGUID())) {
        if (pOwner->GetTypeId() != TYPEID_PLAYER) {
          if (speed < pOwner->GetSpeedRate(mtype) + 0.1f)
            speed = pOwner->GetSpeedRate(mtype) +
                    0.1f; // pets derive speed from owner when not in combat
        } else {
          ...
          float maxDist =
              ownerSpeed >= 1.0f ? minDist * ownerSpeed * 1.5f : minDist * 1.5f;
        ...
        speed *= ToCreature()
                     ->GetCreatureTemplate()
                     ->speed_run; // at this point, MOVE_WALK is never reached
    }

    // Normalize speed by 191 aura SPELL_AURA_USE_NORMAL_MOVEMENT_SPEED if need
    // TODO: possible affect only on MOVE_RUN
    if (int32 normalization =
            GetMaxPositiveAuraModifier(SPELL_AURA_USE_NORMAL_MOVEMENT_SPEED)) {
      // Use speed from aura
      float max_speed =
          normalization / (IsControlledByPlayer() ? playerBaseMoveSpeed[mtype]
                                                  : baseMoveSpeed[mtype]);
  ...
  if (GetTypeId() == TYPEID_UNIT) {
    if (ToCreature()->HasSearchedAssistance())
      speed *=
          0.66f; // best guessed value, so this will be 33% reduction. Based off
                 // initial speed, mob can then "run", "walk fast" or "walk".
  }
...
void Unit::SetSpeed(UnitMoveType mtype, float rate, bool forced) {
  ...
  if (!forced) {
    ...
    default:
      LOG_ERROR("server",
                "Unit::SetSpeed: Unsupported move type (%d), data not sent to "
                "client.",
                mtype);
      return;
    }
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
      ...
      // WorldSession::HandleForceSpeedChangeAck and do it only for real sent
      // packets and use run for run/mounted as client expected
      ...
      if (!IsInCombat()) {
        ...
        if (pet &&
            (pet->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) ||
             pet->IsVehicle()))
          pet = nullptr;

        if (pet && pet->GetTypeId() == TYPEID_UNIT && !pet->IsInCombat() &&
...
void Unit::setDeathState(DeathState s, bool despawn) {
  ...
  if (s == JUST_DIED) {
    ...
    // without this when removing IncreaseMaxHealth aura player may stuck with 1
    // hp do not why since in IncreaseMaxHealth currenthealth is checked
    SetHealth(0);
    ...
    RemoveFlag(UNIT_FIELD_FLAGS,
               UNIT_FLAG_SKINNABLE); // clear skinnable for creature and player
                                     // (at battleground)
  }
...
bool Unit::CanHaveThreatList() const {
  ...
  if (HasUnitTypeMask(UNIT_MASK_MINION | UNIT_MASK_GUARDIAN |
      ...
      IS_PLAYER_GUID(((Pet *)this)->GetOwnerGUID()))
    return false;
...
void Unit::AddThreat(Unit *victim, float fThreat, SpellSchoolMask schoolMask,
...
Unit *Creature::SelectVictim() {
  ...
  if (!tauntAuras.empty())
    for (Unit::AuraEffectList::const_reverse_iterator itr = tauntAuras.rbegin();
         itr != tauntAuras.rend(); ++itr)
      if (Unit *caster = (*itr)->GetCaster())
        if (_CanDetectFeignDeathOf(caster) && CanCreatureAttack(caster) &&
            !caster->HasAuraTypeWithCaster(SPELL_AURA_IGNORED, GetGUID())) {
          target = caster;
          break;
        }
  ...
  if (CanHaveThreatList()) {
    ...
    if (!target && IsSummon())
      if (Unit *owner = ToTempSummon()->GetOwner()) {
        ...
        if (!target)
          for (ControlSet::const_iterator itr = owner->m_Controlled.begin();
  ...
  // it in combat but attacker not make any damage and not enter to aggro radius
  // to have record in threat list Note: creature does not have targeted
  // movement generator but has attacker in this case
  ...
  if (HasReactState(REACT_AGGRESSIVE)) {
    target = SelectNearestTargetInAttackDistance(
        std::max<float>(m_CombatDistance, ATTACK_DISTANCE));
    if (target && _CanDetectFeignDeathOf(target) && CanCreatureAttack(target))
      return target;
  }
...
float Unit::ApplyEffectModifiers(SpellInfo const *spellProto,
                                 uint8 effect_index, float value) const {
  if (Player *modOwner = GetSpellModOwner()) {
...
int32 Unit::CalcSpellDuration(SpellInfo const *spellProto) {
  uint8 comboPoints =
      m_movedByPlayer ? m_movedByPlayer->ToPlayer()->GetComboPoints() : 0;
...
int32 Unit::ModSpellDuration(SpellInfo const *spellProto, Unit const *target,
                             int32 duration, bool positive, uint32 effectMask) {
  ...
  if (!positive) {
    ...
    for (uint8 i = 1; i <= MECHANIC_ENRAGED; ++i) {
      ...
      // Xinef: spells affecting movement imparing effects should not reduce
      // duration if disoriented mechanic is present
      if (i == MECHANIC_SNARE && (mechanic & (1 << MECHANIC_DISORIENTED)))
        continue;

      // Find total mod value (negative bonus)
      int32 new_durationMod_always = target->GetTotalAuraModifierByMiscValue(
...
void Unit::ModSpellCastTime(SpellInfo const *spellInfo, int32 &castTime,
                            Spell *spell) {
  ...
  if (Player *modOwner = GetSpellModOwner())
    ...
    modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_CASTING_TIME, castTime,
                            spell, bool(modOwner != this && !IsPet()));

  switch (spellInfo->DmgClass) {
  case SPELL_DAMAGE_CLASS_NONE:
    if (spellInfo->AttributesEx5 &
        SPELL_ATTR5_HASTE_AFFECT_DURATION) // required double check
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group) {
  for (Diminishing::iterator i = m_Diminishing.begin();
    ...
    if (i->stack == 0 &&
        getMSTimeDiff(i->hitTime, World::GetGameTimeMS()) > 15000) {
...
void Unit::IncrDiminishing(DiminishingGroup group) {
  ...
  m_Diminishing.push_back(
      DiminishingReturn(group, World::GetGameTimeMS(), DIMINISHING_LEVEL_2));
}

float Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32 &duration,
                                       Unit *caster, DiminishingLevels Level,
                                       int32 limitduration) {
...
float Unit::GetSpellMaxRangeForTarget(Unit const *target,
  ...
  if (!spellInfo->RangeEntry)
    return 0;
  if (spellInfo->RangeEntry->maxRangeFriend ==
      spellInfo->RangeEntry->maxRangeHostile)
...
float Unit::GetSpellMinRangeForTarget(Unit const *target,
  ...
  if (!spellInfo->RangeEntry)
    return 0;
  if (spellInfo->RangeEntry->minRangeFriend ==
      spellInfo->RangeEntry->minRangeHostile)
...
bool Unit::HandleStatModifier(UnitMods unitMod, UnitModifierType modifierType,
  ...
  if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_END) {
    LOG_ERROR("server", "ERROR in HandleStatModifier(): non-existing UnitMods "
                        "or wrong UnitModifierType!");
    return false;
  }
  ...
  case BASE_PCT:
  case TOTAL_PCT:
    ApplyPercentModFloatVar(m_auraModifiersGroup[unitMod][modifierType], amount,
                            apply);
    break;
  default:
    break;
...
void Unit::ApplyStatPercentBuffMod(Stats stat, float val, bool apply) {
  ...
  ApplyModSignedFloatValue(
      (var > 0 ? UNIT_FIELD_POSSTAT0 + stat : UNIT_FIELD_NEGSTAT0 + stat), var,
      apply);
}
...
float Unit::GetTotalAttackPowerValue(WeaponAttackType attType,
  ...
  if (attType == RANGED_ATTACK) {
    ...
    if (ap < 0)
      return 0.0f;
    return ap *
           (1.0f + GetFloatValue(UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER));
  } else {
...
void Unit::SetLevel(uint8 lvl, bool showLevelChange) {
  ...
  if (GetTypeId() == TYPEID_PLAYER)
    sWorld->UpdateGlobalPlayerData(ToPlayer()->GetGUIDLow(),
                                   PLAYER_UPDATE_DATA_LEVEL, "", lvl);
}
      ...
      if (Unit *owner = GetOwner())
        if (Player *player = owner->ToPlayer()) {
          if (player->NeedSendSpectatorData() &&
            ...
            ArenaSpectator::SendCommand_UInt32Value(
                player->FindMap(), player->GetGUID(), "PHP",
...
void Unit::SetPower(Powers power, uint32 val) {
  ...
  if (GetTypeId() == TYPEID_PLAYER) {
    ...
    if (getPowerType() == power && player->NeedSendSpectatorData())
      ArenaSpectator::SendCommand_UInt32Value(
          FindMap(), GetGUID(), "CPW",
          power == POWER_RAGE || power == POWER_RUNIC_POWER ? val / 10 : val);
...
void Unit::RemoveFromWorld() {
  ...
  if (IsInWorld()) {
    ...
    if (GetCharmerGUID()) {
      LOG_FATAL("server", "Unit %u has charmer guid when removed from world",
                GetEntry());
      ABORT();
    }
...
void Unit::CleanupBeforeRemoveFromMap(bool finalCleanup) {
  ...
  // This needs to be before RemoveFromWorld to make GetCaster() return a valid
  // pointer on aura removal
  ...
  m_Events.KillAllEvents(false); // non-delatable (currently casted spells) will
                                 // not deleted now but it will deleted at call
...
CharmInfo::CharmInfo(Unit *unit)
    : _unit(unit), _CommandState(COMMAND_FOLLOW), _petnumber(0),
      ...
      _isReturning(false), _forcedSpellId(0), _forcedTargetGUID(0),
...
void CharmInfo::InitPetActionBar() {
  ...
  for (uint32 i = 0; i < ACTION_BAR_INDEX_END - ACTION_BAR_INDEX_PET_SPELL_END;
       ++i)
...
void CharmInfo::InitPossessCreateSpells() {
  if (_unit->GetTypeId() == TYPEID_UNIT) {
    ...
    // Adding entrys to this switch will prevent COMMAND_ATTACK being added to
    // pet bar.
...
void createProcFlags(const SpellInfo *spellInfo, WeaponAttackType attackType,
                     bool positive, uint32 &procAttacker, uint32 &procVictim) {
  if (spellInfo) {
    switch (spellInfo->DmgClass) {
    case SPELL_DAMAGE_CLASS_MAGIC:
      if (positive) {
    ...
    default:
      if (spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
          ...
          spellInfo->HasAttribute(
              SPELL_ATTR2_AUTOREPEAT_FLAG)) // Wands auto attack
      {
        procAttacker = PROC_FLAG_DONE_RANGED_AUTO_ATTACK;
        procVictim = PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK;
      }
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit *target, uint32 procFlag,
                                 uint32 procExtra, WeaponAttackType attType,
                                 SpellInfo const *procSpell, uint32 damage,
                                 SpellInfo const *procAura) {
  ...
  if (procFlag & MELEE_BASED_TRIGGER_MASK && target) {
    ...
    if (GetTypeId() == TYPEID_PLAYER) {
      // On melee based hit/miss/resist need update skill (for victim and
      // attacker)
      ...
      // Update defence if player is victim and parry/dodge/block
               ...
               procExtra & (PROC_EX_DODGE | PROC_EX_PARRY | PROC_EX_BLOCK))
        ToPlayer()->UpdateCombatSkills(target, attType, true);
    }
    // If exist crit/parry/dodge/block need update aura state (for victim and
    // attacker)
  ...
  DamageInfo damageInfo =
      DamageInfo(actor, actionTarget, damage, procSpell,
                 procSpell ? SpellSchoolMask(procSpell->SchoolMask)
                           : SPELL_SCHOOL_MASK_NORMAL,
                 SPELL_DIRECT_DAMAGE);
  HealInfo healInfo =
      HealInfo(actor, actionTarget, damage, procSpell,
               procSpell ? SpellSchoolMask(procSpell->SchoolMask)
                         : SPELL_SCHOOL_MASK_NORMAL);
  ProcEventInfo eventInfo =
      ProcEventInfo(actor, actionTarget, target, procFlag, 0, 0, procExtra,
                    nullptr, &damageInfo, &healInfo, procAura);
  ...
  for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin();
    ...
    // Defensive procs are active on absorbs (so absorption effects are not a
    // hindrance)
    ...
    // xinef: fix spell procing from damaging / healing casts if spell has DoT /
    // HoT effect only only player spells are taken into account
    if (!active && !isVictim && !(procFlag & PROC_FLAG_DONE_PERIODIC) &&
        procSpell && procSpell->SpellFamilyName &&
        (procSpell->HasAura(SPELL_AURA_PERIODIC_DAMAGE) ||
         procSpell->HasAura(SPELL_AURA_PERIODIC_HEAL)))
      active = true;

    if (!IsTriggeredAtSpellProcEvent(
            target, triggerData.aura, procSpell, procFlag, procExtra, attType,
            isVictim, active, triggerData.spellProcEvent, eventInfo))
      continue;
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
      if (itr->second->HasEffect(i)) {
        ...
        if (!isTriggerAura[aurEff->GetAuraType()] &&
            triggerData.spellProcEvent == nullptr)
          continue;
  ...
  for (ProcTriggeredList::const_iterator i = procTriggered.begin();
    ...
    if (!handled && HandleAuraProc(target, damage, i->aura, procSpell, procFlag,
                                   procExtra, cooldown, &handled)) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      uint32 Id = i->aura->GetId();
      LOG_DEBUG("spells.aura",
                "ProcDamageAndSpell: casting spell %u (triggered with value by "
                "%s aura of spell %u)",
                spellInfo->Id, (isVictim ? "a victim's" : "an attacker's"), Id);
#endif
      takeCharges = true;
    }
      ...
      for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("spells.aura",
                    "ProcDamageAndSpell: casting spell %u (triggered by %s "
                    "aura of spell %u)",
                    spellInfo->Id, (isVictim ? "a victim's" : "an attacker's"),
                    triggeredByAura->GetId());
#endif
          // Don`t drop charge or add cooldown for not started trigger
          if (HandleProcTriggerSpell(target, damage, triggeredByAura, procSpell,
                                     procFlag, procExtra, cooldown))
            takeCharges = true;
          break;
        }
        case SPELL_AURA_PROC_TRIGGER_DAMAGE: {
          ...
          triggeredByAura->HandleProcTriggerDamageAuraProc(
              aurApp,
              eventInfo); // this function is part of the new proc system
          takeCharges = true;
          break;
        }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("spells.aura",
                    "ProcDamageAndSpell: casting spell id %u (triggered by %s "
                    "dummy aura of spell %u)",
                    spellInfo->Id, (isVictim ? "a victim's" : "an attacker's"),
                    triggeredByAura->GetId());
#endif
          if (HandleDummyAuraProc(target, damage, triggeredByAura, procSpell,
                                  procFlag, procExtra, cooldown))
            takeCharges = true;
          break;
        }
        ...
        case SPELL_AURA_OVERRIDE_CLASS_SCRIPTS: {
          ...
          LOG_DEBUG("spells.aura",
                    ...
                    triggeredByAura->GetId());
#endif
          if (HandleOverrideClassScriptAuraProc(target, damage, triggeredByAura,
                                                procSpell, cooldown))
            takeCharges = true;
          break;
        }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("spells.aura",
                    "ProcDamageAndSpell: casting mending (triggered by %s "
                    "dummy aura of spell %u)",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
          LOG_DEBUG("spells.aura",
                    "ProcDamageAndSpell: casting spell %u (triggered with "
                    "value by %s aura of spell %u)",
                    spellInfo->Id, (isVictim ? "a victim's" : "an attacker's"),
                    triggeredByAura->GetId());
#endif
        ...
        case SPELL_AURA_REFLECT_SPELLS_SCHOOL:
          ...
          if (procSpell && (triggeredByAura->GetMiscValue() &
        ...
        case SPELL_AURA_MOD_MECHANIC_RESISTANCE:
          // Compare mechanic
          if (procSpell &&
              procSpell->Mechanic == uint32(triggeredByAura->GetMiscValue()))
            takeCharges = true;
          break;
        ...
        case SPELL_AURA_TRANSFORM: {
          ...
          // Xinef: Or when the aura is at full duration (assume that such auras
          // should be added at the end, skipping all damage procs etc.)
          if (procSpell)
            if ((!i->aura->IsPermanent() &&
                 i->aura->GetDuration() == i->aura->GetMaxDuration()) ||
                procSpell->Id == triggeredByAura->GetId() ||
                procSpell->HasAttribute(SPELL_ATTR4_DAMAGE_DOESNT_BREAK_AURAS))
              break;
        ...
        case SPELL_AURA_ABILITY_IGNORE_AURASTATE:
          if (procSpell &&
              procSpell->Id ==
                  20647) // hack for warriors execute, both dummy and damage
                         // spell are affected by ignore aurastate aura
            break;
          takeCharges = true;
          break;
        default:
        ...
        i->aura->CallScriptAfterEffectProcHandlers(triggeredByAura, aurApp,
                                                   eventInfo);
      }
    // Remove charge (aura can be removed by triggers)
    // xinef: take into account attribute6 of proc spell
    if (prepare && useCharges && takeCharges)
      if (!procSpell || isVictim ||
          !procSpell->HasAttribute(SPELL_ATTR6_DONT_CONSUME_PROC_CHARGES))
        i->aura->DropCharge();
...
void Unit::GetProcAurasTriggeredOnEvent(
    ...
    std::list<AuraApplication *> *procAuras, ProcEventInfo eventInfo) {
  // use provided list of auras which can proc
  if (procAuras) {
    for (std::list<AuraApplication *>::iterator itr = procAuras->begin();
         itr != procAuras->end(); ++itr) {
...
void Unit::TriggerAurasProcOnEvent(CalcDamageInfo &damageInfo) {
  ...
  TriggerAurasProcOnEvent(nullptr, nullptr, damageInfo.target,
                          damageInfo.procAttacker, damageInfo.procVictim, 0, 0,
                          damageInfo.procEx, nullptr, &dmgInfo, nullptr);
}

void Unit::TriggerAurasProcOnEvent(
    std::list<AuraApplication *> *myProcAuras,
    std::list<AuraApplication *> *targetProcAuras, Unit *actionTarget,
    uint32 typeMaskActor, uint32 typeMaskActionTarget, uint32 spellTypeMask,
    uint32 spellPhaseMask, uint32 hitMask, Spell *spell, DamageInfo *damageInfo,
    HealInfo *healInfo) {
  // prepare data for self trigger
  ProcEventInfo myProcEventInfo = ProcEventInfo(
      this, actionTarget, actionTarget, typeMaskActor, spellTypeMask,
      spellPhaseMask, hitMask, spell, damageInfo, healInfo, nullptr);
  std::list<AuraApplication *> myAurasTriggeringProc;
  GetProcAurasTriggeredOnEvent(myAurasTriggeringProc, myProcAuras,
                               myProcEventInfo);

  // prepare data for target trigger
  ProcEventInfo targetProcEventInfo = ProcEventInfo(
      this, actionTarget, this, typeMaskActionTarget, spellTypeMask,
      spellPhaseMask, hitMask, spell, damageInfo, healInfo, nullptr);
  std::list<AuraApplication *> targetAurasTriggeringProc;
  if (typeMaskActionTarget)
    GetProcAurasTriggeredOnEvent(targetAurasTriggeringProc, targetProcAuras,
                                 targetProcEventInfo);
...
void Unit::TriggerAurasProcOnEvent(
    ProcEventInfo &eventInfo,
    std::list<AuraApplication *> &aurasTriggeringProc) {
  for (std::list<AuraApplication *>::iterator itr = aurasTriggeringProc.begin();
       itr != aurasTriggeringProc.end(); ++itr) {
    if (!(*itr)->GetRemoveMode())
...
void Unit::RestoreDisplayId() {
  ...
  Unit::AuraEffectList const &transforms =
      GetAuraEffectsByType(SPELL_AURA_TRANSFORM);
  if (!transforms.empty()) {
    // iterate over already applied transform auras - from newest to oldest
    for (Unit::AuraEffectList::const_reverse_iterator i = transforms.rbegin();
         i != transforms.rend(); ++i) {
  ...
  if (handledAuraForced)
    handledAuraForced->HandleEffect(this, AURA_EFFECT_HANDLE_SEND_FOR_CLIENT,
                                    true);
...
void Unit::ClearComboPointHolders() {
  while (!m_ComboPointHolders.empty()) {
    ...
    Player *player = ObjectAccessor::GetPlayer(
        *this, MAKE_NEW_GUID(lowguid, 0, HIGHGUID_PLAYER));
...
Unit *Unit::SelectNearbyTarget(Unit *exclude, float dist) const {
  ...
  Warhead::UnitListSearcher<Warhead::AnyUnfriendlyUnitInObjectRangeCheck>
...
Unit *Unit::SelectNearbyNoTotemTarget(Unit *exclude, float dist) const {
  ...
  Warhead::UnitListSearcher<Warhead::AnyUnfriendlyNoTotemUnitInObjectRangeCheck>
      searcher(this, targets, u_check);
  VisitNearbyObject(dist, searcher);
...
void Unit::ApplyAttackTimePercentMod(WeaponAttackType att, float val,
  ...
  m_attackTimer[att] =
      uint32(GetAttackTime(att) * m_modAttackSpeedPct[att] * remainingTimePct);
}
...
uint32 Unit::GetCastingTimeForBonus(SpellInfo const *spellProto,
  ...
  // 50% for damage and healing spells for leech spells from damage bonus and 0%
  // from healing
...
bool Unit::IsTriggeredAtSpellProcEvent(
    Unit *victim, Aura *aura, SpellInfo const *procSpell, uint32 procFlag,
    uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active,
    SpellProcEventEntry const *&spellProcEvent,
  ...
  // player proc auras Xinef: skip victim auras
  if (!isVictim &&
      GetTypeId() ==
          TYPEID_PLAYER) // spellProto->SpellFamilyName != SPELLFAMILY_GENERIC)
    if (!(EventProcFlag & (PROC_FLAG_KILL | PROC_FLAG_DEATH)) && procSpell &&
        procSpell->SpellFamilyName == SPELLFAMILY_GENERIC &&
  ...
  if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(spellProto, spellProcEvent,
                                                 EventProcFlag, procSpell,
                                                 procFlag, procExtra, active))
    return false;
  ...
  if (spellProcEvent && spellProcEvent->ppmRate != 0) {
    if (!isVictim) {
      ...
      chance =
          GetPPMProcChance(WeaponSpeed, spellProcEvent->ppmRate, spellProto);
    } else if (victim) {
      uint32 WeaponSpeed = victim->GetAttackTime(attType);
      chance = victim->GetPPMProcChance(WeaponSpeed, spellProcEvent->ppmRate,
                                        spellProto);
    }
  }
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect *triggeredByAura) {
      ...
      if (player) {
        ...
        if (!nearMembers.empty()) {
          ...
          if (Unit *target = nearMembers.front()) {
            CastSpell(
                target,
                41637 /*Dummy visual effect triggered by main spell cast*/,
                true);
            CastCustomSpell(target, spellProto->Id, &heal, nullptr, nullptr,
                            true, nullptr, triggeredByAura, caster_guid);
  ...
  // heal
  CastCustomSpell(this, 33110, &heal, nullptr, nullptr, true, nullptr, nullptr,
                  caster_guid);
  return true;
}
bool Unit::HandleAuraRaidProcFromCharge(AuraEffect *triggeredByAura) {
  ...
  case 57949: // shiver
    damageSpellId = 57952;
    // animationSpellId = 57951; dummy effects for jump spell have unknown use
    // (see also 41637)
    break;
  case 59978: // shiver
    damageSpellId = 59979;
  ...
  default:
    LOG_ERROR(
        "server",
        "Unit::HandleAuraRaidProcFromCharge, received unhandled spell: %u",
        spellProto->Id);
    return false;
  }
  ...
  if (jumps > 0) {
    if (Unit *caster = triggeredByAura->GetCaster()) {
      ...
      if (Unit *target = GetNextRandomRaidMemberOrPet(radius)) {
        CastSpell(target, spellProto, true, nullptr, triggeredByAura,
                  caster_guid);
        if (Aura *aura = target->GetAura(spellProto->Id, caster->GetGUID()))
          aura->SetCharges(jumps);
      }
...
void Unit::Kill(Unit *killer, Unit *victim, bool durabilityLoss,
                WeaponAttackType attackType, SpellInfo const *spellProto) {
        ...
        if (group->GetLooterGuid() && group->GetLootMethod() != FREE_FOR_ALL) {
          ...
          if (looter) {
            ...
            creature->SetLootRecipient(
                looter); // update creature loot recipient to the allowed
                         // looter.
          }
        }
      }
  ...
  // Do KILL and KILLED procs. KILL proc is called only for the unit who landed
  // the killing blow (and its owner - for pets and totems) regardless of who
  // tapped the victim
  if (killer && (killer->IsPet() || killer->IsTotem()))
    if (Unit *owner = killer->GetOwner()) {
      owner->ProcDamageAndSpell(victim, PROC_FLAG_KILL, PROC_FLAG_NONE,
                                PROC_EX_NONE, 0, attackType, spellProto);
  ...
  if (killer != victim && !victim->IsCritter())
    killer->ProcDamageAndSpell(victim, killer ? PROC_FLAG_KILL : 0,
                               PROC_FLAG_KILLED, PROC_EX_NONE, 0, attackType,
                               spellProto);

  // Proc auras on death - must be before aura/combat remove
  victim->ProcDamageAndSpell(nullptr, PROC_FLAG_DEATH, PROC_FLAG_NONE,
                             PROC_EX_NONE, 0, attackType, spellProto);

  // update get killing blow achievements, must be done before setDeathState to
  // be able to require auras on target and before Spirit of Redemption as it
  // also removes auras
  if (killer)
  ...
  if (victim->GetTypeId() == TYPEID_PLAYER &&
    ...
    if (AuraEffect *aurEff = victim->GetAuraEffectDummy(20711)) {
      ...
      if (victim->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION) ||
        ...
        /*LOG_INFO("misc", "Player (%u) died with spirit of redemption. Killer
        (Entry: %u, Name: %s), Map: %u, x: %f, y: %f, z: %f",
            victim->GetGUIDLow(), killer ? killer->GetEntry() : 1, killer ?
        killer->GetName().c_str() : "", victim->GetMapId(),
        ...
        // Remove all expected to remove at death auras (most important negative
        // case like DoT or periodic triggers)
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "We are dead, losing %f percent durability",
  ...
  } else // creature died
  {
    ...
    // Call KilledUnit for creatures, this needs to be called after the lootable
    // flag is set
    if (killer && killer->GetTypeId() == TYPEID_UNIT && killer->IsAIEnabled)
      killer->ToCreature()->AI()->KilledUnit(victim);
  ...
  // outdoor pvp things, do these after setting the death state, else the player
  // activity notify won't work... doh... handle player kill only if not suicide
  // (spirit of redemption for example)
...
void Unit::SetStunned(bool apply) {
  if (apply) {
    ...
    // MOVEMENTFLAG_MASK_MOVING (tested 3.3.5a) this will freeze clients. That's
    // why we remove MOVEMENTFLAG_MASK_MOVING before setting MOVEMENTFLAG_ROOT
    ...
    if (GetTypeId() == TYPEID_UNIT) {
      // don't remove UNIT_FLAG_STUNNED for pet when owner is mounted (disabled
      // pet's interface)
      Unit *owner = GetOwner();
      if (!owner || owner->GetTypeId() != TYPEID_PLAYER ||
...
void Unit::SetRooted(bool apply) {
  if (apply) {
    ...
    if (!HasUnitState(
            UNIT_STATE_STUNNED)) // prevent moving if it also has stun effect
    {
      if (GetTypeId() == TYPEID_PLAYER) {
        WorldPacket data(SMSG_FORCE_MOVE_UNROOT, 10);
        data.append(GetPackGUID());
...
void Unit::SetFeared(bool apply) {
  if (apply) {
    ...
    Unit *caster = nullptr;
    Unit::AuraEffectList const &fearAuras =
        GetAuraEffectsByType(SPELL_AURA_MOD_FEAR);
    if (!fearAuras.empty())
      caster =
          ObjectAccessor::GetUnit(*this, fearAuras.front()->GetCasterGUID());
    if (!caster)
      caster = getAttackerForHelper();
    GetMotionMaster()->MoveFleeing(
        caster, fearAuras.empty()
...
bool Unit::SetCharmedBy(Unit *charmer, CharmType type,
  ...
  LOG_DEBUG(
      ...
      charmer->GetEntry(), charmer->GetGUIDLow(), GetEntry(), GetGUIDLow(),
      uint32(type));
#endif
  ...
  if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetTransport()) {
    LOG_FATAL("server",
              "Unit::SetCharmedBy: Player on transport is trying to charm %u "
              "(GUID %u)",
              GetEntry(), GetGUIDLow());
    return false;
  }
  ...
  if (GetCharmerGUID()) {
    LOG_FATAL("server",
              "Unit::SetCharmedBy: %u (GUID %u) has already been charmed but "
              "%u (GUID %u) is trying to charm it!",
  ...
  if (!IsInWorld()) {
    LOG_FATAL("server",
              "Unit::SetCharmedBy: %u (GUID %u) is not in world but %u (GUID "
              "%u) is trying to charm it!",
              GetEntry(), GetGUIDLow(), charmer->GetEntry(),
              charmer->GetGUIDLow());
    return false;
  }
    ...
    case CHARM_TYPE_CHARM:
      if (GetTypeId() == TYPEID_UNIT && charmer->getClass() == CLASS_WARLOCK) {
        ...
        if (cinfo && cinfo->type == CREATURE_TYPE_DEMON) {
          ...
          // if charmed two demons the same session, the 2nd gets the 1st one's
          // name
          SetUInt32Value(UNIT_FIELD_PET_NAME_TIMESTAMP,
                         uint32(time(nullptr))); // cast can't be helped
        }
      }
  ...
  for (ThreatContainer::StorageType::const_iterator itr = threatList.begin();
    ...
    if (Unit *target = (*itr)->getTarget())
      _charmThreatInfo[target->GetGUID()] =
          (*itr)->getThreat() - (*itr)->getTempThreatModifier();
  ...
  if (GetTypeId() == TYPEID_PLAYER)
    sScriptMgr->OnPlayerBeingCharmed(ToPlayer(), charmer, _oldFactionId,
                                     charmer->getFaction());
...
void Unit::RemoveCharmedBy(Unit *charmer) {
  ...
  {
    //        LOG_FATAL("server", "Unit::RemoveCharmedBy: this: " UI64FMTD "
    //        true charmer: " UI64FMTD " false charmer: " UI64FMTD,
      ...
      if (GetTypeId() == TYPEID_UNIT && charmer->getClass() == CLASS_WARLOCK) {
        ...
        if (cinfo && cinfo->type == CREATURE_TYPE_DEMON) {
          ...
          if (GetCharmInfo())
            ...
            LOG_ERROR("server",
                      "Aura::HandleModCharm: target=" UI64FMTD
                      " with typeid=%d has a charm aura but no charm info!",
                      GetGUID(), GetTypeId());
        }
      }
      break;
...
void Unit::RemoveVehicleKit() {
  ...
  RemoveFlag(UNIT_NPC_FLAGS,
             UNIT_NPC_FLAG_SPELLCLICK | UNIT_NPC_FLAG_PLAYER_VEHICLE);
}
...
bool Unit::IsInPartyWith(Unit const *unit) const {
  ...
  // Xinef: creature type_flag should work for party check only if player group
  // is not a raid
  ...
  if (group) {
    ...
    for (GroupReference *itr = group->GetFirstMember(); itr != nullptr;
      ...
      if (Target && Target->IsInMap(owner) &&
        ...
        for (Unit::ControlSet::iterator itr = Target->m_Controlled.begin();
             itr != Target->m_Controlled.end(); ++itr) {
          if (Unit *pet = *itr)
...
Aura *Unit::AddAura(uint32 spellId, Unit *target) {
  ...
  if (!target->IsAlive() && !spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE) &&
      !spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_DEAD))
    return nullptr;
...
void Unit::ApplyResilience(Unit const *victim, float *crit, int32 *damage,
                           bool isCrit, CombatRating type) {
...
// some time)
float Unit::MeleeSpellMissChance(const Unit *victim, WeaponAttackType attType,
...
void Unit::SetPhaseMask(uint32 newPhaseMask, bool update) {
  ...
  if (IsInWorld()) {
    ...
    // pussywizard: goign to other phase (valithria, algalon) should not remove
    // such auras
    // RemoveNotOwnSingleTargetAuras(newPhaseMask, true);            // we can
    // lost access to caster or target

    // modify hostile references for new phasemask, some special cases deal with
    // hostile references themselves
    if (GetTypeId() == TYPEID_UNIT ||
      ...
      while (ref) {
        if (Unit *unit = ref->GetSource()->GetOwner())
          if (Creature *creature = unit->ToCreature())
            refManager.setOnlineOfflineState(
      ...
      if (GetTypeId() != TYPEID_PLAYER) {
        ...
        ThreatContainer::StorageType offlineThreatList =
            getThreatManager().getOfflineThreatList();
        ...
        for (ThreatContainer::StorageType::const_iterator itr =
          ...
          if (Unit *unit = (*itr)->getTarget())
            unit->getHostileRefManager().setOnlineOfflineState(
...
void Unit::UpdateObjectVisibility(bool forced, bool /*fromUpdate*/) {
  if (!forced)
    ...
    // pussywizard: generally this is not needed here, delayed notifier will
    // handle this, call only for pets
...
uint32 Unit::GetModelForForm(ShapeshiftForm form) const {
  ...
  if (formEntry && formEntry->modelID_A) {
    ...
    if (GetTypeId() != TYPEID_PLAYER)
      ...
      // If the player is horde but there are no values for the horde modelid -
      // take the alliance modelid
  ...
  for (SpellClickInfoContainer::const_iterator itr = clickPair.first;
    ...
    if (seatId > -1) {
      ...
      while (i < MAX_SPELL_EFFECTS) {
        if (spellEntry->Effects[i].ApplyAuraName ==
      ...
      if (!valid) {
        LOG_ERROR("sql.sql",
                  "Spell %u specified in npc_spellclick_spells is not a valid "
                  "vehicle enter aura!",
                  itr->second.spellId);
        continue;
      }
      ...
      if (IsInMap(caster))
        caster->CastCustomSpell(
            itr->second.spellId, SpellValueMod(SPELLVALUE_BASE_POINT0 + i),
            seatId + 1, target,
            GetVehicleKit() ? TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE
                            : TRIGGERED_NONE,
            nullptr, nullptr, origCasterGUID);
      else // This can happen during Player::_LoadAuras
      {
        ...
        bp0[i] = seatId;
        Aura::TryRefreshStackOrCreate(spellEntry, MAX_EFFECT_MASK, this,
                                      clicker, bp0, nullptr, origCasterGUID);
      }
    } else {
      if (IsInMap(caster))
        caster->CastSpell(target, spellEntry,
                          GetVehicleKit()
...
void Unit::EnterVehicle(Unit *base, int8 seatId) {
  CastCustomSpell(VEHICLE_SPELL_RIDE_HARDCODED, SPELLVALUE_BASE_POINT0,
                  seatId + 1, base,
                  TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE);
}
...
void Unit::_EnterVehicle(Vehicle *vehicle, int8 seatId,
                         AuraApplication const *aurApp) {
    ...
    if (m_vehicle == vehicle) {
      if (seatId >= 0 && seatId != GetTransSeat()) {
        ...
        LOG_DEBUG("vehicles",
                  ...
                  GetEntry(), m_vehicle->GetBase()->GetEntry(), GetTransSeat(),
                  seatId);
#endif
        ChangeSeat(seatId);
      }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("vehicles", "EnterVehicle: %u exit %u and enter %u.",
...
void Unit::ExitVehicle(Position const * /*exitPosition*/) {
  //! This function can be called at upper level code to initialize an exit from
  ...
  //! without specifying an exitposition. The subsequent call below would return
  //! on if (!m_vehicle).
...
void Unit::_ExitVehicle(Position const *exitPosition) {
  ...
  // If player is on mouted duel and exits the mount should immediatly lose the
  // duel
  ...
  SetControlled(false,
                UNIT_STATE_ROOT); // SMSG_MOVE_FORCE_UNROOT, ~MOVEMENTFLAG_ROOT

  Position pos;
  if (!exitPosition)                // Exit position not specified
    vehicleBase->GetPosition(&pos); // This should use passenger's current
  ...
  else if (vehicle->GetVehicleInfo()->m_ID ==
           349) // AT Mounts, dismount to the right
  {
    float x =
        pos.GetPositionX() + 2.0f * cos(pos.GetOrientation() - M_PI / 2.0f);
    float y =
        pos.GetPositionY() + 2.0f * sin(pos.GetOrientation() - M_PI / 2.0f);
  ...
  if (vehicle->GetVehicleInfo()->m_ID != 341) {
    ...
    init.MoveTo(pos.GetPositionX() + 8 * cos(o),
                pos.GetPositionY() + 8 * sin(o), pos.GetPositionZ() + 16.0f);
    init.SetFacing(GetOrientation());
    init.SetTransportExit();
    init.Launch();
    DisableSpline();
  ...
  if (GetTypeId() == TYPEID_UNIT && !CanFly() &&
      (vehicle->GetVehicleInfo()->m_ID == 113 ||
       vehicle->GetVehicleInfo()->m_ID == 8 ||
       vehicle->GetVehicleInfo()->m_ID == 290 ||
       vehicle->GetVehicleInfo()->m_ID == 298))
    GetMotionMaster()->MoveFall();
  // GetMotionMaster()->MoveFall();            // Enable this once passenger
  // positions are calculater properly (see above)
  ...
  if (HasUnitTypeMask(UNIT_MASK_ACCESSORY)) {
    ...
    // If for other reason we as minion are exiting the vehicle (ejected, master
    // dismounted) - unsummon
    else {
      SetVisible(false);
...
void Unit::SendTeleportPacket(Position &pos) {
  Position oldPos = {GetPositionX(), GetPositionY(), GetPositionZ(),
                     GetOrientation()};
  if (GetTypeId() == TYPEID_UNIT)
    Relocate(&pos);
...
void Unit::RewardRage(uint32 damage, uint32 weaponSpeedHitFactor,
  ...
  float rageconversion =
      ((0.0091107836f * getLevel() * getLevel()) + 3.225598133f * getLevel()) +
      4.2652911f;
...
void Unit::OutDebugInfo() const {
  ...
  LOG_INFO("server", "GUID " UI64FMTD ", entry %u, type %u, name %s", GetGUID(),
           GetEntry(), (uint32)GetTypeId(), GetName().c_str());
  LOG_INFO("server",
           "OwnerGUID " UI64FMTD ", MinionGUID " UI64FMTD
           ", CharmerGUID " UI64FMTD ", CharmedGUID " UI64FMTD,
           GetOwnerGUID(), GetMinionGUID(), GetCharmerGUID(), GetCharmGUID());
  LOG_INFO("server", "In world %u, unit type mask %u",
  ...
  if (IsVehicle()) {
    ...
    for (SeatMap::iterator itr = GetVehicleKit()->Seats.begin();
         itr != GetVehicleKit()->Seats.end(); ++itr)
      if (Unit *passenger = ObjectAccessor::GetUnit(*GetVehicleBase(),
                                                    itr->second.Passenger.Guid))
...
public:
  AuraMunchingQueue(Unit &owner, uint64 targetGUID, int32 basePoints,
                    uint32 spellId)
  ...
  bool Execute(uint64 /*eventTime*/, uint32 /*updateTime*/) override {
    if (_owner.IsInWorld() && _owner.FindMap())
      if (Unit *target = ObjectAccessor::GetUnit(_owner, _targetGUID))
        _owner.CastCustomSpell(_spellId, SPELLVALUE_BASE_POINT0, _basePoints,
                               target,
                               TriggerCastFlags(TRIGGERED_FULL_MASK &
                                                ~TRIGGERED_NO_PERIODIC_RESET),
                               nullptr, nullptr, _owner.GetGUID());
...
void Unit::CastDelayedSpellWithPeriodicAmount(Unit *caster, uint32 spellId,
                                              AuraType auraType,
                                              int32 addAmount,
                                              uint8 effectIndex) {
  for (AuraEffectList::iterator i = m_modAuras[auraType].begin();
       i != m_modAuras[auraType].end(); ++i) {
    AuraEffect *aurEff = *i;
    if (aurEff->GetCasterGUID() != caster->GetGUID() ||
        aurEff->GetId() != spellId || aurEff->GetEffIndex() != effectIndex ||
    ...
    addAmount +=
        ((aurEff->GetOldAmount() *
          std::max<int32>(
              aurEff->GetTotalTicks() - int32(aurEff->GetTickNumber()), 0)) /
         aurEff->GetTotalTicks());
    break;
  }
  ...
  if (this == caster)
    caster->CastCustomSpell(
        spellId, SPELLVALUE_BASE_POINT0, addAmount, this,
...
void CharmInfo::SaveStayPosition(bool atCurrentPos) {
  ...
  if (atCurrentPos) {
    ...
    _unit->UpdateAllowedPositionZ(_unit->GetPositionX(), _unit->GetPositionY(),
                                  z);
    stayPos = G3D::Vector3(_unit->GetPositionX(), _unit->GetPositionY(),
...
void Unit::PetSpellFail(const SpellInfo *spellInfo, Unit *target,
                        uint32 result) {
  CharmInfo *charmInfo = GetCharmInfo();
  if (!charmInfo || GetTypeId() != TYPEID_UNIT)
    return;

  if ((MMAP::MMapFactory::IsPathfindingEnabled(GetMap()) ||
    ...
    if ((result == SPELL_FAILED_LINE_OF_SIGHT ||
      ...
      if (Unit *owner = GetOwner()) {
        if (spellInfo->IsPositive() && IsFriendlyTo(target)) {
          ...
          charmInfo->SetIsCommandAttack(
              !ToCreature()->HasReactState(REACT_PASSIVE));
          charmInfo->SetIsReturning(false);
          charmInfo->SetIsFollowing(false);

          GetMotionMaster()->MoveFollow(target, PET_FOLLOW_DIST,
                                        rand_norm() * 2 * M_PI);
    ...
    if (result == SPELL_FAILED_LINE_OF_SIGHT ||
        result == SPELL_FAILED_OUT_OF_RANGE) {
      charmInfo->SetForcedSpell(spellInfo->IsPositive() ? -int32(spellInfo->Id)
                                                        : spellInfo->Id);
...
int32 Unit::CalculateAOEDamageReduction(int32 damage, uint32 schoolMask,
                                        Unit *caster) const {
...
bool ConflagrateAuraStateDelayEvent::Execute(uint64 /*e_time*/,
  ...
  if (Unit *owner = ObjectAccessor::FindUnit(m_owner))
    if (owner && m_caster && owner->IsInWorld())
      if (owner->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_WARLOCK,
                               0x4, 0, 0, m_caster) || // immolate
          owner->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_WARLOCK,
                               0, 0, 0x2, m_caster)) // shadowflame
...
uint32 Unit::GetZoneId(bool forceRecalc) const {
  ...
  if (!forceRecalc && GetExactDistSq(&m_last_zone_position) < 4.0f * 4.0f)
    ...
    const_cast<Position *>(&m_last_zone_position)
...
uint32 Unit::GetAreaId(bool forceRecalc) const {
  ...
  if (!forceRecalc && GetExactDistSq(&m_last_area_position) < 4.0f * 4.0f)
    ...
    const_cast<Position *>(&m_last_area_position)
        ->Relocate(GetPositionX(), GetPositionY(), GetPositionZ());
...
void Unit::GetZoneAndAreaId(uint32 &zoneid, uint32 &areaid,
                            bool forceRecalc) const {
  // xinef: optimization, zone and area calculated every few yards
  if (!forceRecalc && GetExactDistSq(&m_last_area_position) < 4.0f * 4.0f &&
      GetExactDistSq(&m_last_zone_position) < 4.0f * 4.0f) {
...
bool Unit::IsOutdoors() const {
  ...
  if (GetExactDistSq(&m_last_outdoors_position) < 4.0f * 4.0f)
    ...
    const_cast<Position *>(&m_last_outdoors_position)
        ->Relocate(GetPositionX(), GetPositionY(), GetPositionZ());
    *(const_cast<bool *>(&m_last_outdoors_status)) =
        GetMap()->IsOutdoors(GetPositionX(), GetPositionY(), GetPositionZ());
    return m_last_outdoors_status;
  }
}
...
void Unit::ExecuteDelayedUnitRelocationEvent() {
  ...
  if (this->HasSharedVision())
    for (SharedVisionList::const_iterator itr =
         ...
         itr != this->GetSharedVisionList().end(); ++itr)
      if (Player *player = (*itr)) {
        if (player->IsOnVehicle(this) || !player->IsInWorld() ||
            player->IsDuringRemoveFromWorld()) // players on vehicles have their
                                               // own event executed (due to
        ...
        if (Unit *active = viewPoint->ToUnit()) {
          // if (active->IsVehicle()) // always check original unit here, last
          // notify position is not relocated
          //  active = player;
        ...
        Warhead::PlayerRelocationNotifier relocateNoLarge(
            *player,
            false); // visit only objects which are not large; default distance
        viewPoint->VisitNearbyObject(player->GetSightRange() +
                                         VISIBILITY_INC_FOR_GOBJECTS,
                                     relocateNoLarge);
        relocateNoLarge.SendToSelf();
        Warhead::PlayerRelocationNotifier relocateLarge(
  ...
  if (Player *player = this->ToPlayer()) {
    ...
    Warhead::PlayerRelocationNotifier relocateNoLarge(
        *player,
        false); // visit only objects which are not large; default distance
    viewPoint->VisitNearbyObject(
        player->GetSightRange() + VISIBILITY_INC_FOR_GOBJECTS, relocateNoLarge);
    relocateNoLarge.SendToSelf();
    Warhead::PlayerRelocationNotifier relocateLarge(
        *player, true); // visit only large objects; maximum distance
    viewPoint->VisitNearbyObject(MAX_VISIBILITY_DISTANCE, relocateLarge);
    ...
    unit->VisitNearbyObject(
        unit->GetVisibilityRange() + VISIBILITY_COMPENSATION, relocate);
...
void Unit::SetFacingTo(float ori) {
  ...
  if (HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && GetTransGUID())
    init.DisableTransportPathTransformations(); // It makes no sense to target
                                                // global orientation
  init.SetFacing(ori);
  init.Launch();
}
...
void Unit::SetFacingToObject(WorldObject *object) {
  ...
  init.MoveTo(GetPositionX(), GetPositionY(), GetPositionZ());
  init.SetFacing(
      GetAngle(object)); // when on transport, GetAngle will still return global
...
bool Unit::SetHover(bool enable, bool /*packetOnly = false*/) {
  ...
  if (enable) {
    ...
    SendMovementFlagUpdate(); // pussywizard: needed for falling after death
                              // (instead of falling onto air at hover height)
  }
...
void Unit::BuildValuesUpdate(uint8 updateType, ByteBuffer *data,
                             Player *target) const {
  if (!target)
    return;
  ...
  for (uint16 index = 0; index < m_valuesCount; ++index) {
    if (_fieldNotifyFlags & flags[index] ||
      ...
      if (index == UNIT_NPC_FLAGS) {
        ...
        // Check per caster aura states to not enable using a spell in client if
        // specified aura is not by target
        fieldBuffer << BuildAuraStateUpdateForTarget(target);
      }
      // FIXME: Some values at server stored in float format but must be sent to
      // client in uint32 format
      else if (index >= UNIT_FIELD_BASEATTACKTIME &&
               index <= UNIT_FIELD_RANGEDATTACKTIME) {
        // convert from float to uint32 and send
        fieldBuffer << uint32(m_floatValues[index] < 0 ? 0
                                                       : m_floatValues[index]);
      }
      // there are some float values which may be negative or can't get negative
      // due to other checks
      ...
      // selectable flag
        ...
        if (target->IsGameMaster() &&
            AccountMgr::IsGMAccount(target->GetSession()->GetSecurity()))
        ...
        if (creature) {
          ...
          if (cinfo->flags_extra & CREATURE_FLAG_EXTRA_TRIGGER) {
            if (target->IsGameMaster() &&
              ...
              if (cinfo->Modelid1)
                displayId =
                    cinfo->Modelid1; // Modelid1 is a visible model for gms
              else
                displayId = 17519; // world visible trigger's model
            } else {
              if (cinfo->Modelid2)
                displayId = cinfo->Modelid2; // Modelid2 is an invisible model
                                             // for players
              else
...
void Unit::BuildCooldownPacket(WorldPacket &data, uint8 flags, uint32 spellId,
                               uint32 cooldown) {
...
void Unit::BuildCooldownPacket(WorldPacket &data, uint8 flags,
...
bool Unit::IsInCombatWith(Unit const *who) const {
  ...
  for (ThreatContainer::StorageType::const_iterator i =
           m_ThreatManager.getThreatList().begin();
       i != m_ThreatManager.getThreatList().end(); ++i) {
...
float Unit::GetCollisionHeight() const {
  ...
  if (IsMounted()) {
    ...
    collisionHeight = scaleMod * modelData->CollisionHeight * modelData->Scale *
                      displayInfo->scale;
