...
void PlayerSocial::SendSocialList(Player *player, uint32 flags) {
  ...
  for (auto &itr : m_playerSocialMap) {
    ...
    data << uint32(
        contactFlags); // player flag (0x1 = Friend, 0x2 = Ignored, 0x4 = Muted)
...
void SocialMgr::GetFriendInfo(Player *player, uint64 friendGUID,
...
void SocialMgr::MakeFriendStatusPacket(FriendsResult result, uint64 guid,
                                       WorldPacket *data) {
...
void SocialMgr::SendFriendStatus(Player *player, FriendsResult result,
                                 uint64 friendGuid, bool broadcast) {
  FriendInfo fi;
