...
struct PvPInfo {
  ...
  bool IsInFFAPvPArea{false}; ///> Marks if player is in an FFAPvP area (such as
                              ///Gurubashi Arena)
...
enum PlayerFlags {
  ...
  PLAYER_FLAGS_UNK16 = 0x00010000, // pre-3.0.3 PLAYER_FLAGS_SANCTUARY flag for
                                   // player entered sanctuary
  ...
  PLAYER_ALLOW_ONLY_ABILITY =
      0x00800000, // used by bladestorm and killing spree, allowed only spells
                  // with SPELL_ATTR0_REQ_AMMO, SPELL_EFFECT_ATTACK, checked
...
enum TransferAbortReason {
  ...
  TRANSFER_ABORT_INSUF_EXPAN_LVL = 0x07, // You must have <TBC, WotLK> expansion
  ...
  TRANSFER_ABORT_TOO_MANY_REALM_INSTANCES =
      0x0A, // Additional instances cannot be launched, please try again later.
...
public:
  ...
  [[nodiscard]] uint32 GetTaxiSource() const {
    return m_TaxiDestinations.size() <= _taxiSegment + 1
               ? 0
               : m_TaxiDestinations[_taxiSegment];
  }
  [[nodiscard]] uint32 GetTaxiDestination() const {
    return m_TaxiDestinations.size() <= _taxiSegment + 1
               ? 0
               : m_TaxiDestinations[_taxiSegment + 1];
  }
  ...
  [[nodiscard]] std::vector<uint32> const &GetPath() const {
...
public: // constructors
  TradeData(Player *player, Player *trader)
      : m_player(player), m_trader(trader), m_accepted(false),
        m_acceptProccess(false), m_money(0), m_spell(0), m_spellCastItem(0) {
...
private:            // fields
  ...
  uint64 m_items[TRADE_SLOT_COUNT]; // traded itmes from m_player side including
                                    // non-traded slot
};
...
public:
  ...
  bool TeleportTo(uint32 mapid, float x, float y, float z, float orientation,
                  uint32 options = 0, Unit *target = nullptr);
  ...
  [[nodiscard]] bool IsSummonAsSpectator() const {
    return m_summon_asSpectator && m_summon_expire >= time(nullptr);
  }
  ...
  GetGameObjectIfCanInteractWith(uint64 guid, GameobjectTypes type) const;
  ...
  bool ActivateTaxiPathTo(std::vector<uint32> const &nodes,
                          Creature *npc = nullptr, uint32 spellid = 1);
  ...
  [[nodiscard]] bool IsDeveloper() const {
    return HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER);
  }
  ...
  setDeathState(DeathState s,
                bool despawn = false) override; // overwrite Unit::setDeathState
  ...
  bool IsPetDismissed();
  void SummonPet(uint32 entry, float x, float y, float z, float ang,
                 PetType petType, uint32 despwtime, uint32 createdBySpell,
  ...
  uint32 GetItemCount(uint32 item, bool inBankAlso = false,
  ...
  [[nodiscard]] inline Item *GetUseableItemByPos(
      uint8 bag, uint8 slot) const // Does additional check for disarmed weapons
  {
  ...
  [[nodiscard]] bool HasItemCount(uint32 item, uint32 count = 1,
                                  bool inBankAlso = false) const;
  bool HasItemFitToSpellRequirements(SpellInfo const *spellInfo,
                                     Item const *ignoreItem = nullptr) const;
  bool CanNoReagentCast(SpellInfo const *spellInfo) const;
  [[nodiscard]] bool
  HasItemOrGemWithIdEquipped(uint32 item, uint32 count,
                             uint8 except_slot = NULL_SLOT) const;
  [[nodiscard]] bool
  HasItemOrGemWithLimitCategoryEquipped(uint32 limitCategory, uint32 count,
                                        uint8 except_slot = NULL_SLOT) const;
  ...
  InventoryResult CanStoreNewItem(uint8 bag, uint8 slot, ItemPosCountVec &dest,
  ...
  InventoryResult CanEquipNewItem(uint8 slot, uint16 &dest, uint32 item,
                                  bool swap) const;
  InventoryResult CanEquipItem(uint8 slot, uint16 &dest, Item *pItem, bool swap,
                               bool not_loading = true) const;

  InventoryResult CanEquipUniqueItem(Item *pItem, uint8 except_slot = NULL_SLOT,
  ...
  InventoryResult CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec &dest,
                              Item *pItem, bool swap,
                              bool not_loading = true) const;
  InventoryResult CanUseItem(Item *pItem, bool not_loading = true) const;
  [[nodiscard]] bool HasItemTotemCategory(uint32 TotemCategory) const;
  bool IsTotemCategoryCompatiableWith(const ItemTemplate *pProto,
  ...
  Item *StoreNewItem(ItemPosCountVec const &pos, uint32 item, bool update,
                     int32 randomPropertyId, AllowedLooterSet &allowedLooters);
  ...
  void AutoStoreLoot(uint32 loot_id, LootStore const &store,
  ...
  InventoryResult
  CanTakeMoreSimilarItems(uint32 entry, uint32 count, Item *pItem,
                          uint32 *no_space_count = nullptr) const;
  InventoryResult CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec &dest,
                               uint32 entry, uint32 count,
                               Item *pItem = nullptr, bool swap = false,
                               uint32 *no_space_count = nullptr) const;
  ...
  void SendEquipError(InventoryResult msg, Item *pItem, Item *pItem2 = nullptr,
                      uint32 itemid = 0);
  void SendBuyError(BuyResult msg, Creature *creature, uint32 item,
                    uint32 param);
  ...
  bool BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item,
  ...
  void PrepareGossipMenu(WorldObject *source, uint32 menuId = 0,
  ...
  [[nodiscard]] bool HasQuestForItem(uint32 itemId, uint32 excludeQuestId = 0,
  ...
  void SendQuestUpdateAddItem(Quest const *quest, uint32 item_idx,
                              uint16 count);
  void SendQuestUpdateAddCreatureOrGo(Quest const *quest, uint64 guid,
                                      uint32 creatureOrGO_idx, uint16 old_count,
                                      uint16 add_count);
  void SendQuestUpdateAddPlayer(Quest const *quest, uint16 old_count,
                                uint16 add_count);
  ...
  static void SetUInt32ValueInArray(Tokenizer &data, uint16 index,
                                    uint32 value);
  ...
  static void SavePositionInDB(uint32 mapid, float x, float y, float z, float o,
  ...
  void AddMail(Mail *mail) {
    totalMailCount++; // for call from WorldSession::SendMailTo
    m_mailCache.push_front(mail);
  }
  ...
  bool RemoveMItem(uint32 id) { return !!mMitems.erase(id); }
  ...
  bool addSpell(uint32 spellId, uint8 addSpecMask, bool updateActive,
  ...
  bool IsAffectedBySpellmod(SpellInfo const *spellInfo, SpellModifier *mod,
                            Spell *spell = nullptr);
  bool HasSpellMod(SpellModifier *mod, Spell *spell);
  template <class T>
  T ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue,
                  Spell *spell = nullptr, bool temporaryPet = false);
  ...
  [[nodiscard]] bool HasSpellCooldown(uint32 spell_id) const override {
    ...
    return itr != m_spellCooldowns.end() &&
           itr->second.end > World::GetGameTimeMS();
  }
  ...
  [[nodiscard]] uint32 GetSpellCooldownDelay(uint32 spell_id) const {
    SpellCooldowns::const_iterator itr = m_spellCooldowns.find(spell_id);
    return uint32(itr != m_spellCooldowns.end() &&
                          itr->second.end > World::GetGameTimeMS()
                      ? itr->second.end - World::GetGameTimeMS()
                      : 0);
  }
  void AddSpellAndCategoryCooldowns(SpellInfo const *spellInfo, uint32 itemId,
                                    Spell *spell = nullptr,
                                    bool infinityCooldown = false);
  void AddSpellCooldown(uint32 spell_id, uint32 itemid, uint32 end_time,
                        bool needSendToClient = false,
                        bool forceSendToSpectator = false) override;
  void ModifySpellCooldown(uint32 spellId, int32 cooldown);
  void SendCooldownEvent(SpellInfo const *spellInfo, uint32 itemId = 0,
                         Spell *spell = nullptr, bool setCooldown = true);
  ...
  void setResurrectRequestData(uint64 guid, uint32 mapId, float X, float Y,
                               float Z, uint32 health, uint32 mana) {
  ...
  [[nodiscard]] bool isResurrectRequestedBy(uint64 guid) const {
    return m_resurrectGUID && m_resurrectGUID == guid;
  }
  [[nodiscard]] bool isResurrectRequested() const {
  ...
  void GetZoneAndAreaId(uint32 &zoneid, uint32 &areaid,
                        bool forceRecalc = false) const override;
  ...
  [[nodiscard]] Difficulty GetStoredRaidDifficulty() const {
    return m_raidMapDifficulty; // only for use in difficulty packet after
                                // exiting to raid map
  }
  ...
  void CalculateMinMaxDamage(WeaponAttackType attType, bool normalized,
                             bool addTotalPct, float &minDamage,
                             float &maxDamage) override;
  ...
  void BuildCreateUpdateBlockForPlayer(UpdateData *data,
                                       Player *target) const override;
  void DestroyForPlayer(Player *target, bool onDeath = false) const override;
  void SendLogXPGain(uint32 GivenXP, Unit *victim, uint32 BonusXP,
                     bool recruitAFriend = false, float group_rate = 1.0f);
  ...
  void SendMessageToSetInRange(
      WorldPacket *data, float dist, bool self, bool includeMargin = false,
      Player const *skipped_rcvr = nullptr) override; // pussywizard!
  void SendMessageToSetInRange_OwnTeam(
      WorldPacket *data, float dist,
      bool self); // pussywizard! param includeMargin not needed here
  void SendMessageToSet(WorldPacket *data,
                        Player const *skipped_rcvr) override {
    SendMessageToSetInRange(data, GetVisibilityRange(), skipped_rcvr != this,
                            true, skipped_rcvr); // pussywizard!
  }
  ...
  int32 CalculateReputationGain(ReputationSource source,
                                ...
                                int32 faction, bool noQuestBonus = false);
   ...
   * PlayerScript:OnAchiComplete): Right after calling this function, character
   * can reward talent points by calling function player->InitTalentForLevel().
   *
  ...
  void ModifyHonorPoints(
      int32 value,
      SQLTransaction *trans = nullptr); //! If trans is specified, honor save
                                        //! query will be added to trans
  void ModifyArenaPoints(
      int32 value,
      SQLTransaction *trans = nullptr); //! If trans is specified, arena point
                                        //! save query will be added to trans
  [[nodiscard]] uint32
  ...
  void SetBaseModValue(BaseModGroup modGroup, BaseModType modType,
                       float value) {
  ...
  void _ApplyWeaponDependentAuraMods(Item *item, WeaponAttackType attackType,
                                     bool apply);
  void _ApplyWeaponDependentAuraCritMod(Item *item, WeaponAttackType attackType,
                                        AuraEffect const *aura, bool apply);
  ...
  GetBattlegroundQueueTypeId(uint32 index) const {
    return m_bgBattlegroundQueueID[index];
  }
  ...
  void SetBattlegroundId(uint32 id, BattlegroundTypeId bgTypeId,
  ...
  [[nodiscard]] TeamId GetBgTeamId() const {
    return m_bgData.bgTeamId != TEAM_NEUTRAL ? m_bgData.bgTeamId : GetTeamId();
  }
  ...
  [[nodiscard]] bool HaveAtClient(uint64 guid) const {
    return guid == GetGUID() || m_clientGUIDs.find(guid) != m_clientGUIDs.end();
  }
  ...
  template <class T>
  void UpdateVisibilityOf(T *target, UpdateData &data,
  ...
  [[nodiscard]] bool IsPetNeedBeTemporaryUnsummoned() const {
    return GetSession()->PlayerLogout() || !IsInWorld() || !IsAlive() ||
           IsMounted() /*+in flight*/ || GetVehicle() || IsBeingTeleported();
  }
  ...
  bool Satisfy(AccessRequirement const *ar, uint32 target_map,
               bool report = false);
  ...
  bool HasReceivedSpectatorResetFor(uint32 guid) {
  ...
  void AddReceivedSpectatorResetFor(uint32 guid) {
    m_receivedSpectatorResetFor.insert(guid);
  }
  void RemoveReceivedSpectatorResetFor(uint32 guid) {
  ...
  [[nodiscard]] uint32 GetActiveCinematicCamera() const {
  ...
  [[nodiscard]] bool IsOnCinematic() const {
    return (m_cinematicCamera != nullptr);
  }
  void BeginCinematic();
  void EndCinematic();
...
protected:
  ...
  void _LoadMailInit(PreparedQueryResult resultMailCount,
...
private:
  ...
  InventoryResult CanStoreItem_InBag(uint8 bag, ItemPosCountVec &dest,
                                     ItemTemplate const *pProto, uint32 &count,
                                     bool merge, bool non_specialized,
                                     Item *pSrcItem, uint8 skip_bag,
  ...
  Item *_StoreItem(uint16 pos, Item *pItem, uint32 count, bool clone,
                   bool update);
  Item *_LoadItem(SQLTransaction &trans, uint32 zoneId, uint32 timeDiff,
                  Field *fields);
  ...
  [[nodiscard]] bool MustDelayTeleport() const {
    return m_bMustDelayTeleport; // pussywizard: must delay teleports during
                                 // player update to the very end
  }
...
template <class T>
T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue,
                        Spell *spell, bool temporaryPet) {
  ...
  for (auto mod : m_spellMods[op]) {
    ...
    // xinef: temporary pets cannot use charged mods of owner, needed for mirror
    // image QQ they should use their own auras
    ...
    if (mod->type == SPELLMOD_FLAT) {
      ...
      // skip percent mods for null basevalue (most important for spell mods
      // with charges)
      ...
      if (mod->op == SPELLMOD_CASTING_TIME && basevalue >= T(10000) &&
          mod->value <= -100)
        continue;
      // xinef: special exception for surge of light, dont affect crit chance if
      // previous mods were not applied
  ...
  if (op == SPELLMOD_CASTING_TIME || op == SPELLMOD_DURATION)
    diff =
        ((float)basevalue + totalflat) * (totalmul - 1.0f) + (float)totalflat;
  else
