...
bool PlayerTaxi::LoadTaxiDestinationsFromString(const std::string &values,
  ...
  for (size_t i = 0; i < m_TaxiDestinations.size() - 1; ++i) {
    uint32 cost;
    uint32 path;
    sObjectMgr->GetTaxiPath(m_TaxiDestinations[i], m_TaxiDestinations[i + 1],
                            path, cost);
    if (!path)
      return false;
  }
...
uint32 PlayerTaxi::GetCurrentTaxiPath() const {
  if (m_TaxiDestinations.size() < 2 ||
      m_TaxiDestinations.size() <= _taxiSegment + 1)
    return 0;

  uint32 path;
  uint32 cost;

  sObjectMgr->GetTaxiPath(m_TaxiDestinations[_taxiSegment],
                          m_TaxiDestinations[_taxiSegment + 1], path, cost);
...
// distance; 2.2. _sumLevel - sum of levels of alive group members within reward
// distance; 2.3. _maxLevel - maximum level of alive group member within reward
// distance; 2.4. _maxNotGrayMember - maximum level of alive group member within
// reward distance,
//      for whom victim is not gray;
// 2.5. _isFullXP - flag identifying that for all group members victim is not
// gray,
...
KillRewarder::KillRewarder(Player *killer, Unit *victim, bool isBattleGround)
    : // 1. Initialize internal variables to default values.
      ...
      _groupRate(1.0f), _maxNotGrayMember(nullptr), _count(0), _sumLevel(0),
      _xp(0), _isFullXP(false), _maxLevel(0), _isBattleGround(isBattleGround),
      _isPvP(false) {
...
inline void KillRewarder::_InitGroupData() {
  if (_group) {
    // 2. In case when player is in group, initialize variables necessary for
    // group calculations:
    for (GroupReference *itr = _group->GetFirstMember(); itr != nullptr;
      ...
      if (Player *member = itr->GetSource())
        if ((_killer == member || member->IsAtGroupRewardDistance(_victim)) &&
          ...
          ++_count;
          // 2.2. _sumLevel - sum of levels of alive group members within reward
          // distance;
          _sumLevel += lvl;
          // 2.3. _maxLevel - maximum level of alive group member within reward
          // distance;
          if (_maxLevel < lvl)
            _maxLevel = lvl;
          // 2.4. _maxNotGrayMember - maximum level of alive group member within
          // reward distance,
          //      for whom victim is not gray;
    ...
    // 2.5. _isFullXP - flag identifying that for all group members victim is
    // not gray,
    //      so 100% XP will be rewarded (50% otherwise).
    _isFullXP =
...
inline void KillRewarder::_RewardXP(Player *player, float rate) {
  ...
  if (_group) {
    ...
    if (_maxNotGrayMember && player->IsAlive() &&
      ...
      xp = _isFullXP ? uint32(xp * rate)
                     : // Reward FULL XP if all group members are not gray.
               uint32(xp * rate / 2) + 1; // Reward only HALF of XP if some of
                                          // group members are gray.
    else
      xp = 0;
  }
  if (xp) {
    ...
    if (Pet *pet = player->GetPet())
      // 4.2.4. If player has pet, reward pet with XP (100% for single player,
      // 50% for group case).
...
inline void KillRewarder::_RewardKillCredit(Player *player) {
  // 4.4. Give kill credit (player must not be in group, or he must be alive or
  // without corpse).
  if (!_group || player->IsAlive() || !player->GetCorpse())
    if (Creature *target = _victim->ToCreature()) {
      ...
      player->UpdateAchievementCriteria(
          ...
          target->GetCreatureType(), 1, target);
    }
}
...
void KillRewarder::_RewardPlayer(Player *player, bool isDungeon) {
  ...
  if (!_isPvP || _isBattleGround) {
    const float rate = _group
                           ? _groupRate * float(player->getLevel()) / _sumLevel
    ...
    if (!_isBattleGround) {
      // If killer is in dungeon then all members receive full reputation at
      // kill.
...
void KillRewarder::_RewardGroup() {
  if (_maxLevel) {
    if (_maxNotGrayMember)
      // 3.1.1. Initialize initial XP amount based on maximum level of group
      // member,
      //        for whom victim is not gray.
    ...
    if (!_isBattleGround || _xp) {
      const bool isDungeon =
          !_isPvP && sMapStore.LookupEntry(_killer->GetMapId())->IsDungeon();
      if (!_isBattleGround) {
        // 3.1.2. Alter group rate if group is in raid (not for battlegrounds).
        const bool isRaid =
            !_isPvP && sMapStore.LookupEntry(_killer->GetMapId())->IsRaid() &&
            _group->isRaidGroup();
        _groupRate = Warhead::XP::xp_in_group_rate(_count, isRaid);
      }

      // 3.1.3. Reward each group member (even dead or corpse) within reward
      // distance.
      for (GroupReference *itr = _group->GetFirstMember(); itr != nullptr;
           itr = itr->next()) {
        if (Player *member = itr->GetSource()) {
...
void KillRewarder::Reward() {
  ...
  if (_group)
    ...
    if (!_isBattleGround || _xp)
      // 3.2.2. Reward killer.
      if (_killer->IsInMap(_victim)) // pussywizard: killer may be on other map
                                     // (crashfix), when killing in a group same
                                     // map is required, so its not a problem
...
void Player::Update(uint32 p_time) {
                          ...
                          // GetZoneAndAreaId
  {
    LOG_INFO("misc",
             ...
             (FindMap() ? FindMap()->GetId() : 0), GetMapId(), GetGUIDLow());
...
void Player::setDeathState(DeathState s, bool /*despawn = false*/) {
  ...
  if (s == JUST_DIED) {
    if (!cur) {
      LOG_ERROR("server", "setDeathState: attempt to kill a dead player %s(%d)",
...
bool Player::BuildEnumData(PreparedQueryResult result, WorldPacket *data) {
  ...
  if (!info) {
    ...
    LOG_ERROR("server",
              "Player (%u) has incorrect gender (%u), don't build enum.", guid,
              gender);
    return false;
  }
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z,
                        float orientation, uint32 options /*= 0*/,
                        Unit *target /*= nullptr*/) {
  if (!MapManager::IsValidMapCoord(mapid, x, y, z, orientation)) {
    LOG_ERROR("server",
              "TeleportTo: invalid map (%d) or invalid coordinates (X: %f, Y: "
              "%f, Z: %f, O: %f) given when teleporting player (GUID: %u, "
              "name: %s, map: %d, X: %f, Y: %f, Z: %f, O: %f).",
              mapid, x, y, z, orientation, GetGUIDLow(), GetName().c_str(),
  ...
  if (AccountMgr::IsPlayerAccount(GetSession()->GetSecurity()) &&
    ...
    LOG_ERROR("server",
              ...
              GetGUIDLow(), GetName().c_str(), mapid);
...
void Player::RemoveFromWorld() {
  ...
  if (m_uint32Values) {
    if (WorldObject *viewpoint = GetViewpoint()) {
      LOG_FATAL(
          "server", "Player %s has viewpoint %u %u when removed from world",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("maps",
                "IsGameObjectOfTypeInRange: GameObject '%s' [GUID: %u] is too "
                "far away from player %s [GUID: %u] to be used by him "
                "(distance=%f, maximal 10 is allowed)",
                go->GetGOInfo()->name.c_str(), go->GetGUIDLow(),
                GetName().c_str(), GetGUIDLow(), go->GetDistance(this));
#endif
    }
  }
  return nullptr;
...
void Player::RemoveFromGroup(
    ...
    uint64 kicker /* = 0 */, const char *reason /* = nullptr */) {
  if (group) {
...
void Player::SendMailResult(uint32 mailId, MailResponseType mailAction,
                            ...
                            uint32 item_guid, uint32 item_count) {
...
void Player::UpdateNextMailTimeAndUnreads() {
  ...
  PreparedStatement *stmtNextDeliveryTime =
  ...
  PreparedStatement *stmtUnreadAmount = CharacterDatabase.GetPreparedStatement(
...
bool Player::addSpell(uint32 spellId, uint8 addSpecMask, bool updateActive,
                      bool temporary) {
...
bool Player::_addSpell(uint32 spellId, uint8 addSpecMask, bool temporary) {
  ...
  if (IsInWorld() && !isBeingLoaded() && temporary &&
      (!spellInfo->HasAttribute(
           SpellAttr0(SPELL_ATTR0_PASSIVE | SPELL_ATTR0_HIDDEN_CLIENTSIDE)) ||
  ...
  if (temporary && spellInfo->HasEffect(SPELL_EFFECT_LEARN_SPELL)) {
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
      if (spellInfo->Effects[i].IsEffect()) {
        if (spellInfo->Effects[i].Effect != SPELL_EFFECT_LEARN_SPELL) {
          LOG_INFO("server",
                   "TRYING TO LEARN SPELL WITH EFFECT LEARN: %u, PLAYER: %u",
                   spellId, GetGUIDLow());
          return false;
          // ABORT();
  ...
  if (spellInfo->HasEffect(SPELL_EFFECT_LEARN_SPELL)) {
    LOG_INFO("server",
             "TRYING TO LEARN SPELL WITH EFFECT LEARN 2: %u, PLAYER: %u",
             spellId, GetGUIDLow());
  ...
  if (spellLearnSkill) {
    ...
    for (SkillLineAbilityMap::const_iterator _spell_idx = skill_bounds.first;
      ...
      if (_spell_idx->second->learnOnGetSkill ==
              ABILITY_LEARNED_ON_GET_RACE_OR_CLASS_SKILL || // pussywizard:
                                                            // learning
                                                            // autolearned spell
                                                            // from skill
                                                            // ensures having
          ...
          ((pSkill->id == SKILL_LOCKPICKING ||
           ...
           _spell_idx->second->max_value ==
               0)) // pussywizard: learning any spell from lockpicking or
                   // runeforging ensures having the skill
      {
...
void Player::learnSpell(uint32 spellId) {
  ...
  if (HasActiveSpell(spellId)) {
    LOG_ERROR("server", "Player (%u) tries to learn already active spell: %u",
              GetGUIDLow(), spellId);
    return;
  }
...
void Player::removeSpell(uint32 spellId, uint8 removeSpecMask,
                         bool onlyTemporary) {
  PlayerSpellMap::iterator itr = m_spells.find(spellId);
  if (itr == m_spells.end())
    return;
  ...
  if (spellLearnSkill) {
    ...
    for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first;
      ...
      // pussywizard: don't understand why whole skill is removed when just
      // single spell from it is removed
      if ((_spell_idx->second->learnOnGetSkill ==
               ABILITY_LEARNED_ON_GET_RACE_OR_CLASS_SKILL &&
           pSkill->categoryId !=
               SKILL_CATEGORY_CLASS) || // pussywizard: don't unlearn class
                                        // skills
          ((pSkill->id == SKILL_LOCKPICKING ||
            pSkill->id == SKILL_RUNEFORGING) &&
        ...
        // pussywizard: this is needed for weapon/armor/language skills to
        // remove them when loosing spell
...
void Player::_LoadSpellCooldowns(PreparedQueryResult result) {
  ...
  // FROM character_spell_cooldown WHERE guid = '%u'", GetGUIDLow());
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId,
  ...
  if (deleteFinally ||
      Player::GetLevelFromStorage(playerguid) < charDelete_minLvl)
    charDelete_method = CHAR_DELETE_REMOVE;

  uint32 guid = GUID_LOPART(playerguid);

  // convert corpse to bones if exist (to prevent exiting Corpse in World
  // without DB entry) bones will be deleted by corpse/bones deleting thread
  // shortly
  ...
  case CHAR_DELETE_REMOVE: {
    ...
    if (resultFriends) {
      do {
        if (Player *pFriend =
                ObjectAccessor::FindPlayerInOrOutOfWorld(MAKE_NEW_GUID(
                    (*resultFriends)[0].GetUInt32(), 0, HIGHGUID_PLAYER))) {
  ...
  default:
    LOG_ERROR("server", "Player::DeleteFromDB: Unsupported delete method: %u.",
              charDelete_method);
    return;
  }
...
void Player::BuildPlayerRepop() {
  ...
  // SMSG.MOVE_WATER_WALK there must be SMSG.STOP_MIRROR_TIMER there we must
  // send 888 opcode

  // the player cannot have a corpse already, only bones which are not returned
  // by GetCorpse
  if (GetCorpse()) {
    LOG_ERROR("server", "BuildPlayerRepop: player %s(%d) already has a corpse",
              GetName().c_str(), GetGUIDLow());
    return;
  }
  ...
  if (!corpse) {
    LOG_ERROR("server", "Error creating corpse for Player %s [%u]",
              GetName().c_str(), GetGUIDLow());
    return;
  }
...
void Player::SpawnCorpseBones() {
  if (sObjectAccessor->ConvertCorpseForPlayer(GetGUID()))
    if (!GetSession()->PlayerLogoutWithSave()) // at logout we will already
                                               // store the player
    {
...
uint32 Player::DurabilityRepair(uint16 pos, bool cost, float discountMod,
  ...
  if (cost) {
    ...
    if (LostDurability > 0) {
      ...
      if (!dcost) {
        LOG_ERROR("server", "RepairDurability: Wrong item lvl %u",
      ...
      if (!dQualitymodEntry) {
        LOG_ERROR("server", "RepairDurability: Wrong dQualityModEntry %u",
                  dQualitymodEntryId);
        return TotalCost;
      }
  ...
  for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first;
    ...
    if (_spell_idx->second->skillId) {
      ...
      return UpdateSkillPro(
          _spell_idx->second->skillId,
          SkillGainChance(
              SkillValue, _spell_idx->second->max_value,
              (_spell_idx->second->max_value + _spell_idx->second->min_value) /
                  2,
  ...
  } else if (newVal) // add
  {
    ...
    for (int i = 0; i < PLAYER_MAX_SKILLS; ++i)
      if (!GetUInt32Value(PLAYER_SKILL_INDEX(i))) {
        ...
        if (!pSkill) {
          LOG_ERROR("server", "Skill not found in SkillLineStore: skill #%u",
                    id);
          return;
        }
...
void Player::SendActionButtons(uint32 state) const {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Sending Action Buttons for '%u' spec '%u'", GetGUIDLow(),
            m_activeSpec);
#endif
  ...
  */
  ...
  GetSession()->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Action Buttons for '%u' spec '%u' Sent", GetGUIDLow(),
            m_activeSpec);
#endif
}
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Player '%u' Added Action '%u' (type %u) to Button '%u'",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Action Button '%u' Removed from Player '%u'", button,
            GetGUIDLow());
#endif
}
...
// pussywizard!
void Player::SendMessageToSetInRange_OwnTeam(WorldPacket *data, float dist,
                                             bool self) {
  if (self)
    GetSession()->SendPacket(data);
...
void Player::CheckAreaExploreAndOutdoor() {
  ...
  if (!areaEntry) {
    LOG_ERROR(
        "server",
        "Player '%s' (%u) discovered unknown area (x: %f y: %f z: %f map: %u)",
        GetName().c_str(), GetGUIDLow(), GetPositionX(), GetPositionY(),
        GetPositionZ(), GetMapId());
    return;
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "Player %u discovered a new area: %u", GetGUIDLow(),
                areaId);
#endif
    }
  }
...
TeamId Player::TeamIdForRace(uint8 race) {
  if (ChrRacesEntry const *rEntry = sChrRacesStore.LookupEntry(race)) {
    ...
    LOG_ERROR("server", "Race (%u) not found in DBC: wrong DBC files?",
              uint32(race));
...
int32 Player::CalculateReputationGain(ReputationSource source,
  ...
  if (rate != 1.0f &&
      creatureOrQuestLevel <= Warhead::XP::GetGrayLevel(getLevel()))
    percent *= rate;
  ...
  */
  ...
  if (honor_f <= 0) {
    ...
    if (uVictim->GetTypeId() == TYPEID_PLAYER) {
      ...
      honor_f = ceil(Warhead::Honor::hk_honor_at_level_f(k_level) *
...
void Player::GetZoneAndAreaId(uint32 &zoneid, uint32 &areaid,
...
void Player::_ApplyItemBonuses(ItemTemplate const *proto, uint8 slot,
  ...
  for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS; ++i) {
    ...
    if (ssd && ssv) {
      ...
      val =
          (ssv->getssdMultiplier(proto->ScalingStatValue) * ssd->Modifier[i]) /
          10000;
    } else {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server", "WORLD: cast %s Equip spellId - %i",
...
void Player::CastItemCombatSpell(Unit *target, WeaponAttackType attType,
  ...
  // if (damageInfo->procVictim & PROC_FLAG_TAKEN_ANY_DAMAGE)
  {
    for (uint8 i = 0; i < MAX_ITEM_SPELLS; ++i) {
      ...
      if (!spellInfo) {
        LOG_ERROR("server", "WORLD: unknown Item spellid %i",
                  spellData.SpellId);
        continue;
      }
  ...
  for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot) {
    ...
    for (uint8 s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; ++s) {
      ...
      if (!spellInfo) {
        LOG_ERROR("server",
                  "Player::CastItemCombatSpell(GUID: %u, name: %s, enchant: "
                  "%i): unknown spell %i is casted, ignoring...",
                  GetGUIDLow(), GetName().c_str(), pEnchant->ID,
                  pEnchant->spellid[s]);
        continue;
      }
...
void Player::CastItemUseSpell(Item *item, SpellCastTargets const &targets,
                              uint8 cast_count, uint32 glyphIndex) {
  ItemTemplate const *proto = item->GetTemplate();
  ...
  if (Player::bgZoneIdToFillWorldStates.find(zoneid) !=
    ...
    case 3483: // Hellfire Peninsula
      if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_HP)
        ...
        data << uint32(2475) << uint32(100); //: ally / horde slider grey area
    ...
    case 4298:
      ...
      if (Creature *mograine = ObjectAccessor::GetCreature(
              *this, MAKE_NEW_GUID(130956, 29173, HIGHGUID_UNIT))) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("server", "Player gain %u xp (+ %u Rested Bonus). Rested points=%f",
...
void Player::ResetPetTalents() {
  ...
  if (!charmInfo) {
    LOG_ERROR("server",
              "Object (GUID: %u TypeId: %u) is considered pet-like but doesn't "
              "have a charminfo!",
...
InventoryResult Player::CanTakeMoreSimilarItems(uint32 entry, uint32 count,
                                                Item *pItem,
...
bool Player::IsTotemCategoryCompatiableWith(
    const ItemTemplate *pProto, uint32 requiredTotemCategoryId) const {
...
InventoryResult Player::CanBankItem(uint8 bag, uint8 slot,
  ...
  if (pItemslot >= CURRENCYTOKEN_SLOT_START &&
    ...
    LOG_ERROR("server",
              "Possible hacking attempt: Player %s [guid: %u] tried to move "
              "token [guid: %u, entry: %u] out of the currency bag!",
              GetName().c_str(), GetGUIDLow(), pItem->GetGUIDLow(),
              pProto->ItemId);
...
Item *Player::StoreNewItem(ItemPosCountVec const &dest, uint32 item,
                           bool update, int32 randomPropertyId) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("entities.player.items",
            "STORAGE: StoreItem bag = %u, slot = %u, item = %u, count = %u, "
            "guid = %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("entities.player.items", "Player::UpdateItemDuration(%u, %u)", time,
            realtimeonly);
#endif
...
void Player::RewardQuest(Quest const *quest, uint32 reward, Object *questGiver,
  ...
  uint32 XP =
      rewarded ? 0
               : uint32(quest->XPValue(this) * sWorld->getRate(RATE_XP_QUEST));
...
bool Player::CanShareQuest(uint32 quest_id) const {
  ...
  if (qInfo && qInfo->HasFlag(QUEST_FLAGS_SHARABLE)) {
    ...
    if (itr != m_QuestStatus.end()) {
      ...
      // pussywizard: in pool and not currently available (wintergrasp weekly,
      // dalaran weekly) - can't share
    ...
    if ((q_status.Status == QUEST_STATUS_INCOMPLETE) ||
      ...
      if (GetGroup() && GetGroup()->isRaidGroup() &&
        ...
        if (!InBattleground()) // there are two ways.. we can make every
                               // bg-quest a raidquest, or add this code here.. i
                               // don't know if this can be exploited by other
...
void Player::SendQuestUpdateAddCreatureOrGo(Quest const *quest, uint64 guid,
...
void Player::_LoadArenaTeamInfo() {
  ...
  for (uint8 slot = 0; slot < MAX_ARENA_SLOT; ++slot)
    if (uint32 arenaTeamId =
            Player::GetArenaTeamIdFromStorage(GetGUIDLow(), slot)) {
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder) {
  ...
  if (dbAccountId != GetSession()->GetAccountId()) {
    LOG_ERROR(
        "server",
        "Player (GUID: %u) loading from wrong account (is: %u, should be: %u)",
        guid, GetSession()->GetAccountId(), dbAccountId);
    return false;
  }
...
#define RelocateToHomebind()                                                   \
  ...
  // currently we do not support transport in bg
    ...
    if (!transGO) // pussywizard: if not MotionTransport, look for
                  // StaticTransport
    {
    ...
    if (transGO)
      if (transGO->IsInWorld() &&
          transGO->FindMap()) // pussywizard: must be on map, for one world tick
                              // transport is not in map and has old GetMapId(),
                              // player would be added to old map and to the
  ...
  if (mapEntry) {
    ...
    if (instanceId) {
      InstanceSave *save = sInstanceSaveMgr->PlayerGetInstanceSave(
          GetGUIDLow(), mapId, GetDifficulty(mapEntry->IsRaid()));
  ...
  if (HasAtLoginFlag(AT_LOGIN_RENAME)) {
    LOG_ERROR(
        "server",
        "Player (GUID: %u) tried to login while forced to rename, can't load.'",
        GetGUIDLow());
    return false;
  }
...
void Player::_LoadAuras(PreparedQueryResult result, uint32 timediff) {
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("entities.player.loading", "Loading auras for player %u",
            GetGUIDLow());
#endif
  ...
  remainCharges FROM character_aura WHERE guid = '%u'", GetGUIDLow());
  */
    ...
    do {
      ...
      if (remaintime != -1 &&
          ((!spellInfo->IsPositive() && spellInfo->Id != 15007) ||
           spellInfo->HasAttribute(
               SPELL_ATTR4_FADES_WHILE_LOGGED_OUT))) // Xinef: resurrection
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
        LOG_DEBUG("server", "Added aura spellid %u, effectmask %u",
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff) {
  ...
  // character_inventory.item = item_instance.guid WHERE character_inventory.guid
  // = '%u' ORDER BY bag, slot", GetGUIDLow()); NOTE: the "order by `bag`" is
  // important because it makes sure the bagMap is filled before items in the
  // bags are loaded NOTE2: the "order by `slot`" is needed because mainhand
  // weapons are (wrongly?) expected to be equipped before offhand items (TODO:
  // fixme)
      ...
      if (Item *item = _LoadItem(trans, zoneId, timeDiff, fields)) {
        ...
        if (!bagGuid) {
          ...
          if (itr != bagMap.end()) {
            ...
            std::map<uint32, Item *>::iterator itr =
                invalidBagMap.find(bagGuid);
            if (std::find(problematicItems.begin(), problematicItems.end(),
                          itr->second) != problematicItems.end())
              err = EQUIP_ERR_INT_BAG_ERROR;
          } else {
            LOG_ERROR("server",
                      "Player::_LoadInventory: player (GUID: %u, name: '%s') "
                      "has item (GUID: %u, entry: %u) which doesnt have a "
                      "valid bag (Bag GUID: %u, slot: %u). Possible cheat?",
                      GetGUIDLow(), GetName().c_str(), item->GetGUIDLow(),
                      item->GetEntry(), bagGuid, slot);
        ...
        if (err == EQUIP_ERR_OK)
          ...
          LOG_ERROR(
              ...
              "(GUID: %u, entry: %u) which can't be loaded into inventory (Bag "
              "GUID: %u, slot: %u) by reason %u. Item will be sent by mail.",
              GetGUIDLow(), GetName().c_str(), item->GetGUIDLow(),
              item->GetEntry(), bagGuid, slot, err);
          item->DeleteFromInventoryDB(trans);
...
Item *Player::_LoadItem(SQLTransaction &trans, uint32 zoneId, uint32 timeDiff,
                        Field *fields) {
  Item *item = nullptr;
    ...
    if (item->LoadFromDB(itemGuid, GetGUID(), fields, itemEntry)) {
      ...
      if (IsAlive() && item->IsLimitedToAnotherMapOrZone(GetMapId(), zoneId)) {
        ...
        LOG_DEBUG("entities.player.loading",
                  ...
                  "(%u). Deleting item.",
                  GetGUIDLow(), GetName().c_str(), GetMapId(),
                  item->GetGUIDLow(), item->GetEntry(), zoneId);
#endif
        remove = true;
      }
      ...
      // minutes"
        ...
        LOG_DEBUG("entities.player.loading",
                  ...
                  GetGUIDLow(), GetName().c_str(), timeDiff, item->GetGUIDLow(),
                  item->GetEntry());
#endif
        remove = true;
        ...
        if (item->GetPlayedTime() > (2 * HOUR)) {
          ...
          LOG_DEBUG("entities.player.loading",
                    ...
                    GetGUIDLow(), GetName().c_str(), item->GetGUIDLow(),
                    item->GetEntry(), item->GetPlayedTime());
#endif
          stmt = CharacterDatabase.GetPreparedStatement(
      ...
      LOG_ERROR(
          "server",
          "Player::_LoadInventory: player (GUID: %u, name: '%s') has broken "
  ...
  do {
    ...
    if (!proto) {
      LOG_ERROR("server",
                ...
                GetGUIDLow(), itemGuid, itemTemplate, mail->messageID);
    ...
    if (!item->LoadFromDB(
            itemGuid, MAKE_NEW_GUID(fields[13].GetUInt32(), 0, HIGHGUID_PLAYER),
            fields, itemTemplate)) {
      LOG_ERROR("server",
...
void Player::_LoadMailInit(PreparedQueryResult resultMailCount,
  ...
  // UI64FMTD "'", GUID_LOPART(playerGuid), (uint64)cTime);
  if (resultUnread)
  ...
  // WHERE receiver = '%u' AND (checked & 1)=0", GUID_LOPART(playerGuid));
  if (resultDelivery)
    ...
    do {
      ...
      if (quest) {
        ...
        if (qstatus < MAX_QUEST_STATUS)
          ...
          questStatusData.Status = QUEST_STATUS_INCOMPLETE;
          LOG_ERROR("server",
                    "Player %s (GUID: %u) has invalid quest %d status (%u), "
        ...
        LOG_DEBUG("entities.player.loading",
                  ...
                  questStatusData.Status, quest_id, GetGUIDLow());
#endif
      }
    } while (result->NextRow());
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("entities.player.loading",
                "Weekly quest {%u} cooldown for player (GUID: %u)", quest_id,
                GetGUIDLow());
#endif
    } while (result->NextRow());
  }
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("entities.player.loading",
                "Seasonal quest {%u} cooldown for player (GUID: %u)", quest_id,
                GetGUIDLow());
#endif
    } while (result->NextRow());
  }
...
void Player::_LoadMonthlyQuestStatus(PreparedQueryResult result) {
  ...
  if (result) {
    do {
      ...
      LOG_DEBUG("entities.player.loading",
                "Monthly quest {%u} cooldown for player (GUID: %u)", quest_id,
                GetGUIDLow());
    } while (result->NextRow());
  }
...
void Player::BindToInstance() {
  ...
  data << uint32(0);
  GetSession()->SendPacket(&data);
  sInstanceSaveMgr->PlayerBindToInstance(this->GetGUIDLow(), mapSave, true,
                                         this);
}
...
void Player::SendRaidInfo() {
  ...
  for (uint8 i = 0; i < MAX_DIFFICULTY; ++i) {
    BoundInstancesMap const &m_boundInstances =
        sInstanceSaveMgr->PlayerGetBoundInstances(GetGUIDLow(), Difficulty(i));
    for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
      ...
      if (itr->second.perm) {
        ...
        data << uint64(MAKE_NEW_GUID(save->GetInstanceId(), 0,
                                     HIGHGUID_INSTANCE));       // instance id
...
bool Player::Satisfy(AccessRequirement const *ar, uint32 target_map,
  ...
  if (!IsGameMaster() && ar) {
    ...
    if (GetTeamId(true) == TEAM_ALLIANCE && ar->quest_A &&
        !GetQuestRewardStatus(ar->quest_A))
      missingQuest = ar->quest_A;
    else if (GetTeamId(true) == TEAM_HORDE && ar->quest_H &&
    ...
    if (LevelMin || LevelMax || missingItem || missingQuest ||
      ...
      if (report) {
        if (missingQuest && !ar->questFailedText.empty())
          ChatHandler(GetSession())
              .PSendSysMessage("%s", ar->questFailedText.c_str());
        else if (mapDiff->hasErrorMessage) // if (missingAchievement) covered by
                                           // this case
          ...
          GetSession()->SendAreaTriggerMessage(
              ...
              LevelMin,
              sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
        else if (LevelMin)
          GetSession()->SendAreaTriggerMessage(
...
void Player::_SaveInventory(SQLTransaction &trans) {
  ...
  for (RefundableItemsSet::iterator itr = m_refundableItems.begin();
    ...
    if (iPtr) {
      ...
      LOG_ERROR("server",
                "Can't find item guid %u but is in refundable storage for "
                "player %u ! Removing.",
                *itr, GetGUIDLow());
    ...
    if (item->GetState() != ITEM_REMOVED) {
      ...
      if (test == nullptr) {
        ...
        LOG_ERROR(
            ...
            "slot(%u) values for the item with guid %u (state %d) are "
            "incorrect, the player doesn't have an item at that position!",
            lowGuid, GetName().c_str(), item->GetBagSlot(), item->GetSlot(),
        ...
        LOG_ERROR("server",
                  "Player(GUID: %u Name: %s)::_SaveInventory - the bag(%u) and "
                  "slot(%u) values for the item with guid %u are incorrect, "
                  "the item with guid %u is there instead!",
                  lowGuid, GetName().c_str(), item->GetBagSlot(),
                  item->GetSlot(), item->GetGUIDLow(), test->GetGUIDLow());
...
void Player::UpdateSpeakTime(uint32 specialMessageLimit) {
  ...
  if (m_speakTime > current) {
    uint32 max_count =
        specialMessageLimit
            ? specialMessageLimit
            : sWorld->getIntConfig(CONFIG_CHATFLOOD_MESSAGE_COUNT);
    if (!max_count)
      return;
...
void Player::SetUInt32ValueInArray(Tokenizer &tokens, uint16 index,
                                   uint32 value) {
  char buf[11];
...
void Player::ResetInstances(uint64 guid, uint8 method, bool isRaid) {
  ...
  case INSTANCE_RESET_ALL: {
    ...
    BoundInstancesMap const &m_boundInstances =
        sInstanceSaveMgr->PlayerGetBoundInstances(
            p->GetGUIDLow(), Difficulty(DUNGEON_DIFFICULTY_NORMAL));
    for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
         itr != m_boundInstances.end(); ++itr) {
  ...
  case INSTANCE_RESET_CHANGE_DIFFICULTY: {
    ...
    std::vector<InstanceSave *> toUnbind;
    BoundInstancesMap const &m_boundInstances =
        sInstanceSaveMgr->PlayerGetBoundInstances(p->GetGUIDLow(),
    ...
    for (uint8 d = 0; d < MAX_DIFFICULTY; ++d) {
      ...
      for (std::vector<InstanceSave *>::const_iterator itr = toUnbind.begin();
        ...
        sInstanceSaveMgr->PlayerUnbindInstance(
            p->GetGUIDLow(), (*itr)->GetMapId(), (*itr)->GetDifficulty(), true,
            p);
    }
  } break;
  case INSTANCE_RESET_GROUP_LEAVE: {
    ...
    for (uint8 d = 0; d < MAX_DIFFICULTY; ++d) {
      std::vector<InstanceSave *> toUnbind;
      BoundInstancesMap const &m_boundInstances =
          sInstanceSaveMgr->PlayerGetBoundInstances(GUID_LOPART(guid),
                                                    Difficulty(d));
      for (BoundInstancesMap::const_iterator itr = m_boundInstances.begin();
           itr != m_boundInstances.end(); ++itr) {
        if (itr->second.perm)
      ...
      for (std::vector<InstanceSave *>::const_iterator itr = toUnbind.begin();
           itr != toUnbind.end(); ++itr)
        sInstanceSaveMgr->PlayerUnbindInstance(
            GUID_LOPART(guid), (*itr)->GetMapId(), (*itr)->GetDifficulty(),
            true, p);
    }
  } break;
  }
...
void Player::UpdatePvPFlag(time_t currTime) {
  ...
  if (currTime < (pvpInfo.EndTimer + 300 + 5)) {
    if (currTime > (pvpInfo.EndTimer + 4) &&
        !HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_PVP_TIMER))
...
Pet *Player::GetPet() const {
  if (uint64 pet_guid = GetPetGUID()) {
    ...
    // LOG_ERROR("server", "Player::GetPet: Pet %u not exist.",
...
void Player::StopCastingCharm() {
  ...
  if (GetCharmGUID()) {
    LOG_FATAL("server",
              "Player %s (GUID: " UI64FMTD
              " is not able to uncharm unit (GUID: " UI64FMTD
              " Entry: %u, Type: %u)",
              GetName().c_str(), GetGUID(), GetCharmGUID(), charm->GetEntry(),
              charm->GetTypeId());
    if (charm->GetCharmerGUID()) {
      LOG_FATAL("server", "Charmed unit has charmer guid " UI64FMTD,
...
void Player::TextEmote(const std::string &text) {
  ...
  Warhead::AnyPlayerInObjectRangeCheck checker(
      this, sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE));
  Warhead::PlayerListSearcher<Warhead::AnyPlayerInObjectRangeCheck> searcher(
      this, players, checker);
  this->VisitNearbyWorldObject(
      sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), searcher);
...
void Player::Whisper(const std::string &text, uint32 language,
  ...
  Player *rPlayer =
      ObjectAccessor::GetObjectInOrOutOfWorld(receiver, (Player *)nullptr);
...
void Player::PossessSpellInitialize() {
  ...
  if (!charmInfo) {
    LOG_ERROR("server",
              "Player::PossessSpellInitialize(): charm (" UI64FMTD
              ") has no charminfo!",
              charm->GetGUID());
    return;
  }
...
void Player::CharmSpellInitialize() {
  ...
  if (!charmInfo) {
    LOG_ERROR("server",
              "Player::CharmSpellInitialize(): the player's charm (" UI64FMTD
              ") has no charminfo!",
              charm->GetGUID());
    return;
  }
...
void Player::RemovePetitionsAndSigns(uint64 guid, uint32 type) {
  ...
  for (SignatureContainer::iterator itr = signatureStore->begin();
    ...
    if (signItr != itr->second.signatureMap.end()) {
      ...
      uint64 petitionguid =
          MAKE_NEW_GUID(petition->petitionGuid, 0, HIGHGUID_ITEM);
...
uint32 Player::GetArenaTeamId(uint8 slot) const {
  ...
  if (slot >= MAX_ARENA_SLOT) {
    ...
    rtVal = GetUInt32Value(PLAYER_FIELD_ARENA_TEAM_INFO_1_1 +
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("entities.unit", "WORLD: Restart character %u taxi flight",
            GetGUIDLow());
#endif
...
inline bool
Player::_StoreOrEquipNewItem(uint32 vendorslot, uint32 item, uint8 count,
                             uint8 bag, uint8 slot, int32 price,
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("network",
              "WORLD: BuyItemFromVendor - Unit (GUID: %u) not found or you "
  ...
  if (crItem->ExtendedCost) {
    ItemExtendedCostEntry const *iece =
        sItemExtendedCostStore.LookupEntry(crItem->ExtendedCost);
    if (!iece) {
      LOG_ERROR("server", "Item %u have wrong ExtendedCost field value %u",
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("maps",
              "PLAYER: Player '%s' (GUID: %u) will be teleported to homebind "
              "in 60 seconds",
              GetName().c_str(), GetGUIDLow());
#endif
  }
...
void Player::AddSpellAndCategoryCooldowns(SpellInfo const *spellInfo,
  ...
  if (infinityCooldown) {
    ...
    if (catrec > 0 &&
        !spellInfo->HasAttribute(SPELL_ATTR6_IGNORE_CATEGORY_COOLDOWN_MODS))
...
void Player::AddSpellCooldown(uint32 spellid, uint32 itemid, uint32 end_time,
...
template <class T>
inline void UpdateVisibilityOf_helper(Player::ClientGUIDs &s64, T *target,
                                      std::vector<Unit *> & /*v*/) {
  s64.insert(target->GetGUID());
}

template <>
inline void UpdateVisibilityOf_helper(Player::ClientGUIDs &s64,
                                      GameObject *target,
                                      std::vector<Unit *> & /*v*/) {
...
template <>
inline void UpdateVisibilityOf_helper(Player::ClientGUIDs &s64, Player *target,
                                      std::vector<Unit *> &v) {
  s64.insert(target->GetGUID());
  v.push_back(target);
}
...
void Player::UpdateVisibilityForPlayer(bool mapChange) {
  Warhead::VisibleNotifier notifierNoLarge(
      *this, mapChange,
      false); // visit only objects which are not large; default distance
  m_seer->VisitNearbyObject(GetSightRange() + VISIBILITY_INC_FOR_GOBJECTS,
                            notifierNoLarge);
  notifierNoLarge.SendToSelf();
  Warhead::VisibleNotifier notifierLarge(
...
void Player::SendInstanceResetWarning(uint32 mapid, Difficulty difficulty,
                                      uint32 time, bool onEnterMap) {
  // pussywizard:
  InstancePlayerBind *bind =
      sInstanceSaveMgr->PlayerGetBoundInstance(GetGUIDLow(), mapid, difficulty);
...
void Player::SetBattlegroundId(uint32 id, BattlegroundTypeId bgTypeId,
                               uint32 queueSlot, bool invited, bool isRandom,
                               TeamId teamId) {
  ...
  for (AuraEffectList::const_iterator itr = dummyAuras.begin();
    ...
    if (prio < 2 && (*itr)->GetSpellInfo()->SpellVisual[0] == 99 &&
      ...
      case 47883:
        ...
        break; // rank 7
      default:
        LOG_ERROR("server", "Unhandled spell %u: S.Resurrection",
                  (*itr)->GetId());
        continue;
      }
...
void Player::RewardPlayerAndGroupAtEvent(uint32 creature_id,
  ...
  if (!pRewardSource)
    return;
  uint64 creature_guid = (pRewardSource->GetTypeId() == TYPEID_UNIT)
...
uint32 Player::GetBaseWeaponSkillValue(WeaponAttackType attType) const {
  ...
  uint32 skill = (item && item->GetSkill() != SKILL_FIST_WEAPONS)
                     ? item->GetSkill()
                     : uint32(SKILL_UNARMED);
...
void Player::SetMover(Unit *target) {
  if (this != target && target->m_movedByPlayer &&
    ...
    LOG_INFO("misc", "Player::SetMover (A1) - %u, %u, %u, %u, %u, %u, %u, %u",
             GetGUIDLow(), GetMapId(), GetInstanceId(), FindMap()->GetId(),
             IsInWorld() ? 1 : 0, IsDuringRemoveFromWorld() ? 1 : 0,
             IsBeingTeleported() ? 1 : 0, isBeingLoaded() ? 1 : 0);
    LOG_INFO(
        "misc",
        "Player::SetMover (A2) - %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u",
        target->GetTypeId(), target->GetEntry(), target->GetUnitTypeMask(),
        target->GetGUIDLow(), target->GetMapId(), target->GetInstanceId(),
        target->FindMap()->GetId(), target->IsInWorld() ? 1 : 0,
        target->IsDuringRemoveFromWorld() ? 1 : 0,
        (target->ToPlayer() && target->ToPlayer()->IsBeingTeleported() ? 1 : 0),
        target->isBeingLoaded() ? 1 : 0);
    LOG_INFO("misc", "Player::SetMover (A3) - %u, %u, %u, %u, %u, %u, %u, %u",
             target->m_movedByPlayer->GetGUIDLow(),
  ...
  if (this != target &&
    ...
    LOG_INFO("misc", "Player::SetMover (B1) - %u, %u, %u, %u, %u, %u, %u, %u",
             GetGUIDLow(), GetMapId(), GetInstanceId(), FindMap()->GetId(),
             IsInWorld() ? 1 : 0, IsDuringRemoveFromWorld() ? 1 : 0,
             IsBeingTeleported() ? 1 : 0, isBeingLoaded() ? 1 : 0);
    LOG_INFO(
        "misc",
        "Player::SetMover (B2) - %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u",
        target->GetTypeId(), target->GetEntry(), target->GetUnitTypeMask(),
        target->GetGUIDLow(), target->GetMapId(), target->GetInstanceId(),
        target->FindMap()->GetId(), target->IsInWorld() ? 1 : 0,
        target->IsDuringRemoveFromWorld() ? 1 : 0,
...
void Player::StoreLootItem(uint8 lootSlot, Loot *loot) {
  ...
  // master loot Xinef: only quest, ffa and conditioned items
  if (!IS_ITEM_GUID(GetLootGUID()) && GetGroup() &&
  ...
  if (msg == EQUIP_ERR_OK) {
    ...
    if (loot->containerId > 0)
      sLootItemStorage->RemoveStoredLootItem(loot->containerId, item->itemid,
                                             item->count, loot);
...
void Player::_LoadSkills(PreparedQueryResult result) {
  ...
  if (result) {
    do {
      ...
      if (!pSkill) {
        LOG_ERROR("server", "Character %u has skill %u that does not exist.",
                  GetGUIDLow(), skill);
        continue;
      }
      ...
      {
        LOG_ERROR("server", "Character %u has more than %u skills.",
...
void Player::HandleFall(MovementInfo const &movementInfo) {
  ...
  if (z_diff >= 14.57f && !isDead() && !IsGameMaster() &&
    ...
    if (damageperc > 0 &&
      ...
      // reduction
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("server", "FALLDAMAGE mZ=%f z=%f fallTime=%u damage=%u SF=%d",
...
void Player::ResetAchievementCriteria(
    AchievementCriteriaCondition condition, uint32 value,
...
void Player::LearnPetTalent(uint64 petGuid, uint32 talentId,
  ...
  uint32 spellid = talentInfo->RankID[talentRank];
  if (spellid == 0) {
    LOG_ERROR("server", "Talent.dbc have for talent: %u Rank: %u spell id = 0",
              talentId, talentRank);
    return;
  }
  ...
  *data << uint32(slotUsedMask); // slotUsedMask < 0x80000
  ...
  for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i) {
    ...
    *data << uint16(item->GetItemRandomPropertyId()); // item random property id
    data->appendPackGUID(
        item->GetUInt64Value(ITEM_FIELD_CREATOR)); // item creator
...
void Player::ActivateSpec(uint8 spec) {
  ...
  stmt->setUInt32(0, GetGUIDLow());
  stmt->setUInt8(1, m_activeSpec);
  GetSession()->_loadActionsSwitchSpecCallback =
      CharacterDatabase.AsyncQuery(stmt); // FutureResult
...
void Player::PrepareCharmAISpells() {
  ...
  for (PlayerSpellMap::iterator itr = m_spells.begin(); itr != m_spells.end();
    ...
    if (!spellInfo->SpellFamilyName || spellInfo->IsPassive() ||
        ...
        (spellInfo->Stances &&
         !spellInfo->HasAttribute(SPELL_ATTR2_NOT_NEED_SHAPESHIFT)))
      continue;
...
void Player::AddRefundReference(uint32 it) { m_refundableItems.insert(it); }
...
void Player::SummonPet(uint32 entry, float x, float y, float z, float ang,
                       PetType petType, uint32 duration, uint32 createdBySpell,
                       uint64 casterGUID, uint8 asynchLoadType) {
