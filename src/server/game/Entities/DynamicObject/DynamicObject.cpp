...
void DynamicObject::RemoveFromWorld() {
  ///- Remove the dynamicObject from the accessor and from all lists of objects
  ///in world
  if (IsInWorld()) {
    if (_isViewpoint)
...
bool DynamicObject::CreateDynamicObject(uint32 guidlow, Unit *caster,
                                        uint32 spellId, Position const &pos,
                                        float radius, DynamicObjectType type) {
  SetMap(caster->GetMap());
  Relocate(pos);
  if (!IsPositionValid()) {
    LOG_ERROR("server",
              "DynamicObject (spell %u) not created. Suggested coordinates "
              "isn't valid (X: %f Y: %f)",
  ...
  // The lower word of DYNAMICOBJECT_BYTES must be 0x0001. This value means that
  // the visual radius will be overriden by client for most of the "ground
  // patch" visual effect spells and a few "skyfall" ones like Hurricane. If any
  // other value is used, the client will _always_ use the radius provided in
  // DYNAMICOBJECT_RADIUS, but precompensation is necessary (eg radius *= 2) for
  // many spells. Anyway, blizz sends 0x0001 for all the spells I saw sniffed...
