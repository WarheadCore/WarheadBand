...
TempSummon::TempSummon(SummonPropertiesEntry const *properties, uint64 owner,
                       bool isWorldObject)
...
Unit *TempSummon::GetSummoner() const {
  return m_summonerGUID ? ObjectAccessor::GetUnit(*this, m_summonerGUID)
                        : nullptr;
}
...
void TempSummon::InitSummon() {
  ...
  // Xinef: Allow to call this hook when npc is summoned by gameobject, in this
  // case pass this as summoner to avoid possible null checks
  if (IsAIEnabled)
...
void TempSummon::RemoveFromWorld() {
  ...
  // if (GetOwnerGUID())
  //    LOG_ERROR("server", "Unit %u has owner guid when removed from world",
  //    GetEntry());

  Creature::RemoveFromWorld();
}

Minion::Minion(SummonPropertiesEntry const *properties, uint64 owner,
               bool isWorldObject)
...
Guardian::Guardian(SummonPropertiesEntry const *properties, uint64 owner,
                   bool isWorldObject)
...
Puppet::Puppet(SummonPropertiesEntry const *properties, uint64 owner)
...
void Puppet::InitSummon() {
  ...
  if (!SetCharmedBy(GetOwner(), CHARM_TYPE_POSSESS)) {
    if (Player *p = GetOwner())
      LOG_INFO("misc",
               "Puppet::InitSummon (A1) - %u, %u, %u, %u, %u, %u, %u, %u, %u, "
               "%u, %u",
               p->GetTypeId(), p->GetEntry(), p->GetUnitTypeMask(),
               p->GetGUIDLow(), p->GetMapId(), p->GetInstanceId(),
               p->FindMap()->GetId(), p->IsInWorld() ? 1 : 0,
               p->IsDuringRemoveFromWorld() ? 1 : 0,
               p->IsBeingTeleported() ? 1 : 0, p->isBeingLoaded() ? 1 : 0);
    else {
