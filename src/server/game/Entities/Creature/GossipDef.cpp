...
void GossipMenu::AddMenuItem(int32 menuItemId, uint8 icon,
                             std::string const &message, uint32 sender,
                             uint32 action, std::string const &boxMessage,
  ...
  if (menuItemId == -1) {
    menuItemId = 0;
    if (!_menuItems.empty()) {
      for (GossipMenuItemContainer::const_iterator itr = _menuItems.begin();
           itr != _menuItems.end(); ++itr) {
 ...
 */
void GossipMenu::AddMenuItem(uint32 menuId, uint32 menuItemId, uint32 sender,
                             uint32 action) {
  ...
  for (GossipMenuItemsContainer::const_iterator itr = bounds.first;
    ...
    if (optionBroadcastText)
      ObjectMgr::GetLocaleString(optionBroadcastText->MaleText, GetLocale(),
                                 strOptionText);
    else
    ...
    /// BoxText
    if (boxBroadcastText)
      ObjectMgr::GetLocaleString(boxBroadcastText->MaleText, GetLocale(),
                                 strBoxText);
    else
    ...
    if (GetLocale() != DEFAULT_LOCALE) {
      if (!optionBroadcastText) {
        ...
        if (GossipMenuItemsLocale const *gossipMenuLocale =
          ...
          ObjectMgr::GetLocaleString(gossipMenuLocale->OptionText, GetLocale(),
                                     strOptionText);
      }
      ...
      if (!boxBroadcastText) {
        ...
        if (GossipMenuItemsLocale const *gossipMenuLocale =
                sObjectMgr->GetGossipMenuItemsLocale(
                    MAKE_PAIR32(menuId, menuItemId)))
          ObjectMgr::GetLocaleString(gossipMenuLocale->BoxText, GetLocale(),
                                     strBoxText);
      }
    }
    ...
    AddMenuItem(-1, itr->second.OptionIcon, strOptionText, sender, action,
                strBoxText, itr->second.BoxMoney, itr->second.BoxCoded);
  }
}
...
void PlayerMenu::SendGossipMenu(uint32 titleTextId, uint64 objectGUID) const {
  ...
  for (GossipMenuItemContainer::const_iterator itr =
       ...
       itr != _gossipMenu.GetMenuItems().end(); ++itr) {
  ...
  for (uint32 iI = 0; iI < _questMenu.GetMenuItemCount(); ++iI) {
    ...
    if (Quest const *quest = sObjectMgr->GetQuestTemplate(questID)) {
      ...
      data << uint8(
          0); // 3.3.3 changes icon: blue question or yellow exclamation
...
void PlayerMenu::SendPointOfInterest(uint32 poiId) const {
  ...
  if (!poi) {
    LOG_ERROR("sql.sql", "Request to send non-existing POI (Id: %u), ignored.",
              poiId);
    return;
  }
...
void PlayerMenu::SendQuestGiverQuestList(QEmote const &eEmote,
  ...
  WorldPacket data(SMSG_QUESTGIVER_QUEST_LIST,
                   100 + _questMenu.GetMenuItemCount() * 75); // guess size
  data << uint64(npcGUID);
  data << Title;
  ...
  LOG_DEBUG("network", "WORLD: Sent SMSG_QUESTGIVER_QUEST_LIST NPC Guid=%u",
            GUID_LOPART(npcGUID));
#endif
}
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const *quest, uint64 npcGUID,
  ...
  if (QuestLocale const *localeData =
    ...
    ObjectMgr::GetLocaleString(localeData->AreaDescription, locale,
                               questAreaDescription);
  }
  ...
  data << uint32(quest->GetRewSpell()); // reward spell, this spell will display
  ...
  LOG_DEBUG("network",
            ...
            GUID_LOPART(npcGUID), quest->GetQuestId());
#endif
}
...
void PlayerMenu::SendQuestQueryResponse(Quest const *quest) const {
  ...
  if (QuestLocale const *localeData =
    ...
    ObjectMgr::GetLocaleString(localeData->AreaDescription, locale,
                               questAreaDescription);
    ObjectMgr::GetLocaleString(localeData->CompletedText, locale,
                               questCompletedText);

    for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
      ObjectMgr::GetLocaleString(localeData->ObjectiveText[i], locale,
  ...
  data << uint32(
      quest->GetQuestLevel()); // may be -1, static data, in other cases must be
                               // used dynamic level: Player::GetQuestLevel (0
                               // is not known, but assuming this is no longer
                               // valid for quest intended for client)
  ...
  data << uint32(quest->GetRepObjectiveFaction()); // shown in quest log as part
                                                   // of quest objective
  data << uint32(quest->GetRepObjectiveValue());   // shown in quest log as part
                                                   // of quest objective

  data << uint32(
      quest->GetRepObjectiveFaction2()); // shown in quest log as part of quest
  ...
  for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i) {
    if (quest->RequiredNpcOrGo[i] < 0)
      data << uint32((quest->RequiredNpcOrGo[i] * (-1)) |
                     0x80000000); // client expects gameobject template id in
                                  // form (id|0x80000000)
    else
  ...
  _session->SendPacket(&data);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("network", "WORLD: Sent SMSG_QUEST_QUERY_RESPONSE questid=%u",
            quest->GetQuestId());
#endif
}

void PlayerMenu::SendQuestGiverOfferReward(Quest const *quest, uint64 npcGUID,
  ...
  if (QuestOfferRewardLocale const *questOfferRewardLocale =
    ...
    ObjectMgr::GetLocaleString(questOfferRewardLocale->RewardText, locale,
                               RewardText);
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const *quest, uint64 npcGUID,
                                            bool canComplete,
                                            bool closeOnCancel) const {
  // We can always call to RequestItems, but this packet only goes out if there
  // are actually items.  Otherwise, we'll skip straight to the OfferReward
  ...
  if (QuestRequestItemsLocale const *questRequestItemsLocale =
    ...
    ObjectMgr::GetLocaleString(questRequestItemsLocale->CompletionText, locale,
                               requestItemsText);
