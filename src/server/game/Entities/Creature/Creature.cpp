...
bool ForcedDespawnDelayEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/) {
  m_owner.DespawnOrUnsummon(); // since we are here, we are not TempSummon as
...
Creature::Creature(bool isWorldObject)
    : Unit(isWorldObject), MovableMapObject(), m_groupLootTimer(0),
      lootingGroupLowGUID(0), m_PlayerDamageReq(0), m_lootRecipient(0),
      m_lootRecipientGroup(0), m_corpseRemoveTime(0), m_respawnTime(0),
      m_respawnDelay(300), m_corpseDelay(60), m_wanderDistance(0.0f),
      ...
      m_DBTableGuid(0), m_equipmentId(0), m_originalEquipmentId(0),
      m_originalAnimTier(UNIT_BYTE1_FLAG_GROUND),
      m_AlreadyCallAssistance(false), m_AlreadySearchedAssistance(false),
      m_regenHealth(true), m_AI_locked(false),
      ...
      m_creatureData(nullptr), m_waypointID(0), m_path_id(0),
      m_formation(nullptr), _lastDamagedTime(0), m_cannotReachTarget(false),
...
void Creature::SearchFormation() {
  ...
  CreatureGroupInfoType::iterator frmdata =
...
bool Creature::InitEntry(uint32 Entry, const CreatureData *data) {
  ...
  for (uint8 diff = uint8(GetMap()->GetSpawnMode()); diff > 0 && !IsPet();) {
    ...
    // for instances heroic to normal, other cases attempt to retrieve previous
    // difficulty
...
bool Creature::UpdateEntry(uint32 Entry, const CreatureData *data,
  ...
  float armor = (float)stats->GenerateArmor(
      cInfo); // TODO: Why is this treated as uint32 when it's a float?
  ...
  // updates spell bars for vehicles and set player's faction - should be called
  // here, to overwrite faction that is set from the new template
  if (IsVehicle()) {
    if (Player *owner = Creature::
            GetCharmerOrOwnerPlayerOrPlayerItself()) // this check comes in case
                                                     // we don't have a player
    {
      setFaction(
          owner->getFaction()); // vehicles should have same as owner faction
...
void Creature::Update(uint32 diff) {
  ...
  switch (m_deathState) {
  case JUST_RESPAWNED:
    // Must not be called, see Creature::setDeathState JUST_RESPAWNED -> ALIVE
    // promoting.
    LOG_ERROR(
        "server",
        "Creature (GUID: %u Entry: %u) in wrong state: JUST_RESPAWNED (4)",
        GetGUIDLow(), GetEntry());
    break;
  case JUST_DIED:
  ...
  case DEAD: {
    ...
    if (m_respawnTime <= now) {
      bool allowed = IsAIEnabled ? AI()->CanRespawn()
                                 : true; // First check if there are any scripts
                                         // that object to us respawning
  ...
  case CORPSE: {
    ...
    if (m_groupLootTimer && lootingGroupLowGUID) {
      ...
      LOG_DEBUG("server", "Removing corpse... %u ",
  ...
  case ALIVE: {
    ...
    // CORPSE/DEAD state will processed at next tick (in other case death timer
    ...
    if (NeedChangeAI) {
      ...
      // xinef: update combat state, if npc is not in combat - return to spawn
      // correctly by calling EnterEvadeMode
      SelectVictim();
    }
    ...
    if (Unit *victim = GetVictim()) {
      ...
      if (diff >= m_moveBackwardsMovementTime) {
        ...
        m_moveBackwardsMovementTime = urand(MOVE_BACKWARDS_CHECK_INTERVAL,
      ...
      if (diff >= m_moveCircleMovementTime) {
        ...
        m_moveCircleMovementTime =
            urand(MOVE_CIRCLE_CHECK_INTERVAL, MOVE_CIRCLE_CHECK_INTERVAL * 2);
      } else {
    ...
    if (m_regenTimer <= 0) {
      if (!IsInEvadeMode()) {
        ...
        if (!IsInCombat() || IsPolymorphed())
          ...
          // regenerate health if cannot reach the target and the setting is set
          // to do so. this allows to disable the health regen of raid bosses if
          // pathfinding has issues for whatever reason
          if (sWorld->getBoolConfig(
                  CONFIG_REGEN_HP_CANNOT_REACH_TARGET_IN_RAID) ||
              !GetMap()->IsRaid()) {
            RegenerateHealth();
            LOG_DEBUG("entities.unit",
                      "RegenerateHealth() enabled because Creature cannot "
                      "reach the target. Detail: %s",
                      GetDebugInfo().c_str());
          } else
            LOG_DEBUG("entities.unit",
                      "RegenerateHealth() disabled even if the Creature cannot "
                      "reach the target. Detail: %s",
                      GetDebugInfo().c_str());
        }
      }
    ...
    if (IS_PLAYER_GUID(GetOwnerGUID())) {
      if (m_transportCheckTimer <= diff) {
        ...
        Transport *newTransport =
            GetMap()->GetTransportForPos(GetPhaseMask(), GetPositionX(),
                                         GetPositionY(), GetPositionZ(), this);
...
void Creature::Regenerate(Powers power) {
  ...
  case POWER_MANA: {
    ...
    if (IsInCombat() || GetCharmerOrOwnerGUID()) {
      if (GetEntry() == NPC_IMP || GetEntry() == NPC_WATER_ELEMENTAL_TEMP ||
        ...
        addvalue = uint32(
            (GetStat(STAT_SPIRIT) / (IsUnderLastManaUseEffect() ? 8.0f : 5.0f) +
             17.0f));
  ...
  addvalue +=
      ...
      (power == POWER_FOCUS ? PET_FOCUS_REGEN_INTERVAL
...
void Creature::DoFleeToGetAssistance() {
  ...
  if (radius > 0) {
    ...
    Cell cell(p);
    cell.SetNoCreate();
    Warhead::NearestAssistCreatureInCreatureRangeCheck u_check(
        this, GetVictim(), radius);
    Warhead::CreatureLastSearcher<
        Warhead::NearestAssistCreatureInCreatureRangeCheck>
...
bool Creature::Create(uint32 guidlow, Map *map, uint32 phaseMask, uint32 Entry,
                      uint32 vehId, float x, float y, float z, float ang,
  ...
  if (!cinfo) {
    LOG_ERROR("sql.sql",
              "Creature::Create(): creature template (guidlow: %u, entry: %u) "
              "does not exist.",
              guidlow, Entry);
    return false;
  }
  ...
  if (!IsPositionValid()) {
    LOG_ERROR("server",
              "Creature::Create(): given coordinates for creature (guidlow %d, "
              "entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)",
  ...
  if (IsSpiritHealer() || IsSpiritGuide() ||
    ...
    m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GHOST,
             ...
             CREATURE_TYPE_FLAG_GHOST_VISIBLE) // Xinef: Add ghost visibility
                                               // for ghost units
    m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GHOST,
...
bool Creature::IsValidTrainerForPlayer(Player *player,
  ...
  case TRAINER_TYPE_PETS:
    if (m_creatureInfo->trainer_class &&
        m_creatureInfo->trainer_class != player->getClass()) {
      if (npcFlags)
...
bool Creature::isTappedBy(Player const *player) const {
  ...
  if (!playerGroup ||
      playerGroup != GetLootRecipientGroup()) // if we dont have a group we
                                              // arent the recipient
    return false; // if creature doesnt have group bound it means it was solo
...
void Creature::SelectLevel(bool changelevel) {
  ...
  SetModifierValue(UNIT_MOD_ATTACK_POWER_RANGED, BASE_VALUE,
...
bool Creature::CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId,
  ...
  if (!normalInfo) {
    LOG_ERROR("sql.sql",
              "Creature::CreateFromProto(): creature template (guidlow: %u, "
              "entry: %u) does not exist.",
              guidlow, Entry);
    return false;
  }
  ...
  Object::_Create(guidlow, Entry,
                  (vehId || normalInfo->VehicleId) ? HIGHGUID_VEHICLE
                                                   : HIGHGUID_UNIT);
...
bool Creature::LoadCreatureFromDB(uint32 guid, Map *map, bool addToMap,
  ...
  if (!Create(guid, map, data->phaseMask, data->id, 0, data->posX, data->posY,
              data->posZ, data->orientation, data))
    return false;
  ...
  if (m_respawnTime) // respawn on Update
  {
    m_deathState = DEAD;
    if (CanFly()) {
      float tz = map->GetHeight(GetPhaseMask(), data->posX, data->posY,
                                data->posZ, true, MAX_FALL_DISTANCE);
      if (data->posZ - tz > 0.1f && Warhead::IsValidMapCoord(tz)) {
...
bool Creature::CanStartAttack(Unit const *who) const {
  ...
  if ((HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC) &&
      ...
      (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC) &&
       who->GetTypeId() ==
           TYPEID_PLAYER)) // immune to PC and target is a player, return false
  {
    return false;
  }
  ...
  if (Unit *owner = who->GetOwner())
    if (owner->GetTypeId() == TYPEID_PLAYER &&
        HasFlag(
            UNIT_FIELD_FLAGS,
            UNIT_FLAG_IMMUNE_TO_PC)) // immune to PC and target has player owner
      return false;
  ...
  // attacker, help him if the distance is right (help because who is hostile or
  // help because attacker is friendly)
  bool assist = false;
  ...
  if (!assist)
    if (IsNeutralToAll() ||
        !IsWithinDistInMap(
            who, GetAggroRange(who) +
                     m_CombatDistance)) // pussywizard: +m_combatDistance for
                                        // turrets and similar
      return false;
...
void Creature::setDeathState(DeathState s, bool despawn) {
  ...
  if (s == JUST_DIED) {
    ...
    SetTarget(0); // remove target selection in any cases (can be set at aura
...
void Creature::Respawn(bool force) {
  ...
  if (getDeathState() == DEAD) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("server",
              "Respawning creature %s (GuidLow: %u, Full GUID: " UI64FMTD
              " Entry: %u)",
    ...
    if (IsAIEnabled) {
      // reset the AI to be sure no dirty or uninitialized values will be used
      // till next tick
      AI()->Reset();
      TriggerJustRespawned = true; // delay event to next tick so all creatures
                                   // are created on the map before processing
    }

    uint32 poolid = GetDBTableGUIDLow()
  ...
  // xinef: relocate notifier, fixes npc appearing in corpse position after
  // forced respawn (instead of spawn)
  m_last_notify_position.Relocate(-5000.0f, -5000.0f, -5000.0f, 0.0f);
...
bool Creature::HasMechanicTemplateImmunity(uint32 mask) const {
  return !IS_PLAYER_GUID(GetOwnerGUID()) &&
...
bool Creature::IsImmunedToSpellEffect(SpellInfo const *spellInfo,
  ...
  if (spellInfo->Effects[index].Mechanic > MECHANIC_NONE &&
      HasMechanicTemplateImmunity(1
                                  << (spellInfo->Effects[index].Mechanic - 1)))
    return true;
...
Unit *Creature::SelectNearestTarget(float dist,
...

  {
    ...
    Warhead::UnitLastSearcher<Warhead::NearestHostileUnitCheck> searcher(
        this, target, u_check);

    TypeContainerVisitor<
        Warhead::UnitLastSearcher<Warhead::NearestHostileUnitCheck>,
...
Unit *Creature::SelectNearestTargetInAttackDistance(float dist) const {
...

  {
    ...
    Warhead::UnitLastSearcher<Warhead::NearestHostileUnitInAttackDistanceCheck>
        searcher(this, target, u_check);

    TypeContainerVisitor<Warhead::UnitLastSearcher<
                             Warhead::NearestHostileUnitInAttackDistanceCheck>,
                         WorldTypeMapContainer>
        world_unit_searcher(searcher);
    TypeContainerVisitor<Warhead::UnitLastSearcher<
                             Warhead::NearestHostileUnitInAttackDistanceCheck>,
  ...
  if (!m_AlreadyCallAssistance && GetVictim() && !IsPet() && !IsCharmed()) {
    ...
    if (radius > 0) {
...

      {
        ...
        Warhead::AnyAssistCreatureInRangeCheck u_check(this, GetVictim(),
                                                       radius);
        Warhead::CreatureListSearcher<Warhead::AnyAssistCreatureInRangeCheck>
            searcher(this, assistList, u_check);

        TypeContainerVisitor<Warhead::CreatureListSearcher<
                                 Warhead::AnyAssistCreatureInRangeCheck>,
                             GridTypeMapContainer>
            grid_creature_searcher(searcher);
      ...
      if (!assistList.empty()) {
        ...
        while (!assistList.empty()) {
          // Pushing guids because in delay can happen some creature gets
          // despawned => invalid pointer
          e->AddAssistant((*assistList.begin())->GetGUID());
          assistList.pop_front();
        }
        m_Events.AddEvent(e, m_Events.CalculateTime(sWorld->getIntConfig(
...
void Creature::CallForHelp(float radius) {
  ...
  Warhead::CreatureWorker<Warhead::CallOfHelpCreatureInRangeDo> worker(this,
                                                                       u_do);

  TypeContainerVisitor<
      Warhead::CreatureWorker<Warhead::CallOfHelpCreatureInRangeDo>,
      GridTypeMapContainer>
...
void Creature::UpdateMoveInLineOfSightState() {
  ...
  if (IsPet() ||
      HasUnitTypeMask(UNIT_MASK_MINION | UNIT_MASK_SUMMON | UNIT_MASK_GUARDIAN |
  ...
  if (FactionTemplateEntry const *factionTemplate =
    ...
    if (factionTemplate->hostileMask || factionTemplate->enemyFaction[0] ||
        factionTemplate->enemyFaction[1] || factionTemplate->enemyFaction[2] ||
...
bool Creature::CanCreatureAttack(Unit const *victim, bool skipDistCheck) const {
  ...
  if (!IS_PLAYER_GUID(GetCharmerOrOwnerGUID())) {
    ...
    // pussywizard: don't check distance to home position if recently damaged
    // (allow kiting away from spawnpoint!) xinef: this should include taunt
    // auras
    if (!isWorldBoss() && (GetLastDamagedTime() > sWorld->GetGameTime() ||
  ...
  if (Unit *unit = GetCharmerOrOwner())
    ...
    // to prevent creatures in air ignore attacks because distance is already
    // too high...
...
CreatureAddon const *Creature::GetCreatureAddon() const {
  if (m_DBTableGuid) {
    if (CreatureAddon const *addon =
            sObjectMgr->GetCreatureAddon(m_DBTableGuid))
      return addon;
  }
...
bool Creature::LoadCreaturesAddon(bool reload) {
  ...
  if (cainfo->bytes1 != 0) {
    ...
    SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_STAND_STATE,
    ...
    SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_VIS_FLAG,
    ...
    //! If no inhabittype_fly (if no MovementFlag_DisableGravity or
    //! MovementFlag_CanFly flag found in sniffs) Check using InhabitType as
    //! movement flags are assigned dynamically basing on whether the creature
  ...
  if (cainfo->bytes2 != 0) {
    ...
    // SetByteValue(UNIT_FIELD_BYTES_2, 2, uint8((cainfo->bytes2 >> 16) &
    // 0xFF));
  ...
  if (!cainfo->auras.empty()) {
    for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin();
         itr != cainfo->auras.end(); ++itr) {
      SpellInfo const *AdditionalSpellInfo = sSpellMgr->GetSpellInfo(*itr);
      if (!AdditionalSpellInfo) {
        LOG_ERROR("sql.sql",
                  "Creature (GUID: %u Entry: %u) has wrong spell %u defined in "
                  "`auras` field.",
                  GetGUIDLow(), GetEntry(), *itr);
        continue;
      }
      ...
      if (HasAura(*itr)) {
        if (!reload)
          LOG_ERROR("sql.sql",
                    "Creature (GUID: %u Entry: %u) has duplicate aura (spell "
                    "%u) in `auras` field.",
                    GetGUIDLow(), GetEntry(), *itr);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
      LOG_DEBUG("entities.unit",
                "Spell: %u added to creature (GUID: %u Entry: %u)", *itr,
                GetGUIDLow(), GetEntry());
#endif
    }
  }
...
void Creature::SetInCombatWithZone() {
  if (!CanHaveThreatList()) {
    LOG_ERROR("server",
              "Creature entry %u call SetInCombatWithZone but creature cannot "
              "have threat list.",
              GetEntry());
    return;
  }
  ...
  if (!map->IsDungeon()) {
    LOG_ERROR("server",
              "Creature entry %u call SetInCombatWithZone for map (id: %u) "
              "that isn't an instance.",
              GetEntry(), map->GetId());
    return;
  }
...
bool Creature::IsSpellProhibited(SpellSchoolMask idSchoolMask) const {
  ...
  if (!(cinfo && cinfo->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH) &&
      (isWorldBoss() || IsDungeonBoss()))
    return false;
...
void Creature::AddSpellCooldown(uint32 spell_id, uint32 /*itemid*/,
                                uint32 end_time, bool /*needSendToClient*/,
  ...
  if (categorycooldown)
    if (spellInfo->GetCategory()) {
      SpellCategoryStore::const_iterator i_scstore =
          sSpellsByCategoryStore.find(spellInfo->GetCategory());
...
uint32 Creature::GetSpellCooldown(uint32 spell_id) const {
  ...
  return itr->second > World::GetGameTimeMS()
             ? itr->second - World::GetGameTimeMS()
             : 0;
}
...
void Creature::GetRespawnPosition(float &x, float &y, float &z, float *ori,
                                  float *dist) const {
  if (m_DBTableGuid) {
    if (CreatureData const *data =
            sObjectMgr->GetCreatureData(GetDBTableGUIDLow())) {
      x = data->posX;
      y = data->posY;
      z = data->posZ;
      if (ori)
...
void Creature::applyInhabitFlags() {
  if (IsAlive() && !HasUnitState(UNIT_STATE_ROOT) &&
      !HasUnitMovementFlag(MOVEMENTFLAG_ROOT)) {
    if (IsLevitating()) {
      SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_ANIM_TIER,
                   UNIT_BYTE1_FLAG_FLY);
      return;
...
void Creature::RefreshSwimmingFlag(bool recheck) {
  if (!_isMissingSwimmingFlagOutOfCombat || recheck)
    _isMissingSwimmingFlagOutOfCombat =
        !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SWIMMING);
...
bool Creature::SetCanFly(bool enable, bool /*packetOnly*/ /* = false */) {
  ...
  WorldPacket data(
      enable ? SMSG_SPLINE_MOVE_SET_FLYING : SMSG_SPLINE_MOVE_UNSET_FLYING, 9);
  data.append(GetPackGUID());
  SendMessageToSet(&data, false);
  return true;
}
...
bool Creature::SetFeatherFall(bool enable, bool packetOnly /* = false */) {
  ...
  WorldPacket data(
      enable ? SMSG_SPLINE_MOVE_FEATHER_FALL : SMSG_SPLINE_MOVE_NORMAL_FALL, 9);
  data.append(GetPackGUID());
  SendMessageToSet(&data, true);
  return true;
}
...
void Creature::SetObjectScale(float scale) {
  ...
  SetFloatValue(UNIT_FIELD_COMBATREACH,
                combatReach * GetFloatValue(OBJECT_FIELD_SCALE_X) * scale);
}
...
void Creature::SetDisplayId(uint32 modelId) {
  ...
  if (CreatureModelInfo const *minfo =
    ...
    SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS,
                  (IsPet() ? 1.0f : minfo->bounding_radius) * GetObjectScale());
    if (minfo->combat_reach > 0)
      combatReach = minfo->combat_reach;
  }
...
void Creature::FocusTarget(Spell const *focusSpell, WorldObject const *target) {
  ...
  if (this == target && (movespline->Finalized() ||
                         GetMotionMaster()->GetCurrentMovementGeneratorType() ==
                             CHASE_MOTION_TYPE))
...
float Creature::GetAttackDistance(Unit const *player) const {
  ...
  // "The maximum Aggro Radius has a cap of 25 levels under. Example: A level 30
  // char has the same Aggro Radius of a level 5 char on a level 60 mob."
  if (levelDiff < -25)
    levelDiff = -25;
  ...
  if (creatureLevel + 5 <= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
    // detect range auras
    retDistance +=
        static_cast<float>(GetTotalAuraModifier(SPELL_AURA_MOD_DETECT_RANGE));
...
bool Creature::IsMovementPreventedByCasting() const {
  ...
  // first check if currently a movement allowed channel is active and we're not
  // casting
  if (!!spell && spell->getState() != SPELL_STATE_FINISHED &&
      spell->IsChannelActive() &&
      spell->GetSpellInfo()->IsMoveAllowedChannel()) {
    return false;
  }
...
void Creature::SetCannotReachTarget(bool cannotReach) {
  ...
  if (cannotReach)
    LOG_DEBUG("entities.unit",
              "Creature::SetCannotReachTarget() called with true. Details: %s",
              GetDebugInfo().c_str());
}
