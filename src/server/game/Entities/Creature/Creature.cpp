...
void Creature::SetInCombatWithZone()
{
    ...
    {
        LOG_ERROR("entities.unit", "Creature entry {} call SetInCombatWithZone but creature cannot have threat list.", GetEntry());
        return;
    }
    ...
    if (!map->IsDungeon())
    {
        LOG_ERROR("entities.unit", "Creature entry {} call SetInCombatWithZone for map (id: {}) that isn't an instance.", GetEntry(), map->GetId());
        return;
    }
...
void Creature::ProhibitSpellSchool(SpellSchoolMask idSchoolMask, uint32 unTimeMs)
{
    for (uint8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; ++i)
        if (idSchoolMask & (1 << i))
            m_ProhibitSchoolTime[i] = getMSTime() + unTimeMs;
}
...
bool Creature::IsSpellProhibited(SpellSchoolMask idSchoolMask) const
{
    ...
    for (uint8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; ++i)
        if (idSchoolMask & (1 << i))
            if (m_ProhibitSchoolTime[i] >= getMSTime())
                return true;
...
{
    m_CreatureSpellCooldowns[spell_id] = getMSTime() + end_time;
}
...
uint32 Creature::GetSpellCooldown(uint32 spell_id) const
{
    ...
    return itr->second > getMSTime() ? itr->second - getMSTime() : 0;
}

bool Creature::HasSpellCooldown(uint32 spell_id) const
{
    CreatureSpellCooldowns::const_iterator itr = m_CreatureSpellCooldowns.find(spell_id);
    return (itr != m_CreatureSpellCooldowns.end() && itr->second > getMSTime());
}
...
float Creature::GetAttackDistance(Unit const* player) const
{
    ...
    if (creatureLevel + 5 <= CONF_GET_UINT("MaxPlayerLevel"))
    {
        // detect range auras
