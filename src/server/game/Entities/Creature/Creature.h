...
enum CreatureFlagsExtra : uint32 {
  ...
  CREATURE_FLAG_EXTRA_UNUSED_10 =
      0x00000200, // TODO: Implement CREATURE_FLAG_EXTRA_NO_MOVE_FLAGS_UPDATE
                  // (creature won't update movement flags)
  CREATURE_FLAG_EXTRA_GHOST_VISIBILITY =
      0x00000400, // creature will be only visible for dead players
  ...
  CREATURE_FLAG_EXTRA_TAUNT_DIMINISH =
      0x00080000, // Taunt is a subject to diminishing returns on this creautre
  CREATURE_FLAG_EXTRA_ALL_DIMINISH =
      0x00100000, // Creature is subject to all diminishing returns as player
                  // are
  ...
  CREATURE_FLAG_EXTRA_DUNGEON_BOSS =
      0x10000000, // creature is a dungeon boss (SET DYNAMICALLY, DO NOT ADD IN
                  // DB)
  ...
  CREATURE_FLAG_EXTRA_UNUSED =
      (CREATURE_FLAG_EXTRA_UNUSED_10 | CREATURE_FLAG_EXTRA_UNUSED_12 |
       CREATURE_FLAG_EXTRA_UNUSED_14 | CREATURE_FLAG_EXTRA_UNUSED_17 |
       CREATURE_FLAG_EXTRA_UNUSED_22 | CREATURE_FLAG_EXTRA_UNUSED_25 |
       CREATURE_FLAG_EXTRA_UNUSED_26 | CREATURE_FLAG_EXTRA_UNUSED_27 |
       CREATURE_FLAG_EXTRA_UNUSED_28 | CREATURE_FLAG_EXTRA_UNUSED_32),
  CREATURE_FLAG_EXTRA_DB_ALLOWED =
      (0xFFFFFFFF &
       ~(CREATURE_FLAG_EXTRA_UNUSED | CREATURE_FLAG_EXTRA_DUNGEON_BOSS))
};
...
struct CreatureTemplate {
  ...
  float RangeVariance;
  uint32
      unit_class; // enum Classes. Note only 4 classes are known for creatures.
  ...
  [[nodiscard]] bool IsTameable(bool exotic) const {
    if (type != CREATURE_TYPE_BEAST || family == 0 ||
        (type_flags & CREATURE_TYPE_FLAG_TAMEABLE_PET) == 0)
      return false;
...
struct TrainerSpellData {
  ...
  // req. for correct show non-prof. trainers like weaponmaster, allowed values
  // 0 and 2.
...
public:
  ...
  void GetRespawnPosition(float &x, float &y, float &z, float *ori = nullptr,
  ...
  [[nodiscard]] bool IsValidTrainerForPlayer(Player *player,
  ...
  struct {
    ...
    uint32 Delay = 0; // ms until the creature's target should snap back (0 = no
                      // snapback scheduled)
  ...
  [[nodiscard]] bool CanGeneratePickPocketLoot() const {
    return lootPickPocketRestoreTime == 0 ||
           lootPickPocketRestoreTime < time(nullptr);
  }
  void SetPickPocketLootTime() {
    lootPickPocketRestoreTime =
        time(nullptr) + MINUTE + GetCorpseDelay() + GetRespawnTime();
  }
  ...
  void SetPosition(const Position &pos) {
    SetPosition(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(),
                pos.GetOrientation());
  }
  ...
  void GetTransportHomePosition(float &x, float &y, float &z,
                                float &ori) const {
  ...
  void LowerPlayerDamageReq(uint32 unDamage) {
    if (m_PlayerDamageReq)
      m_PlayerDamageReq > unDamage ? m_PlayerDamageReq -= unDamage
                                   : m_PlayerDamageReq = 0;
  }
...
protected:
  ...
  uint32 m_DBTableGuid; ///< For new or temporary creatures is 0 for saved it is
                        ///< lowguid
  uint8 m_equipmentId;
  ...
  uint16 m_LootMode; // bitmask, default LOOT_MODE_DEFAULT, determines what loot
                     // will be lootable
