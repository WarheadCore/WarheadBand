  ...
  for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    if (itr->second.Passenger.Guid) {
      if (Unit *unit = ObjectAccessor::FindUnit(itr->second.Passenger.Guid)) {
        LOG_INFO("server",
                 "ZOMG! ~Vehicle(), unit: %s, entry: %u, typeid: %u, "
                 "this_entry: %u, this_typeid: %u!",
                 unit->GetName().c_str(), unit->GetEntry(), unit->GetTypeId(),
                 _me ? _me->GetEntry() : 0, _me ? _me->GetTypeId() : 0);
        unit->_ExitVehicle();
      } else
...
void Vehicle::InstallAllAccessories(bool evading) {
  ...
  for (VehicleAccessoryList::const_iterator itr = accessories->begin();
       itr != accessories->end(); ++itr)
    if (!evading || itr->IsMinion) // only install minions on evade mode
      InstallAccessory(itr->AccessoryEntry, itr->SeatId, itr->IsMinion,
                       itr->SummonedType, itr->SummonTime);
}
...
void Vehicle::Uninstall() {
  ...
  if (_status == STATUS_UNINSTALLING &&
    ...
    LOG_ERROR("server",
              "Vehicle GuidLow: %u, Entry: %u attempts to uninstall, but "
              "already has STATUS_UNINSTALLING! "
  ...
  LOG_DEBUG("vehicles", "Vehicle::Uninstall Entry: %u, GuidLow: %u",
            _creatureEntry, _me->GetGUIDLow());
#endif
  RemoveAllPassengers();
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("vehicles", "Vehicle::Reset Entry: %u, GuidLow: %u", _creatureEntry,
            _me->GetGUIDLow());
#endif
...
void Vehicle::ApplyAllImmunities() {
  ...
  // immunities in DB ) should be also immune on healing ( exceptions in switch
  // below )
  if (_me->ToCreature() &&
    ...
    _me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_HEAL_MAX_HEALTH,
                          true); // Xinef
  ...
  case 543: // Isle of Conquest
    ...
    _me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_DECREASE_SPEED,
                          true);
    break;
  ...
  case 336:
  case 338:
    _me->ApplySpellImmune(0, IMMUNITY_STATE,
                          SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, false);
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("vehicles", "Vehicle::RemoveAllPassengers. Entry: %u, GuidLow: %u",
            _creatureEntry, _me->GetGUIDLow());
#endif
...
void Vehicle::InstallAccessory(uint32 entry, int8 seatId, bool minion,
                               uint8 type, uint32 summonTime) {
  /// @Prevent adding accessories when vehicle is uninstalling. (Bad script in
  /// OnUninstall/OnRemovePassenger/PassengerBoarded hook.)
  if (_status == STATUS_UNINSTALLING) {
    LOG_ERROR("server",
              "Vehicle GuidLow: %u, Entry: %u attempts to install accessory "
              "Entry: %u on seat %d with STATUS_UNINSTALLING! "
              "Check Uninstall/PassengerBoarded script hooks for errors.",
  ...
  LOG_DEBUG(
      "vehicles",
      "Vehicle: Installing accessory entry %u on vehicle entry %u (seat:%i)",
      entry, GetCreatureEntry(), seatId);
#endif
  ...
  if (TempSummon *accessory =
          _me->SummonCreature(entry, *_me, TempSummonType(type), summonTime)) {
    if (minion)
...
bool Vehicle::AddPassenger(Unit *unit, int8 seatId) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
    LOG_DEBUG("vehicles",
              "Passenger GuidLow: %u, Entry: %u, attempting to board vehicle "
              "GuidLow: %u, Entry: %u during uninstall! SeatId: %i",
              unit->GetGUIDLow(), unit->GetEntry(), _me->GetGUIDLow(),
              _me->GetEntry(), (int32)seatId);
#endif
    return false;
  }
  ...
  if (seatId < 0) // no specific seat requirement
  {
    ...
    if (!seat->second.IsEmpty()) {
      if (Unit *passenger =
              ObjectAccessor::GetUnit(*GetBase(), seat->second.Passenger.Guid))
  ...
  LOG_DEBUG("vehicles",
            ...
            _me->GetGUIDLow(), (int32)seat->first);
#endif
  ...
  if (_me->GetTypeId() == TYPEID_UNIT && unit->GetTypeId() == TYPEID_PLAYER &&
    ...
    try {
      ...
      LOG_INFO("server",
               "ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). not null: %u",
               _me ? 1 : 0);
      if (!_me)
        return false;
      LOG_INFO("server",
               "ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). Is: %u!",
               _me->IsInWorld());
      LOG_INFO("server",
               "ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). Is2: %u!",
               _me->IsDuringRemoveFromWorld());
      LOG_INFO("server",
               "ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). Unit %s!",
               _me->GetName().c_str());
      LOG_INFO("server",
               "ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). typeid: %u!",
               _me->GetTypeId());
      LOG_INFO(
          "server",
          "ZOMG! CRASH! Try-catch in Unit::SetCharmedBy(). Unit %s, typeid: "
          "%u, in world: %u, duringremove: %u has wrong CharmType! Charmer %s, "
          "typeid: %u, in world: %u, duringremove: %u.",
          _me->GetName().c_str(), _me->GetTypeId(), _me->IsInWorld(),
          _me->IsDuringRemoveFromWorld(), unit->GetName().c_str(),
          unit->GetTypeId(), unit->IsInWorld(),
  ...
  if (_me->IsInWorld()) {
    ...
    unit->SetControlled(
        true, UNIT_STATE_ROOT); // SMSG_FORCE_ROOT - In some cases we send
                                // SMSG_SPLINE_MOVE_ROOT here (for creatures)
    // also adds MOVEMENTFLAG_ROOT
    Movement::MoveSplineInit init(unit);
    init.DisableTransportPathTransformations();
    init.MoveTo(veSeat->m_attachmentOffsetX, veSeat->m_attachmentOffsetY,
                veSeat->m_attachmentOffsetZ);
    // Xinef: did not found anything unique in dbc, maybe missed something
    if (veSeat->m_ID == 3566 || veSeat->m_ID == 3567 || veSeat->m_ID == 3568 ||
        veSeat->m_ID == 3570) {
      float x = veSeat->m_attachmentOffsetX, y = veSeat->m_attachmentOffsetY,
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("vehicles", "Unit %s exit vehicle entry %u id %u dbguid %u seat %d",
            unit->GetName().c_str(), _me->GetEntry(), _vehicleInfo->m_ID,
            _me->GetGUIDLow(), (int32)seat->first);
#endif

  if (seat->second.SeatInfo->CanEnterOrExit() && ++_usableSeatNum)
    _me->SetFlag(UNIT_NPC_FLAGS, (_me->GetTypeId() == TYPEID_PLAYER
...
void Vehicle::RelocatePassengers() {
  ...
  for (auto const &itr : Seats) {
    if (Unit *passenger =
            ObjectAccessor::GetUnit(*GetBase(), itr.second.Passenger.Guid)) {
...
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
  LOG_DEBUG("vehicles", "Vehicle::Dismiss Entry: %u, GuidLow %u",
            _creatureEntry, _me->GetGUIDLow());
#endif
  Uninstall();
...
uint8 Vehicle::GetAvailableSeatCount() const {
  ...
  for (itr = Seats.begin(); itr != Seats.end(); ++itr)
    if (itr->second.IsEmpty() && (itr->second.SeatInfo->CanEnterOrExit() ||
