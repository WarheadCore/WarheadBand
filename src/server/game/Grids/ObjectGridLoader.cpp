...
template <class T>
void AddObjectHelper(CellCoord &cell, GridRefManager<T> &m, uint32 &count,
                     Map * /*map*/, T *obj) {
  obj->AddToGrid(m);
...
template <>
void AddObjectHelper(CellCoord &cell, CreatureMapType &m, uint32 &count,
                     Map *map, Creature *obj) {
  obj->AddToGrid(m);
  ObjectGridLoader::SetObjectCell(obj, cell);
  obj->AddToWorld();
...
template <>
void AddObjectHelper(CellCoord &cell, GameObjectMapType &m, uint32 &count,
                     Map *map, GameObject *obj) {
  obj->AddToGrid(m);
  ObjectGridLoader::SetObjectCell(obj, cell);
  obj->AddToWorld();
  if (obj->isActiveObject())
...
void LoadHelper(CellGuidSet const &guid_set, CellCoord &cell,
  ...
  for (CellGuidSet::const_iterator i_guid = guid_set.begin();
    ...
    T *obj = new T;
    uint32 guid = *i_guid;
    // LOG_INFO("server", "DEBUG: LoadHelper from table: %s for (guid: %u)
    // Loading", table, guid);
...
template <>
void LoadHelper(CellGuidSet const &guid_set, CellCoord &cell,
                GridRefManager<GameObject> &m, uint32 &count, Map *map) {
  for (CellGuidSet::const_iterator i_guid = guid_set.begin();
       i_guid != guid_set.end(); ++i_guid) {
    uint32 guid = *i_guid;
...
void LoadHelper(CellCorpseSet const &cell_corpses, CellCoord &cell,
...
void ObjectWorldLoader::Visit(CorpseMapType &m) {
  ...
  CellObjectGuids const &cell_guids =
      sObjectMgr->GetCellObjectGuids(i_map->GetId(), 0, cellCoord.GetId());
...
void ObjectGridLoader::LoadN(void) {
  ...
  for (unsigned int x = 0; x < MAX_NUMBER_OF_CELLS; ++x) {
    ...
    for (unsigned int y = 0; y < MAX_NUMBER_OF_CELLS; ++y) {
      ...
      {
        TypeContainerVisitor<ObjectGridLoader, GridTypeMapContainer> visitor(
            *this);
      ...
      // Load corpses (not bones)
      {
        ...
        TypeContainerVisitor<ObjectWorldLoader, WorldTypeMapContainer> visitor(
            worker);
        i_grid.VisitGrid(x, y, visitor);
  ...
  LOG_DEBUG("maps",
            "%u GameObjects, %u Creatures, and %u Corpses/Bones loaded for "
            "grid %u on map %u",
...
template <class T> void ObjectGridCleaner::Visit(GridRefManager<T> &m) {
