...
public:
  DelayedCorpseAction(Corpse *corpse, uint8 action, uint32 mapId,
                      uint32 instanceId)
      : _corpse(corpse), _action(action), _mapId(mapId),
...
public:
  ...
  // TODO: override these template functions for each holder type and add
  // assertions
  ...
  template <class T>
  static T *GetObjectInWorld(uint32 mapid, float x, float y, uint64 guid,
                             T * /*fake*/);

  // these functions return objects only if in map of specified object
  static WorldObject *GetWorldObject(WorldObject const &, uint64);
  static Object *GetObjectByTypeMask(WorldObject const &, uint64,
                                     uint32 typemask);
  ...
  static Player *FindPlayerByName(std::string const &name,
                                  bool checkInWorld = true);
  static std::map<std::string, Player *>
      playerNameToPlayerPointer; // pussywizard: optimization
  ...
  void AddUpdateObject(Object *obj) {
    ...
    if (obj->GetTypeId() < TYPEID_UNIT) // these are not in map: TYPEID_OBJECT,
                                        // TYPEID_ITEM, TYPEID_CONTAINER
  ...
  void AddDelayedCorpseAction(Corpse *corpse, uint8 action, uint32 mapId = 0,
                              uint32 instanceId = 0);
...
private:
  ...
  typedef std::unordered_map<Player *, UpdateData>::value_type
      UpdateDataValueType;
