...
struct TempSummonGroupKey {
  TempSummonGroupKey(uint32 summonerEntry, SummonerType summonerType,
                     uint8 group)
...
private:
  ...
  SummonerType
      _summonerType;  ///< Summoner's type, see SummonerType for available types
...
enum ScriptCommands {
  SCRIPT_COMMAND_TALK =
      0, // source/target = Creature, target = any, datalong = talk type (0=say,
         // 1=whisper, 2=yell, 3=emote text, 4=boss emote text), datalong2 & 1 =
         // player talk (instead of creature), dataint = string_id
  ...
  SCRIPT_COMMAND_TELEPORT_TO =
      6, // source/target = Creature/Player (see datalong2), datalong = map_id,
         // datalong2 = 0: Player; 1: Creature, x/y/z = destination, o =
         // orientation
  ...
  SCRIPT_COMMAND_TEMP_SUMMON_CREATURE =
      10, // source = WorldObject (summoner), datalong = creature entry,
          // datalong2 = despawn delay, x/y/z = summon position, o = orientation
  SCRIPT_COMMAND_OPEN_DOOR =
      11, // source = Unit, datalong = GO guid, datalong2 = reset delay (min 15)
  SCRIPT_COMMAND_CLOSE_DOOR =
      12, // source = Unit, datalong = GO guid, datalong2 = reset delay (min 15)
  SCRIPT_COMMAND_ACTIVATE_OBJECT = 13, // source = Unit, target = GO
  SCRIPT_COMMAND_REMOVE_AURA =
      14, // source (datalong2 != 0) or target (datalong2 == 0) = Unit, datalong
          // = spell id
  SCRIPT_COMMAND_CAST_SPELL =
      15, // source and/or target = Unit, datalong2 = cast direction (0: s->t 1:
          // s->s 2: t->t 3: t->s 4: s->creature with dataint entry), dataint &
          // 1 = triggered flag
  ...
  SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT =
      21, // source = WorldObject (if present used as a search center), datalong
          // = script id, datalong2 = unit lowguid, dataint = script table to
          // use (see ScriptsType)
  SCRIPT_COMMAND_KILL =
...
struct BroadcastText {
  ...
  GetText(LocaleConstant locale = DEFAULT_LOCALE, uint8 gender = GENDER_MALE,
...
typedef std::unordered_map<uint32 /*player guid*/, uint32 /*instance*/>
    CellCorpseSet;
...
struct QuestPOI {
  ...
  QuestPOI(uint32 id, int32 objIndex, uint32 mapId, uint32 areaId,
           uint32 floorId, uint32 unk3, uint32 unk4)
...
public:
  ...
  typedef std::unordered_map<uint32, AccessRequirement *>
  ...
  int LoadReferenceVendor(int32 vendor, int32 item_id,
  ...
  void GetPlayerClassLevelInfo(uint32 class_, uint8 level,
                               PlayerClassLevelInfo *info) const;

  [[nodiscard]] PlayerInfo const *GetPlayerInfo(uint32 race,
                                                uint32 class_) const;

  void GetPlayerLevelInfo(uint32 race, uint32 class_, uint8 level,
  ...
  [[nodiscard]] uint32
  GetPlayerAccountIdByPlayerName(std::string const &name) const;

  uint32 GetNearestTaxiNode(float x, float y, float z, uint32 mapid,
                            uint32 teamId);
  void GetTaxiPath(uint32 source, uint32 destination, uint32 &path,
                   uint32 &cost);
  uint32 GetTaxiMountDisplayId(uint32 id, TeamId teamId,
  ...
  [[nodiscard]] Quest const *GetQuestTemplate(uint32 quest_id) const {
    return quest_id < _questTemplatesFast.size() ? _questTemplatesFast[quest_id]
                                                 : nullptr;
  }
  ...
  [[nodiscard]] bool IsTavernAreaTrigger(uint32 Trigger_ID) const {
    return _tavernAreaTriggerStore.find(Trigger_ID) !=
           _tavernAreaTriggerStore.end();
  }
  ...
  [[nodiscard]] AccessRequirement const *
  GetAccessRequirement(uint32 mapid, Difficulty difficulty) const {
    AccessRequirementContainer::const_iterator itr =
        _accessRequirementStore.find(MAKE_PAIR32(mapid, difficulty));
  ...
  int32 GetBaseReputationOf(FactionEntry const *factionEntry, uint8 race,
                            uint8 playerClass);
  ...
  void AddSpellToTrainer(uint32 entry, uint32 spell, uint32 spellCost,
  ...
  CellObjectGuids const &GetCellObjectGuids(uint16 mapid, uint8 spawnMode,
                                            uint32 cell_id) {
   ...
   * group data
   */
  [[nodiscard]] std::vector<TempSummonData> const *
  GetSummonGroup(uint32 summonerId, SummonerType summonerType,
                 uint8 group) const {
  ...
  CreatureData &NewOrExistCreatureData(uint32 guid) {
    return _creatureDataStore[guid];
  }
  ...
  [[nodiscard]] GameObjectData const *GetGOData(uint32 guid) const {
    GameObjectDataContainer::const_iterator itr =
        _gameObjectDataStore.find(guid);
  ...
  void AddCorpseCellData(uint32 mapid, uint32 cellid, uint32 player_guid,
                         uint32 instance);
  ...
  uint32 AddGOData(uint32 entry, uint32 map, float x, float y, float z, float o,
                   uint32 spawntimedelay = 0, float rotation0 = 0,
                   float rotation1 = 0, float rotation2 = 0,
                   float rotation3 = 0);
  uint32 AddCreData(uint32 entry, uint32 map, float x, float y, float z,
                    float o, uint32 spawntimedelay = 0);
  ...
  void AddVendorItem(uint32 entry, uint32 item, int32 maxcount, uint32 incrtime,
                     uint32 extendedCost, bool persist = true); // for event
  bool RemoveVendorItem(uint32 entry, uint32 item,
                        bool persist = true); // for event
  bool IsVendorItemValid(uint32 vendor_entry, uint32 item, int32 maxcount,
                         uint32 ptime, uint32 ExtendedCost,
  ...
  [[nodiscard]] const char *GetScriptName(uint32 id) const {
    return id < _scriptNamesStore.size() ? _scriptNamesStore[id].c_str() : "";
  }
  ...
  static void AddLocaleString(std::string const &s, LocaleConstant locale,
                              StringVector &data);
  static inline void GetLocaleString(const StringVector &data, int loc_idx,
                                     std::string &value) {
...
private:
  ...
  void PlayerCreateInfoAddItemHelper(uint32 race_, uint32 class_, uint32 itemId,
                                     int32 count);
  ...
  void BuildPlayerLevelInfo(uint8 race, uint8 class_, uint8 level,
