...
Player *ObjectAccessor::FindPlayerByName(std::string const &name,
  ...
  *HashMapHolder<Player>::GetLock()); std::string nameStr = name;
  std::transform(nameStr.begin(), nameStr.end(), nameStr.begin(), ::tolower);
  HashMapHolder<Player>::MapType const& m = GetPlayers();
  for (HashMapHolder<Player>::MapType::const_iterator iter = m.begin(); iter !=
  m.end(); ++iter)
  {
  ...
  currentName.begin(), ::tolower); if (nameStr.compare(currentName) == 0) return
  iter->second;
  }*/
  ...
  std::map<std::string, Player *>::iterator itr =
      playerNameToPlayerPointer.find(nameStr);
...
void ObjectAccessor::RemoveCorpse(Corpse *corpse, bool final) {
  ...
  if (!final) {
    ...
    Player2CorpsesMapType::iterator iter =
        i_player2corpse.find(corpse->GetOwnerGUID());
    if (iter == i_player2corpse.end())
      return;
  ...
  // Critical section
  {
    ...
    CellCoord cellCoord = Warhead::ComputeCellCoord(corpse->GetPositionX(),
                                                    corpse->GetPositionY());
    sObjectMgr->DeleteCorpseCellData(corpse->GetMapId(), cellCoord.GetId(),
                                     GUID_LOPART(corpse->GetOwnerGUID()));
  }

  delete corpse; // pussywizard: as it is delayed now, delete is moved here
                 // (previously in ConvertCorpseForPlayer)
}
...
void ObjectAccessor::AddCorpse(Corpse *corpse) {
  ...
  // Critical section
  {
    ...
    ASSERT(i_player2corpse.find(corpse->GetOwnerGUID()) ==
           i_player2corpse.end());
    ...
    CellCoord cellCoord = Warhead::ComputeCellCoord(corpse->GetPositionX(),
                                                    corpse->GetPositionY());
    sObjectMgr->AddCorpseCellData(corpse->GetMapId(), cellCoord.GetId(),
                                  GUID_LOPART(corpse->GetOwnerGUID()),
                                  corpse->GetInstanceId());
  }
}

void ObjectAccessor::AddCorpsesToGrid(GridCoord const &gridpair, GridType &grid,
                                      Map *map) {
  ACORE_READ_GUARD(ACE_RW_Thread_Mutex, i_corpseLock);
...
Corpse *ObjectAccessor::ConvertCorpseForPlayer(uint64 player_guid,
  ...
  if (map && corpse->IsPositionValid() && inWorld &&
      (insignia || (map->IsBattlegroundOrArena()
    ...
    // bones->m_time = m_time;                              // don't overwrite
    // time bones->m_type = m_type;                              // don't
    // overwrite type
    ...
    if (insignia) // pussywizard: in case of insignia we need bones right now,
                  // map is the same so not a problem
      map->AddToMap(bones);
    else {
      bones->ResetMap();
      sObjectAccessor->AddDelayedCorpseAction(bones, 1, map->GetId(),
                                              map->GetInstanceId());
    }
...
void ObjectAccessor::RemoveOldCorpses() {
  ...
  ACORE_WRITE_GUARD(ACE_RW_Thread_Mutex, i_corpseLock);
  for (std::list<uint64>::iterator itr = i_playerBones.begin();
       itr != i_playerBones.end(); itr = next2) {
    next2 = itr;
    ++next2;
...
void ObjectAccessor::AddDelayedCorpseAction(Corpse *corpse, uint8 action,
                                            uint32 mapId, uint32 instanceId) {
  ACORE_GUARD(ACE_Thread_Mutex, DelayedCorpseLock);
  i_delayedCorpseActions.push_back(
      DelayedCorpseAction(corpse, action, mapId, instanceId));
}

void ObjectAccessor::ProcessDelayedCorpseActions() {
  ACORE_GUARD(ACE_Thread_Mutex, DelayedCorpseLock);
  for (std::list<DelayedCorpseAction>::iterator itr =
           i_delayedCorpseActions.begin();
       itr != i_delayedCorpseActions.end(); ++itr) {
    ...
    case 1: // add bones
      if (Map *map = sMapMgr->FindMap(a._mapId, a._instanceId))
        if (!map->IsRemovalGrid(a._corpse->GetPositionX(),
                                a._corpse->GetPositionY())) {
...
template <class T>
/*static*/ T *ObjectAccessor::GetObjectInWorld(uint32 mapid, float x, float y,
                                               uint64 guid, T * /*fake*/) {
  ...
  if (!p.IsCoordValid()) {
    LOG_ERROR("server",
              "ObjectAccessor::GetObjectInWorld: invalid coordinates supplied "
              "X:%f Y:%f grid cell [%u:%u]",
              x, y, p.x_coord, p.y_coord);
    return nullptr;
  }
  ...
  if (!q.IsCoordValid()) {
    LOG_ERROR("server",
              "ObjectAccessor::GetObjecInWorld: object (GUID: %u TypeId: %u) "
              "has invalid coordinates X:%f Y:%f grid cell [%u:%u]",
              obj->GetGUIDLow(), obj->GetTypeId(), obj->GetPositionX(),
              obj->GetPositionY(), q.x_coord, q.y_coord);
    return nullptr;
  }
...
template Pet *ObjectAccessor::GetObjectInWorld<Pet>(uint32 mapid, float x,
                                                    ...
                                                    Pet * /*fake*/);
template Creature *
ObjectAccessor::GetObjectInWorld<Creature>(uint32 mapid, float x, float y,
                                           uint64 guid, Creature * /*fake*/);
template Corpse *ObjectAccessor::GetObjectInWorld<Corpse>(uint32 mapid, float x,
                                                          float y, uint64 guid,
                                                          Corpse * /*fake*/);
template GameObject *ObjectAccessor::GetObjectInWorld<GameObject>(
    uint32 mapid, float x, float y, uint64 guid, GameObject * /*fake*/);
