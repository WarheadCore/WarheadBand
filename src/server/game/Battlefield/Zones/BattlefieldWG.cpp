...
bool BattlefieldWG::SetupBattlefield() {
  ...
  // value?...
  InitStalker(BATTLEFIELD_WG_NPC_STALKER, WintergraspStalkerPos[0],
              WintergraspStalkerPos[1], WintergraspStalkerPos[2],
  ...
  for (uint8 i = 0; i < WG_MAX_KEEP_NPC; i++) {
    ...
    if (Creature *creature = SpawnCreature(
            WGKeepNPC[i].entryHorde, WGKeepNPC[i].x, WGKeepNPC[i].y,
            WGKeepNPC[i].z, WGKeepNPC[i].o, TEAM_HORDE))
      KeepCreature[TEAM_HORDE].insert(creature->GetGUID());
    // Alliance npc
    if (Creature *creature = SpawnCreature(
            WGKeepNPC[i].entryAlliance, WGKeepNPC[i].x, WGKeepNPC[i].y,
            WGKeepNPC[i].z, WGKeepNPC[i].o, TEAM_ALLIANCE))
      KeepCreature[TEAM_ALLIANCE].insert(creature->GetGUID());
  }

  // Hide NPCs from the Attacker's team in the keep
  for (GuidSet::const_iterator itr = KeepCreature[GetAttackerTeam()].begin();
       itr != KeepCreature[GetAttackerTeam()].end(); ++itr)
  ...
  for (uint8 i = 0; i < WG_OUTSIDE_ALLIANCE_NPC; i++)
    if (Creature *creature = SpawnCreature(
            WGOutsideNPC[i].entryHorde, WGOutsideNPC[i].x, WGOutsideNPC[i].y,
            WGOutsideNPC[i].z, WGOutsideNPC[i].o, TEAM_HORDE))
  ...
  for (uint8 i = WG_OUTSIDE_ALLIANCE_NPC; i < WG_MAX_OUTSIDE_NPC; i++)
    if (Creature *creature = SpawnCreature(
            WGOutsideNPC[i].entryAlliance, WGOutsideNPC[i].x, WGOutsideNPC[i].y,
            WGOutsideNPC[i].z, WGOutsideNPC[i].o, TEAM_ALLIANCE))
      OutsideCreature[TEAM_ALLIANCE].insert(creature->GetGUID());

  // Hide units outside the keep that are defenders
  for (GuidSet::const_iterator itr = OutsideCreature[GetDefenderTeam()].begin();
       itr != OutsideCreature[GetDefenderTeam()].end(); ++itr)
    if (Unit *unit = ObjectAccessor::FindUnit(*itr))
      if (Creature *creature = unit->ToCreature())
        HideNpc(creature);
  ...
  for (uint8 i = 0; i < WG_MAX_TURRET; i++) {
    ...
    if (Creature *creature = SpawnCreature(NPC_WINTERGRASP_TOWER_CANNON,
                                           towerCannonPos, TEAM_ALLIANCE)) {
  ...
  for (uint8 i = 0; i < WG_MAX_OBJ; i++) {
    GameObject *go =
        SpawnGameObject(WGGameObjectBuilding[i].entry,
                        WGGameObjectBuilding[i].x, WGGameObjectBuilding[i].y,
                        WGGameObjectBuilding[i].z, WGGameObjectBuilding[i].o);
    BfWGGameObjectBuilding *b = new BfWGGameObjectBuilding(this);
    b->Init(go, WGGameObjectBuilding[i].type,
  ...
  for (uint8 i = 0; i < WG_MAX_TELEPORTER; i++) {
    GameObject *go =
        SpawnGameObject(WGPortalDefenderData[i].entry,
                        WGPortalDefenderData[i].x, WGPortalDefenderData[i].y,
                        WGPortalDefenderData[i].z, WGPortalDefenderData[i].o);
...
void BattlefieldWG::OnBattleStart() {
  // Spawn titan relic
  GameObject *go = SpawnGameObject(GO_WINTERGRASP_TITAN_S_RELIC, 5440.0f,
                                   2840.8f, 430.43f, 0);
  if (go) {
  ...
  for (GuidSet::const_iterator itr = CanonList.begin(); itr != CanonList.end();
       ++itr) {
    if (Unit *unit = ObjectAccessor::FindUnit(*itr)) {
      if (Creature *creature = unit->ToCreature()) {
  ...
  for (uint8 team = 0; team < 2; ++team)
    for (GuidSet::const_iterator itr = m_players[team].begin();
...
void BattlefieldWG::UpdateVehicleCountWG() {
  for (uint8 i = 0; i < BG_TEAMS_COUNT; ++i)
    for (GuidSet::iterator itr = m_players[i].begin();
         itr != m_players[i].end(); ++itr)
      if (Player *player = ObjectAccessor::FindPlayer(*itr)) {
        player->SendUpdateWorldState(BATTLEFIELD_WG_WORLD_STATE_VEHICLE_H,
        ...
        player->SendUpdateWorldState(BATTLEFIELD_WG_WORLD_STATE_VEHICLE_A,
...
void BattlefieldWG::OnBattleEnd(bool endByTimer) {
  ...
  for (GuidSet::const_iterator itr = KeepCreature[GetDefenderTeam()].begin();
       itr != KeepCreature[GetDefenderTeam()].end(); ++itr)
    if (Unit *unit = ObjectAccessor::FindUnit(*itr))
      if (Creature *creature = unit->ToCreature())
        ShowNpc(creature, true);
  ...
  for (GuidSet::const_iterator itr = OutsideCreature[GetAttackerTeam()].begin();
       itr != OutsideCreature[GetAttackerTeam()].end(); ++itr)
    if (Unit *unit = ObjectAccessor::FindUnit(*itr))
      if (Creature *creature = unit->ToCreature())
        ShowNpc(creature, true);
  ...
  for (uint8 team = 0; team < 2; ++team) {
    for (GuidSet::const_iterator itr = m_vehicles[team].begin();
         itr != m_vehicles[team].end(); ++itr)
      if (Unit *unit = ObjectAccessor::FindUnit(*itr))
        if (Creature *creature = unit->ToCreature())
  ...
  for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin();
       itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr) {
    if (Player *player = ObjectAccessor::FindPlayer(*itr)) {
      // Victory in Wintergrasp
      player->AreaExploredOrEventHappens(
          GetDefenderTeam() ? 13183 : 13181); // HORDE / ALLY win wg quest id
  ...
  for (GuidSet::const_iterator itr = m_PlayersInWar[GetAttackerTeam()].begin();
       itr != m_PlayersInWar[GetAttackerTeam()].end(); ++itr)
    if (Player *player = ObjectAccessor::FindPlayer(*itr)) {
  ...
  if (!endByTimer) // win alli/horde
    SendWarningToAllInZone((GetDefenderTeam() == TEAM_ALLIANCE)
...
uint8 BattlefieldWG::GetSpiritGraveyardId(uint32 areaId) const {
  ...
  default:
    LOG_ERROR("server",
              "BattlefieldWG::GetSpiritGraveyardId: Unexpected Area Id %u",
              areaId);
    break;
  }
    ...
    if (WGWorkshop *workshop = (*itr)) {
      if (workshop->workshopId == workshopId) {
        WintergraspCapturePoint *capturePoint =
            new WintergraspCapturePoint(this, GetAttackerTeam());
...
void BattlefieldWG::HandleKill(Player *killer, Unit *victim) {
  ...
  if (victim->GetTypeId() == TYPEID_PLAYER || IsKeepNpc(victim->GetEntry()) ||
    ...
    for (GuidSet::const_iterator itr = m_PlayersInWar[killerTeam].begin();
         itr != m_PlayersInWar[killerTeam].end(); ++itr)
      if (Player *player = ObjectAccessor::FindPlayer(*itr))
...
void BattlefieldWG::OnPlayerJoinWar(Player *player) {
  ...
  if (player->GetTeamId() == GetDefenderTeam())
    ...
    if (player->GetTeamId() == TEAM_HORDE)
      player->TeleportTo(571, 5025.857422f, 3674.628906f, 362.737122f,
                         4.135169f);
    else
  ...
  if (player->GetTeamId() == GetAttackerTeam()) {
    if (GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT) < 3)
      player->SetAuraStack(SPELL_TOWER_CONTROL, player,
...
void BattlefieldWG::OnPlayerLeaveWar(Player *player) {
  // Remove all aura from WG // TODO: false we can go out of this zone on retail
  // and keep Rank buff, remove on end of WG
...
void BattlefieldWG::OnPlayerEnterZone(Player *player) {
  ...
  if (player->GetTeamId() == GetAttackerTeam())
    if (player->GetPositionX() > 5400.0f && player->GetPositionX() < 5490.0f &&
        player->GetPositionY() > 2803.0f && player->GetPositionY() < 2878.0f)
...
void BattlefieldWG::SendInitWorldStatesToAll() {
  for (uint8 team = 0; team < 2; team++)
    for (GuidSet::iterator itr = m_players[team].begin();
         itr != m_players[team].end(); ++itr)
      if (Player *player = ObjectAccessor::FindPlayer(*itr))
        SendInitWorldStatesTo(player);
}
    ...
    for (GuidSet::const_iterator itr =
      ...
      if (Player *player = ObjectAccessor::FindPlayer(*itr)) {
        ...
        player->UpdateAchievementCriteria(
            ...
            SPELL_LEANING_TOWER_ACHIEVEMENT, 0, 0);
      }

    // If all three south towers are destroyed (ie. all attack towers), remove
    // ten minutes from battle time
    if (GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT) == 3) {
...
void BattlefieldWG::UpdateTenacity() {
  ...
  if (newStack == m_tenacityStack) {
    for (GuidSet::const_iterator itr = m_updateTenacityList.begin();
         itr != m_updateTenacityList.end(); ++itr)
  ...
  if (team != TEAM_NEUTRAL) {
    for (GuidSet::const_iterator itr = m_PlayersInWar[team].begin();
         itr != m_PlayersInWar[team].end(); ++itr)
      if (Player *player = ObjectAccessor::FindPlayer(*itr))
        player->RemoveAurasDueToSpell(SPELL_TENACITY);
