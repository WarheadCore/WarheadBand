...
void WorldSession::SendBfInvitePlayerToWar(uint32 BattleId, uint32 ZoneId,
                                           uint32 p_time) {
  // Send packet
...
void WorldSession::SendBfQueueInviteResponse(uint32 BattleId, uint32 ZoneId,
                                             bool CanQueue, bool Full) {
  WorldPacket data(SMSG_BATTLEFIELD_MGR_QUEUE_REQUEST_RESPONSE, 11);
  data << uint32(BattleId);
  data << uint32(ZoneId);
  data << uint8((CanQueue ? 1 : 0)); // Accepted          //0 you cannot queue
                                     // wg     //1 you are queued
  data << uint8((Full ? 0 : 1)); // Logging In        //0 wg full //1 queue for
                                 // upcoming
  data << uint8(1); // Warmup
  SendPacket(&data);
}
...
void WorldSession::HandleBfQueueInviteResponse(WorldPacket &recvData) {
  ...
  // LOG_ERROR("server", "HandleQueueInviteResponse: BattleID:%u Accepted:%u",
  // BattleId, Accepted);
...
void WorldSession::HandleBfEntryInviteResponse(WorldPacket &recvData) {
  ...
  recvData >> BattleId >> Accepted;
  // LOG_ERROR("server", "HandleBattlefieldInviteResponse: BattleID:%u
  // Accepted:%u", BattleId, Accepted);
  Battlefield *Bf = sBattlefieldMgr->GetBattlefieldByBattleId(BattleId);
  if (!Bf)
    return;
