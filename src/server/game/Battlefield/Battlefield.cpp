    ...
    if (IsWarTime()) {
      ...
      else // No more vacant places
      {
        ...
        m_PlayersWillBeKick[player->GetTeamId()][player->GetGUID()] =
            time(nullptr) + (player->IsGameMaster() ? 30 * MINUTE : 10);
  ...
  if (m_Timer <= diff) {
    if (!IsEnabled() ||
        (!IsWarTime() && sWorld->GetActiveSessionCount() >
                             3500)) // if WG is disabled or there is more than
                                    // 3500 connections, switch automaticly
    {
      m_isActive = true;
      EndBattle(false);
      return false;
  ...
  if (IsWarTime()) {
    ...
    if (m_uiKickDontAcceptTimer <= diff) {
      ...
      for (int team = 0; team < 2; team++)
        for (PlayerTimerMap::iterator itr = m_InvitedPlayers[team].begin();
             itr != m_InvitedPlayers[team].end(); ++itr)
          if (itr->second <= now)
      ...
      for (int team = 0; team < 2; team++)
        for (PlayerTimerMap::iterator itr = m_PlayersWillBeKick[team].begin();
             itr != m_PlayersWillBeKick[team].end(); ++itr)
          if (itr->second <= now)
            KickPlayerFromBattlefield(itr->first);
      ...
      if (Player *player = ObjectAccessor::FindPlayer(*itr)) {
        ...
        if (m_PlayersInWar[player->GetTeamId()].size() +
          ...
          InvitePlayerToWar(player);
                     ...
                     player->GetGUID()) == 0) // Battlefield is full of players
          m_PlayersWillBeKick[player->GetTeamId()][player->GetGUID()] =
              time(nullptr) + 10;
      }
    }
...
void Battlefield::InvitePlayerToWar(Player *player) {
  ...
  m_InvitedPlayers[player->GetTeamId()][player->GetGUID()] =
      time(nullptr) + m_TimeForAcceptInvite;
  player->GetSession()->SendBfInvitePlayerToWar(m_BattleId, m_ZoneId,
                                                m_TimeForAcceptInvite);
}

void Battlefield::InitStalker(uint32 entry, float x, float y, float z,
                              float o) {
  if (Creature *creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
    ...
    LOG_ERROR("server",
              ...
              "entry %u), zone messeges will be un-available",
              entry);
}
...
void Battlefield::BroadcastPacketToQueue(WorldPacket &data) const {
  for (uint8 team = 0; team < BG_TEAMS_COUNT; ++team)
    for (GuidSet::const_iterator itr = m_PlayersInQueue[team].begin();
         itr != m_PlayersInQueue[team].end(); ++itr)
      if (Player *player = ObjectAccessor::FindPlayer(*itr))
        player->GetSession()->SendPacket(&data);
}
...
void Battlefield::SendWarningToAllInZone(uint32 entry) {
  if (Unit *unit = ObjectAccessor::FindUnit(StalkerGuid))
    if (Creature *stalker = unit->ToCreature())
      sCreatureTextMgr->SendChat(stalker, (uint8)entry, nullptr, CHAT_MSG_ADDON,
                                 LANG_ADDON, TEXT_RANGE_ZONE);
}
...
Group *Battlefield::GetFreeBfRaid(TeamId TeamId) {
  for (GuidSet::const_iterator itr = m_Groups[TeamId].begin();
       itr != m_Groups[TeamId].end(); ++itr)
...
bool Battlefield::AddOrSetPlayerToCorrectBfGroup(Player *player) {
  ...
  if (player->GetGroup() &&
    ...
    LOG_INFO("misc",
             "Battlefield::AddOrSetPlayerToCorrectBfGroup - player is already "
             "in %s group!",
...
BfGraveyard *Battlefield::GetGraveyardById(uint32 id) const {
  if (id < m_GraveyardList.size()) {
    if (m_GraveyardList[id])
      ...
      LOG_ERROR("server", "Battlefield::GetGraveyardById Id:%u not existed",
                id);
  } else
    LOG_ERROR("server", "Battlefield::GetGraveyardById Id:%u cant be found",
              id);
...
void BfGraveyard::Resurrect() {
  ...
  for (GuidSet::const_iterator itr = m_ResurrectQueue.begin();
...
void BfGraveyard::RelocateDeadPlayers() {
  ...
  for (GuidSet::const_iterator itr = m_ResurrectQueue.begin();
    ...
    if (closestGrave)
      player->TeleportTo(player->GetMapId(), closestGrave->x, closestGrave->y,
...
Creature *Battlefield::SpawnCreature(uint32 entry, Position pos,
                                     TeamId teamId) {
  return SpawnCreature(entry, pos.m_positionX, pos.m_positionY, pos.m_positionZ,
                       pos.m_orientation, teamId);
}

Creature *Battlefield::SpawnCreature(uint32 entry, float x, float y, float z,
                                     float o, TeamId teamId) {
  // Get map object
  Map *map = sMapMgr->CreateBaseMap(m_MapId);
  if (!map) {
    LOG_ERROR("server",
              "Battlefield::SpawnCreature: Can't create creature entry: %u map "
              "not found",
              entry);
    return 0;
  }
  ...
  if (!creature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), map,
    ...
    LOG_ERROR("server",
              "Battlefield::SpawnCreature: Can't create creature entry: %u",
              entry);
    delete creature;
    return nullptr;
  }
  ...
  if (!cinfo) {
    LOG_ERROR("sql.sql", "Battlefield::SpawnCreature: entry %u does not exist.",
              entry);
    return nullptr;
  }
...
GameObject *Battlefield::SpawnGameObject(uint32 entry, float x, float y,
                                         float z, float o) {
  // Get map object
  ...
  if (!go->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), entry, map,
                  PHASEMASK_NORMAL, x, y, z, o, G3D::Quat(), 100,
                  GO_STATE_READY)) {
    LOG_ERROR("sql.sql",
              "Battlefield::SpawnGameObject: Gameobject template %u not found "
              "in database! Battlefield not created!",
              entry);
    LOG_ERROR("server",
              "Battlefield::SpawnGameObject: Cannot create gameobject template "
              "%u! Battlefield not created!",
              entry);
    delete go;
    return nullptr;
  }
...
BfCapturePoint::BfCapturePoint(Battlefield *battlefield)
    : m_Bf(battlefield), m_capturePoint(0) {
  m_team = TEAM_NEUTRAL;
  m_value = 0;
  m_minValue = 0.0f;
  m_maxValue = 0.0f;
...
bool BfCapturePoint::SetCapturePointData(GameObject *capturePoint) {
  ...
  LOG_DEBUG("bg.battlefield", "Creating capture point %u",
  ...
  if (goinfo->type != GAMEOBJECT_TYPE_CAPTURE_POINT) {
    LOG_ERROR("server", "OutdoorPvP: GO %u is not capture point!",
              capturePoint->GetEntry());
    return false;
  }
...
bool BfCapturePoint::Update(uint32 diff) {
  ...
  {
    // old phase and current are on the same side, so one team challenges the
    // other
    if (ChallengerId == TEAM_ALLIANCE &&
